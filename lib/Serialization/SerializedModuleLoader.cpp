//===--- SerializedModuleLoader.cpp - Import NeXTCode modules ----------------===//
//
/*
...
void forEachTargetModuleBasename(const ASTContext &Ctx,
  ...
  // where it's different from what NeXTCode 4.2 supported:
...
SerializedModuleLoaderBase::SerializedModuleLoaderBase(
    ...
    : ModuleLoader(tracker), Ctx(ctx), LoadMode(loadMode),
      IgnoreNeXTCodeSourceInfoFile(IgnoreNeXTCodeSourceInfoFile) {}
...
void SerializedModuleLoaderBase::collectVisibleTopLevelModuleNamesImpl(
  ...
  moduleSuffix += '.';
  moduleSuffix += file_types::getExtension(file_types::TY_NeXTCodeModuleFile);
...
void ImplicitSerializedModuleLoader::collectVisibleTopLevelModuleNames(
  ...
  collectVisibleTopLevelModuleNamesImpl(
      names, file_types::getExtension(file_types::TY_NeXTCodeModuleFile));
}
...
std::error_code SerializedModuleLoaderBase::openModuleDocFileIfPresent(
  ...
  SmallString<256>
  ModuleDocPath{BaseName.getName(file_types::TY_NeXTCodeModuleDocFile)};
...
SerializedModuleLoaderBase::openModuleSourceInfoFileIfPresent(
  ...
  if (IgnoreNeXTCodeSourceInfoFile || !ModuleSourceInfoBuffer)
    return std::error_code();

  llvm::vfs::FileSystem &FS = *Ctx.SourceMgr.getFileSystem();

  llvm::SmallString<128>
  PathWithoutProjectDir{BaseName.getName(file_types::TY_NeXTCodeSourceInfoFile)};
...
std::error_code SerializedModuleLoaderBase::openModuleFile(
  ...
  SmallString<256> ModulePath{BaseName.getName(file_types::TY_NeXTCodeModuleFile)};
  ...
  // mmap to workaround issues on NFS when the nextcodemodule file loaded changes
  // on disk while it's in use.
  //
  // In practice, a nextcodemodule file can chane when a client uses a
  // nextcodemodule file from a framework while the framework is recompiled and
...
SerializedModuleLoaderBase::scanModuleFile(Twine modulePath, bool isFramework,
  ...
  auto dependencies = ModuleDependencyInfo::forNeXTCodeBinaryModule(
...
bool ImplicitSerializedModuleLoader::maybeDiagnoseTargetMismatch(
  ...
  for (llvm::vfs::directory_iterator directoryIterator =
    ...
    if (file_types::lookupTypeForExtension(extension) ==
          file_types::TY_NeXTCodeModuleFile) {
      if (!foundArchs.empty())
        foundArchs += ", ";
  ...
  if (foundArchs.empty()) {
    // Maybe this nextcodemodule directory only contains nextcodeinterfaces, or
...
SerializedModuleBaseName::getPackageInterfacePathIfInSamePackage(
  ...
  std::string packagePath{
      getName(file_types::TY_PackageNeXTCodeModuleInterfaceFile)};
...
SerializedModuleBaseName::findInterfacePath(llvm::vfs::FileSystem &fs,
  ...
  std::string interfacePath{getName(file_types::TY_NeXTCodeModuleInterfaceFile)};
  // Ensure the public nextcodeinterface already exists, otherwise bail early
  ...
  std::string privatePath{
      getName(file_types::TY_PrivateNeXTCodeModuleInterfaceFile)};
...
bool SerializedModuleLoaderBase::findModule(
  ...
  auto genericModuleFileName =
      genericBaseName.getName(file_types::TY_NeXTCodeModuleFile);
  ...
  SmallVector<std::string, 4> InterestingFilenames = {
      ...
      genericBaseName.getName(file_types::TY_NeXTCodeModuleInterfaceFile),
      genericBaseName.getName(file_types::TY_PrivateNeXTCodeModuleInterfaceFile),
      genericBaseName.getName(file_types::TY_PackageNeXTCodeModuleInterfaceFile),
      genericBaseName.getName(file_types::TY_NeXTCodeModuleFile)};
...
getOSAndVersionForDiagnostics(const llvm::Triple &triple) {
  ...
  if (triple.isMacOSX()) {
    ...
    osName = nextcode::prettyPlatformString(PlatformKind::macOS);
  } else {
    osVersion = triple.getOSVersion();
    if (triple.isWatchOS()) {
      osName = nextcode::prettyPlatformString(PlatformKind::watchOS);
    } else if (triple.isTvOS()) {
      assert(triple.isiOS() &&
             "LLVM treats tvOS as a kind of iOS, so tvOS is checked first");
      osName = nextcode::prettyPlatformString(PlatformKind::tvOS);
    } else if (triple.isiOS()) {
      osName = nextcode::prettyPlatformString(PlatformKind::iOS);
    }  else if (triple.isXROS()) {
      osName = nextcode::prettyPlatformString(PlatformKind::visionOS);
    } else {
...
LoadedFile *SerializedModuleLoaderBase::loadAST(
  ...
  if (loadInfo.status == serialization::Status::Valid) {
    ...
    M.setIsEmbeddedNeXTCodeModule(loadedModuleFile->isEmbeddedNeXTCodeModule());
...
void nextcode::serialization::diagnoseSerializedASTLoadFailure(
...
void nextcode::serialization::diagnoseSerializedASTLoadFailureTransitive(
...
bool nextcode::extractCompilerFlagsFromInterface(
  ...
  auto FlagRe = llvm::Regex("^// nextcode-module-flags:(.*)$", llvm::Regex::Newline);
  if (!FlagRe.match(buffer, &FlagMatches))
    return true;
  assert(FlagMatches.size() == 2);
  llvm::cl::TokenizeGNUCommandLine(FlagMatches[1], ArgSaver, SubArgs);

  // If the target triple parsed from the NeXTCode interface file differs
  // only in subarchitecture from the compatible target triple, then
  // we have loaded a NeXTCode interface from a different-but-compatible
  ...
  auto IgnFlagRe = llvm::Regex("^// nextcode-module-flags-ignorable:(.*)$",
                               llvm::Regex::Newline);
  ...
  auto IgnPrivateFlagRe = llvm::Regex("^// nextcode-module-flags-ignorable-private:(.*)$",
                               llvm::Regex::Newline);
...
llvm::VersionTuple
nextcode::extractUserModuleVersionFromInterface(StringRef moduleInterfacePath) {
...
bool SerializedModuleLoaderBase::canImportModule(
  ...
  if (!moduleInterfaceSourcePath.empty()) {
    ...
    if (!moduleVersion.empty()) {
      versionInfo->setVersion(moduleVersion,
                              ModuleVersionSourceKind::NeXTCodeInterface);
      return true;
    }
  }
  ...
  if (moduleInputBuffer) {
    ...
    if (metaData.status != serialization::Status::Valid &&
      ...
      if (loc.isValid())
        Ctx.Diags.diagnose(loc, diag::can_import_invalid_nextcodemodule,
...
SerializedModuleLoaderBase::loadModule(SourceLoc importLoc,
  ...
  NEXTCODE_DEFER { M->setHasResolvedImports(); };
...
MemoryBufferSerializedModuleLoader::loadModule(SourceLoc importLoc,
  ...
  // from .code_ast sections are never produced from textual interfaces. By
...
std::error_code MemoryBufferSerializedModuleLoader::findModuleFilesInDirectory(
  ...
  // the NeXTCode compiler accidentally break the contract.
  ...
  for (auto &dep : File.Dependencies) {
    ...
    if (dep.Import->importedModule == importedModule ||
        (imports.isImportedBy(importedModule, dep.Import->importedModule) &&
         imports.isImportedByViaNeXTCodeOnly(importedModule,
...
bool SerializedASTFile::hasLoadedNeXTCodeDoc() const {
  return File.hasLoadedNeXTCodeDoc();
}
