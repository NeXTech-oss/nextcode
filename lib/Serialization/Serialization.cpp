//===--- Serialization.cpp - Read and write NeXTCode modules -----------------===//
//
/*
...
#include "nextcode/ClangImporter/NeXTCodeAbstractBasicWriter.h"
...
using namespace nextcode::serialization;
using namespace llvm::support;
using nextcode::version::Version;
...
static constexpr bool bitOffsetFitsIn32Bits() {
  ...
  // NeXTCode.codemodule is itself 25MB, it seems entirely possible users will
  // exceed this limit.
  using Int32Info = std::numeric_limits<uint32_t>;
  using PtrIntInfo = std::numeric_limits<uintptr_t>;
...
namespace {
  ...
  public:
    ...
    hash_value_type ComputeHash(key_type_ref key) {
      assert(!key.empty());
      return llvm::djbHash(key.str(), NEXTCODEMODULE_HASH_SEED);
    }
  ...
  public:
    ...
    hash_value_type ComputeHash(key_type_ref key) {
      assert(!key.empty());
      return llvm::djbHash(key, NEXTCODEMODULE_HASH_SEED);
    }
  ...
  public:
    ...
    hash_value_type ComputeHash(key_type_ref key) {
      ...
      case DeclBaseName::Kind::Normal:
        assert(!key.empty());
        return llvm::djbHash(key.getIdentifier().str(), NEXTCODEMODULE_HASH_SEED);
      case DeclBaseName::Kind::Subscript:
        return static_cast<uint8_t>(DeclNameKind::Subscript);
      case DeclBaseName::Kind::Constructor:
        return static_cast<uint8_t>(DeclNameKind::Constructor);
...
static uint8_t getRawOpaqueReadOwnership(nextcode::OpaqueReadOwnership ownership) {
  switch (ownership) {
#define CASE(KIND)                                            \
  case nextcode::OpaqueReadOwnership::KIND:                      \
...
static uint8_t getRawReadImplKind(nextcode::ReadImplKind kind) {
  switch (kind) {
#define CASE(KIND)                                     \
  case nextcode::ReadImplKind::KIND:                      \
...
static unsigned getRawWriteImplKind(nextcode::WriteImplKind kind) {
  switch (kind) {
#define CASE(KIND)                                      \
  case nextcode::WriteImplKind::KIND:                      \
...
static unsigned getRawReadWriteImplKind(nextcode::ReadWriteImplKind kind) {
  switch (kind) {
#define CASE(KIND)                                          \
  case nextcode::ReadWriteImplKind::KIND:                      \
...
void Serializer::writeHeader() {
  {
    ...
    versionString << ")/" << version::getNeXTCodeFullVersion();
    ...
    Metadata.emit(ScratchRecord,
                  NEXTCODEMODULE_VERSION_MAJOR, NEXTCODEMODULE_VERSION_MINOR,
    ...
    // Write the producer's NeXTCode revision.
    static const char* forcedDebugRevision =
      ::getenv("NEXTCODE_DEBUG_FORCE_NEXTCODEMODULE_REVISION");
...

    {
      ...
      if (Options.EmbeddedNeXTCodeModule) {
        options_block::IsEmbeddedNeXTCodeModuleLayout IsEmbeddedNeXTCodeModule(Out);
        IsEmbeddedNeXTCodeModule.emit(ScratchRecord);
      }
      ...
      if (Options.SerializeOptionsForDebugging) {
        ...
        for (auto Arg = Opts.begin(), E = Opts.end(); Arg != E; ++Arg) {
          ...
          if (arg.starts_with("-ivfsoverlay")) {
            ...
            // NeXTCode frameworks; but they should never be used to *import* the
...
static uint8_t getRawStableDefaultArgumentKind(nextcode::DefaultArgumentKind kind) {
  switch (kind) {
#define CASE(X) \
  case nextcode::DefaultArgumentKind::X: \
...
static uint8_t 
getRawStableActorIsolationKind(nextcode::ActorIsolation::Kind kind) {
  switch (kind) {
#define CASE(X) \
  case nextcode::ActorIsolation::X: \
...
static serialization::AccessorKind getStableAccessorKind(nextcode::AccessorKind K){
  switch (K) {
#define ACCESSOR(ID) \
  case nextcode::AccessorKind::ID: return serialization::ID;
...
getStableCtorInitializerKind(nextcode::CtorInitializerKind K){
  switch (K) {
#define CASE(NAME) \
  case nextcode::CtorInitializerKind::NAME: return serialization::NAME;
      CASE(Designated)
      CASE(Convenience)
      CASE(Factory)
...
static Identifier getClangTemplateSpecializationXRefDiscriminator(
  ...
  // will let NeXTCode find the correct specialization when this cross
...
/// values, which are guaranteed to be stable.
static uint8_t getRawStableAssociativity(nextcode::Associativity assoc) {
  switch (assoc) {
  case nextcode::Associativity::Left:
    return serialization::Associativity::LeftAssociative;
  case nextcode::Associativity::Right:
    return serialization::Associativity::RightAssociative;
  case nextcode::Associativity::None:
...
getStableStaticSpelling(nextcode::StaticSpellingKind SS) {
  switch (SS) {
  case nextcode::StaticSpellingKind::None:
    return serialization::StaticSpellingKind::None;
  case nextcode::StaticSpellingKind::KeywordStatic:
    return serialization::StaticSpellingKind::KeywordStatic;
  case nextcode::StaticSpellingKind::KeywordClass:
...
static uint8_t getRawStableAccessLevel(nextcode::AccessLevel access) {
  switch (access) {
#define CASE(NAME) \
  case nextcode::AccessLevel::NAME: \
...
getStableSelfAccessKind(nextcode::SelfAccessKind MM) {
  switch (MM) {
  case nextcode::SelfAccessKind::NonMutating:
    return serialization::SelfAccessKind::NonMutating;
  case nextcode::SelfAccessKind::Mutating:
    return serialization::SelfAccessKind::Mutating;
  case nextcode::SelfAccessKind::LegacyConsuming:
    return serialization::SelfAccessKind::LegacyConsuming;
  case nextcode::SelfAccessKind::Consuming:
    return serialization::SelfAccessKind::Consuming;
  case nextcode::SelfAccessKind::Borrowing:
...
static uint8_t getRawStableMacroRole(nextcode::MacroRole context) {
  switch (context) {
#define MACRO_ROLE(Name, Description) \
  case nextcode::MacroRole::Name: \
    return static_cast<uint8_t>(serialization::MacroRole::Name);
#include "nextcode/Basic/MacroRoles.def"
  }
  llvm_unreachable("bad result declaration macro kind");
}

static uint8_t getRawStableMacroIntroducedDeclNameKind(
    nextcode::MacroIntroducedDeclNameKind kind) {
  switch (kind) {
#define CASE(NAME) \
  case nextcode::MacroIntroducedDeclNameKind::NAME: \
...
#define SIMPLE_CASE(TYPENAME, VALUE) \
  case nextcode::TYPENAME::VALUE: return uint8_t(serialization::TYPENAME::VALUE);
...
static uint8_t getRawStableParamDeclSpecifier(nextcode::ParamDecl::Specifier sf) {
  switch (sf) {
  case nextcode::ParamDecl::Specifier::Default:
    return uint8_t(serialization::ParamDeclSpecifier::Default);
  case nextcode::ParamDecl::Specifier::InOut:
    return uint8_t(serialization::ParamDeclSpecifier::InOut);
  case nextcode::ParamDecl::Specifier::Borrowing:
    return uint8_t(serialization::ParamDeclSpecifier::Borrowing);
  case nextcode::ParamDecl::Specifier::Consuming:
    return uint8_t(serialization::ParamDeclSpecifier::Consuming);
  case nextcode::ParamDecl::Specifier::LegacyShared:
    return uint8_t(serialization::ParamDeclSpecifier::LegacyShared);
  case nextcode::ParamDecl::Specifier::LegacyOwned:
    return uint8_t(serialization::ParamDeclSpecifier::LegacyOwned);
  case nextcode::ParamDecl::Specifier::ImplicitlyCopyableConsuming:
    return uint8_t(
...
static uint8_t getRawStableVarDeclIntroducer(nextcode::VarDecl::Introducer intr) {
  switch (intr) {
  case nextcode::VarDecl::Introducer::Let:
    return uint8_t(serialization::VarDeclIntroducer::Let);
  case nextcode::VarDecl::Introducer::Var:
    return uint8_t(serialization::VarDeclIntroducer::Var);
  case nextcode::VarDecl::Introducer::InOut:
    return uint8_t(serialization::VarDeclIntroducer::InOut);
  case nextcode::VarDecl::Introducer::Borrowing:
...
static uint8_t getRawStableAutoDiffDerivativeFunctionKind(
    nextcode::AutoDiffDerivativeFunctionKind kind) {
  switch (kind) {
...
static uint8_t getRawStableDifferentiabilityKind(
    nextcode::DifferentiabilityKind diffKind) {
  switch (diffKind) {
...
static void collectDependenciesFromType(nextcode::SmallSetVector<Type, 4> &seen,
                                        Type ty,
...
static void
collectDependenciesFromRequirement(nextcode::SmallSetVector<Type, 4> &seen,
...
static SmallVector<Type, 4> collectDependenciesFromType(Type ty) {
  nextcode::SmallSetVector<Type, 4> result;
...
#include "nextcode/AST/DeclAttr.def"
    ...
    case DeclAttrKind::NeXTCodeNativeObjCRuntimeBase: {
      auto *theBase = cast<NeXTCodeNativeObjCRuntimeBaseAttr>(DA);
      auto abbrCode
        = S.DeclTypeAbbrCodes[NeXTCodeNativeObjCRuntimeBaseDeclAttrLayout::Code];
      auto nameID = S.addDeclBaseNameRef(theBase->BaseClassName);

      NeXTCodeNativeObjCRuntimeBaseDeclAttrLayout::emitRecord(
          S.Out, S.ScratchRecord, abbrCode,
  ...
  void writeDiscriminatorsIfNeeded(const ValueDecl *value) {
    ...
    bool shouldEmitPrivateDiscriminator =
        access <= nextcode::AccessLevel::FilePrivate &&
    ...
    bool shouldEmitFilenameForPrivate =
        ...
        (access <= nextcode::AccessLevel::FilePrivate ||
         (storage &&
          storage->getFormalAccess() >= nextcode::AccessLevel::Internal &&
...
private:
  ...
  void writeDeserializationSafety(const Decl *decl) {
    ...
    // Everything should be safe in a nextcodeinterface. So, don't emit any safety
    // record when building a nextcodeinterface in release builds. Debug builds
    ...
    LLVM_DEBUG(
      ...
      assert((declIsSafe || !fromModuleInterface) &&
             "All nextcodeinterface decls should be deserialization safe");
    );
  ...
  void writeForeignAsyncConvention(const ForeignAsyncConvention &fac) {
    ...
    unsigned rawErrorParameterIndex =
        nextcode::transform(fac.completionHandlerErrorParamIndex(),
                         [](unsigned index) { return index + 1; })
            .value_or(0);
    unsigned rawErrorFlagParameterIndex =
        nextcode::transform(fac.completionHandlerFlagParamIndex(),
                         [](unsigned index) { return index + 1; })
            .value_or(0);
  ...
  void writeInlinableBodyTextIfNeeded(const AbstractFunctionDecl *AFD) {
    ...
    if (AFD->getResilienceExpansion() != nextcode::ResilienceExpansion::Minimal)
      return;
...
public:
  ...
  void visitExtensionDecl(const ExtensionDecl *extension) {
    ...
    nextcode::SmallSetVector<Type, 4> dependencies;
  ...
  void visitStructDecl(const StructDecl *theStruct) {
    ...
    nextcode::SmallSetVector<Type, 4> dependencyTypes;
  ...
  void visitParamDecl(const ParamDecl *param) {
    ...
    nextcode::DefaultArgumentKind argKind = param->getDefaultArgumentKind();
    if (argKind == nextcode::DefaultArgumentKind::Normal ||
        argKind == nextcode::DefaultArgumentKind::StoredProperty ||
        argKind == nextcode::DefaultArgumentKind::ExpressionMacro) {
      defaultArgumentText =
  ...
  void visitEnumElementDecl(const EnumElementDecl *elem) {
    ...
    // of the ABI. That isn't the case for NeXTCode enums.
...
static uint8_t getRawStableFunctionTypeRepresentation(
                                       nextcode::FunctionType::Representation cc) {
  switch (cc) {
  SIMPLE_CASE(FunctionTypeRepresentation, NeXTCode)
...
static uint8_t getRawStableSILFunctionTypeRepresentation(
                                    nextcode::SILFunctionType::Representation cc) {
  switch (cc) {
...
static uint8_t getRawStableSILCoroutineKind(
                                    nextcode::SILCoroutineKind kind) {
  switch (kind) {
...
/// values, which are guaranteed to be stable.
static uint8_t
getRawStableReferenceOwnership(nextcode::ReferenceOwnership ownership) {
  switch (ownership) {
...
/// Serialization enum values, which are guaranteed to be stable.
static uint8_t getRawStableParameterConvention(nextcode::ParameterConvention pc) {
  switch (pc) {
...
getRawSILParameterInfoOptions(nextcode::SILParameterInfo::Options options) {
...
/// Serialization enum values, which are guaranteed to be stable.
static uint8_t getRawStableResultConvention(nextcode::ResultConvention rc) {
  switch (rc) {
...
getRawSILResultInfoOptions(nextcode::SILResultInfo::Options options) {
...
public:
  ...
  TypeID encodeIsolation(nextcode::FunctionTypeIsolation isolation) {
    switch (isolation.getKind()) {
    case nextcode::FunctionTypeIsolation::Kind::NonIsolated:
      return unsigned(FunctionTypeIsolation::NonIsolated);
    case nextcode::FunctionTypeIsolation::Kind::Parameter:
      return unsigned(FunctionTypeIsolation::Parameter);
    case nextcode::FunctionTypeIsolation::Kind::Erased:
      return unsigned(FunctionTypeIsolation::Erased);
    case nextcode::FunctionTypeIsolation::Kind::GlobalActor:
...
namespace {
class ClangToNeXTCodeBasicWriter :
    public nextcode::DataStreamBasicWriter<ClangToNeXTCodeBasicWriter> {

  Serializer &S;
  SmallVectorImpl<uint64_t> &Record;
  using TypeWriter =
    clang::serialization::AbstractTypeWriter<ClangToNeXTCodeBasicWriter>;
  TypeWriter Types;
...
public:
  ClangToNeXTCodeBasicWriter(Serializer &S, SmallVectorImpl<uint64_t> &record)
    : nextcode::DataStreamBasicWriter<ClangToNeXTCodeBasicWriter>(
  ...
  void writeDeclRef(const clang::Decl *decl) {
    ...
    if (!path) {
      decl->dump(llvm::errs());
      llvm::report_fatal_error("failed to find a stable NeXTCode serialization"
                               " path for the above Clang declaration");
    }

    if (path.isNeXTCodeDecl()) {
      Record.push_back(/*nextcode declaration*/ 1);
      Record.push_back(S.addDeclRef(path.getNeXTCodeDecl()));
      return;
    }
  ...
  void writeAttr(const clang::Attr *attr) {
    auto *nextcodeAttr = dyn_cast_or_null<clang::NeXTCodeAttrAttr>(attr);
    if (!nextcodeAttr) {
      writeUInt32(/*no attribute*/0);
      return;
    }

    writeEnum(nextcodeAttr->getKind() + 1);
    writeIdentifier(nextcodeAttr->getAttrName());
    writeIdentifier(nextcodeAttr->getScopeName());
    writeSourceLocation(nextcodeAttr->getRange().getBegin());
    writeSourceLocation(nextcodeAttr->getRange().getEnd());
    writeSourceLocation(nextcodeAttr->getScopeLoc());
    writeEnum(nextcodeAttr->getParsedKind());
    writeEnum(nextcodeAttr->getSyntax());
    writeUInt64(nextcodeAttr->getAttributeSpellingListIndex());
    writeBool(nextcodeAttr->isRegularKeywordAttribute());
    writeBool(nextcodeAttr->isInherited());
    writeBool(nextcodeAttr->isImplicit());
    writeBool(nextcodeAttr->isPackExpansion());
    writeUInt64(S.addUniquedStringRef(nextcodeAttr->getAttribute()));
  }
};
...
void Serializer::writeASTBlockEntity(const clang::Type *ty) {
  ...
  ClangToNeXTCodeBasicWriter(*this, typeData).writeTypeRef(ty);
...
namespace {
  ...
  public:
    ...
    hash_value_type ComputeHash(key_type_ref key) {
      ...
      return llvm::djbHash(key.getString(scratch), NEXTCODEMODULE_HASH_SEED);
    }
...
static void collectInterestingNestedDeclarations(
  ...
  for (const Decl *member : members) {
    ...
    if (auto nestedType = dyn_cast<TypeDecl>(member)) {
      if (nestedType->getEffectiveAccess() > nextcode::AccessLevel::FilePrivate) {
        if (!nominalParent) {
...
void Serializer::writeToStream(
  ...
  Serializer S{NEXTCODEMODULE_SIGNATURE, DC, options};
...

  {
    ...
    if (!options.DisableCrossModuleIncrementalInfo && DepGraph) {
      fine_grained_dependencies::writeFineGrainedDependencyGraph(
          S.Out, *DepGraph, fine_grained_dependencies::Purpose::ForNeXTCodeModule);
    }
  }
