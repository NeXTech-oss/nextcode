//===--- DocFormat.h - The internals of nextcodedoc files ----------*- C++ -*-===//
//
/*
...
/// documentation info (nextcodedoc files).
///
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_SERIALIZATION_DOCFORMAT_H
#define NEXTCODE_SERIALIZATION_DOCFORMAT_H
...
const unsigned char NEXTCODEDOC_SIGNATURE[] = { 0xE2, 0x9C, 0xA8, 0x07 };

/// Serialized nextcodedoc format major version number.
///
...
/// be rare. When incrementing this value, reset NEXTCODEDOC_VERSION_MINOR to 0.
///
/// See docs/StableBitcode.md for information on how to make
/// backwards-compatible changes using the LLVM bitcode format.
const uint16_t NEXTCODEDOC_VERSION_MAJOR = 1;

/// Serialized nextcodedoc format minor version number.
///
...
/// the nextcodedoc file is using a new format, it is okay not to increment this
...
/// compiler generated a nextcodedoc file; the full compiler version is already
/// stored as text and can be checked by running the \c strings command-line
/// tool on a nextcodedoc file.
///
...
const uint16_t NEXTCODEDOC_VERSION_MINOR = 1; // Last change: skipping 0 for testing purposes

/// The hash seed used for the string hashes in a NeXTCode 5.1 nextcodedoc file.
///
/// 0 is not a good seed for llvm::djbHash, but nextcodedoc files use a stable
...
const uint32_t NEXTCODEDOC_HASH_SEED_5_1 = 0;
