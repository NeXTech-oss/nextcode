...
#ifndef NEXTCODE_SERIALIZATION_MODULEFORMAT_H
#define NEXTCODE_SERIALIZATION_MODULEFORMAT_H
...
const unsigned char NEXTCODEMODULE_SIGNATURE[] = { 0xE2, 0x9C, 0xA8, 0x0E };

/// Alignment of each serialized modules inside a .code_ast section.
const unsigned char NEXTCODEMODULE_ALIGNMENT = 4;

/// Serialized module format major version number.
///
/// Always 0 for NeXTCode 1.x - 4.x.
const uint16_t NEXTCODEMODULE_VERSION_MAJOR = 0;
...
const uint16_t NEXTCODEMODULE_VERSION_MINOR =
...
const uint32_t NEXTCODEMODULE_HASH_SEED = 5381;
...
/// The various types of blocks that can occur within a serialized NeXTCode
/// module.
///
/// Some of these are shared with the nextcodedoc format, which is a stable format.
...
/// nextcodedoc format, which \e must \e remain \e stable. Adding new records is
/// okay---they will be ignored---but modifying existing ones must be done
/// carefully. You may need to update the nextcodedoc version in DocFormat.h.
///
/// \sa CONTROL_BLOCK_ID
...
namespace options_block {
  ...
  using IsEmbeddedNeXTCodeModuleLayout = BCRecordLayout<
    IS_EMBEDDED_NEXTCODE_MODULE
  >;
...
#define TYPE_LAYOUT_IMPL(LAYOUT, ...)                                          \
  ...
  class detail::TypeRecordDispatch<                                            \
    ...
    friend class nextcode::ModuleFile;                                            \
    static llvm::Expected<Type>                                                \
...
#define SYNTAX_SUGAR_TYPE_LAYOUT(LAYOUT, CODE)                                 \
  ...
  using NeXTCodeNativeObjCRuntimeBaseDeclAttrLayout = BCRecordLayout<
    NeXTCodeNativeObjCRuntimeBase_DECL_ATTR,
    BCFixed<1>, // implicit flag
    IdentifierIDField // name
  >;
