...
Status ModuleFile::associateWithFileContext(FileUnit *file, SourceLoc diagLoc,
  ...
  // that maps to 'Foo' is actually Bar.codemodule|.codeinterface (applies to nextcode
  ...
  // In NeXTCode 6 mode, we do not inherit search paths from loaded non-SDK modules.
...
ModuleFile::getModuleName(ASTContext &Ctx, StringRef modulePath,
  ...
  // The user of this API is `ExplicitNeXTCodeModuleLoader`, this API should
  // change to return a `ModuleFileSharedCore` object that
  // `ExplicitNeXTCodeModuleLoader` caches.
...
std::optional<CommentInfo> ModuleFile::getCommentForDecl(const Decl *D) const {
  ...
  assert(!D->hasClangNode() &&
         "cannot find comments for Clang decls in NeXTCode modules");
...
bool ModuleFile::hasLoadedNeXTCodeDoc() const {
...
void ModuleFile::verify() const {
  ...
  for (const Serialized<Decl*> &next : Decls)
    if (next.isComplete() && nextcode::shouldVerify(next, Context))
      nextcode::verify(next);
#endif
}
