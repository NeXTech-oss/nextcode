...
using namespace nextcode::serialization::sil_block;
...
static unsigned
toStableDifferentiabilityKind(nextcode::DifferentiabilityKind kind) {
  switch (kind) {
  case nextcode::DifferentiabilityKind::NonDifferentiable:
    return (unsigned)serialization::DifferentiabilityKind::NonDifferentiable;
  case nextcode::DifferentiabilityKind::Forward:
    return (unsigned)serialization::DifferentiabilityKind::Forward;
  case nextcode::DifferentiabilityKind::Reverse:
    return (unsigned)serialization::DifferentiabilityKind::Reverse;
  case nextcode::DifferentiabilityKind::Normal:
    return (unsigned)serialization::DifferentiabilityKind::Normal;
  case nextcode::DifferentiabilityKind::Linear:
...
void SILSerializer::writeSILFunction(const SILFunction &F, bool DeclOnly) {
  ...
  auto resilience = F.getModule().getNeXTCodeModule()->getResilienceStrategy();
...
void SILSerializer::writeSILInstruction(const SILInstruction &SI) {
  ...
  case SILInstructionKind::BuiltinInst: {
    ...
    SILInstApplyLayout::emitRecord(
        ...
        unsigned(nextcode::ActorIsolation::Unspecified),
        unsigned(nextcode::ActorIsolation::Unspecified), Args);
    break;
  }
  case SILInstructionKind::ApplyInst: {
    ...
    auto callerIsolation = nextcode::ActorIsolation::Unspecified;
    auto calleeIsolation = nextcode::ActorIsolation::Unspecified;
...
void SILSerializer::writeSILVTable(const SILVTable &vt) {
  ...
  if (!ShouldSerializeAll &&
      vt.getClass()->getEffectiveAccess() < nextcode::AccessLevel::Package)
    return;
...
void SILSerializer::writeSILBlock(const SILModule *SILMod) {
  ...
  auto resilience = SILMod->getNeXTCodeModule()->getResilienceStrategy();
  for (const SILFunction &F : *SILMod) {
    auto iter = FuncsToEmit.find(&F);
    if (iter != FuncsToEmit.end()) {
      if (iter->second) {
