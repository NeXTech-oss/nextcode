...
static bool readOptionsBlock(llvm::BitstreamCursor &cursor,
  ...
  while (!cursor.AtEndOfStream()) {
    ...
    case options_block::HAS_HERMETIC_SEAL_AT_LINK:
      ...
      break;
    case options_block::IS_EMBEDDED_NEXTCODE_MODULE:
      extendedInfo.setIsEmbeddedNeXTCodeModule(true);
      break;
  ...
  while (!cursor.AtEndOfStream()) {
    ...
    case control_block::SDK_NAME: {
      ...
      static const char* forceDebugPreSDKRestriction =
        ::getenv("NEXTCODE_DEBUG_FORCE_NEXTCODEMODULE_PER_SDK");
      ...
      // recommended configuration and may lead to unreadable nextcodemodules.
    ...
    case control_block::REVISION: {
      ...
      static const char* ignoreRevision =
        ::getenv("NEXTCODE_IGNORE_NEXTCODEMODULE_REVISION");
      if (ignoreRevision)
        break;
...
bool serialization::isSerializedAST(StringRef data) {
  StringRef signatureStr(reinterpret_cast<const char *>(NEXTCODEMODULE_SIGNATURE),
                         std::size(NEXTCODEMODULE_SIGNATURE));
...
ValidationInfo serialization::validateSerializedAST(
  ...
  if (data.size() % NEXTCODEMODULE_ALIGNMENT != 0 ||
      reinterpret_cast<uintptr_t>(data.data()) % NEXTCODEMODULE_ALIGNMENT != 0)
    return result;

  llvm::BitstreamCursor cursor(data);
  SmallVector<uint64_t, 32> scratch;

  if (!checkModuleSignature(cursor, NEXTCODEMODULE_SIGNATURE) ||
...
void ModuleFileSharedCore::fatal(llvm::Error error) const {
  ...
  out << "*** new nextcodemodule files from the SDK and keep only nextcodeinterfaces.   ***\n";
...
void ModuleFileSharedCore::outputDiagnosticInfo(llvm::raw_ostream &os) const {
  ...
  os << "module '" << Name
     ...
     << "', built from "
     << (Bits.IsBuiltFromInterface? "nextcodeinterface": "source")
...
static std::optional<nextcode::LibraryKind>
getActualLibraryKind(unsigned rawKind) {
  ...
  case serialization::LibraryKind::Library:
    return nextcode::LibraryKind::Library;
  case serialization::LibraryKind::Framework:
    return nextcode::LibraryKind::Framework;
  }
...
bool ModuleFileSharedCore::readModuleDocIfPresent(PathObfuscator &pathRecoverer) {
  ...
  if (!checkModuleSignature(docCursor, NEXTCODEDOC_SIGNATURE) ||
  ...
  while (!docCursor.AtEndOfStream()) {
    ...
    case CONTROL_BLOCK_ID: {
      ...
      info = validateControlBlock(
          docCursor, scratch, {NEXTCODEDOC_VERSION_MAJOR, NEXTCODEDOC_VERSION_MINOR},
          RequiresOSSAModules,
          /*requiresRevisionMatch*/false,
          /*requiredSDK*/StringRef(), /*extendedInfo*/nullptr, pathRecoverer);
      if (info.status != Status::Valid)
        return false;
      // Check that the nextcodedoc is actually for this module.
      if (info.name != Name)
        return false;
...
bool ModuleFileSharedCore::readModuleSourceInfoIfPresent(PathObfuscator &pathRecoverer) {
  ...
  if (!checkModuleSignature(infoCursor, NEXTCODESOURCEINFO_SIGNATURE) ||
  ...
  while (!infoCursor.AtEndOfStream()) {
    ...
    case CONTROL_BLOCK_ID: {
      ...
      info = validateControlBlock(
          infoCursor, scratch,
          {NEXTCODESOURCEINFO_VERSION_MAJOR, NEXTCODESOURCEINFO_VERSION_MINOR},
          RequiresOSSAModules,
          /*requiresRevisionMatch*/false,
          /*requiredSDK*/StringRef(), /*extendedInfo*/nullptr, pathRecoverer);
      if (info.status != Status::Valid)
        return false;
      // Check that the nextcodesourceinfo is actually for this module.
      if (info.name != Name)
        return false;
      hasValidControlBlock = true;
      break;
...
ModuleFileSharedCore::ModuleFileSharedCore(
  ...
  while (!cursor.AtEndOfStream()) {
    ...
    case CONTROL_BLOCK_ID: {
      ...
      Bits.IsEmbeddedNeXTCodeModule = extInfo.isEmbeddedNeXTCodeModule();
