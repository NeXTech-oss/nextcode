//===--- SerializeDoc.cpp - Read and write nextcodedoc files -----------------===//
//
/*
...
public:
  ...
  hash_value_type ComputeHash(key_type_ref key) {
    assert(!key.empty());
    return llvm::djbHash(key, NEXTCODEDOC_HASH_SEED_5_1);
  }
...
#undef BLOCK_RECORD
  ...
  /// Writes the NeXTCode doc module file header and name.
  void writeDocHeader();
};
...
static bool shouldIncludeDecl(Decl *D, bool ForSourceInfo) {
  ...
  case DocCommentSerializationTarget::NeXTCodeDocAndSourceInfo:
    return true;
  }
}
...
void DocSerializer::writeDocHeader() {
  {
    ...
    auto verText = version::getNeXTCodeFullVersion(LangOpts.EffectiveLanguageVersion);
    Metadata.emit(ScratchRecord, NEXTCODEDOC_VERSION_MAJOR, NEXTCODEDOC_VERSION_MINOR,
...
void serialization::writeDocToStream(raw_ostream &os, ModuleOrSourceFile DC,
  ...
  DocSerializer S{NEXTCODEDOC_SIGNATURE, DC};
...
public:
  ...
  hash_value_type ComputeHash(key_type_ref key) {
    assert(!key.empty());
    return llvm::djbHash(key, NEXTCODESOURCEINFO_HASH_SEED);
  }
...
#undef BLOCK_RECORD
  ...
  /// Writes the NeXTCode sourceinfo file header and name.
  void writeSourceInfoHeader() {
    {
      ...
      auto& LangOpts = M->getASTContext().LangOpts;
      auto verText = version::getNeXTCodeFullVersion(LangOpts.EffectiveLanguageVersion);
      Metadata.emit(ScratchRecord, NEXTCODESOURCEINFO_VERSION_MAJOR,
                    NEXTCODESOURCEINFO_VERSION_MINOR,
                    /*short version string length*/0, /*compatibility length*/0,
                    /*user module version major*/0,
                    /*user module version minor*/0,
...
void serialization::writeSourceInfoToStream(raw_ostream &os,
  ...
  assert(DC);
  SourceInfoSerializer S{NEXTCODESOURCEINFO_SIGNATURE, DC};
  // FIXME: This is only really needed for debugging. We don't actually use it.
