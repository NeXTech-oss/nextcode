...
#include "nextcode/ClangImporter/NeXTCodeAbstractBasicReader.h"
...
ModularizationError::diagnose(const ModuleFile *MF,
  ...
  if (!expectedModule->isNonNeXTCodeModule() &&
      expectedUnderlying) {
  ...
  // A NeXTCode language version mismatch could lead to a different set of rules
  ...
  if (clientLangVersion != moduleLangVersion) {
    ctx.Diags.diagnose(loc,
                       diag::modularization_issue_nextcode_version,
                       referenceModuleDecl, moduleLangVersion,
                       clientLangVersion);
  }

  // If the error is in a resilient nextcodemodule adjacent to a nextcodeinterface,
  // deleting the module to rebuild from the nextcodeinterface may fix the issue.
  // Limit this suggestion to distributed NeXTCode modules to not hint at
  ...
  if (expectedUnderlying) {
    ctx.Diags.diagnose(loc,
                       ...
                       expectedModule->isNonNeXTCodeModule(), expectedModule);
  }
...
static std::optional<nextcode::AccessorKind> getActualAccessorKind(uint8_t raw) {
  switch (serialization::AccessorKind(raw)) {
#define ACCESSOR(ID) \
  case serialization::AccessorKind::ID: return nextcode::AccessorKind::ID;
...
static std::optional<nextcode::DefaultArgumentKind>
...
#define CASE(X) \
  case serialization::DefaultArgumentKind::X: \
    return nextcode::DefaultArgumentKind::X;
  CASE(None)
  CASE(Normal)
  CASE(Inherited)
  CASE(Column)
...
static std::optional<nextcode::ActorIsolation::Kind>
...
#define CASE(X) \
  case serialization::ActorIsolation::X: \
    return nextcode::ActorIsolation::Kind::X;
  CASE(Unspecified)
  CASE(ActorInstance)
  CASE(Nonisolated)
...
#undef CASE
  case serialization::ActorIsolation::GlobalActorUnsafe:
    return nextcode::ActorIsolation::GlobalActor;
  }
  return std::nullopt;
}
...
static std::optional<nextcode::CtorInitializerKind>
...
#define CASE(NAME) \
  case serialization::CtorInitializerKind::NAME: \
    return nextcode::CtorInitializerKind::NAME;
  CASE(Designated)
  CASE(Convenience)
  CASE(Factory)
...
static void filterValues(Type expectedTy, ModuleDecl *expectedModule,
                         ...
                         bool isStatic,
                         std::optional<nextcode::CtorInitializerKind> ctorInit,
    ...
    if (recordID == XREF_TYPE_PATH_PIECE ||
      ...
      for (auto nameAndModule : ctx.getLoadedModules()) {
        ...
        if (!values.empty()) {
          ...
          // by the use of different NeXTCode language versions between a library
...
/// the AST.
static std::optional<nextcode::Associativity>
getActualAssociativity(uint8_t assoc) {
  switch (assoc) {
  case serialization::Associativity::LeftAssociative:
    return nextcode::Associativity::Left;
  case serialization::Associativity::RightAssociative:
    return nextcode::Associativity::Right;
  case serialization::Associativity::NonAssociative:
    return nextcode::Associativity::None;
  default:
    return std::nullopt;
  }
}

static std::optional<nextcode::StaticSpellingKind>
getActualStaticSpellingKind(uint8_t raw) {
  ...
  case serialization::StaticSpellingKind::None:
    return nextcode::StaticSpellingKind::None;
  case serialization::StaticSpellingKind::KeywordStatic:
    return nextcode::StaticSpellingKind::KeywordStatic;
  case serialization::StaticSpellingKind::KeywordClass:
    return nextcode::StaticSpellingKind::KeywordClass;
  }
  return std::nullopt;
}
...
static std::optional<nextcode::AccessLevel> getActualAccessLevel(uint8_t raw) {
  switch (serialization::AccessLevel(raw)) {
#define CASE(NAME) \
  case serialization::AccessLevel::NAME: \
    return nextcode::AccessLevel::NAME;
  CASE(Private)
  CASE(FilePrivate)
  CASE(Internal)
  CASE(Package)
...
static std::optional<nextcode::OpaqueReadOwnership>
...
#define CASE(KIND)                               \
  case serialization::OpaqueReadOwnership::KIND: \
    return nextcode::OpaqueReadOwnership::KIND;
  CASE(Owned)
  CASE(Borrowed)
...
static std::optional<nextcode::ReadWriteImplKind>
...
#define CASE(KIND)                             \
  case serialization::ReadWriteImplKind::KIND: \
    return nextcode::ReadWriteImplKind::KIND;
  CASE(Immutable)
  CASE(Stored)
  CASE(MutableAddress)
...
static std::optional<nextcode::AutoDiffDerivativeFunctionKind>
...
#define CASE(ID)                                                               \
  case serialization::AutoDiffDerivativeFunctionKind::ID:                      \
    return {nextcode::AutoDiffDerivativeFunctionKind::ID};
  CASE(JVP)
  CASE(VJP)
#undef CASE
  }
...
static std::optional<nextcode::MacroRole> getActualMacroRole(uint8_t context) {
  switch (context) {
#define MACRO_ROLE(Name, Description)           \
  case (uint8_t)serialization::MacroRole::Name: \
    return nextcode::MacroRole::Name;
#include "nextcode/Basic/MacroRoles.def"
  }
  return std::nullopt;
}

static std::optional<nextcode::MacroIntroducedDeclNameKind>
getActualMacroIntroducedDeclNameKind(uint8_t context) {
  switch (context) {
#define CASE(THE_DK) \
  case (uint8_t)serialization::MacroIntroducedDeclNameKind::THE_DK: \
    return nextcode::MacroIntroducedDeclNameKind::THE_DK;
  CASE(Named)
  CASE(Overloaded)
  CASE(Prefixed)
  CASE(Suffixed)
...
public:
  ...
  Expected<Decl *> deserializeConstructor(ArrayRef<uint64_t> scratch,
    ...
    std::optional<nextcode::CtorInitializerKind> initKind =
  ...
  Expected<Decl *> deserializeOpaqueType(ArrayRef<uint64_t> scratch,
    ...
    if (!MF.FileContext->getParentModule()->isMainModule() &&
      ...
      // is not inlinable in clients. This reflects the nextcodeinterface behavior
...
llvm::Error DeclDeserializer::deserializeDeclCommon() {
  ...
  while (true) {
    ...
    if (recordID == ERROR_FLAG) {
      ...
      case decls_block::NeXTCodeNativeObjCRuntimeBase_DECL_ATTR: {
        bool isImplicit;
        IdentifierID nameID;
        serialization::decls_block::NeXTCodeNativeObjCRuntimeBaseDeclAttrLayout
          ::readRecord(scratch, isImplicit, nameID);

        auto name = MF.getIdentifier(nameID);
        Attr = new (ctx) NeXTCodeNativeObjCRuntimeBaseAttr(name, SourceLoc(),
                                                        SourceRange(),
                                                        isImplicit);
        break;
      }
      ...
      case decls_block::Documentation_DECL_ATTR: {
        ...
        std::optional<nextcode::AccessLevel> realVisibility = std::nullopt;
        if (hasVisibility)
...
static std::optional<nextcode::ParameterConvention>
...
#define CASE(ID) \
  case serialization::ParameterConvention::ID: \
    return nextcode::ParameterConvention::ID;
  CASE(Indirect_In)
  CASE(Indirect_Inout)
  ...
  case serialization::ParameterConvention::Indirect_In_Constant: \
    return nextcode::ParameterConvention::Indirect_In;
  CASE(Direct_Owned)
  CASE(Direct_Unowned)
...
/// which are guaranteed to be stable, to the AST ones.
static std::optional<nextcode::ResultConvention>
getActualResultConvention(uint8_t raw) {
  switch (serialization::ResultConvention(raw)) {
#define CASE(ID) \
  case serialization::ResultConvention::ID: return nextcode::ResultConvention::ID;
  CASE(Indirect)
  CASE(Owned)
  CASE(Unowned)
...
detail::function_deserializer::deserialize(ModuleFile &MF,
  ...
  if (rawIsolation == unsigned(FunctionTypeIsolation::NonIsolated)) {
    ...
    isolation = nextcode::FunctionTypeIsolation::forGlobalActor(globalActorTy.get());
  }
...
Expected<Type> deserializeAnyMetatypeType(ModuleFile &MF,
  ...
  case serialization::MetatypeRepresentation::MR_Thin:
    if (!CanBeThin)
      return MF.diagnoseFatal();
    return ASTType::get(instanceType.get(),
                        nextcode::MetatypeRepresentation::Thin);

  case serialization::MetatypeRepresentation::MR_Thick:
    return ASTType::get(instanceType.get(),
                        nextcode::MetatypeRepresentation::Thick);

  case serialization::MetatypeRepresentation::MR_ObjC:
    return ASTType::get(instanceType.get(),
                        nextcode::MetatypeRepresentation::ObjC);
...
Expected<Type> DESERIALIZE_TYPE(SIL_FUNCTION_TYPE)(
  ...
  unsigned entriesPerParam =
      diffKind != nextcode::DifferentiabilityKind::NonDifferentiable ? 3 : 2;
  ...
  if (*representation == nextcode::SILFunctionTypeRepresentation::WitnessMethod) {
...
class NeXTCodeToClangBasicReader :
    public nextcode::DataStreamBasicReader<NeXTCodeToClangBasicReader> {
...
public:
  NeXTCodeToClangBasicReader(ModuleFile &MF, ClangModuleLoader &clangLoader,
                          ArrayRef<uint64_t> record)
  ...
  Identifier readNeXTCodeIdentifier() {
    return MF.getIdentifier(IdentifierID(readUInt64()));
  }

  clang::IdentifierInfo *readIdentifier() {
    Identifier nextcodeIdent = readNeXTCodeIdentifier();
    return &getASTContext().Idents.get(nextcodeIdent.str());
  }
  ...
  clang::Decl *readDeclRef() {
    ...
    if (refKind == 1) {
      nextcode::Decl *nextcodeDecl = MF.getDecl(DeclID(readUInt64()));
      return const_cast<clang::Decl*>(
        ClangLoader.resolveStableSerializationPath(nextcodeDecl));
    }
  ...
  const clang::Attr *readAttr() {
    ...
    auto *attr =
        clang::NeXTCodeAttrAttr::Create(getASTContext(), attribute.str(), info);
...
ModuleFile::getClangType(ClangTypeID TID) {
  ...
  auto clangType =
    NeXTCodeToClangBasicReader(*this, clangLoader, scratch).readTypeRef()
      .getTypePtr();
...
Type
ModuleFile::loadAssociatedTypeDefault(const nextcode::AssociatedTypeDecl *ATD,
...
void ModuleFile::finishNormalConformance(NormalProtocolConformance *conformance,
  ...
  NEXTCODE_DEFER { conformance->setState(ProtocolConformanceState::Complete); };
  ...
  if (proto->isObjC() && getContext().LangOpts.EnableDeserializationRecovery) {
    ...
    for (const auto &req : proto->getRequirementSignature().getRequirements()) {
      ...
      if (iter != conformancesForProtocols.end()) {
        ...
        // There are no associated types and we don't emit a NeXTCode conformance
        // record.
  ...
  // An imported requirement may have changed type between NeXTCode versions.
