...
#ifndef NEXTCODE_SERIALIZATION_MODULEFILECORETABLEINFO_H
#define NEXTCODE_SERIALIZATION_MODULEFILECORETABLEINFO_H
...
public:
  ...
  hash_value_type ComputeHash(internal_key_type key) {
    if (key.first == DeclBaseName::Kind::Normal) {
      return llvm::djbHash(key.second, serialization::NEXTCODEMODULE_HASH_SEED);
    } else {
...
public:
  ...
  hash_value_type ComputeHash(internal_key_type key) {
    return llvm::djbHash(key, serialization::NEXTCODEMODULE_HASH_SEED);
  }
...
public:
  ...
  hash_value_type ComputeHash(internal_key_type key) {
    assert(!key.empty());
    return llvm::djbHash(key, serialization::NEXTCODEDOC_HASH_SEED_5_1);
  }
...
public:
  ...
  hash_value_type ComputeHash(internal_key_type key) {
    assert(!key.empty());
    return llvm::djbHash(key, serialization::NEXTCODESOURCEINFO_HASH_SEED);
  }
...
public:
  using internal_key_type = uint32_t;
  using external_key_type = DeclID;
  using data_type = nextcode::Fingerprint;
  using hash_value_type = uint32_t;
  using offset_type = unsigned;
