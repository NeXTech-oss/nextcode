...
class PartialApplySimplificationPass : public SILModuleTransform {
  ...
  void run() override {
    ...
    llvm::SetVector<nextcode::PartialApplyInst *> dynamicCallees;
...
///   - the argument is either a single NeXTCode-refcounted word, or trivial and
...
static bool isSimplePartialApply(SILModule &M,
  ...
  if (isOnStack) {
    ...
    // The context type must consist of only a nextcode-refcounted object
    // reference.
    return SILType::getPrimitiveObjectType(argTy)
      .isSingleNeXTCodeRefcounted(M, context.getResilienceExpansion());
  }
...
SILTransform *nextcode::createPartialApplySimplification() {
