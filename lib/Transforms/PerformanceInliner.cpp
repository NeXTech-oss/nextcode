...
#include "nextcode/SILOptimizer/Utils/PerformanceInlinerUtils.h"
...
class SILPerformanceInliner {
  ...
  bool profileBasedDecision(
      ...
      const llvm::DenseMapIterator<
          nextcode::SILBasicBlock *, uint64_t,
          llvm::DenseMapInfo<nextcode::SILBasicBlock *>,
          llvm::detail::DenseMapPair<nextcode::SILBasicBlock *, uint64_t>, true>
          &bbIt);
...
// containing function is a linear map in NeXTCode Autodiff.
...
bool isFunctionAutodiffVJP(SILFunction *callee) {
  nextcode::Demangle::Context Ctx;
  if (auto *Root = Ctx.demangleSymbolAsNode(callee->getName())) {
    if (auto *node = Root->findByKind(
            nextcode::Demangle::Node::Kind::AutoDiffFunctionKind, 3)) {
      if (node->hasIndex()) {
        auto index = (char)node->getIndex();
        auto ADFunctionKind = nextcode::Demangle::AutoDiffFunctionKind(index);
        if (ADFunctionKind == nextcode::Demangle::AutoDiffFunctionKind::VJP) {
          return true;
        }
      }
    }
    ...
    for (SILInstruction &I : *block) {
      ...
      if (FullApplySite FAI = FullApplySite::isa(&I)) {
        ...
        if (def && (isa<FunctionRefInst>(def) || isa<PartialApplyInst>(def)))
          ...
          // For linear maps in NeXTCode Autodiff, callees may be passed as an
          // argument, however, they may be hidden behind a branch-tracing
          // enum (tracing execution flow of the original function).
          //
          // If we can establish that we are inside of a NeXTCode Autodiff linear
...
static std::optional<bool> shouldInlineGeneric(FullApplySite AI,
  ...
  ModuleDecl *NeXTCodeModule = Callee->getModule().getNeXTCodeModule();
  if (Callee->hasSemanticsAttrThatStartsWith("array.") &&
      (NeXTCodeModule->isStdlibModule() || NeXTCodeModule->isOnoneSupportModule()))
    return false;
...
addToBBCounts(llvm::DenseMap<SILBasicBlock *, uint64_t> &BBToWeightMap,
              uint64_t numToAdd, nextcode::TermInst *termInst) {
...
bool SILPerformanceInliner::inlineCallsIntoFunction(SILFunction *Caller) {
  ...
  for (auto AI : llvm::reverse(AppliesToInline)) {
    ...
    if (EnableVerifyAfterEachInlining) {
      ...
      Caller->verify();
      pm->runNeXTCodeFunctionVerification(Caller);
    }
  }
  deleter.cleanupDeadInstructions();
...
SILTransform *nextcode::createAlwaysInlineInliner() {
...
SILTransform *nextcode::createEarlyPerfInliner() {
...
SILTransform *nextcode::createPerfInliner() {
  return new SILPerformanceInlinerPass(
