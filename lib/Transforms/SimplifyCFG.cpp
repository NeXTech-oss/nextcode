...
#include "nextcode/SILOptimizer/Transforms/SimplifyCFG.h"
...
bool SimplifyCFG::simplifyCondBrBlock(CondBranchInst *BI) {
  ...
  if (auto *SEI = dyn_cast<SelectEnumInst>(BI->getCondition())) {
    ...
    bool IsExhaustive =
        E->isEffectivelyExhaustive(Fn.getModule().getNeXTCodeModule(),
...
/// path. This is a common pattern in nextcode code. As an example consider a
...
SILTransform *nextcode::createSimplifyCFG() {
...
SILTransform *nextcode::createJumpThreadSimplifyCFG() {
...
SILTransform *nextcode::createSplitAllCriticalEdges() {
  return new SplitCriticalEdges(false);
}

/// Splits all critical edges from non cond_br terminators in a function.
SILTransform *nextcode::createSplitNonCondBrCriticalEdges() {
  return new SplitCriticalEdges(true);
}

// Simplifies basic block arguments.
SILTransform *nextcode::createSROABBArgs() { return new SROABBArgs(); }

// Simplifies basic block arguments.
SILTransform *nextcode::createSimplifyBBArgs() {
  return new SimplifyBBArgs();
}

// Moves cond_fail instructions to predecessors.
SILTransform *nextcode::createMoveCondFailToPreds() {
