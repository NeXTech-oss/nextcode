...
static SILType getThickMetatypeType(CanType Ty) {
  auto NeXTCodeTy = CanMetatypeType::get(Ty, MetatypeRepresentation::Thick);
  return SILType::getPrimitiveObjectType(NeXTCodeTy);
}
...
public:
  ...
  EagerDispatch(SILFunction *GenericFunc,
    ...
    IsClassF = Builder.getModule().loadFunction(
      "_nextcode_isClassOrObjCExistentialType", SILModule::LinkingMode::LinkAll);
    assert(IsClassF);
  }
...
void EagerDispatch::emitRefCountedObjectCheck(SILBasicBlock *FailedTypeCheckBB,
  ...
  auto *NeXTCodeClassBB = createSplitBranchTarget(SuccessBB, Builder, Loc);
  Builder.createCondBranch(Loc, Cmp1, NeXTCodeClassBB, MayBeClassCheckBB);
...
void EagerSpecializerTransform::run() {
  ...
  for (auto *SA : F.getSpecializeAttrs()) {
    ...
    ReInfoVec.emplace_back(FuncBuilder.getModule().getNeXTCodeModule(),
...
SILTransform *nextcode::createEagerSpecializer() {
  return new EagerSpecializerTransform(false/*onlyCreatePrespecializations*/);
}

SILTransform *nextcode::createOnonePrespecializations() {
