...
propagateCopy(CopyAddrInst *CopyInst) {
  ...
  if (forwardPropagateCopy()) {
    ...
    nextcode::salvageStoreDebugInfo(CurrentCopy, CurrentCopy->getSrc(),
                                 CurrentCopy->getDest());
  ...
  if (CurrentCopy->isInitializationOfDest() && backwardPropagateCopy()) {
    ...
    nextcode::salvageStoreDebugInfo(CurrentCopy, CurrentCopy->getDest(),
                                 CurrentCopy->getSrc());
    CurrentCopy->eraseFromParent();
    HasChanged = true;
    ++NumCopyBackward;
...
forwardDeadTempCopy(CopyAddrInst *destCopy) {
  ...
  nextcode::salvageStoreDebugInfo(srcCopy, srcCopy->getSrc(), srcCopy->getDest());
...
bool CopyForwarding::backwardPropagateCopy() {
  ...
  for (auto *Oper : ValueUses) {
    if (auto *SI = dyn_cast<CopyAddrInst>(Oper->getUser())) {
      ...
      // debug info.
      nextcode::salvageStoreDebugInfo(SI, SI->getSrc(), SI->getDest());
    }
...
SILTransform *nextcode::createCopyForwarding() {
