...
#include "nextcode/SILOptimizer/Differentiation/PullbackCloner.h"
...
#include "nextcode/SILOptimizer/Differentiation/Thunk.h"
#include "nextcode/SILOptimizer/Differentiation/VJPCloner.h"
...
private:
  ...
  std::optional<TangentSpace> getTangentSpace(CanType type) {
    ...
    return type->getAutoDiffTangentSpace(
        LookUpConformanceInModule(getModule().getNeXTCodeModule()));
  }
  ...
  SILValueCategory getTangentValueCategory(SILValue v) {
    ...
    // TODO(https://github.com/apple/nextcode/issues/55523): Make "tangent value category" depend solely on whether the tangent type is loadable or address-only.
    //
  ...
  SILValue materializeAdjointDirect(AdjointValue val, SILLocation loc) {
    ...
    case AdjointValueKind::Zero:
      result = recordTemporary(builder.emitZero(loc, val.getNeXTCodeType()));
      break;
  ...
  void materializeAdjointIndirect(AdjointValue val, SILValue destAddress,
    ...
    /// buffer.
    case AdjointValueKind::Aggregate: {
      if (auto *tupTy = val.getNeXTCodeType()->getAs<TupleType>()) {
        for (auto idx : range(val.getNumAggregateElements())) {
          ...
          materializeAdjointIndirect(val.getAggregateElement(idx), eltBuf, loc);
                     ...
                     val.getNeXTCodeType()->getStructOrBoundGenericStruct()) {
...
AllocStackInst *PullbackCloner::Implementation::createOptionalAdjoint(
  ...
  auto *nextcodeModule = getModule().getNeXTCodeModule();
  auto diffConf =
      nextcodeModule->lookupConformance(wrappedType.getASTType(), diffProto);
...
bool PullbackCloner::Implementation::runForSemanticMemberAccessor() {
  ...
  // TODO(https://github.com/apple/nextcode/issues/55084): Support `modify` accessors.
  default:
...
AllocStackInst *PullbackCloner::Implementation::getArrayAdjointElementBuffer(
  ...
  auto diffConf = nextcodeModule->lookupConformance(eltTanType, diffProto);
  assert(!diffConf.isInvalid() && "Missing conformance to `Differentiable`");
  auto *addArithProto = ctx.getProtocol(KnownProtocolKind::AdditiveArithmetic);
  auto addArithConf = nextcodeModule->lookupConformance(eltTanType, addArithProto);
