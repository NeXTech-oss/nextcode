...
#include "nextcode/SILOptimizer/Differentiation/Common.h"
...
getOrCreateSubsetParametersThunkForLinearMap(
  ...
  auto buildZeroArgument = [&](SILParameterInfo zeroSILParameter) {
    ...
    auto *nextcodeMod = parentThunk->getModule().getNeXTCodeModule();
    auto tangentSpace =
        zeroType->getAutoDiffTangentSpace(LookUpConformanceInModule(nextcodeMod));
