...
bool FunctionAccessStorage::summarizeCall(FullApplySite fullApply) {
  ...
  if (SILFunction *callee = fullApply.getReferencedFunctionOrNull()) {
    if (callee->getName() == "_nextcode_stdlib_malloc_size" ||
        callee->getName() == "_nextcode_stdlib_has_malloc_size") {
      return true;
    }
  }
...
SILAnalysis *nextcode::createAccessStorageAnalysis(SILModule *) {
