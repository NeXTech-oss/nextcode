...
#include "nextcode/SILOptimizer/Analysis/RegionAnalysis.h"
...
#include "nextcode/SIL/OperandDatastructures.h"
...
#include "nextcode/SILOptimizer/Utils/PartitionUtils.h"
#include "nextcode/SILOptimizer/Utils/VariableNameUtils.h"
#include "llvm/ADT/DenseMap.h"
#include "llvm/Support/Debug.h"

using namespace nextcode;
using namespace nextcode::PartitionPrimitives;
using namespace nextcode::PatternMatch;
using namespace nextcode::regionanalysisimpl;
...
static bool isTransferrableFunctionArgument(SILFunctionArgument *arg) {
  ...
  // otherwise. This only occurs when we are not in nextcode-6 mode since in
  // nextcode-6 mode we will error on the earlier error... but in the case of
  // us not being in nextcode 6 mode lets not emit extra errors.
  //
...
public:
  ...
  void translateSILApply(SILInstruction *inst) {
    ...
    if (auto *f = fas.getCalleeFunction()) {
      if (f->getName() == "nextcode_asyncLet_get") {
...
#include "nextcode/SIL/SILNodes.def"
  ...
  void translateSILInstruction(SILInstruction *inst) {
    builder.reset(inst);
    NEXTCODE_DEFER { LLVM_DEBUG(builder.print(llvm::dbgs())); };
...
void PartitionOpBuilder::print(llvm::raw_ostream &os) const {
  ...
  NEXTCODE_DEFER { opsToPrint.clear(); };
...
SILAnalysis *nextcode::createRegionAnalysis(SILModule *) {
