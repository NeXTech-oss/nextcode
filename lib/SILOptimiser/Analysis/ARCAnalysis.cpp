...
bool nextcode::isRetainInstruction(SILInstruction *I) {
  switch (I->getKind()) {
...
bool nextcode::isReleaseInstruction(SILInstruction *I) {
  switch (I->getKind()) {
#define ALWAYS_OR_SOMETIMES_LOADABLE_CHECKED_REF_STORAGE(Name, ...) \
...
bool nextcode::mayDecrementRefCount(SILInstruction *User,
...
bool nextcode::canUseObject(SILInstruction *Inst) {
  switch (Inst->getKind()) {
...
bool nextcode::mayHaveSymmetricInterference(SILInstruction *User, SILValue Ptr, AliasAnalysis *AA) {
...
bool nextcode::mustUseValue(SILInstruction *User, SILValue Ptr,
                         AliasAnalysis *AA) {
...
bool nextcode::mustGuaranteedUseValue(SILInstruction *User, SILValue Ptr,
                                   AliasAnalysis *AA) {
  // Right now just pattern match applies.
  auto *AI = dyn_cast<ApplyInst>(User);
  if (!AI)
...
std::optional<SILBasicBlock::iterator> nextcode::valueHasARCUsesInInstructionRange(
...
nextcode::valueHasARCUsesInReverseInstructionRange(SILValue Op,
...
/// same basic block.
std::optional<SILBasicBlock::iterator>
nextcode::valueHasARCDecrementOrCheckInInstructionRange(
    SILValue Op, SILBasicBlock::iterator Start, SILBasicBlock::iterator End,
    AliasAnalysis *AA) {
  assert(Start->getParent() == End->getParent() &&
         "Start and End should be in the same basic block");
...
bool nextcode::isARCInertTrapBB(const SILBasicBlock *BB) {
