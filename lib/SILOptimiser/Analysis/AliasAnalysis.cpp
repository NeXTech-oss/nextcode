...
llvm::raw_ostream &nextcode::operator<<(llvm::raw_ostream &OS, AliasResult R) {
  switch (R) {
...
SILType nextcode::computeTBAAType(SILValue V) {
...
SILAnalysis *nextcode::createAliasAnalysis(SILModule *M) {
...
MemoryBehavior AliasAnalysis::getMemoryEffectOnEscapedAddress(
  ...
  if (getMemEffectsFunction) {
    return (MemoryBehavior)getMemEffectsFunction({PM->getNeXTCodePassInvocation()}, {addr},
                                                 {toInst->asSILNode()},
...
bool AliasAnalysis::isObjectReleasedByInst(SILValue obj, SILInstruction *inst) {
  if (isObjReleasedFunction) {
    return isObjReleasedFunction({PM->getNeXTCodePassInvocation()}, {obj}, {inst->asSILNode()},
...
bool AliasAnalysis::isAddrVisibleFromObject(SILValue addr, SILValue obj) {
  if (isAddrVisibleFromObjFunction) {
    return isAddrVisibleFromObjFunction({PM->getNeXTCodePassInvocation()}, {addr}, {obj},
...
bool AliasAnalysis::canReferenceSameField(SILValue lhs, SILValue rhs) {
  if (canReferenceSameFieldFunction) {
    return canReferenceSameFieldFunction({PM->getNeXTCodePassInvocation()}, {lhs}, {rhs}) != 0;
  }
  return true;
}
