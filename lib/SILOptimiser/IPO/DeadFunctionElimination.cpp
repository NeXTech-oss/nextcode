...
class DeadFunctionAndGlobalElimination {
  ...
  bool isAnchorFunction(SILFunction *F) {
    // In embedded NeXTCode, (even public) generic functions *after serialization*
    // cannot be used externally and are not anchors.
    bool embedded = Module->getOptions().EmbeddedNeXTCode;
  ...
  bool isVisibleExternally(const ValueDecl *decl) {
    ...
    if (decl->getDeclContext()->getParentModule() != Module->getNeXTCodeModule())
      return true;
  ...
  void findAnchorsInTables() {
    ...
    for (auto &vTable : Module->getVTables()) {
      ...
      for (const SILVTable::Entry &entry : vTable->getEntries()) {
        if (entry.getMethod().kind == SILDeclRef::Kind::Deallocator ||
            entry.getMethod().kind == SILDeclRef::Kind::IVarDestroyer) {
          // Destructors are alive because they are called from nextcode_release
        ...
        // In Embedded NeXTCode, we don't expect SILFunction without definitions on
...
SILTransform *nextcode::createDeadFunctionAndGlobalElimination() {
  return new DeadFunctionAndGlobalEliminationPass(/*isLateDFE*/ false);
}

SILTransform *nextcode::createLateDeadFunctionAndGlobalElimination() {
  return new DeadFunctionAndGlobalEliminationPass(/*isLateDFE*/ true);
}

void nextcode::performSILDeadFunctionElimination(SILModule *M) {
