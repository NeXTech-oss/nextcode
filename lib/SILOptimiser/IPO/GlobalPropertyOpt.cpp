...
class GlobalPropertyOpt {
  ...
  struct Entry {
    ...
    /// The property which we want to track: is the value/field a native nextcode
  ...
  void scanInstruction(nextcode::SILInstruction *Inst);
...
void GlobalPropertyOpt::scanInstruction(nextcode::SILInstruction *Inst) {
  if (auto *AI = dyn_cast<ApplyInst>(Inst)) {
    ...
    // An alloc_stack itself does not introduce any non-native nextcode arrays.
    return;
  }
...
void GlobalPropertyOpt::replacePropertyCalls() {
  for (ApplyInst *AI : propertyCalls) {
    ...
    // Is the argument a native nextcode array?
...
SILTransform *nextcode::createGlobalPropertyOpt() {
