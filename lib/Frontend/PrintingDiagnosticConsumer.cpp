...
static void enqueueDiagnostic(
  ...
  nextcode_ASTGen_addQueuedDiagnostic(
...
void PrintingDiagnosticConsumer::queueBuffer(
  ...
  nextcode_ASTGen_addQueuedSourceFile(
...
void PrintingDiagnosticConsumer::handleDiagnostic(SourceManager &SM,
  ...
  case DiagnosticOptions::FormattingStyle::NeXTCode: {
#if NEXTCODE_BUILD_NEXTCODE_SYNTAX
    // Use the nextcode-syntax formatter.
	...
	// diagnostic, flush any enqueued diagnostics and start fresh.
      if (!queuedDiagnostics || Info.Kind != DiagnosticKind::Note) {
        ...
        queuedDiagnostics = nextcode_ASTGen_createQueuedDiagnostics();
      }
...
void PrintingDiagnosticConsumer::flush(bool includeTrailingBreak) {
  ...
  if (queuedDiagnostics) {
    ...
    nextcode_ASTGen_renderQueuedDiagnostics(queuedDiagnostics, /*contextSize=*/2,
                                         ForceColors ? 1 : 0,
    ...
    if (renderedString.data()) {
      ...
      nextcode_ASTGen_freeBridgedString(renderedString);
    }
    nextcode_ASTGen_destroyQueuedDiagnostics(queuedDiagnostics);
...
PrintingDiagnosticConsumer::~PrintingDiagnosticConsumer() {
  ...
  for (const auto &sourceFileSyntax : sourceFileSyntax) {
    nextcode_ASTGen_destroySourceFile(sourceFileSyntax.second);
  }
#endif
}
