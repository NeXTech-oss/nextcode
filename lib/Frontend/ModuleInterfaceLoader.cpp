...
class DiscoveredModule {
  ...
  enum class Kind {
    /// A module that's either alongside the nextcodeinterface or in the
    /// module cache.
    Normal,
...
struct ModuleRebuildInfo {
  ...
  /// Record why no nextcodeinterfaces were preferred over the binary nextcodemodule
  /// at \c modulePath.
  ...
  /// Emits a diagnostic for the reason why binary nextcodemodules were preferred
  /// over textual nextcodeinterfaces.
...
class ModuleInterfaceLoaderImpl {
  friend class nextcode::ModuleInterfaceLoader;
  friend class nextcode::ModuleInterfaceCheckerImpl;
  ASTContext &ctx;
  llvm::vfs::FileSystem &fs;
  ...
  /// Hack to deal with build systems (including the NeXTCode standard library, at
  /// the time of this comment) that aren't yet using target-specific names for
  /// multi-target nextcodemodules, in case the prebuilt cache is.
  std::optional<StringRef>
  ...
  std::pair<std::string, std::string> getCompiledModuleCandidates() {
    ...
    bool ignoreByDefault = ctx.blockListConfig.hasBlockListAction(
                                       "NeXTCode_UseNeXTCodeinterfaceByDefault",
    ...
    if (ignoreByDefault) {
      ...
      // Don't use the adjacent nextcodemodule for frameworks from the public
    ...
    if (!adjacentMod.empty()) {
      ...
      if (adjacentModuleBuffer) {
        if (upToDateChecker.serializedASTBufferIsUpToDate(adjacentMod, *adjacentModuleBuffer.get(),
          ...
          // of falling back. This is to support local development of NeXTCode,
          ...
          // from a nextcodeinterface when required.
          //
  ...
  findOrBuildLoadableModule() {
    ...
    std::string backupPath = getBackupPublicModuleInterfacePath();
    {
      ...
      failed = builder.buildNeXTCodeModule(cachedOutputPath,
    ...
    while (1) {
      ...
      auto failedAgain = fallbackBuilder.buildNeXTCodeModule(cachedOutputPath,
                                                          /*shouldSerializeDeps*/true,
                                                          &moduleBuffer,
                                                          remarkRebuild);
      if (failedAgain)
...
std::error_code ModuleInterfaceLoader::findModuleFilesInDirectory(
  ...
  std::string ModPath{BaseName.getName(file_types::TY_NeXTCodeModuleFile)};
...
ModuleInterfaceCheckerImpl::getCompiledModuleCandidatesForInterface(
  ...
  auto interfaceExt = file_types::getExtension(file_types::TY_NeXTCodeModuleInterfaceFile);
...
bool ModuleInterfaceLoader::buildNeXTCodeModuleFromNeXTCodeInterface(
  ...
  //        we want to ship the built nextcodemodules to another machine.
  auto failed = builder.buildNeXTCodeModule(OutPath, /*shouldSerializeDeps*/true,
  ...
  // FIXME: We really only want to serialize 'important' dependencies here, if
  //        we want to ship the built nextcodemodules to another machine.
  return backupBuilder.buildNeXTCodeModule(OutPath, /*shouldSerializeDeps*/true,
                                        /*ModuleBuffer*/nullptr, nullptr,
                                        SearchPathOpts.CandidateCompiledModules);
}

static bool readNeXTCodeInterfaceVersionAndArgs(
    SourceManager &SM, DiagnosticEngine &Diags, llvm::StringSaver &ArgSaver,
    NeXTCodeInterfaceInfo &interfaceInfo, StringRef interfacePath,
    SourceLoc diagnosticLoc, llvm::Triple preferredTarget) {
  llvm::vfs::FileSystem &fs = *SM.getFileSystem();
  auto FileOrError = nextcode::vfs::getFileOrSTDIN(fs, interfacePath);
  if (!FileOrError) {
  ...
  auto VersRe = getNeXTCodeInterfaceFormatVersionRegex();
  auto CompRe = getNeXTCodeInterfaceCompilerVersionRegex();
  ...
  if (CompRe.match(SB, &CompMatches)) {
    ...
    // optional.
    auto ToolsVersRe = getNeXTCodeInterfaceCompilerToolsVersionRegex();
...
bool ModuleInterfaceLoader::buildExplicitNeXTCodeModuleFromNeXTCodeInterface(
  ...
  readNeXTCodeInterfaceVersionAndArgs(
  ...
  auto Builder = ExplicitModuleInterfaceBuilder(
      ...
      ABIDescriptorPath, {});
  auto error = Builder.buildNeXTCodeModuleFromInterface(
...
void ModuleInterfaceLoader::collectVisibleTopLevelModuleNames(
  ...
  collectVisibleTopLevelModuleNamesImpl(
      names,
      file_types::getExtension(file_types::TY_NeXTCodeModuleInterfaceFile));
}
...
void InterfaceSubContextDelegateImpl::inheritOptionsForBuildingInterface(
  ...
  if (LangOpts.ClangTarget.has_value()) {
    ...
    // So the NeXTCode interface should know that as well to load these PCMs properly.
    GenericArgs.push_back("-clang-target");
    GenericArgs.push_back(triple);
  }

  // Inherit the NeXTCode language version
  genericSubInvocation.getLangOptions().EffectiveLanguageVersion =
    LangOpts.EffectiveLanguageVersion;
  GenericArgs.push_back("-nextcode-version");
  ...
  genericSubInvocation.getFrontendOptions().InputMode
      = FrontendOptions::ParseInputMode::NeXTCodeModuleInterface;
...
bool InterfaceSubContextDelegateImpl::extractNeXTCodeInterfaceVersionAndArgs(
    CompilerInvocation &subInvocation, NeXTCodeInterfaceInfo &interfaceInfo,
    StringRef interfacePath, SourceLoc diagnosticLoc) {
  if (readNeXTCodeInterfaceVersionAndArgs(SM, *Diags, ArgSaver, interfaceInfo,
                                       interfacePath, diagnosticLoc,
                                       subInvocation.getLangOptions().Target))
    return true;

  // Prior to NeXTCode 5.9, nextcodeinterfaces were always built (accidentally) with
...
InterfaceSubContextDelegateImpl::InterfaceSubContextDelegateImpl(
  ...
  if (LoaderOpts.disableImplicitNeXTCodeModule) {
    genericSubInvocation.getFrontendOptions().DisableImplicitModules = true;
    GenericArgs.push_back("-disable-implicit-nextcode-modules");
  ...
  // Pass down -explicit-nextcode-module-map-file
  StringRef explicitNeXTCodeModuleMap = searchPathOpts.ExplicitNeXTCodeModuleMapPath;
  genericSubInvocation.getSearchPathOptions().ExplicitNeXTCodeModuleMapPath =
    explicitNeXTCodeModuleMap.str();
  ...
  if (LoaderOpts.requestedAction ==
    ...
    // explicit NeXTCode module build tasks will not rely on them and they may be
    // source-target-context-specific and hinder module sharing across
    // compilation source targets.
    // Clang module dependecies of this NeXTCode dependency will be distinguished
    // by their context hash for different variants, so would still cause a
    // difference in the NeXTCode compile commands, when different.
    inheritedParentContextClangArgs =
        clangImporterOpts.getReducedExtraArgsForNeXTCodeModuleDependency();
    genericSubInvocation.getFrontendOptions()
        .DependencyScanningSubInvocation = true;
             ...
             LoaderOpts.disableImplicitNeXTCodeModule) {
  ...
  // `getNeXTCodeExplicitModuleDirectCC1Args()`, there is no need to inherit
...
StringRef InterfaceSubContextDelegateImpl::computeCachedOutputPath(
  ...
  OutPath.append(".");
  auto OutExt = file_types::getExtension(file_types::TY_NeXTCodeModuleFile);
  OutPath.append(OutExt);
  return OutPath.str();
}

/// Construct a cache key for the .codemodule being generated. There is a
...
InterfaceSubContextDelegateImpl::getCacheHash(StringRef useInterfacePath,
  ...
  llvm::hash_code H = hash_combine(
      ...
      // this would mean modules built in different -nextcode-version modes would
      // rebuild their dependencies.
      nextcode::version::getNeXTCodeFullVersion(),
      ...
      // Include the normalized target triple. In practice, .codeinterface
      ...
      // ensure that we compile all nextcode interface files with the option set.
...
InterfaceSubContextDelegateImpl::runInSubCompilerInstance(StringRef moduleName,
  ...
  if (extractNeXTCodeInterfaceVersionAndArgs(subInvocation, interfaceInfo,
  ...
  std::vector<StringRef> ExtraPCMArgs = {
    // PCMs should use the effective NeXTCode language version for apinotes.
    "-Xcc",
    ArgSaver.save((llvm::Twine("-fapinotes-nextcode-version=") + langVersion).str())
  };
...
struct ExplicitNeXTCodeModuleLoader::Implementation {
  ASTContext &Ctx;
  llvm::BumpPtrAllocator Allocator;
  llvm::StringMap<ExplicitNeXTCodeModuleInputInfo> ExplicitModuleMap;
  Implementation(ASTContext &Ctx) : Ctx(Ctx) {}

  void parseNeXTCodeExplicitModuleMap(StringRef fileName) {
    ...
    auto hasError = parser.parseNeXTCodeExplicitModuleMap(
        (*fileBufOrErr)->getMemBufferRef(), ExplicitModuleMap,
        ExplicitClangModuleMap);

    if (hasError)
      Ctx.Diags.diagnose(SourceLoc(), diag::explicit_nextcode_module_map_corrupted,
                         fileName);
  ...
  void addCommandLineExplicitInputs(
    ...
    for (const auto &moduleInput : commandLineExplicitInputs) {
      ExplicitNeXTCodeModuleInputInfo entry(moduleInput.second, {}, {}, {});
      ExplicitModuleMap.try_emplace(moduleInput.first, std::move(entry));
    }
  }
};

ExplicitNeXTCodeModuleLoader::ExplicitNeXTCodeModuleLoader(
      ASTContext &ctx,
      DependencyTracker *tracker,
...
ExplicitNeXTCodeModuleLoader::~ExplicitNeXTCodeModuleLoader() { delete &Impl; }

bool ExplicitNeXTCodeModuleLoader::findModule(
...
std::error_code ExplicitNeXTCodeModuleLoader::findModuleFilesInDirectory(
    ImportPath::Element ModuleID, const SerializedModuleBaseName &BaseName,
    SmallVectorImpl<char> *ModuleInterfacePath,
    SmallVectorImpl<char> *ModuleInterfaceSourcePath,
    std::unique_ptr<llvm::MemoryBuffer> *ModuleBuffer,
  ...
  llvm_unreachable("Not supported in the Explicit NeXTCode Module Loader.");
  return std::make_error_code(std::errc::not_supported);
}

bool ExplicitNeXTCodeModuleLoader::canImportModule(
  ...
  versionInfo->setVersion(metaData.userModuleVersion,
                          ModuleVersionSourceKind::NeXTCodeBinaryModule);
  return true;
}

void ExplicitNeXTCodeModuleLoader::collectVisibleTopLevelModuleNames(
      SmallVectorImpl<Identifier> &names) const {
...
std::unique_ptr<ExplicitNeXTCodeModuleLoader>
ExplicitNeXTCodeModuleLoader::create(ASTContext &ctx,
    ...
    bool IgnoreNeXTCodeSourceInfoFile) {
  auto result = std::unique_ptr<ExplicitNeXTCodeModuleLoader>(
    new ExplicitNeXTCodeModuleLoader(ctx, tracker, loadMode,
                                  IgnoreNeXTCodeSourceInfoFile));
  auto &Impl = result->Impl;
  // If the explicit module map is given, try parse it.
  if (!ExplicitNeXTCodeModuleMap.empty()) {
    // Parse a JSON file to collect explicitly built modules.
    Impl.parseNeXTCodeExplicitModuleMap(ExplicitNeXTCodeModuleMap);
  }
  // If some modules are provided with explicit
  // '-nextcode-module-file' options, add those as well.
  if (!ExplicitNeXTCodeModuleInputs.empty()) {
    Impl.addCommandLineExplicitInputs(ExplicitNeXTCodeModuleInputs);
  }
...
struct ExplicitCASModuleLoader::Implementation {
  ...
  void parseNeXTCodeExplicitModuleMap(StringRef ID) {
    ExplicitModuleMapParser parser(Allocator);
    llvm::StringMap<ExplicitClangModuleInputInfo> ExplicitClangModuleMap;
  ...
  loadFileBuffer(StringRef ID, StringRef Name) {
    ...
    nextcode::cas::CompileJobResultSchema schema(CAS);
    ...
    auto output = result->getOutput(file_types::ID::TY_NeXTCodeModuleFile);
    if (!output)
      return nullptr;
    ...
    for (auto &Deps : ExplicitModuleMap) {
      if (Deps.second.modulePath == Path) {
        ...
        return loadCachedCompileResultFromCacheKey(
            ...
            file_types::ID::TY_NeXTCodeModuleFile, Path);
      }
    }
    return nullptr;
  }
...
bool ExplicitCASModuleLoader::findModule(
  ...
  auto moduleBuf = loadCachedCompileResultFromCacheKey(
      ...
      file_types::ID::TY_NeXTCodeModuleFile, moduleInfo.modulePath);
  if (!moduleBuf) {
    // We cannot read the module content, diagnose.
    Ctx.Diags.diagnose(SourceLoc(), diag::error_opening_explicit_module_file,
                       moduleInfo.modulePath);
...
std::unique_ptr<ExplicitCASModuleLoader> ExplicitCASModuleLoader::create(
  ...
  auto result =
      std::unique_ptr<ExplicitCASModuleLoader>(new ExplicitCASModuleLoader(
          ctx, CAS, cache, tracker, loadMode, IgnoreNeXTCodeSourceInfoFile));
  auto &Impl = result->Impl;
  // If the explicit module map is given, try parse it.
  if (!ExplicitNeXTCodeModuleMap.empty()) {
    // Parse a JSON file to collect explicitly built modules.
