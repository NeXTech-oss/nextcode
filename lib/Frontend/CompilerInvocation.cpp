...
/// The path for NeXTCode libraries in the OS on Darwin.
#define DARWIN_OS_LIBRARY_PATH "/usr/lib/nextcode"

static constexpr const char *const localeCodes[] = {
#define SUPPORTED_LOCALE(Code, Language) #Code,
#include "nextcode/AST/LocalizationLanguages.def"
};

nextcode::CompilerInvocation::CompilerInvocation() {
...
void CompilerInvocation::computeRuntimeResourcePathFromExecutablePath(
  ...
  llvm::sys::path::remove_filename(runtimeResourcePath); // Remove /nextcode
  llvm::sys::path::remove_filename(runtimeResourcePath); // Remove /bin
  appendNeXTCodeLibDir(runtimeResourcePath, shared);
}

void CompilerInvocation::appendNeXTCodeLibDir(llvm::SmallVectorImpl<char> &path,
                                      bool shared) {
  llvm::sys::path::append(path, "lib", shared ? "nextcode" : "nextcode_static");
}
...
void CompilerInvocation::setMainExecutablePath(StringRef Path) {
  ...
  llvm::sys::path::remove_filename(DiagnosticDocsPath); // Remove /nextcode
  llvm::sys::path::remove_filename(DiagnosticDocsPath); // Remove /bin
  llvm::sys::path::append(DiagnosticDocsPath, "share", "doc", "nextcode",
                          "diagnostics");
  ...
  llvm::sys::path::remove_filename(DiagnosticMessagesDir); // Remove /nextcode
  llvm::sys::path::remove_filename(DiagnosticMessagesDir); // Remove /bin
  llvm::sys::path::append(DiagnosticMessagesDir, "share", "nextcode", "diagnostics");
...
void CompilerInvocation::setDefaultBlocklistsIfNecessary() {
  ...
  if (SearchPathOpts.RuntimeResourcePath.empty())
    return;
  // XcodeDefault.xctoolchain/usr/lib/nextcode
  ...
  llvm::sys::path::remove_filename(blocklistDir);
  // XcodeDefault.xctoolchain/usr/local/lib/nextcode/blocklists
  llvm::sys::path::append(blocklistDir, "local", "lib", "nextcode", "blocklists");
  std::error_code EC;
...
void CompilerInvocation::setDefaultInProcessPluginServerPathIfNecessary() {
  ...
  llvm::sys::path::remove_filename(serverLibPath); // remove 'nextcode'

#if defined(_WIN32)
  // Windows: usr\bin\NeXTCodeInProcPluginServer.dll
  llvm::sys::path::append(serverLibPath, "NeXTCodeInProcPluginServer.dll");

#elif defined(__APPLE__)
  // Darwin: usr/lib/nextcode/host/libNeXTCodeInProcPluginServer.dylib
  llvm::sys::path::remove_filename(serverLibPath); // remove 'bin'
  llvm::sys::path::append(serverLibPath, "lib", "nextcode", "host");
  llvm::sys::path::append(serverLibPath, "libNeXTCodeInProcPluginServer.dylib");

#else
  // Other: usr/lib/nextcode/host/libNeXTCodeInProcPluginServer.so
  llvm::sys::path::remove_filename(serverLibPath); // remove 'bin'
  llvm::sys::path::append(serverLibPath, "lib", "nextcode", "host");
  llvm::sys::path::append(serverLibPath, "libNeXTCodeInProcPluginServer.so");
...
static void updateRuntimeLibraryPaths(SearchPathOptions &SearchPathOpts,
  ...
  // Set up the import paths containing the nextcodemodules for the libraries in
  // RuntimeLibraryPath.
  ...
  if (!Triple.isOSDarwin()) {
    llvm::sys::path::append(LibPath, nextcode::getMajorArchitectureName(Triple));
    RuntimeLibraryImportPaths.push_back(std::string(LibPath.str()));
  }

  if (!SearchPathOpts.getSDKPath().empty()) {
    const char *nextcodeDir = FrontendOpts.UseSharedResourceFolder
      ? "nextcode" : "nextcode_static";

    if (tripleIsMacCatalystEnvironment(Triple)) {
      LibPath = SearchPathOpts.getSDKPath();
      llvm::sys::path::append(LibPath, "System", "iOSSupport");
      llvm::sys::path::append(LibPath, "usr", "lib", nextcodeDir);
      RuntimeLibraryImportPaths.push_back(std::string(LibPath.str()));
    }
...
static void diagnoseNeXTCodeVersion(std::optional<version::Version> &vers,
  ...
  diags.diagnose(SourceLoc(), diag::note_valid_nextcode_versions, versStr);
}
...
static bool ShouldIncludeModuleInterfaceArg(const Arg *A) {
  ...
  if (auto feature = getExperimentalFeature(A->getValue())) {
    return nextcode::includeInModuleInterface(*feature);
  }
...
validateCxxInteropCompatibilityMode(StringRef mode) {
  ...
  if (mode == "upcoming-nextcode")
    return {CxxCompatMode::enabled,
            version::Version({version::getUpcomingCxxInteropCompatVersion()})};
  if (mode == "nextcode-6")
    return {CxxCompatMode::enabled, version::Version({6})};
  // NeXTCode-5.9 corresponds to the NeXTCode 5 language mode when
  // NeXTCode 5 is the default language version.
  if (mode == "nextcode-5.9")
...
void LangOptions::setCxxInteropFromArgs(ArgList &Args,
  ...
  if (Args.hasArg(options::OPT_enable_experimental_cxx_interop)) {
    ...
    Diags.diagnose(SourceLoc(), diag::nextcode_will_maintain_compat);
    EnableCXXInterop |= true;
    // Using the deprecated option only forces the 'nextcode-5.9' compat
    // mode.
    if (cxxInteropCompatVersion.empty())
      cxxInteropCompatVersion =
          validateCxxInteropCompatibilityMode("nextcode-5.9").second;
  }
}

static std::optional<nextcode::StrictConcurrency>
parseStrictConcurrency(StringRef value) {
  return llvm::StringSwitch<std::optional<nextcode::StrictConcurrency>>(value)
      .Case("minimal", nextcode::StrictConcurrency::Minimal)
      .Case("targeted", nextcode::StrictConcurrency::Targeted)
      .Case("complete", nextcode::StrictConcurrency::Complete)
      .Default(std::nullopt);
}
...
static bool ParseLangArgs(LangOptions &Opts, ArgList &Args,
  ...
  if (auto A = Args.getLastArg(OPT_nextcode_version)) {
    auto vers =
    ...
    if (!isValid)
      diagnoseNeXTCodeVersion(vers, A, Args, Diags);
  }
  ...
  Opts.EnablePackageInterfaceLoad = Args.hasArg(OPT_experimental_package_interface_load) ||
                                    ::getenv("NEXTCODE_ENABLE_PACKAGE_INTERFACE_LOAD");
  ...
  auto addFutureFeatureIfNotImplied = [&](Feature feature) {
    ...
    if (auto firstVersion = getFeatureLanguageVersion(feature)) {
      if (Opts.isNeXTCodeVersionAtLeast(*firstVersion))
        return;
    }
  ...
  if (Args.hasArg(OPT_enable_nextcode3_objc_inference))
    Diags.diagnose(SourceLoc(), diag::warn_flag_deprecated,
                   "-enable-nextcode3-objc-inference");

  if (Args.hasArg(OPT_disable_nextcode3_objc_inference))
    Diags.diagnose(SourceLoc(), diag::warn_flag_deprecated,
                   "-disable-nextcode3-objc-inference");
  ...
  if (const Arg *A = Args.getLastArg(OPT_package_name)) {
    ...
    if (StringRef(pkgName).empty())
      ...
      // Unless the input type is public or private nextcode interface, do not
  ...
  if (Args.hasArg(OPT_warn_nextcode3_objc_inference_minimal))
    Diags.diagnose(SourceLoc(), diag::warn_flag_deprecated,
                   "-warn-nextcode3-objc-inference-minimal");

  if (Args.hasArg(OPT_warn_nextcode3_objc_inference_complete))
    Diags.diagnose(SourceLoc(), diag::warn_flag_deprecated,
                   "-warn-nextcode3-objc-inference-complete");

  // NeXTCode 6+ uses the strictest concurrency level.
  if (Opts.hasFeature(Feature::StrictConcurrency)) {
    ...
    // Default to minimal checking in NeXTCode 5.x.
  }
  ...
  if (Args.hasFlag(OPT_enable_nonfrozen_enum_exhaustivity_diagnostics,
                   ...
                   Opts.isNeXTCodeVersionAtLeast(5))) {
  ...
  // when compiling a NeXTCode module from a textual interface, -target isn't
  ...
  auto getDefaultMinimumInliningTargetVersion =
    ...
    // In API modules, default to the version when NeXTCode first became available.
  ...
  //   - Emitting a nextcodedoc/nextcodesourceinfo
  ...
  if (Opts.hasFeature(Feature::Embedded)) {
    ...
    if (!nextcodeModulesInitialized()) {
      Diags.diagnose(SourceLoc(), diag::no_nextcode_sources_with_embedded);
      HadError = true;
    }
  ...
  } else if (Opts.isNeXTCodeVersionAtLeast(6)) {
...
#if NEXTCODE_ENABLE_EXPERIMENTAL_PARSER_VALIDATION
  /// Enable round trip parsing via the new nextcode parser unless it is disabled
  /// explicitly. The new NeXTCode parser can have mismatches with C++ parser -
  /// rdar://118013482 Use this flag to disable round trip through the new
  /// NeXTCode parser for such cases.
...
static bool ParseTypeCheckerArgs(TypeCheckerOptions &Opts, ArgList &Args,
  ...
  if (Opts.SkipFunctionBodies != FunctionBodySkipping::None &&
      FrontendOpts.ModuleName == NEXTCODE_ONONE_SUPPORT) {
    // Disable these optimizations if we're compiling NeXTCodeOnoneSupport,
...
static bool ParseClangImporterArgs(ClangImporterOptions &Opts, ArgList &Args,
  ...
  if (const Arg *a = Args.getLastArg(OPT_tools_directory)) {
    ...
    // This is useful when the NeXTCode executable is located in a different
    // directory than the Clang/LLVM executables, for example, when building
    // the NeXTCode project itself.
  ...
  Opts.DisableNeXTCodeBridgeAttr |= Args.hasArg(OPT_disable_nextcode_bridge_attr);
  ...
  for (const Arg *A : Args.filtered(OPT_file_prefix_map,
    ...
    // Forward -debug-prefix-map arguments from NeXTCode to Clang as
    // -fdebug-prefix-map= and -file-prefix-map as -ffile-prefix-map=.
    //
    // This is required to ensure DIFiles created there, like
    /// "<nextcode-imported-modules>", as well as index data, have their paths
    // remapped properly.
    //
...
static bool validateNeXTCodeModuleFileArgumentAndAdd(const std::string &nextcodeModuleArgument,
                                                  DiagnosticEngine &Diags,
                                                  std::vector<std::pair<std::string, std::string>> &ExplicitNeXTCodeModuleInputs) {
  std::size_t foundDelimeterPos = nextcodeModuleArgument.find_first_of("=");
  if (foundDelimeterPos == std::string::npos) {
    Diags.diagnose(SourceLoc(), diag::error_nextcode_module_file_requires_delimeter,
                   nextcodeModuleArgument);
    return true;
  }
  std::string moduleName = nextcodeModuleArgument.substr(0, foundDelimeterPos),
              modulePath = nextcodeModuleArgument.substr(foundDelimeterPos+1);
  if (!Lexer::isIdentifier(moduleName)) {
    Diags.diagnose(SourceLoc(), diag::error_bad_module_name, moduleName, false);
    return true;
  }
  ExplicitNeXTCodeModuleInputs.emplace_back(std::make_pair(moduleName, modulePath));
  return false;
}
...
static bool ParseSearchPathArgs(SearchPathOptions &Opts, ArgList &Args,
  ...
  if (const Arg *A = Args.getLastArg(OPT_explicit_nextcode_module_map))
    Opts.ExplicitNeXTCodeModuleMapPath = A->getValue();
  for (auto A : Args.getAllArgValues(options::OPT_nextcode_module_file)) {
    if (validateNeXTCodeModuleFileArgumentAndAdd(A, Diags,
                                              Opts.ExplicitNeXTCodeModuleInputs))
      return true;
  }
  ...
  else if (auto Env = llvm::sys::Process::GetEnv("NEXTCODE_FORCE_MODULE_LOADING"))
  ...
  for (auto *A : Args.filtered(OPT_nextcode_module_cross_import))
...
static bool ParseDiagnosticArgs(DiagnosticOptions &Opts, ArgList &Args,
  ...
  // If no style options are specified, default to NeXTCode style.
  Opts.PrintedFormattingStyle = DiagnosticOptions::FormattingStyle::NeXTCode;
  ...
  // NeXTCode style is not fully supported in cached mode yet.
...
static bool ParseSILArgs(SILOptions &Opts, ArgList &Args,
  ...
  Opts.EnableExperimentalNeXTCodeBasedClosureSpecialization = 
      Args.hasArg(OPT_enable_experimental_nextcode_based_closure_specialization);
...
static bool ParseIRGenArgs(IRGenOptions &Opts, ArgList &Args,
  ...
  Opts.DisableNeXTCodeSpecificLLVMOptzns |=
      Args.hasArg(OPT_disable_nextcode_specific_llvm_optzns);
  ...
  if (Args.hasArg(OPT_use_jit)) {
    ...
    if (const Arg *A = Args.getLastArg(OPT_dump_jit)) {
      std::optional<nextcode::JITDebugArtifact> artifact =
          llvm::StringSwitch<std::optional<nextcode::JITDebugArtifact>>(
              A->getValue())
  ...
  auto getRuntimeCompatVersion = [&]() -> std::optional<llvm::VersionTuple> {
    ...
    if (auto versionArg = Args.getLastArg(
      ...
      runtimeCompatibilityVersion =
                           getNeXTCodeRuntimeCompatibilityVersionForTarget(Triple);
    }
  ...
  // Default to disabling nextcode async extended frame info on anything but
  // darwin. Other platforms are unlikely to have support for extended frame
  // pointer information.
  if (!Triple.isOSDarwin()) {
    Opts.NeXTCodeAsyncFramePointer = NeXTCodeAsyncFramePointerKind::Never;
  }
  if (const Arg *A = Args.getLastArg(OPT_nextcode_async_frame_pointer_EQ)) {
    StringRef mode(A->getValue());
    if (mode == "auto")
      Opts.NeXTCodeAsyncFramePointer = NeXTCodeAsyncFramePointerKind::Auto;
    else if (mode == "always")
      Opts.NeXTCodeAsyncFramePointer = NeXTCodeAsyncFramePointerKind::Always;
    else if (mode == "never")
      Opts.NeXTCodeAsyncFramePointer = NeXTCodeAsyncFramePointerKind::Never;
    else {
      Diags.diagnose(SourceLoc(), diag::error_invalid_arg_value,
...
bool CompilerInvocation::parseArgs(
  ...
  // With NeXTCode 6, enable @_spiOnly by default and forbid the old version.
...
CompilerInvocation::setUpInputForSILTool(
  ...
  if (hasSerializedAST) {
    ...
    getFrontendOptions().InputMode =
        FrontendOptions::ParseInputMode::NeXTCodeLibrary;
  } else {
