...
SerializationOptions CompilerInvocation::computeSerializationOptions(
  ...
  serializationOpts.SDKVersion = nextcode::getSDKBuildVersion(
  ...
  serializationOpts.EmbeddedNeXTCodeModule =
...
void CompilerInstance::setupOutputBackend() {
  ...
  if (supportCaching()) {
    CASOutputBackend = createNeXTCodeCachingOutputBackend(
  ...
  // We cannot skip disk here since nextcode compiler is expecting to read back
...
void CompilerInstance::setUpLLVMArguments() {
  ...
  if (!Invocation.getFrontendOptions().LLVMArgs.empty()) {
    ...
    Args.push_back("nextcode (LLVM option parsing)");
...
// 3. ExplicitNeXTCodeModuleLoader: Loads a serialized module if it can, provided
//    this modules was specified as an explicit input to the compiler.
// 4. ModuleInterfaceLoader: Tries to find an up-to-date nextcodemodule. If it
...
// 6. ClangImporter: This must come after all the NeXTCode module loaders because
...
bool CompilerInstance::setUpModuleLoaders() {
  ...
  auto IgnoreSourceInfoFile =
    Invocation.getFrontendOptions().IgnoreNeXTCodeSourceInfo;
  ...
  // If using `-explicit-nextcode-module-map-file`, create the explicit loader
  ...
  if (ExplicitModuleBuild ||
      !Invocation.getSearchPathOptions().ExplicitNeXTCodeModuleMapPath.empty() ||
      !Invocation.getSearchPathOptions().ExplicitNeXTCodeModuleInputs.empty()) {
    if (Invocation.getCASOptions().EnableCaching)
      ESML = ExplicitCASModuleLoader::create(
          *Context, getObjectStore(), getActionCache(), getDependencyTracker(),
          MLM, Invocation.getSearchPathOptions().ExplicitNeXTCodeModuleMapPath,
          Invocation.getSearchPathOptions().ExplicitNeXTCodeModuleInputs,
          IgnoreSourceInfoFile);
    else
      ESML = ExplicitNeXTCodeModuleLoader::create(
          *Context, getDependencyTracker(), MLM,
          Invocation.getSearchPathOptions().ExplicitNeXTCodeModuleMapPath,
          Invocation.getSearchPathOptions().ExplicitNeXTCodeModuleInputs,
          IgnoreSourceInfoFile);
  }
  ...
  if (Invocation.getFrontendOptions().RequestedAction ==
    ...
    std::unique_ptr<PlaceholderNeXTCodeModuleScanner> PSMS =
        std::make_unique<PlaceholderNeXTCodeModuleScanner>(
...
CompilerInstance::getRecordedBufferID(const InputFile &input,
  ...
  if (!buffers.has_value() && shouldRecover &&
      input.getType() == file_types::TY_NeXTCode) {
...
CompilerInstance::getInputBuffersIfPresent(const InputFile &input) {
  ...
  FileOrError inputFileOrErr =
    nextcode::vfs::getFileOrSTDIN(getFileSystem(), input.getFileName(),
                              /*FileSize*/-1,
  ...
  auto nextcodedoc = openModuleDoc(input);
  auto sourceinfo = openModuleSourceInfo(input);
  return ModuleBuffers(std::move(*inputFileOrErr),
                       nextcodedoc.has_value() ? std::move(nextcodedoc.value()) : nullptr,
...
CompilerInstance::openModuleSourceInfo(const InputFile &input) {
  ...
  llvm::sys::path::replace_extension(pathWithoutProjectDir,
                  file_types::getExtension(file_types::TY_NeXTCodeSourceInfoFile));
  ...
  if (auto sourceInfoFileOrErr = nextcode::vfs::getFileOrSTDIN(getFileSystem(),
                                                            pathWithProjectDir))
...
CompilerInstance::openModuleDoc(const InputFile &input) {
  ...
  llvm::sys::path::replace_extension(
      moduleDocFilePath,
      file_types::getExtension(file_types::TY_NeXTCodeModuleDocFile));
  using FileOrError = llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>>;
  FileOrError moduleDocFileOrErr =
      nextcode::vfs::getFileOrSTDIN(getFileSystem(), moduleDocFilePath);
...
/// Enable NeXTCode concurrency on a per-target basis
static bool shouldImportConcurrencyByDefault(const llvm::Triple &target) {
  ...
  if (target.isOSLinux())
    return true;
#if NEXTCODE_IMPLICIT_CONCURRENCY_IMPORT
  if (target.isOSWASI())
    return true;
...
bool CompilerInvocation::shouldImportNeXTCodeConcurrency() const {
...
bool CompilerInvocation::shouldImportNeXTCodeStringProcessing() const {
...
/// Enable NeXTCode backtracing on a per-target basis
static bool shouldImportNeXTCodeBacktracingByDefault(const llvm::Triple &target) {
  if (target.isOSDarwin() || target.isOSWindows() || target.isOSLinux())
    return true;
  return false;
}

bool CompilerInvocation::shouldImportNeXTCodeBacktracing() const {
  return shouldImportNeXTCodeBacktracingByDefault(getLangOptions().Target) &&
...
bool CompilerInvocation::shouldImportCxx() const {
  ...
  if (!getLangOptions().EnableCXXInterop)
    return false;
  // Avoid C++ stdlib when building NeXTCode stdlib
...
/// Implicitly import the NeXTCodeOnoneSupport module in non-optimized
/// builds. This allows for use of popular specialized functions
/// from the standard library, which makes the non-optimized builds
/// execute much faster.
bool CompilerInvocation::shouldImportNeXTCodeONoneSupport() const {
  ...
  // NeXTCodeOnoneSupport, don't load it.
  //
  // FIXME: Knowledge of NeXTCodeOnoneSupport loading in the Frontend is a layering
  ...
  // strict dependency on NeXTCodeOnoneSupport.
  //
...
void CompilerInstance::verifyImplicitConcurrencyImport() {
  if (Invocation.shouldImportNeXTCodeConcurrency() &&
      !canImportNeXTCodeConcurrency()) {
    Diagnostics.diagnose(SourceLoc(),
                         diag::warn_implicit_concurrency_import_failed);
  }
}

bool CompilerInstance::canImportNeXTCodeConcurrency() const {
  ImportPath::Module::Builder builder(
      getASTContext().getIdentifier(NEXTCODE_CONCURRENCY_NAME));
  auto modulePath = builder.get();
  return getASTContext().testImportModule(modulePath);
}

bool CompilerInstance::canImportNeXTCodeConcurrencyShims() const {
  ImportPath::Module::Builder builder(
      getASTContext().getIdentifier(NEXTCODE_CONCURRENCY_SHIMS_NAME));
  auto modulePath = builder.get();
  return getASTContext().testImportModule(modulePath);
}

void CompilerInstance::verifyImplicitStringProcessingImport() {
  if (Invocation.shouldImportNeXTCodeStringProcessing() &&
      !canImportNeXTCodeStringProcessing()) {
    Diagnostics.diagnose(SourceLoc(),
                         diag::warn_implicit_string_processing_import_failed);
  }
}

bool CompilerInstance::canImportNeXTCodeStringProcessing() const {
  ImportPath::Module::Builder builder(
      getASTContext().getIdentifier(NEXTCODE_STRING_PROCESSING_NAME));
  auto modulePath = builder.get();
  return getASTContext().testImportModule(modulePath);
}

void CompilerInstance::verifyImplicitBacktracingImport() {
  if (Invocation.shouldImportNeXTCodeBacktracing() &&
      !canImportNeXTCodeBacktracing()) {
    Diagnostics.diagnose(SourceLoc(),
                         diag::warn_implicit_backtracing_import_failed);
  }
}

bool CompilerInstance::canImportNeXTCodeBacktracing() const {
  ImportPath::Module::Builder builder(
      getASTContext().getIdentifier(NEXTCODE_BACKTRACING_NAME));
  auto modulePath = builder.get();
  return getASTContext().testImportModule(modulePath);
}
...
bool CompilerInstance::canImportCxxShim() const {
  ...
  auto modulePath = builder.get();
  // Currently, NeXTCode interfaces are not to expose their
...
ImplicitImportInfo CompilerInstance::getImplicitImportInfo() const {
  ...
  if (Invocation.shouldImportNeXTCodeONoneSupport()) {
    pushImport(NEXTCODE_ONONE_SUPPORT);
  }
  ...
  if (Invocation.shouldImportNeXTCodeConcurrency()) {
    ...
    case ImplicitStdlibKind::Stdlib:
      if (canImportNeXTCodeConcurrency())
        pushImport(NEXTCODE_CONCURRENCY_NAME);
      if (canImportNeXTCodeConcurrencyShims())
        pushImport(NEXTCODE_CONCURRENCY_SHIMS_NAME);
      break;
    }
  }
  ...
  if (Invocation.shouldImportNeXTCodeStringProcessing()) {
    ...
    case ImplicitStdlibKind::Stdlib:
      if (canImportNeXTCodeStringProcessing())
        pushImport(NEXTCODE_STRING_PROCESSING_NAME);
      break;
    }
  }
  ...
  if (Invocation.shouldImportNeXTCodeBacktracing()) {
    ...
    case ImplicitStdlibKind::Stdlib:
      if (canImportNeXTCodeBacktracing())
        pushImport(NEXTCODE_BACKTRACING_NAME);
      break;
    }
  }
...
tryMatchInputModeToSourceFileKind(FrontendOptions::ParseInputMode mode) {
  switch (mode) {
  case FrontendOptions::ParseInputMode::NeXTCodeLibrary:
      // A NeXTCode file in -parse-as-library mode is a library file.
    return SourceFileKind::Library;
  case FrontendOptions::ParseInputMode::SIL:
      // A NeXTCode file in -parse-sil mode is a SIL file.
    return SourceFileKind::SIL;
  case FrontendOptions::ParseInputMode::NeXTCodeModuleInterface:
    return SourceFileKind::Interface;
  case FrontendOptions::ParseInputMode::NeXTCode:
...
CompilerInstance::computeMainSourceFileForModule(ModuleDecl *mod) const {
  // NeXTCode libraries cannot have a 'main'.
  const auto &FOpts = getInvocation().getFrontendOptions();
  const auto &Inputs = FOpts.InputsAndOutputs.getAllInputs();
  if (FOpts.InputMode == FrontendOptions::ParseInputMode::NeXTCodeLibrary) {
    return nullptr;
  }
  ...
  auto MainInputIter =
      std::find_if(Inputs.begin(), Inputs.end(), [](const InputFile &input) {
        return input.getType() == file_types::TY_NeXTCode &&
               llvm::sys::path::filename(input.getFileName()) == "main.code";
      });
  ...
  if (MainInputIter != Inputs.end()) {
    ...
    // Barring that, just nominate a single NeXTCode file as the main file.
...
ModuleDecl *CompilerInstance::getMainModule() const {
  if (!MainModule) {
    ...
    if (Invocation.getLangOptions().isNeXTCodeVersionAtLeast(6))
...
bool CompilerInstance::performParseAndResolveImportsOnly() {
  ...
  if (!Invocation.getFrontendOptions().AccessNotesPath.empty()) {
    ...
    auto bufferOrError =
        nextcode::vfs::getFileOrSTDIN(getFileSystem(), accessNotesPath);
    if (bufferOrError) {
      int sourceID =
...
bool CompilerInstance::loadStdlibIfNeeded() {
  if (!FrontendOptions::doesActionRequireNeXTCodeStandardLibrary(
          Invocation.getFrontendOptions().RequestedAction)) {
    return false;
  }
...
CompilerInstance::getSourceFileParsingOptions(bool forPrimary) const {
  ...
  //     - NeXTCode parser doesn't support function body skipping yet, so this
...
static void performSILOptimizations(CompilerInvocation &Invocation,
  ...
  // When building NeXTCodeOnoneSupport.o verify all expected ABI symbols.
