...
#define DEBUG_TYPE "nextcode-cas-backend"
...
namespace {
class NeXTCodeCASOutputFile final : public OutputFileImpl {
public:
  ...
  NeXTCodeCASOutputFile(StringRef Path, OnKeepType OnKeep)
...
void NeXTCodeCASOutputBackend::anchor() {}

class NeXTCodeCASOutputBackend::Implementation {
public:
  ...
  createFileImpl(llvm::StringRef ResolvedPath,
    ...
    return std::make_unique<NeXTCodeCASOutputFile>(
        ResolvedPath,
...
private:
  friend class NeXTCodeCASOutputBackend;
  ObjectStore &CAS;
  ActionCache &Cache;
  ObjectRef BaseKey;
  const FrontendInputsAndOutputs &InputsAndOutputs;
...
NeXTCodeCASOutputBackend::NeXTCodeCASOutputBackend(
    ObjectStore &CAS, ActionCache &Cache, ObjectRef BaseKey,
    const FrontendInputsAndOutputs &InputsAndOutputs,
    FrontendOptions::ActionType Action)
    : Impl(*new NeXTCodeCASOutputBackend::Implementation(
          CAS, Cache, BaseKey, InputsAndOutputs, Action)) {}

NeXTCodeCASOutputBackend::~NeXTCodeCASOutputBackend() { delete &Impl; }

IntrusiveRefCntPtr<OutputBackend> NeXTCodeCASOutputBackend::cloneImpl() const {
  return makeIntrusiveRefCnt<NeXTCodeCASOutputBackend>(
      Impl.CAS, Impl.Cache, Impl.BaseKey, Impl.InputsAndOutputs, Impl.Action);
}

Expected<std::unique_ptr<OutputFileImpl>>
NeXTCodeCASOutputBackend::createFileImpl(StringRef ResolvedPath,
                                      std::optional<OutputConfig> Config) {
  return Impl.createFileImpl(ResolvedPath, Config);
}

file_types::ID NeXTCodeCASOutputBackend::getOutputFileType(StringRef Path) const {
  return file_types::lookupTypeForExtension(llvm::sys::path::extension(Path));
}

Error NeXTCodeCASOutputBackend::storeImpl(StringRef Path, StringRef Bytes,
                                       unsigned InputIndex,
                                       file_types::ID OutputKind) {
  return Impl.storeImpl(Path, Bytes, InputIndex, OutputKind);
}

Error NeXTCodeCASOutputBackend::storeCachedDiagnostics(unsigned InputIndex,
                                                    StringRef Bytes) {
  return storeImpl("<cached-diagnostics>", Bytes, InputIndex,
                   file_types::ID::TY_CachedDiagnostics);
}

void NeXTCodeCASOutputBackend::Implementation::initBackend(
    const FrontendInputsAndOutputs &InputsAndOutputs) {
...
Error NeXTCodeCASOutputBackend::Implementation::storeImpl(
...
Error NeXTCodeCASOutputBackend::Implementation::finalizeCacheKeysFor(
    unsigned InputIndex) {
