...
bool ArgsToFrontendOptionsConverter::convert(
  ...
  Opts.DisableImplicitModules |= Args.hasArg(OPT_disable_implicit_nextcode_modules);
  ...
  Opts.IgnoreNeXTCodeSourceInfo |= Args.hasArg(OPT_ignore_module_source_info);
  computeHelpOptions();
  ...
  if (Args.hasArg(OPT_parse_sil) || Opts.InputsAndOutputs.shouldTreatAsSIL()) {
    ...
    Opts.InputMode = FrontendOptions::ParseInputMode::NeXTCodeModuleInterface;
  } else if (Args.hasArg(OPT_parse_as_library)) {
    Opts.InputMode = FrontendOptions::ParseInputMode::NeXTCodeLibrary;
  } else {
    Opts.InputMode = FrontendOptions::ParseInputMode::NeXTCode;
  }
...
void ArgsToFrontendOptionsConverter::computeTBDOptions() {
  ...
  if (const Arg *A = Args.getLastArg(OPT_validate_tbd_against_ir_EQ)) {
    ...
    // interop is enabled.
    // See https://github.com/apple/nextcode/issues/56458.
...
bool ArgsToFrontendOptionsConverter::computeModuleName() {
  ...
  if (!FrontendOptions::needsProperModuleName(Opts.RequestedAction) ||
      Opts.isCompilingExactlyOneNeXTCodeFile()) {
