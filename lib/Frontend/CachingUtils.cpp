...
llvm::IntrusiveRefCntPtr<NeXTCodeCASOutputBackend>
createNeXTCodeCachingOutputBackend(
    llvm::cas::ObjectStore &CAS, llvm::cas::ActionCache &Cache,
    llvm::cas::ObjectRef BaseKey,
    const FrontendInputsAndOutputs &InputsAndOutputs,
    FrontendOptions::ActionType Action) {
  return makeIntrusiveRefCnt<NeXTCodeCASOutputBackend>(CAS, Cache, BaseKey,
...

  {
    ...
    if (Schema.isRootNode(*ResultProxy)) {
      ...
      if (auto Err = Result->forEachOutput(
              [&](nextcode::cas::CompileJobCacheResult::Output Output) -> Error {
