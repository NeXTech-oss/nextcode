...
struct SerializedDiagnosticInfo {
  static_assert(sizeof(nextcode::DiagID) == sizeof(uint32_t), "DiagID size");
  static_assert(sizeof(nextcode::DiagnosticKind) == sizeof(uint8_t),
                "DiagKind size");
  uint32_t ID;
...
class CachingDiagnosticsProcessor::Implementation
    : public nextcode::DiagnosticConsumer {
public:
