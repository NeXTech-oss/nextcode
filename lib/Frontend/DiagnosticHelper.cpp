...
#include "nextcode/Frontend/AccumulatingDiagnosticConsumer.h"
...
private:
  ...
  bool finishProcessing() override {
    ...
    nextcode::writeEditsInJson(AllEdits, *OS);
    return false;
  }
};
...
void DiagnosticHelper::Implementation::initDiagConsumers(
  ...
  // See https://github.com/apple/nextcode/issues/45288 for details.
...
static nextcode::file_types::ID computeFileTypeForPath(const StringRef Path) {
  if (!llvm::sys::path::has_extension(Path))
    return nextcode::file_types::ID::TY_INVALID;

  auto Extension = llvm::sys::path::extension(Path).str();
  auto FileType = file_types::lookupTypeForExtension(Extension);
  if (FileType == nextcode::file_types::ID::TY_INVALID) {
...
void DiagnosticHelper::Implementation::diagnoseFatalError(const char *reason,
  ...
  DiagnosticInfo errorInfo(
      ...
      "fatal error encountered during compilation; " NEXTCODE_BUG_REPORT_MESSAGE,
