//===------- ModuleInterfaceSupport.cpp - nextcodeinterface files ------------===//
//
/*
...
version::Version nextcode::InterfaceFormatVersion({1, 0});
...
static void printToolVersionAndFlagsComment(raw_ostream &out,
  ...
  auto ToolsVersion =
      getNeXTCodeInterfaceCompilerVersionForCurrentCompiler(Ctx);
  out << "// " NEXTCODE_INTERFACE_FORMAT_VERSION_KEY ": "
      << InterfaceFormatVersion << "\n";
  out << "// " NEXTCODE_COMPILER_VERSION_KEY ": "
      << ToolsVersion << "\n";
  out << "// " NEXTCODE_MODULE_FLAGS_KEY ": "
      << Opts.Flags;
  ...
  if (Opts.AliasModuleNames) {
    ...
    for (ImportedModule import: imports) {
      ...
      // Skip NeXTCode as it's commonly used in inlinable code,
  ...
  if (!Opts.IgnorableFlags.empty()) {
    out << "// " NEXTCODE_MODULE_FLAGS_IGNORABLE_KEY ": "
        << Opts.IgnorableFlags << "\n";
  }

  auto hasPrivateIgnorableFlags = !Opts.printPublicInterface() && !Opts.IgnorablePrivateFlags.empty();
  if (hasPrivateIgnorableFlags) {
    out << "// " NEXTCODE_MODULE_FLAGS_IGNORABLE_PRIVATE_KEY ": "
        << Opts.IgnorablePrivateFlags << "\n";
  }
}

std::string
nextcode::getNeXTCodeInterfaceCompilerVersionForCurrentCompiler(ASTContext &ctx) {
  return nextcode::version::getNeXTCodeFullVersion(
             ctx.LangOpts.EffectiveLanguageVersion);
}

llvm::Regex nextcode::getNeXTCodeInterfaceFormatVersionRegex() {
  return llvm::Regex("^// " NEXTCODE_INTERFACE_FORMAT_VERSION_KEY
                     ": ([0-9\\.]+)$", llvm::Regex::Newline);
}

llvm::Regex nextcode::getNeXTCodeInterfaceCompilerVersionRegex() {
  return llvm::Regex("^// " NEXTCODE_COMPILER_VERSION_KEY
                     ": (.+)$", llvm::Regex::Newline);
}

llvm::Regex nextcode::getNeXTCodeInterfaceCompilerToolsVersionRegex() {
  return llvm::Regex("NeXTCode version ([0-9\\.]+)", llvm::Regex::Newline);
}

// MARK(https://github.com/apple/nextcode/issues/43510): Module name shadowing warnings
//
// When codec emits a module interface, it qualifies most types with their
...
static void printImports(raw_ostream &out,
  ...
  // When printing the private or package nextcodeinterface file, print implementation-only
  ...
  for (auto import : allImports) {
    ...
    if (!Opts.printPublicInterface()) {
      // An import visible in the private or package nextcodeinterface only.
      //
      ...
      // compatible nextcodeinterfaces and we can live without
...
bool nextcode::emitNeXTCodeInterface(raw_ostream &out,
                               ModuleInterfaceOptions const &Opts,
                               ModuleDecl *M) {
  assert(M);
  ...
  const PrintOptions printOptions = PrintOptions::printNeXTCodeInterfaceFile(
  ...
  if (Opts.DebugPrintInvalidSyntax)
    out << "#__debug_emit_invalid_nextcodeinterface_syntax__\n";
