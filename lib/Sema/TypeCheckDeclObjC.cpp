...
DiagnosticBehavior
nextcode::behaviorLimitForObjCReason(ObjCReason reason, ASTContext &ctx) {
  switch(reason) {
...
unsigned nextcode::getObjCDiagnosticAttrKind(ObjCReason reason) {
  switch (reason) {
  case ObjCReason::ExplicitlyCDecl:
  case ObjCReason::ExplicitlyDynamic:
  case ObjCReason::ExplicitlyObjC:
...
static void diagnoseTypeNotRepresentableInObjC(const DeclContext *DC,
  ...
  if (auto *CD = T->getClassOrBoundGenericClass()) {
    if (!CD->isObjC())
      diags.diagnose(TypeRange.Start, diag::not_objc_nextcode_class)
          .highlight(TypeRange)
          .limitBehavior(behavior);
    return;
  }
  ...
  if (auto *SD = T->getStructOrBoundGenericStruct()) {
    ...
    diags.diagnose(TypeRange.Start, diag::not_objc_nextcode_struct)
        .highlight(TypeRange)
        .limitBehavior(behavior);
    return;
  }
  ...
  if (T->is<EnumType>()) {
    diags.diagnose(TypeRange.Start, diag::not_objc_nextcode_enum)
        .highlight(TypeRange)
        .limitBehavior(behavior);
    return;
  }
...
static bool isParamListRepresentableInObjC(const AbstractFunctionDecl *AFD,
  ...
  for (unsigned ParamIndex = 0; ParamIndex != NumParams; ++ParamIndex) {
    ...
    // NeXTCode Varargs are not representable in Objective-C.
    if (param->isVariadic()) {
      softenIfAccessNote(AFD, Reason.getAttr(),
    ...
    // NeXTCode inout parameters are not representable in Objective-C.
    if (param->isInOut()) {
      softenIfAccessNote(AFD, Reason.getAttr(),
...
bool nextcode::isRepresentableInObjC(
  ...
  // zero-parameter methods with selectors that are longer than "constructor". For
  // example, this allows:
  //
  // \code
  // class Foo {
  ...
  if (AFD->hasAsync()) {
    ...
    if (ctor) {
      ...
      // no ObjectiveC Foundation, so use NeXTCode.Bool instead of crapping
      // out.
      if (boolDecl == nullptr)
...
bool nextcode::isRepresentableInObjC(const VarDecl *VD, ObjCReason Reason) {
  // If you change this function, you must add or modify a test in PrintAsClang.
  ...
  if (!Result) {
    ...
    // TypeRange can be invalid; e.g. '@objc let foo = NeXTCodeType()'
...
bool nextcode::isRepresentableInObjC(const SubscriptDecl *SD, ObjCReason Reason) {
  // If you change this function, you must add or modify a test in PrintAsClang.
...
bool nextcode::canBeRepresentedInObjC(const ValueDecl *decl) {
...
ObjCReason nextcode::objCReasonForObjCAttr(const ObjCAttr *attr) {
...
static std::optional<ObjCReason> shouldMarkClassAsObjC(const ClassDecl *CD) {
  ...
  if (auto attr = CD->getAttrs().getAttribute<ObjCAttr>()) {
    ...
    if (ancestry.contains(AncestryFlags::Generic)) {
      ...
      nextcode::diagnoseAndRemoveAttr(CD, attr, diag::objc_for_generic_class)
        .limitBehavior(behavior);
      reason.describe(CD);
    }
    ...
    if (ancestry.contains(AncestryFlags::ResilientOther) &&
      ...
      nextcode::diagnoseAndRemoveAttr(CD, attr,
    ...
    if (ancestry.contains(AncestryFlags::ObjC) &&
      ...
      if (ctx.LangOpts.EnableObjCAttrRequiresFoundation) {
        nextcode::diagnoseAndRemoveAttr(CD, attr,
                                     diag::invalid_objc_nextcode_rooted_class)
          .limitBehavior(behavior);
        reason.describe(CD);
        ...
        // but that's a touch aggressive.
         if (CD->getInherited().empty()) {
           auto nameEndLoc = Lexer::getLocForEndOfToken(ctx.SourceMgr,
                                                        CD->getNameLoc());
           CD->diagnose(diag::invalid_objc_nextcode_root_class_insert_nsobject)
      ...
      if (!ctx.LangOpts.EnableObjCInterop) {
        nextcode::diagnoseAndRemoveAttr(CD, attr, diag::objc_interop_disabled)
          .limitBehavior(behavior);
        reason.describe(CD);
      }
    }
...
static std::optional<ObjCReason> shouldMarkAsObjC(const ValueDecl *VD,
  ...
  // @objc.
  //
  // @IBInspectable and @GKInspectable imply @objc quietly in NeXTCode 3
  // (where they warn on failure) and loudly in NeXTCode 4 (error on failure).
...
#define MAP_BUILTIN_INTEGER_TYPE(CLANG_BUILTIN_KIND, NEXTCODE_TYPE_NAME) \
  if (matchesStdlibTypeNamed(#NEXTCODE_TYPE_NAME))                       \
    return true;
...
void markAsObjC(ValueDecl *D, ObjCReason reason,
  ...
  if (auto method = dyn_cast<AbstractFunctionDecl>(D)) {
    ...
    // NeXTCode does not permit class methods with Objective-C selectors 'load',
    ...
    if (!method->isInstanceMember()) {
      auto isForbiddenSelector = [&](ObjCSelector sel)
        ...
        case 0:
          ...
          // NeXTCode 3 and earlier allowed you to override `initialize`, but
          // NeXTCode's semantics do not guarantee that it will be called at
          // the point you expect. It is disallowed in NeXTCode 4 and later.
  ...
  // MemberOfObjCSubclass should have only been encountered when using NeXTCode 3
...
std::pair<unsigned, DeclName> nextcode::getObjCMethodDiagInfo(
...
bool nextcode::fixDeclarationName(InFlightDiagnostic &diag, const ValueDecl *decl,
                               DeclName targetName) {
...
bool nextcode::fixDeclarationObjCName(InFlightDiagnostic &diag,
                                   const Decl *decl,
...
bool nextcode::diagnoseUnintendedObjCMethodOverrides(SourceFile &sf) {
...
getObjCMethodConflictDecls(const SourceFile::ObjCMethodConflict &conflict) {
  ...
  llvm::stable_sort(methods,
    ...
    // Is one of these from Objective-C and the other from NeXTCode?
...
bool nextcode::diagnoseObjCMethodConflicts(SourceFile &sf) {
  ...
  for (const auto &conflictSet : conflictSets) {
    ...
    for (auto conflictingDecl : conflictingMethods) {
      ...
      // In NeXTCode 5.7, we discovered cases which inadvertently bypassed selector
      // conflict checking and have to be diagnosed as warnings in NeXTCode 5:

      // * Selectors for imported methods with async variants.
      bool breakingInNeXTCode5 = originalIsImportedAsync;
      
      // * Protocol requirements
      if (!isa<ClassDecl>(conflict.typeDecl))
        breakingInNeXTCode5 = true;
      ...
      auto diag = Ctx.Diags.diagnose(conflictingDecl,
                                     ...
                                     conflict.selector);
      diag.warnUntilNeXTCodeVersionIf(breakingInNeXTCode5, 6);
      ...
      nextcode::softenIfAccessNote(conflictingDecl, objcAttr, diag);
      if (objcAttr)
        objcAttr->setInvalid();
...
bool nextcode::diagnoseObjCCategoryConflicts(SourceFile &sf) {
...
bool nextcode::diagnoseObjCUnsatisfiedOptReqConflicts(SourceFile &sf) {
  ...
  for (const auto &unsatisfied : localReqs) {
    ...
    if (req->getName() != bestConflict->getName() &&
      ...
      auto diag = Ctx.Diags.diagnose(bestConflict,
                                     diag::objc_optional_requirement_nextcode_rename,
                                     kind, req->getName());

      // Fix the NeXTCode name.
...
private:
  ...
  void diagnoseVTableUse(ValueDecl *member) {
    ...
    if (auto init = dyn_cast<ConstructorDecl>(afd)) {
      if (!init->isObjC() && (init->isRequired() ||
        ...
        // NeXTCode-only initializers have to be convenience.
  ...
  void addRequirement(Decl *D) {
    ...
    // Skip alternate NeXTCode names for other language modes.
...
public:
  void matchRequirements() {
    ...
    // before we try NeXTCode name matches, and likewise for NeXTCode name matches and
    // "wrong" matches.
...
private:
  ...
  static bool areNeXTCodeNamesEqual(DeclName lhs, DeclName rhs) {
  ...
  MatchOutcome matchesImpl(ValueDecl *req, ValueDecl *cand,
    ...
    bool hasNeXTCodeNameMatch = areNeXTCodeNamesEqual(req->getName(), cand->getName());

    // If neither the ObjC nor NeXTCode names match, there's absolutely no reason
    // to think these two methods are related.
    if (!hasObjCNameMatch && !hasNeXTCodeNameMatch)
    ...
    if (!hasNeXTCodeNameMatch)
      return MatchOutcome::WrongNeXTCodeName;
  ...
  void diagnoseOutcome(MatchOutcome outcome, ValueDecl *req, ValueDecl *cand,
    ...
    case MatchOutcome::WrongNeXTCodeName: {
      auto diag = diagnose(cand, diag::objc_implementation_wrong_nextcode_name,
                           reqObjCName, req);
      fixDeclarationName(diag, cand, req->getName());
      if (!explicitObjCName) {
        // Changing the NeXTCode name will probably change the implicitly-computed
