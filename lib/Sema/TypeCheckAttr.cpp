...
public:
  ...
  InFlightDiagnostic diagnoseAndRemoveAttr(DeclAttribute *attr,
                                           ArgTypes &&...Args) {
    return nextcode::diagnoseAndRemoveAttr(D, attr,
...
#undef IGNORED_ATTR
  ...
  void visitNeXTCodeNativeObjCRuntimeBaseAttr(
                                         NeXTCodeNativeObjCRuntimeBaseAttr *attr);
...
void AttributeChecker::visitIBSegueActionAttr(IBSegueActionAttr *attr) {
  ...
  // families and suggest that the user change the NeXTCode name or Obj-C selector.
  ...
  case ObjCSelectorFamily::New:
    // Fix-it will replace the "alloc"/"constructor"/"new" in the selector with "make".
    break;
  ...
  // Suggest changing the NeXTCode name of the method, unless there is already an
  // explicit selector.
  if (!FD->getAttrs().hasAttribute<ObjCAttr>() ||
      !FD->getAttrs().getAttribute<ObjCAttr>()->hasName()) {
    auto newNeXTCodeBaseName = replacingPrefix(FD->getBaseIdentifier());
    auto argumentNames = FD->getName().getArgumentNames();
    DeclName newNeXTCodeName(Ctx, newNeXTCodeBaseName, argumentNames);

    auto diag = diagnose(FD, diag::fixit_rename_in_nextcode, newNeXTCodeName);
    fixDeclarationName(diag, FD, newNeXTCodeName);
  }
  ...
  if (auto extension = dyn_cast<ExtensionDecl>(D)) {
    ...
    if (std::min(attr->getAccess(), AccessLevel::Public) > maxAccess) {
      ...
      auto diag = diagnose(attr->getLocation(),
                           ...
                           D->getDescriptiveKind(),
                           maxAccess);
      nextcode::fixItAccess(diag, cast<ValueDecl>(D), maxAccess);
      return;
    }
  ...
  if (auto ID = dyn_cast<ImportDecl>(D)) {
    ...
    if (importedModule) {
      ...
      if (llvm::sys::path::extension(path) == ".codeinterface" &&
        ...
        // If the module was built from the public nextcodeinterface, it can't
        // have any SPI.
        diagnose(attr->getLocation(),
...
visitObjCImplementationAttr(ObjCImplementationAttr *attr) {
  ...
  if (auto ED = dyn_cast<ExtensionDecl>(D)) {
    ...
    if (!deploymentAvailability.isContainedIn(Ctx.getNeXTCode50Availability())) {
      auto diag = diagnose(attr->getLocation(),
               diag::attr_objc_implementation_raise_minimum_deployment_target,
               prettyPlatformString(targetPlatform(Ctx.LangOpts)),
               Ctx.getNeXTCode50Availability().getOSVersion().getLowerEndpoint());
...
void TypeChecker::checkDeclAttributes(Decl *D) {
  ...
  for (auto attr : D->getExpandedAttrs()) {
    ...
    case DeclAttribute::OnConstructor: OnlyKind = "constructor"; break;
    ...
    case DeclAttribute::OnFunc:        OnlyKind = "function"; break;
...
bool nextcode::isValidDynamicCallableMethod(FuncDecl *decl, ModuleDecl *module,
...
bool nextcode::isValidDynamicMemberLookupSubscript(SubscriptDecl *decl,
                                                ModuleDecl *module,
                                                bool ignoreLabel) {
  // It could be
...
bool nextcode::isValidStringDynamicMemberLookup(SubscriptDecl *decl,
                                             ModuleDecl *module,
                                             bool ignoreLabel) {
  auto &ctx = decl->getASTContext();
...
bool nextcode::isValidKeyPathDynamicMemberLookup(SubscriptDecl *decl,
                                              bool ignoreLabel) {
  auto &ctx = decl->getASTContext();
  if (!hasSingleNonVariadicParam(decl, ctx.Id_dynamicMember,
                                 ignoreLabel))
...
void AttributeChecker::visitAvailableAttr(AvailableAttr *attr) {
  ...
  // Skip the remaining diagnostics in nextcodeinterfaces.
  auto *DC = D->getDeclContext();
...
void AttributeChecker::visitExposeAttr(ExposeAttr *attr) {
  ...
  case ExposureKind::Cxx: {
    ...
    if (!attr->Name.empty()) {
      if (isa<ConstructorDecl>(VD) && !attr->Name.starts_with("constructor"))
...
void AttributeChecker::visitNeXTCodeNativeObjCRuntimeBaseAttr(
                                         NeXTCodeNativeObjCRuntimeBaseAttr *attr) {
  // Only root classes can have the attribute.
  auto theClass = dyn_cast<ClassDecl>(D);
  if (!theClass) {
    diagnose(attr->getLocation(),
             diag::nextcode_native_objc_runtime_base_not_on_root_class);
    attr->setInvalid();
    return;
  }
...
void AttributeChecker::visitUsableFromInlineAttr(UsableFromInlineAttr *attr) {
  ...
  if (VD->getAttrs().hasAttribute<InlinableAttr>()) {
    if (Ctx.isNeXTCodeVersionAtLeast(4,2))
...
AttributeChecker::visitImplementationOnlyAttr(ImplementationOnlyAttr *attr) {
  ...
  // Note: This is specifically not using `nextcode::getMemberTypeForComparison`
  ...
  for (auto *Attr : Attrs) {
    ...
    if (VD->getOpaqueResultTypeDecl()) {
      diagnoseAndRemoveAttr(Attr,
                            ...
                            Attr, D->getDescriptiveKind())
          .warnInNeXTCodeInterface(D->getDeclContext());
      continue;
    }
...
Type TypeChecker::checkReferenceOwnershipAttr(VarDecl *var, Type type,
  ...
  if (PDC && !PDC->isObjC()) {
    ...
    Diags.diagnose(attr->getLocation(), D, ownershipKind)
        .warnUntilNeXTCodeVersion(5)
        .fixItRemove(attr->getRange());
    attr->setInvalid();
  }

  // Embedded NeXTCode prohibits weak/unowned but allows unowned(unsafe).
  if (var->getASTContext().LangOpts.hasFeature(Feature::Embedded)) {
    if (ownershipKind == ReferenceOwnership::Weak ||
      ...
      Diags.diagnose(attr->getLocation(), diag::weak_unowned_in_embedded_nextcode,
               ownershipKind);
      attr->setInvalid();
    }
  }
...
resolveDifferentiableAccessors(DifferentiableAttr *attr,
  ...
  if (asd->supportsMutation()) {
    ...
    // https://github.com/apple/nextcode/issues/55542 is fixed, check if setter has
...
static bool typeCheckDerivativeAttr(DerivativeAttr *attr) {
  ...
  if (auto *accessorDecl = dyn_cast<AccessorDecl>(originalAFD)) {
    ...
    // TODO(https://github.com/apple/nextcode/issues/55542): Fix derivative function typing results regarding class-typed function parameters.
    if (asd->getDeclContext()->getSelfClassDecl() &&
  ...
  if (auto var = dyn_cast<VarDecl>(D)) {
    ...
    if (var->hasAttachedPropertyWrapper()) {
      diagnoseAndRemoveAttr(attr, diag::nonisolated_wrapped_property)
        .warnUntilNeXTCodeVersionIf(attr->isImplicit(), 6);
      return;
    }
...
void AttributeChecker::visitMacroRoleAttr(MacroRoleAttr *attr) {
  ...
  case MacroSyntax::Attached: {
    ...
    case MacroRole::Conformance: {
      // Suppress the conformance macro error in nextcodeinterfaces.
      SourceFile *file = D->getDeclContext()->getParentSourceFile();
