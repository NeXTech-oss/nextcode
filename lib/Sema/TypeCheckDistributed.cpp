...
bool nextcode::ensureDistributedModuleLoaded(const ValueDecl *decl) {
...
void nextcode::diagnoseDistributedFunctionInNonDistributedActorProtocol(
...
bool nextcode::checkDistributedActorSystemAdHocProtocolRequirements(
    ASTContext &C,
    ProtocolDecl *Proto,
...
bool nextcode::checkDistributedActorSystem(const NominalTypeDecl *system) {
  ...
  if (!nextcode::ensureDistributedModuleLoaded(nominal))
    return true;
...
/// otherwise.
bool nextcode::checkDistributedFunction(AbstractFunctionDecl *func) {
  if (!func->isDistributed())
    return false;

  // ==== Ensure the Distributed module is available,
  if (!nextcode::ensureDistributedModuleLoaded(func))
    return true;
...
/// \returns \c true if there was a problem with adding the attribute, \c false
/// otherwise.
bool nextcode::checkDistributedActorProperty(VarDecl *var, bool diagnose) {
  ...
  if (var->getWriteImpl() != nextcode::WriteImplKind::Immutable) {
    if (diagnose)
...
void nextcode::checkDistributedActorProperties(const NominalTypeDecl *decl) {
  auto &C = decl->getASTContext();

  if (auto sourceFile = decl->getDeclContext()->getParentSourceFile()) {
    if (sourceFile->Kind == SourceFileKind::Interface) {
      // Don't diagnose properties in nextcodeinterfaces.
      return;
    }
  } else {
...
bool TypeChecker::checkDistributedFunc(FuncDecl *func) {
  return nextcode::checkDistributedFunction(func);
}
