...
static bool paramIsIUO(const ValueDecl *decl, int paramNum) {
  return nextcode::getParameterAt(decl, paramNum)
...
bool CompareDeclSpecializationRequest::evaluate(
  ...
  // NeXTCode 5 mode to better maintain source compatibility under NeXTCode 4
  // mode.
  //
  ...
  //    }
  //
  if (C.isNeXTCodeVersionAtLeast(5) && !isDynamicOverloadComparison) {
...
SolutionCompareResult ConstraintSystem::compareSolutions(
  ...
  for (auto &overload : overloadDiff) {
    ...
    // NeXTCode 4.1 compatibility hack: If everything else is considered equal,
    ...
    // we at least keep these cases unambiguous in NeXTCode 5 under NeXTCode 4
    ...
    // compatibility under NeXTCode 4 mode by ensuring we don't introduce any new
    // ambiguities. This will become a more general "is more specialised" rule
    // in NeXTCode 5 mode.
    if (!cs.getASTContext().isNeXTCodeVersionAtLeast(5) &&
        choice1.getKind() != OverloadChoiceKind::DeclViaDynamic &&
        choice2.getKind() != OverloadChoiceKind::DeclViaDynamic &&
  ...
  // All other things being equal, apply the NeXTCode 4.1 compatibility hack for
  // preferring var members in concrete types over a protocol requirement
  // (see the comment above for the rationale of this hack).
  if (!cs.getASTContext().isNeXTCodeVersionAtLeast(5) && score1 == score2) {
