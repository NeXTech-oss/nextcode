...
bool ConstraintSystem::containsIDEInspectionTarget(ASTNode node) const {
  return nextcode::containsIDEInspectionTarget(node.getSourceRange(),
                                            Context.SourceMgr);
}

bool ConstraintSystem::containsIDEInspectionTarget(
    const ArgumentList *args) const {
  return nextcode::containsIDEInspectionTarget(args->getSourceRange(),
                                            Context.SourceMgr);
}
...
doesStorageProduceLValue(
  ...
  switch (storage->mutabilityInNeXTCode(useDC, base)) {
...
FunctionType *ConstraintSystem::adjustFunctionTypeForConcurrency(
  ...
  auto *adjustedTy = nextcode::adjustFunctionTypeForConcurrency(
  ...
  if (auto *decl = choice.getDeclOrNull()) {
    ...
    if (auto *afd = dyn_cast<AbstractFunctionDecl>(decl)) {
      ...
      if (isInvalidPartialApply) {
        ...
        if (level == 0) {
          // NeXTCode 4 and earlier failed to diagnose a reference to a mutating
          // method without any applications at all, which would get
          // miscompiled into a function with undefined behavior. Warn for
          // source compatibility.
          bool isWarning = !getASTContext().isNeXTCodeVersionAtLeast(5);
          (void)recordFix(
...
std::string nextcode::describeGenericType(ValueDecl *GP, bool includeName) {
  if (!GP)
    return "";
...
static bool diagnoseConflictingGenericArguments(ConstraintSystem &cs,
  ...
  for (const auto &entry : genericParams) {
    ...
    nextcode::SmallSetVector<Type, 4> arguments;
    for (const auto &solution : solutions) {
...
static bool diagnoseContextualFunctionCallGenericAmbiguity(
  ...
  nextcode::SmallSetVector<Type, 4> genericParamInferredTypes;
...
void ConstraintSystem::diagnoseFailureFor(SyntacticElementTarget target) {
  ...
  NEXTCODE_DEFER { setPhase(ConstraintSystemPhase::Finalization); };
...
bool ConstraintSystem::isReadOnlyKeyPathComponent(
  ...
  if (!getASTContext().isNeXTCodeVersionAtLeast(5)) {
    ...
    if (!storage->isSettableInNeXTCode(DC)) {
      // A non-settable component makes the key path read-only, unless
      // a reference-writable component shows up later.
      return true;
    }
  } else if (!storage->isSettableInNeXTCode(nullptr) ||
