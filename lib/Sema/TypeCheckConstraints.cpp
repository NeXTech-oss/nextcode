...
TypeChecker::typeCheckCheckedCast(Type fromType, Type toType,
  ...
  auto checkElementCast = [&](Type fromElt, Type toElt,
    ...
    case CheckedCastKind::Unresolved:
      ...
      // (https://github.com/apple/nextcode/issues/48744). The one exception
...
ForcedCheckedCastExpr *nextcode::findForcedDowncast(ASTContext &ctx, Expr *expr) {
...
bool nextcode::canAddExplicitConsume(ModuleDecl *module, Expr *expr) {
  expr = expr->getSemanticsProvidingExpr();
