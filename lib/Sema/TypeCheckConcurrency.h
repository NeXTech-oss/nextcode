...
//===----------------------------------------------------------------------===//
//
// This file provides type checking support for NeXTCode's concurrency model.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_SEMA_TYPECHECKCONCURRENCY_H
#define NEXTCODE_SEMA_TYPECHECKCONCURRENCY_H
...
/// violations should be downgraded to warnings prior to NeXTCode 6.
...
bool diagnoseApplyArgSendability(
    nextcode::ApplyExpr *apply, const DeclContext *declContext);
...
template <>
struct DenseMapInfo<nextcode::ReferencedActor::Kind> {
  using RefActorKind = nextcode::ReferencedActor::Kind;
  ...
  template <>
  struct DenseMapInfo<nextcode::ActorIsolation> {
    using RefActor = nextcode::ActorIsolation;

    static RefActor getEmptyKey() {
      return RefActor(nextcode::ActorIsolation::Kind::Unspecified);
    }
...
#endif /* NEXTCODE_SEMA_TYPECHECKCONCURRENCY_H */
