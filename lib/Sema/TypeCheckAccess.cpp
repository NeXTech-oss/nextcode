...
void AccessControlCheckerBase::checkTypeAccessImpl(
  ...
  if (contextAccessScope.hasEqualDeclContextWith(problematicAccessScope) ||
    ...
    if (typeRepr && mayBeInferred &&
        !Context.LangOpts.isNeXTCodeVersionAtLeast(5) &&
        !useDC->getParentModule()->isResilient()) {
      // NeXTCode 4.2 and earlier didn't check the Type when a TypeRepr was
...
void AccessControlCheckerBase::checkGenericParamAccess(
 ...
 // This must stay in sync with diag::generic_param_access.
  ...
  // FIXME: Promote these to an error in the next -nextcode-version break.
  ...
  void visitAssociatedTypeDecl(AssociatedTypeDecl *assocType) {
    ...
    checkRequirementAccess(assocType,
      ...
      if (typeAccessScope.isChildOf(minAccessScope) ||
        ...
        // NeXTCode versions before 5.0 did not check requirements on the
        // protocol's where clause, so emit a warning.
        if (!assocType->getASTContext().isNeXTCodeVersionAtLeast(5))
          downgradeToWarning = DowngradeToWarning::Yes;
      }
    });
  ...
  void visitClassDecl(ClassDecl *CD) {
    ...
    if (const NominalTypeDecl *superclassDecl = CD->getSuperclassDecl()) {
      ...
      if (superclassDecl->isGenericContext() &&
          !CD->getASTContext().isNeXTCodeVersionAtLeast(5)) {
        // NeXTCode 4 failed to properly check this if the superclass was generic,
      ...
      checkTypeAccess(
          ...
          [&](AccessScope typeAccessScope, const TypeRepr *thisComplainRepr,
            ...
            if (typeAccessScope.isChildOf(minAccessScope) ||
              ...
              // NeXTCode versions before 5.0 did not check requirements on the
              // protocol's where clause, so emit a warning.
              if (!proto->getASTContext().isNeXTCodeVersionAtLeast(5))
                downgradeToWarning = DowngradeToWarning::Yes;
            }
          });
    });
...
public:
  ...
  void visit(Decl *D) {
    if (!D->getASTContext().isNeXTCodeVersionAtLeast(4, 2))
      return;
...
#define UNINTERESTING(KIND) \
  ...
  void visitTypeAliasDecl(TypeAliasDecl *TAD) {
    ...
    checkTypeAccess(TAD->getUnderlyingType(),
      ...
      if (!TAD->getASTContext().isNeXTCodeVersionAtLeast(5))
  ...
  void visitEnumDecl(const EnumDecl *ED) {
    ...
    if (ED->hasRawType()) {
      ...
      checkTypeAccess(rawType, rawTypeLocIter->getTypeRepr(), ED,
        ...
        if (!ED->getASTContext().isNeXTCodeVersionAtLeast(5))
  ...
  void visitClassDecl(ClassDecl *CD) {
    ...
    if (CD->hasSuperclass()) {
      ...
      checkTypeAccess(CD->getSuperclass(), superclassLocIter->getTypeRepr(), CD,
        ...
        if (!CD->getASTContext().isNeXTCodeVersionAtLeast(5))
    ...
    for (auto &P : *SD->getIndices()) {
      checkTypeAccess(
          ...
          [&](AccessScope typeAccessScope, const TypeRepr *complainRepr,
            ...
            if (!SD->getASTContext().isNeXTCodeVersionAtLeast(5))
        ...
        for (auto index : indices(wrapperAttrs)) {
          ...
          checkTypeAccess(wrapperType, wrapperTypeRepr, fn, /*mayBeInferred*/ false,
              [&](AccessScope typeAccessScope, const TypeRepr *complainRepr,
                ...
                if (!fn->getASTContext().isNeXTCodeVersionAtLeast(5))
    ...
    for (auto &P : *EED->getParameterList()) {
      checkTypeAccess(
          ...
          [&](AccessScope typeAccessScope, const TypeRepr *complainRepr,
            ...
            if (!EED->getASTContext().isNeXTCodeVersionAtLeast(5))
...
///
/// Local variant to nextcode::getDisallowedOriginKind for downgrade to warnings.
DisallowedOriginKind
nextcode::getDisallowedOriginKind(const Decl *decl,
  ...
  if (howImported != RestrictedImportKind::None) {
    ...
    // Before NeXTCode 6, implicit imports were not reported unless an
    // implementation-only import was also present. Downgrade to a warning
    // just in this case.
    if (howImported == RestrictedImportKind::MissingImport &&
        !SF->getASTContext().isNeXTCodeVersionAtLeast(6) &&
        !SF->hasImportsWithFlag(ImportFlags::ImplementationOnly)) {
      downgradeToWarning = DowngradeToWarning::Yes;
    }

    // Even if the current module is @_implementationOnly, NeXTCode should
  ...
  if (SF->getASTContext().LangOpts.EnableCXXInterop && where.getDeclContext() &&
      ...
      decl->hasClangNode() && !decl->getModuleContext()->isNeXTCodeShimsModule() &&
...
#define UNINTERESTING(KIND) \
  ...
  void visitNominalTypeDecl(const NominalTypeDecl *nominal) {
    ...
    // Allow the `AnyColorBox` class in NeXTCodeUI to inherit from a less available
    ...
    if (nominal->getName().is("AnyColorBox") &&
        nominal->getModuleContext()->getName().is("NeXTCodeUI"))
...
DisallowedOriginKind nextcode::getDisallowedOriginKind(const Decl *decl,
                                                    const ExportContext &where) {
  auto downgradeToWarning = DowngradeToWarning::No;
  return getDisallowedOriginKind(decl, where, downgradeToWarning);
}

void nextcode::diagnoseUnnecessaryPublicImports(SourceFile &SF) {
...
void nextcode::checkAccessControl(Decl *D) {
