...
//===----------------------------------------------------------------------===//
//
//  This file implements the 'program counter simulation' for NeXTCode.
...
//  through NeXTCode code in "live coding" environments like Playgrounds without
...
public:
  ...
  BraceStmt *transformBraceStmt(BraceStmt *BS,
    ...
    SmallVector<nextcode::ASTNode, 3> Elements(OriginalElements.begin(),
    ...
    return nextcode::BraceStmt::create(Context, BS->getLBraceLoc(), Elements,
                                    BS->getRBraceLoc());
  }
...
void nextcode::performPCMacro(SourceFile &SF) {
