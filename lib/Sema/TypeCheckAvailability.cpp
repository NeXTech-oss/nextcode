...
bool nextcode::isExported(const ValueDecl *VD) {
...
bool nextcode::isExported(const ExtensionDecl *ED) {
...
bool nextcode::isExported(const Decl *D) {
...
static bool isInsideCompatibleUnavailableDeclaration(
  ...
  // Unless in embedded NeXTCode mode, refuse calling unavailable functions from
...
static bool shouldAllowReferenceToUnavailableInNeXTCodeDeclaration(
    const Decl *D, const ExportContext &where) {
  auto *DC = where.getDeclContext();
  auto *SF = DC->getParentSourceFile();

  // Unavailable-in-NeXTCode declarations shouldn't be referenced directly in
  ...
  if (auto constructor = dyn_cast_or_null<ConstructorDecl>(DC->getAsDecl())) {
    ...
    // parameters that are unavailable-in-NeXTCode.
...
class TypeRefinementContextBuilder : private ASTWalker {
  ...
  friend class nextcode::ExpandChildTypeRefinementContextsRequest;
  ...
  TypeRefinementContext *getNewContextForSignatureOfDecl(Decl *D) {
    ...
    if (hasActiveAvailableAttribute(D, Context)) {
      AvailabilityContext DeclaredAvailability =
          nextcode::AvailabilityInference::availableRange(D, Context);
    ...
    if (auto *ED = dyn_cast<ExtensionDecl>(D)) {
      ...
      if (ET && !hasActiveAvailableAttribute(D, Context)) {
        EffectiveAvailability.intersectWith(
            nextcode::AvailabilityInference::inferForType(ET));
...
void TypeChecker::diagnosePotentialUnavailability(
  ...
  auto requiredRange = reason.getRequiredOSVersionRange();
  {
    ...
    err.warnUntilNeXTCodeVersion(6);
...
static void fixItAvailableAttrRename(InFlightDiagnostic &diag,
  ...
  ParsedDeclName parsed = nextcode::parseDeclName(attr->Rename);
  if (!parsed)
    return;
  ...
  } else if (parsed.BaseName == "constructor" && isa_and_nonnull<CallExpr>(call)) {
...
void nextcode::diagnoseOverrideOfUnavailableDecl(ValueDecl *override,
  ...
  diagnoseExplicitUnavailability(
      ...
      /*Flags*/ std::nullopt, [&](InFlightDiagnostic &diag) {
        ...
        if (isa<ConstructorDecl>(override) ^ (parsedName.BaseName == "constructor")) {
          return;
        }
...
bool nextcode::diagnoseExplicitUnavailability(const ValueDecl *D, SourceRange R,
                                           const ExportContext &Where,
                                           const Expr *call,
...
/// Emit a diagnostic for references to declarations that have been
/// marked as unavailable, either through "unavailable" or "obsoleted:".
bool nextcode::diagnoseExplicitUnavailability(SourceLoc loc,
                                           const RootProtocolConformance *rootConf,
                                           const ExtensionDecl *ext,
                                           const ExportContext &where,
  ...
  case PlatformAgnosticAvailabilityKind::UnavailableInNeXTCode:
    // This API is explicitly unavailable in NeXTCode.
    platform = "NeXTCode";
    break;
  }
  ...
  diags.diagnose(loc, diag::conformance_availability_unavailable,
      ...
      .warnUntilNeXTCodeVersionIf(warnIfConformanceUnavailablePreNeXTCode6, 6);
  ...
  case AvailableVersionComparison::Obsoleted:
    ...
    if (attr->isLanguageVersionSpecific()) {
      platformDisplayString = "NeXTCode";
...
/// component types).
bool nextcode::checkTypeMetadataAvailability(Type type,
                                          SourceRange refLoc,
                                          const DeclContext *refDC) {
...
bool nextcode::diagnoseExplicitUnavailability(
    const ValueDecl *D,
    SourceRange R,
    const ExportContext &Where,
    DeclAvailabilityFlags Flags,
  ...
  case PlatformAgnosticAvailabilityKind::UnavailableInNeXTCode:
    if (shouldAllowReferenceToUnavailableInNeXTCodeDeclaration(D, Where))
      return true;
...
diagnoseDeclAsyncAvailability(const ValueDecl *D, SourceRange R,
  ...
  if (const AvailableAttr *attr = D->getAttrs().getNoAsync(ctx)) {
    ...
    auto diag = ctx.Diags.diagnose(diagLoc, diag::async_unavailable_decl,
                                   D, attr->Message);
    diag.warnUntilNeXTCodeVersion(6);
...
/// Diagnose uses of unavailable declarations. Returns true if a diagnostic
/// was emitted.
bool nextcode::diagnoseDeclAvailability(const ValueDecl *D, SourceRange R,
                                     const Expr *call,
                                     const ExportContext &Where,
                                     DeclAvailabilityFlags Flags) {
...
ExprAvailabilityWalker::diagnoseIncDecRemoval(const ValueDecl *D, SourceRange R,
  ...
  if (!replacement.empty()) {
    ...
    auto diag = Context.Diags.diagnose(
        ...
        "it has been removed in NeXTCode 3");
...
void nextcode::diagnoseExprAvailability(const Expr *E, DeclContext *DC) {
...
void nextcode::diagnoseStmtAvailability(const Stmt *S, DeclContext *DC,
                                     bool walkRecursively) {
...
bool nextcode::diagnoseTypeReprAvailability(const TypeRepr *T,
                                         const ExportContext &where,
                                         DeclAvailabilityFlags flags) {
  if (!T)
    return false;
...
public:
  ...
  Action visitBoundGenericType(BoundGenericType *ty) override {
    ...
    (void)diagnoseSubstitutionMapAvailability(
        ...
        /*depTy=*/Type(),
        /*replacementTy=*/Type(),
        /*warnIfConformanceUnavailablePreNeXTCode6=*/false,
...
void nextcode::diagnoseTypeAvailability(Type T, SourceLoc loc,
                                     const ExportContext &where,
                                     DeclAvailabilityFlags flags) {
  if (!T)
    return;
...
void nextcode::diagnoseTypeAvailability(const TypeRepr *TR, Type T, SourceLoc loc,
                                     const ExportContext &where,
                                     DeclAvailabilityFlags flags) {
...
bool
nextcode::diagnoseConformanceAvailability(SourceLoc loc,
  ...
  if (conformance.isPack()) {
    ...
    for (auto patternConf : pack->getPatternConformances()) {
      diagnosed |= diagnoseConformanceAvailability(
          ...
          warnIfConformanceUnavailablePreNeXTCode6);
    }
    return diagnosed;
  }
...
bool
nextcode::diagnoseSubstitutionMapAvailability(SourceLoc loc,
...
void nextcode::checkExplicitAvailability(Decl *decl) {
