...
static void forwardParameters(AbstractFunctionDecl *afd,
  ...
  for (auto param : *afd->getParameters()) {
    forwardingParams.push_back(new (C) DeclRefExpr(
        ...
        nextcode::AccessSemantics::Ordinary,
...
static FuncDecl *createSameSignatureDistributedThunkDecl(DeclContext *DC,
  ...
  if (auto accessor = dyn_cast<AccessorDecl>(func)) {
    ...
    thunk = FuncDecl::createImplicit(
        C, nextcode::StaticSpellingKind::None,
        thunkName, SourceLoc(),
        /*async=*/true, /*throws=*/true, // since it's a distributed thunk
        /*thrownType=*/Type(),
        genericParamList,
...
addDistributedActorCodableConformance(
  ...
  assert(proto->isSpecificProtocol(nextcode::KnownProtocolKind::Decodable) ||
         proto->isSpecificProtocol(nextcode::KnownProtocolKind::Encodable));
...
void nextcode::assertRequiredSynthesizedPropertyOrder(ASTContext &Context,
...
FuncDecl *GetDistributedThunkRequest::evaluate(Evaluator &evaluator,
  ...
  if (nextcode::checkDistributedFunction(distributedTarget)) {
    return nullptr;
  }
...
GetDistributedActorAsActorConformanceRequest::evaluate(
  ...
  auto nextcodeModule = ctx.getStdlibModule();

  auto actorProto = ctx.getProtocol(KnownProtocolKind::Actor);

  auto ext = findDistributedActorAsActorExtension(
      distributedActorProto, nextcodeModule);
  if (!ext)
    return nullptr;
