...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_SEMA_CSDIAGNOSTICS_H
#define NEXTCODE_SEMA_CSDIAGNOSTICS_H
...
#include "nextcode/Sema/FixBehavior.h"
...
private:
  ...
  /// For example, if NeXTCode cannot find "vector::data" because it is unsafe, try
...
/// ```
///
/// Here `max` refers to a global function `max<T>(_: T, _: T)` in `NeXTCode`
/// module and can only be accessed by adding `NeXTCode.` to it, because `Sequence`
...
/// Diagnose situations where NeXTCode -> C pointer implicit conversion
/// is attempted on a NeXTCode function instead of one imported from C header.
///
/// \code
...
/// }
/// \endcode
class NeXTCodeToCPointerConversionInInvalidContext final
    : public FailureDiagnostic {
public:
  NeXTCodeToCPointerConversionInInvalidContext(const Solution &solution,
                                            ConstraintLocator *locator)
      : FailureDiagnostic(solution, locator) {}
...
#endif // NEXTCODE_SEMA_CSDIAGNOSTICS_H
