...
Expr *nextcode::buildSelfReference(VarDecl *selfDecl,
...
Argument nextcode::buildSelfArgument(VarDecl *selfDecl,
                                  SelfAccessorKind selfAccessorKind,
                                  bool isMutable) {
  auto &ctx = selfDecl->getASTContext();
...
/// list.
ArgumentList *nextcode::buildForwardingArgumentList(ArrayRef<ParamDecl *> params,
                                                 ASTContext &ctx) {
...
static ConstructorDecl *createImplicitConstructor(NominalTypeDecl *decl,
  ...
  if (ICK == ImplicitConstructorKind::Memberwise) {
    ...
    if (nextcode::ensureDistributedModuleLoaded(decl)) {
...
synthesizeStubBody(AbstractFunctionDecl *fn, void *) {
  ...
  // Create a call to NeXTCode._unimplementedInitializer
...
static void collectNonOveriddenSuperclassInits(
  ...
  for (auto decl : lookupResults) {
    ...
    // implementation decl because it's in the NeXTCode module. Go find the
...
static void addImplicitInheritedConstructorsToClass(ClassDecl *decl) {
  ...
  // but downgrade them to warnings prior to NeXTCode 6.
  ...
  for (auto *superclassCtor : nonOverriddenSuperclassCtors) {
    ...
    for (auto *member : results) {
      ...
      auto type = nextcode::getMemberTypeForComparison(ctor, nullptr);
...
ValueDecl *nextcode::getProtocolRequirement(ProtocolDecl *protocol,
                                         Identifier name) {
...
bool nextcode::hasLetStoredPropertyWithInitialValue(NominalTypeDecl *nominal) {
  return llvm::any_of(nominal->getStoredProperties(), [&](VarDecl *v) {
    return v->isLet() && v->hasInitialValue();
  });
}

bool nextcode::addNonIsolatedToSynthesized(NominalTypeDecl *nominal,
                                        ValueDecl *value) {
...
void nextcode::applyInferredSPIAccessControlAttr(Decl *decl,
