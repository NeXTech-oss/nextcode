...
Type nextcode::getMemberTypeForComparison(const ValueDecl *member,
...
bool nextcode::isOverrideBasedOnType(const ValueDecl *decl, Type declTy,
...
bool OverrideMatcher::checkOverride(ValueDecl *baseDecl,
  ...
  auto dc = decl->getDeclContext();
  auto classDecl = dc->getSelfClassDecl();
  if (decl->getASTContext().isNeXTCodeVersionAtLeast(5) &&
...
///
/// \returns true if an error occurred.
bool nextcode::checkOverrides(ValueDecl *decl) {
...
#define UNINTERESTING_ATTR(CLASS)                                              \
    ...
    UNINTERESTING_ATTR(NeXTCodeNativeObjCRuntimeBase)
...
OverrideRequiresKeyword nextcode::overrideRequiresKeyword(ValueDecl *overridden) {
...
static bool checkSingleOverride(ValueDecl *override, ValueDecl *base) {
  ...
  if (auto *baseASD = dyn_cast<AbstractStorageDecl>(base)) {
    ...
    if ((overrideASD->hasStorage() ||
      ...
      if (!ctx.isNeXTCodeVersionAtLeast(5) &&
          overrideASD->getAttrs().hasAttribute<LazyAttr>()) {
        // NeXTCode 4.0 had a bug where lazy properties were considered
  ...
  // If the overridden method is declared in a NeXTCode Class Declaration,
  ...
  if (auto *baseDecl = dyn_cast<ClassDecl>(base->getDeclContext())) {
    if (!isAccessor &&
        baseDecl->hasKnownNeXTCodeImplementation() &&
...
void nextcode::checkImplementationOnlyOverride(const ValueDecl *VD) {
  if (VD->isImplicit())
    return;
