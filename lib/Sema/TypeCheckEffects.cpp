...
bool nextcode::isRethrowLikeTypedThrows(AbstractFunctionDecl *func) {
  // This notion is only for compatibility in NeXTCode 5 and is disabled
...
public:
  ...
  void diagnoseUncoveredThrowSite(ASTContext &ctx, ASTNode E,
    ...
    Diags.diagnose(loc, message).highlight(highlight)
      .warnUntilNeXTCodeVersionIf(classification.shouldDowngradeToWarning(), 6);
...
nextcode::compareThrownErrorsForSubtyping(
