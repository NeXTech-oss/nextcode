...
//===----------------------------------------------------------------------===//
//
// This file implements the nextcode::performTypeChecking entry point for
// semantic analysis.
//
//===----------------------------------------------------------------------===//
...
void nextcode::bindExtensions(ModuleDecl &mod) {
...
void nextcode::performTypeChecking(SourceFile &SF) {
...
void nextcode::performWholeModuleTypeChecking(SourceFile &SF) {
  auto &Ctx = SF.getASTContext();
  FrontendStatsTracer tracer(Ctx.Stats,
...
void nextcode::loadDerivativeConfigurations(SourceFile &SF) {
...
bool nextcode::isAdditiveArithmeticConformanceDerivationEnabled(SourceFile &SF) {
  auto &ctx = SF.getASTContext();
...
Type nextcode::performTypeResolution(TypeRepr *TyR, ASTContext &Ctx,
...
GenericSignature
nextcode::handleSILGenericParams(GenericParamList *genericParams,
...
void nextcode::typeCheckPatternBinding(PatternBindingDecl *PBD,
...
bool nextcode::typeCheckASTNodeAtLoc(TypeCheckASTNodeAtLocContext TypeCheckCtx,
                                  SourceLoc TargetLoc) {
...
bool nextcode::typeCheckForCodeCompletion(
...
Expr *nextcode::resolveDeclRefExpr(UnresolvedDeclRefExpr *UDRE, DeclContext *Context,
  ...
  case AnyFunctionType::Representation::CFunctionPointer:
    if (!fnTy->isRepresentableIn(ForeignLanguage::ObjectiveC, dc)) {
      StringRef strName =
        ...
        : "c";
      auto extInfo2 =
        extInfo.withRepresentation(AnyFunctionType::Representation::NeXTCode);
