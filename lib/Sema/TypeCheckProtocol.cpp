...
#include "nextcode/AST/TypeMatcher.h"
...
RequirementMatch nextcode::matchWitness(
  ...
  if (decomposeFunctionType) {
    ...
    if (!ignoreReturnType) {
      ...
      if (dc->getASTContext().LangOpts.isNeXTCodeVersionAtLeast(6)) {
    ...
    for (unsigned i = 0, n = reqParams.size(); i != n; ++i) {
      ...
      if (dc->getASTContext().isNeXTCodeVersionAtLeast(6)) {
...
RequirementMatch
nextcode::matchWitness(WitnessChecker::RequirementEnvironmentCache &reqEnvCache,
  ...
  auto finalize = [&](bool anyRenaming, 
    ...
    bool requiresNonSendable = [&]() {
      ...
      // This is not an error until NeXTCode 6 because `nextcode_attr` wasn't
...
bool
nextcode::witnessHasImplementsAttrForRequiredName(ValueDecl *witness,
...
bool
nextcode::witnessHasImplementsAttrForExactRequirement(ValueDecl *witness,
                                                   ValueDecl *requirement) {
...
nextcode::lookupValueWitnesses(DeclContext *DC, ValueDecl *req, bool *ignoringNames) {
  ...
  if (doUnqualifiedLookup) {
    ...
    for (auto candidate : lookup) {
      ...
      if (!isa<ProtocolDecl>(decl->getDeclContext()) &&
          nextcode::isMemberOperator(cast<FuncDecl>(decl), DC->getSelfInterfaceType())) {
        witnesses.push_back(decl);
      }
    }
  }
...
static void diagnoseConformanceImpliedByConditionalConformance(
  ...
  // (especially conditional ones) is good NeXTCode style, and so we don't
...
checkIndividualConformance(NormalProtocolConformance *conformance) {
  ...
  if (Proto->isObjC()) {
    ...
    // @objc protocols in extensions of NeXTCode generic classes, because there's
...
nextcode::getProtocolRequirementKind(ValueDecl *Requirement) {
  assert(Requirement->isProtocolRequirement());
...
diagnoseMatch(ModuleDecl *module, NormalProtocolConformance *conformance,
  ...
  case MatchKind::RequiresNonSendable:
    diags.diagnose(match.Witness, diag::protocol_witness_non_sendable,
                   withAssocTypes,
                   module->getASTContext().isNeXTCodeVersionAtLeast(6));
    break;
  ...
  for (const auto &Missing: MissingWitnesses) {
    ...
    if (llvm::any_of(DeclsByName[Req->getName()], [&](ValueDecl *OtherReq) {
          ...
          return conflicting(Req->getASTContext(), Req->getOverloadSignature(),
                             ...
                             OtherOverloadTy,
                             /*wouldConflictInNeXTCode5*/ nullptr,
...
ConformanceChecker::resolveWitnessViaLookup(ValueDecl *requirement) {
  ...
  if (findBestWitness(requirement,
    ...
    case CheckKind::AccessOfSetter: {
      // NeXTCode 4.2 relaxed some rules for protocol witness matching.
      //
      // This meant that it was possible for an optional protocol requirement
      // to have a witness where previously in NeXTCode 4.1 it did not.
      //
...
void ConformanceChecker::resolveValueWitnesses() {
  ...
  for (auto *requirement : Proto->getProtocolRequirements()) {
    ...
    // async-looking ObjC protocol method requirement into two NeXTCode protocol
  ...
  //
  // These protocol requirements are not expressible in NeXTCode today, but as
...
void nextcode::diagnoseConformanceFailure(Type T,
                                       ProtocolDecl *Proto,
                                       DeclContext *DC,
  ...
  // in NeXTCode. Suggest inheritance from NSObject instead.
...
static void diagnoseUnstableName(ProtocolConformance *conformance,
  ...
  if (kind && C.LangOpts.EnableNSKeyedArchiverDiagnostics &&
    ...
    // Note: this is intentionally using the NeXTCode 3 mangling,
    // to provide compatibility with archives created in the NeXTCode 3
    // time frame.
...
nextcode::findWitnessedObjCRequirements(const ValueDecl *witness,
