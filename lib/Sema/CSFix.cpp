...
getConcurrencyFixBehavior(ConstraintSystem &cs, ConstraintKind constraintKind,
  ...
  // Otherwise, warn until NeXTCode 6.
  if (!cs.getASTContext().LangOpts.isNeXTCodeVersionAtLeast(6))
...
AllowSendingMismatch *AllowSendingMismatch::create(ConstraintSystem &cs,
                                                   ...
                                                   ConstraintLocator *locator) {
  auto fixBehavior = cs.getASTContext().LangOpts.isNeXTCodeVersionAtLeast(6)
                         ? FixBehavior::Error
...
bool AllowNeXTCodeToCPointerConversion::diagnose(const Solution &solution,
                                              bool asNote) const {
  NeXTCodeToCPointerConversionInInvalidContext failure(solution, getLocator());
  return failure.diagnose(asNote);
}

AllowNeXTCodeToCPointerConversion *
AllowNeXTCodeToCPointerConversion::create(ConstraintSystem &cs,
                                       ConstraintLocator *locator) {
  return new (cs.getAllocator()) AllowNeXTCodeToCPointerConversion(cs, locator);
}
