...
static bool inferFinalAndDiagnoseIfNeeded(ValueDecl *D, ClassDecl *cls,
  ...
  if (D->getFormalAccess() == AccessLevel::Open) {
    ...
    if (!context.isNeXTCodeVersionAtLeast(5))
      diagID = diag::implicitly_final_cannot_be_open_nextcode4;
    ...
    if (auto convenAttr = decl->getAttrs().getAttribute<ConvenienceAttr>()) {
      if (auto classDecl = dyn_cast<ClassDecl>(nominal)) {
        if (classDecl->isAnyActor()) {
          ...
          // Forbid convenience inits on Foreign CF types, as NeXTCode does not yet
...
BodyInitKindRequest::evaluate(Evaluator &evaluator,
  ...
  if (Kind == BodyInitKind::None && isa<StructDecl>(NTD)) {
    ...
    if (NTD->isResilient() &&
      ...
      // Prior to NeXTCode 5, cross-module initializers were permitted to be
      ...
      if (NTD->isResilient() ||
          (ctx.isNeXTCodeVersionAtLeast(5) &&
...
NeedsNewVTableEntryRequest::evaluate(Evaluator &evaluator,
  ...
  // As above, convenience initializers are not formally overridable in NeXTCode
...
nextcode::computeAutomaticEnumValueKind(EnumDecl *ED) {
  ...
  // NeXTCode enums require that the raw type is convertible from one of the
...
EnumRawValuesRequest::evaluate(Evaluator &eval, EnumDecl *ED,
  ...
  // Avoid computing raw values for enum cases in nextcodeinterface files since raw
...
nextcode::findNonImplicitRequiredInit(const ConstructorDecl *CD) {
  while (CD->isImplicit()) {
...
void nextcode::validatePrecedenceGroup(PrecedenceGroupDecl *PGD) {
...
IsImplicitlyUnwrappedOptionalRequest::evaluate(Evaluator &evaluator,
  ...
  case DeclKind::Var:
    if (decl->hasClangNode()) {
      ...
      // is called for the first time. (See apple/nextcode#61026.)
      //
...
bool nextcode::isMemberOperator(FuncDecl *decl, Type selfTy) {
...
ResultTypeRequest::evaluate(Evaluator &evaluator, ValueDecl *decl) const {
  ...
  if (!resultTyRepr && decl->getClangDecl() &&
    ...
    if (returnType)
      return *returnType;
    // Mark the imported NeXTCode function as unavailable.
    // That will ensure that the function will not be
    // usable from NeXTCode, even though it is imported.
    if (!decl->getAttrs().isUnavailable(ctx)) {
      StringRef unavailabilityMsgRef = "return type is unavailable in NeXTCode";
      auto ua =
  ...
  if (auto ownershipRepr = dyn_cast<OwnershipTypeRepr>(nestedRepr)) {
    if (ownershipRepr->getSpecifier() == ParamSpecifier::InOut
      ...
      ctx.Diags.diagnose(param->getStructuralDefaultExpr()->getLoc(),
                         nextcode::diag::cannot_provide_default_value_inout,
                         param->getName());
      return ParamSpecifier::Default;
    }
