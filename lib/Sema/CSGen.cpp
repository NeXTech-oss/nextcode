...
#ifdef NEXTCODE_BUILD_NEXTCODE_SYNTAX
      auto &ctx = CS.getASTContext();
...
#include "nextcode/AST/MagicIdentifierKinds.def"
    ...
    Type visitUnresolvedDotExpr(UnresolvedDotExpr *expr) {
      ...
      if (TypeChecker::getSelfForInitDelegationInConstructor(CS.DC, expr)) {
        ...
        if (!CS.getASTContext().isNeXTCodeVersionAtLeast(6)) {
    ...
    Type visitOptionalTryExpr(OptionalTryExpr *expr) {
      ...
      // Prior to NeXTCode 5, 'try?' always adds an additional layer of
      // optionality, even if the sub-expression was already optional.
      //
      // NB Keep adding the additional layer in NeXTCode 5 and on if this 'try?'
    ...
    Type getTypeForPattern(
      ...
      case PatternKind::EnumElement: {
        ...
        // Tuple splat is still allowed for patterns (with a warning in NeXTCode 5)
...
/// expression that conforms to `NeXTCode.Sequence`.
...
ResolvedMemberResult nextcode::resolveValueMember(DeclContext &DC, Type BaseTy,
                                               DeclName Name) {
