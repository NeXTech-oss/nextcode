...
//===----------------------------------------------------------------------===//
//
//  This file implements the playground transform for NeXTCode.
//
//===----------------------------------------------------------------------===//
...
using namespace nextcode::instrumenter_support;
...
private:
  ...
  using ElementVector = SmallVector<nextcode::ASTNode, 3>;
...
public:
  ...
  BraceStmt *transformBraceStmt(BraceStmt *BS,
    ...
    for (size_t EI = 0; EI != Elements.size(); ++EI) {
      nextcode::ASTNode &Element = Elements[EI];
    ...
    return nextcode::BraceStmt::create(Context, BS->getLBraceLoc(),
...
void nextcode::performPlaygroundTransform(SourceFile &SF, PlaygroundOptionSet Opts) {
...
void nextcode::performPlaygroundTransform(SourceFile &SF, bool HighPerformance) {
...
#include "nextcode/Basic/PlaygroundOptions.def"
  nextcode::performPlaygroundTransform(SF, HighPerfTransformOpts);
}
