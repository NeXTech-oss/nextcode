    ...
    if (inheritedTy->isAnyObject()) {
      ...
      bool isWrittenAsClass =
          ...
          Lexer::getTokenAtLocation(ctx.SourceMgr, sourceRange.Start)
              .is(tok::kw_class);
      if (ctx.LangOpts.isNeXTCodeVersionAtLeast(5) && isWrittenAsClass) {
        diags
      ...
      if (inheritedAnyObject) {
        ...
        if (!ctx.LangOpts.isNeXTCodeVersionAtLeast(5) &&
            isa<ProtocolDecl>(decl) &&
...
CheckRedeclarationRequest::evaluate(Evaluator &eval, ValueDecl *current,
  ...
  for (auto other : otherDefinitions) {
    ...
    if (isRedeclaration || wouldBeNeXTCode5Redeclaration) {
            ...
            if (currParams.size() == otherParams.size()) {
              ...
              for (unsigned i : indices(currParams)) {
                ...
                if (currParams[i].getPlainType()->getOptionalObjectType()) {
                  optionalRedecl = true;
                  auto *param = nextcode::getParameterAt(current, i);
                  assert(param);
                ...
                if (otherParams[i].getPlainType()->getOptionalObjectType()) {
                  auto *param = nextcode::getParameterAt(other, i);
                  assert(param);
                  if (param->isImplicitlyUnwrappedOptional())
                    otherIsIUO = true;
                }
      ...
      // matching the NeXTCode 4 behaviour (i.e. just emitting the future-compat
      // warning) will result in SILGen crashes due to both properties mangling
      // the same, so it's better to just follow the NeXTCode 5 behaviour and emit
      // the actual error.
      if (wouldBeNeXTCode5Redeclaration && isa<VarDecl>(current) &&
          isa<VarDecl>(other) &&
          current->getInterfaceType()->isEqual(other->getInterfaceType())) {
        wouldBeNeXTCode5Redeclaration = false;
      }
      ...
      // If this isn't a redeclaration in the current version of NeXTCode, but
      // would be in NeXTCode 5 mode, emit a warning instead of an error.
      if (wouldBeNeXTCode5Redeclaration) {
        current->diagnose(diag::invalid_redecl_nextcode5_warning, current);
        other->diagnose(diag::invalid_redecl_prev, other);
      } else {
        const auto *otherInit = dyn_cast<ConstructorDecl>(other);
...
static bool checkExpressionMacroDefaultValueRestrictions(ParamDecl *param) {
  ...
  const SourceFile *SF = DC->getParentSourceFile();
  return nextcode_ASTGen_checkDefaultArgumentMacroExpression(
...
void nextcode::checkVariadicParameters(ParameterList *params, DeclContext *dc) {
...
InFlightDiagnostic
nextcode::softenIfAccessNote(const Decl *D, const DeclAttribute *attr,
...
void nextcode::diagnoseAttrsAddedByAccessNote(SourceFile &SF) {
...
public:
  ...
  void visitImportDecl(ImportDecl *ID) {
    ...
    if (target && // module would be nil if loading fails
        ...
        !target->isNonNeXTCodeModule() && // target is a NeXTCode module
  ...
  static bool hasHistoricallyWrongAvailability(FuncDecl *func) {
    return func->getName().isCompoundName("nextcode_deletedAsyncMethodError", { });
  }
  ...
  void visitFuncDecl(FuncDecl *FD) {
    ...
    // productize @_cdecl.
    if (auto CDeclAttr = FD->getAttrs().getAttribute<nextcode::CDeclAttr>()) {
  ...
  void visitTopLevelCodeDecl(TopLevelCodeDecl *TLCD) {
    // See nextcode::performTypeChecking for TopLevelCodeDecl handling.
