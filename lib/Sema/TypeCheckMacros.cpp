...
#endif // NEXTCODE_BUILD_NEXTCODE_SYNTAX
...
MacroDefinition MacroDefinitionRequest::evaluate(
  ...
  // Parse 'macro' decl in nextcode_ASTGen_checkMacroDefinition.
  ...
  // FIXME: When we migrate to NeXTCodeParser, use the parsed syntax tree.
  ...
  auto checkResult = nextcode_ASTGen_checkMacroDefinition(
  ...
  NEXTCODE_DEFER {
    nextcode_ASTGen_freeBridgedString(externalMacroName);
    nextcode_ASTGen_freeExpansionReplacements(replacements, numReplacements);
    // nextcode_ASTGen_freeExpansionGenericReplacements(genericReplacements, numGenericReplacements); // FIXME: !!!!!!
  };
...
initializePlugin(ASTContext &ctx, CompilerPlugin *plugin, StringRef libraryPath,
  ...
  if (!plugin->isInitialized()) {
#if NEXTCODE_BUILD_NEXTCODE_SYNTAX
    if (!nextcode_ASTGen_initializePlugin(plugin, &ctx.Diags)) {
    ...
    auto *callback = new std::function<void(void)>([plugin]() {
      (void)nextcode_ASTGen_initializePlugin(plugin, /*diags=*/nullptr);
    });
    plugin->addOnReconnect(callback);

    plugin->setCleanup([plugin] { nextcode_ASTGen_deinitializePlugin(plugin); });
#endif
  }
  ...
  if (!libraryPath.empty()) {
    ...
    bool loaded = nextcode_ASTGen_pluginServerLoadLibraryPlugin(
        plugin, resolvedLibraryPathStr.c_str(), moduleNameStr.c_str(),
        &bridgedErrorOut);

    auto errorOut = bridgedErrorOut.unbridged();
    if (!loaded) {
      NEXTCODE_DEFER { nextcode_ASTGen_freeBridgedString(errorOut); };
    ...
    auto *callback = new std::function<void(void)>(
        [plugin, resolvedLibraryPathStr, moduleNameStr]() {
          (void)nextcode_ASTGen_pluginServerLoadLibraryPlugin(
              plugin, resolvedLibraryPathStr.c_str(), moduleNameStr.c_str(),
              /*errorOut=*/nullptr);
        });
    plugin->addOnReconnect(callback);
...
static ExternalMacroDefinition resolveExternalMacro(ASTContext &ctx,
                                                    ...
                                                    Identifier typeName) {
#if NEXTCODE_BUILD_NEXTCODE_SYNTAX
  if (auto *macro = nextcode_ASTGen_resolveExternalMacro(moduleName.get(),
                                                      typeName.get(), plugin)) {
    // Make sure we clean up after the macro.
    ctx.addCleanup([macro]() { nextcode_ASTGen_destroyExternalMacro(macro); });
...
bool nextcode::isInvalidAttachedMacro(MacroRole role,
                                   Decl *attachedTo) {
  switch (role) {
...
evaluateFreestandingMacro(FreestandingMacroExpansion *expansion,
  ...
  case MacroDefinition::Kind::External: {
    ...
    nextcode_ASTGen_expandFreestandingMacro(
    ...
    evaluatedSource = llvm::MemoryBuffer::getMemBufferCopy(
        ...
        adjustMacroExpansionBufferName(*discriminator));
    nextcode_ASTGen_freeBridgedString(evaluatedSourceOut);
    break;
#else
...
std::optional<unsigned> nextcode::expandMacroExpr(MacroExpansionExpr *mee) {
...
std::optional<unsigned>
nextcode::expandFreestandingMacro(MacroExpansionDecl *med) {
...
static SourceFile *evaluateAttachedMacro(MacroDecl *macro, Decl *attachedTo,
  ...
  // wrapper transform applied. This only impacts nextcodeinterfaces, and if
  // a nextcodeinterface was produced against a Concurrency library that does
  ...
  // all nextcodeinterfaces have been built against the Concurrency library
  ...
  case MacroDefinition::Kind::External: {
    ...
    BridgedStringRef evaluatedSourceOut{nullptr, 0};
    assert(!externalDef.isError());
    nextcode_ASTGen_expandAttachedMacro(
        &ctx.Diags, externalDef.get(), discriminator->c_str(),
...
bool nextcode::accessorMacroOnlyIntroducesObservers(
    MacroDecl *macro,
...
bool nextcode::accessorMacroIntroducesInitAccessor(
...
std::optional<unsigned> nextcode::expandAccessors(AbstractStorageDecl *storage,
                                               CustomAttr *attr,
                                               MacroDecl *macro) {
  if (auto var = dyn_cast<VarDecl>(storage)) {
...
std::optional<unsigned>
nextcode::expandAttributes(CustomAttr *attr, MacroDecl *macro, Decl *member) {
  // Evaluate the macro.
...
std::optional<unsigned> nextcode::expandMembers(CustomAttr *attr, MacroDecl *macro,
                                             Decl *decl) {
...
std::optional<unsigned> nextcode::expandPeers(CustomAttr *attr, MacroDecl *macro,
                                           Decl *decl) {
  auto macroSourceFile =
      ::evaluateAttachedMacro(macro, decl, attr,
...
std::optional<unsigned> nextcode::expandExtensions(CustomAttr *attr,
                                                MacroDecl *macro,
                                                MacroRole role,
...
SourceFile *nextcode::evaluateAttachedMacro(MacroDecl *macro, Decl *attachedTo,
                                         CustomAttr *attr,
                                         bool passParentContext, MacroRole role,
...
SourceFile *
nextcode::evaluateFreestandingMacro(FreestandingMacroExpansion *expansion,
                                 StringRef discriminator) {
