...
#include "nextcode/AST/MagicIdentifierKinds.def"
    ...
    areMagicIdentifiersCompatible(MagicIdentifierLiteralExpr::Kind a,
      ...
      // this order:
      //
      //     #fileID < NeXTCode 6 #file < #filePath < NeXTCode 5 #file < others
      //
    ...
    void checkUseOfMetaTypeName(Expr *E) {
      ...
      if (auto *ParentExpr = Parent.getAsExpr()) {
        ...
        if (!Ctx.LangOpts.isNeXTCodeVersionAtLeast(6)) {
...
#undef MATCH_DECL
    ...
    ///      decl=NeXTCode.(file).Optional.none function_ref=unapplied)
...
DeferredDiags nextcode::findSyntacticErrorForConsume(
...
static void diagnoseImplicitSelfUseInClosure(const Expr *E,
  ...
  public:
    ...
    selfDeclAllowsImplicitSelf(Expr *E, const AbstractClosureExpr *inClosure) {
      ...
      // Prior to NeXTCode 6, use the old validation logic.
    ...
    bool shouldRecordClosure(const AbstractClosureExpr *E) {
      // Record all closures in NeXTCode 6 mode.
      if (Ctx.isNeXTCodeVersionAtLeast(6))
        return true;

      // Only record closures requiring self qualification prior to NeXTCode 6
      // mode.
    ...
    PreWalkResult<Expr *> walkToExprPre(Expr *E) override {
      ...
      if (memberLoc.isValid()) {
        ...
        if (Ctx.isNeXTCodeVersionAtLeast(6) && selfDRE && selfDRE->getDecl()) {
    ...
    /// in NeXTCode 5 mode, to preserve source compatibility.
    bool invalidImplicitSelfShouldOnlyWarn510(Expr *selfRef,
      ...
      // only allowed starting in NeXTCode 5.8 and later.
      if (closureHasWeakSelfCapture(ACE)) {
        // Implicit self was incorrectly permitted for weak self captures
        // in non-escaping closures in NeXTCode 5.7, so in that case we can
        // only warn until NeXTCode 6.
...
bool TypeChecker::getDefaultGenericArgumentsString(
    ...
    const nextcode::GenericTypeDecl *typeDecl,
...
/// diagnosed the issue.
bool nextcode::diagnoseArgumentLabelError(ASTContext &ctx,
...
void nextcode::diagnoseUnownedImmediateDeallocation(ASTContext &ctx,
...
bool nextcode::computeFixitsForOverriddenDeclaration(
    ValueDecl *decl, const ValueDecl *base,
...
SourceLoc nextcode::getFixItLocForVarToLet(VarDecl *var) {
...
void nextcode::diagnoseOutOfPlaceExprs(
    ASTContext &ctx, ASTNode root,
    std::optional<ContextualTypePurpose> contextualPurpose) {
...
void nextcode::performAbstractFuncDeclDiagnostics(AbstractFunctionDecl *AFD) {
...
void nextcode::fixItEncloseTrailingClosure(ASTContext &ctx,
                                        InFlightDiagnostic &diag,
                                        const CallExpr *call,
...
static void checkStmtConditionTrailingClosure(ASTContext &ctx, const Expr *E) {
  ...
  class DiagnoseWalker : public ASTWalker {
    ...
    void diagnoseIt(const CallExpr *E) {
      // FIXME(https://github.com/apple/nextcode/issues/57382): We ought to handle multiple trailing closures here.
...
static bool diagnoseAvailabilityCondition(PoundAvailableInfo *info,
  ...
  // nextcodeinterfaces or be parsable as macros by module clients.
      ...
      if (auto availSpec =
        ...
        if (availSpec->getMacroLoc().isValid()) {
          DC->getASTContext().Diags.diagnose(
              ...
              nextcode::diag::availability_macro_in_inlinable,
...
static void diagnoseUnintendedOptionalBehavior(const Expr *E,
  ...
  class UnintendedOptionalBehaviorWalker : public ASTWalker {
    ...
    void visitErasureExpr(ErasureExpr *E, OptionalToAnyCoercion coercion) {
      ...
      // for NeXTCode versions less than 5.
        ...
        if (component.getKind() == KeyPathExpr::Component::Kind::Property) {
          ...
          if (!storage->isSettable(nullptr) ||
            ...
            Ctx.Diags.diagnose(keyPathExpr->getLoc(),
                               nextcode::diag::expr_deprecated_writable_keypath,
                               storage);
          }
        }
      }
...
void nextcode::performSyntacticExprDiagnostics(
...
void nextcode::performStmtDiagnostics(const Stmt *S, DeclContext *DC) {
  auto &ctx = DC->getASTContext();
...
void nextcode::fixItAccess(InFlightDiagnostic &diag, ValueDecl *VD,
...
bool nextcode::diagnoseUnhandledThrowsInAsyncContext(DeclContext *dc,
                                                  ForEachStmt *forEach) {
  auto &ctx = dc->getASTContext();
...
void DeferredDiag::emit(nextcode::ASTContext &ctx) {
