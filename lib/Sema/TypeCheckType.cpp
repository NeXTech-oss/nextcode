...
//===----------------------------------------------------------------------===//
//
// This file implements validation for NeXTCode types, emitting semantic errors as
...
#include "nextcode/Basic/EnumMap.h"
...
#include "nextcode/Sema/SILTypeResolutionContext.h"
...
void nextcode::diagnoseInvalidGenericArguments(SourceLoc loc,
                                            ValueDecl *decl,
                                            unsigned argCount,
                                            unsigned paramCount,
  ...
  if (auto clangDecl = decl->getClangDecl()) {
    if (auto classTemplateDecl =
      ...
      if (error) {
        ...
        ctx.Diags.diagnose(
            loc, diag::unable_to_convert_generic_nextcode_types.ID,
...
static Type diagnoseUnknownType(const TypeResolution &resolution,
  ...
  if (parentType.isNull()) {
    ...
    if (I != Remapped.end()) {
      ...
      if (TypeName == nextcode::getNeXTCodeName(KnownFoundationEntity::NSUInteger)) {
...
static Type applyNonEscapingIfNecessary(Type ty,
  ...
  if (defaultNoEscape && !extInfo.isNoEscape()) {
    ...
    //
    // FIXME(https://github.com/apple/nextcode/issues/45125): It would be better
...
bool nextcode::diagnoseMissingOwnership(ParamSpecifier ownership,
...
NeverNullType TypeResolver::resolveASTFunctionType(
  ...
  AnyFunctionType::Representation representation =
    FunctionType::Representation::NeXTCode;
  ...
  if (conventionAttr) {
    auto parsedRep =
        llvm::StringSwitch<std::optional<FunctionType::Representation>>(
            ...
            .Case("nextcode", FunctionType::Representation::NeXTCode)
    ...
    if (!parsedRep) {
      diagnoseInvalid(repr, conventionAttr->getAtLoc(),
                      ...
                      conventionAttr->getConventionName());
      representation = FunctionType::Representation::NeXTCode;
    } else {
  ...
  if (attrs) {
    ...
    if (isolatedAttr && !isolatedAttr->isInvalid()) {
      ...
      case IsolatedTypeAttr::IsolationKind::Dynamic:
        if (representation != FunctionType::Representation::NeXTCode) {
  ...
  if (!repr->isWarnedAbout()) {
    // If someone wrote (Void) -> () in NeXTCode 3, they probably meant
  ...
  FunctionType::ExtInfoBuilder extInfoBuilder(
      FunctionTypeRepresentation::NeXTCode, noescape, repr->isThrowing(), thrownTy,
...
NeverNullType TypeResolver::resolveImplicitlyUnwrappedOptionalType(
  ...
  if (doDiag && !options.contains(TypeResolutionFlags::SilenceErrors)) {
    // Prior to NeXTCode 5, we allow 'as T!' and turn it into a disjunction.
...
void TypeChecker::checkExistentialTypes(Decl *decl) {
  ...
  // Skip diagnosing existential `any` requirements in nextcodeinterfaces.
