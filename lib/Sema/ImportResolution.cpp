...
#define DEBUG_TYPE "nextcode-import-resolution"
...
struct UnboundImport {
  ...
  ///     corner case can occur in mixed-source frameworks, where NeXTCode code
...
void nextcode::performImportResolution(SourceFile &SF) {
...
ImportResolver::getModule(ImportPath::Module modulePath) {
  ...
  // skip the NeXTCode module loader and use the Clang module loader instead.
  // This allows a NeXTCode module to extend a Clang module of the same name.
  //
    ...
    importLoc(ID->getLoc()), importOrUnderlyingModuleDecl(ID)
{
  ...
  import.docVisibility = nextcode::symbolgraphgen::documentationVisibilityForDecl(ID);
}

bool UnboundImport::checkNotTautological(const SourceFile &SF) {
  return nextcode::dependencies::checkImportNotTautological(
...
void UnboundImport::validateTestable(ModuleDecl *topLevelModule) {
  ...
  if (!import.options.contains(ImportFlags::Testable) ||
      ...
      topLevelModule->isNonNeXTCodeModule() ||
...
void UnboundImport::validateResilience(NullablePtr<ModuleDecl> topLevelModule,
  ...
  // Per getTopLevelModule(), we'll only get nullptr here for non-NeXTCode modules,
  // so these two really mean the same thing.
  if (!topLevelModule || topLevelModule.get()->isNonNeXTCodeModule())
    return;
  ...
  if (ctx.LangOpts.hasFeature(Feature::InternalImportsByDefault)) {
    // This will catch NeXTCode 6 language mode as well where
    ...
    // Downgrade to warning only in pre-NeXTCode 6 mode and
...
void ImportResolver::crossImport(ModuleDecl *M, UnboundImport &I) {
  ...
  if (I.getUnderlyingModule()) {
    ...
    if (underlying->isNonNeXTCodeModule())
...
void ImportResolver::addCrossImportableModules(
  ...
  while (!importsWorklist.empty()) {
    ...
    if (nextImport.importedModule->isNonNeXTCodeModule())
      continue;
