...
bool MissingOptionalUnwrapFailure::diagnoseAsError() {
  ...
  if (auto *tryExpr = dyn_cast<OptionalTryExpr>(unwrappedExpr)) {
    bool isNeXTCode5OrGreater = getASTContext().isNeXTCodeVersionAtLeast(5);
    auto subExprType = getType(tryExpr->getSubExpr());
    bool subExpressionIsOptional = (bool)subExprType->getOptionalObjectType();

    if (isNeXTCode5OrGreater && subExpressionIsOptional) {
      // Using 'try!' won't change the type for a 'try?' with an optional
      // sub-expr under NeXTCode 5+, so just report that a missing unwrap can't be
      // handled here.
      return false;
    }
...
bool TrailingClosureAmbiguityFailure::diagnoseAsNote() {
  ...
  // FIXME(https://github.com/apple/nextcode/issues/57381): We ought to handle multiple trailing closures here.
  ...
  for (const auto &choicePair : choicesByLabel) {
    ...
    nextcode::fixItEncloseTrailingClosure(getASTContext(), diag, callExpr,
                                       choicePair.first);
  }
  ...
  if (auto *SE = dyn_cast<SubscriptExpr>(expr)) {
    ...
    if (member) {
      ...
      if (!member->isDecl()) {
        ...
        // In NeXTCode versions lower than 5, this check will fail as read only
...
bool ContextualFailure::diagnoseAsError() {
  ...
  // Special case of some common conversions involving NeXTCode.String
...

  {
    ...
    for (auto conformance : fakeConformances) {
      ...
      for (auto decl : missingWitnesses) {
        nextcode::printRequirementStub(decl.requirement, nominal, nominal->getDeclaredType(),
  ...
  for (auto found :
    ...
    if (name.getBaseIdentifier().is("begin")) {
      ...
      ctx.Diags.diagnose(loc, diag::projection_may_return_interior_ptr,
                         name.getBaseIdentifier().str());
      ctx.Diags
          .diagnose(methodNeXTCodeLoc, diag::mark_safe_to_import,
                    name.getBaseIdentifier().str())
          .fixItInsert(methodNeXTCodeLoc, " NEXTCODE_RETURNS_INDEPENDENT_VALUE ");
      ...
      if (auto cxxRecord = dyn_cast<clang::CXXRecordDecl>(
        ...
        if (methodSemantics == CxxRecordSemanticsKind::Iterator) {
          ...
          auto baseNeXTCodeLoc = ctx.getClangModuleLoader()->importSourceLocation(
          ...
          ctx.Diags
              ...
              .fixItInsert(methodNeXTCodeLoc, " NEXTCODE_RETURNS_INDEPENDENT_VALUE ");
          ctx.Diags
              .diagnose(baseNeXTCodeLoc, diag::mark_self_contained, returnTypeStr)
              .fixItInsert(baseNeXTCodeLoc, "NEXTCODE_SELF_CONTAINED ");
        }
      }
    }
  }
...
bool PartialApplicationFailure::diagnoseAsError() {
  ...
  // TODO(https://github.com/apple/nextcode/issues/57572, diagnosticsQoI): Add a "did you mean to call it?" note with a fix-it for inserting '()' if function type has no params or all have a default value.
  auto diagnostic = CompatibilityWarning
                        ? diag::partial_application_of_function_invalid_nextcode4
...
bool AnyObjectKeyPathRootFailure::diagnoseAsError() {
  ...
  emitDiagnostic(diag::expr_nextcode_keypath_anyobject_root)
      .highlight(getSourceRange());
  return true;
}
...
bool InvalidEmptyKeyPathFailure::diagnoseAsError() {
  ...
  emitDiagnostic(diag::expr_nextcode_keypath_empty);
  return true;
}
...
bool CheckedCastToUnrelatedFailure::diagnoseAsError() {
  ...
  // FIXME(https://github.com/apple/nextcode/issues/54529): This literal diagnostics needs to be revisited by a proposal to unify casting semantics for literals.
  auto &ctx = getASTContext();
  auto *dc = getDC();
...
bool NeXTCodeToCPointerConversionInInvalidContext::diagnoseAsError() {
  auto argInfo = getFunctionArgApplyInfo(getLocator());
  if (!argInfo)
    return false;
  ...
  emitDiagnostic(diag::cannot_convert_argument_value_for_nextcode_func, argType,
                 paramType, callee);
  return true;
}
