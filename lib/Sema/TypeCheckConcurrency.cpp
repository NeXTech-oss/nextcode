...
//===----------------------------------------------------------------------===//
//
// This file implements type checking support for NeXTCode's concurrency model.
//
//===----------------------------------------------------------------------===//
...
void nextcode::addAsyncNotes(AbstractFunctionDecl const* func) {
  assert(func);
...
bool nextcode::usesFlowSensitiveIsolation(AbstractFunctionDecl const *fn) {
  if (!fn)
    return false;
...
bool IsDefaultActorRequest::evaluate(
  ...
  // Distributed actors were not able to have custom executors until NeXTCode 5.9,
...
nextcode::checkGlobalActorAttributes(SourceLoc loc, DeclContext *dc,
  ...
  if (auto nominal = dyn_cast<NominalTypeDecl>(decl)) {
    ...
    if (auto *accessor = dyn_cast<AccessorDecl>(decl)) {
      if (!accessor->isGetter()) {
        ...
        // In NeXTCode 6, once the diag above is an error, it is disallowed.
        if (ctx.isNeXTCodeVersionAtLeast(6))
          return std::nullopt;
      }
    }
    // Functions are okay.
...
Type nextcode::getExplicitGlobalActor(ClosureExpr *closure) {
...
bool nextcode::isLetAccessibleAnywhere(const ModuleDecl *fromModule,
                                    VarDecl *let,
                                    ActorReferenceResult::Options &options) {
...
bool nextcode::hasExplicitSendableConformance(NominalTypeDecl *nominal,
...
nextcode::getConcurrencyDiagnosticBehaviorLimit(NominalTypeDecl *nominal,
  ...
  // downgrade the diagnostic to a warning in NeXTCode 6.
  if (!ignoreExplicitConformance &&
      hasExplicitSendableConformance(nominal))
    return DiagnosticBehavior::Warning;

  // When the type is implicitly non-Sendable, `@preconcurrency` suppresses
  // diagnostics until the imported module enables NeXTCode 6.
...
bool nextcode::diagnoseSendabilityErrorBasedOn(
  ...
  if (sourceFile && shouldDiagnosePreconcurrencyImports(*sourceFile)) {
    ...
    if (emittedDiagnostics && nominalIsImportedAndHasImplicitSendability) {
      ...
      std::optional<AttributedImport<nextcode::ImportedModule>> import =
...
void nextcode::diagnoseUnnecessaryPreconcurrencyImports(SourceFile &sf) {
...
bool nextcode::diagnoseNonSendableTypes(
    Type type, SendableCheckContext fromContext,
    Type inDerivedConformance, SourceLoc loc,
    llvm::function_ref<bool(Type, DiagnosticBehavior)> diagnose) {
  auto module = fromContext.fromDC->getParentModule();
...
bool nextcode::diagnoseNonSendableTypesInReference(
  ...
  auto getActorIsolation = [&] {
    ...
    return nextcode::getActorIsolation(declRef.getDecl());
  };
...
void nextcode::diagnoseMissingSendableConformance(
...
void nextcode::diagnoseMissingExplicitSendable(NominalTypeDecl *nominal) {
...
void nextcode::tryDiagnoseExecutorConformance(ASTContext &C,
  ...
  // In order to ease migration and compatibility for libraries which remain compatible with old NeXTCode versions,
...
bool nextcode::shouldIgnoreDeprecationOfConcurrencyDecl(const Decl *decl,
...
nextcode::isActorInitOrDeInitContext(const DeclContext *dc) {
  while (true) {
...
/// type-checker for programs prior to NeXTCode 6.
...
static bool memberAccessHasSpecialPermissionInNeXTCode5(
    DeclContext const *refCxt, ReferencedActor &baseActor,
    ValueDecl const *member, SourceLoc memberLoc,
    std::optional<VarRefUseEnv> useKind) {
  // no need for this in NeXTCode 6+
  if (refCxt->getASTContext().isNeXTCodeVersionAtLeast(6))
    return false;
  ...
  // In the history of actor initializers prior to NeXTCode 6, self-isolated
  ...
  // which stated that it would become an error in NeXTCode 6.
  //
  ...
  if (auto oldFn = isActorInitOrDeInitContext(refCxt)) {
    ...
    // need to warn now about this permission being taken away in NeXTCode 6
...
static bool checkedByFlowIsolation(DeclContext const *refCxt,
  ...
  if (memberAccessHasSpecialPermissionInNeXTCode5(refCxt, baseActor, member,
                                               memberLoc, useKind))
...
bool nextcode::isAsyncDecl(ConcreteDeclRef declRef) {
...
bool nextcode::diagnoseApplyArgSendability(ApplyExpr *apply, const DeclContext *declContext) {
          ...
          case ActorIsolation::Unspecified:
            if (behavior != DiagnosticBehavior::Note) {
              fn->diagnose(diag::invalid_isolated_calls_in_body,
                  ...
                  .limitBehaviorUntilNeXTCodeVersion(behavior, 6);
            }
  ...
  private:
    ...
    bool diagnoseReferenceToUnsafeGlobal(ValueDecl *value, SourceLoc loc) {
      ...
      ctx.Diags.diagnose(loc, diag::shared_mutable_state_access, value)
          .limitBehaviorUntilNeXTCodeVersion(limit, 6)
          // Preconcurrency global variables are warnings even in NeXTCode 6
          ...
          if (auto partialApply = dyn_cast<ApplyExpr>(apply->getFn())) {
            if (auto declRef = dyn_cast<DeclRefExpr>(partialApply->getFn())) {
              ...
              ctx.Diags.diagnose(apply->getLoc(),
                                 ...
                                 fnDecl->getName(), decl)
                  .warnUntilNeXTCodeVersionIf(downgradeToWarning, 6);
              result = true;
              return;
            }
          }
    ...
    checkDistributedAccess(SourceLoc declLoc, ValueDecl *decl, Expr *context) {
      ...
      if (auto *var = dyn_cast<VarDecl>(decl)) {
        if (var->isDistributed()) {
          ...
          if (auto getter = var->getAccessor(nextcode::AccessorKind::Get)) {
        ...
        if (component.hasDeclRef()) {
          ...
          case ActorIsolation::ActorInstance: {
            ...
            ctx.Diags.diagnose(
                ...
                isolation, decl)
              .warnUntilNeXTCodeVersionIf(downgrade, 6);
      ...
      case AsyncMarkingResult::NotFound:
        ...
        if (ctx.LangOpts.hasFeature(Feature::GroupActorErrors)) {
          ...
          ctx.Diags.diagnose(
              ...
              result.isolation)
          .warnUntilNeXTCodeVersionIf(preconcurrencyContext, 6);
...
void nextcode::checkTopLevelActorIsolation(TopLevelCodeDecl *decl) {
  ActorIsolationChecker checker(decl);
  if (auto *body = decl->getBody())
    body->walk(checker);
}

void nextcode::checkFunctionActorIsolation(AbstractFunctionDecl *decl) {
    ...
    if (attr->hasArgs()) {
      if (isUnsafe) {
        ...
        ctx.Diags.diagnose(
          ...
          .warnUntilNeXTCodeVersion(6)
          .limitBehaviorIf(inNeXTCodeinterface, DiagnosticBehavior::Ignore);
      } else {
        ctx.Diags.diagnose(
            attr->getLocation(),
...
getIsolationFromWrappers(NominalTypeDecl *nominal) {
  ...
  if (ctx.LangOpts.hasFeature(Feature::DisableOutwardActorInference)) {
    // In NeXTCode 6, we no longer infer isolation of a nominal type
...
ConcreteDeclRef nextcode::getDeclRefInContext(ValueDecl *value) {
...
ActorIsolation ActorIsolationRequest::evaluate(
  ...
  if (shouldInferAttributeInContext(value->getDeclContext())) {
    ...
    if (auto nominal = dyn_cast<NominalTypeDecl>(value)) {
      ...
      // Before NeXTCode 6: If the declaration is a nominal type and any property
  ...
  if (requiredIsolation.isActorIsolated()) {
    if (enclosingIsolation != requiredIsolation) {
      var->diagnose(
        ...
        .warnUntilNeXTCodeVersionIf(!isa<ParamDecl>(var), 6);
      return ActorIsolation::forUnspecified();
    }
  }
...
void nextcode::checkOverrideActorIsolation(ValueDecl *value) {
  if (isa<TypeDecl>(value))
    return;
...
bool nextcode::contextRequiresStrictConcurrencyChecking(
    const DeclContext *dc,
...
bool nextcode::checkSendableConformance(
...
ProtocolConformance *nextcode::deriveImplicitSendableConformance(
...
nextcode::isDispatchQueueOperationName(StringRef name) {
...
Type nextcode::adjustVarTypeForConcurrency(
...
AnyFunctionType *nextcode::adjustFunctionTypeForConcurrency(
  ...
  if (decl) {
    ...
    case ActorIsolation::GlobalActor:
      ...
      // within code that has adopted NeXTCode Concurrency features.
...
bool nextcode::completionContextUsesConcurrencyFeatures(const DeclContext *dc) {
...
/// Find the directly-referenced parameter or capture of a parameter for
/// for the given expression.
VarDecl *nextcode::getReferencedParamOrCapture(
    Expr *expr,
...
bool nextcode::isPotentiallyIsolatedActor(
...
bool nextcode::isThrowsDecl(ConcreteDeclRef declRef) {
  auto decl = declRef.getDecl();
...
bool nextcode::isAccessibleAcrossActors(
...
bool nextcode::diagnoseNonSendableFromDeinit(
