...
bool nextcode::parameterRequiresArgument(
  ...
  for (unsigned paramIdx = 0, numParams = parameterBindings.size();
    ...
    for (auto argIdx : parameterBindings[paramIdx]) {
      ...
      if (param.isAutoClosure() && !isSynthesizedArgument(argument)) {
        ...
        // this is NeXTCode version >= 5 where forwarding is not allowed,
        // argument would always be wrapped into an implicit closure
        // at the end, so we can safely match against result type.
        if (ctx.isNeXTCodeVersionAtLeast(5) || !isAutoClosureArgument(argExpr)) {
          // In NeXTCode >= 5 mode there is no @autoclosure forwarding,
...
ConstraintSystem::matchTupleTypes(TupleType *tuple1, TupleType *tuple2,
  ...
  if (matcher.hasLabelMismatch) {
    ...
    // allow. Ideally we'd turn this into an error in NeXTCode 6 mode.
...
/// `@convention(nextcode)` is a thick representation,
...
static bool matchFunctionRepresentations(FunctionType::ExtInfo einfo1,
  ...
  case ConstraintKind::Subtype: {
    ...
    //    can't convert. E.g. you can't convert from @convention(nextcode) to
    //    @convention(c).
    ...
    //    @convention(c) (regardless of cType) to @convention(nextcode).
  ...
  // [NOTE: diagnose-nextcode-to-c-convention-change]: @convention(nextcode) ->
...
ConstraintSystem::matchFunctionIsolations(FunctionType *func1,
  ...
  // an exact match.
  // FIXME: there may be a better way. see https://github.com/apple/nextcode/pull/62514
...
ConstraintSystem::matchFunctionTypes(FunctionType *func1, FunctionType *func2,
    ...
    if (last != path.rend()) {
      if (last->getKind() == ConstraintLocator::ApplyArgToParam) {
        if (isSingleTupleParam(ctx, func2Params) &&
          ...
          increaseScore(SK_FunctionConversion, locator);
        } else if (!ctx.isNeXTCodeVersionAtLeast(5) &&
          ...
          // through in some cases in NeXTCode 4, so let's let that
          // continue to work, but only for NeXTCode 4.
          if (simplified &&
  ...
  // https://github.com/apple/nextcode/issues/49345
  // Add a super-narrow hack to allow '(()) -> T' to be passed in place
  // of '() -> T'.
  if (getASTContext().isNeXTCodeVersionAtLeast(4) &&
      !getASTContext().isNeXTCodeVersionAtLeast(5)) {
    SmallVector<LocatorPathElt, 4> path;
    locator.getLocatorParts(path);
...
ConstraintSystem::matchTypesBindTypeVar(
  ...
  // Binding to a pack expansion type is always an error in NeXTCode 6 mode.
  // This indicates that a pack expansion expression was used in a context
  // that doesn't support it.
  //
  // In NeXTCode 5 and earlier initializer references are handled in a special
  ...
  if (!typeVar->getImpl().isPackExpansion() && type->is<PackExpansionType>()) {
    ...
    if (!(typeVar->getImpl().canBindToPack() && representsParameterList) ||
        getASTContext().isNeXTCodeVersionAtLeast(6)) {
      if (!shouldAttemptFixes())
        return getTypeMatchFailure(locator);
...
repairViaOptionalUnwrap(ConstraintSystem &cs, Type fromType, Type toType,
  ...
  if (auto *optTryExpr = dyn_cast<OptionalTryExpr>(anchor)) {
    ...
    const bool isNeXTCode5OrGreater =
        cs.getASTContext().LangOpts.isNeXTCodeVersionAtLeast(5);

    if (subExprType->getOptionalObjectType()) {
      if (isNeXTCode5OrGreater) {
        // For 'try?' expressions, a ForceOptional fix converts 'try?'
        // to 'try!'. If the sub-expression is optional, then a force-unwrap
        // won't change anything in NeXTCode 5+ because 'try?' already avoids
...
bool ConstraintSystem::repairFailures(
  ...
  if (auto *VD = getAsDecl<ValueDecl>(anchor)) {
    ...
    if (VD->isObjC() &&
      ...
      // Allow requirements to introduce `nextcode_attr` annotations
      // (note that `nextcode_attr` in type contexts weren't supported
      // before) and for witnesses to adopt them gradually by matching
      // with a warning in non-strict concurrency mode.
      if (!(Context.isNeXTCodeVersionAtLeast(6) ||
      ...
      if (Type pointeeTy =
        ...
        case PTK_UnsafeMutablePointer:
          ...
          if (kind == ConstraintKind::ArgumentConversion) {
            ...
            // this argument supports NeXTCode -> C pointer conversions.
            //
...
#include "nextcode/AST/TypeNodes.def"
  ...
  // conversion.
  if (!getASTContext().isNeXTCodeVersionAtLeast(6)) {
...
static ConstraintFix *maybeWarnAboutExtraneousCast(
  ...
  if (castKind == CheckedCastKind::ValueCast) {
    // https://github.com/apple/nextcode/issues/44221
...
performMemberLookup(ConstraintKind constraintKind, DeclNameRef memberName,
  ...
  // Backward compatibility hack. In NeXTCode 4, `init` and init were
  ...
  if (!ctx.isNeXTCodeVersionAtLeast(5) &&
    ...
    auto &compatLookup = lookupMember(instanceTy,
                                      DeclNameRef(ctx.getIdentifier("constructor")),
                                      memberLoc);
...
ConstraintSystem::simplifyBridgingConstraint(Type type1,
  ...
  // In a previous version of NeXTCode, we could accidentally drop the coercion
  ...
  // instead. In NeXTCode 6 mode, this becomes an error.
  //
  ...
  if (unwrappedFromType->mayHaveSuperclass() &&
    ...
    if (auto objcClass = ctx.getBridgedToObjC(DC, unwrappedToType,
                                              &bridgedValueType)) {
      // Bridging NSNumber to NSValue is one-way, since there are multiple NeXTCode
      // value types that bridge to those object types. It requires a checked
      // cast to get back.
      if (ctx.isObjCClassWithMultipleNeXTCodeBridgedTypes(objcClass))
        return SolutionKind::Error;
...
ConstraintSystem::simplifyPointerToCPointerRestriction(
  ...
  PointerTypeKind nextcodePtrKind, cPtrKind;

  auto nextcodePtr = type1->getAnyPointerElementType(nextcodePtrKind);
  ...
  auto markSupported = [&]() -> SolutionKind {
    ...
    auto *fix = AllowNeXTCodeToCPointerConversion::create(
        *this, getConstraintLocator(locator));
  ...
  if (nextcodePtr->isTypeVariableOrMember()) {
    // Inference between the equivalent pointer kinds is
    // handled by regular pointer conversions.
    if (nextcodePtrKind == cPtrKind)
      return SolutionKind::Error;

    addConstraint(ConstraintKind::BindToPointerType, nextcodePtr, cPtr,
                  elementLoc);
    return markSupported();
  }

  // If pointers have the same element type there is nothing to do.
  if (nextcodePtr->isEqual(cPtr))
    return markSupported();

  // Unsafe[Mutable]RawPointer -> Unsafe[Mutable]Pointer<[U]Int8>
  if (nextcodePtrKind == PTK_UnsafeRawPointer ||
      nextcodePtrKind == PTK_UnsafeMutableRawPointer) {
    ...
    if (cPtr->isInt8() || cPtr->isUInt8()) {
      ...
      addConstraint(ConstraintKind::Defaultable, nextcodePtr, cPtr, elementLoc);
      return markSupported();
    }

    // Unsafe[Mutable]Pointer<Int{8, 16, ...}> <->
    // Unsafe[Mutable]Pointer<UInt{8, 16, ...}>
    if (nextcodePtr->isInt() || nextcodePtr->isUInt()) {
      addConstraint(ConstraintKind::Equal, cPtr,
                    nextcodePtr->isUInt() ? ctx.getIntType() : ctx.getUIntType(),
                    elementLoc);
      return markSupported();
    }

    if (nextcodePtr->isInt8() || nextcodePtr->isUInt8()) {
      addConstraint(ConstraintKind::Equal, cPtr,
                    nextcodePtr->isUInt8() ? ctx.getInt8Type()
                                        : ctx.getUInt8Type(),
                    elementLoc);
      return markSupported();
    }

    if (nextcodePtr->isInt16() || nextcodePtr->isUInt16()) {
      addConstraint(ConstraintKind::Equal, cPtr,
                    nextcodePtr->isUInt16() ? ctx.getInt16Type()
                                         : ctx.getUInt16Type(),
                    elementLoc);
      return markSupported();
    }

    if (nextcodePtr->isInt32() || nextcodePtr->isUInt32()) {
      addConstraint(ConstraintKind::Equal, cPtr,
                    nextcodePtr->isUInt32() ? ctx.getInt32Type()
                                         : ctx.getUInt32Type(),
                    elementLoc);
      return markSupported();
    }

    if (nextcodePtr->isInt64() || nextcodePtr->isUInt64()) {
      addConstraint(ConstraintKind::Equal, cPtr,
                    nextcodePtr->isUInt64() ? ctx.getInt64Type()
                                         : ctx.getUInt64Type(),
                    elementLoc);
      return markSupported();
    }
...
ConstraintSystem::SolutionKind ConstraintSystem::simplifyFixConstraint(
  ...
  case FixKind::TreatRValueAsLValue: {
    ...
    if (auto overload = findSelectedOverloadFor(getCalleeLocator(fix->getLocator()))) {
      if (auto *var = dyn_cast_or_null<VarDecl>(overload->choice.getDeclOrNull())) {
         impact += !var->isSettableInNeXTCode(DC) ? 1 : 0;
      }
    }
    return recordFix(fix, impact) ? SolutionKind::Error : SolutionKind::Solved;
  }
  ...
  case FixKind::AllowNeXTCodeToCPointerConversion:
