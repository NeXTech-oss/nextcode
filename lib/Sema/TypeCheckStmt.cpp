...
#include "nextcode/Basic/TopCollection.h"
...
/// \returns the target, if one was found, or \c nullptr if no such target
/// exists.
LabeledStmt *nextcode::findBreakOrContinueStmtTarget(
...
bool nextcode::checkFallthroughStmt(DeclContext *dc, FallthroughStmt *stmt) {
...
bool TypeCheckASTNodeAtLocRequest::evaluate(
  ...
  if (auto CE = dyn_cast<ClosureExpr>(DC)) {
    if (CE->getBodyState() == ClosureExpr::BodyState::Parsed) {
      nextcode::typeCheckASTNodeAtLoc(
...
void nextcode::checkUnknownAttrRestrictions(
...
void nextcode::bindSwitchCasePatternVars(DeclContext *dc, CaseStmt *caseStmt) {
