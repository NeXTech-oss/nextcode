...
static void recordTypeWitness(NormalProtocolConformance *conformance,
  ...
  if (typeDecl == nullptr) {
    ...
    if (!ctx.isNeXTCodeVersionAtLeast(5) &&
      ...
      // HACK: In pre-NeXTCode-5, these typealiases were synthesized with the
...
static ResolveWitnessResult resolveTypeWitnessViaLookup(
  ...
  for (auto candidate : candidates) {
    ...
    // Prior to NeXTCode 6, ignore a member named Failure when matching
    ...
    // constraint will be disallowed in NeXTCode 6, because existential types
...
private:
  ...
  ///   involving multiple files/modules, and not in tests within the NeXTCode
  ///   project itself.
...
Type nextcode::adjustInferredAssociatedType(TypeAdjustment adjustment, Type type,
                                         bool &performed) {
...
AssociatedTypeDecl *nextcode::findDefaultedAssociatedType(
                                             DeclContext *dc,
