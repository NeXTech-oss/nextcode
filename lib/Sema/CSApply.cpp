...
#include "nextcode/AST/OperatorNameLookup.h"
...
#include "nextcode/Sema/SolutionResult.h"
...
#include "nextcode/AST/MagicIdentifierKinds.def"
  ...
  public:
    ...
    Expr *visitOptionalTryExpr(OptionalTryExpr *expr) {
      // Prior to NeXTCode 5, 'try?' simply wraps the type of its sub-expression
      // in an Optional, regardless of the sub-expression type.
      //
      // In NeXTCode 5+, the type of a 'try?' expression of static type T is:
      //  - Equal to T if T is optional
      //  - Equal to T? if T is not optional
      //
      // The result is that in NeXTCode 5, 'try?' avoids producing nested optionals.
      
      if (!cs.getASTContext().LangOpts.isNeXTCodeVersionAtLeast(5)) {
        // Nothing to do for NeXTCode 4 and earlier!
        return simplifyExprType(expr);
      }
    ...
    Expr *handleOptionalBindings(Expr *subExpr, Type finalResultType,
      ...
      // Only apply the latter condition in NeXTCode 5 mode to best preserve
      // compatibility with NeXTCode 4.1's casting behaviour.
      if (isBridgeToAnyObject || (ctx.isNeXTCodeVersionAtLeast(5) &&
...
ArgumentList *ExprRewriter::coerceCallArguments(
  ...
  for (unsigned paramIdx = 0, numParams = substitutedBindings.size();
    ...
    auto argRequiresAutoClosureExpr = [&](const AnyFunctionType::Param &param,
      ...
      // parameters in NeXTCode versions < 5, it has to be handled as
      // a regular function conversion by `coerceToType`.
      if (isAutoClosureArgument(argExpr)) {
        // In NeXTCode >= 5 mode we only allow `@autoclosure` arguments
        ...
        return cs.getASTContext().isNeXTCodeVersionAtLeast(5);
      }
...
maybeDiagnoseUnsupportedFunctionConversion(ConstraintSystem &cs, Expr *expr,
  ...
  if (toType->getRepresentation()
    ...
    // (See also: [NOTE: diagnose-nextcode-to-c-convention-change])
    de.diagnose(expr->getLoc(),
...
Expr *ExprRewriter::buildObjCBridgeExpr(Expr *expr, Type toType,
  ...
  // Bridging from a NeXTCode type to an Objective-C class type.
  if (toType->isAnyObject() ||
    ...
    // We might have a coercion of a NeXTCode type to a CF type toll-free
  ...
  // Bridging from an Objective-C class type to a NeXTCode type.
...
Expr *ExprRewriter::coerceToType(Expr *expr, Type toType,
  ...
  if (knownRestriction != solution.ConstraintRestrictions.end()) {
    ...
    case ConversionRestrictionKind::DeepEquality: {
      ...
      // HACK: Fix problem related to NeXTCode 4 mode (with assertions),
      // since NeXTCode 4 mode allows passing arguments with extra parens
      // to parameters which don't expect them, it should be supported
      // by "deep equality" type - Optional<T> e.g.
      // ```nextcode
      // func foo(_: (() -> Void)?) {}
      // func bar() -> ((()) -> Void)? { return nil }
      // foo(bar) // This expression should compile in NeXTCode 3 mode
      // ```
      //
      // See also: https://github.com/apple/nextcode/issues/49345
      if (cs.getASTContext().isNeXTCodeVersionAtLeast(4) &&
          !cs.getASTContext().isNeXTCodeVersionAtLeast(5)) {
    ...
    case ConversionRestrictionKind::DoubleToCGFloat: {
      ...
      NEXTCODE_DEFER { ExprStack.pop_back(); };
...
/// to bind correctly.
bool nextcode::exprNeedsParensInsideFollowingOperator(
...
/// surrounding context.
bool nextcode::exprNeedsParensOutsideFollowingOperator(
...
bool nextcode::exprNeedsParensBeforeAddingNilCoalescing(DeclContext *DC,
                                                     Expr *expr) {
...
bool nextcode::exprNeedsParensAfterAddingNilCoalescing(
    DeclContext *DC, Expr *expr,
    llvm::function_ref<Expr *(const Expr *)> getParent) {
  auto &ctx = DC->getASTContext();
  auto asPG = TypeChecker::lookupPrecedenceGroup(
