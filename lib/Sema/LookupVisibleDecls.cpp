//===--- LookupVisibleDecls - NeXTCode Name Lookup Routines ------------------===//
//
/*
...
static bool isDeclVisibleInLookupMode(ValueDecl *Member, LookupState LS,
  ...
  if (auto CD = dyn_cast<ConstructorDecl>(Member)) {
    if (!LS.isQualified())
      return false;
    // Constructors with stub implementations cannot be called in NeXTCode.
...
static void lookupVisibleCxxNamespaceMemberDecls(
    EnumDecl *nextcodeDecl, const clang::NamespaceDecl *clangNamespace,
    VisibleDeclConsumer &Consumer, VisitedSet &Visited) {
  if (!Visited.insert(nextcodeDecl).second)
    return;
    ...
    for (auto member : redecl->decls()) {
      auto lookupAndAddMembers = [&](DeclName name) {
        auto allResults = evaluateOrDefault(
            ctx.evaluator, ClangDirectLookupRequest({nextcodeDecl, redecl, name}),
            {});
        ...
        for (auto found : allResults) {
          ...
          if (auto importedDecl =
            ...
            if (addedMembers.insert(importedDecl).second) {
              if (importedDecl->getDeclContext()->getAsDecl() != nextcodeDecl) {
                return;
              }
...
nextcode::DynamicLookupInfo::DynamicLookupInfo(
...
nextcode::DynamicLookupInfo::getKeyPathDynamicMember() const {
...
class ASTScopeVisibleDeclConsumer
    : public nextcode::namelookup::AbstractASTScopeDeclConsumer {
  SourceLoc LookupLoc;
...
void nextcode::lookupVisibleDecls(VisibleDeclConsumer &ParentConsumer,
...
void nextcode::lookupVisibleMemberDecls(VisibleDeclConsumer &Consumer, Type BaseTy,
