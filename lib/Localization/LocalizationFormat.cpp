...
void SerializedLocalizationWriter::insert(nextcode::DiagID id,
...
llvm::StringRef
LocalizationProducer::getMessageOr(nextcode::DiagID id,
...
llvm::StringRef
SerializedLocalizationProducer::getMessage(nextcode::DiagID id) const {
...
void DefToStringsConverter::convert(llvm::raw_ostream &out) {
  // "<id>" = "<msg>";
  for (auto i : nextcode::indices(IDs)) {
    out << "\"" << IDs[i] << "\"";
    out << " = ";
...
llvm::StringRef
StringsLocalizationProducer::getMessage(nextcode::DiagID id) const {
...
void StringsLocalizationProducer::forEachAvailable(
  ...
  for (uint32_t i = 0, n = diagnostics.size(); i != n; ++i) {
    ...
    if (!translation.empty())
      callback(static_cast<nextcode::DiagID>(i), translation);
  }
}
