...
bool nextcode::canBeArgumentLabel(StringRef identifier) {
...
bool nextcode::canBeMemberName(StringRef identifier) {
  return llvm::StringSwitch<bool>(identifier)
...
bool nextcode::isPreposition(StringRef word) {
...
PartOfSpeech nextcode::getPartOfSpeech(StringRef word) {
...
StringRef nextcode::matchLeadingTypeName(StringRef name,
...
const char *nextcode::copyCString(StringRef string,
...
static Words::iterator matchTypeNameFromBackWithSpecialCases(
  ...
  while (nameWordRevIter != nameWordRevIterEnd &&
    ...
    // Special case: "Index" in the name matches "Integer" or "Integer" in the type.
...
bool nextcode::omitNeedlessWords(
  ...
  for (unsigned i = 0, n = argNames.size(); i != n; ++i) {
    ...
    NameRole role = i > 0 ? NameRole::SubsequentParameter
      ...
      : baseName == "constructor" ? NameRole::SubsequentParameter
...
nextcode::stripWithCompletionHandlerSuffix(StringRef name) {
...
void nextcode::writeEscaped(llvm::StringRef Str, llvm::raw_ostream &OS) {
...
bool nextcode::pathStartsWith(StringRef prefix, StringRef path) {
