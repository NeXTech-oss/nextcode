...
///
/// Helper for nextcode::atomicallyWritingToFile.
///
...
std::error_code nextcode::atomicallyWritingToFile(
    const llvm::StringRef outputPath,
  ...
  return nextcode::moveFileIfDifferent(temporaryPath.value(), outputPath);
}

llvm::ErrorOr<FileDifference>
nextcode::areFilesDifferent(const llvm::Twine &source,
...
std::error_code nextcode::moveFileIfDifferent(const llvm::Twine &source,
                                           const llvm::Twine &destination) {
  namespace fs = llvm::sys::fs;
...
nextcode::vfs::getFileOrSTDIN(llvm::vfs::FileSystem &FS,
