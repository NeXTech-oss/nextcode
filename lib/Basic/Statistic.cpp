//===--- Statistic.cpp - NeXTCode unified stats reporting --------------------===//
//
/*
...
bool environmentVariableRequestedMaximumDeterminism() {
  if (const char *S = ::getenv("NEXTCODEC_MAXIMUM_DETERMINISM"))
    return (S[0] != '\0');
  return false;
}
...
public:
  ...
  void beginTimer(StringRef Name) {
    ...
    if (T.RecursionDepth == 0) {
      T.Timer.emplace(Name, Name, "nextcode", "NeXTCode compilation");
    }
    ++T.RecursionDepth;
  }
...
#include "nextcode/Basic/Statistics.def"
