...
struct OwnershipModelEliminator : SILFunctionTransform {
  ...
  void run() override {
    ...
    // Verify here to make sure ownership is correct before we strip.
    {
      ...
      getPassManager()->runNeXTCodeFunctionVerification(f);
    }
...
SILTransform *nextcode::createOwnershipModelEliminator() {
  return new OwnershipModelEliminator(false /*skip transparent*/,
                                      false /*ignore stdlib*/);
}

SILTransform *nextcode::createNonTransparentFunctionOwnershipModelEliminator() {
