...
/// NeXTCode needs to emit debug info, we pre-propagate debug info for moved values
...
/// multiple times by hand and potentially messing up. This /should/ be a nextcode
...
void DebugInfoPropagator::performInitialLocalDataflow() {
  ...
  for (auto &block : *fn) {
    ...
    // blockLastGenInst is per block state, so use NEXTCODE_DEFER to make sure we
    // don't forget to clean it up before processing the next block.
    NEXTCODE_DEFER { blockLastGenInst.clear(); };
...
SILTransform *nextcode::createMovedAsyncVarDebugInfoPropagator() {
