...
static unsigned insertIndirectReturnArgs(AddressLoweringState &pass) {
  ...
  if (!declCtx) {
    ...
    // example.
    declCtx = pass.function->getModule().getNeXTCodeModule();
  }
...
void UseRewriter::visitLifetimeIntroducer(Introducer *introducer) {
  ...
  if (introducer->isLexical()) {
    ...
    NEXTCODE_ASSERT_ONLY(address->dump());
...
SILTransform *nextcode::createAddressLowering() { return new AddressLowering(); }
