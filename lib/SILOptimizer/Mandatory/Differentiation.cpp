...
#include "nextcode/SILOptimizer/Differentiation/JVPCloner.h"
...
#include "nextcode/SILOptimizer/Utils/DifferentiationMangler.h"
...
using namespace nextcode::autodiff;
using llvm::DenseMap;
...
static bool diagnoseUnsatisfiedRequirements(ADContext &context,
  ...
  auto *nextcodeModule = context.getModule().getNeXTCodeModule();
  ...
  for (auto req : requirements) {
    ...
    case RequirementKind::Conformance: {
      ...
      if (!nextcodeModule->lookupConformance(firstType, protocol))
        unsatisfiedRequirements.push_back(req);
      continue;
    }
    }
    ...
    if (innerNewFunc->getType().castTo<SILFunctionType>()->isPolymorphic()) {
      if (!newFuncGenSig) {
        ...
        substMap = SubstitutionMap::get(
            ...
            LookUpConformanceInModule(builder.getModule().getNeXTCodeModule()));
      }
    }
    ...
    if (!minimalWitness) {
      ...
      auto derivativeConstrainedGenSig =
          autodiff::getConstrainedDerivativeGenericSignature(
              ...
              desiredParameterIndices, desiredResultIndices,
              contextualDerivativeGenSig,
              LookUpConformanceInModule(context.getModule().getNeXTCodeModule()));
...
static SILFunction *createEmptyVJP(ADContext &context,
  ...
  auto vjpType = originalTy->getAutoDiffDerivativeFunctionType(
      ...
      module.Types, LookUpConformanceInModule(module.getNeXTCodeModule()),
      vjpCanGenSig,
...
void Differentiation::run() {
    ...
    for (SILBasicBlock &bb : f) {
      for (SILInstruction &i : bb) {
        if (auto *dfi = dyn_cast<DifferentiableFunctionInst>(&i)) {
          ...
          // FIXME(https://github.com/apple/nextcode/issues/54256): Finish support for linear map transposition.
  ...
  // Differentiation relies on the stdlib (the NeXTCode module).
  // If it's not imported, it's an internal error.
  if (!astCtx.getStdlibModule()) {
    astCtx.Diags.diagnose(SourceLoc(),
                          diag::autodiff_internal_nextcode_not_imported);
    return;
  }
...
SILTransform *nextcode::createDifferentiation() { return new Differentiation; }
