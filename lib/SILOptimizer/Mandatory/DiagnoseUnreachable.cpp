...
getAllocStackSingleInitializingInjectEnumAddr(SwitchEnumAddrInst *SEAI) {
  ...
  SmallVector<Operand *, 16> worklist(stackSlot->use_begin(),
                                      stackSlot->use_end());
  LLVM_DEBUG(NEXTCODE_DEFER { llvm::dbgs() << "Exiting!\n"; });
  while (worklist.size()) {
...
void nextcode::performSILDiagnoseUnreachable(SILModule *M) {
  for (auto &Fn : *M)
...
SILTransform *nextcode::createNoReturnFolding() {
...
SILTransform *nextcode::createDiagnoseUnreachable() {
