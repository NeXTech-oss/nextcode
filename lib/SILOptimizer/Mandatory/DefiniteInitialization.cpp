...
#include "nextcode/SILOptimizer/Utils/DistributedActor.h"
...
namespace {
  ...
  class LifetimeChecker {
    ...
    /// initializer is not allowed to be fieldwise in NeXTCode 5, so we produce a
    /// warning in NeXTCode 4 and earlier.
...
void LifetimeChecker::handleStoreUse(unsigned UseID) {
  ...
  // than DelegatingSelf for NeXTCode 4 compatibility. We look for a problem case by
...
AvailabilitySet
LifetimeChecker::getLivenessAtNonTupleInst(nextcode::SILInstruction *Inst,
                                           nextcode::SILBasicBlock *InstBB,
...
SILTransform *nextcode::createDefiniteInitialization() {
