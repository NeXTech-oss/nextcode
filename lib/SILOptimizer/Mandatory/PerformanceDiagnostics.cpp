...
public:
  ...
  bool visitFunctionEmbeddedNeXTCode(SILFunction *function) {
    return visitFunctionEmbeddedNeXTCode(function, /*parentLoc*/ nullptr);
  }
...
/// Prints Embedded NeXTCode specific performance diagnostics (no existentials,
/// no metatypes, optionally no allocations) for \p function.
bool PerformanceDiagnostics::visitFunctionEmbeddedNeXTCode(
    SILFunction *function, LocWithParent *parentLoc) {
  // Don't check generic functions in embedded NeXTCode, they're about to be
  // removed anyway.
    ...
    for (SILInstruction &inst : block) {
      ...
      if (auto as = FullApplySite::isa(&inst)) {
        ...
        for (SILFunction *callee : bca->getCalleeList(as)) {
          if (visitFunctionEmbeddedNeXTCode(callee, loc))
            return true;
        }
        ...
        case BuiltinValueKind::OnceWithContext:
          if (auto *fri = dyn_cast<FunctionRefInst>(bi->getArguments()[1])) {
            ...
            if (visitFunctionEmbeddedNeXTCode(fri->getReferencedFunction(), loc))
              return true;
          }
          break;
        default:
  ...
  for (Operand *use : mt->getUses()) {
    ...
    if (auto *apply = dyn_cast<ApplyInst>(use->getUser())) {
      if (auto *callee = apply->getReferencedFunctionOrNull()) {
        // Exclude `NeXTCode._diagnoseUnexpectedEnumCaseValue<A, B>(type: A.Type, rawValue: B) -> NeXTCode.Never`
        ...
        if (callee->getName() == "$ss32_diagnoseUnexpectedEnumCaseValue4type03rawE0s5NeverOxm_q_tr0_lF" &&
            !mt->getModule().getOptions().EmbeddedNeXTCode) {
          continue;
        }
      }
    }
...
static bool allowedMetadataUseInEmbeddedNeXTCode(SILInstruction *inst) {
  ...
  if ((isa<ValueMetatypeInst>(inst) || isa<MetatypeInst>(inst))) {
    ...
    if (metaTy->getRepresentation() == MetatypeRepresentation::Thick) {
      ...
      // Class metadata are supported in embedded NeXTCode
...
bool PerformanceDiagnostics::visitInst(SILInstruction *inst,
  ...
  if (module.getOptions().EmbeddedNeXTCode) {
    if (impact & RuntimeEffect::Existential) {
      PrettyStackTracePerformanceDiagnostics stackTrace("existential", inst);
      if (impactType) {
        diagnose(loc, diag::embedded_nextcode_existential_type, impactType.getASTType());
      } else {
        diagnose(loc, diag::embedded_nextcode_existential);
      }
      return true;
    }
    ...
    if (impact & RuntimeEffect::MetaData) {
      if (isa<ReleaseValueInst>(inst)) {
        ...
        diagnose(loc, diag::embedded_nextcode_value_deinit, impactType.getASTType());
        return true;
      }
      if (isa<KeyPathInst>(inst)) {
        diagnose(loc, diag::embedded_nextcode_keypath);
        return true;
      }
      if (isa<CheckedCastAddrBranchInst>(inst) || isa<UnconditionalCheckedCastAddrInst>(inst)) {
        diagnose(loc, diag::embedded_nextcode_dynamic_cast);
        return true;
      }
      if (!allowedMetadataUseInEmbeddedNeXTCode(inst)) {
        PrettyStackTracePerformanceDiagnostics stackTrace("metatype", inst);
        if (impactType) {
          diagnose(loc, diag::embedded_nextcode_metatype_type, impactType.getASTType());
        } else {
          diagnose(loc, diag::embedded_nextcode_metatype);
        }
        return true;
      }
    }
    ...
    if (module.getOptions().NoAllocations) {
      if (impact & RuntimeEffect::Allocating) {
        ...
        if (impactType) {
          diagnose(loc, diag::embedded_nextcode_allocating_type, impactType.getASTType());
        } else {
          diagnose(loc, diag::embedded_nextcode_allocating);
        }
        return true;
      }
    }
...
private:
  void run() override {
    ...
    if (!annotatedFunctionsFound && !getModule()->getOptions().EmbeddedNeXTCode)
      return;
    ...
    if (getModule()->getOptions().EmbeddedNeXTCode) {
      // Run embedded NeXTCode SIL checks for metatype/existential use, and
      ...
      for (SILFunction &function : *module) {
        ...
        // nextcode_readAtKeyPath generated by SILGen for keypaths. It's okay to
      ...
      for (SILFunction *function : externallyVisibleFunctions) {
        diagnoser.visitFunctionEmbeddedNeXTCode(function);
      }
...
SILTransform *nextcode::createPerformanceDiagnostics() {
