...
bool nextcode::optimizeMemoryAccesses(SILFunction *fn, DominanceInfo *domInfo) {
  bool changed = false;
...
bool nextcode::eliminateDeadAllocations(SILFunction *fn, DominanceInfo *domInfo) {
  if (!fn->hasOwnership())
    return false;
...
SILTransform *nextcode::createPredictableMemoryAccessOptimizations() {
  return new PredictableMemoryAccessOptimizations();
}

SILTransform *nextcode::createPredictableDeadAllocationElimination() {
