...
/// and mutating self.
///
/// Move Address Checking in NeXTCode
/// ------------------------------
///
/// In order to not have to rewrite all of SILGen to avoid copies, NeXTCode has
...
void UseState::initializeLiveness(
  ...
  bool beginsInitialized = false;
  {
    ...
    if (auto *fArg = dyn_cast<SILFunctionArgument>(operand)) {
      switch (fArg->getArgumentConvention()) {
      case nextcode::SILArgumentConvention::Indirect_In:
      case nextcode::SILArgumentConvention::Indirect_In_Guaranteed:
      case nextcode::SILArgumentConvention::Indirect_Inout:
      case nextcode::SILArgumentConvention::Indirect_InoutAliasable:
      case nextcode::SILArgumentConvention::Indirect_In_CXX:
        ...
        break;
      case nextcode::SILArgumentConvention::Indirect_Out:
        llvm_unreachable("Should never have out addresses here");
      case nextcode::SILArgumentConvention::Direct_Owned:
      case nextcode::SILArgumentConvention::Direct_Unowned:
      case nextcode::SILArgumentConvention::Direct_Guaranteed:
      case nextcode::SILArgumentConvention::Pack_Inout:
      case nextcode::SILArgumentConvention::Pack_Guaranteed:
      case nextcode::SILArgumentConvention::Pack_Owned:
      case nextcode::SILArgumentConvention::Pack_Out:
...
shouldEmitPartialMutationErrorForType(SILType ty, NominalTypeDecl *nominal,
  ...
  if (nominal->getModuleContext() == fn->getModule().getNeXTCodeModule())
    return std::nullopt;
...
bool GlobalLivenessChecker::testInstVectorLiveness(
  ...
  for (auto takeInstAndValue : instsToTest) {
    ...
    while (auto *block = worklist.pop()) {
      ...
      NEXTCODE_DEFER { isLiveArray.clear(); };
...
bool MoveOnlyAddressCheckerPImpl::performSingleCheck(
  ...
  NEXTCODE_DEFER { diagnosticEmitter.clearUsesWithDiagnostic(); };
