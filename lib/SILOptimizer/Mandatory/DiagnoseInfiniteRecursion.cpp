...
static bool isRecursiveCall(FullApplySite applySite) {
  ...
  if (auto *CMI = dyn_cast<ClassMethodInst>(callee)) {
    ...
    if (classDecl && classDecl->getModuleContext() != module.getNeXTCodeModule())
      return false;
...
typedef nextcode::SmallSetVector<Invariants, 4> InvariantsSet;
...
SILTransform *nextcode::createDiagnoseInfiniteRecursion() {
