...
bool CheckerLivenessInfo::compute() {
  ...
  while (SILValue value = defUseWorklist.pop()) {
    ...
    NEXTCODE_DEFER { LLVM_DEBUG(llvm::dbgs() << "Finished Value: " << value); };
...
bool ConsumeOperatorCopyableValuesChecker::check() {
  ...
  while (!valuesToProcess.empty()) {
    ...
    NEXTCODE_DEFER { livenessInfo.clear(); };
...
SILTransform *nextcode::createConsumeOperatorCopyableValuesChecker() {
