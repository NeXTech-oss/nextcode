//===--- PassPipeline.cpp - NeXTCode Compiler SIL Pass Entrypoints -----------===//
//
/*
...
static void addMandatoryDiagnosticOptPipeline(SILPassPipelinePlan &P) {
  ...
  // NeXTCode os log APIs with C os_log ABIs.
  ...
  // Canonical nextcode requires all non cond_br critical edges to be split.
  P.addSplitNonCondBrCriticalEdges();

  // For embedded NeXTCode: Specialize generic class vtables.
  ...
  // Eliminate them via the DeadFunctionAndGlobalElimination in embedded NeXTCode
  // to avoid getting metadata/existential use errors in them. We don't want to
  // run this pass in regular NeXTCode: Even unused functions are expected to be
  // available in debug (-Onone) builds for debugging and development purposes.
  if (P.getOptions().EmbeddedNeXTCode) {
...
void addFunctionPasses(SILPassPipelinePlan &P,
  ...
  // in embedded NeXTCode mode, running the generic specializer might introduce
  // more generic calls from non-generic functions, which breaks the assumptions
  // of embedded NeXTCode.
  if (!P.getOptions().EmbeddedNeXTCode) {
...
//
// 1. Optimize the standard library NeXTCode module prior to serialization. This
...
static void addHighLevelFunctionPipeline(SILPassPipelinePlan &P) {
  ...
  // Skip EagerSpecializer on embedded NeXTCode, which already specializes
  // everything. Otherwise this would create metatype references for functions
  // with @_specialize attribute and those are incompatible with Emebdded NeXTCode.
  if (!P.getOptions().EmbeddedNeXTCode) {
...
static void addClosureSpecializePassPipeline(SILPassPipelinePlan &P) {
  ...
  // Specialize closure.
  if (P.getOptions().EnableExperimentalNeXTCodeBasedClosureSpecialization) {
    P.addExperimentalNeXTCodeBasedClosureSpecialization();
  } else {
...
SILPassPipelinePlan::getPerformancePassPipeline(const SILOptions &Options) {
  ...
  // stdlib.
  //
  // This also performs early OSSA based optimizations on *all* nextcode code.
...
SILPassPipelinePlan::getOnonePassPipeline(const SILOptions &Options) {
  ...
  // For embedded NeXTCode: CMO is used to serialize libraries.
  P.addCrossModuleOptimization();
  ...
  if (Options.EmbeddedNeXTCode) {
    // For embedded NeXTCode: Remove all unspecialized functions. This is important
