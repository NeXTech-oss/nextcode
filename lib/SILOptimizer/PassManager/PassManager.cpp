//===--- PassManager.cpp - NeXTCode Pass Manager -----------------------------===//
//
/*
...
#include "nextcode/SIL/DynamicCasts.h"
...
#include "nextcode/SILOptimizer/IPO/ClosureSpecializer.h"
...
#include "nextcode/SILOptimizer/Utils/OptimizerStatsUtils.h"
...
llvm::cl::opt<bool> DisableNeXTCodeVerification(
    "disable-nextcode-verification", llvm::cl::init(false),
    llvm::cl::desc("Disable verification which is implemented in the NeXTCodeCompilerSources"));
...
void nextcode::executePassPipelinePlan(SILModule *SM,
...
void SILPassManager::runPassOnFunction(unsigned TransIdx, SILFunction *F) {
  ...
  if ((SILVerifyBeforePass.end() != std::find_if(SILVerifyBeforePass.begin(),
    ...
    verifyAnalyses();
    runNeXTCodeFunctionVerification(F);
  }
  ...
  for (unsigned runIdx = 0; runIdx < numRepeats; runIdx++) {
    nextcodePassInvocation.startFunctionPassRun(SFT);

    // Run it!
    SFT->run();

    nextcodePassInvocation.finishedFunctionPassRun();
...
void SILPassManager::runModulePass(unsigned TransIdx) {
  ...
  if ((SILVerifyBeforePass.end() != std::find_if(SILVerifyBeforePass.begin(),
    ...
    verifyAnalyses();
    runNeXTCodeModuleVerification();
  }

  nextcodePassInvocation.startModulePassRun(SMT);
  ...
  assert(analysesUnlocked() && "Expected all analyses to be unlocked!");
  Mod->flushDeletedInsts();
  nextcodePassInvocation.finishedModulePassRun();
...
#define PASS(ID, TAG, NAME)                                                    \
  case PassKind::ID: {                                                         \
    SILTransform *T = nextcode::create##ID();                                     \
...
//===----------------------------------------------------------------------===//
//                           NeXTCodePassInvocation
//===----------------------------------------------------------------------===//

FixedSizeSlab *NeXTCodePassInvocation::allocSlab(FixedSizeSlab *afterSlab) {
...
FixedSizeSlab *NeXTCodePassInvocation::freeSlab(FixedSizeSlab *slab) {
...
BasicBlockSet *NeXTCodePassInvocation::allocBlockSet() {
...
void NeXTCodePassInvocation::freeBlockSet(BasicBlockSet *set) {
...
NodeSet *NeXTCodePassInvocation::allocNodeSet() {
...
void NeXTCodePassInvocation::freeNodeSet(NodeSet *set) {
...
OperandSet *NeXTCodePassInvocation::allocOperandSet() {
...
void NeXTCodePassInvocation::freeOperandSet(OperandSet *set) {
...
void NeXTCodePassInvocation::startModulePassRun(SILModuleTransform *transform) {
  assert(!this->function && !this->transform && "a pass is already running");
  this->function = nullptr;
  this->transform = transform;
}

void NeXTCodePassInvocation::startFunctionPassRun(SILFunctionTransform *transform) {
  assert(!this->transform && "a pass is already running");
  this->transform = transform;
  beginTransformFunction(transform->getFunction());
}

void NeXTCodePassInvocation::startInstructionPassRun(SILInstruction *inst) {
  assert(inst->getFunction() == function &&
         "running instruction pass on wrong function");
}

void NeXTCodePassInvocation::finishedModulePassRun() {
  endPass();
...
void NeXTCodePassInvocation::finishedFunctionPassRun() {
  endPass();
...
void NeXTCodePassInvocation::finishedInstructionPassRun() {
  endPass();
}

irgen::IRGenModule *NeXTCodePassInvocation::getIRGenModule() {
  return passManager->getIRGenModule();
}

void NeXTCodePassInvocation::endPass() {
  assert(allocatedSlabs.empty() && "StackList is leaking slabs");
...
void NeXTCodePassInvocation::beginTransformFunction(SILFunction *function) {
...
void NeXTCodePassInvocation::endTransformFunction() {
  assert(function && transform && "not running a pass");
...
void NeXTCodePassInvocation::beginVerifyFunction(SILFunction *function) {
  if (transform) {
...
void NeXTCodePassInvocation::endVerifyFunction() {
  assert(function);
  if (!transform) {
    assert(changeNotifications == SILAnalysis::InvalidationKind::Nothing &&
...
bool BridgedFunction::isTrapNoReturn() const {
  return nextcode::isTrapNoReturnFunction(getFunction());
}

bool BridgedFunction::isAutodiffVJP() const {
  return nextcode::isDifferentiableFuncComponent(
      getFunction(), nextcode::AutoDiffFunctionComponent::VJP);
}

NeXTCodeInt BridgedFunction::specializationLevel() const {
  return nextcode::getSpecializationLevel(getFunction());
}
...
#include "nextcode/SILOptimizer/OptimizerBridgingImpl.h"
#endif
...
static const irgen::TypeInfo &getTypeInfoOfBuiltin(nextcode::SILType type, irgen::IRGenModule &IGM) {
  SILType lowered = IGM.getLoweredType(nextcode::Lowering::AbstractionPattern::getOpaque(), type.getASTType());
  return IGM.getTypeInfo(lowered);
}

static NeXTCodeInt integerValueFromConstant(llvm::Constant *c, NeXTCodeInt add = 0) {
...
NeXTCodeInt BridgedPassContext::getStaticSize(BridgedType type) const {
...
NeXTCodeInt BridgedPassContext::getStaticAlignment(BridgedType type) const {
  irgen::IRGenModule *IGM = invocation->getIRGenModule();
  if (!IGM)
    return -1;
  auto &ti = getTypeInfoOfBuiltin(type.unbridged(), *IGM);
...
NeXTCodeInt BridgedPassContext::getStaticStride(BridgedType type) const {
  irgen::IRGenModule *IGM = invocation->getIRGenModule();
  if (!IGM)
    return -1;
  auto &ti = getTypeInfoOfBuiltin(type.unbridged(), *IGM);
...
nextcode::SILVTable * BridgedPassContext::specializeVTableForType(BridgedType type, BridgedFunction function) const {
...
BridgedOwnedString BridgedPassContext::mangleWithDeadArgs(const NeXTCodeInt * _Nullable deadArgs,
  ...
  for (NeXTCodeInt idx = 0; idx < numDeadArgs; idx++) {
...
BridgedOwnedString BridgedPassContext::mangleWithClosureArgs(
  ...
  llvm::SmallVector<nextcode::SILValue, 16> closureArgsStorage;
  auto closureArgs = bridgedClosureArgs.getValues(closureArgsStorage);
  auto closureArgIndices = bridgedClosureArgIndices.unbridged<NeXTCodeInt>();
...
OptionalBridgedFunction BridgedPassContext::lookupStdlibFunction(BridgedStringRef name) const {
  ...
  mod->getASTContext().lookupInNeXTCodeModule(name.unbridged(), results);
  if (results.size() != 1)
    return {nullptr};
...
bool FullApplySite_canInline(BridgedInstruction apply) {
  return nextcode::SILInliner::canInlineApplySite(
      nextcode::FullApplySite(apply.unbridged()));
}
...
BridgedDynamicCastResult classifyDynamicCastBridged(BridgedType sourceTy, BridgedType destTy,
  ...
  return static_cast<BridgedDynamicCastResult>(
    classifyDynamicCast(function.getFunction()->getModule().getNeXTCodeModule(),
...
// Need to put ClonerWithFixedLocation into namespace nextcode to forward reference
...
void SILPassManager::runNeXTCodeFunctionVerification(SILFunction *f) {
  ...
  if (DisableNeXTCodeVerification)
    return;

  getNeXTCodePassInvocation()->beginVerifyFunction(f);
  verifyFunctionFunction({getNeXTCodePassInvocation()}, {f});
  getNeXTCodePassInvocation()->endVerifyFunction();
}

void SILPassManager::runNeXTCodeModuleVerification() {
...
void BridgedSpecializationCloner::cloneFunctionBody(BridgedFunction originalFunction, BridgedBasicBlock clonedEntryBlock, BridgedValueArray clonedEntryBlockArgs) const {
  llvm::SmallVector<nextcode::SILValue, 16> clonedEntryBlockArgsStorage;
...
void BridgedBuilder::destroyCapturedArgs(BridgedInstruction partialApply) const {
  if (auto *pai = llvm::dyn_cast<PartialApplyInst>(partialApply.unbridged()); pai->isOnStack()) {
    auto b = unbridged();
    return nextcode::insertDestroyOfCapturedArguments(pai, b); 
  } else {
