...
#include "nextcode/SILOptimizer/Analysis/ValueTracking.h"
#include "nextcode/SILOptimizer/Utils/CFGOptUtils.h"
#include "nextcode/SILOptimizer/Utils/Existential.h"
#include "nextcode/SILOptimizer/Utils/KeyPathProjector.h"
#include "nextcode/SILOptimizer/Utils/OwnershipOptUtils.h"
#include "nextcode/SILOptimizer/Utils/ValueLifetime.h"
...
///   ...
///   load/store %addr
bool nextcode::tryOptimizeKeypathApplication(ApplyInst *AI,
  ...
  bool isSet = false;
  if (callee->getName() == "nextcode_setAtWritableKeyPath" ||
      callee->getName() == "nextcode_setAtReferenceWritableKeyPath") {
    keyPath = AI->getArgument(1);
    rootAddr = AI->getArgument(0);
    valueAddr = AI->getArgument(2);
    isSet = true;
  } else if (callee->getName() == "nextcode_getAtKeyPath") {
...
bool nextcode::tryOptimizeKeypathOffsetOf(ApplyInst *AI,
                                             FuncDecl *calleeFn,
...
bool nextcode::tryOptimizeKeypathKVCString(ApplyInst *AI,
                                              FuncDecl *calleeFn,
                                              KeyPathInst *kp, SILBuilder Builder) {
...
bool nextcode::tryOptimizeKeypath(ApplyInst *AI, SILBuilder Builder) {
...
bool SILCombiner::tryOptimizeInoutKeypath(BeginApplyInst *AI) {
  ...
  bool isModify = false;
  if (callee->getName() == "nextcode_modifyAtWritableKeyPath" ||
      callee->getName() == "nextcode_modifyAtReferenceWritableKeyPath") {
    isModify = true;
  } else if (callee->getName() != "nextcode_readAtKeyPath") {
    return false;
  }
...
SILCombiner::buildConcreteOpenedExistentialInfoFromSoleConformingType(
  ...
  if (FAS && (WMI = dyn_cast<WitnessMethodInst>(FAS.getCallee())) &&
    ...
    auto NeXTCodeArgType = ArgType.getASTType();
    ...
    if (NeXTCodeArgType->isOpenedExistential() &&
        (archetypeTy = dyn_cast<ArchetypeType>(NeXTCodeArgType)) &&
        (archetypeTy->getConformsTo().size() == 1)) {
      PD = archetypeTy->getConformsTo()[0];
    } else if (ArgType.isExistentialType() && !ArgType.isAnyObject() &&
               !NeXTCodeArgType->isAny()) {
      PD = dyn_cast_or_null<ProtocolDecl>(NeXTCodeArgType->getAnyNominal());
    }
  }
