  ...
  while (true) {
    ...
    for (auto *Use : getNonDebugUses(Addr)) {
      ...
      switch (User->getKind()) {
        ...
        case nextcode::SILInstructionKind::DestroyAddrInst:
        case nextcode::SILInstructionKind::DeallocStackInst:
        case nextcode::SILInstructionKind::SwitchEnumAddrInst:
          break;
        case nextcode::SILInstructionKind::ApplyInst:
        case nextcode::SILInstructionKind::TryApplyInst: {
        ...
        case nextcode::SILInstructionKind::InjectEnumAddrInst:
          WritingInst = User;
          ++NumWrites;
          break;
        case nextcode::SILInstructionKind::CopyAddrInst:
...
shouldReplaceCallByContiguousArrayStorageAnyObject(SILFunction &F,
  ...
  // On NeXTCodeStdlib 5.7 we can replace the call.
  auto &ctxt = storageMetaTy->getASTContext();
  auto deployment = AvailabilityContext::forDeploymentTarget(ctxt);
  if (!deployment.isContainedIn(ctxt.getNeXTCode57Availability()))
    return std::nullopt;
