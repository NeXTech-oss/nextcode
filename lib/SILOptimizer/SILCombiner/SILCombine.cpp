...
#include "nextcode/SILOptimizer/Utils/CanonicalizeBorrowScope.h"
...
#include "nextcode/SILOptimizer/Utils/DebugOptUtils.h"
...
SILCombiner::SILCombiner(SILFunctionTransform *trans,
  ...
  nextcodePassInvocation(trans->getPassManager(),
...
/// Runs a NeXTCode instruction pass.
void SILCombiner::runNeXTCodeInstructionPass(SILInstruction *inst,
                              void (*runFunction)(BridgedInstructionPassCtxt)) {
  nextcodePassInvocation.startInstructionPassRun(inst);
  runFunction({ {inst->asSILNode()}, {&nextcodePassInvocation} });
  nextcodePassInvocation.finishedInstructionPassRun();
}

/// Registered briged instruction pass run functions.
static llvm::StringMap<BridgedInstructionPassRunFn> nextcodeInstPasses;
...
void SILCombine_registerInstructionPass(BridgedStringRef instClassName,
  ...
  nextcodeInstPasses[instClassName.unbridged()] = runFn;
  passesRegistered = true;
}

#define NEXTCODE_SILCOMBINE_PASS(INST) \
SILInstruction *SILCombiner::visit##INST(INST *inst) {                     \
  ...
  if (!runFunction) {                                                      \
    runFunction = nextcodeInstPasses[#INST];                                  \
    if (!runFunction) {                                                    \
      if (passesRegistered) {                                              \
        llvm::errs() << "NeXTCode pass " << #INST << " is not registered\n";  \
        abort();                                                           \
      } else {                                                             \
        return nullptr;                                                    \
      }                                                                    \
  ...
  runNeXTCodeInstructionPass(inst, runFunction);                              \
  return nullptr;                                                          \
}                                                                          \
...
SILTransform *nextcode::createSILCombine() {
  return new SILCombine();
}

//===----------------------------------------------------------------------===//
//                          NeXTCodeFunctionPassContext
//===----------------------------------------------------------------------===//

void NeXTCodePassInvocation::eraseInstruction(SILInstruction *inst) {
  if (silCombiner) {
    silCombiner->eraseInstFromFunction(*inst);
  } else {
    nextcode::salvageDebugInfo(inst);
