...
bool CrossModuleOptimization::isPackageOrPublic(SILLinkage linkage) {
  if (isPackageCMOEnabled(M.getNeXTCodeModule()))
...
bool CrossModuleOptimization::isSerializedWithRightKind(const SILModule &mod,
  ...
  // optimization.
  return isPackageCMOEnabled(mod.getNeXTCodeModule()) ? f->isAnySerialized()
                                                   : f->isSerialized();
}
bool CrossModuleOptimization::isSerializedWithRightKind(const SILModule &mod,
                                      SILGlobalVariable *g) {
  return isPackageCMOEnabled(mod.getNeXTCodeModule()) ? g->isAnySerialized()
                                                   : g->isSerialized();
}
SerializedKind_t CrossModuleOptimization::getRightSerializedKind(const SILModule &mod) {
  return isPackageCMOEnabled(mod.getNeXTCodeModule()) ? IsSerializedForPackage
                                                   : IsSerialized;
}
...
static bool isSerializeCandidate(SILFunction *F, SILOptions options) {
  ...
  if (isPackageCMOEnabled(F->getModule().getNeXTCodeModule()))
...
bool CrossModuleOptimization::isReferenceSerializeCandidate(SILGlobalVariable *G,
                                                            SILOptions options) {
  if (isPackageCMOEnabled(G->getModule().getNeXTCodeModule())) {
...
static bool couldBeLinkedStatically(DeclContext *funcCtxt, SILModule &module) {
  ...
  if (module.getNeXTCodeModule() == funcModule)
    return false;
...
bool CrossModuleOptimization::canUseFromInline(DeclContext *declCtxt) {
  ...
  if (!M.getNeXTCodeModule()->canBeUsedForCrossModuleOptimization(declCtxt))
    return false;
...
bool CrossModuleOptimization::shouldSerialize(SILFunction *function) {
  ...
  if (!M.getNeXTCodeModule()->serializePackageEnabled()) {
...
void CrossModuleOptimization::serializeInstruction(SILInstruction *inst,
  ...
  if (auto *GAI = dyn_cast<GlobalAddrInst>(inst)) {
    ...
    if (!hasPublicOrPackageVisibility(
            global->getLinkage(),
            M.getNeXTCodeModule()->serializePackageEnabled())) {
...
void CrossModuleOptimization::makeDeclUsableFromInline(ValueDecl *decl) {
  ...
  if (M.getNeXTCodeModule() != decl->getDeclContext()->getParentModule())
    return;
...
class CrossModuleOptimizationPass: public SILModuleTransform {
  void run() override {
    auto &M = *getModule();
    if (M.getNeXTCodeModule()->isResilient() &&
        !M.getNeXTCodeModule()->serializePackageEnabled())
      return;
    if (!M.isWholeModule())
      return;
    ...
    switch (M.getOptions().CMOMode) {
      case nextcode::CrossModuleOptimizationMode::Off:
        break;
      case nextcode::CrossModuleOptimizationMode::Default:
        conservative = true;
        break;
      case nextcode::CrossModuleOptimizationMode::Aggressive:
        conservative = false;
        break;
      case nextcode::CrossModuleOptimizationMode::Everything:
        everything = true;
        break;
    }

    if (!everything &&
        M.getOptions().CMOMode == nextcode::CrossModuleOptimizationMode::Off) {
      return;
    }
...
SILTransform *nextcode::createCrossModuleOptimization() {
