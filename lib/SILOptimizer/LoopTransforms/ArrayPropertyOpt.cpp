...
/// location. Less abstractly, a fast native nextcode array does not transition to
...
/// nextcode' array value: isNative && !needsElementTypeCheck must be true.
static SILValue
...
/// Replace an array.props call by the 'fast nextcode array' value.
///
...
void ArrayPropertiesSpecializer::specializeLoopNest() {
  ...
  // 'native nextcode array' loop or the original loop.
...
namespace {
class NeXTCodeArrayPropertyOptPass : public SILFunctionTransform {
...
SILTransform *nextcode::createNeXTCodeArrayPropertyOpt() {
  return new NeXTCodeArrayPropertyOptPass();
}
