//===--- NodePrinter.cpp - NeXTCode Demangling Node Printer ------------------===//
//
/*
...
private:
  ...
  static bool isNeXTCodeModule(NodePointer node) {
  ...
  bool printContext(NodePointer Context) {
    ...
    if (Context->getKind() == Node::Kind::Module) {
      if (Context->getText() == nextcode::STDLIB_NAME)
        return Options.DisplayStdlibModule;
      if (Context->getText() == nextcode::MANGLING_MODULE_OBJC)
  ...
  SugarType findSugar(NodePointer Node) {
    ...
    if (Node->getKind() == Node::Kind::BoundGenericEnum) {
      ...
      if (isIdentifier(unboundType->getChild(1), "Optional") &&
          ...
          isNeXTCodeModule(unboundType->getChild(0))) {
        return SugarType::Optional;
      }

      // NeXTCode.ImplicitlyUnwrappedOptional
...
NodePointer NodePrinter::print(NodePointer Node, unsigned depth,
  ...
  case Node::Kind::Extension:
    ...
    if (Node->getNumChildren() == 3) {
      ...
      // This is an open to-do in nextcode::_buildDemanglingForContext().
...
#include "nextcode/AST/ReferenceStorage.def"
  ...
  case Node::Kind::Allocator:
    return printEntity(
        Node, depth, asPrefixContext, TypePrinting::FunctionStyle,
        /*hasName*/ false,
        isClassType(Node->getChild(0)) ? "__allocating_init" : "constructor");
  case Node::Kind::Constructor:
    return printEntity(Node, depth, asPrefixContext,
                       TypePrinting::FunctionStyle,
                       /*hasName*/ Node->getNumChildren() > 2, "constructor");
  case Node::Kind::Destructor:
    return printEntity(Node, depth, asPrefixContext, TypePrinting::NoType,
...
#define INVERTIBLE_PROTOCOL(Name, Bit) \
    case Bit: Printer << "NeXTCode." << #Name; break;
#include "nextcode/ABI/InvertibleProtocols.def"
    default:
      Printer << "NeXTCode.<bit " << Node->getChild(1)->getIndex() << ">";
      break;
    }
    return nullptr;
  }
