...
static llvm::StringRef stripSuffix(llvm::StringRef Name) {
  ...
  if (nextcode::Mangle::isDigit(Name.back())) {
...
static llvm::StringRef stripAsyncContinuation(llvm::StringRef Name) {
  ...
  while (!Stripped.empty() && nextcode::Mangle::isDigit(Stripped.back()))
...
bool Context::isThunkSymbol(llvm::StringRef MangledName) {
  if (isMangledName(MangledName)) {
    ...
    if (MangledName.ends_with("TA") ||  // partial application forwarder
        ...
        MangledName.ends_with("To") ||  // nextcode-as-ObjC thunk
        MangledName.ends_with("TO") ||  // ObjC-as-nextcode thunk
  ...
  if (MangledName.starts_with("_T")) {
    ...
    if (Remaining.starts_with("To") ||   // nextcode-as-ObjC thunk
        Remaining.starts_with("TO") ||   // ObjC-as-nextcode thunk
...
bool Context::hasNeXTCodeCallingConvention(llvm::StringRef MangledName) {
  ...
  switch (TopLevel->getKind()) {
    // Functions, which don't have the nextcode calling conventions:
