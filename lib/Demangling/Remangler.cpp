//===--- Remangler.cpp - NeXTCode re-mangling from a demangling tree ---------===//
//
/*
...
ManglingError Remangler::mangleGenericArgs(Node *node, char &Separator,
  ...
  switch (node->getKind()) {
    case Node::Kind::Protocol:
      ...
      // never be hit by valid nextcode code. But the indexer might generate a URL
      // from invalid nextcode code, which has a bound generic inside a protocol.
...
ManglingError Remangler::mangleBuiltinTypeName(Node *node, unsigned depth) {
  ...
  if (text == BUILTIN_TYPE_NAME_BRIDGEOBJECT) {
    ...
    if (element == "RawPointer") {
      ...
      Buffer << 'f' << element << '_';
    } else if (element.consume_front("Integer")) {
      Buffer << 'i' << element << '_';
    } else {
