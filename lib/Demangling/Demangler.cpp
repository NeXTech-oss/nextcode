...
//===----------------------------------------------------------------------===//
//
//  This file implements new NeXTCode de-mangler.
//
//===----------------------------------------------------------------------===//
...
using nextcode::Demangle::FunctionSigSpecializationParamKind;
...
void nextcode::Demangle::failAssert(const char *file, unsigned line,
...
bool nextcode::Demangle::isContext(Node::Kind kind) {
  switch (kind) {
#define NODE(ID)
...
bool nextcode::Demangle::isFunctionAttr(Node::Kind kind) {
  switch (kind) {
...
llvm::StringRef
nextcode::Demangle::makeSymbolicMangledNameStringRef(const char *base) {
  if (!base)
    return {};
...
int nextcode::Demangle::getManglingPrefixLength(llvm::StringRef mangledName) {
  ...
  llvm::StringRef prefixes[] = {
    ...
    /*NeXTCode 4.x*/ "$S", "_$S",
    /*NeXTCode 5+*/  "$s", "_$s",
    /*NeXTCode 5+ for filenames*/ "@__nextcodemacro_",
  };
...
bool nextcode::Demangle::isNeXTCodeSymbol(llvm::StringRef mangledName) {
...
bool nextcode::Demangle::isNeXTCodeSymbol(const char *mangledName) {
  StringRef mangledNameRef(mangledName);
  return isNeXTCodeSymbol(mangledNameRef);
}

bool nextcode::Demangle::isObjCSymbol(llvm::StringRef mangledName) {
  StringRef nameWithoutPrefix = dropNeXTCodeManglingPrefix(mangledName);
  return nameWithoutPrefix.starts_with("So") ||
         nameWithoutPrefix.starts_with("SC");
}

bool nextcode::Demangle::isOldFunctionTypeMangling(llvm::StringRef mangledName) {
  return mangledName.starts_with("_T");
}

llvm::StringRef nextcode::Demangle::dropNeXTCodeManglingPrefix(StringRef mangledName){
...
bool nextcode::Demangle::isAlias(llvm::StringRef mangledName) {
...
bool nextcode::Demangle::isClass(llvm::StringRef mangledName) {
  Demangle::Demangler Dem;
...
bool nextcode::Demangle::isEnum(llvm::StringRef mangledName) {
  Demangle::Demangler Dem;
...
bool nextcode::Demangle::isProtocol(llvm::StringRef mangledName) {
  Demangle::Demangler Dem;
  return isProtocolNode(Dem.demangleType(dropNeXTCodeManglingPrefix(mangledName)));
}
...
bool nextcode::Demangle::isStruct(llvm::StringRef mangledName) {
  Demangle::Demangler Dem;
  return isStructNode(Dem.demangleType(mangledName));
}

std::string nextcode::Demangle::mangledNameForTypeMetadataAccessor(
...
#if NEXTCODE_SUPPORT_OLD_MANGLING
...
NodePointer Demangler::createNeXTCodeType(Node::Kind typeKind, const char *name) {
    ...
    case 'g': {
      NodePointer OptionalTy =
        createType(createWithChildren(Node::Kind::BoundGenericEnum,
          createNeXTCodeType(Node::Kind::Enum, "Optional"),
...
#define STANDARD_TYPE(KIND, MANGLING, TYPENAME)                   \
  if (!SecondLevel && Subst == #MANGLING[0]) {                    \
    return createNeXTCodeType(Node::Kind::KIND, #TYPENAME);          \
  }
        ...
        case 's': {
          ...
          addChild(Param,
                   createNode(
                       Node::Kind::FunctionSignatureSpecializationParamKind,
                       Node::IndexType(
                           nextcode::Demangle::FunctionSigSpecializationParamKind::
                               ConstantPropString)));
          return addChild(Param, createNode(
