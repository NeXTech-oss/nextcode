...
// accepted when it goes through the nextcode clang importer.
static std::string adjustClangTriple(StringRef TripleStr) {
  ...
  case llvm::Triple::SubArchType::ARMSubArch_v4t:
    OS << "armv4t"; break;
  default:
    // Adjust i386-macosx to x86_64 because there is no NeXTCode stdlib for i386.
...
bool ide::initCompilerInvocation(
  ...
  // Derive 'codec' path from 'nextcode-frontend' path (nextcodeExecutablePath).
