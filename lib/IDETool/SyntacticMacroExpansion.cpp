...
#include "nextcode/IDETool/SyntacticMacroExpansion.h"
...
SyntacticMacroExpansion::getInstance(ArrayRef<const char *> args,
  ...
  bool failed = instance->setup(NeXTCodeExecutablePath, args, Plugins, error);
  if (failed)
    return nullptr;
...
bool SyntacticMacroExpansionInstance::setup(
    StringRef NeXTCodeExecutablePath, ArrayRef<const char *> args,
    std::shared_ptr<PluginRegistry> plugins, std::string &error) {
  SmallString<256> driverPath(NeXTCodeExecutablePath);
  llvm::sys::path::remove_filename(driverPath);
  llvm::sys::path::append(driverPath, "codec");
  ...
  bool hadError = driver::getSingleFrontendInvocationFromDriverArguments(
      ...
      [&](ArrayRef<const char *> frontendArgs) {
        return invocation.parseArgs(
            ...
            /*workingDirectory=*/{}, NeXTCodeExecutablePath);
      },
      /*ForceNoOutput=*/true);
  if (hadError) {
...
expandFreestandingMacro(MacroDecl *macro,
  ...
  SourceFile *expandedSource =
      nextcode::evaluateFreestandingMacro(expansion, discriminator);
  if (expandedSource)
...
expandAttachedMacro(MacroDecl *macro, CustomAttr *attr, Decl *attachedDecl) {
  ...
  auto evaluate = [&](Decl *target, bool passParent, MacroRole role) {
    ...
    SourceFile *expandedSource = nextcode::evaluateAttachedMacro(
