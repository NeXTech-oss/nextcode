...
///
/// \file
/// This file implements NeXTCode markup AST nodes.
///
//===----------------------------------------------------------------------===//
...
Document *Document::create(MarkupContext &MC,
                           ArrayRef<nextcode::markup::MarkupASTNode *> Children) {
...
#include "nextcode/Markup/SimpleFields.def"
...
#include "nextcode/Markup/ASTNodes.def"
  }
...
void nextcode::markup::printInlinesUnder(const MarkupASTNode *Node,
                                     llvm::raw_ostream &OS,
                                     bool PrintDecorators) {
  auto printChildren = [](const ArrayRef<const MarkupASTNode *> Children,
                          llvm::raw_ostream &OS) {
    for (auto Child = Children.begin(); Child != Children.end(); ++Child)
      nextcode::markup::printInlinesUnder(*Child, OS);
  };

  switch (Node->getKind()) {
  case nextcode::markup::ASTNodeKind::HTML: {
    auto H = cast<HTML>(Node);
    OS << H->getLiteralContent();
    break;
  }
  case nextcode::markup::ASTNodeKind::InlineHTML: {
    auto IH = cast<InlineHTML>(Node);
    OS << IH->getLiteralContent();
    break;
  }
  case nextcode::markup::ASTNodeKind::HRule:
    OS << '\n';
    break;
  case nextcode::markup::ASTNodeKind::Text: {
    auto T = cast<Text>(Node);
    OS << T->getLiteralContent();
    break;
  }
  case nextcode::markup::ASTNodeKind::SoftBreak:
    OS << ' ';
    break;
  case nextcode::markup::ASTNodeKind::LineBreak:
    OS << '\n';
    break;
  case nextcode::markup::ASTNodeKind::Code: {
  ...
  case nextcode::markup::ASTNodeKind::CodeBlock: {
  ...
  case nextcode::markup::ASTNodeKind::Emphasis: {
  ...
  case nextcode::markup::ASTNodeKind::Strong: {
...
nextcode::markup::MarkupASTNode *nextcode::markup::createSimpleField(
    MarkupContext &MC,
    StringRef Tag,
    ArrayRef<nextcode::markup::MarkupASTNode *> Children) {
  if (false) {
...
bool nextcode::markup::isAFieldTag(StringRef Tag) {
  if (false) {
...
void nextcode::markup::dump(const MarkupASTNode *Node, llvm::raw_ostream &OS,
                        unsigned indent) {
  auto dumpChildren = [](const ArrayRef<const MarkupASTNode *> Children,
                         llvm::raw_ostream &OS, unsigned indent) {
    OS << '\n';
    for (auto Child = Children.begin(); Child != Children.end(); ++Child) {
      nextcode::markup::dump(*Child, OS, indent + 1);
  ...
  OS << '(';
  switch (Node->getKind()) {
  case nextcode::markup::ASTNodeKind::Document: {
    OS << "Document: Children=" << Node->getChildren().size();
    dumpChildren(Node->getChildren(), OS, indent + 1);
    break;
  }
  case nextcode::markup::ASTNodeKind::InlineAttributes: {
  ...
  case nextcode::markup::ASTNodeKind::BlockQuote: {
    OS << "BlockQuote: Children=" << Node->getChildren().size();
    dumpChildren(Node->getChildren(), OS, indent + 1);
    break;
  }
  case nextcode::markup::ASTNodeKind::List: {
  ...
  case nextcode::markup::ASTNodeKind::Item: {
  ...
  case nextcode::markup::ASTNodeKind::Paragraph: {
    OS << "Paragraph: Children=" << Node->getChildren().size();
    dumpChildren(Node->getChildren(), OS, indent + 1);
    break;
  }
  case nextcode::markup::ASTNodeKind::Header: {
  ...
  case nextcode::markup::ASTNodeKind::Link: {
  ...
  case nextcode::markup::ASTNodeKind::Image: {
  ...
  case nextcode::markup::ASTNodeKind::ParamField: {
...
#define MARKUP_SIMPLE_FIELD(Id, Keyword, XMLKind) \
  case nextcode::markup::ASTNodeKind::Id: { \
