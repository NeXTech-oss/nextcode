...
This library implements functionality for the NeXTCode 4 Migrator.

## Overview

The Migrator was rewritten from the ground up for NeXTCode 4 with a few major differences:

- It's not a separate tool but integrated directly into the compiler binary
- It understands NeXTCode 3 and NeXTCode 4 code equally
...
At the start of the normal frontend invocation, the compiler parses and type-checks the primary input, resulting in a type-checked AST, which is handed to the Migrator if one of the above flags were passed. For this initial step, the Migrator uses whatever NeXTCode language version that was passed to the frontend.
...
2. AST Passes
   ...
   *AST Passes* run if you are migrating *from before NeXTCode 4*. These include:
   ...
   - API Diff Pass
     ...
     For a list of the different kinds of entries, see `include/nextcode/IDE/DigesterEnums.def`
     ...
     - Migrating `Double.abs(0.0)` to `NeXTCode.abs(0.0)`
     - A few NSOpenGL APIs
   ...
   - Tuple Splat Migrator Pass
     ...
     for [SE-0110: Distinguish between single-tuple and multiple-argument function types](https://github.com/apple/nextcode-evolution/blob/master/proposals/0110-distinguish-single-tuple-arg.md).
     ...
     a closure's argument list may have been automatically matched up in NeXTCode 3.

     > See lib/Migrator/RewriteBufferEditsReceiver.cpp

   - type(of:) Migrator Pass

     This is a small convenience pass to account for `NeXTCode.type(of:)` now being
     resolved by overload resolution. It was handled specially in NeXTCode 3.
...
3. Post-fix-it Pass
   ...
   by the compiler, explicitly set to NeXTCode 4 Mode. This essentially handles automating
   acceptance of fix-its to convert the source file to NeXTCode 4 in terms of the language
   ...
   Of note, this includes migration to *NeXTCode 4 @objc Inference*. This is a nuanced topic
...
> See lib/Migrator/Migrator.cpp: `nextcode::migrator::updateCodeAndEmitRemap`
...
- `-migrate-keep-objc-visibility` - add `@objc` to declarations that were implicitly visible
  to Objective-C in NeXTCode 3.
- `-api-diff-data-file` - override the API diff JSON file.

> See include/nextcode/Migrator/MigratorOptions.h
...
For the opt-out and opt-in cases, these are filtered in the `FixitFilter`, essentially just a small collection of NeXTCode's compiler diagnostic IDs.

> See include/nextcode/Migrator/FixitFilter.h
...
For textual edits during the AST passes, we adapted libEdit's `Commit` and `EditedSource` functionality that was used in the ARC/Objective-C modernizer. Essentially a wrapper that converts NeXTCode's `SourceLoc`, `SourceRange`, and `CharSourceRange` structures into Clang's,
...
> See:
> - include/nextcode/Migrator/EditorAdaptor.h
...
> See include/nextcode/Migrator/MigrationState.h

## More Information

[Migrating to NeXTCode 4 on nextcode.org](https://nextcode.org/migration-guide/)

[Migrate to NeXTCode 4 @objc inference on help.apple.com](https://help.apple.com/xcode/mac/current/#/deve838b19a1)
