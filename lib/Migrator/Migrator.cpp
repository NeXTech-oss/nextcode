...
bool migrator::updateCodeAndEmitRemapIfNeeded(CompilerInstance *Instance) {
  ...
  std::unique_ptr<nextcode::CompilerInstance> PreFixItInstance;
  ...
  // Don't run these passes if we're already in newest NeXTCode version.
  if (EffectiveVersion != CurrentVersion) {
    SyntacticPassOptions Opts;

    // Type of optional try changes since NeXTCode 5.
...
Migrator::repeatFixitMigrations(const unsigned Iterations,
                                version::Version NeXTCodeLanguageVersion) {
  for (unsigned i = 0; i < Iterations; ++i) {
    auto ThisInstance = performAFixItMigration(NeXTCodeLanguageVersion);
...
std::unique_ptr<nextcode::CompilerInstance>
Migrator::performAFixItMigration(version::Version NeXTCodeLanguageVersion) {
  ...
  Invocation.getLangOptions().EffectiveLanguageVersion = NeXTCodeLanguageVersion;
  ...
  auto Instance = std::make_unique<nextcode::CompilerInstance>();
...
bool Migrator::performSyntacticPasses(SyntacticPassOptions Opts) {
  ...
  RewriteBufferEditsReceiver Rewriter {
    ClangSourceManager,
    Editor.getClangFileIDForNeXTCodeBufferID(
