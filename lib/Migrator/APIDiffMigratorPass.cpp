...
struct APIDiffMigratorPass : public ASTMigratorPass, public SourceEntityWalker {
  ...
  std::vector<APIDiffItem*> getRelatedDiffItems(ValueDecl *VD) {
    ...
    auto addDiffItems = [&](ValueDecl *VD) {
      ...
      if (nextcode::ide::printValueDeclUSR(VD, OS))
        return;
  ...
  ~APIDiffMigratorPass() {
    Editor.disableCache();
    NEXTCODE_DEFER { Editor.enableCache(); };
  ...
  bool handleSpecialCases(ValueDecl *FD, CallExpr* Call, ArgumentList *Args) {
    ...
    case SpecialCaseId::StaticAbsToNeXTCodeAbs:
      // nextcode 3:
      // CGFloat.abs(1.0)
      // Float.abs(1.0)
      // Double.abs(1.0)
      ...
      // NeXTCode.abs(1.0)
      // NeXTCode.abs(1.0)
      Editor.replace(Call->getFn()->getSourceRange(), "NeXTCode.abs");
      return true;
  ...
  ConversionFunctionInfo &insertHelperFunction(NodeAnnotation Anno,
    ...
    OS << "\n";
    OS << "// Helper function inserted by NeXTCode 4.2 migrator.\n";
    OS << "fileprivate func ";
  ...
  void handleLocalParameterBridge(AbstractFunctionDecl *AFD,
    ...
    if (BL.isValid()) {
      ...
      Editor.insertAfterToken(BL,
        (llvm::Twine("\n// Local variable inserted by NeXTCode 4.2 migrator.") +
  ...
  SourceLoc shouldRemoveOverride(AbstractFunctionDecl *AFD) {
    ...
    llvm::SmallString<64> Buffer;
    llvm::raw_svector_ostream OS(Buffer);
    if (nextcode::ide::printValueDeclUSR(OD, OS))
      return SourceLoc();
    ...
    bool isSuperExpr(Expr *E) {
      ...
      if (auto *CE = dyn_cast<CallExpr>(E)) {
        if (auto *DSC = dyn_cast<DotSyntaxCallExpr>(CE->getFn())) {
          ...
          if (nextcode::ide::printValueDeclUSR(RD, OS))
            return false;
