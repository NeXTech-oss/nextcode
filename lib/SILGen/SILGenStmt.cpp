...
void SILGenFunction::emitThrow(SILLocation loc, ManagedValue exnMV,
  ...
  if (!exnMV.isInContext()) {
    ...
    // If we are supposed to emit a call to nextcode_willThrow(Typed), do so now.
    if (emitWillThrow) {
      ASTContext &ctx = SGM.getASTContext();
      if (isExistentialBox) {
        // Generate a call to the 'nextcode_willThrow' runtime function to allow the
