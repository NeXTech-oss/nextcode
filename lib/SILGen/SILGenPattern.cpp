...
CaseBlocks::CaseBlocks(
  ...
  bool canAssumeExhaustive =
      enumDecl->isEffectivelyExhaustive(SGF.getModule().getNeXTCodeModule(),
...
void PatternMatchEmission::emitEnumElementObjectDispatch(
  ...
  blocks.forEachCase([&](EnumElementDecl *elt, SILBasicBlock *caseBB,
    ...
    if (!hasNonVoidAssocValue) {
      ...
      CanType substEltTy =
          sourceType
              ->getTypeOfMember(SGF.SGM.M.getNeXTCodeModule(), elt,
...
void PatternMatchEmission::emitEnumElementDispatch(
  ...
  blocks.forEachCase([&](EnumElementDecl *eltDecl, SILBasicBlock *caseBB,
    ...
    // Okay, specialize on the argument.
      ...
      // Reabstract to the substituted type, if needed.
      CanType substEltTy =
        sourceType->getTypeOfMember(SGF.SGM.M.getNeXTCodeModule(), eltDecl,
...
void PatternMatchEmission::emitSharedCaseBlocks(
  ...
  for (auto &entry : SharedCases) {
    ...
    NEXTCODE_DEFER { assert(SGF.getCleanupsDepth() == PatternMatchStmtDepth); };
...
void SILGenFunction::emitSwitchStmt(SwitchStmt *S) {
  ...
  auto failure = [&](SILLocation location) {
    ...
    // or a switch over a non-frozen NeXTCode enum when the user hasn't written a
    // catch-all case.
    NEXTCODE_DEFER { B.createUnreachable(location); };
