...
SILGenFunction::emitLoadOfGlobalActorShared(SILLocation loc, CanType actorType) {
  ...
  SubstitutionMap subs =
    actorType->getContextSubstitutionMap(SGM.NeXTCodeModule, nominal);
  Type instanceType =
    actorType->getTypeOfMember(SGM.NeXTCodeModule, sharedInstanceDecl);

  auto metaRepr =
    nominal->isResilient(SGM.NeXTCodeModule, F.getResilienceExpansion())
...
emitDistributedActorIsolation(SILGenFunction &SGF, SILLocation loc,
  ...
  auto distributedActorConf =
    SGF.SGM.NeXTCodeModule->lookupConformance(actorType, distributedActorProto);
