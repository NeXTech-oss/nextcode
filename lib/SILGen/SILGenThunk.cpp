...
SILFunction *SILGenModule::getDynamicThunk(SILDeclRef constant,
  ...
  if (F->empty()) {
    ...
    SILGenFunction SGF(*this, *F, NeXTCodeModule);
...
SILFunction *SILGenModule::getOrCreateForeignAsyncCompletionHandlerImplFunction(
  ...
  // bridged NeXTCode types, we still lower the parameters to their ultimate
  // ObjC types.
  ...
  if (F->empty()) {
    ...
    SILGenFunction SGF(*this, *F, NeXTCodeModule);
    {
      ...
      auto errorIndex = nextcode::transform(convention.completionHandlerErrorParamIndex(),
          [](auto original) { return original + 1; });
      auto flagIndex = nextcode::transform(convention.completionHandlerFlagParamIndex(),
          [](auto original) { return original + 1; });
      ...
      // arguments to the callback.
      {
        ...
        auto prepareArgument = [&](SILValue destBuf, CanType destFormalType,
          ...
          // Convert the ObjC argument to the bridged NeXTCode representation we
          // want.
...
SILFunction *SILGenModule::getOrCreateDerivativeVTableThunk(
  ...
  SILGenFunction SGF(*this, *thunk, NeXTCodeModule);
  SmallVector<ManagedValue, 4> params;
