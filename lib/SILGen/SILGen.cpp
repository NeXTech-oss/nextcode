...
#include "nextcode/SIL/SILProfiler.h"
...
SILGenModule::SILGenModule(SILModule &M, ModuleDecl *SM)
    : M(M), Types(M.Types), NeXTCodeModule(SM),
...
SILGenModule::getConformanceToObjectiveCBridgeable(SILLocation loc, Type type) {
  ...
  auto result = NeXTCodeModule->lookupConformance(type, proto);
  if (result.isInvalid())
    return nullptr;
...
SILGenModule::getConformanceToBridgedStoredNSError(SILLocation loc, Type type) {
  ...
  return NeXTCodeModule->lookupConformance(type, proto);
}
...
FuncDecl *SILGenModule::getNeXTCodeJobRun() {
  return lookupConcurrencyIntrinsic(getASTContext(), "_nextcodeJobRun");
}
...
FuncDecl *SILGenModule::getExit() {
  ...
  if (triple.isOSDarwin()) {
    ...
    mostLikelyIdentifier = C.getIdentifier("NeXTCodeWASILibc");
  } else if (triple.isWindowsMSVCEnvironment()) {
    mostLikelyIdentifier = C.getIdentifier("ucrt");
  } else {
    mostLikelyIdentifier = C.getIdentifier("NeXTCodeGlibc");
  }
  ...
  if (!exitFunction) {
    ...
    // NeXTCode modules or nextcode-corelibs modules may have absorbed <stdlib.h>
...
ProtocolConformance *SILGenModule::getNSErrorConformanceToError() {
  ...
  auto conformance =
    NeXTCodeModule->lookupConformance(nsErrorTy, cast<ProtocolDecl>(error));
...
void SILGenModule::emitFunctionDefinition(SILDeclRef constant, SILFunction *f) {
  ...
  case SILDeclRef::Kind::EntryPoint: {
    ...
    // to nextcode_job_run to execute the first thunk, and starts the runloop to
...
SILGenModule::canStorageUseStoredKeyPathComponent(AbstractStorageDecl *decl,
  ...
  // computed.
  if (decl->isResilient(M.getNeXTCodeModule(), expansion))
    return false;
...
static bool canStorageUseTrivialDescriptor(SILGenModule &SGM,
  ...
  if (!SGM.M.getNeXTCodeModule()->isResilient()) {
...
public:
  ...
  explicit SILGenModuleRAII(SILModule &M) : SGM{M, M.getNeXTCodeModule()} {}
...
ASTLoweringRequest::evaluate(Evaluator &evaluator,
  ...
  if (shouldDeserialize) {
    ...
    silMod->getSILLoader()->getAllForModule(silMod->getNeXTCodeModule()->getName(),
                                            primary);
  }
...
std::unique_ptr<SILModule>
nextcode::performASTLowering(ModuleDecl *mod, Lowering::TypeConverter &tc,
...
std::unique_ptr<SILModule> nextcode::performASTLowering(CompilerInstance &CI,
...
std::unique_ptr<SILModule>
nextcode::performASTLowering(FileUnit &sf, Lowering::TypeConverter &tc,
                          const SILOptions &options,
                          const IRGenOptions *irgenOptions) {
  auto desc =
