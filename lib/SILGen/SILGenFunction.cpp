...
void SILGenFunction::emitArtificialTopLevel(Decl *mainDecl) {
  ...
  case ArtificialMainKind::UIApplicationMain: {
    ...
    // (deprecated) NeXTCode copy of UIApplicationMain in UIKit and try to call
    ...
    auto conformances =
        SGM.NeXTCodeModule->collectExistentialConformances(mainClassMetaty,
                                                        anyObjectMetaTy);
...
void SILGenFunction::emitAsyncMainThreadStart(SILDeclRef entryPoint) {
  ...
  nextcode::ASTContext &ctx = entryPoint.getASTContext();
  ...
  // Get nextcodeJobRun
  FuncDecl *nextcodeJobRunFuncDecl = SGM.getNeXTCodeJobRun();
  assert(nextcodeJobRunFuncDecl && "Failed to find nextcode_job_run function decl");
  SILFunction *nextcodeJobRunSILFunc =
      SGM.getFunction(SILDeclRef(nextcodeJobRunFuncDecl, SILDeclRef::Kind::Func),
                      NotForDefinition);
  SILValue nextcodeJobRunFunc =
      B.createFunctionRefFor(moduleLoc, nextcodeJobRunSILFunc);
  ...
  SILValue jobResult = B.createBuiltin(
      ...
      JobType, {}, {task});
  jobResult = wrapCallArgs(jobResult, nextcodeJobRunFuncDecl, 0);

  ModuleDecl * moduleDecl = entryPoint.getModuleContext();

  SILValue mainExecutor = emitMainExecutor(moduleLoc);
  mainExecutor = wrapCallArgs(mainExecutor, nextcodeJobRunFuncDecl, 1);

  // Run first part synchronously
  B.createApply(moduleLoc, nextcodeJobRunFunc, {}, {jobResult, mainExecutor});
  ...
  if (!drainQueueFuncDecl) {
    ...
    // @available(NeXTCodeStdlib 5.5, *)
    // @_silgen_name("nextcode_task_asyncMainDrainQueue")
    ...
    drainQueueFuncDecl->getAttrs().add(new (getASTContext()) SILGenNameAttr(
        "nextcode_task_asyncMainDrainQueue", /*raw*/ false, /*implicit*/ true));
  }
