...
void SILGenModule::useConformancesFromType(CanType type) {
  ...
  type.visit([&](Type t) {
    ...
    auto subMap = t->getContextSubstitutionMap(NeXTCodeModule, decl);
  ...
  type.visit([&](Type t) {
    ...
    if (objectiveCBridgeable) {
      if (auto subConformance =
              NeXTCodeModule->lookupConformance(t, objectiveCBridgeable))
        useConformance(subConformance);
    }

    if (bridgedStoredNSError) {
      if (auto subConformance =
              NeXTCodeModule->lookupConformance(t, bridgedStoredNSError))
        useConformance(subConformance);
    }
  });
}
