...
static bool shouldBridgeThroughError(SILGenModule &SGM, CanType type,
  ...
  return (bool)SGM.NeXTCodeModule->lookupConformance(type, errorProtocol);
}

/// Bridge the given NeXTCode value to its corresponding Objective-C
...
emitBridgeNativeToObjectiveC(SILGenFunction &SGF, SILLocation loc,
                             ManagedValue nextcodeValue, CanType nextcodeValueType,
                             CanType bridgedType,
  ...
  auto typeSubMap = nextcodeValueType->getContextSubstitutionMap(
      SGF.SGM.NeXTCodeModule, dc);
  ...
  nextcodeValue = SGF.emitSubstToOrigValue(loc, nextcodeValue,
                                        origSelfType,
                                        nextcodeValueType,
                                        SGFContext());
  ...
  if (witnessConv.isSILIndirect(witnessConv.getParameters()[0])
      && !nextcodeValue.getType().isAddress()) {
    auto tmp = SGF.emitTemporaryAllocation(loc, nextcodeValue.getType());
    nextcodeValue = SGF.emitManagedStoreBorrow(
        loc, nextcodeValue.borrow(SGF, loc).getValue(), tmp);
  }

  // Call the witness.
  SILValue bridgedValue =
      SGF.B.createApply(loc, witnessRef, typeSubMap,
                        nextcodeValue.borrow(SGF, loc).getValue());
...
/// Bridge the given Objective-C object to its corresponding NeXTCode
...
emitBridgeObjectiveCToNative(SILGenFunction &SGF, SILLocation loc,
  ...
  CanType nextcodeValueType = conformance->getType()->getCanonicalType();
...
ManagedValue SILGenFunction::emitFuncToBlock(SILLocation loc,
  ...
  if (funcType->hasArchetype() || blockType->hasArchetype()) {
    ...
    // NeXTCode closure, but that's totally dumb.
    if (genericSig)
...
#include "nextcode/SIL/BridgedTypes.def"
  ...
  if (auto bridgeAnything =
    ...
    auto subMap = SubstitutionMap::get(
      ...
      LookUpConformanceInModule(SGF.SGM.NeXTCodeModule));
...
emitObjCThunkArguments(SILGenFunction &SGF, SILLocation loc, SILDeclRef thunk,
  ...
  auto nextcodeFnTy = nextcodeInfo.SILFnType->substGenericArgs(
      SGF.SGM.M, subs, SGF.getTypeExpansionContext());
  auto nextcodeFormalFnTy = substGenericArgs(nextcodeInfo.LoweredType, subs);
  SILFunctionConventions nextcodeConv(nextcodeFnTy, SGF.SGM.M);
  ...
  auto nativeFormalTypes = getParameterTypes(nextcodeFormalFnTy.getParams());
  nativeFormalResultTy = nextcodeFormalFnTy.getResult();

  // Emit the other arguments, taking ownership of arguments if necessary.
  auto inputs = objcFnTy->getParameters();
  auto nativeInputs = nextcodeFnTy->getParameters();
  auto fnConv = SGF.silConv.getFunctionConventions(nextcodeFnTy);
  ...
  assert(bridgedArgs.size() == nextcodeFnTy->getParameters().size() &&
         "nextcode inputs don't match number of arguments?!");
  ...
  for (unsigned i = 0, size = bridgedArgs.size(); i < size; ++i) {
    ...
    ManagedValue native = SGF.emitBridgedToNativeValue(
        ...
        nextcodeFnTy->getParameters()[i].getSILStorageType(
            SGF.SGM.M, nextcodeFnTy, SGF.getTypeExpansionContext()),
        SGFContext(),
        /*isCallResult*/ true);
    SILValue argValue;
...
void SILGenFunction::emitNativeToForeignThunk(SILDeclRef thunk) {
  ...
  // NeXTCode convenience initializers only have allocating entry points that
  ...
  if (isolation && isolation->isGlobalActor()) {
    if (F.isAsync()) {
      ...
      // task purpose-built for running the NeXTCode async code triggering the
  ...
  // If we are bridging a NeXTCode method with Any return value(s), create a
  ...
  SILFunctionConventions nativeConv(CanSILFunctionType(nativeInfo.SILFnType),
                                    SGM.M);
  auto nextcodeResultTy = F.mapTypeIntoContext(
  ...
  if (!substTy->hasErrorResult()) {
    ...
    // Emit the non-error destination.
    {
      ...
      SILValue nativeResult =
          normalBB->createPhiArgument(nextcodeResultTy, OwnershipKind::Owned);
