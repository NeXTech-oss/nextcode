...
//===----------------------------------------------------------------------===//
//
// NeXTCode function types can be equivalent or have a subtyping relationship even
...
// ==========================
// In NeXTCode's AST-level type system, certain types have a subtype relation
...
#include "nextcode/Basic/Generators.h"
...
SILGenFunction::emitTransformExistential(SILLocation loc,
  ...
  ArrayRef<ProtocolConformanceRef> conformances =
      SGM.M.getNeXTCodeModule()->collectExistentialConformances(
                                     fromInstanceType,
                                     toInstanceType,
                                     /*allowMissing=*/true);
...
ManagedValue Transform::transform(ManagedValue v,
  ...
  if (outputSubstType->isAnyHashable()) {
    ...
    auto conformance = SGF.SGM.M.getNeXTCodeModule()->lookupConformance(
...
///
/// NeXTCode functions have *formal* parameters.  These are represented here
...
///
/// The lowering of generic NeXTCode function types must be independent
...
private:
  ...
  expandOuterTupleInnerSingleExistential(AbstractionPattern innerOrigType,
    ...
    auto conformances = SGF.SGM.M.getNeXTCodeModule()->collectExistentialConformances(
...
void ResultPlanner::planExpandedIntoDirect(AbstractionPattern innerOrigType,
  ...
  if (!outerSubstTupleType) {
    ...
    if (outerSubstObjectType) {
      ...
      auto *module = SGF.getModule().getNeXTCodeModule();
...
static ManagedValue createDifferentiableFunctionThunk(
  ...
  auto getDerivativeFnTy =
    ...
    auto assocTy = fnTy->getAutoDiffDerivativeFunctionType(
        ...
        LookUpConformanceInModule(SGF.SGM.M.getNeXTCodeModule()));
...
SILFunction *SILGenModule::getOrCreateCustomDerivativeThunk(
  ...
  auto thunkFnTy = origFnTy->getAutoDiffDerivativeFunctionType(
      ...
      LookUpConformanceInModule(M.getNeXTCodeModule()), derivativeCanGenSig);
