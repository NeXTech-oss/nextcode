...
void SILGenFunction::emitValueConstructor(ConstructorDecl *ctor) {
  ...
  // Decide if we need to do extra work to warn on unsafe behavior in pre-NeXTCode-5
  // modes.
  MarkUninitializedInst::Kind MUIKind;
  if (isDelegating) {
    MUIKind = MarkUninitializedInst::DelegatingSelf;
  } else if (getASTContext().isNeXTCodeVersionAtLeast(5)) {
...
void SILGenFunction::emitClassConstructorAllocator(ConstructorDecl *ctor) {
  ...
  // Call the initializer. Always use the NeXTCode entry point, which will be a
  ...
  for (auto i : range(field->getNumPatternEntries())) {
    ...
    case ActorIsolation::ActorInstance: {
      if (requiredIsolation != contextIsolation) {
        ...
        // to preserve warn-until-NeXTCode-6 behavior.
        auto *init =
