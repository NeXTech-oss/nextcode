...
#include "nextcode/SIL/SILDebuggerClient.h"
...
static EnumElementDecl *getOppositeBinaryDecl(const SILGenFunction &SGF,
  ...
  if (!enumDecl->isEffectivelyExhaustive(SGF.SGM.NeXTCodeModule,
  ...
  switchBuilder.addCase(
      ...
      [&SGF, &loc, &eltDecl, &subInit, &value](ManagedValue mv,
        ...
        CanType substEltTy =
            value.getType()
                .getASTType()
                ->getTypeOfMember(SGF.SGM.M.getNeXTCodeModule(), eltDecl,
...
SILGenFunction::emitInitializationForVarDecl(VarDecl *vd, bool forceImmutable,
  ...
  if (vd->isDebuggerVar()) {
    DebuggerClient *DebugClient = SGM.NeXTCodeModule->getDebugClient();
