...
getPartialApplyOfDynamicMethodFormalType(SILGenModule &SGM, SILDeclRef member,
  ...
  // Adjust the ExtInfo by using a NeXTCode representation.
  auto extInfo = completeMethodTy->getExtInfo()
                   .withRepresentation(FunctionTypeRepresentation::NeXTCode);
...
static bool canUseStaticDispatch(SILGenFunction &SGF,
  ...
  // directly.
  auto thisModule = SGF.SGM.M.getNeXTCodeModule();
...
private:
  /// An IndirectValue callee represents something like a nextcode closure or a c
...
/// For ObjC init methods, we generate a shared-linkage NeXTCode allocating entry
...
public:
  ...
  void applySuper(SelfApplyExpr *apply) {
    ...
    if (!canUseStaticDispatch(SGF, constant)) {
      ...
      // Native NeXTCode super calls to final methods are direct.
...
static void emitRawApply(SILGenFunction &SGF,
  ...
  for (auto i : indices(args)) {
    ...
    if (inputParams[i].isConsumedInCaller()) {
      ...
      // Move only is not represented in the NeXTCode level type system, so if we
    ...
    if (IndirectErrorResult && IndirectErrorResult == innerError) {
      ...
      if (innerErrorType != outerErrorType) {
        ...
        ProtocolConformanceRef conformances[1] = {
          getModule().getNeXTCodeModule()->checkConformance(
...
void SILGenFunction::emitApplyOfUnavailableCodeReached() {
  ...
  if (SGM.requiresBackDeploymentThunk(fd, F.getResilienceExpansion())) {
    ...
    // introduced in NeXTCode 5.9 so we call the back deployment thunk in case this
...
SILGenFunction::emitUninitializedArrayAllocation(Type ArrayTy,
  ...
  auto subMap = ArrayTy->getContextSubstitutionMap(SGM.M.getNeXTCodeModule(),
                                                   Ctx.getArrayDecl());
...
static Callee getBaseAccessorFunctionRef(SILGenFunction &SGF,
  ...
  // or non-final native NeXTCode methods.
  if (!canUseStaticDispatch(SGF, constant))
