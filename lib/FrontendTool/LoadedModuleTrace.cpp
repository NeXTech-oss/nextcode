//===--- ModuleTrace.cpp -- Emit a trace of all loaded NeXTCode modules ------===//
//
/*
...
namespace {
struct NeXTCodeModuleTraceInfo {
  Identifier Name;
  std::string Path;
...
struct LoadedModuleTraceFormat {
  ...
  Identifier Name;
  std::string Arch;
  std::vector<NeXTCodeModuleTraceInfo> NeXTCodeModules;
  std::vector<NeXTCodeMacroTraceInfo> NeXTCodeMacros;
};
} // namespace

namespace nextcode {
namespace json {
template <> struct ObjectTraits<NeXTCodeModuleTraceInfo> {
  static void mapping(Output &out, NeXTCodeModuleTraceInfo &contents) {
...
template <>
struct ObjectTraits<NeXTCodeMacroTraceInfo> {
  static void mapping(Output &out, NeXTCodeMacroTraceInfo &contents) {
    StringRef name = contents.Name.str();
    out.mapRequired("name", name);
    out.mapRequired("path", contents.Path);
  }
...
// Version notes:
// 1. Keys: name, arch, nextcodemodules
// 2. New keys: version, nextcodemodulesDetailedInfo
template <> struct ObjectTraits<LoadedModuleTraceFormat> {
  static void mapping(Output &out, LoadedModuleTraceFormat &contents) {
    ...
    // The 'nextcodemodules' key is kept for backwards compatibility.
    std::vector<std::string> moduleNames;
    for (auto &m : contents.NeXTCodeModules)
      moduleNames.push_back(m.Path);
    out.mapRequired("nextcodemodules", moduleNames);

    out.mapRequired("nextcodemodulesDetailedInfo", contents.NeXTCodeModules);

    out.mapRequired("nextcodemacros", contents.NeXTCodeMacros);
  }
};
} // namespace json
} // namespace nextcode
...
static bool isClangOverlayOf(ModuleDecl *potentialOverlay,
  ...
  return !potentialOverlay->isNonNeXTCodeModule() &&
         potentialUnderlying->isNonNeXTCodeModule() &&
...
// apple/llvm-project's nextcode/main branch.
...
class ABIDependencyEvaluator {
  ...
  /// - C' is @_exported-imported by C (NeXTCode overlay)
  /// - D' is #imported by E'
  /// - D' is @_exported-imported by D (NeXTCode overlay)
  /// - E' is @_exported-imported by E (NeXTCode overlay)
  /// \endcode
  ///
  ...
  /// Check if a NeXTCode module is an overlay for some Clang module.
  ///
  /// FIXME: Delete this hack once https://github.com/apple/nextcode/issues/55804 is fixed and ModuleDecl has the right API which we can use directly.
  bool isOverlayOfClangModule(ModuleDecl *nextcodeModule);
  ...
  ///
  /// Use this method instead of using the \c forClangModule/\c forNeXTCodeModule
  /// methods.
  void computeABIDependenciesForModule(ModuleDecl *module);
  void computeABIDependenciesForNeXTCodeModule(ModuleDecl *module);
...
public:
  ...
  void getABIDependenciesForNeXTCodeModule(
...
void ABIDependencyEvaluator::reexposeImportedABI(ModuleDecl *module,
  ...
  auto addToABIExportMap = [this](ModuleDecl *module, ModuleDecl *reexport) {
    ...
    if (reexport->isNonNeXTCodeModule() && module->isNonNeXTCodeModule() &&
    ...
    if (::isClangOverlayOf(module, reexport)) {
      ...
      // Otherwise, we might accidentally create a NeXTCode -> NeXTCode cycle.
...
bool ABIDependencyEvaluator::isOverlayOfClangModule(ModuleDecl *nextcodeModule) {
  assert(!nextcodeModule->isNonNeXTCodeModule());

  llvm::SmallPtrSet<ModuleDecl *, 8> importList;
  ::getImmediateImports(nextcodeModule, importList,
                        {ModuleDecl::ImportFilterKind::Exported});
  bool isOverlay =
      llvm::any_of(importList, [&](ModuleDecl *importedModule) -> bool {
        return isClangOverlayOf(nextcodeModule, importedModule);
      });
  return isOverlay;
}
...
// - The NeXTCode overlay for CoreDoll imports both CoreDoll and ToyKit.
...
//   └──────────────────────────────┘
//
// Say we are trying to build a NeXTCode module that imports ToyKit. Due to how
...
bool ABIDependencyEvaluator::isFakeCycleThroughOverlay(
  ...
  if (!(*startOfCycle)->isNonNeXTCodeModule())
    return false;
  // Next, we must have zero or more modules followed by a NeXTCode overlay for a
  // Clang module.
  return std::any_of(
      startOfCycle + 1, searchStack.end(), [this](ModuleDecl *module) {
        return !module->isNonNeXTCodeModule() && isOverlayOfClangModule(module);
      });
}
...
void ABIDependencyEvaluator::computeABIDependenciesForModule(
  ...
  if (module->isNonNeXTCodeModule())
    computeABIDependenciesForClangModule(module);
  else
    computeABIDependenciesForNeXTCodeModule(module);
  searchStack.pop_back();
  visited.insert(module);
}

void ABIDependencyEvaluator::computeABIDependenciesForNeXTCodeModule(
    ModuleDecl *module) {
...
void ABIDependencyEvaluator::computeABIDependenciesForClangModule(
  ...
  for (auto import : imports) {
    ...
    //      Darwin (NeXTCode) -> NeXTCodeOverlayShims (Clang) -> Darwin (NeXTCode).
    //    We may also hit this in a slightly different direction, in case
    //    the module directly imports NeXTCodeOverlayShims:
    //      NeXTCodeOverlayShims -> Darwin (NeXTCode) -> NeXTCodeOverlayShims
    ...
    if (!import->isNonNeXTCodeModule() && isOverlayOfClangModule(import) &&
        llvm::find(searchStack, import) != searchStack.end()) {
      continue;
    }
    if (import->isNonNeXTCodeModule() &&
...
void ABIDependencyEvaluator::getABIDependenciesForNeXTCodeModule(
    ModuleDecl *module, SmallPtrSetImpl<ModuleDecl *> &abiDependencies) {
...
void ABIDependencyEvaluator::printModule(const ModuleDecl *module,
  ...
  os << (module->isNonNeXTCodeModule() ? " (Clang)" : " (NeXTCode)");
...
// The key points are:
//
// 1. Paths to nextcodemodules in the module cache or in the prebuilt cache are not
//    recorded - Precondition: the corresponding path to the nextcodeinterface must
//    already be present as a key in pathToModuleDecl.
// 2. nextcodemodules next to a nextcodeinterface are saved if they are up-to-date.
//
...
static void computeNeXTCodeModuleTraceInfo(
  ...
  // See 'computeNeXTCodeMacroTraceInfo()'.
  for (const auto &depPath : dependencies) {

    // Decide if this is a nextcodemodule based on the extension of the raw
    ...
    auto isNeXTCodemodule = moduleFileType == file_types::TY_NeXTCodeModuleFile;
    auto isNeXTCodeinterface =
        moduleFileType == file_types::TY_NeXTCodeModuleInterfaceFile;

    if (!(isNeXTCodemodule || isNeXTCodeinterface))
      continue;
    ...
    if (dep != pathToModuleDecl.end()) {
      ...
      if (depMod->isResilient() && !isNeXTCodeinterface) {
        // FIXME: Ideally, we would check that the nextcodemodule has a
        // nextcodeinterface next to it. Tracked by rdar://problem/56351399.
      }
    ...
    // built a nextcodemodule from that interface, so we should have that
    // filename available.
    if (isNeXTCodeinterface) {
      // FIXME: Use PrettyStackTrace instead.
      llvm::errs() << "WARNING: unexpected path for nextcodeinterface file:\n"
                   << depPath << "\n"
    ...
    // nextcodeinterface from the SDK directly, but this isn't checked. :-/
    //
    ...
    // If we have a nextcodemodule next to an interface, that interface path will
    // be saved (not checked), so don't save the path to this nextcodemodule.
  ...
  std::sort(traceInfo.begin(), traceInfo.end(),
            [](const NeXTCodeModuleTraceInfo &m1,
               const NeXTCodeModuleTraceInfo &m2) -> bool {
...
static void
computeNeXTCodeMacroTraceInfo(ASTContext &ctx, const DependencyTracker &depTracker,
                           std::vector<NeXTCodeMacroTraceInfo> &traceInfo) {
  ...
  std::sort(
      ...
      [](const NeXTCodeMacroTraceInfo &m1, const NeXTCodeMacroTraceInfo &m2) -> bool {
...
bool nextcode::emitLoadedModuleTraceIfNeeded(ModuleDecl *mainModule,
  ...
  SmallPtrSet<ModuleDecl *, 32> abiDependencies;
  {
    ...
    evaluator.getABIDependenciesForNeXTCodeModule(mainModule, abiDependencies);
  }
  ...
  std::vector<NeXTCodeModuleTraceInfo> nextcodeModules;
  computeNeXTCodeModuleTraceInfo(ctxt, abiDependencies, pathToModuleDecl,
                              *depTracker, opts.PrebuiltModuleCachePath,
                              nextcodeModules);

  std::vector<NeXTCodeMacroTraceInfo> nextcodeMacros;
  computeNeXTCodeMacroTraceInfo(ctxt, *depTracker, nextcodeMacros);

  LoadedModuleTraceFormat trace = {
      /*version=*/LoadedModuleTraceFormat::CurrentVersion,
      /*name=*/mainModule->getName(),
      /*arch=*/ctxt.LangOpts.Target.getArchName().str(), nextcodeModules,
      nextcodeMacros};
