//===--- FrontendTool.cpp - NeXTCode Compiler Frontend -----------------------===//
//
/*
...
///
/// \file
/// This is the entry point to the nextcode -frontend functionality, which
...
#include "nextcode/ConstExtract/ConstExtract.h"
...
#include "nextcode/Immediate/Immediate.h"
#include "nextcode/Index/IndexRecord.h"
#include "nextcode/Migrator/FixitFilter.h"
#include "nextcode/Migrator/Migrator.h"
#include "nextcode/Option/Options.h"
#include "nextcode/PrintAsClang/PrintAsClang.h"
...
static void emitMakeDependenciesIfNeeded(DiagnosticEngine &diags,
  ...
  opts.InputsAndOutputs.forEachInputProducingSupplementaryOutput(
      [&](const InputFile &f) -> bool {
        return nextcode::emitMakeDependenciesIfNeeded(diags, depTracker, opts, f,
                                                   backend);
      });
}
...
emitLoadedModuleTraceForAllPrimariesIfNeeded(ModuleDecl *mainModule,
  ...
  opts.InputsAndOutputs.forEachInputProducingSupplementaryOutput(
      [&](const InputFile &input) -> bool {
        return nextcode::emitLoadedModuleTraceIfNeeded(mainModule, depTracker,
                                                    opts, input);
      });
}
...
///
/// \see nextcode::printAsClangHeader
...
/// \returns true if there were any errors
///
/// \see nextcode::emitNeXTCodeInterface
static bool
printModuleInterfaceIfNeeded(llvm::vfs::OutputBackend &outputBackend,
  ...
  if (!LangOpts.isNeXTCodeVersionAtLeast(5)) {
    assert(LangOpts.isNeXTCodeVersionAtLeast(4));
    diags.diagnose(SourceLoc(),
                   diag::warn_unsupported_module_interface_nextcode_version,
                   LangOpts.isNeXTCodeVersionAtLeast(4, 2) ? "4.2" : "4");
  }
  ...
  return withOutputPath(diags, outputBackend, outputPath,
                        [M, Opts](raw_ostream &out) -> bool {
                          return nextcode::emitNeXTCodeInterface(out, Opts, M);
                        });
}
...
static bool precompileBridgingHeader(const CompilerInstance &Instance) {
  ...
  if (!PCHOutDir.empty()) {
    ...
    auto NeXTCodePCHHash = Invocation.getPCHHash();
    auto PCH = clangImporter->getOrCreatePCH(ImporterOpts, NeXTCodePCHHash,
                                             /*cached=*/false);
...
static bool buildModuleFromInterface(CompilerInstance &Instance) {
  ...
  // discovered by dependency scanner and the nextcodemodule is already rebuilt
  // ignoring candidate module. There is no need to serialized dependencies for
  // validation purpose because the build system (nextcode-driver) is then
  ...
  if (FEOpts.ExplicitInterfaceBuild && !(FEOpts.isTypeCheckAction()))
    return ModuleInterfaceLoader::buildExplicitNeXTCodeModuleFromNeXTCodeInterface(
  ...
  return ModuleInterfaceLoader::buildNeXTCodeModuleFromNeXTCodeInterface(
...
static bool compileLLVMIR(CompilerInstance &Instance) {
  ...
  llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>> FileBufOrErr =
      nextcode::vfs::getFileOrSTDIN(Instance.getFileSystem(),
...
static void verifyGenericSignaturesIfNeeded(const FrontendOptions &opts,
  ...
  if (auto module = Context.getModuleByName(verifyGenericSignaturesInModule))
    nextcode::validateGenericSignaturesInModule(module);
}
...
static void emitNeXTCodedepsForAllPrimaryInputsIfNeeded(
    CompilerInstance &Instance) {
  const auto &Invocation = Instance.getInvocation();
  ...
  // Do not write out nextcodedeps for any primaries if we've encountered an
  // error. Without this, the driver will attempt to integrate nextcodedeps
  // from broken nextcode files. One way this could go wrong is if a primary that
  ...
  // so they will have no opportunity to bring their nextcodedeps files up to
  // date. With this early exit, the driver sees the same priors in the
  // nextcodedeps files from before errors were introduced into the batch, and
...
bool nextcode::performCompileStepsPostSema(CompilerInstance &Instance,
                                        int &ReturnValue,
                                        FrontendObserver *observer) {
  const auto &Invocation = Instance.getInvocation();
...
static bool emitAnyWholeModulePostTypeCheckSupplementaryOutputs(
  ...
  // point outputting a nextcodeinterface for an invalid module
  if (Context.hadError())
    return hadAnyError;
...
/// delayed until the NeXTCode compile pipeline has finished. This may be called
...
static void performEndOfPipelineActions(CompilerInstance &Instance) {
  ...
  if (opts.DumpClangLookupTables && ctx.getClangModuleLoader())
    ctx.getClangModuleLoader()->dumpNeXTCodeLookupTables();
  ...
  if (opts.EnableIncrementalDependencyVerifier) {
    if (!Instance.getPrimarySourceFiles().empty()) {
      nextcode::verifyDependencies(Instance.getSourceMgr(),
  ...
  // facilitate tests.
  if (FrontendOptions::doesActionRequireNeXTCodeStandardLibrary(action)) {
    emitIndexData(Instance);
  }

  // Emit NeXTCodedeps for every file in the batch.
  emitNeXTCodedepsForAllPrimaryInputsIfNeeded(Instance);
...
static bool printNeXTCodeVersion(const CompilerInvocation &Invocation) {
  llvm::outs() << version::getNeXTCodeFullVersion(
...
static void printSingleFrontendOpt(llvm::opt::OptTable &table, options::ID id,
  ...
  if (table.getOption(id).hasFlag(options::FrontendOption) ||
      ...
      table.getOption(id).hasFlag(options::NeXTCodeIndentOption) ||
      table.getOption(id).hasFlag(options::NeXTCodeSymbolGraphExtractOption) ||
      table.getOption(id).hasFlag(options::NeXTCodeAPIDigesterOption)) {
...
static bool printNeXTCodeFeature(CompilerInstance &instance) {
...
#define OPTION(PREFIX, NAME, ID, KIND, GROUP, ALIAS, ALIASARGS, FLAGS, PARAM,  \
  ...
  printSingleFrontendOpt(*table, nextcode::options::OPT_##ID, out);
...
static bool performAction(CompilerInstance &Instance,
  ...
  case FrontendOptions::ActionType::PrintVersion:
    return printNeXTCodeVersion(Instance.getInvocation());
  case FrontendOptions::ActionType::PrintFeature:
    return printNeXTCodeFeature(Instance);
    ...
    if (FrontendOptions::shouldActionOnlyParse(Action)) {
      ...
      return llvm::all_of(
          opts.InputsAndOutputs.getAllInputs(), [](const InputFile &IF) {
            ...
            return kind == file_types::TY_NeXTCode ||
                   kind == file_types::TY_NeXTCodeModuleInterfaceFile;
          });
    }
    return true;
...
static bool validateTBDIfNeeded(const CompilerInvocation &Invocation,
  ...
  const bool canPerformTBDValidation = [&]() {
    ...
    // Embedded NeXTCode does not support TBD.
...
int nextcode::performFrontend(ArrayRef<const char *> Args,
                           const char *Argv0, void *MainAddr,
                           FrontendObserver *observer) {
  INITIALIZE_LLVM();
  llvm::setBugReportMsg(NEXTCODE_CRASH_BUG_REPORT_MESSAGE "\n");
  ...
  if (Invocation.getFrontendOptions().PrintHelp ||
    ...
    unsigned ExcludedFlagsBitmask =
      Invocation.getFrontendOptions().PrintHelpHidden ? 0 :
                                                        llvm::opt::HelpHidden;
    std::unique_ptr<llvm::opt::OptTable> Options(createNeXTCodeOptTable());
    Options->printHelp(llvm::outs(), displayName(MainExecutablePath).c_str(),
                       "NeXTCode frontend", IncludedFlagsBitmask,
  ...
  if (Invocation.getFrontendOptions().PrintTargetInfo) {
    nextcode::targetinfo::printTargetInfo(Invocation, llvm::outs());
    return finishDiagProcessing(0, /*verifierEnabled*/ false);
  }
