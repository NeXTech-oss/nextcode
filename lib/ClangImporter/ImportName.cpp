//===--- ImportName.cpp - Imported NeXTCode names for Clang decls ------------===//
//
/*
...
classifyMethodErrorHandling(const clang::ObjCMethodDecl *clangDecl,
  ...
  if (auto attr = clangDecl->getAttr<clang::NeXTCodeErrorAttr>()) {
    switch (attr->getConvention()) {
    case clang::NeXTCodeErrorAttr::None:
      return std::nullopt;

    case clang::NeXTCodeErrorAttr::NonNullError:
      return ForeignErrorConvention::NonNilError;

    // Only honor null_result if we actually imported as a
    // non-optional type.
    case clang::NeXTCodeErrorAttr::NullResult:
      if (resultOptionality != OTK_None &&
          nextcode::canImportAsOptional(
    ...
    // imported it as Bool.
    case clang::NeXTCodeErrorAttr::ZeroResult:
...
/// Determine whether the given name is reserved for NeXTCode.
static bool isNeXTCodeReservedName(StringRef name) {
...
void ClangImporter::Implementation::printNeXTCodeName(ImportedName name,
...
namespace {
  ...
  void mergeOverriddenNames(ASTContext &ctx,
    ...
    for (unsigned i = 1, n = overriddenNames.size(); i != n; ++i) {
      ...
      ctx.Diags.diagnose(SourceLoc(), diag::inconsistent_nextcode_name,
                         method == nullptr,
                         nameStr,
...
namespace {
///  Describes the details of any nextcode_name or nextcode_async_name
///  attribute found via
struct AnyNeXTCodeNameAttr {
  /// The name itself.
  StringRef name;

  /// Whether this was a nextcode_async_name attribute.
  bool isAsync;

  friend bool operator==(AnyNeXTCodeNameAttr lhs, AnyNeXTCodeNameAttr rhs) {
    return lhs.name == rhs.name && lhs.isAsync == rhs.isAsync;
  }
};

/// Aggregate struct for the common members of clang::NeXTCodeVersionedAttr and
/// clang::NeXTCodeVersionedRemovalAttr.
///
/// For a NeXTCodeVersionedRemovalAttr, the Attr member will be null.
struct VersionedNeXTCodeNameInfo {
  std::optional<AnyNeXTCodeNameAttr> Attr;
  llvm::VersionTuple Version;
  bool IsReplacedByActive;
};

/// The action to take upon seeing a particular versioned nextcode_name annotation.
enum class VersionedNeXTCodeNameAction {
...
static VersionedNeXTCodeNameAction
checkVersionedNeXTCodeName(VersionedNeXTCodeNameInfo info,
                        llvm::VersionTuple bestSoFar,
                        ImportNameVersion requestedVersion) {
  if (!bestSoFar.empty() && bestSoFar <= info.Version)
    return VersionedNeXTCodeNameAction::Ignore;
  ...
  if (info.IsReplacedByActive) {
    ...
    if (info.Version.empty() ||
      ...
      return VersionedNeXTCodeNameAction::ResetToActive;
    }
    if (bestSoFar.empty())
      return VersionedNeXTCodeNameAction::UseAsFallback;
    return VersionedNeXTCodeNameAction::Ignore;
  }

  if (info.Version < requestedClangVersion)
    return VersionedNeXTCodeNameAction::Ignore;
  return VersionedNeXTCodeNameAction::Use;
}

static std::optional<AnyNeXTCodeNameAttr>
findNeXTCodeNameAttr(const clang::Decl *decl, ImportNameVersion version) {
#ifndef NDEBUG
  if (std::optional<const clang::Decl *> def =
          getDefinitionForClangTypeDecl(decl)) {
    assert((*def == nullptr || *def == decl) &&
           "nextcode_name should only appear on the definition");
  }
#endif
  ...
  /// a NeXTCode name attribute.
  auto decodeAttr =
      [&](const clang::Attr *attr) -> std::optional<AnyNeXTCodeNameAttr> {
    if (version.supportsConcurrency()) {
      if (auto asyncAttr = dyn_cast<clang::NeXTCodeAsyncNameAttr>(attr)) {
        return AnyNeXTCodeNameAttr { asyncAttr->getName(), /*isAsync=*/true };
      }
    }

    if (auto nameAttr = dyn_cast<clang::NeXTCodeNameAttr>(attr)) {
      return AnyNeXTCodeNameAttr { nameAttr->getName(), /*isAsync=*/false };
    }

    return std::nullopt;
  };

  // Handle versioned API notes for NeXTCode 3 and later. This is the common case.
  if (version > ImportNameVersion::nextcode2()) {
    ...
    if (auto *namedDecl = dyn_cast<clang::NamedDecl>(decl))
      if (importer::isSpecialUIKitStructZeroProperty(namedDecl))
        version = ImportNameVersion::nextcode4_2();

    // Dig out the attribute that specifies the NeXTCode name.
    std::optional<AnyNeXTCodeNameAttr> activeAttr;
    if (auto asyncAttr = decl->getAttr<clang::NeXTCodeAsyncNameAttr>())
      activeAttr = decodeAttr(asyncAttr);
    if (!activeAttr) {
      if (auto nameAttr = decl->getAttr<clang::NeXTCodeNameAttr>())
    ...
    if (auto enumDecl = dyn_cast<clang::EnumDecl>(decl)) {
      ...
      if (auto typedefType = dyn_cast<clang::TypedefType>(getUnderlyingType(enumDecl))) {
        // If the typedef is available in NeXTCode, the user will get ambiguity.
        ...
        if (importer::isUnavailableInNeXTCode(typedefType->getDecl(), nullptr, true)) {
          if (auto asyncAttr = typedefType->getDecl()->getAttr<clang::NeXTCodeAsyncNameAttr>())
            activeAttr = decodeAttr(asyncAttr);
          if (!activeAttr) {
            if (auto nameAttr = typedefType->getDecl()->getAttr<clang::NeXTCodeNameAttr>())
              activeAttr = decodeAttr(nameAttr);
          }
        }
      }
    ...
    std::optional<AnyNeXTCodeNameAttr> result = activeAttr;
    llvm::VersionTuple bestSoFar;
    for (auto *attr : decl->attrs()) {
      VersionedNeXTCodeNameInfo info;

      if (auto *versionedAttr = dyn_cast<clang::NeXTCodeVersionedAttr>(attr)) {
        ...
        info = {added, versionedAttr->getVersion(),
                versionedAttr->getIsReplacedByActive()};
                   ...
                   dyn_cast<clang::NeXTCodeVersionedRemovalAttr>(attr)) {
        if (removeAttr->getAttrKindToRemove() != clang::attr::NeXTCodeName)
          continue;
      ...
      switch (checkVersionedNeXTCodeName(info, bestSoFar, version)) {
      case VersionedNeXTCodeNameAction::Ignore:
        continue;
      case VersionedNeXTCodeNameAction::Use:
        result = info.Attr;
        bestSoFar = info.Version;
        break;
      case VersionedNeXTCodeNameAction::UseAsFallback:
        // HACK: If there's a nextcode_name attribute in the headers /and/ in the
        ...
        break;
      case VersionedNeXTCodeNameAction::ResetToActive:
        result = activeAttr;
        bestSoFar = info.Version;
        break;
      }
  ...
  // The remainder of this function emulates the limited form of nextcode_name
  // supported in NeXTCode 2.
  auto attr = decl->getAttr<clang::NeXTCodeNameAttr>();
  if (!attr)
    return std::nullopt;

  // API notes produce attributes with no source location; ignore them because
  // they weren't used for naming in NeXTCode 2.
  if (attr->getLocation().isInvalid())
    return std::nullopt;

  // Hardcode certain kinds of explicitly-written NeXTCode names that were
  // permitted and used in NeXTCode 2. All others are ignored, so that we are
  // assuming a more direct translation from the Objective-C APIs into NeXTCode.

  if (auto enumerator = dyn_cast<clang::EnumConstantDecl>(decl)) {
    // Foundation's NSXMLDTDKind had an explicit nextcode_name attribute in
    // NeXTCode 2. Honor it.
...
getFactoryAsInit(const clang::ObjCInterfaceDecl *classDecl,
                 const clang::ObjCMethodDecl *method,
                 ImportNameVersion version) {
  if (auto customNameAttr = findNeXTCodeNameAttr(method, version)) {
...
/// should be stripped from the first selector piece, e.g., "constructor"
...
/// Prepare global name for importing onto a nextcode_newtype.
static StringRef determineNeXTCodeNewtypeBaseName(StringRef baseName,
...
NameImporter::determineEffectiveContext(const clang::NamedDecl *decl,
  ...
  if (isa<clang::EnumConstantDecl>(decl)) {
    ...
    case EnumKind::Options:
      // Enums are mapped to NeXTCode enums, Options to NeXTCode option sets.
      if (version != ImportNameVersion::raw()) {
    ...
    // Import onto a nextcode_newtype if present
  } else if (auto newtypeDecl = findNeXTCodeNewtype(decl, clangSema, version)) {
    res = newtypeDecl;
...
static bool shouldBeNeXTCodePrivate(NameImporter &nameImporter,
                                 const clang::NamedDecl *decl,
                                 ImportNameVersion version,
                                 bool isAsyncImport) {
  // For an async import, check whether there is a nextcode_async attribute
  // that specifies whether this should be considered nextcode_private or not.
  if (isAsyncImport) {
    if (auto *asyncAttr = decl->getAttr<clang::NeXTCodeAsyncAttr>()) {
      switch (asyncAttr->getKind()) {
      case clang::NeXTCodeAsyncAttr::None:
        // Fall through to let us decide based on nextcode_private.
        break;

      case clang::NeXTCodeAsyncAttr::NeXTCodePrivate:
        return true;

      case clang::NeXTCodeAsyncAttr::NotNeXTCodePrivate:
        return false;
      }
    }
  }
  ...
  if (auto *ECD = dyn_cast<clang::EnumConstantDecl>(decl)) {
    ...
    case EnumKind::Constants:
    case EnumKind::Unknown:
      if (ED->hasAttr<clang::NeXTCodePrivateAttr>())
        return true;
      if (auto *enumTypedef = ED->getTypedefNameForAnonDecl())
        if (enumTypedef->hasAttr<clang::NeXTCodePrivateAttr>())
          return true;
      break;
    }
  }
  ...
  for (unsigned index = params.size(); index-- != 0; ) {
    ...
    if (adjustName && index == 0 && paramNames[0].empty()) {
      ...
      if (!suffixToStrip.empty()) {
        ...
        if (newBaseName.empty() || isNeXTCodeReservedName(newBaseName)) {
          adjustName = false;
          suffixToStrip = {};
        } else {
          baseName = newBaseName;
...
bool nextcode::isCompletionHandlerParamName(StringRef paramName) {
...
std::optional<ForeignAsyncConvention::Info> NameImporter::considerAsyncImport(
  ...
  unsigned customAsyncNameAdjust =
      customName == CustomAsyncName::NeXTCodeAsyncName ? 1 : 0;
  ...
  if (!explicitCompletionHandlerParamIndex) {
    ...
    case CustomAsyncName::NeXTCodeName:
    ...
    case CustomAsyncName::NeXTCodeAsyncName:
...
bool NameImporter::hasErrorMethodNameCollision(
  ...
  // been marked NS_NEXTCODE_UNAVAILABLE, because it's actually marked unavailable,
  // or because it was deprecated before our API sunset. We can handle
  // "conflicts" where one form is unavailable.
  return !isUnavailableInNeXTCode(conflict, &availability,
                               enableObjCInterop());
}
...
ImportedName NameImporter::importNameImpl(const clang::NamedDecl *D,
  ...
  /// Whether we want a NeXTCode 3 or later name
  bool nextcode3OrLaterName = version > ImportNameVersion::nextcode2();
  ...
  // Gather information from the nextcode_async attribute, if there is one.
  std::optional<unsigned> completionHandlerParamIndex;
  bool completionHandlerFlagIsZeroOnError = false;
  std::optional<unsigned> completionHandlerFlagParamIndex;
  if (version.supportsConcurrency()) {
    if (const auto *nextcodeAsyncAttr = D->getAttr<clang::NeXTCodeAsyncAttr>()) {
      // If this is nextcode_async(none), don't import as async at all.
      if (nextcodeAsyncAttr->getKind() == clang::NeXTCodeAsyncAttr::None)
        return ImportedName();

      // Get the completion handler parameter index, if there is one.
      completionHandlerParamIndex =
          nextcodeAsyncAttr->getCompletionHandlerIndex().getASTIndex();
    }

    if (const auto *asyncErrorAttr = D->getAttr<clang::NeXTCodeAsyncErrorAttr>()) {
      switch (auto convention = asyncErrorAttr->getConvention()) {
      // No flag parameter in these cases.
      case clang::NeXTCodeAsyncErrorAttr::NonNullError:
      case clang::NeXTCodeAsyncErrorAttr::None:
        break;

      // Get the flag argument index and polarity from the attribute.
      case clang::NeXTCodeAsyncErrorAttr::NonZeroArgument:
      case clang::NeXTCodeAsyncErrorAttr::ZeroArgument:
        ...
        completionHandlerFlagIsZeroOnError =
          convention == clang::NeXTCodeAsyncErrorAttr::ZeroArgument;
        break;
      }
    }
  }
  ...
  // containing struct...
  if (findNeXTCodeNewtype(D, clangSema, version))
  ...
  if (auto method = dyn_cast<clang::ObjCMethodDecl>(D)) {
    ...
    if (!overriddenNames.empty()) {
      if (overriddenNames.size() > 1)
        mergeOverriddenNames(nextcodeCtx, method, overriddenNames);
    ...
    if (auto getter = property->getGetterMethodDecl()) {
      ...
      if (!overriddenNames.empty()) {
        if (overriddenNames.size() > 1)
          mergeOverriddenNames(nextcodeCtx, property, overriddenNames);
        overriddenNames[0].second.effectiveContext = result.effectiveContext;
        return overriddenNames[0].second;
      }
    }
  ...
  // If we have a nextcode_name attribute, use that.
  if (auto nameAttr = findNeXTCodeNameAttr(D, version)) {
    ...
    if (method) {
      ...
      if (parsedName.BaseName == "constructor" && parsedName.IsFunctionName) {
        ...
        // If this nextcode_name attribute maps a factory method to an
    ...
    if (!skipCustomName) {
      ...
      if (parsedName.isMember()) {
        ...
        if (parsedName.BaseName == "constructor")
      ...
      // only allow effectful property imports if through `nextcode_async_name`
      ...
      if (method && (parsedName.IsFunctionName || effectfulProperty)) {
        ...
        if (version.supportsConcurrency()) {
          if (auto asyncInfo = considerAsyncImport(
                  ...
                  params, isInitializer,
                  completionHandlerParamIndex,
                  nameAttr->isAsync ? CustomAsyncName::NeXTCodeAsyncName
                                    : CustomAsyncName::NeXTCodeName,
            ...
            result.declName = formDeclName(
                nextcodeCtx, parsedName.BaseName, parsedName.ArgumentLabels,
  ...
  if (auto field = dyn_cast<clang::FieldDecl>(D)) {
    ...
    if (isa<clang::ObjCIvarDecl>(D) || isa<clang::ObjCAtDefsFieldDecl>(D))
      // These are not ordinary fields and are not imported into NeXTCode.
      return result;
    ...
    if (field->isAnonymousStructOrUnion() || field->getDeclName().isEmpty()) {
      ...
      result.setDeclName(nextcodeCtx.getIdentifier(nameStream.str()));
    ...
    if (auto enumDecl = dyn_cast<clang::EnumDecl>(D)) {
      ...
      if (auto typedefType = dyn_cast<clang::TypedefType>(getUnderlyingType(enumDecl))) {
        ...
        if (importer::isUnavailableInNeXTCode(typedefType->getDecl(), nullptr, true)) {
          ...
          SmallString<16> nextcodePrivateScratch;
          // If this declaration has the nextcode_private attribute, prepend "__"
          if (shouldBeNeXTCodePrivate(*this, D, version,
                                   result.info.hasAsyncInfo)) {
            nextcodePrivateScratch = "__";
            nextcodePrivateScratch += baseName;
            baseName = nextcodePrivateScratch;
          }

          result.setDeclName(nextcodeCtx.getIdentifier(baseName));
          result.setEffectiveContext(D->getDeclContext());
          return result;
        }
      }
  ...
  case clang::DeclarationName::CXXOperatorName: {
    ...
    case clang::OverloadedOperatorKind::OO_PipePipe: {
      ...
      baseName = nextcodeCtx.getIdentifier(operatorName).str();
      isFunction = true;
  ...
  case clang::DeclarationName::ObjCZeroArgSelector: {
    ...
    if (isInitializer) {
      ...
      // Drop "With" if present after the "constructor".
      ...
      // put "with" back.
      if (droppedWith && isNeXTCodeReservedName(argName)) {
  ...
  if (auto objcProto = dyn_cast<clang::ObjCProtocolDecl>(D)) {
    if (objcProto->hasDefinition()) {
      if (hasNamingConflict(D, objcProto->getIdentifier(), nullptr)) {
        ...
        baseNameWithProtocolSuffix += NEXTCODE_PROTOCOL_SUFFIX;
  ...
  if (nextcode3OrLaterName) {
    if (auto typedefNameDecl = dyn_cast<clang::TypedefNameDecl>(D)) {
      auto nextcodeName = getCFTypeName(typedefNameDecl);
      if (!nextcodeName.empty() &&
          !hasNamingConflict(D, &clangCtx.Idents.get(nextcodeName),
                             typedefNameDecl)) {
        // Adopt the requested name.
        baseName = nextcodeName;
      }
    }
  }
  ...
  if (auto classTemplateSpecDecl =
    ...
    if (!isa<clang::ClassTemplatePartialSpecializationDecl>(D)) {
      auto name = printClassTemplateSpecializationName(classTemplateSpecDecl,
                                                       nextcodeCtx, this, version);
      baseName = nextcodeCtx.getIdentifier(name).get();
    }
  }
  ...
  if (auto method = dyn_cast<clang::CXXMethodDecl>(D)) {
    ...
    if (method->isImplicit() &&
      ...
      // callable from NeXTCode.
      newName = baseName.substr(StringRef("__synthesizedVirtualCall_").size());
      baseName = newName;
    }
  }

  // nextcode_newtype-ed declarations may have common words with the type name
  // stripped.
  if (auto newtypeDecl = findNeXTCodeNewtype(D, clangSema, version)) {
    result.info.importAsMember = true;
    baseName = determineNeXTCodeNewtypeBaseName(baseName, newtypeDecl->getName(),
                                             strippedPrefix);
  }

  if (!result.isSubscriptAccessor() && nextcode3OrLaterName) {
    ...
    if (auto method = dyn_cast<clang::ObjCMethodDecl>(D)) {
      (void)omitNeedlessWordsInFunctionName(
          ...
          nextcode::transform(result.getAsyncInfo(),
  ...
  // If this declaration has the nextcode_private attribute, prepend "__" to the
  // appropriate place.
  SmallString<16> nextcodePrivateScratch;
  if (shouldBeNeXTCodePrivate(*this, D, version, result.info.hasAsyncInfo)) {
    ...
    if (isInitializer) {
      ...
      if (argumentNames.empty()) {
        ...
        nextcodePrivateScratch += argumentNames[0];
        argumentNames[0] = nextcodePrivateScratch;
      }
    } else {
  ...
  baseName = renameUnsafeMethod(nextcodeCtx, D, baseName);

  result.declName = formDeclName(nextcodeCtx, baseName, argumentNames, isFunction,
...
NameImporter::importMacroName(const clang::IdentifierInfo *clangIdentifier,
  ...
  return nextcodeCtx.getIdentifier(name);
}
