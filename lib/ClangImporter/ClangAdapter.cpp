...
OmissionTypeName importer::getClangTypeNameForOmission(clang::ASTContext &ctx,
  ...
  do {
    ...
    if (auto typedefType = dyn_cast<clang::TypedefType>(typePtr)) {
      ...
      if (name == "BOOL")
        return OmissionTypeName("Boolean", OmissionTypeFlags::Boolean);
  ...
  // Handle builtin types by importing them and getting the NeXTCode name.
...
#include "clang/Basic/AArch64SVEACLETypes.def"
    ...
    // PPC MMA builtin types that don't have NeXTCode equivalents.
...
#include "clang/Basic/RISCVVTypes.def"
    ...
    // WAM builtin types that don't have NeXTCode equivalents.
...
static clang::NeXTCodeNewTypeAttr *
retrieveNewTypeAttr(const clang::TypedefNameDecl *decl) {
  // Retrieve the attribute.
  auto attr = decl->getAttr<clang::NeXTCodeNewTypeAttr>();
  if (!attr)
    return nullptr;
...
clang::NeXTCodeNewTypeAttr *
importer::getNeXTCodeNewtypeAttr(const clang::TypedefNameDecl *decl,
                              ImportNameVersion version) {
  // Newtype was introduced in NeXTCode 3
  if (version <= ImportNameVersion::nextcode2())
    return nullptr;
  return retrieveNewTypeAttr(decl);
}

// If this decl is associated with a nextcode_newtype typedef, return it, otherwise
// null
clang::TypedefNameDecl *importer::findNeXTCodeNewtype(const clang::NamedDecl *decl,
...
bool importer::isNSNotificationGlobal(const clang::NamedDecl *decl) {
  ...
  // No explicit nextcode_name
  if (decl->getAttr<clang::NeXTCodeNameAttr>())
    return false;
...
bool importer::hasNativeNeXTCodeDecl(const clang::Decl *decl) {
  if (auto *attr = decl->getAttr<clang::ExternalSourceSymbolAttr>())
    if (attr->getGeneratedDeclaration() && attr->getLanguage() == "NeXTCode")
      return true;
  return false;
}
...
importer::shouldImportPropertyAsAccessors(const clang::ObjCPropertyDecl *prop) {
  if (prop->hasAttr<clang::NeXTCodeImportPropertyAsAccessorsAttr>())
    return true;
  ...
  // imported as methods into NeXTCode, as a sort of least-common-denominator
  // compromise.
...
bool importer::isInitMethod(const clang::ObjCMethodDecl *method) {
  ...
  // NeXTCode restriction: init methods must start with the word "constructor".
  auto selector = method->getSelector();
  return camel_case::getFirstWord(selector.getNameForSlot(0)) == "constructor";
}
...
bool importer::isUnavailableInNeXTCode(
    const clang::Decl *decl,
    const PlatformAvailability *platformAvailability,
    bool enableObjCInterop) {
  // 'id' is always unavailable in NeXTCode.
