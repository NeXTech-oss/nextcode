...
lookupDirectWithoutExtensions(NominalTypeDecl *decl, Identifier id) {
  ...
  // Check if there are any synthesized NeXTCode members that match the name.
...
/// underlying operator function is unavailable in NeXTCode for the current target
...
static bool synthesizeCXXOperator(ClangImporter::Implementation &impl,
  ...
  // namespace would get imported as a member function of a NeXTCode enum), which
  // would make the operator un-discoverable to NeXTCode name lookup.
  auto declContext =
...
bool nextcode::isIterator(const clang::CXXRecordDecl *clangDecl) {
  return getIteratorCategoryDecl(clangDecl);
}

ValueDecl *
nextcode::importer::getImportedMemberOperator(const DeclBaseName &name,
...
void nextcode::conformToCxxIteratorIfNeeded(
  ...
  if (!equalEqual) {
    ...
    if (instantiated && !impl.isUnavailableInNeXTCode(instantiated)) {
...
void nextcode::conformToCxxConvertibleToBoolIfNeeded(
    ClangImporter::Implementation &impl, nextcode::NominalTypeDecl *decl,
    const clang::CXXRecordDecl *clangDecl) {
...
void nextcode::conformToCxxOptionalIfNeeded(
    ClangImporter::Implementation &impl, NominalTypeDecl *decl,
    const clang::CXXRecordDecl *clangDecl) {
...
void nextcode::conformToCxxSequenceIfNeeded(
    ClangImporter::Implementation &impl, NominalTypeDecl *decl,
    const clang::CXXRecordDecl *clangDecl) {
  ...
  // implementation, which is not possible with the existing NeXTCode features.
  ...
  // developer to explicitly convert a C++ sequence to a NeXTCode Array (making a
...
bool nextcode::isUnsafeStdMethod(const clang::CXXMethodDecl *methodDecl) {
  auto parentDecl =
...
void nextcode::conformToCxxSetIfNeeded(ClangImporter::Implementation &impl,
...
void nextcode::conformToCxxPairIfNeeded(ClangImporter::Implementation &impl,
                                     NominalTypeDecl *decl,
                                     const clang::CXXRecordDecl *clangDecl) {
...
void nextcode::conformToCxxDictionaryIfNeeded(
    ClangImporter::Implementation &impl, NominalTypeDecl *decl,
    const clang::CXXRecordDecl *clangDecl) {
  ...
  // similarly to NeXTCode.Dictionary.
  for (auto member : decl->getCurrentMembersWithoutLoading()) {
...
void nextcode::conformToCxxVectorIfNeeded(ClangImporter::Implementation &impl,
                                       NominalTypeDecl *decl,
                                       const clang::CXXRecordDecl *clangDecl) {
...
void nextcode::conformToCxxFunctionIfNeeded(
    ClangImporter::Implementation &impl, NominalTypeDecl *decl,
    const clang::CXXRecordDecl *clangDecl) {
