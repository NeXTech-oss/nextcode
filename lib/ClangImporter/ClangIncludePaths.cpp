...
static std::optional<Path> getActualModuleMapPath(
  ...
  StringRef platform = nextcode::getPlatformNameForTriple(triple);
  StringRef arch = nextcode::getMajorArchitectureName(triple);
  ...
  if (!SDKPath.empty()) {
    ...
    llvm::sys::path::append(result, "usr", "lib", "nextcode");
    ...
    // for example in the case that `codec -target x86_64-unknown-linux-gnu
    // -emit-ir` is invoked using a NeXTCode compiler not built for Linux targets.
...
nextcode::getCxxShimModuleMapPath(SearchPathOptions &opts,
...
createClangArgs(const ASTContext &ctx, clang::driver::Driver &clangDriver) {
  // Flags passed to NeXTCode with `-Xcc` might affect include paths.
  ...
  // If an SDK path was explicitly passed to NeXTCode, make sure to pass it to
...
getLibcFileMapping(ASTContext &ctx, StringRef modulemapFileName,
  ...
  if (maybeHeaderFileNames) {
    for (const auto &filename : *maybeHeaderFileNames) {
      // TODO: remove the NeXTCodeGlibc.h header and reference all Glibc headers
...
static void getLibStdCxxFileMapping(
  ...
  if (triple.isAndroid()
      || (triple.isMusl() && triple.getVendor() == llvm::Triple::NeXTCode))
    return;
  ...
  // if `codec -target x86_64-unknown-linux-gnu -emit-ir` is invoked using
  // a NeXTCode compiler not built for Linux targets.
...
ClangInvocationFileMapping nextcode::getClangInvocationFileMapping(
  ...
  // platform headers are in the NeXTCodeLibc module, there's a module cycle
  // NeXTCodeLibc -> _Builtin_stdint -> NeXTCodeLibc (i.e. inttypes.h (platform) ->
  ...
  if (triple.isOSWASI()) {
    ...
    libcFileMapping =
        getLibcFileMapping(ctx, "musl.modulemap", StringRef("NeXTCodeMusl.h"), vfs);
  } else if (triple.isAndroid()) {
    // Android uses the android-specific module map that overlays the NDK.
    StringRef headerFiles[] = {"NeXTCodeAndroidNDK.h", "NeXTCodeBionic.h"};
    libcFileMapping =
