...
// macros into NeXTCode declarations.
//
//===----------------------------------------------------------------------===//
...
createMacroConstant(ClangImporter::Implementation &Impl,
  ...
  return NeXTCodeDeclSynthesizer(Impl).createConstant(
...
static ValueDecl *importNumericLiteral(ClangImporter::Implementation &Impl,
         ...
         "not a numeric token");
  {
    ...
    // <rdar://problem/16445608> NeXTCode should set up a DiagnosticConsumer for
    // Clang
...
static ValueDecl *importStringLiteral(ClangImporter::Implementation &Impl,
  ...
  Type importTy = Impl.getNamedNeXTCodeType(Impl.getStdlibModule(), "String");
  if (!importTy)
    return nullptr;
  ...
  return NeXTCodeDeclSynthesizer(Impl).createConstant(name, DC, importTy, text,
                                                   ConstantConvertKind::None,
                                                   /*static*/ false, ClangN);
}
...
static ValueDecl *importNil(ClangImporter::Implementation &Impl,
  ...
  auto type = TupleType::getEmpty(Impl.NeXTCodeContext);
...
static ValueDecl *importMacro(ClangImporter::Implementation &impl,
  ...
  if (numTokens > 3 &&
    ...
    if (!castType.isNull()) {
      ...
      // TODO(https://github.com/apple/nextcode/issues/57735): Diagnose nested cast.
      return nullptr;
    }
    ...
    if (tokenI[1].is(clang::tok::identifier)) {
      ...
      if (parsedType && diagPool.empty()) {
        ...
        // TODO(https://github.com/apple/nextcode/issues/57735): Add diagnosis.
        return nullptr;
      }
...
#include "MacroTable.def"
    ...
    // TODO(https://github.com/apple/nextcode/issues/57735): Seems rare to have a single token that is neither a literal nor an identifier, but add diagnosis.
    return nullptr;
  }
  case 2: {
  ...
  case 3: {
    ...
    Type resultNeXTCodeType = nullptr;
    ...
    if (firstIntSpec == std::max(firstIntSpec, secondIntSpec)) {
      ...
      resultNeXTCodeType = firstNeXTCodeType;
    } else {
      secondIntSpec.apply(firstValue);
      resultNeXTCodeType = secondNeXTCodeType;
    }
    ...
    // Logical OR.
      ...
      resultNeXTCodeType = impl.NeXTCodeContext.getBoolType();
    ...
    return createMacroConstant(impl, macro, name, DC, resultNeXTCodeType,
