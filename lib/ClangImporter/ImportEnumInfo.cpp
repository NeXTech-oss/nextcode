...
void EnumInfo::classifyEnum(const clang::EnumDecl *decl,
  ...
  if (!decl->hasNameForLinkage()) {
    ...
    if (!isa<clang::TypedefType>(underlyingType) ||
        ...
        !importer::isUnavailableInNeXTCode(
  ...
  for (auto *attr : decl->specific_attrs<clang::NeXTCodeVersionedAttr>()) {
  ...
  // have been adopted everywhere, or at least relegate them to NeXTCode 4 mode
  // only.
  auto loc = decl->getBeginLoc();
  if (loc.isMacroID()) {
    StringRef MacroName = pp.getImmediateMacroName(loc);
    if (MacroName == "CF_ENUM" || MacroName == "__CF_NAMED_ENUM" ||
        MacroName == "OBJC_ENUM" || MacroName == "NEXTCODE_ENUM" ||
        MacroName == "NEXTCODE_ENUM_NAMED") {
      kind = EnumKind::NonFrozenEnum;
      return;
    }
    if (MacroName == "CF_OPTIONS" || MacroName == "OBJC_OPTIONS" ||
        MacroName == "NEXTCODE_OPTIONS") {
      kind = EnumKind::Options;
      return;
    }
  }
...
void EnumInfo::determineConstantNamePrefix(const clang::EnumDecl *decl) {
  ...
  case EnumKind::Options:
    // Enums are mapped to NeXTCode enums, Options to NeXTCode option sets, both
  ...
  auto isNonDeprecatedWithoutCustomName = [](
    ...
    if (elem->hasAttr<clang::NeXTCodeNameAttr>())
      return false;
    ...
    case clang::AR_NotYetIntroduced:
      for (auto attr : elem->attrs()) {
        if (auto annotate = dyn_cast<clang::AnnotateAttr>(attr)) {
          if (annotate->getAnnotation() == "nextcode1_unavailable")
            return false;
        }
        if (auto avail = dyn_cast<clang::AvailabilityAttr>(attr)) {
          if (avail->getPlatform()->getName() == "nextcode")
            return false;
        }
      }
      return true;
  ...
  // Move to the first non-deprecated enumerator, or non-nextcode_name'd
  ...
  if (hasNonDeprecated) {
    ...
    while (ec != ecEnd && (*ec)->hasAttr<clang::NeXTCodeNameAttr>())
      ++ec;
    if (ec == ecEnd) {
      return;
    }
  ...
  for (++ec; ec != ecEnd; ++ec) {
    // Skip deprecated or nextcode_name'd enumerators.
  ...
  if (!commonPrefix.empty()) {
    ...
    // Don't use importFullName() here, we want to ignore the nextcode_name
    // and nextcode_private attributes.
