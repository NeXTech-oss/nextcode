...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_CLANG_IMPORTER_IMPL_H
#define NEXTCODE_CLANG_IMPORTER_IMPL_H
...
enum class ImportTypeAttr : uint8_t {
  ...
  /// Type should be imported as though declaration was marked with
  /// \c __attribute__((nextcode_attr("@MainActor"))) .
  MainActor = 1 << 1,

  /// Type should be imported as though declaration was marked with
  /// \c __attribute__((nextcode_attr("@Sendable"))) .
  Sendable = 1 << 2,
...
/// Find and iterate over nextcode attributes embedded in the type
/// without looking through typealiases.
void findNeXTCodeAttributes(
    clang::QualType type,
...
///
/// \param NeXTCodeContext The context.
...
void getConcurrencyAttrs(ASTContext &NeXTCodeContext, ImportTypeKind importKind,
...
/// NeXTCode type or the original Clang type.
///
...
getTypedefBridgeability(const clang::TypedefNameDecl *decl) {
  if (decl->hasAttr<clang::NeXTCodeBridgedTypedefAttr>() ||
...
/// a NeXTCode standard library type.
...
#define NEXTCODE_PROTOCOL_SUFFIX "Protocol"
#define NEXTCODE_CFTYPE_SUFFIX "Ref"
...
using LookupTableMap =
    llvm::DenseMap<StringRef, std::unique_ptr<NeXTCodeLookupTable>>;

/// The result of importing a clang type. It holds both the NeXTCode Type
...
public:
  ...
  const bool DisableNeXTCodeBridgeAttr;
...
private:
  ...
  /// The NeXTCode lookup tables, per module.
  ///
  /// Annoyingly, we list this table early so that it gets torn down after
  /// the underlying Clang instances that reference it
  /// (through the NeXTCode name lookup module file extension).
  LookupTableMap LookupTables;

  /// A helper class used to bring Clang buffers into NeXTCode's SourceManager
  ...
  /// Mapping from Clang nextcode_attr attribute text to the NeXTCode source buffer
  /// IDs that contain that attribute text. These are re-used when parsing the
  /// NeXTCode attributes on import.
  llvm::StringMap<unsigned> ClangNeXTCodeAttrSourceBuffers;

  /// Mapping from modules in which a Clang nextcode_attr attribute occurs, to be
  /// used when parsing the attribute text.
  llvm::SmallDenseMap<ModuleDecl *, SourceFile *> ClangNeXTCodeAttrSourceFiles;

public:
  /// The NeXTCode lookup table for the bridging header.
  std::unique_ptr<NeXTCodeLookupTable> BridgingHeaderLookupTable;

  /// Mapping of already-imported declarations.
  llvm::DenseMap<std::pair<const clang::Decl *, Version>, Decl *> ImportedDecls;

  /// The set of "special" typedef-name declarations, which are
  /// mapped to specific NeXTCode types.
  ///
  /// Normal typedef-name declarations imported into NeXTCode will maintain
  ...
  /// type of "signed char", but is mapped to a special NeXTCode struct type
  /// ObjCBool.
  ...
  /// with their corresponding NeXTCode declaration.
  ///
  ...
  bool isCxxInteropCompatVersionAtLeast(unsigned major,
    ...
    return NeXTCodeContext.LangOpts.isCxxInteropCompatVersionAtLeast(major, minor);
  }

private:
  /// The Importer may be configured to load modules of a different OS Version
  /// than the underlying NeXTCode compilation. This is the `TargetOptions`
  /// corresponding to the instantiating NeXTCode compilation's triple. These are
...
public:
  void setNeXTCodeTargetInfo(clang::TargetInfo *NeXTCodeTargetInfo) {
    CodeGenTargetInfo.reset(NeXTCodeTargetInfo);
  }
  clang::TargetInfo *getNeXTCodeTargetInfo() const {
    return CodeGenTargetInfo.get();
  }

  void setNeXTCodeCodeGenOptions(clang::CodeGenOptions *NeXTCodeCodeGenOpts) {
    CodeGenOpts.reset(NeXTCodeCodeGenOpts);
  }
  clang::CodeGenOptions *getNeXTCodeCodeGenOptions() const {
...
private:
  ...
  void bumpGeneration() {
    ++Generation;
    NeXTCodeContext.bumpGeneration();
  }
...
private:
  ...
  /// different NeXTCode declarations.
...
private:
  /// NSObject, imported into NeXTCode.
  Type NSObjectTy;
...
public:
  /// A map from Clang modules to their NeXTCode wrapper modules.
  ...
  ///
  /// These are used to look up NeXTCode classes forward-declared with \@class.
...
private:
  ...
  DependencyTracker *NeXTCodeDependencyTracker = nullptr;
...
public:
  ...
  /// Converts the given NeXTCode identifier for Clang.
  clang::DeclarationName exportName(Identifier name);

  /// Imports the full name of the given Clang declaration into NeXTCode.
  ///
  ...
  /// Print an imported name as a string suitable for the nextcode_name attribute,
  /// or the 'Rename' field of AvailableAttr.
  void printNeXTCodeName(importer::ImportedName name,
  ...
  void diagnose(Args &&...args) {
    ...
    if (NeXTCodeContext.Diags.isPrettyPrintingDecl()) {
      return;
    }

    NeXTCodeContext.Diags.diagnose(std::forward<Args>(args)...);
  }
  ...
  void diagnose(SourceLoc loc, Args &&...args) {
    ...
    NeXTCodeContext.Diags.diagnose(loc, std::forward<Args>(args)...);
  }

  /// Emit a diagnostic at a clang source location, falling back to a NeXTCode
  ...
  InFlightDiagnostic diagnose(HeaderLoc loc, Args &&...args) {
    ...
    auto nextcodeLoc = loc.fallbackLoc;
    if (loc.clangLoc.isValid()) {
      ...
      nextcodeLoc = bufferImporter.resolveSourceLocation(clangSrcMgr,
                                                      loc.clangLoc);
    }

    return NeXTCodeContext.Diags.diagnose(nextcodeLoc, std::forward<Args>(args)...);
  }
  ...
  /// Import the given Clang identifier into NeXTCode.
  ///
  /// \param identifier The Clang identifier to map into NeXTCode.
  ///
  /// \param removePrefix The prefix to remove from the Clang name to produce
  /// the NeXTCode name. If the Clang name does not start with this prefix,
  /// nothing is removed.
  ...
  /// Import a NeXTCode name as a Clang selector.
  clang::Selector exportSelector(DeclName name, bool allowSimpleName = true);

  /// Export a NeXTCode Objective-C selector as a Clang Objective-C selector.
  clang::Selector exportSelector(ObjCSelector selector);

  /// Import the given NeXTCode source location into Clang.
  clang::SourceLocation exportSourceLoc(SourceLoc loc);

  /// Import the given Clang source location into NeXTCode.
  SourceLoc importSourceLoc(clang::SourceLocation loc);

  /// Import the given Clang source range into NeXTCode.
  SourceRange importSourceRange(clang::SourceRange loc);

  /// Import the given Clang preprocessor macro as a NeXTCode value decl.
  ///
  /// \p macroNode must be a MacroInfo or a ModuleMacro.
  ///
  /// \returns The imported declaration, or null if the macro could not be
  /// translated into NeXTCode.
  ValueDecl *importMacro(Identifier name, ClangNode macroNode);

  /// Map a Clang identifier name to its imported NeXTCode equivalent.
  StringRef getNeXTCodeNameFromClangName(StringRef name);

  /// Retrieve the NeXTCode source buffer ID that corresponds to the given
  /// nextcode_attr attribute text, creating one if necessary.
  unsigned getClangNeXTCodeAttrSourceBuffer(StringRef attributeText);

  /// Retrieve the placeholder source file for use in parsing NeXTCode attributes
  /// in the given module.
  SourceFile &getClangNeXTCodeAttrSourceFile(ModuleDecl &module);

  /// Utility function to import Clang attributes from a source NeXTCode decl to
  /// synthesized NeXTCode decl.
  ///
  /// \param SourceDecl The NeXTCode decl to copy the atteribute from.
  /// \param SynthesizedDecl The synthesized NeXTCode decl to attach attributes to.
  void
  importAttributesFromClangDeclToSynthesizedNeXTCodeDecl(Decl *SourceDecl,
                                                      Decl *SynthesizedDecl);

  /// Import attributes from the given Clang declaration to its NeXTCode
  /// equivalent.
  ///
  ...
  /// If we already imported a given decl, return the corresponding NeXTCode decl.
  ...
  /// Import the given Clang declaration into NeXTCode.  Use this function
  ...
  /// not be represented in NeXTCode.
  ...
  /// Import the given Clang declaration context into NeXTCode.
  ///
...
private:
  ...
  /// Used to break cycles when a nextcode_name attribute is circular in a way that
...
public:
  ...
  /// 'unavailable' in NeXTCode.
  bool isUnavailableInNeXTCode(const clang::Decl *decl) {
    return importer::isUnavailableInNeXTCode(
        decl, &platformAvailability, NeXTCodeContext.LangOpts.EnableObjCInterop);
  }

  /// Add "Unavailable" annotation to the nextcode declaration.
  ...
  /// Retrieves the NeXTCode wrapper for the given Clang module, creating
  /// it if necessary.
  ClangModuleUnit *getWrapperForModule(const clang::Module *underlying,
                                       SourceLoc importLoc = SourceLoc());

  /// Constructs a NeXTCode module for the given Clang module.
  ...
  /// Retrieve the named NeXTCode type, e.g., Int32.
  ///
  ...
  Type getNamedNeXTCodeType(StringRef moduleName, StringRef name);
  ...
  ///
  /// \returns The named type, or null if the type could not be found.
  Type getNamedNeXTCodeType(ModuleDecl *module, StringRef name);
  ...
  /// NeXTCode AST.  This should only be called after successfully importing
  ...
  /// Try to find a stable NeXTCode serialization path for the given Clang
  /// declaration.
  ...
  /// Import the given Clang type into NeXTCode.
  ///
  ...
  ///   determine whether a type can be imported in a more NeXTCodey way than
  ...
  ///   Without full bridgeability, we can still do some NeXTCodey importing (e.g.
  ///   mapping NSString to String) if we're in an immediate context \p kind
  ///   that allows bridging, but only in cases where NeXTCode's default mapping
  ...
  ///   optionality is applied to the resulting NeXTCode type.
  ///
  ...
  ///   behavior, but isn't necessary when we use NeXTCode's \c throws anyway.
  ...
  ///          the type cannot be represented in NeXTCode, then the type
  ///          field will be null.
  ...
  /// Import the given Clang type into NeXTCode, returning the
  /// NeXTCode parameters and result type and whether we should treat it
  ...
  /// type is null if it cannot be represented in NeXTCode.
  ...
  struct ImportParameterTypeResult {
    /// The imported parameter NeXTCode type.
    nextcode::Type nextcodeTy;
  ...
  void recordImportedProtocols(Decl *decl,
    ...
    ImportedProtocols[decl] = NeXTCodeContext.AllocateCopy(protocols);
...
private:
  ...
  void loadAllMembersOfRecordDecl(NominalTypeDecl *nextcodeDecl,
...
public:
  ...
  DeclTy *createDeclWithClangNode(ClangNode ClangN, AccessLevel access,
                                  Targs &&... Args) {
    assert(ClangN);
    void *DeclPtr = allocateMemoryForDecl<DeclTy>(NeXTCodeContext, sizeof(DeclTy),
                                                  true);
    ...
    // NeXTCodeAttrs on ParamDecls are interpreted by applyParamAttributes().
    if (!isa<ParamDecl>(D))
      importNeXTCodeAttrAttributes(D);

    return D;
  }

  void importNeXTCodeAttrAttributes(Decl *decl);

  /// Find the lookup table that corresponds to the given Clang module.
  ///
  /// \param clangModule The module, or null to indicate that we're talking
  /// about the directly-parsed headers.
  NeXTCodeLookupTable *findLookupTable(const clang::Module *clangModule);

  /// Find the lookup table that should contain the given Clang declaration.
  NeXTCodeLookupTable *findLookupTable(const clang::Decl *decl);
  ...
  /// false otherwise.
  bool forEachLookupTable(llvm::function_ref<bool(NeXTCodeLookupTable &table)> fn);

  /// Determine whether the given Clang entry is visible.
  ///
  /// FIXME: this is an elaborate hack to badly reflect Clang's
  /// submodule visibility into NeXTCode.
  bool isVisibleClangEntry(const clang::NamedDecl *clangDecl);
  bool isVisibleClangEntry(NeXTCodeLookupTable::SingleEntry entry);

  /// Look for namespace-scope values with the given name in the given
  /// NeXTCode lookup table.
  bool lookupValue(NeXTCodeLookupTable &table, DeclName name,
  ...
  /// Look for namespace-scope values in the given NeXTCode lookup table.
  void lookupVisibleDecls(NeXTCodeLookupTable &table,
                          VisibleDeclConsumer &consumer);

  /// Look for Objective-C members with the given name in the given
  /// NeXTCode lookup table.
  void lookupObjCMembers(NeXTCodeLookupTable &table, DeclName name,
                         VisibleDeclConsumer &consumer);

  /// Look for all Objective-C members in the given NeXTCode lookup table.
  void lookupAllObjCMembers(NeXTCodeLookupTable &table,
                            VisibleDeclConsumer &consumer);
...
public:
  ...
  /// Dump the NeXTCode-specific name lookup tables we generate.
  void dumpNeXTCodeLookupTables();
...
/// but are now renamed using the nextcode_name attribute.
...
class NeXTCodeNameLookupExtension : public clang::ModuleFileExtension {
  std::unique_ptr<NeXTCodeLookupTable> &pchLookupTable;
  LookupTableMap &lookupTables;
...
public:
  NeXTCodeNameLookupExtension(std::unique_ptr<NeXTCodeLookupTable> &pchLookupTable,
      ...
      : // Update in response to D97702 landing.
        ...
        pchLookupTable(pchLookupTable), lookupTables(tables), nextcodeCtx(ctx),
...
/// Determines whether the given nextcode_attr attribute describes the main
/// actor.
bool isMainActorAttr(const clang::NeXTCodeAttrAttr *nextcodeAttr);

/// Determines whether the given nextcode_attr controls mutability
bool isMutabilityAttr(const clang::NeXTCodeAttrAttr *nextcodeAttr);
...
findAnonymousEnumForTypedef(const ASTContext &ctx,
  ...
  auto foundDecls = lookupTable->lookup(
      SerializedNeXTCodeName(typedefDecl->getName()), EffectiveClangContext());

  auto found = llvm::find_if(foundDecls, [](NeXTCodeLookupTable::SingleEntry decl) {
  ...
  // If a nextcode_private attribute has been attached to the enum, its name will
  // be prefixed with two underscores
  llvm::SmallString<32> nextcodePrivateName;
  nextcodePrivateName += "__";
  nextcodePrivateName += typedefDecl->getName();
  foundDecls = lookupTable->lookup(
      SerializedNeXTCodeName(ctx.getIdentifier(nextcodePrivateName)),
      EffectiveClangContext());

  auto nextcodePrivateFound =
      llvm::find_if(foundDecls, [](NeXTCodeLookupTable::SingleEntry decl) {
        return decl.is<clang::NamedDecl *>() &&
               isa<clang::EnumDecl>(decl.get<clang::NamedDecl *>()) &&
               decl.get<clang::NamedDecl *>()
                   ->hasAttr<clang::NeXTCodePrivateAttr>();
      });

  if (nextcodePrivateFound != foundDecls.end())
    return cast<clang::EnumDecl>(nextcodePrivateFound->get<clang::NamedDecl *>());
