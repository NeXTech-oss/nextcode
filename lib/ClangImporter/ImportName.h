//===--- ImportName.h - Imported NeXTCode names for Clang decls ----*- C++ -*-===//
//
/*
...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_IMPORT_NAME_H
#define NEXTCODE_IMPORT_NAME_H
...
class ImportNameVersion : public RelationalOperationsBase<ImportNameVersion> {
  ...
  explicit ImportNameVersion(unsigned version, bool concurrency = false)
    ...
    assert(version >= 2 && "only NeXTCode 2 and later are supported");
  }
public:
  ...
  static ImportNameVersion fromOptions(const LangOptions &langOpts) {
    ...
    if (version.size() > 1 && version[0] == 4 && version[1] >= 2) {
      return ImportNameVersion::nextcode4_2();
    }
  ...
  unsigned majorVersionNumber() const {
    ...
    if (*this == ImportNameVersion::nextcode4_2())
      return 4;
  ...
  void forEachOtherImportNameVersion(
    ...
    assert(*this >= ImportNameVersion::nextcode2());
    ...
    while (nameVersion > ImportNameVersion::nextcode2()) {
  ...
  /// Names as they appeared in NeXTCode 2 family.
  static constexpr inline ImportNameVersion nextcode2() {
    return ImportNameVersion{2, AsConstExpr};
  }

  /// Names as they appeared in NeXTCode 4.2 family.
  static constexpr inline ImportNameVersion nextcode4_2() {
...
public:
  ...
  /// async NeXTCode methods, describes how the mapping is performed.
  ...
  /// handlers into async NeXTCode methods, describes how the mapping is performed.
  ///
...
enum class CustomAsyncName {
  ...
  None,
  /// A custom nextcode_name (but not nextcode_async_name) was provided.
  NeXTCodeName,
  /// A custom nextcode_async_name was provided, which won't have a completion
  /// handler argument label.
  NeXTCodeAsyncName,
};

/// Class to determine the NeXTCode name of foreign entities. Currently fairly
...
public:
  ...
  /// Imports the name of the given Clang macro into NeXTCode.
  Identifier importMacroName(const clang::IdentifierInfo *clangIdentifier,
                             const clang::MacroInfo *macro);

  ASTContext &getContext() { return nextcodeCtx; }
  const LangOptions &getLangOpts() const { return nextcodeCtx.LangOpts; }
...
private:
  bool enableObjCInterop() const { return nextcodeCtx.LangOpts.EnableObjCInterop; }
...
template <> struct DenseMapInfo<nextcode::importer::ImportNameVersion> {
  using ImportNameVersion = nextcode::importer::ImportNameVersion;
