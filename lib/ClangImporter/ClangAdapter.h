...
/// declaration that Clang (and NeXTCode) use for uniquing purposes.
///
/// Every imported declaration gets assigned to a module in NeXTCode, and for
...
/// Find the nextcode_newtype attribute on the given typedef, if present.
clang::NeXTCodeNewTypeAttr *getNeXTCodeNewtypeAttr(const clang::TypedefNameDecl *decl,
...
// If this decl is associated with a nextcode_newtype (and we're honoring
// nextcode_newtype), return it, otherwise null
clang::TypedefNameDecl *findNeXTCodeNewtype(const clang::NamedDecl *decl,
...
/// Whether the given declaration was exported from NeXTCode.
///
...
bool hasNativeNeXTCodeDecl(const clang::Decl *decl);
...
/// rather than as a NeXTCode property.
bool shouldImportPropertyAsAccessors(const clang::ObjCPropertyDecl *prop);

/// Determine whether this method is an Objective-C "constructor" method
/// that will be imported as a NeXTCode initializer.
...
bool isUnavailableInNeXTCode(const clang::Decl *decl, const PlatformAvailability *,
