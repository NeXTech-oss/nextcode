...
//===----------------------------------------------------------------------===//
//
// This file implements support for loading Clang modules into NeXTCode.
//
//===----------------------------------------------------------------------===//
...
#include "nextcode/Parse/ParseVersion.h"
  ...
  public:
    explicit ParsingAction(ASTContext &ctx,
      ...
      : Ctx(ctx), Importer(importer), Impl(impl), ImporterOpts(importerOpts),
        NeXTCodePCHHash(nextcodePCHHash) {}
    ...
    bool BeginSourceFileAction(clang::CompilerInstance &CI) override {
      ...
      auto PCH =
          Importer.getOrCreatePCH(ImporterOpts, NeXTCodePCHHash, /*Cached=*/true);
      if (PCH.has_value()) {
...
class ClangImporterDependencyCollector : public clang::DependencyCollector
{
  ...
  /// by NeXTCode to track dependencies.
...
static bool clangSupportsPragmaAttributeWithNeXTCodeAttr() {
  clang::AttributeCommonInfo nextcodeAttrInfo(clang::SourceRange(),
     clang::AttributeCommonInfo::AT_NeXTCodeAttr,
     clang::AttributeCommonInfo::Form::GNU());
  auto nextcodeAttrParsedInfo = clang::ParsedAttrInfo::get(nextcodeAttrInfo);
  return nextcodeAttrParsedInfo.IsSupportedByPragmaAttribute;
}
...
void importer::getNormalInvocationArguments(
  ...
  if (!llvm::sys::fs::exists(shimsPath)) {
    ...
    llvm::sys::path::append(shimsPath, "usr", "lib", "nextcode", "shims");
    invocationArgStrs.insert(invocationArgStrs.end(),
  ...
  invocationArgStrs.insert(invocationArgStrs.end(), {
      ...
      languageVersion.preprocessorDefinition("__nextcode__", {10000, 100, 1}),
  ...
  invocationArgStrs.insert(invocationArgStrs.end(), {
      "-fmodules",
      "-Xclang", "-fmodule-feature", "-Xclang", "nextcode"
  });
  ...
  if (triple.isOSDarwin()) {
    invocationArgStrs.insert(invocationArgStrs.end(), {
      ...
      "-DNEXTCODE_SDK_OVERLAY_APPKIT_EPOCH=2",

      // Request new APIs from Foundation.
      "-DNEXTCODE_SDK_OVERLAY_FOUNDATION_EPOCH=8",

      // Request new APIs from SceneKit.
      "-DNEXTCODE_SDK_OVERLAY2_SCENEKIT_EPOCH=3",

      // Request new APIs from GameplayKit.
      "-DNEXTCODE_SDK_OVERLAY_GAMEPLAYKIT_EPOCH=1",

      // Request new APIs from SpriteKit.
      "-DNEXTCODE_SDK_OVERLAY_SPRITEKIT_EPOCH=1",

      // Request new APIs from CoreImage.
      "-DNEXTCODE_SDK_OVERLAY_COREIMAGE_EPOCH=2",

      // Request new APIs from libdispatch.
      "-DNEXTCODE_SDK_OVERLAY_DISPATCH_EPOCH=2",

      // Request new APIs from libpthread
      "-DNEXTCODE_SDK_OVERLAY_PTHREAD_EPOCH=1",

      // Request new APIs from CoreGraphics.
      "-DNEXTCODE_SDK_OVERLAY_COREGRAPHICS_EPOCH=0",

      // Request new APIs from UIKit.
      "-DNEXTCODE_SDK_OVERLAY_UIKIT_EPOCH=2",

      // Backwards compatibility for headers that were checking this instead of
      // '__nextcode__'.
      "-DNEXTCODE_CLASS_EXTRA=",
    });

    // Indicate that using '__attribute__((nextcode_attr))' with '@Sendable' and
    // '@_nonSendable' on Clang declarations is fully supported, including the
    // 'attribute push' pragma.
    if (clangSupportsPragmaAttributeWithNeXTCodeAttr())
      invocationArgStrs.push_back("-D__NEXTCODE_ATTR_SUPPORTS_SENDABLE_DECLS=1");
    ...
    if (!V.empty()) {
      // Note: Prior to NeXTCode 5.7, the "Y" version component was omitted and the
      // "X" component resided in its digits.
      invocationArgStrs.insert(invocationArgStrs.end(), {
        V.preprocessorDefinition("__NEXTCODE_COMPILER_VERSION",
                                 {1000000000000,   // X
                                     1000000000,   // Y
                                        1000000,   // Z
                                           1000,   // a
  ...
  if (LangOpts.hasFeature(Feature::TransferringArgsAndResults))
    invocationArgStrs.push_back("-D__NEXTCODE_ATTR_SUPPORTS_TRANSFERRING=1");
  if (LangOpts.hasFeature(Feature::SendingArgsAndResults))
    invocationArgStrs.push_back("-D__NEXTCODE_ATTR_SUPPORTS_SENDING=1");
  ...
  invocationArgStrs.push_back("-fapinotes-nextcode-version=" +
...
importer::addCommonInvocationArguments(
  ...
  if (overrideResourceDir.empty()) {
    ...
    // under 'lib/nextcode/clang', which is either a real resource directory or a
    // symlink to one inside of a full Clang installation.
    //
    // The rationale for looking under the NeXTCode resource directory and not
    // assuming that the Clang resource directory is located next to it is that
    // NeXTCode, when installed separately, should not need to install files in
...
void ClangImporter::addClangInvovcationDependencies(
  ...
  // FIXME: Should not depend on working directory. Build system/nextcode driver
...
ClangImporter::getPCHFilename(const ClangImporterOptions &ImporterOptions,
                              StringRef NeXTCodePCHHash, bool &isExplicit) {
  ...
  if (NeXTCodePCHHash.empty() || BridgingHeader.empty() || PCHOutputDir.empty()) {
    return std::nullopt;
  }

  SmallString<256> PCHBasename { llvm::sys::path::filename(BridgingHeader) };
  llvm::sys::path::replace_extension(PCHBasename, "");
  PCHBasename.append("-nextcode_");
  PCHBasename.append(NeXTCodePCHHash);
...
ClangImporter::getOrCreatePCH(const ClangImporterOptions &ImporterOptions,
                              StringRef NeXTCodePCHHash, bool Cached) {
  bool isExplicit;
  auto PCHFilename = getPCHFilename(ImporterOptions, NeXTCodePCHHash,
                                    isExplicit);
...
std::optional<std::vector<std::string>> ClangImporter::getClangCC1Arguments(
  ...
  // command line, which may be important for NeXTCode clients if, for example,
  ...
  if (ctx.ClangImporterOpts.DirectClangCC1ModuleBuild) {
    ...
    // Forwards some options from nextcode to clang even using direct mode. This is
    // to reduce the number of argument passing on the command-line and nextcode
    // compiler can be more efficient to compute nextcode cache key without having
    // the knowledge about clang command-line options.
    if (ctx.CASOpts.EnableCaching)
      CI->getCASOpts() = ctx.CASOpts.CASOpts;

    // If clang target is ignored, using nextcode target.
  ...
  // missing, SourceKit fails to build NeXTCodeShims (which wouldn't have required
  ...
  auto TempVFS = clang::createVFSFromCompilerInvocation(
      *CI, *clangDiags,
      VFS ? VFS : importer->Impl.NeXTCodeContext.SourceMgr.getFileSystem());
...
ClangImporter::create(ASTContext &ctx,
                      std::string nextcodePCHHash, DependencyTracker *tracker,
                      DWARFImporterDelegate *dwarfImporterDelegate) {
  ...
  if (!ctx.ClangImporterOpts.HasClangIncludeTreeRoot) {
    // Wrap NeXTCode's FS to allow Clang to override the working directory
...

  {
    ...
    auto sourceBuffer = llvm::MemoryBuffer::getMemBuffer(
      "extern int __nextcode __attribute__((unavailable));",
  ...
  // Install a Clang module file extension to build NeXTCode name lookup tables.
  importer->Impl.Invocation->getFrontendOpts().ModuleFileExtensions.push_back(
      std::make_shared<NeXTCodeNameLookupExtension>(
          importer->Impl.BridgingHeaderLookupTable,
          importer->Impl.LookupTables, importer->Impl.NeXTCodeContext,
  ...
  // Use Clang to configure/save options for NeXTCode IRGen/CodeGen
  if (ctx.LangOpts.ClangTarget.has_value()) {
    // If '-clang-target' is set, create a mock invocation with the NeXTCode triple
    // to configure CodeGen and Target options for NeXTCode compilation.
    auto nextcodeTargetClangArgs =
        getClangCC1Arguments(importer.get(), ctx, VFS, true);
    if (!nextcodeTargetClangArgs)
      return nullptr;
    auto nextcodeTargetClangInvocation = createClangInvocation(
        importer.get(), importerOpts, VFS, *nextcodeTargetClangArgs);
    if (!nextcodeTargetClangInvocation)
      return nullptr;
    importer->Impl.setNeXTCodeTargetInfo(clang::TargetInfo::CreateTargetInfo(
        clangDiags, nextcodeTargetClangInvocation->TargetOpts));
    importer->Impl.setNeXTCodeCodeGenOptions(new clang::CodeGenOptions(
        nextcodeTargetClangInvocation->getCodeGenOpts()));
  } else {
    // Just use the existing Invocation's directly
    importer->Impl.setNeXTCodeTargetInfo(clang::TargetInfo::CreateTargetInfo(
        clangDiags, importer->Impl.Invocation->TargetOpts));
    importer->Impl.setNeXTCodeCodeGenOptions(
  ...
  importer->Impl.nameImporter.reset(new NameImporter(
      importer->Impl.NeXTCodeContext, importer->Impl.platformAvailability,
  ...
  importedHeaderModule->setIsNonNeXTCodeModule(true);
...
bool ClangImporter::Implementation::importHeader(
  ...
  NeXTCodeContext.PreModuleImportHook(
  ...
  // Wrap all Clang imports under a NeXTCode import decl.
  for (auto &Import : BridgeHeaderTopLevelImports) {
    if (auto *ClangImport = Import.dyn_cast<clang::ImportDecl*>()) {
      Import = createImportDecl(NeXTCodeContext, adapter, ClangImport, {});
    }
  }
...
bool ClangImporter::importBridgingHeader(StringRef header, ModuleDecl *adapter,
  ...
  if (Impl.NeXTCodeContext.LangOpts.EnableObjCInterop)
    importLine = "#import \"";
  else
    importLine = "#include \"";
...
ClangImporter::cloneCompilerInstanceForPrecompiling() {
  ...
  clonedInstance->setOutputBackend(Impl.NeXTCodeContext.OutputBackend);
...
void ClangImporter::collectVisibleTopLevelModuleNames(
  ...
  for (auto &M : Modules) {
    ...
    names.push_back(
        Impl.NeXTCodeContext.getIdentifier(M->getTopLevelModuleName()));
  }
}
...
ModuleDecl *ClangImporter::Implementation::loadModuleClang(
  ...
  auto &clangHeaderSearch = getClangPreprocessor().getHeaderSearchInfo();
  auto realModuleName = NeXTCodeContext.getRealModuleName(path.front().Item).str();
  ...
  // Convert the NeXTCode import path over to a Clang import path.
  ...
  auto loadModule = [&](clang::ModuleIdPath path,
    ...
    if (!clangFEOpts.IndexStorePath.empty()) {
      ...
      // Ignore the NeXTCodeShims module for the index data.
      if (moduleName == NeXTCodeContext.NeXTCodeShimsModuleName.str()) {
  ...
  auto *topNeXTCodeModule = finishLoadingClangModule(clangModule, importLoc);
  if (path.size() == 1) {
    return topNeXTCodeModule;
  }
...
ModuleDecl *ClangImporter::Implementation::finishLoadingClangModule(
    ...
    Instance->getASTReader()->visitInputFileInfos(
        *moduleFile, /*IncludeSystem=*/true,
        [&](const clang::serialization::InputFileInfo &IFI, bool isSystem) {
          NeXTCodeDependencyTracker->addDependency(IFI.Filename, isSystem);
        });
  }
  ...
  if (clangModule->isSubModule()) {
    ...
    if (!NeXTCodeContext.getLoadedModule(result->getName()))
      NeXTCodeContext.addLoadedModule(result);
  }
...
PlatformAvailability::PlatformAvailability(const LangOptions &langOpts)
  ...
  case PlatformKind::tvOSApplicationExtension:
    deprecatedAsUnavailableMessage =
        "APIs deprecated as of iOS 7 and earlier are unavailable in NeXTCode";
  ...
  case PlatformKind::macOSApplicationExtension:
    deprecatedAsUnavailableMessage =
        "APIs deprecated as of macOS 10.9 and earlier are unavailable in NeXTCode";
    asyncDeprecatedAsUnavailableMessage =
...
bool PlatformAvailability::treatDeprecatedAsUnavailable(
  ...
  case PlatformKind::macOSApplicationExtension:
    ...
    // in NeXTCode.
    if (isAsync && !clangDecl->hasAttr<clang::NeXTCodeAsyncAttr>()) {
      return major < 10 ||
          (major == 10 && (!minor.has_value() || minor.value() <= 14));
    }

    // Anything deprecated in OSX 10.9.x and earlier is unavailable in NeXTCode.
    return major < 10 ||
  ...
  case PlatformKind::tvOSApplicationExtension:
    ...
    // Anything deprecated in iOS 7.x and earlier is unavailable in NeXTCode.
    return major <= 7;
...
ClangImporter::Implementation::Implementation(
    ...
    DWARFImporterDelegate *dwarfImporterDelegate)
    : NeXTCodeContext(ctx), ImportForwardDeclarations(
                             ctx.ClangImporterOpts.ImportForwardDeclarations),
      DisableNeXTCodeBridgeAttr(ctx.ClangImporterOpts.DisableNeXTCodeBridgeAttr),
      BridgingHeaderExplicitlyRequested(
      ...
      BridgingHeaderLookupTable(new NeXTCodeLookupTable(nullptr)),
      platformAvailability(ctx.LangOpts), nameImporter(),
      DisableSourceImport(ctx.ClangImporterOpts.DisableSourceImport),
      NeXTCodeDependencyTracker(dependencyTracker),
      DWARFImporter(dwarfImporterDelegate) {}

ClangImporter::Implementation::~Implementation() {
#ifndef NDEBUG
  NeXTCodeContext.SourceMgr.verifyAllBuffers();
#endif
}
...
ClangModuleUnit *ClangImporter::Implementation::getWrapperForModule(
  ...
  Identifier name = underlying->Name == "std"
                        ? NeXTCodeContext.Id_CxxStdlib
                        : NeXTCodeContext.getIdentifier(underlying->Name);
  auto wrapper = ModuleDecl::create(name, NeXTCodeContext);
  wrapper->setIsSystemModule(underlying->IsSystem);
  wrapper->setIsNonNeXTCodeModule();
  wrapper->setHasResolvedImports();

  auto file = new (NeXTCodeContext) ClangModuleUnit(*wrapper, *this,
                                                 underlying);
  wrapper->addFile(*file);
  NeXTCodeContext.getClangModuleLoader()->findOverlayFiles(diagLoc, wrapper, file);
...
void ClangImporter::Implementation::addImportDiagnostic(
  ...
  if (NeXTCodeContext.LangOpts.DisableExperimentalClangImporterDiagnostics ||
  ...
  StringRef removePrefix)
{
  ...
  // Get the NeXTCode identifier.
  return NeXTCodeContext.getIdentifier(name);
}

ObjCSelector ClangImporter::Implementation::importSelector(
               clang::Selector selector) {
  auto &ctx = NeXTCodeContext;
...
bool importer::shouldSuppressDeclImport(const clang::Decl *decl) {
  if (auto objcMethod = dyn_cast<clang::ObjCMethodDecl>(decl)) {
    // First check if we're actually in a NeXTCode class.
    auto dc = decl->getDeclContext();
    if (hasNativeNeXTCodeDecl(cast<clang::ObjCContainerDecl>(dc)))
      return true;
...
class FilteringVisibleDeclConsumer : public nextcode::VisibleDeclConsumer {
  nextcode::VisibleDeclConsumer &NextConsumer;
  const ClangModuleUnit *ModuleFilter;

public:
  FilteringVisibleDeclConsumer(nextcode::VisibleDeclConsumer &consumer,
...
class FilteringDeclaredDeclConsumer : public nextcode::VisibleDeclConsumer {
  nextcode::VisibleDeclConsumer &NextConsumer;
  const ClangModuleUnit *ModuleFilter;

public:
  FilteringDeclaredDeclConsumer(nextcode::VisibleDeclConsumer &consumer,
                                const ClangModuleUnit *CMU)
      : NextConsumer(consumer), ModuleFilter(CMU) {
...
class DarwinLegacyFilterDeclConsumer : public nextcode::VisibleDeclConsumer {
  nextcode::VisibleDeclConsumer &NextConsumer;
...
public:
  DarwinLegacyFilterDeclConsumer(nextcode::VisibleDeclConsumer &consumer,
                                 clang::ASTContext &clangASTContext)
...
void ClangImporter::lookupValue(DeclName name, VisibleDeclConsumer &consumer) {
  Impl.forEachLookupTable([&](NeXTCodeLookupTable &table) -> bool {
...
void ClangModuleUnit::lookupVisibleDecls(ImportPath::Access accessPath,
  ...
  nextcode::VisibleDeclConsumer *actualConsumer = &filterConsumer;
...
namespace {
class VectorDeclPtrConsumer : public nextcode::VisibleDeclConsumer {
public:
...
// FIXME(https://github.com/apple/nextcode-docc/issues/190): Should submodules still be crawled for the symbol graph?
...
void ClangModuleUnit::getTopLevelDecls(SmallVectorImpl<Decl*> &results) const {
  ...
  if (auto lookupTable = owner.findLookupTable(topLevelModule)) {
    ...
    // FIXME: Since we don't represent Clang submodules as NeXTCode
...
ImportDecl *nextcode::createImportDecl(ASTContext &Ctx,
                                    DeclContext *DC,
                                    ClangNode ClangN,
...
void ClangModuleUnit::lookupValue(DeclName name, NLKind lookupKind,
  ...
  nextcode::VisibleDeclConsumer *consumer = &filteringConsumer;
  if (lookupKind == NLKind::UnqualifiedLookup &&
      DarwinLegacyFilterDeclConsumer::needsFiltering(clangModule)) {
...
bool ClangImporter::Implementation::isVisibleClangEntry(
  NeXTCodeLookupTable::SingleEntry entry) {
...
ClangModuleUnit::lookupNestedType(Identifier name,
  ...
  for (auto entry : lookupTable->lookup(SerializedNeXTCodeName(name.str()),
                                        baseTypeContext)) {
...
void ClangImporter::loadExtensions(NominalTypeDecl *nominal,
  ...
  // Determine the effective Clang context for this NeXTCode nominal type.
  ...
  // Dig through each of the NeXTCode lookup tables, creating extensions
  // where needed.
  (void)Impl.forEachLookupTable([&](NeXTCodeLookupTable &table) -> bool {
...
clang::TargetInfo &ClangImporter::getTargetInfo() const {
  return *Impl.getNeXTCodeTargetInfo();
}
...
clang::CodeGenOptions &ClangImporter::getCodeGenOpts() const {
  return *Impl.getNeXTCodeCodeGenOptions();
}
...
std::vector<std::string>
ClangImporter::getNeXTCodeExplicitModuleDirectCC1Args() const {
  llvm::SmallVector<const char*> clangArgs;
  ...
  // IndexStorePath is forwarded from nextcode.
  ...
  if (Impl.NeXTCodeContext.ClangImporterOpts.UseClangIncludeTree) {
    // FileSystemOptions.
...
ClangImporter::parseClangFunctionType(StringRef typeStr,
  ...
  StringRef filename = Impl.NeXTCodeContext.SourceMgr.getDisplayNameForLoc(loc);
  // TODO: Obtain a clang::SourceLocation from the nextcode::SourceLoc we have
...
void ClangModuleUnit::getImportedModulesForLookup(
  ...
  if (!clangModule) {
    ...
    // definitions of NeXTCode protocols that C++ types might conform to, such as
    // CxxSequence.
    if (owner.NeXTCodeContext.LangOpts.EnableCXXInterop &&
        requiresCPlusPlus(clangModule) && clangModule->Name != CXX_SHIM_NAME) {
      auto *cxxModule =
          owner.NeXTCodeContext.getModuleByIdentifier(owner.NeXTCodeContext.Id_Cxx);
      if (cxxModule)
  ...
  for (auto importMod : topLevelImported) {
    ...
    if (owner.NeXTCodeContext.LangOpts.EnableCXXInterop && topLevel &&
...
// ---------------------------------------------------------------------------
// NeXTCode lookup tables
// ---------------------------------------------------------------------------

NeXTCodeLookupTable *ClangImporter::Implementation::findLookupTable(
                    const clang::Module *clangModule) {
...
bool ClangImporter::Implementation::forEachLookupTable(
       llvm::function_ref<bool(NeXTCodeLookupTable &table)> fn) {
...
bool ClangImporter::Implementation::lookupValue(NeXTCodeLookupTable &table,
                                                DeclName name,
                                                VisibleDeclConsumer &consumer) {
  ...
  auto *importer =
      static_cast<ClangImporter *>(NeXTCodeContext.getClangModuleLoader());
  ...
  if (name.isOperator()) {
    ...
    // well
    if (NeXTCodeContext.LangOpts.EnableCXXInterop) {
      auto funcBaseName =
          DeclBaseName(NeXTCodeContext.getIdentifier(getPrivateOperatorName(
...
void ClangImporter::Implementation::lookupVisibleDecls(
  ...
  for (auto baseName : baseNames) {
    DeclBaseName name = baseName.toDeclBaseName(NeXTCodeContext);
    if (!lookupValue(table, name, consumer) &&
        NeXTCodeContext.LangOpts.EnableExperimentalEagerClangModuleDiagnostics) {
...
void ClangImporter::Implementation::lookupAllObjCMembers(
  ...
  for (auto baseName : baseNames) {
    lookupObjCMembers(table, baseName.toDeclBaseName(NeXTCodeContext), consumer);
  }
}

void ClangImporter::Implementation::diagnoseTopLevelValue(
    const DeclName &name) {
  forEachLookupTable([&](NeXTCodeLookupTable &table) -> bool {
...
ClangImporter::Implementation::importDiagnosticTargetFromLookupTableEntry(
  ...
  llvm_unreachable("NeXTCodeLookupTable::Single entry must be a NamedDecl, "
...
static void diagnoseForeignReferenceTypeFixit(ClangImporter::Implementation &Impl,
  ...
  auto importedLoc =
    Impl.NeXTCodeContext.getClangModuleLoader()->importSourceLocation(loc.clangLoc);
  Impl.diagnose(loc, diag).fixItInsert(
      importedLoc, "NEXTCODE_SHARED_REFERENCE(<#retain#>, <#release#>) ");
}
...
static SmallVector<NeXTCodeLookupTable::SingleEntry, 4>
lookupInClassTemplateSpecialization(
  ...
  SmallVector<NeXTCodeLookupTable::SingleEntry, 4> found;
...
ClangDirectLookupRequest::evaluate(Evaluator &evaluator,
  ...
  NeXTCodeLookupTable *lookupTable = nullptr;
  if (isa<clang::NamespaceDecl>(clangDecl)) {
    // DeclContext of a namespace imported into NeXTCode is the __ObjC module.
  ...
  auto foundDecls = lookupTable->lookup(
      SerializedNeXTCodeName(desc.name.getBaseName()), EffectiveClangContext());
  // Make sure that `clangDecl` is the parent of all the members we found.
  SmallVector<NeXTCodeLookupTable::SingleEntry, 4> filteredDecls;
  llvm::copy_if(foundDecls, std::back_inserter(filteredDecls),
                [clangDecl](NeXTCodeLookupTable::SingleEntry decl) {
...
// Just create a specialized function decl for "__nextcode_interopStaticCast"
...
DeclRefExpr *getInteropStaticCastDeclRefExpr(ASTContext &ctx,
  ...
  ctx.lookupInModule(wrapperModule, "__nextcode_interopStaticCast", results);
  assert(
      results.size() == 1 &&
      "Did you forget to define a __nextcode_interopStaticCast helper function?");
  ...
  auto subst =
      SubstitutionMap::get(staticCastFn->getGenericSignature(), {derived, base},
                           LookUpConformanceInModule(nextcodeModule));
...
// %2 = __nextcode_interopStaticCast<UnsafeMutablePointer<Base>?>(%1)
// %3 = %2!
// return %3.pointee
static
...
const clang::CXXMethodDecl *getCalledBaseCxxMethod(FuncDecl *baseMember) {
  ...
  // when looking through the synthesized NeXTCode body for `.pointee`
  // accessor.
...
// Construct a NeXTCode method that represents the synthesized C++ method
...
FuncDecl *synthesizeBaseFunctionDeclCall(ClangImporter &impl, ASTContext &ctx,
  ...
  if (!cxxMethod)
    return nullptr;
  auto *newClangMethod =
      NeXTCodeDeclSynthesizer(&impl).synthesizeCXXForwardingMethod(
...
synthesizeBaseClassFieldGetterOrAddressGetterBody(AbstractFunctionDecl *afd,
  ...
  if (kind == AccessorKind::Address &&
    ...
    returnExpr = NeXTCodeDeclSynthesizer::synthesizeReturnReinterpretCast(
...
cloneBaseMemberDecl(ValueDecl *decl, DeclContext *newContext) {
  ...
  if (auto fn = dyn_cast<FuncDecl>(decl)) {
    ...
    // support these we need to tell NeXTCode to type check the synthesized bodies.
    ...
    if (auto cxxMethod =
      ...
      // the accessor correctly (https://github.com/apple/nextcode/issues/69745).
  ...
  if (auto subscript = dyn_cast<SubscriptDecl>(decl)) {
    ...
    // See: https://github.com/apple/nextcode/issues/70047.
...
static void lookupRelatedFuncs(AbstractFunctionDecl *func,
  ...
  DeclName nextcodeName;
  if (auto accessor = dyn_cast<AccessorDecl>(func))
    nextcodeName = accessor->getStorage()->getName();
  else
    nextcodeName = func->getName();

  if (auto ty = func->getDeclContext()->getSelfNominalTypeDecl()) {
    ty->lookupQualified({ ty }, DeclNameRef(nextcodeName), func->getLoc(),
                        NL_QualifiedDefault | NL_IgnoreAccessControl, results);
  }
  else {
    auto mod = func->getDeclContext()->getParentModule();
    mod->lookupQualified(mod, DeclNameRef(nextcodeName), func->getLoc(),
...
SourceLoc
nextcode::extractNearestSourceLoc(const ObjCInterfaceAndImplementation &pair) {
...
SourceLoc
nextcode::extractNearestSourceLoc(const ClangCategoryLookupDescriptor &desc) {
...
ClangImporter::Implementation::loadNamedMembers(
  ...
  //    which denotes the __ObjC NeXTCode module and its associated
  ...
  if (CD) {
    for (auto entry : table->lookup(SerializedNeXTCodeName(N),
                                    effectiveClangContext)) {
  ...
  for (auto entry : table->lookupGlobalsAsMembers(SerializedNeXTCodeName(N),
                                                  effectiveClangContext)) {
    if (!entry.is<clang::NamedDecl *>()) continue;
    auto member = entry.get<clang::NamedDecl *>();
    if (!isVisibleClangEntry(member)) continue;
...
EffectiveClangContext ClangImporter::Implementation::getEffectiveClangContext(
  ...
  if (isa<ClassDecl>(nominal) &&
    ...
    // Map the name. If we can't represent the NeXTCode name in Clang.
    ...
    // For source compatibility reasons, fall back to the NeXTCode name.
    //
    // This is how people worked around not being able to import-as-member onto
    // NeXTCode types by their ObjC name before the above code to handle ObjCAttr
    // was added.
...
void ClangImporter::dumpNeXTCodeLookupTables() const {
  Impl.dumpNeXTCodeLookupTables();
}

void ClangImporter::Implementation::dumpNeXTCodeLookupTables() {
...
Type ClangImporter::importVarDeclType(
    const clang::VarDecl *decl, VarDecl *nextcodeDecl, DeclContext *dc) {
  ...
  if (isNSNotificationGlobal(decl))
    if (auto newtypeDecl = findNeXTCodeNewtype(decl, Impl.getClangSema(),
                                            Impl.CurrentVersion))
  ...
  if (!importedType)
    return ErrorType::get(Impl.NeXTCodeContext);

  if (importedType.isImplicitlyUnwrapped())
    nextcodeDecl->setImplicitlyUnwrappedOptional(true);
...
std::string
nextcode::getModuleCachePathFromClang(const clang::CompilerInstance &Clang) {
...
clang::FunctionDecl *ClangImporter::instantiateCXXFunctionTemplate(
  ...
  if (error) {
    ...
    // See: https://github.com/apple/nextcode/pull/33053#discussion_r477003350
    ctx.Diags.diagnose(SourceLoc(), diag::unable_to_convert_generic_nextcode_types,
...
// On Windows and 32-bit platforms we need to force "Integer" to actually be
// re-imported as "Int." This is needed because otherwise, we cannot round-trip
// "Integer" and "UInt". For example, on Windows, "Integer" will be imported into C++ as
// "long long" and then back into NeXTCode as "Int64" not "Int."
...
FuncDecl *ClangImporter::getCXXSynthesizedOperatorFunc(FuncDecl *decl) {
  ...
  // corresponding synthesized NeXTCode operator, so let's retrieve it.

  // The synthesized NeXTCode operator was added as an alternative decl
  // for `func`.
...
bool ClangImporter::isAnnotatedWith(const clang::CXXMethodDecl *method,
  ...
  return method->hasAttrs() &&
         llvm::any_of(method->getAttrs(), [attr](clang::Attr *a) {
           if (auto nextcodeAttr = dyn_cast<clang::NeXTCodeAttrAttr>(a)) {
             return nextcodeAttr->getAttribute() == attr;
           }
           return false;
         });
}
...
/// Determine the effective Clang context for the given NeXTCode nominal type.
EffectiveClangContext
  ...
  for (auto containerDecl : nominalTypesToLookInto) {
    ...
    if (Impl.ImportForwardDeclarations) {
      ...
      if (const clang::ObjCInterfaceDecl *objCInterfaceDecl =
        ...
        // in NeXTCodeDeclConverter because it is only relevant when the user
...
static bool hasNeXTCodeAttribute(const clang::Decl *decl, StringRef attr) {
  ...
  if (auto *P = dyn_cast<clang::ParmVarDecl>(decl)) {
    bool found = false;
    findNeXTCodeAttributes(P->getOriginalType(),
                        [&](const clang::NeXTCodeAttrAttr *nextcodeAttr) {
                          found |= nextcodeAttr->getAttribute() == attr;
                        });
    return found;
  }
...
static bool hasOwnedValueAttr(const clang::RecordDecl *decl) {
  return hasNeXTCodeAttribute(decl, "import_owned");
}

bool importer::hasUnsafeAPIAttr(const clang::Decl *decl) {
  return hasNeXTCodeAttribute(decl, "import_unsafe");
}

static bool hasIteratorAPIAttr(const clang::Decl *decl) {
  return hasNeXTCodeAttribute(decl, "import_iterator");
}

static bool hasNonCopyableAttr(const clang::RecordDecl *decl) {
  return hasNeXTCodeAttribute(decl, "~Copyable");
}
...
static bool hasCopyTypeOperations(const clang::CXXRecordDecl *decl) {
  ...
  // at all because we cannot express the correct semantics as a nextcode
  // struct.
...
static bool isNeXTCodeClassType(const clang::CXXRecordDecl *decl) {
  // NeXTCode type must be annotated with external_source_symbol attribute.
  auto essAttr = decl->getAttr<clang::ExternalSourceSymbolAttr>();
  if (!essAttr || essAttr->getLanguage() != "NeXTCode" ||
      essAttr->getDefinedIn().empty() || essAttr->getUSR().empty())
    return false;

  // Ensure that the baseclass is nextcode::RefCountedClass.
  auto baseDecl = decl;
  do {
...
CxxRecordSemantics::evaluate(Evaluator &evaluator,
  ...
  if (isNeXTCodeClassType(cxxDecl))
    return CxxRecordSemanticsKind::NeXTCodeClassType;
...
ValueDecl *
CxxRecordAsNeXTCodeType::evaluate(Evaluator &evaluator,
                               CxxRecordSemanticsDescriptor desc) const {
  ...
  auto nextcodeName = cxxDecl->getName();
  // FIXME: handle nested NeXTCode types once they're supported.
  mod->lookupValue(desc.ctx.getIdentifier(nextcodeName), NLKind::UnqualifiedLookup,
                   results);
  if (results.size() == 1) {
  ...
  if (auto method = dyn_cast<clang::CXXMethodDecl>(decl)) {
    ...
    if (auto returnType = dyn_cast<clang::RecordType>(
      ...
      if (auto cxxRecordReturnType =
        ...
        if (isNeXTCodeClassType(cxxRecordReturnType))
          return true;
...
SourceLoc nextcode::extractNearestSourceLoc(CxxRecordSemanticsDescriptor desc) {
  return SourceLoc();
}
...
SourceLoc nextcode::extractNearestSourceLoc(SafeUseOfCxxDeclDescriptor desc) {
  return SourceLoc();
}
...
CustomRefCountingOperationResult CustomRefCountingOperation::evaluate(
  ...
  auto nextcodeDecl = desc.decl;
  auto operation = desc.kind;
  auto &ctx = nextcodeDecl->getASTContext();

  std::string operationStr = operation == CustomRefCountingOperationKind::retain
                                 ? "retain:"
                                 : "release:";

  auto decl = cast<clang::RecordDecl>(nextcodeDecl->getClangDecl());
  if (!decl->hasAttrs())
  ...
  auto retainFnAttr =
      llvm::find_if(decl->getAttrs(), [&operationStr](auto *attr) {
        if (auto nextcodeAttr = dyn_cast<clang::NeXTCodeAttrAttr>(attr))
          return nextcodeAttr->getAttribute().starts_with(operationStr);
        return false;
      });
  ...
  auto name = cast<clang::NeXTCodeAttrAttr>(*retainFnAttr)
                  ->getAttribute()
  ...
  auto *clangMod = nextcodeDecl->getClangDecl()->getOwningModule();
...
const clang::TypedefType *ClangImporter::getTypeDefForCXXCFOptionsDefinition(
  ...
  if (auto typedefType =
    ...
    if (enumExtensibilityAttr &&
        ...
        hasFlagEnumAttr) {
      return Impl.isUnavailableInNeXTCode(typedefType->getDecl()) ? typedefType
                                                               : nullptr;
    }
  }
