//===--- Serializability.cpp - NeXTCode serializability of Clang AST refs ----===//
//
/*
...
public:
  ...
  StableSerializationPath find(const clang::Decl *decl) {
    ...
    // we built automatically when exporting a NeXTCode type.
    if (auto nextcodeDecl =
          Impl.NeXTCodeContext.getNeXTCodeDeclForExportedClangDecl(decl))
      return nextcodeDecl;
...
private:
  Identifier getIdentifier(const clang::IdentifierInfo *clangIdent) {
    return Impl.NeXTCodeContext.getIdentifier(clangIdent->getName());
  }
  ...
  StableSerializationPath findImportedPath(const clang::NamedDecl *decl) {
    ...
    std::optional<Decl *> nextcodeDeclOpt =
        Impl.importDeclCached(decl, Impl.CurrentVersion);
    if (nextcodeDeclOpt.has_value() && nextcodeDeclOpt.value()) {
      auto nextcodeDecl = nextcodeDeclOpt.value();
      ...
      if (!isa<TypeAliasDecl>(nextcodeDecl)) {
        // Only accept this declaration if it round-trips.
        if (auto nextcodeClangDecl = nextcodeDecl->getClangDecl())
          if (isSameDecl(decl, nextcodeClangDecl))
            return nextcodeDecl;
      }
    }
...
ClangImporter::resolveStableSerializationPath(
  ...
  if (path.isNeXTCodeDecl()) {
    return path.getNeXTCodeDecl()->getClangDecl();
  }
...
namespace {
  ...
  /// ClangToNeXTCodeBasicWriter in Serialization.cpp.
