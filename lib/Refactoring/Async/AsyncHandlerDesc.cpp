...
nextcode::Type AsyncHandlerDesc::getType() const {
  if (auto Var = Handler.dyn_cast<const VarDecl *>()) {
...
std::optional<nextcode::Type> AsyncHandlerDesc::getErrorType() const {
  if (HasError) {
    switch (Type) {
...
nextcode::Type
AsyncHandlerDesc::getSuccessParamAsyncReturnType(nextcode::Type Ty) const {
  switch (Type) {
  case HandlerType::PARAMS: {
