...
static llvm::cl::opt<bool> KeepWillThrowCall(
    ...
    llvm::cl::desc(
      "Keep calls to nextcode_willThrow, even if the throw is optimized away"));

std::optional<SILBasicBlock::iterator>
nextcode::getInsertAfterPoint(SILValue val) {
...
nextcode::createIncrementBefore(SILValue ptr, SILInstruction *insertPt) {
...
/// reference-semantics.
NullablePtr<SILInstruction>
nextcode::createDecrementBefore(SILValue ptr, SILInstruction *insertPt) {
...
bool nextcode::canTriviallyDeleteOSSAEndScopeInst(SILInstruction *i) {
...
bool nextcode::isInstructionTriviallyDead(SILInstruction *inst) {
...
bool nextcode::isIntermediateRelease(SILInstruction *inst,
...
bool nextcode::hasOnlyEndOfScopeOrEndOfLifetimeUses(SILInstruction *inst) {
...
unsigned nextcode::getNumInOutArguments(FullApplySite applySite) {
  assert(applySite);
...
void nextcode::collectUsesOfValue(SILValue v,
...
void nextcode::eraseUsesOfValue(SILValue v) {
...
bool nextcode::hasValueDeinit(SILType type) {
...
bool nextcode::mayBindDynamicSelf(SILFunction *F) {
...
bool nextcode::isAddressOfArrayElement(SILValue addr) {
...
void nextcode::placeFuncRef(ApplyInst *ai, DominanceInfo *domInfo) {
...
/// instruction.
TermInst *nextcode::addArgumentsToBranch(ArrayRef<SILValue> vals,
...
SILLinkage nextcode::getSpecializedLinkage(SILFunction *f, SILLinkage linkage) {
...
nextcode::castValueToABICompatibleType(SILBuilder *builder, SILPassManager *pm,
                                    SILLocation loc,
    ...
    if (destTy.is<SILFunctionType>()) {
      assert(srcTy.getAs<SILFunctionType>()->isNoEscape()
             ...
             || srcTy.getAs<SILFunctionType>()->getRepresentation()
                    ...
                    && "NeXTCode thick functions that differ in escapeness are "
                       "not ABI "
                       "compatible");
...
bool nextcode::layoutIsTypeDependent(NominalTypeDecl *decl) {
...
ProjectBoxInst *nextcode::getOrCreateProjectBox(AllocBoxInst *abi,
                                             unsigned index) {
...
nextcode::findInitAddressForTrivialEnum(UncheckedTakeEnumDataAddrInst *utedai) {
...
void nextcode::emitDestroyOperation(SILBuilder &builder, SILLocation loc,
...
void nextcode::releasePartialApplyCapturedArg(SILBuilder &builder, SILLocation loc,
                                           SILValue arg,
                                           SILParameterInfo paramInfo,
...
void nextcode::getConsumedPartialApplyArgs(PartialApplyInst *pai,
...
bool nextcode::collectDestroys(SingleValueInstruction *inst,
...
bool nextcode::tryDeleteDeadClosure(SingleValueInstruction *closure,
                                 InstModCallbacks callbacks,
...
bool nextcode::simplifyUsers(SingleValueInstruction *inst) {
  bool changed = false;
  InstModCallbacks callbacks;
...
bool nextcode::shouldExpand(SILModule &module, SILType ty) {
  // FIXME: Expansion
...
bool nextcode::analyzeStaticInitializer(
...
bool nextcode::canReplaceLoadSequence(SILInstruction *inst) {
...
/// tuple_element_addr?
void nextcode::replaceLoadSequence(SILInstruction *inst, SILValue value) {
  if (auto *cai = dyn_cast<CopyAddrInst>(inst)) {
...
nextcode::findLocalApplySites(FunctionRefBaseInst *fri) {
...
void nextcode::insertDestroyOfCapturedArguments(
...
void nextcode::insertDeallocOfCapturedArguments(
...
AbstractFunctionDecl *nextcode::getBaseMethod(AbstractFunctionDecl *FD) {
...
FullApplySite
nextcode::cloneFullApplySiteReplacingCallee(FullApplySite applySite,
                                         SILValue newCallee,
...
// deleting them.
SILBasicBlock::iterator nextcode::replaceAllUses(SILValue oldValue,
                                              SILValue newValue,
...
nextcode::replaceAllUsesAndErase(SingleValueInstruction *svi, SILValue newValue,
                              InstModCallbacks &callbacks) {
...
SILBasicBlock::iterator
nextcode::replaceAllUsesAndErase(SILValue oldValue, SILValue newValue,
                              InstModCallbacks &callbacks) {
...
SILBasicBlock::iterator nextcode::replaceSingleUse(Operand *use, SILValue newValue,
                                                InstModCallbacks &callbacks) {
...
SILValue nextcode::makeCopiedValueAvailable(SILValue value, SILBasicBlock *inBlock) {
...
SILValue nextcode::makeValueAvailable(SILValue value, SILBasicBlock *inBlock) {
  if (isa<SILUndef>(value))
    return value;
...
bool nextcode::tryEliminateOnlyOwnershipUsedForwardingInst(
...
void nextcode::endLifetimeAtLeakingBlocks(SILValue value,
...
void nextcode::salvageStoreDebugInfo(SILInstruction *SI,
...
// the same variable.
void nextcode::salvageDebugInfo(SILInstruction *I) {
  if (!I)
    return;
...
void nextcode::salvageLoadDebugInfo(LoadOperation load) {
...
// TODO: this currently fails to notify the pass with notifyNewInstruction.
void nextcode::createDebugFragments(SILValue oldValue, Projection proj,
                                 SILValue newValue) {
...
IntegerLiteralInst *nextcode::optimizeBuiltinCanBeObjCClass(BuiltinInst *bi,
                                                         SILBuilder &builder) {
...
SILValue nextcode::createEmptyAndUndefValue(SILType ty,
...
bool nextcode::findUnreferenceableStorage(StructDecl *decl, SILType structType,
                                       SILFunction *func) {
...
SILValue nextcode::getInitOfTemporaryAllocStack(AllocStackInst *asi) {
