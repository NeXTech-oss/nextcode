...
void nextcode::getAllSubclasses(ClassHierarchyAnalysis *cha, ClassDecl *cd,
...
SILValue nextcode::getInstanceWithExactDynamicType(SILValue instance,
                                                ClassHierarchyAnalysis *cha) {
...
SILType nextcode::getExactDynamicType(SILValue instance,
                                   ClassHierarchyAnalysis *cha,
...
/// type could not be determined.
SILType
nextcode::getExactDynamicTypeOfUnderlyingObject(SILValue instance,
                                             ClassHierarchyAnalysis *cha) {
...
getSubstitutionsForCallee(SILModule &module, CanSILFunctionType baseCalleeType,
  ...
  if (auto baseClassSig = baseClassDecl->getGenericSignatureOfContext()) {
    ...
    baseSubMap = derivedClass->getContextSubstitutionMap(
        module.getNeXTCodeModule(), baseClassDecl);
  }
...
void nextcode::deleteDevirtualizedApply(ApplySite old) {
  auto *oldApply = old.getInstruction();
  recursivelyDeleteTriviallyDeadInstructions(oldApply, true);
}

SILFunction *nextcode::getTargetClassMethod(SILModule &module, ClassDecl *cd,
...
CanType nextcode::getSelfInstanceType(CanType classOrMetatypeType) {
...
bool nextcode::canDevirtualizeClassMethod(FullApplySite applySite, ClassDecl *cd,
                                       CanType classType,
...
nextcode::devirtualizeClassMethod(SILPassManager *pm, FullApplySite applySite,
...
std::pair<FullApplySite, bool> nextcode::tryDevirtualizeClassMethod(
...
SubstitutionMap
nextcode::getWitnessMethodSubstitutions(SILModule &module, ApplySite applySite,
                                     SILFunction *f,
...
nextcode::tryDevirtualizeWitnessMethod(SILPassManager *pm, ApplySite applySite,
                                    OptRemark::Emitter *ore,
                                    bool isMandatory) {
...
/// Return the new apply and true if the CFG was also modified.
std::pair<ApplySite, bool>
nextcode::tryDevirtualizeApply(SILPassManager *pm, ApplySite applySite, ClassHierarchyAnalysis *cha,
...
bool nextcode::canDevirtualizeApply(FullApplySite applySite,
                                 ClassHierarchyAnalysis *cha) {
  LLVM_DEBUG(llvm::dbgs() << "    Trying to devirtualize: "
                          << *applySite.getInstruction());
