...
void SILInlineCloner::cloneInline(ArrayRef<SILValue> AppliedArgs) {
  ...
  if (!Apply->getFunction()->hasOwnership()) {
    ...
    for (auto p : llvm::enumerate(AppliedArgs)) {
      SILValue callArg = p.value();
      NEXTCODE_DEFER { entryArgs.push_back(callArg); };
...
InlineCost nextcode::instructionInlineCost(SILInstruction &I) {
  switch (I.getKind()) {
  ...
  // TODO: These are free if the metatype is for a NeXTCode class.
