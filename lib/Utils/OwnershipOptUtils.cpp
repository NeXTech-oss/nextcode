...
void nextcode::extendOwnedLifetime(SILValue ownedValue,
...
void nextcode::extendLocalBorrow(BeginBorrowInst *beginBorrow,
...
bool nextcode::computeGuaranteedBoundary(SILValue value,
...
bool nextcode::areUsesWithinLexicalValueLifetime(SILValue value,
...
class BorrowedLifetimeExtender {
  ...
  NEXTCODE_ASSERT_ONLY_DECL(llvm::SmallDenseSet<PhiOperand, 4> reborrowedOperands);
...
void BorrowedLifetimeExtender::analyzeExtendedScope() {
  auto visitReborrow = [&](Operand *endScope) {
    if (auto borrowingOper = BorrowingOperand(endScope)) {
      ...
      NEXTCODE_ASSERT_ONLY(reborrowedOperands.insert(endScope));
...
SILValue nextcode::makeGuaranteedValueAvailable(SILValue value,
                                             SILInstruction *user,
...
OwnershipRAUWHelper::OwnershipRAUWHelper(OwnershipFixupContext &inputCtx,
  ...
  NEXTCODE_ASSERT_ONLY_DECL(auto borrowedVal = BorrowedValue(inputOldValue));
...
SILValue OwnershipRAUWHelper::prepareReplacement(SILValue rewrittenNewValue) {
  ...
  NEXTCODE_DEFER { newValue = SILValue(); };
...
// OSSA is fully valid.
bool nextcode::createBorrowScopeForPhiOperands(SILPhiArgument *newPhi) {
...
bool nextcode::extendStoreBorrow(StoreBorrowInst *sbi,
...
void nextcode::updateBorrowedFrom(SILPassManager *pm, SILFunction *f) {
  if (updateBorrowedFromFunction)
    updateBorrowedFromFunction({pm->getNeXTCodePassInvocation()}, {f});
}

void nextcode::updateBorrowedFromPhis(SILPassManager *pm, ArrayRef<SILPhiArgument *> phis) {
  ...
  updateBorrowedFromPhisFunction({pm->getNeXTCodePassInvocation()}, ArrayRef(bridgedPhis));
}
