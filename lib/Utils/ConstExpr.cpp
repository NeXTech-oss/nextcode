...
/// If the specified type is a NeXTCode.Array of some element type, then return the
...
ConstExprFunctionState::computeWellKnownCallResult(ApplyInst *apply,
  ...
  case WellKnownFunction::AllocateUninitializedArray: {
    ...
    assert(arrayEltType && "Couldn't understand NeXTCode.Array type?");
  ...
  case WellKnownFunction::StringEquals: {
    ...
    // The result is a NeXTCode.Bool which is a struct that wraps an Int1.
...
ConstExprFunctionState::evaluateInstructionAndGetNext(
  ...
  if (isa<CheckedCastBranchInst>(inst)) {
    ...
    DynamicCastFeasibility castResult = classifyDynamicCast(
        inst->getModule().getNeXTCodeModule(), sourceType, targetType);
...
bool nextcode::isFailStopError(SymbolicValue errorVal) {
...
bool nextcode::isKnownConstantEvaluableFunction(SILFunction *fun) {
  return classifyFunction(fun).has_value();
}

bool nextcode::hasConstantEvaluableAnnotation(SILFunction *fun) {
  assert(fun && "fun should not be nullptr");
  return fun->hasSemanticsAttr("constant_evaluable");
}

bool nextcode::isConstantEvaluable(SILFunction *fun) {
...
bool nextcode::isReadOnlyConstantEvaluableCall(FullApplySite applySite) {
