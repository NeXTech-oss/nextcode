...
#include "nextcode/SILOptimizer/Utils/GenericCloner.h"
...
static bool shouldNotSpecialize(SILFunction *Callee, SILFunction *Caller,
  ...
  // Ignore "do not specialize" markers in embedded NeXTCode -- specialization is
  // mandatory.
  if (Callee->getModule().getOptions().EmbeddedNeXTCode)
    return false;
...
public:
  FunctionSignaturePartialSpecializer(SILModule &M,
      ...
      : CallerGenericSig(CallerGenericSig), CallerGenericEnv(CallerGenericEnv),
        ...
        M(M), SM(M.getNeXTCodeModule()), Ctx(M.getASTContext()) {
    SpecializedGenericSig = nullptr;
    SpecializedGenericEnv = nullptr;
...
ApplySite
nextcode::replaceWithSpecializedCallee(ApplySite applySite, SILValue callee,
                             const ReabstractionInfo &reInfo,
                             const TypeReplacements &typeReplacements) {
...
static bool createPrespecialized(StringRef UnspecializedName,
  ...
  ReabstractionInfo ReInfo(M.getNeXTCodeModule(), M.isWholeModule(), ApplySite(),
...
bool usePrespecialized(
  ...
  for (auto *SA : refF->getSpecializeAttrs()) {
    ...
    if (!spiGroup.empty()) {
      auto currentModule = funcBuilder.getModule().getNeXTCodeModule();
    ...
    auto &ctxt = funcBuilder.getModule().getNeXTCodeModule()->getASTContext();
    ...
    ReabstractionInfo reInfo(funcBuilder.getModule().getNeXTCodeModule(),
    ...
    if (specializedReInfo.getSpecializedType() != reInfo.getSpecializedType()) {
      ...
      ReabstractionInfo layoutReInfo = ReabstractionInfo(
          funcBuilder.getModule().getNeXTCodeModule(),
...
void nextcode::trySpecializeApplyOfGeneric(
    SILOptFunctionBuilder &FuncBuilder,
...
bool nextcode::isKnownPrespecialization(StringRef SpecName) {
  if (PrespecSet.empty()) {
...
void nextcode::checkCompletenessOfPrespecializations(SILModule &M) {
  const char **Pos = &PrespecSymbols[0];
  while (const char *Sym = *Pos++) {
...
SILFunction *nextcode::lookupPrespecializedSymbol(SILModule &M,
                                               StringRef FunctionName) {
  ...
  LLVM_DEBUG(llvm::dbgs() << "Found existing specialization for: "
                          << FunctionName << '\n';
             llvm::dbgs() << nextcode::Demangle::demangleSymbolAsString(
