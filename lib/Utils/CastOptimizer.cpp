...
#include "nextcode/SILOptimizer/Utils/CastOptimizer.h"
...
//===----------------------------------------------------------------------===//
//                  ObjC -> NeXTCode Bridging Cast Optimization
//===----------------------------------------------------------------------===//

static SILFunction *
getObjCToNeXTCodeBridgingFunction(SILOptFunctionBuilder &funcBuilder,
...
static SubstitutionMap lookupBridgeToObjCProtocolSubs(SILModule &mod,
  ...
  auto conf = mod.getNeXTCodeModule()->lookupConformance(target, bridgedProto);
...
/// instance into a corresponding NeXTCode type, conforming to
...
///
SILInstruction *
CastOptimizer::optimizeBridgedObjCToNeXTCodeCast(SILDynamicCastInst dynamicCast) {
  ...
  SILFunction *bridgingFunc =
      getObjCToNeXTCodeBridgingFunction(functionBuilder, dynamicCast);
  if (!bridgingFunc)
    return nullptr;
  ...
  // case. E.g. casts from NSArray into [NeXTCodeType] may fail, i.e. return .None.
  if (isConditional) {
...
//===----------------------------------------------------------------------===//
//                  NeXTCode -> ObjC Bridging Cast Optimization
//===----------------------------------------------------------------------===//

static bool canOptimizeCast(const nextcode::Type &BridgedTargetTy,
                            nextcode::SILModule &M,
                            nextcode::SILFunctionConventions &substConv,
  ...
  if (ConvTy.getASTType() ==
      getNSBridgedClassOfCFClass(M.getNeXTCodeModule(),
                                 DestTy.getASTType())) {
    return true;
  }
...
findBridgeToObjCFunc(SILOptFunctionBuilder &functionBuilder,
  ...
  auto conf = mod.getNeXTCodeModule()->lookupConformance(
  ...
  if (resultsRef.empty()) {
    mod.getNeXTCodeModule()->lookupMember(
  ...
  auto subMap = sourceFormalType->getContextSubstitutionMap(
      mod.getNeXTCodeModule(), resultDecl->getDeclContext());
...
static SILValue computeFinalCastedValue(SILBuilderWithScope &builder,
  ...
  if (convTy.getASTType() ==
          getNSBridgedClassOfCFClass(m.getNeXTCodeModule(), destLoweredTy.getASTType()) ||
      destLoweredTy.getASTType() ==
          getNSBridgedClassOfCFClass(m.getNeXTCodeModule(), convTy.getASTType())) {
    // Handle NS <-> CF toll-free bridging here.
    return SILValue(builder.createUncheckedRefCast(loc, newAI, destLoweredTy));
  }

  llvm_unreachable(
      "optimizeBridgedNeXTCodeToObjCCast: should never reach this condition: if "
...
SILInstruction *
CastOptimizer::optimizeBridgedNeXTCodeToObjCCast(SILDynamicCastInst dynamicCast) {
  SILInstruction *Inst = dynamicCast.getInstruction();
...
CastOptimizer::optimizeBridgedCasts(SILDynamicCastInst dynamicCast) {
  ...
  if (CanBridgedSourceTy != source && CanBridgedTargetTy != target) {
    // Both source and target type are NeXTCode types.
    return nullptr;
  }
  ...
  // Check what kind of conversion it is? ObjC->NeXTCode or NeXTCode-ObjC?
  if (CanBridgedTargetTy != target) {
    // This is an ObjC to NeXTCode cast.
    return optimizeBridgedObjCToNeXTCodeCast(dynamicCast);
  } else {
    // This is a NeXTCode to ObjC cast
    return optimizeBridgedNeXTCodeToObjCCast(dynamicCast);
  }
...
SILInstruction *CastOptimizer::simplifyCheckedCastAddrBranchInst(
  ...
  if (!BridgedI) {
    ...
    bool result = emitSuccessfulIndirectUnconditionalCast(
      Builder, Builder.getModule().getNeXTCodeModule(), Loc, Src,
...
SILInstruction *CastOptimizer::optimizeUnconditionalCheckedCastAddrInst(
  ...
  if (Feasibility == DynamicCastFeasibility::WillSucceed ||
    ...
    // NeXTCode optimizer does not claim to be crash-preserving.
    SILValue dest = dynamicCast.getDest();
    bool ResultNotUsed = isa<AllocStackInst>(dest);
