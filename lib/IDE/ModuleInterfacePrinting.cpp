...
void nextcode::ide::collectModuleGroups(ModuleDecl *M,
...
/// created it (vs. being a NeXTCode extension).
static bool extensionHasClangNode(ExtensionDecl *ext) {
  return static_cast<bool>(nextcode::ide::extensionGetClangNode(ext));
}

std::optional<StringRef> nextcode::ide::findGroupNameForUSR(ModuleDecl *M,
                                                         StringRef USR) {
...
static bool printModuleInterfaceDecl(Decl *D,
  ...
  if (auto Ext = dyn_cast<ExtensionDecl>(D)) {
    ...
    // NeXTCode extensions are printed with their associated type unless it's
...
/// Sorts NeXTCode declarations for display.
static bool compareNeXTCodeDecls(Decl *LHS, Decl *RHS) {
...
getDeclsFromCrossImportOverlay(ModuleDecl *Overlay, ModuleDecl *Declaring,
  ...
  nextcode::getTopLevelDeclsForDisplay(Overlay, Decls);
  ...
  nextcode::getTopLevelDeclsForDisplay(Declaring, DeclaringDecls);
  ...
  std::sort(Remainder.begin(), Remainder.end(), compareNeXTCodeDecls);
...
void nextcode::ide::printModuleInterface(
  ...
  auto &NeXTCodeContext = TopLevelMod->getASTContext();
  auto &Importer =
  ...
  nextcode::getTopLevelDeclsForDisplay(TopLevelMod, Decls);

  SmallVector<ImportDecl *, 1> ImportDecls;
  llvm::DenseSet<const clang::Module *> ClangModulesForImports;
  SmallVector<Decl *, 1> NeXTCodeDecls;
  ...
  for (Decl *D : Decls) {
    ...
    if (!IsSubmodule) {
      ...
      // Add NeXTCode decls if we are printing the top-level module.
      NeXTCodeDecls.push_back(D);
    }
  }
  if (!GroupNames.empty()) {
    assert(NeXTCodeDecls.empty());
  ...
  // Sort NeXTCode declarations so that we print them in a consistent order.
  ...
  if (GroupNames.empty())
    std::stable_sort(NeXTCodeDecls.begin(), NeXTCodeDecls.end(), compareNeXTCodeDecls);
  ...
  if (!(TraversalOptions & ModuleTraversal::SkipOverlay) || !TargetClangMod) {
    for (auto *D : NeXTCodeDecls) {
      if (PrintDecl(D) && Options.EmptyLineBetweenDecls)
        Printer.printNewline();
    }

    // If we're printing the entire target module (not specific sub-groups),
    // also print the decls from any underscored NeXTCode cross-import overlays it
    // is the underlying module of, transitively.
    if (GroupNames.empty()) {
      printCrossImportOverlays(TargetMod, NeXTCodeContext, Printer,
...
void nextcode::ide::printNeXTCodeSourceInterface(SourceFile &File,
                                           ASTPrinter &Printer,
                                           const PrintOptions &Options) {

  // We print all comments before the first line of NeXTCode code.
...
void nextcode::ide::printHeaderInterface(
       StringRef Filename,
       ASTContext &Ctx,
       ASTPrinter &Printer,
       const PrintOptions &Options) {
  ...
  for (auto *D : ClangDecls) {
    ...
    // equivalent NeXTCode decl may not be. E.g. a top-level function may be mapped
    // to a property accessor in NeXTCode.
    D = getTopLevelDecl(D);
...
void nextcode::ide::printSymbolicNeXTCodeClangModuleInterface(
    ModuleDecl *M, ASTPrinter &Printer, const clang::Module *clangModule) {
  std::string headerComment;
  llvm::raw_string_ostream(headerComment)
      << "// NeXTCode interface for " << (clangModule->IsSystem ? "system " : "")
  ...
  auto &NeXTCodeContext = M->getTopLevelModule()->getASTContext();
  auto &Importer =
      static_cast<ClangImporter &>(*NeXTCodeContext.getClangModuleLoader());
