...
void nextcode::getTopLevelDeclsForDisplay(ModuleDecl *M,
...
void nextcode::getTopLevelDeclsForDisplay(
...
struct SynthesizedExtensionAnalyzer::Implementation {
  ...
  struct ExtensionMergeInfo {
    ...
    void addRequirement(nextcode::Requirement Req) {
...
ProtocolDecl* nextcode::resolveProtocolName(DeclContext *dc, StringRef name) {
  return evaluateOrDefault(dc->getASTContext().evaluator,
                           ResolveProtocolNameRequest(ProtocolNameOwner(dc, name)),
                           nullptr);
}

ArrayRef<ExpressionTypeInfo> nextcode::collectExpressionType(
...
void nextcode::collectVariableType(
...
bool nextcode::isExtensionApplied(const DeclContext *DC, Type BaseTy,
                               const ExtensionDecl *ED) {
  return evaluateOrDefault(DC->getASTContext().evaluator,
    IsDeclApplicableRequest(DeclApplicabilityOwner(DC, BaseTy, ED)), false);
}

bool nextcode::isMemberDeclApplied(const DeclContext *DC, Type BaseTy,
                                const ValueDecl *VD) {
  return evaluateOrDefault(DC->getASTContext().evaluator,
    IsDeclApplicableRequest(DeclApplicabilityOwner(DC, BaseTy, VD)), false);
}

Type nextcode::tryMergeBaseTypeForCompletionLookup(Type ty1, Type ty2,
                                                DeclContext *dc) {
...
bool nextcode::isConvertibleTo(Type T1, Type T2, bool openArchetypes,
                            DeclContext &DC) {
  return evaluateOrDefault(DC.getASTContext().evaluator,
...
bool nextcode::isSubtypeOf(Type T1, Type T2, DeclContext *DC) {
  return evaluateOrDefault(DC->getASTContext().evaluator,
    TypeRelationCheckRequest(TypeRelationCheckInput(DC, T1, T2,
      TypeRelation::SubtypeOf, /*openArchetypes*/ false)), false);
}

Type nextcode::getRootTypeOfKeypathDynamicMember(SubscriptDecl *SD) {
  return evaluateOrDefault(SD->getASTContext().evaluator,
    RootTypeOfKeypathDynamicMemberRequest{SD}, Type());
}

Type nextcode::getResultTypeOfKeypathDynamicMember(SubscriptDecl *SD) {
  return evaluateOrDefault(SD->getASTContext().evaluator,
...
nextcode::getShorthandShadows(CaptureListExpr *CaptureList, DeclContext *DC) {
...
SmallVector<std::pair<ValueDecl *, ValueDecl *>, 1>
nextcode::getShorthandShadows(LabeledConditionalStmt *CondStmt, DeclContext *DC) {
  SmallVector<std::pair<ValueDecl *, ValueDecl *>, 1> Result;
