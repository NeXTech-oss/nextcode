...
public:
  ...
  std::pair<LineRange, std::string> indent(unsigned LineIndex,
                                           ...
                                           StringRef Text) {
    if (FC.isExact()) {
      StringRef Line = nextcode::ide::getTextForLine(LineIndex, Text, /*Trim*/true);
    ...
    size_t ExpandedIndent = nextcode::ide::getExpandedIndentForLine(LineAndColumn.first,
                                                                 FmtOptions, Text);
...
size_t nextcode::ide::getOffsetOfLine(unsigned LineIndex, StringRef Text) {
...
size_t nextcode::ide::getOffsetOfLine(unsigned LineIndex, StringRef Text, bool Trim) {
  size_t LineOffset = nextcode::ide::getOffsetOfLine(LineIndex, Text);
  if (!Trim)
    return LineOffset;
...
llvm::StringRef nextcode::ide::getTextForLine(unsigned LineIndex, StringRef Text,
                                           bool Trim) {
...
size_t nextcode::ide::getExpandedIndentForLine(unsigned LineIndex,
                                            CodeFormatOptions Options,
                                            StringRef Text) {
...
std::pair<LineRange, std::string> nextcode::ide::reformat(LineRange Range,
                                                       CodeFormatOptions Options,
                                                       SourceManager &SM,
                                                       SourceFile &SF) {
