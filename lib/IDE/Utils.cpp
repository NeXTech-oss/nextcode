...
static const char *OSXModuleList[] = {
  ...
  "PubSub",
  "Python",
  //  "QTKit", QTKit is unavailable on NeXTCode.
  "Quartz",
  "QuartzCore",
  "QuickLook",
  "QuickTime",
...
void nextcode::ide::SourceEditConsumer::
...
void nextcode::ide::SourceEditConsumer::acceptMacroExpansionBuffer(
...
struct nextcode::ide::SourceEditJsonConsumer::Implementation {
  llvm::raw_ostream &OS;
...
nextcode::ide::SourceEditJsonConsumer::SourceEditJsonConsumer(llvm::raw_ostream &OS) :
  Impl(*new Implementation(OS)) {}

nextcode::ide::SourceEditJsonConsumer::~SourceEditJsonConsumer() { delete &Impl; }

void nextcode::ide::SourceEditJsonConsumer::
...
void nextcode::ide::SourceEditTextConsumer::
accept(SourceManager &SM, RegionType Type, ArrayRef<Replacement> Replacements) {
  for (const auto &Replacement: Replacements) {
    OS << "// ";
...
struct nextcode::ide::SourceEditOutputConsumer::Implementation {
...
nextcode::ide::SourceEditOutputConsumer::
SourceEditOutputConsumer(SourceManager &SM, unsigned BufferId,
  llvm::raw_ostream &OS) : Impl(*new Implementation(SM, BufferId, OS)) {}

nextcode::ide::SourceEditOutputConsumer::~SourceEditOutputConsumer() { delete &Impl; }

void nextcode::ide::SourceEditOutputConsumer::
...
void nextcode::ide::BroadcastingSourceEditConsumer::accept(
...
bool nextcode::ide::isFromClang(const Decl *D) {
...
ClangNode nextcode::ide::getEffectiveClangNode(const Decl *decl) {
...
ClangNode nextcode::ide::extensionGetClangNode(const ExtensionDecl *ext) {
...
std::pair<Type, ConcreteDeclRef> nextcode::ide::getReferencedDecl(Expr *expr,
                                                               bool semantic) {
  if (semantic)
...
bool nextcode::ide::isBeingCalled(ArrayRef<Expr *> ExprStack) {
  if (ExprStack.empty())
    return false;
...
Expr *nextcode::ide::getBase(ArrayRef<Expr *> ExprStack) {
  if (ExprStack.empty())
    return nullptr;
...
bool nextcode::ide::isDeclOverridable(ValueDecl *D) {
...
bool nextcode::ide::isDynamicRef(Expr *Base, ValueDecl *D, llvm::function_ref<Type(Expr *)> getType) {
...
void nextcode::ide::getReceiverType(Expr *Base,
...
extern "C" {
/// Low-level entry point to run the NameMatcher written in nextcode-syntax.
///
/// - Parameters:
...
void *nextcode_NeXTCodeIDEUtilsBridging_runNameMatcher(const void *sourceFilePtr,
                                                 BridgedSourceLoc *locations,
                                                 size_t locationsCount);
}

std::vector<ResolvedLoc>
nextcode::ide::runNameMatcher(const SourceFile &sourceFile,
  ...
  BridgedResolvedLocVector bridgedResolvedLocs =
      nextcode_NeXTCodeIDEUtilsBridging_runNameMatcher(
