...
std::string nextcode::ide::removeCodeCompletionTokens(
...
class CodeCompletionCallbacksImpl : public CodeCompletionCallbacks,
  ...
  bool typecheckParsedType() {
    ...
    const auto ty = nextcode::performTypeResolution(
...
void CodeCompletionCallbacksImpl::completeExprKeyPath(KeyPathExpr *KPE,
  ...
  Kind = (!KPE || KPE->isObjC()) ? CompletionKind::KeyPathExprObjC
                                 : CompletionKind::KeyPathExprNeXTCode;
  ParsedExpr = KPE;
  this->DotLoc = DotLoc;
  CurDeclContext = P.CurDeclContext;
}
...
bool nextcode::ide::isDynamicLookup(Type T) {
...
static void addAccessorKeywords(CodeCompletionResultSink &Sink) {
  ...
  addKeyword(Sink, "constructor", CodeCompletionKeywordKind::None);
}
...
void nextcode::ide::addExprKeywords(CodeCompletionResultSink &Sink,
                                 DeclContext *DC) {
...
void nextcode::ide::addSuperKeyword(CodeCompletionResultSink &Sink,
                                 DeclContext *DC) {
  if (!DC)
    return;
...
void CodeCompletionCallbacksImpl::addKeywords(CodeCompletionResultSink &Sink,
  ...
  case CompletionKind::KeyPathExprNeXTCode:
...
static void addPoundDirectives(CodeCompletionResultSink &Sink) {
  ...
  addWithName("sourceLocation", CodeCompletionKeywordKind::pound_sourceLocation,
    ...
    Builder.addCallParameterColon();
    Builder.addSimpleTypedParameter("Integer");
    Builder.addRightParen();
  });

#ifndef NEXTCODE_BUILD_NEXTCODE_SYNTAX
...
static void addPlatformConditions(CodeCompletionResultSink &Sink) {
  ...
  addWithName("nextcode", [](CodeCompletionResultBuilder &Builder) {
    Builder.addTextChunk(">=");
  ...
  addKeyword(Sink, "true", CodeCompletionKeywordKind::kw_true, "Boolean");
  addKeyword(Sink, "false", CodeCompletionKeywordKind::kw_false, "Boolean");
}
...
void nextcode::ide::postProcessCompletionResults(
...
void nextcode::ide::collectCompletionResults(
  ...
  for (auto &Request: Lookup.RequestedCachedResults) {
    ...
    auto handleImport = [&](ImportedModule Import) {
      ...
      if (!ModuleFilename.empty()) {
        ...
        using PairType = llvm::DenseSet<nextcode::ide::CodeCompletionCache::Key,
...
void CodeCompletionCallbacksImpl::postfixCompletion(SourceLoc CompletionLoc,
  ...
  if (auto AE = dyn_cast<ApplyExpr>(ParsedExpr)) {
    if (AE->getArgs()->hasAnyTrailingClosures() &&
      ...
      AE->setArgs(ArgList);
      NEXTCODE_DEFER { AE->setArgs(OriginalArgs); };
...
nextcode::ide::makeCodeCompletionCallbacksFactory(
    CodeCompletionContext &CompletionContext,
    CodeCompletionConsumer &Consumer) {
  return new CodeCompletionCallbacksFactoryImpl(CompletionContext, Consumer);
}

void nextcode::ide::lookupCodeCompletionResultsFromModule(
