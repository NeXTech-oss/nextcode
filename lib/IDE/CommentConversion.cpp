...
using namespace nextcode::markup;
...
#include "nextcode/Markup/SimpleFields.def"
  ...
  void printDocument(const Document *D) {
    llvm_unreachable("Can't print a nextcode::markup::Document as XML directly");
  }
  ...
  void printPrivateExtension(const PrivateExtension *PE) {
    llvm_unreachable("Can't directly print a NeXTCode Markup PrivateExtension");
  }
  ...
  void visitCommentParts(const nextcode::markup::CommentParts &Parts);
};
} // unnamed namespace

void CommentToXMLConverter::visitCommentParts(const nextcode::markup::CommentParts &Parts) {
...
static void
replaceObjcDeclarationsWithNeXTCodeOnes(const Decl *D, StringRef Doc,
                                     raw_ostream &OS,
...
static LineList getLineListFromComment(SourceManager &SourceMgr,
  ...
  auto Tokens = nextcode::tokenize(LangOpts, SourceMgr,
  ...
  if (MaybeClangNode) {
    if (auto *CD = MaybeClangNode.getAsDecl()) {
      ...
      if (getClangDocumentationCommentAsXML(CD, SS)) {
        replaceObjcDeclarationsWithNeXTCodeOnes(D, SS.str(), OS,
                                             SynthesizedTarget);
        return true;
      }
    }
...
bool ide::getRawDocumentationComment(const Decl *D, raw_ostream &OS) {
  ...
  OS << nextcode::markup::MarkupContext().getLineList(rawComment).str();
  OS.flush();
  return true;
}
...
bool ide::convertMarkupToXML(StringRef Text, raw_ostream &OS) {
  ...
  if (auto *Doc = nextcode::markup::parseDocument(MC, LL)) {
    CommentToXMLConverter Converter(OS);
...
public:
  ...
  void visitPrivateExtension(const PrivateExtension *PE) {
    llvm_unreachable("Can't directly print Doxygen for a NeXTCode Markup PrivateExtension");
  }
