...
#include "nextcode/IDE/SyntaxModel.h"
...
SyntaxModelContext::SyntaxModelContext(SourceFile &SrcFile)
  ...
  for (unsigned I = 0, E = Tokens.size(); I != E; ++I) {
    ...
    if (AttrLoc.isValid()) {
      ...
      // Type attribute, decl attribute, or '@unknown' for nextcode case statement.
      if (TypeAttribute::getAttrKindFromString(Tok.getText()).has_value() ||
        ...
        // syntax coloring. If nextcode gets user attributes then all identifiers
...
ASTWalker::PostWalkAction ModelASTWalker::walkToDeclPost(nextcode::Decl *D) {
  while (!SubStructureStack.empty() &&
