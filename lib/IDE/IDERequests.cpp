...
void nextcode::registerIDERequestFunctions(Evaluator &evaluator) {
...
void nextcode::simple_display(llvm::raw_ostream &out, const CursorInfoOwner &owner) {
  if (!owner.isValid())
    return;
...
void nextcode::ide::simple_display(llvm::raw_ostream &out,
...
void
nextcode::ide::simple_display(llvm::raw_ostream &out, const ResolvedRangeInfo &info) {
  info.print(out);
}
