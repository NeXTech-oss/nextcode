...
void nextcode::ide::printCodeCompletionResultDescription(
...
class AnnotatingResultPrinter {
  ...
  void printWithTag(StringRef tag, StringRef content) {
    ...
    OS << "<" << tag << ">";
    nextcode::markup::appendWithXMLEscaping(
  ...
  void printTextChunk(CodeCompletionString::Chunk C) {
    ...
    case ChunkKind::Whitespace:
      ...
      break;
    default:
      nextcode::markup::appendWithXMLEscaping(OS, C.getText());
      break;
    }
  }
...
void nextcode::ide::printCodeCompletionResultDescriptionAnnotated(
...
void nextcode::ide::printCodeCompletionResultTypeName(const CodeCompletionResult &Result,
                                                   llvm::raw_ostream &OS) {
  auto Chunks = Result.getCompletionString()->getChunks();
  auto i = Chunks.begin(), e = Chunks.end();
...
void nextcode::ide::printCodeCompletionResultTypeNameAnnotated(const CodeCompletionResult &Result, llvm::raw_ostream &OS) {
  AnnotatingResultPrinter printer(OS);
...
void nextcode::ide::printCodeCompletionResultSourceText(
...
NullTerminatedStringRef nextcode::ide::getCodeCompletionResultFilterName(
