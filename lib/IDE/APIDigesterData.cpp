...
#include "nextcode/IDE/APIDigesterData.h"
...
StringRef nextcode::ide::api::getDeclKindStr(const DeclKind Value, bool lower) {
  if (lower) {
    switch (Value) {
...
raw_ostream &nextcode::ide::api::operator<<(raw_ostream &Out, const DeclKind Value) {
  return Out << getDeclKindStrRaw(Value);
}

std::optional<SDKNodeKind>
nextcode::ide::api::parseSDKNodeKind(StringRef Content) {
...
NodeAnnotation nextcode::ide::api::parseSDKNodeAnnotation(StringRef Content) {
...
SpecialCaseId nextcode::ide::api::parseSpecialCaseId(StringRef Content) {
...
nextcode::ide::api::CommonDiffItem::
...
StringRef nextcode::ide::api::CommonDiffItem::head() {
  return "SDK_CHANGE";
}

bool nextcode::ide::api::CommonDiffItem::operator<(CommonDiffItem Other) const {
...
void nextcode::ide::api::CommonDiffItem::describe(llvm::raw_ostream &os) {
  os << "#ifndef " << head() << "\n";
...
void nextcode::ide::api::CommonDiffItem::undef(llvm::raw_ostream &os) {
  os << "#undef " << head() << "\n";
}

void nextcode::ide::api::CommonDiffItem::streamDef(llvm::raw_ostream &S) const {
...
StringRef nextcode::ide::api::TypeMemberDiffItem::head() {
  return "SDK_CHANGE_TYPE_MEMBER";
}

TypeMemberDiffItemSubKind
nextcode::ide::api::TypeMemberDiffItem::getSubKind() const {
...
void nextcode::ide::api::TypeMemberDiffItem::describe(llvm::raw_ostream &os) {
  os << "#ifndef " << head() << "\n";
...
void nextcode::ide::api::TypeMemberDiffItem::undef(llvm::raw_ostream &os) {
  os << "#undef " << head() << "\n";
}

void nextcode::ide::api::TypeMemberDiffItem::streamDef(llvm::raw_ostream &os) const {
...
bool nextcode::ide::api::TypeMemberDiffItem::
operator<(TypeMemberDiffItem Other) const {
  return usr.compare(Other.usr) < 0;
}

StringRef nextcode::ide::api::NoEscapeFuncParam::head() {
  return "NOESCAPE_FUNC_PARAM";
}

void nextcode::ide::api::NoEscapeFuncParam::describe(llvm::raw_ostream &os) {
  os << "#ifndef " << head() << "\n";
  os << "#define " << head() << "(USR, Index)\n";
  os << "#endif\n";
}

void nextcode::ide::api::NoEscapeFuncParam::undef(llvm::raw_ostream &os) {
  os << "#undef " << head() << "\n";
}

void nextcode::ide::api::NoEscapeFuncParam::
streamDef(llvm::raw_ostream &os) const {
  os << head() << "(" << "\"" << Usr << "\"" << ", "
     << "\"" << Index << "\"" << ")";
}

bool nextcode::ide::api::NoEscapeFuncParam::
...
StringRef nextcode::ide::api::OverloadedFuncInfo::head() {
  return "OVERLOAD_FUNC_TRAILING_CLOSURE";
}

void nextcode::ide::api::OverloadedFuncInfo::describe(llvm::raw_ostream &os) {
  os << "#ifndef " << head() << "\n";
  os << "#define " << head() << "(USR)\n";
  os << "#endif\n";
}

void nextcode::ide::api::OverloadedFuncInfo::undef(llvm::raw_ostream &os) {
  os << "#undef " << head() << "\n";
}

void nextcode::ide::api::OverloadedFuncInfo::
streamDef(llvm::raw_ostream &os) const {
  os << head() << "(" << "\"" << Usr << "\"" << ")";
}

bool nextcode::ide::api::OverloadedFuncInfo::
operator<(OverloadedFuncInfo Other) const {
  return Usr.compare(Other.Usr) < 0;
}

#define DIFF_ITEM_KIND(NAME)                                                   \
bool nextcode::ide::api::NAME::classof(const APIDiffItem *D) {                    \
...
void nextcode::ide::api::APIDiffItemStore::
...
struct nextcode::ide::api::APIDiffItemStore::Implementation {
private:
...
ArrayRef<APIDiffItem*> nextcode::ide::api::APIDiffItemStore::
...
nextcode::ide::api::APIDiffItemStore::APIDiffItemStore(DiagnosticEngine &Diags) :
  Impl(*new Implementation(Diags)) {}

nextcode::ide::api::APIDiffItemStore::~APIDiffItemStore() { delete &Impl; }

void nextcode::ide::api::APIDiffItemStore::addStorePath(StringRef Path) {
  Impl.addStorePath(Path);
}

void nextcode::ide::api::APIDiffItemStore::printIncomingUsr(bool print) {
  Impl.PrintUsr = print;
}
