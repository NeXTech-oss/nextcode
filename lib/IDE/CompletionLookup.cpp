...
static bool NeXTCodeKeyPathFilter(ValueDecl *decl, DeclVisibilityKind) {
  switch (decl->getKind()) {
  case DeclKind::Var:
...
bool nextcode::ide::DefaultFilter(ValueDecl *VD, DeclVisibilityKind Kind,
                               DynamicLookupInfo dynamicLookupInfo) {
  return true;
}

bool nextcode::ide::KeyPathFilter(ValueDecl *decl, DeclVisibilityKind,
                               DynamicLookupInfo dynamicLookupInfo) {
  return isa<TypeDecl>(decl) ||
         (isa<VarDecl>(decl) && decl->getDeclContext()->isTypeContext());
}

bool nextcode::ide::MacroFilter(ValueDecl *decl, DeclVisibilityKind,
                             DynamicLookupInfo dynamicLookupInfo) {
  return isa<MacroDecl>(decl);
}

bool nextcode::ide::isCodeCompletionAtTopLevel(const DeclContext *DC) {
...
bool nextcode::ide::isCompletionDeclContextLocalContext(DeclContext *DC) {
...
bool nextcode::ide::canDeclContextHandleAsync(const DeclContext *DC) {
...
bool nextcode::ide::isCodeCompletionAtTopLevelOfLibraryFile(
...
void CompletionLookup::addValueBaseName(CodeCompletionResultBuilder &Builder,
  ...
  if (Name.isSpecial()) {
    ...
    shouldEscapeKeywords = NameStr == "self" || NameStr == "constructor";
  } else {
...
void CompletionLookup::addMethodCall(const FuncDecl *FD,
  ...
  // Do not add imported C++ methods that are treated as unsafe in NeXTCode.
...
void CompletionLookup::addConstructorCall(const ConstructorDecl *CD,
  ...
  auto addConstructorImpl = [&](bool includeDefaultArgs = true) {
    ...
    if (needInit) {
      assert(addName.empty());
      addLeadingDot(Builder);
      Builder.addBaseName("constructor");
...
void CompletionLookup::addConstructorCallsForType(
  ...
  auto results =
      nextcode::lookupSemanticMember(const_cast<DeclContext *>(CurrDeclContext),
...
void CompletionLookup::addSubscriptCall(const SubscriptDecl *SD,
  ...
  // Subscript after '.' is valid only after type part of NeXTCode keypath
  // expression. (e.g. '\TyName.SubTy.[0])
  if (HaveDot && !IsAfterNeXTCodeKeyPathRoot)
    return;
...
void CompletionLookup::foundDecl(ValueDecl *D, DeclVisibilityKind Reason,
  ...
  if (IsNeXTCodeKeyPathExpr && !NeXTCodeKeyPathFilter(D, Reason))
    return;
...
bool CompletionLookup::tryUnwrappedCompletions(Type ExprType, bool isIUO) {
  ...
  if (Type Unwrapped = ExprType->getOptionalObjectType()) {
    llvm::SaveAndRestore<bool> ChangeNeedOptionalUnwrap(NeedOptionalUnwrap,
                                                        true);
    if (DotLoc.isValid()) {
      // Let's not erase the dot if the completion is after a nextcode key path
      // root because \A?.?.member is the correct way to access wrapped type
      // member from an optional key path root.
      auto loc = IsAfterNeXTCodeKeyPathRoot ? DotLoc.getAdvancedLoc(1) : DotLoc;
...
void CompletionLookup::addValueLiteralCompletions() {
  ...
  // FIXME: the pedantically correct way is to resolve NeXTCode.*LiteralType.
...
#define AVAILABILITY_PLATFORM(X, PrettyName)                                   \
  addDeclAttrParamKeyword(nextcode::platformString(PlatformKind::X),              \
...
void CompletionLookup::getTypeAttributeKeywordCompletions() {
  ...
  addTypeAttr("convention(nextcode)");
