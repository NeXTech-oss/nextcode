//===--- ParseType.cpp - NeXTCode Language Parser for Types ------------------===//
//
/*
...
#include "nextcode/AST/SourceFile.h" // only for isMacroSignatureFile
...
Parser::parseType(Diag<> MessageID, ParseTypeReason reason, bool fromASTGen) {
  ...
  if (IsForASTGen && !fromASTGen) {
    ...
    // Note: there is a representational difference between the nextcode-syntax
    // tree and the C++ parser tree regarding variadic parameters. In the
    // nextcode-syntax tree, the ellipsis is part of the parameter declaration.
...
bool Parser::canParseTypeTupleBody() {
  if (Tok.isNot(tok::r_paren) && Tok.isNot(tok::r_brace) &&
      ...
      // tuple type.
      (Tok.is(tok::kw_inout) || !isStartOfNeXTCodeDecl())) {
    do {
      ...
      if (hadParameterName && consumeIf(tok::equal)) {
        while (Tok.isNot(tok::eof) && Tok.isNot(tok::r_paren) &&
               ...
               Tok.isNot(tok::comma) && !isStartOfNeXTCodeDecl()) {
          skipSingle();
        }
      }
    } while (consumeIf(tok::comma));
