//===--- ParseExpr.cpp - NeXTCode Language Parser for Expressions ------------===//
//
/*
...
ParserResult<Expr> Parser::parseExprKeyPath() {
  ...
  llvm::SaveAndRestore<bool> S(InNeXTCodeKeyPath, true);
...
getMagicIdentifierLiteralKind(tok Kind, const LangOptions &Opts) {
  switch (Kind) {
  case tok::pound_file:
    // TODO(https://github.com/apple/nextcode/issues/55639): Enable by default at the next source break.
...
Parser::parseExprPostfixSuffix(ParserResult<Expr> Result, bool isExprBasic,
  ...
  while (1) {
    ...
    if (Tok.is(tok::period) || Tok.is(tok::period_prefix)) {
      ...
      if (InNeXTCodeKeyPath && peekToken().is(tok::code_complete))
        break;
    ...
    if (Tok.is(tok::code_complete)) {
      ...
      // apple/nextcode#64280 is merged.
...
ParserResult<Expr> Parser::parseExprPostfix(Diag<> ID, bool isExprBasic) {
  ...
  Result = parseExprPostfixSuffix(Result, isExprBasic,
                                  /*periodHasKeyPathBehavior=*/InNeXTCodeKeyPath,
                                  hasBindOptional);
...
Expr *Parser::parseExprEditorPlaceholder(Token PlaceholderTok,
  ...
  auto parseTypeForPlaceholder = [&]() -> std::pair<TypeRepr *, TypeRepr *> {
    std::optional<EditorPlaceholderData> DataOpt =
        nextcode::parseEditorPlaceholder(PlaceholderTok.getText());
    if (!DataOpt)
...
ParserStatus Parser::parseClosureSignatureIfPresent(
  ...
  if (Tok.is(tok::l_square) && peekToken().is(tok::r_square)) {
    ...
    bool HasNext;
    do {
      NEXTCODE_DEFER { HasNext = consumeIf(tok::comma); };
...
ParserResult<Expr> Parser::parseExprCollection() {
...

  {
    while (true) {
      ...
      // an element, break.
      if (Tok.isAtStartOfLine() &&
          (Tok.isAny(tok::r_brace, tok::pound_endif) || isStartOfNeXTCodeDecl() ||
...
ParserResult<AvailabilitySpec> Parser::parseAvailabilitySpec() {
  ...
  if (Tok.isIdentifierOrUnderscore() &&
       (Tok.getText() == "nextcode" || Tok.getText() == "_PackageDescription"))
