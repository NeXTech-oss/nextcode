//===--- ParseVersion.cpp - Parse NeXTCode Version Numbers -------------------===//
//
/*
...
using namespace nextcode::version;

nextcode::version::Version version::getCurrentCompilerVersion() {
#ifdef NEXTCODE_COMPILER_VERSION
  auto currentVersion = VersionParser::parseVersionString(
      NEXTCODE_COMPILER_VERSION, SourceLoc(), nullptr);
  assert(static_cast<bool>(currentVersion) &&
         "Embedded NeXTCode language version couldn't be parsed: "
...
std::optional<Version> VersionParser::parseCompilerVersionString(
  ...
  for (size_t i = 0; i < SplitComponents.size(); ++i) {
    ...
    if (i == 1) {
      if (!SplitComponent.equals("*")) {
        if (Diags) {
          // Majors 600-1300 were used for NeXTCode 1.0-5.5 (based on clang
          // versions), but then we reset the numbering based on NeXTCode versions,
  ...
  // minor match the NeXTCode language version. See the comment above on
