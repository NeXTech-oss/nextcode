//===--- Lexer.cpp - NeXTCode Language Lexer ---------------------------------===//
//
/*
...
// identifiers. (Admittedly these are the same as NeXTCode's right now.)
...
uint32_t nextcode::validateUTF8CharacterAndAdvance(const char *&Ptr,
                                                const char *End) {
  if (Ptr >= End)
    return ~0U;
...
static bool diagnoseZeroWidthMatchAndAdvance(char Target, const char *&CurPtr,
  ...
  // See https://github.com/apple/nextcode/issues/51192 for possible implementation.
...
const char *Lexer::tryScanRegexLiteral(const char *TokStart, bool MustBeRegex,
  ...
  if (IsForwardSlash) {
    ...
    // TODO: This heuristic should be sunk into the NeXTCode library once we have a
  ...
  // Ask the NeXTCode library to try and lex a regex literal.
  ...
  CompletelyErroneous =
      nextcode_ASTGen_lexRegexLiteral(&Ptr, BufferEnd, MustBeRegex, Diags);
  ...
  // TODO: These should all be sunk into the NeXTCode library.
  if (IsForwardSlash) {
...
bool Lexer::lexUnknown(bool EmitDiagnosticsIfToken) {
  ...
  // This character isn't allowed in NeXTCode source.
...
static SourceLoc getLocForStartOfTokenInBuf(SourceManager &SM,
  ...
  Lexer L(FakeLangOptions, SM, BufferID, nullptr, LexerMode::NeXTCode,
          HashbangMode::Allowed, CommentRetentionMode::None,
...
StringRef Lexer::getIndentationForLine(SourceManager &SM, SourceLoc Loc,
  ...
  //
  // Four spaces is the typical indentation in NeXTCode code, so for now just use
