//===--- Parser.cpp - NeXTCode Language Parser -------------------------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements the NeXTCode parser.
//
//===----------------------------------------------------------------------===//
...
static void getStringPartTokens(const nextcode::Token &Tok,
                                const nextcode::LangOptions &LangOpts,
                                const nextcode::SourceManager &SM, int BufID,
                                std::vector<nextcode::Token> &Toks);
...
void tokenize(const LangOptions &LangOpts, const SourceManager &SM,
  ...
  Lexer L(LangOpts, SM, BufferID, Diags, LexerMode::NeXTCode,
...
void nextcode::performIDEInspectionSecondPass(
...
nextcode::Parser::BacktrackingScopeImpl::~BacktrackingScopeImpl() {
  if (Backtrack) {
...
void nextcode::Parser::CancellableBacktrackingScope::cancelBacktrack() {
  if (!Backtrack)
    return;
...
static void getStringPartTokens(const Token &Tok, const LangOptions &LangOpts,
  ...
  for (unsigned i = 0, e = Segments.size(); i != e; ++i) {
    ...
    if (Seg.Kind == Lexer::StringSegment::Literal) {
      ...
      std::vector<Token> NewTokens = nextcode::tokenize(LangOpts, SM, BufID,
                                                     Offset, EndOffset,
                                                     /*Diags=*/nullptr,
                                                     /*KeepComments=*/true);
...
std::vector<Token> nextcode::tokenize(const LangOptions &LangOpts,
...
static LexerMode sourceFileKindToLexerMode(SourceFileKind kind) {
  switch (kind) {
    case nextcode::SourceFileKind::Interface:
      return LexerMode::NeXTCodeInterface;
    case nextcode::SourceFileKind::SIL:
      return LexerMode::SIL;
    case nextcode::SourceFileKind::Library:
    case nextcode::SourceFileKind::Main:
    case nextcode::SourceFileKind::MacroExpansion:
    case nextcode::SourceFileKind::DefaultArgument:
      return LexerMode::NeXTCode;
  }
...
class TokenRecorder: public ConsumeTokenReceiver {
  ...
  void relexComment(CharSourceRange CommentRange,
    ...
    Lexer L(Ctx.LangOpts, SM, BufferID, nullptr, LexerMode::NeXTCode,
...
#include "nextcode/AST/TokenKinds.def"
    ...
    if (Tok.is(tok::kw_Self))
      break;
    if (isStartOfStmt(/*preferExpr*/ false) || isStartOfNeXTCodeDecl() ||
...
Parser::parseListItem(ParserStatus &Status, tok RightK, SourceLoc LeftLoc,
  ...
  if (Tok.isAtStartOfLine() && (Tok.is(tok::r_brace) || isStartOfNeXTCodeDecl() ||
...
ParsedDeclName nextcode::parseDeclName(StringRef name) {
...
DeclNameRef ParsedDeclName::formDeclNameRef(ASTContext &ctx,
                                            bool isSubscript,
                                            bool isCxxClassTemplateSpec) const {
  return nextcode::formDeclNameRef(ctx, BaseName, ArgumentLabels, IsFunctionName,
                                /*IsInitializer=*/true, isSubscript,
                                isCxxClassTemplateSpec);
}

DeclName nextcode::formDeclName(ASTContext &ctx,
                             StringRef baseName,
...
DeclNameRef nextcode::formDeclNameRef(ASTContext &ctx,
                                   StringRef baseName,
                                   ArrayRef<StringRef> argumentLabels,
                                   bool isFunctionName,
                                   bool isInitializer,
  ...
  if (isInitializer && baseName == "constructor")
