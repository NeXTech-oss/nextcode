//===--- ParseDecl.cpp - NeXTCode Language Parser for Declarations -----------===//
//
/*
...
namespace {
  ...
  /// uses nextcode to parse expressions, it needs to emulate the current
...
extern "C" void parseTopLevelNeXTCode(const char *buffer,
                                   void *declContext,
                                   void *astContext,
                                   void *outputContext,
                                   void (*)(void *, void *));
...
void Parser::parseTopLevelItems(SmallVectorImpl<ASTNode> &items) {
  ...
  while (!Tok.is(tok::eof)) {
    // If we run into a SIL decl, skip over until the next NeXTCode decl. We need
    // to delay parsing these, as SIL parsing currently requires type checking
    // NeXTCode decls.
    if (isStartOfSILDecl()) {
      assert(!isStartOfNeXTCodeDecl() && "Start of both a NeXTCode and SIL decl?");
      skipSILUntilNeXTCodeDecl();
      continue;
    }
  ...
  if (parsingOpts.contains(ParsingFlags::RoundTrip) &&
      nextcode_ASTGen_roundTripCheck(exportedSourceFile)) {
    SourceLoc loc;
  ...
  if (parsingOpts.contains(ParsingFlags::ValidateNewParserDiagnostics) &&
    ...
    auto hadSyntaxError = nextcode_ASTGen_emitParserDiagnostics(
    ...
    if (hadSyntaxError) {
      ...
      // emitting the same warnings from NeXTCodeParser, only emit errors from
      // NeXTCodeParser
      SourceLoc loc;
      if (auto bufferID = SF.getBufferID()) {
          loc = Context.SourceMgr.getLocForBufferStart(*bufferID);
...
void *ExportedSourceFileRequest::evaluate(Evaluator &evaluator,
...
#if NEXTCODE_BUILD_NEXTCODE_SYNTAX
  // The NeXTCodeSyntax parser doesn't (yet?) handle SIL.
  ...
  auto exportedSourceFile = nextcode_ASTGen_parseSourceFile(
  ...
  ctx.addCleanup([exportedSourceFile] {
    nextcode_ASTGen_destroySourceFile(exportedSourceFile);
  });
...
void Parser::parseSourceFileViaASTGen(
  ...
  if (langOpts.hasFeature(Feature::ParserASTGen)) {
    ...
    nextcode_ASTGen_buildTopLevelASTNodes(&Diags, exportedSourceFile,
...
bool Parser::parseTopLevelSIL() {
  ...
  auto hadError = false;
  while (!Tok.is(tok::eof)) {
    // If we run into a NeXTCode decl, skip over until we find the next SIL decl.
    if (isStartOfNeXTCodeDecl()) {
      assert(!isStartOfSILDecl() && "Start of both a NeXTCode and SIL decl?");
      skipToNextSILDecl();
      continue;
    }
...
#undef CASE_SIL
    default:
      // If we reached here, we have something malformed that isn't a NeXTCode decl
...
ParserResult<AvailableAttr> Parser::parseExtendedAvailabilitySpecList(
  ...
  // Treat 'nextcode' as a valid version-qualifying token, when
  ...
  if (!PlatformKind.has_value() &&
      (Platform == "nextcode" || Platform == "_PackageDescription")) {
    ...
    PlatformAgnostic = (Platform == "nextcode") ?
...
bool Parser::parseSpecializeAttributeArguments(
    nextcode::tok ClosingBrace, bool &DiscardAttribute,
    std::optional<bool> &Exported,
    std::optional<SpecializeAttr::SpecializationKind> &Kind,
    nextcode::TrailingWhereClause *&TrailingWhereClause,
...
bool Parser::parseAvailability(
  ...
  if (Platform != "*" &&
    ...
    //   @available(nextcode 3.1)
    // we will synthesize
    //   @available(nextcode, introduced: 3.1)
    // or, if we have
...
bool Parser::parseSpecializeAttribute(
    nextcode::tok ClosingBrace, SourceLoc AtLoc, SourceLoc Loc,
...
void Parser::parseAllAvailabilityMacroArguments() {
  ...
  for (StringRef macro: LangOpts.AvailabilityMacros) {
    ...
    nextcode::ParserUnit PU(SM, SourceFileKind::Main, bufferID, LangOpts,
...
#include "nextcode/AST/DeclAttr.def"
  ...
  case DeclAttrKind::NeXTCodeNativeObjCRuntimeBase: {
    ...
    Attributes.add(new (Context) NeXTCodeNativeObjCRuntimeBaseAttr(*name,
...
bool Parser::parseVersionTuple(llvm::VersionTuple &Version,
  ...
  if (Tok.is(tok::integer_literal)) {
    ...
    if (Version.empty()) {
      ...
      diagnose(Range.Start, D).warnUntilNeXTCodeVersion(6);
      return true;
    }
    return false;
  }
...
ParserStatus Parser::parseDeclAttribute(DeclAttributes &Attributes,
  ...
  // In NeXTCode 5 and above, these become hard errors. In NeXTCode 4.2, emit a
  ...
  if (Context.isNeXTCodeVersionAtLeast(5)) {
    ...
    checkInvalidAttrName("_inlineable", "inlinable", DeclAttrKind::Inlinable,
                         diag::attr_renamed);
  } else if (Context.isNeXTCodeVersionAtLeast(4, 2)) {
    checkInvalidAttrName("_versioned", "usableFromInline",
  ...
  // embedded NeXTCode mode, or into nothing when in regular mode.
  if (!DK && Tok.getText() == "_unavailableInEmbedded") {
    SourceLoc attrLoc = consumeToken();
    if (Context.LangOpts.hasFeature(Feature::Embedded)) {
      StringRef Message = "unavailable in embedded NeXTCode", Renamed;
...
#undef CONTEXTUAL_CASE
      ...
      if (Kind == DeclAttrKind::Actor) {
        // If the next token is a startOfNeXTCodeDecl, we are part of the modifier
...

        {
          ...
          isActorModifier = isStartOfNeXTCodeDecl(
    ...
    case tok::kw_class: {
      ...
      // then treat it as a modifier.
      {
        ...
        if (isStartOfNeXTCodeDecl() || (isa<ClassDecl>(CurDeclContext) &&
...
static unsigned skipUntilMatchingRBrace(Parser &P,
  ...
  while ((OpenBraces != 0 || OpenPoundIf != 0) && P.Tok.isNot(tok::eof)) {
    ...
    // - We might encounter an invalid NeXTCode token that might be valid in a
    // regex.
...
bool nextcode::isKeywordPossibleDeclStart(const LangOptions &options,
                                       const Token &Tok) {
  switch (Tok.getKind()) {
  case tok::kw_inout:
...
bool Parser::isStartOfNeXTCodeDecl(bool allowPoundIfAttributes,
  ...
  // happen at the top level of a scope, but we do use isStartOfNeXTCodeDecl()
  ...
  if (Tok.is(tok::at_sign)) {
    ...
    return isStartOfNeXTCodeDecl(allowPoundIfAttributes,
  ...
  if (Tok.is(tok::pound_if) && allowPoundIfAttributes) {
    ...
    if (!sawAnyAttributes)
      return false;
    return isStartOfNeXTCodeDecl(/*allowPoundIfAttributes=*/true,
                              /*hadAttrsOrModifiers=*/true);
  }

  // If we have a decl modifying keyword, check if the next token is a valid
  // decl start. This is necessary to correctly handle NeXTCode keywords that are
  // shared by SIL, e.g 'private' in 'sil private @foo :'. We need to make sure
  // this isn't considered a valid NeXTCode decl start.
  if (Tok.isKeyword()) {
    ...
    if (DAK && DeclAttribute::isDeclModifier(*DAK)) {
      ...
      if (consumeIf(tok::l_paren)) {
        while (Tok.isNot(tok::r_brace, tok::eof, tok::pound_endif)) {
          ...
          if (isStartOfNeXTCodeDecl(/*allowPoundIfAttributes=*/false,
                                 /*hadAttrsOrModifiers=*/true))
            return true;
      ...
      return isStartOfNeXTCodeDecl(/*allowPoundIfAttributes=*/false,
                                /*hadAttrsOrModifiers=*/true);
    }
  }
  ...
  if (Tok.isContextualKeyword("actor")) {
    ...
    do {
      consumeToken();
    } while (isStartOfNeXTCodeDecl(/*allowPoundIfAttributes=*/false,
                                /*hadAttrsOrModifiers=*/true));
  ...
  default:
    ...
    if (Flags.contains(PD_HasContainerType) &&
      ...
      if (IsProbablyFuncDecl) {
        ...
        diagnose(Tok.getLoc(), diag::expected_keyword_in_decl, "function",
                 DescriptiveKind)
...
parseDeclTypeAlias(Parser::ParseDeclOptions Flags, DeclAttributes &Attributes) {
  ...
  if (Flags.contains(PD_InProtocol) && !genericParams && !Tok.is(tok::equal)) {
    // If we're in a protocol and don't see an '=' this looks like leftover NeXTCode 2
...
ParserResult<TypeDecl> Parser::parseDeclAssociatedType(Parser::ParseDeclOptions Flags,
  ...
  // ask us to fix up leftover NeXTCode 2 code intending to be an associatedtype.
...
void Parser::skipSILUntilNeXTCodeDecl() {
  // Tell the lexer we're about to start lexing SIL.
  Lexer::SILBodyRAII sbr(*L);

  while (!Tok.is(tok::eof) &&
         !isStartOfNeXTCodeDecl(/*allowPoundIfAttributes=*/false)) {
...
Parser::parseDeclVar(ParseDeclOptions Flags,
  ...
  do {
    ...
    if (Tok.is(tok::l_brace)) {
      ...
      // See https://github.com/apple/nextcode/issues/57183.
...
Parser::parseDeclOperatorImpl(SourceLoc OperatorLoc, Identifier Name,
  ...
  if (Tok.is(tok::colon)) {
    ...
    if (Context.TypeCheckerOpts.EnableOperatorDesignatedTypes) {
      ...
      // compatibility with old nextcodeinterfaces) and emit a warning.
