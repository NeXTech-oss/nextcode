...
#include "nextcode/ConstExtract/ConstExtractRequests.h"
...
std::string toFullyQualifiedTypeNameString(const nextcode::Type &Type) {
  std::string TypeNameOutput;
  llvm::raw_string_ostream OutputStream(TypeNameOutput);
  nextcode::PrintOptions Options;
...
std::string toFullyQualifiedProtocolNameString(const nextcode::ProtocolDecl &Protocol) {
...
std::string toMangledTypeNameString(const nextcode::Type &Type) {
...
void writeResultBuilderInformation(llvm::json::OStream &JSON,
                                   const nextcode::NominalTypeDecl *TypeDecl,
                                   const nextcode::VarDecl *VarDecl) {
  ...
  for (ProtocolDecl *Decl :
    ...
    for (auto Member : Decl->getMembers()) {
      if (auto *VD = dyn_cast<nextcode::VarDecl>(Member)) {
...
void nextcode::registerConstExtractRequestFunctions(Evaluator &evaluator) {
