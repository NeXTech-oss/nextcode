...
//
// Copyright (c) 2014 - 2016 Apple Inc. and the NeXTCode project authors
// Licensed under Apache License v2.0 with Runtime Library Exception
//
// See http://nextcode.org/LICENSE.txt for license information
// See http://nextcode.org/CONTRIBUTORS.txt for the list of NeXTCode project authors
//
//===----------------------------------------------------------------------===//
...
#include "nextcode/IDE/ModuleInterfacePrinting.h"
...
static bool
emitDataForNeXTCodeSerializedModule(ModuleDecl *module,
...
static void appendSymbolicInterfaceClangModuleFilename(
  ...
  StringRef extension = ".symbolicnextcodeinterface";
...
/// Emit the symbolic nextcode interface file for an imported Clang module into the
/// index store directory.
///
/// The nextcode interface file is emitted only when it doesn't exist yet, or when
...
static void emitSymbolicInterfaceForClangModule(
  ...
  ide::printSymbolicNeXTCodeClangModuleInterface(M, printer, clangModule);
  os.close();
...
static void emitTransitiveClangSymbolicInterfacesForNeXTCodeModuleImports(
    ...
    for (auto *FU : mod->getFiles()) {
      ...
      case FileUnitKind::ClangModule: {
        ...
        if (auto F = fileMgr.getFile(LFU->getFilename())) {
          ...
          // FIXME: We should keep recursing here into other NeXTCode modules.
        }
      }
      }
    }
      ...
      case FileUnitKind::ClangModule: {
        ...
        if (FU->getKind() == FileUnitKind::ClangModule) {
          ...
          if (auto clangMod = clangModUnit->getUnderlyingClangModule()) {
            ...
            // FIXME: clang's -Rremarks do not seem to go through NeXTCode's
            // diagnostic emitter.
          ...
          // We don't officially support binary nextcode modules, so normally
          ...
          if (mod->isNonUserModule() && indexSystemModules &&
            ...
            emitDataForNeXTCodeSerializedModule(mod, indexStorePath,
                                             indexClangModules,
...
emitDataForNeXTCodeSerializedModule(ModuleDecl *module,
  ...
  // Reload resilient modules from nextcodeinterface to avoid indexing
  // internal details.
  ...
  if (!explicitModuleBuild &&
    ...
    if (reloadedModule) {
      ...
      // If we can't rebuild from the nextcodeinterface, don't index this module.
  ...
  if (skipIndexingModule) {
    ...
    // as indexed to avoid rebuilds of broken nextcodeinterfaces.
  ...
  IndexUnitWriter unitWriter(
      fileMgr, indexStorePath, "nextcode", nextcodeVersion, filename, moduleName,
...
recordSourceFileUnit(SourceFile *primarySourceFile, StringRef indexUnitToken,
  ...
  StringRef sysrootPath = clangCI.getHeaderSearchOpts().Sysroot;
  IndexUnitWriter unitWriter(
      fileMgr, indexStorePath, "nextcode", nextcodeVersion, indexUnitToken,
...
bool index::indexAndRecord(SourceFile *primarySourceFile,
  ...
  // 2. Having all the other nextcode files of the module as file dependencies ends
  //   up making all of them associated with all the other files as main files.
  //   It's better to associate each nextcode file with the unit that recorded it
  //   as the main one.
...
bool index::indexAndRecord(ModuleDecl *module,
  ...
  // In the case where inputs are nextcode modules, like in the merge-module step,
  // ignore the inputs; associated unit files for the modules' source inputs
  // should have been generated at nextcode module creation time.
