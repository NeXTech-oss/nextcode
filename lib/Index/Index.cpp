...
#include "nextcode/Index/Index.h"
...
static bool
printArtificialName(const nextcode::AbstractStorageDecl *ASD, AccessorKind AK, llvm::raw_ostream &OS) {
  switch (AK) {
...
bool index::printDisplayName(const nextcode::ValueDecl *D, llvm::raw_ostream &OS) {
...
static bool isMemberwiseInit(nextcode::ValueDecl *D) {
...
class IndexNeXTCodeASTWalker : public SourceEntityWalker {
...
public:
  IndexNeXTCodeASTWalker(IndexDataConsumer &IdxConsumer, ASTContext &Ctx,
  ...
  ~IndexNeXTCodeASTWalker() override {
...
void IndexNeXTCodeASTWalker::visitDeclContext(DeclContext *Context) {
...
void IndexNeXTCodeASTWalker::visitModule(ModuleDecl &Mod) {
  CurrentModule = &Mod;
...
bool IndexNeXTCodeASTWalker::handleSourceOrModuleFile(SourceFileOrModule SFOrMod) {
...
bool IndexNeXTCodeASTWalker::visitImports(
    SourceFileOrModule TopMod, llvm::SmallPtrSetImpl<ModuleDecl *> &Visited) {
  // Dependencies of the stdlib module (like NeXTCodeShims module) are
...
bool IndexNeXTCodeASTWalker::handleWitnesses(Decl *D, SmallVectorImpl<IndexedWitness> &explicitWitnesses) {
...
bool IndexNeXTCodeASTWalker::startEntity(Decl *D, IndexSymbol &Info, bool IsRef) {
  switch (IdxConsumer.startSourceEntity(Info)) {
    case nextcode::index::IndexDataConsumer::Abort:
      Cancelled = true;
      LLVM_FALLTHROUGH;
    case nextcode::index::IndexDataConsumer::Skip:
      return false;
    case nextcode::index::IndexDataConsumer::Continue: {
      SmallVector<IndexedWitness, 6> explicitWitnesses;
      if (!IsRef) {
...
bool IndexNeXTCodeASTWalker::startEntityDecl(ValueDecl *D) {
  ...
  if (auto Parent = getParentDecl()) {
    ...
    if (auto ParentVD = dyn_cast<ValueDecl>(Parent)) {
      ...
      if (Info.symInfo.SubKind == SymbolSubKind::AccessorGetter ||
          ...
          (Info.symInfo.SubKind >= SymbolSubKind::NeXTCodeAccessorWillSet &&
           Info.symInfo.SubKind <= SymbolSubKind::NeXTCodeAccessorMutableAddressor))
...
bool IndexNeXTCodeASTWalker::reportRelatedRef(ValueDecl *D, SourceLoc Loc, bool isImplicit,
...
bool IndexNeXTCodeASTWalker::reportInheritedTypeRefs(InheritedTypes Inherited,
                                                  Decl *Inheritee) {
...
bool IndexNeXTCodeASTWalker::reportRelatedTypeRef(const TypeLoc &Ty, SymbolRoleSet Relations,
...
bool IndexNeXTCodeASTWalker::reportPseudoAccessor(AbstractStorageDecl *D,
...
bool IndexNeXTCodeASTWalker::reportExtension(ExtensionDecl *D) {
...
bool IndexNeXTCodeASTWalker::report(ValueDecl *D) {
  ...
  if (startEntityDecl(D)) {
    ...
    if (auto StoreD = dyn_cast<AbstractStorageDecl>(D)) {
      ...
      if (isa<VarDecl>(D) && !isa<ParamDecl>(D) &&
        ...
        // Note that an ObjC class subclassing from a NeXTCode class, may still
...
bool IndexNeXTCodeASTWalker::reportRef(ValueDecl *D, SourceLoc Loc,
                                    IndexSymbol &Info,
                                    std::optional<AccessKind> AccKind) {
  if (!shouldIndex(D, /*IsRef=*/true))
...
bool IndexNeXTCodeASTWalker::reportIsDynamicRef(ValueDecl *D, IndexSymbol &Info) {
...
bool IndexNeXTCodeASTWalker::reportImplicitConformance(ValueDecl *witness, ValueDecl *requirement,
                                                    Decl *container) {
...
bool IndexNeXTCodeASTWalker::initIndexSymbol(ValueDecl *D, SourceLoc Loc,
...
bool IndexNeXTCodeASTWalker::initIndexSymbol(ExtensionDecl *ExtD, ValueDecl *ExtendedD,
...
bool IndexNeXTCodeASTWalker::initFuncDeclIndexSymbol(FuncDecl *D,
                                                  IndexSymbol &Info) {
...
bool IndexNeXTCodeASTWalker::initFuncRefIndexSymbol(ValueDecl *D, SourceLoc Loc,
                                                 IndexSymbol &Info) {
...
bool IndexNeXTCodeASTWalker::initVarRefIndexSymbols(
  ...
  switch (Kind) {
  case nextcode::AccessKind::Read:
    Info.roles |= (unsigned)SymbolRole::Read;
    break;
  case nextcode::AccessKind::ReadWrite:
    Info.roles |= (unsigned)SymbolRole::Read;
    LLVM_FALLTHROUGH;
  case nextcode::AccessKind::Write:
...
bool IndexNeXTCodeASTWalker::indexComment(const Decl *D) {
...
void IndexNeXTCodeASTWalker::getRecursiveModuleImports(
  ...
  warn([&Imports](llvm::raw_ostream &OS) {
    std::for_each(Imports.begin(), Imports.end(), [&OS](ModuleDecl *M) {
      if (M->getModuleFilename().empty()) {
        std::string Info = "nextcode::ModuleDecl with empty file name!! \nDetails: \n";
        Info += "  name: ";
        Info += M->getName().get();
        Info += "\n";
        ...
        OS << "nextcode::ModuleDecl with empty file name! " << Info << "\n";
      }
    });
  });
...
void IndexNeXTCodeASTWalker::collectRecursiveModuleImports(
  ...
  // imports.
  // FIXME: What happens if the clang module imports a nextcode module ? So far
  // the assumption is that the path to the nextcode module will be fixed, so no
...
void index::indexDeclContext(DeclContext *DC, IndexDataConsumer &consumer) {
  ...
  IndexNeXTCodeASTWalker walker(consumer, DC->getASTContext(), SF);
...
void index::indexSourceFile(SourceFile *SF, IndexDataConsumer &consumer) {
  assert(SF);
  IndexNeXTCodeASTWalker walker(consumer, SF->getASTContext(), SF);
...
void index::indexModule(ModuleDecl *module, IndexDataConsumer &consumer) {
  assert(module);
  IndexNeXTCodeASTWalker walker(consumer, module->getASTContext());
