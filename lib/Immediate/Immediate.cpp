//===--- Immediate.cpp - the nextcode immediate mode -------------------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
// This is the implementation of the nextcode interpreter, which takes a
...
using namespace nextcode::immediate;
...
void *nextcode::immediate::loadNeXTCodeRuntime(ArrayRef<std::string>
                                         runtimeLibPaths) {
#if defined(_WIN32)
  return loadRuntimeLib("nextcodeCore" LTDL_SHLIB_EXT, runtimeLibPaths);
#elif (defined(__linux__) || defined(_WIN64) || defined(__FreeBSD__))
  return loadRuntimeLib("libnextcodeCore" LTDL_SHLIB_EXT, runtimeLibPaths);
#else
  return loadRuntimeLib("libnextcodeCore" LTDL_SHLIB_EXT, {"/usr/lib/nextcode"});
#endif
}
...
bool nextcode::immediate::tryLoadLibraries(ArrayRef<LinkLibrary> LinkLibraries,
                                        SearchPathOptions SearchPathOpts,
...
static void addMergedLibraries(SmallVectorImpl<LinkLibrary> &AllLinkLibraries,
  ...
  static const llvm::StringMap<MergedLibrary> MergedLibs = {
    ...
    {"AppKit", {"libnextcodeAppKit.dylib", VersionTuple{14}}},
    {"HealthKit", {"libnextcodeHealthKit.dylib", VersionTuple{14}}},
    {"Network", {"libnextcodeNetwork.dylib", VersionTuple{14}}},
    {"Photos", {"libnextcodePhotos.dylib", VersionTuple{14}}},
    {"PhotosUI", {"libnextcodePhotosUI.dylib", VersionTuple{14}}},
    {"SoundAnalysis", {"libnextcodeSoundAnalysis.dylib", VersionTuple{14}}},
    {"Virtualization", {"libnextcodeVirtualization.dylib", VersionTuple{14}}},
    // Merged in macOS 13.0
    {"Foundation", {"libnextcodeFoundation.dylib", VersionTuple{13}}},
  };
...
bool nextcode::immediate::autolinkImportedModules(ModuleDecl *M,
...
int nextcode::RunImmediately(CompilerInstance &CI, const ProcessCmdLine &CmdLine,
                          const IRGenOptions &IRGenOpts,
  ...
  // Load libNeXTCodeCore to setup process arguments.
  //
  // This must be done here, before any library loading has been done, to avoid
  // racing with the static initializers in user code.
  // Setup interpreted process arguments.
  using ArgOverride = void (*NEXTCODE_CC(nextcode))(const char **, int);
#if defined(_WIN32)
  auto stdlib = loadNeXTCodeRuntime(Context.SearchPathOpts.RuntimeLibraryPaths);
  if (!stdlib) {
  ...
  auto emplaceProcessArgs = reinterpret_cast<ArgOverride>(
      GetProcAddress(module, "_nextcode_stdlib_overrideUnsafeArgvArgc"));
  if (emplaceProcessArgs == nullptr)
    return -1;
#else
  // In case the compiler is built with nextcode modules, it already has the stdlib
  // linked to. First try to lookup the symbol with the standard library
  // resolving.
  auto emplaceProcessArgs =
      (ArgOverride)dlsym(RTLD_DEFAULT, "_nextcode_stdlib_overrideUnsafeArgvArgc");

  if (dlerror()) {
    // If this does not work (= the NeXTCode modules are not linked to the tool),
    ...
    dlerror();
    emplaceProcessArgs =
        (ArgOverride)dlsym(stdlib, "_nextcode_stdlib_overrideUnsafeArgvArgc");
    if (dlerror())
      return -1;
  }
#endif
  ...
  if (autolinkImportedModules(nextcodeModule, IRGenOpts))
    return -1;

  auto JIT = NeXTCodeJIT::Create(CI);
  if (auto Err = JIT.takeError()) {
    logError(std::move(Err));
    return -1;
  }

  auto MU = std::make_unique<EagerNeXTCodeMaterializationUnit>(
      **JIT, CI, IRGenOpts, std::move(SM));
  if (auto Err = (*JIT)->addNeXTCode((*JIT)->getMainJITDylib(), std::move(MU))) {
    logError(std::move(Err));
    return -1;
  }
...
int nextcode::RunImmediatelyFromAST(CompilerInstance &CI) {
  CI.performSema();
  auto &Context = CI.getASTContext();
  if (Context.hadError()) {
    return -1;
  ...
  auto &Target = nextcodeModule->getASTContext().LangOpts.Target;
  assert(Target.isMacOSX());
  auto JIT = NeXTCodeJIT::Create(CI);
  if (auto Err = JIT.takeError()) {
    logError(std::move(Err));
  ...
  (*JIT)->addRenamer();
  auto MU = LazyNeXTCodeMaterializationUnit::Create(**JIT, CI);
  if (auto Err = (*JIT)->addNeXTCode((*JIT)->getMainJITDylib(), std::move(MU))) {
    logError(std::move(Err));
    return -1;
  }
