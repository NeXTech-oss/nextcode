//===--- NeXTCodeMaterializationUnit.cpp - JIT NeXTCode ASTs ----------*- C++ -*-===//
//
/*
...
llvm::Expected<std::unique_ptr<NeXTCodeJIT>>
NeXTCodeJIT::Create(CompilerInstance &CI) {
  ...
  if (auto Err = setUpInProcessLCTMReentryViaEPCIU(**EPCIU))
    return std::move(Err);
  return std::unique_ptr<NeXTCodeJIT>(
      new NeXTCodeJIT(std::move(*J), std::move(*EPCIU)));
}

NeXTCodeJIT::~NeXTCodeJIT() {
  if (auto Err = EPCIU->cleanup())
    J->getExecutionSession().reportError(std::move(Err));
}

llvm::Expected<int> NeXTCodeJIT::runMain(llvm::ArrayRef<std::string> Args) {
...
llvm::orc::JITDylib &NeXTCodeJIT::getMainJITDylib() {
  return J->getMainJITDylib();
}

std::string NeXTCodeJIT::mangle(StringRef Name) { return J->mangle(Name); }

llvm::orc::SymbolStringPtr NeXTCodeJIT::mangleAndIntern(StringRef Name) {
  return J->mangleAndIntern(Name);
}

llvm::orc::SymbolStringPtr NeXTCodeJIT::intern(StringRef Name) {
  return J->getExecutionSession().intern(Name);
}

llvm::orc::IRCompileLayer &NeXTCodeJIT::getIRCompileLayer() {
  return J->getIRCompileLayer();
}

llvm::orc::ObjectTransformLayer &NeXTCodeJIT::getObjTransformLayer() {
  return J->getObjTransformLayer();
}

llvm::Expected<std::unique_ptr<llvm::orc::LLJIT>>
NeXTCodeJIT::CreateLLJIT(CompilerInstance &CI) {
  ...
  for (auto &Sec : G.sections()) {
    ...
    for (auto *Sym : Sec.symbols()) {
      ...
      if (ToRename.count(Sym->getName())) {
        // FIXME: Get rid of the temporary when NeXTCode's llvm-project is
        // updated to LLVM 17.
...
void NeXTCodeJIT::Plugin::modifyPassConfig(
...
llvm::Error
NeXTCodeJIT::Plugin::notifyFailed(llvm::orc::MaterializationResponsibility &MR) {
  return llvm::Error::success();
}

llvm::Error
NeXTCodeJIT::Plugin::notifyRemovingResources(llvm::orc::JITDylib &JD,
                                          llvm::orc::ResourceKey K) {
  return llvm::Error::success();
}

void NeXTCodeJIT::Plugin::notifyTransferringResources(
    llvm::orc::JITDylib &JD, llvm::orc::ResourceKey DstKey,
    llvm::orc::ResourceKey SrcKey) {}

void NeXTCodeJIT::handleLazyCompilationFailure() {
  llvm::errs() << "Lazy compilation error\n";
  exit(1);
}

NeXTCodeJIT::NeXTCodeJIT(std::unique_ptr<llvm::orc::LLJIT> J,
...
generateModule(const CompilerInstance &CI, std::unique_ptr<SILModule> SM) {
  ...
  auto *nextcodeModule = CI.getMainModule();
  ...
  auto GenModule = performIRGeneration(
      nextcodeModule, IRGenOpts, TBDOpts, std::move(SM),
      nextcodeModule->getName().str(), PSPs, ArrayRef<std::string>());
...
std::unique_ptr<LazyNeXTCodeMaterializationUnit>
LazyNeXTCodeMaterializationUnit::Create(NeXTCodeJIT &JIT, CompilerInstance &CI) {
  ...
  return std::unique_ptr<LazyNeXTCodeMaterializationUnit>(
      new LazyNeXTCodeMaterializationUnit(JIT, CI, std::move(Sources),
                                       std::move(PublicInterface)));
}

StringRef LazyNeXTCodeMaterializationUnit::getName() const {
  return "NeXTCodeMaterializationUnit";
}

LazyNeXTCodeMaterializationUnit::LazyNeXTCodeMaterializationUnit(
    NeXTCodeJIT &JIT, CompilerInstance &CI, const SymbolSourceMap *Sources,
    llvm::orc::SymbolFlagsMap Symbols)
    : MaterializationUnit({std::move(Symbols), nullptr}), Sources(Sources),
      JIT(JIT), CI(CI) {}

void LazyNeXTCodeMaterializationUnit::materialize(
  ...
  std::unique_ptr<MaterializationUnit> UnrequestedMU(
      new LazyNeXTCodeMaterializationUnit(JIT, CI, Sources,
...
llvm::Error
NeXTCodeJIT::addNeXTCode(llvm::orc::JITDylib &JD,
...
void LazyNeXTCodeMaterializationUnit::discard(
    const llvm::orc::JITDylib &JD, const llvm::orc::SymbolStringPtr &Sym) {}

EagerNeXTCodeMaterializationUnit::EagerNeXTCodeMaterializationUnit(
    NeXTCodeJIT &JIT, const CompilerInstance &CI, const IRGenOptions &IRGenOpts,
    std::unique_ptr<SILModule> SM)
    : MaterializationUnit(getInterface(JIT, CI)), JIT(JIT), CI(CI),
      IRGenOpts(IRGenOpts), SM(std::move(SM)) {}

StringRef EagerNeXTCodeMaterializationUnit::getName() const {
  return "EagerNeXTCodeMaterializationUnit";
}

void EagerNeXTCodeMaterializationUnit::materialize(
...
EagerNeXTCodeMaterializationUnit::getInterface(NeXTCodeJIT &JIT,
...
void EagerNeXTCodeMaterializationUnit::discard(
    const llvm::orc::JITDylib &JD, const llvm::orc::SymbolStringPtr &Sym) {}
...
void EagerNeXTCodeMaterializationUnit::dumpJIT(const llvm::Module &M) {
