//===------------ DependencyScanningTool.cpp - NeXTCode Compiler -------------===//
//
/*
...
llvm::ErrorOr<nextcodescan_string_ref_t> getTargetInfo(ArrayRef<const char *> Command,
  ...
  nextcode::targetinfo::printTargetInfo(Invocation, StrOS);
...
DependencyScanningTool::DependencyScanningTool()
    : ScanningService(std::make_unique<NeXTCodeDependencyScanningService>()),
...
DependencyScanningTool::getDependencies(
  ...
  // The primary instance used to scan the query NeXTCode source-code
...
DependencyScanningTool::getDependencies(
  ...
  // The primary instance used to scan NeXTCode modules
  auto QueryContextOrErr = initScannerForAction(Command, WorkingDirectory);
  if (std::error_code EC = QueryContextOrErr.getError())
    return std::vector<llvm::ErrorOr<nextcodescan_dependency_graph_t>>(
...
void DependencyScanningTool::resetCache() {
  llvm::sys::SmartScopedLock<true> Lock(DependencyScanningToolStateLock);
  ScanningService.reset(new NeXTCodeDependencyScanningService());
}
...
DependencyScanningTool::initCompilerInstanceForScan(
  ...
  // to using per-scan diagnostic output embedded in the `nextcodescan_dependency_graph_s`
