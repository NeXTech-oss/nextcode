...
class ModuleDependenciesCacheDeserializer {
  ...
  bool readMetadata();
  bool readGraph(NeXTCodeDependencyScanningService &cache);
...
public:
  ...
  bool readInterModuleDependenciesCache(NeXTCodeDependencyScanningService &cache);
};
...
/// cache.
bool ModuleDependenciesCacheDeserializer::readGraph(NeXTCodeDependencyScanningService &cache) {
  using namespace graph_block;
  ...
  while (!Cursor.AtEndOfStream()) {
    ...
    case NEXTCODE_INTERFACE_MODULE_DETAILS_NODE: {
      if (!hasCurrentModule)
        llvm::report_fatal_error(
            "Unexpected NEXTCODE_TEXTUAL_MODULE_DETAILS_NODE record");
      ...
      NeXTCodeInterfaceModuleDetailsLayout::readRecord(
      ...
      if (!outputModulePath)
         llvm::report_fatal_error("Bad .codemodule output path");
      std::optional<std::string> optionalNeXTCodeInterfaceFile;
      if (interfaceFileID != 0) {
        auto nextcodeInterfaceFile = getIdentifier(interfaceFileID);
        if (!nextcodeInterfaceFile)
          llvm::report_fatal_error("Bad nextcode interface file path");
        optionalNeXTCodeInterfaceFile = *nextcodeInterfaceFile;
      }
      ...
      // forNeXTCodeInterface API demands references here.
      ...
      auto moduleDep = ModuleDependencyInfo::forNeXTCodeInterfaceModule(
          outputModulePath.value(), optionalNeXTCodeInterfaceFile.value(),
      ...
      // Add NeXTCode overlay dependencies
    ...
    case NEXTCODE_SOURCE_MODULE_DETAILS_NODE: {
      if (!hasCurrentModule)
        llvm::report_fatal_error(
            "Unexpected NEXTCODE_SOURCE_MODULE_DETAILS_NODE record");
      // Expected context hash ID is 0
      if (currentContextHashID)
        llvm::report_fatal_error(
            "Unexpected context hash on MODULE_NODE corresponding to a "
            "NEXTCODE_SOURCE_MODULE_DETAILS_NODE record");
      ...
      NeXTCodeSourceModuleDetailsLayout::readRecord(
      ...
      // Form the dependencies storage object
      auto moduleDep = ModuleDependencyInfo::forNeXTCodeSourceModule(
    ...
    case NEXTCODE_BINARY_MODULE_DETAILS_NODE: {
      if (!hasCurrentModule)
        llvm::report_fatal_error(
            "Unexpected NEXTCODE_BINARY_MODULE_DETAILS_NODE record");
      cache.configureForContextHash(getContextHash());
      unsigned compiledModulePathID, moduleDocPathID, moduleSourceInfoPathID,
               ...
               moduleCacheKeyID;
      NeXTCodeBinaryModuleDetailsLayout::readRecord(
      ...
      // TODO: LinkLibraries
      // Form the dependencies storage object
      auto moduleDep = ModuleDependencyInfo::forNeXTCodeBinaryModule(
    ...
    case NEXTCODE_PLACEHOLDER_MODULE_DETAILS_NODE: {
      if (!hasCurrentModule)
        llvm::report_fatal_error(
            "Unexpected NEXTCODE_PLACEHOLDER_MODULE_DETAILS_NODE record");
      cache.configureForContextHash(getContextHash());
      unsigned compiledModulePathID, moduleDocPathID, moduleSourceInfoPathID;
      NeXTCodePlaceholderModuleDetailsLayout::readRecord(
          Scratch, compiledModulePathID, moduleDocPathID,
      ...
      // Form the dependencies storage object
      auto moduleDep = ModuleDependencyInfo::forPlaceholderNeXTCodeModuleStub(
          *compiledModulePath, *moduleDocPath, *moduleSourceInfoPath);
  ...
  if (encodedIdentifierStringArray.has_value()) {
    ...
    for (const auto &encodedIdentifierString : *encodedIdentifierStringArray) {
      ...
      if (!encodedIdentifierString.compare(0, textualPrefix.size(), textualPrefix)) {
        ...
        id = {moduleName, ModuleDependencyKind::NeXTCodeInterface};
        ...
        id = {moduleName, ModuleDependencyKind::NeXTCodeBinary};
        ...
        id = {moduleName, ModuleDependencyKind::NeXTCodePlaceholder};
      } else {
...
bool nextcode::dependencies::module_dependency_cache_serialization::
...
class ModuleDependenciesCacheSerializer {
  ...
  void collectStringsAndArrays(const NeXTCodeDependencyScanningService &cache);
...
public:
  void
  writeInterModuleDependenciesCache(const NeXTCodeDependencyScanningService &cache);
};
...
#define BLOCK_RECORD(K, X) emitRecordID(K::X, #X, nameBuffer)
  ...
  BLOCK_RECORD(graph_block, NEXTCODE_INTERFACE_MODULE_DETAILS_NODE);
  BLOCK_RECORD(graph_block, NEXTCODE_SOURCE_MODULE_DETAILS_NODE);
  BLOCK_RECORD(graph_block, NEXTCODE_BINARY_MODULE_DETAILS_NODE);
  BLOCK_RECORD(graph_block, NEXTCODE_PLACEHOLDER_MODULE_DETAILS_NODE);
...
void ModuleDependenciesCacheSerializer::writeMetadata() {
  ...
  MetadataLayout::emitRecord(Out, ScratchRecord,
                             ...
                             version::getNeXTCodeFullVersion());
}
...
void ModuleDependenciesCacheSerializer::writeModuleInfo(
  ...
  case nextcode::ModuleDependencyKind::NeXTCodeInterface: {
    ...
    auto nextcodeTextDeps = dependencyInfo.getAsNeXTCodeInterfaceModule();
    assert(nextcodeTextDeps);
    unsigned outputModulePathFileId =
        getIdentifier(nextcodeTextDeps->moduleOutputPath);
    unsigned nextcodeInterfaceFileId =
        getIdentifier(nextcodeTextDeps->nextcodeInterfaceFile);
    unsigned bridgingHeaderFileId =
        nextcodeTextDeps->textualModuleDetails.bridgingHeaderFile
            ? getIdentifier(nextcodeTextDeps->textualModuleDetails
                                .bridgingHeaderFile.value())
            : 0;
    NeXTCodeInterfaceModuleDetailsLayout::emitRecord(
        Out, ScratchRecord, AbbrCodes[NeXTCodeInterfaceModuleDetailsLayout::Code],
        outputModulePathFileId, nextcodeInterfaceFileId,
        getArrayID(moduleID,
        ...
        getIdentifier(nextcodeTextDeps->contextHash), 
        nextcodeTextDeps->isFramework, nextcodeTextDeps->isStatic,
        ...
        getArrayID(moduleID,
                   ModuleIdentifierArrayKind::NeXTCodeOverlayDependencyIDs),
        getIdentifier(nextcodeTextDeps->textualModuleDetails.CASFileSystemRootID),
        getIdentifier(nextcodeTextDeps->textualModuleDetails
                          .CASBridgingHeaderIncludeTreeRootID),
        getIdentifier(nextcodeTextDeps->moduleCacheKey));
    break;
  }
  case nextcode::ModuleDependencyKind::NeXTCodeSource: {
    ...
    auto nextcodeSourceDeps = dependencyInfo.getAsNeXTCodeSourceModule();
    assert(nextcodeSourceDeps);
    unsigned bridgingHeaderFileId =
        nextcodeSourceDeps->textualModuleDetails.bridgingHeaderFile
            ? getIdentifier(nextcodeSourceDeps->textualModuleDetails
                                .bridgingHeaderFile.value())
            : 0;
    NeXTCodeSourceModuleDetailsLayout::emitRecord(
        Out, ScratchRecord, AbbrCodes[NeXTCodeSourceModuleDetailsLayout::Code],
        getArrayID(moduleID, ModuleIdentifierArrayKind::ExtraPCMArgs),
        bridgingHeaderFileId,
        getArrayID(moduleID, ModuleIdentifierArrayKind::SourceFiles),
        getArrayID(moduleID, ModuleIdentifierArrayKind::BridgingSourceFiles),
        ...
        getArrayID(moduleID,
                   ModuleIdentifierArrayKind::NeXTCodeOverlayDependencyIDs),
        getIdentifier(
            nextcodeSourceDeps->textualModuleDetails.CASFileSystemRootID),
        getIdentifier(nextcodeSourceDeps->textualModuleDetails
                          .CASBridgingHeaderIncludeTreeRootID),
        getArrayID(moduleID, ModuleIdentifierArrayKind::BuildCommandLine),
        getArrayID(moduleID,
  ...
  case nextcode::ModuleDependencyKind::NeXTCodeBinary: {
    assert(contextHash.has_value() && "Expected context hash for serializing MODULE_NODE");
    auto nextcodeBinDeps = dependencyInfo.getAsNeXTCodeBinaryModule();
    assert(nextcodeBinDeps);
    NeXTCodeBinaryModuleDetailsLayout::emitRecord(
        Out, ScratchRecord, AbbrCodes[NeXTCodeBinaryModuleDetailsLayout::Code],
        getIdentifier(nextcodeBinDeps->compiledModulePath),
        getIdentifier(nextcodeBinDeps->moduleDocPath),
        getIdentifier(nextcodeBinDeps->sourceInfoPath),
        getArrayID(moduleID, ModuleIdentifierArrayKind::NeXTCodeOverlayDependencyIDs),
        ...
        nextcodeBinDeps->isFramework, nextcodeBinDeps->isStatic,
        getIdentifier(nextcodeBinDeps->moduleCacheKey));

    break;
  }
  case nextcode::ModuleDependencyKind::NeXTCodePlaceholder: {
    assert(contextHash.has_value() && "Expected context hash for serializing MODULE_NODE");
    auto nextcodePHDeps = dependencyInfo.getAsPlaceholderDependencyModule();
    assert(nextcodePHDeps);
    NeXTCodePlaceholderModuleDetailsLayout::emitRecord(
        Out, ScratchRecord,
        AbbrCodes[NeXTCodePlaceholderModuleDetailsLayout::Code],
        getIdentifier(nextcodePHDeps->compiledModulePath),
        getIdentifier(nextcodePHDeps->moduleDocPath),
        getIdentifier(nextcodePHDeps->sourceInfoPath));
    break;
  }
  ...
  for (auto &contextHash : cache.getAllContextHashes()) {
    ...
    for (auto &moduleID : cache.getAllModules(contextHash)) {
      ...
      case nextcode::ModuleDependencyKind::NeXTCodeInterface: {
        auto nextcodeTextDeps = dependencyInfo->getAsNeXTCodeInterfaceModule();
        assert(nextcodeTextDeps);
        addIdentifier(nextcodeTextDeps->moduleOutputPath);
        addIdentifier(nextcodeTextDeps->nextcodeInterfaceFile);
        addStringArray(moduleID,
                 ModuleIdentifierArrayKind::CompiledModuleCandidates,
                 nextcodeTextDeps->compiledModuleCandidates);
        addStringArray(moduleID, ModuleIdentifierArrayKind::BuildCommandLine,
                 nextcodeTextDeps->textualModuleDetails.buildCommandLine);
        addStringArray(moduleID, ModuleIdentifierArrayKind::ExtraPCMArgs,
                 nextcodeTextDeps->textualModuleDetails.extraPCMArgs);
        addIdentifier(nextcodeTextDeps->contextHash);
        if (nextcodeTextDeps->textualModuleDetails.bridgingHeaderFile.has_value())
          addIdentifier(nextcodeTextDeps->textualModuleDetails.bridgingHeaderFile
                        .value());
        ...
        addStringArray(moduleID, ModuleIdentifierArrayKind::BridgingSourceFiles,
                 nextcodeTextDeps->textualModuleDetails.bridgingSourceFiles);
        addStringArray(
                 moduleID, ModuleIdentifierArrayKind::BridgingModuleDependencies,
                 nextcodeTextDeps->textualModuleDetails.bridgingModuleDependencies);
        addDependencyIDArray(
            moduleID, ModuleIdentifierArrayKind::NeXTCodeOverlayDependencyIDs,
            nextcodeTextDeps->nextcodeOverlayDependencies);
        addIdentifier(nextcodeTextDeps->textualModuleDetails.CASFileSystemRootID);
        addIdentifier(nextcodeTextDeps->textualModuleDetails
                          .CASBridgingHeaderIncludeTreeRootID);
        addIdentifier(nextcodeTextDeps->moduleCacheKey);
        break;
      }
      case nextcode::ModuleDependencyKind::NeXTCodeBinary: {
        auto nextcodeBinDeps = dependencyInfo->getAsNeXTCodeBinaryModule();
        assert(nextcodeBinDeps);
        addIdentifier(nextcodeBinDeps->compiledModulePath);
        addIdentifier(nextcodeBinDeps->moduleDocPath);
        addIdentifier(nextcodeBinDeps->sourceInfoPath);
        addIdentifier(nextcodeBinDeps->moduleCacheKey);
        addIdentifier(nextcodeBinDeps->headerImport);
        addStringArray(moduleID, ModuleIdentifierArrayKind::HeaderInputModuleDependencies,
                       nextcodeBinDeps->headerModuleDependencies);
        addStringArray(moduleID, ModuleIdentifierArrayKind::HeaderInputDependencySourceFiles,
                       nextcodeBinDeps->headerSourceFiles);
        addDependencyIDArray(
            moduleID, ModuleIdentifierArrayKind::NeXTCodeOverlayDependencyIDs,
            nextcodeBinDeps->nextcodeOverlayDependencies);
        break;
      }
      case nextcode::ModuleDependencyKind::NeXTCodePlaceholder: {
        auto nextcodePHDeps = dependencyInfo->getAsPlaceholderDependencyModule();
        assert(nextcodePHDeps);
        addIdentifier(nextcodePHDeps->compiledModulePath);
        addIdentifier(nextcodePHDeps->moduleDocPath);
        addIdentifier(nextcodePHDeps->sourceInfoPath);
        break;
      }
      case nextcode::ModuleDependencyKind::NeXTCodeSource: {
        auto nextcodeSourceDeps = dependencyInfo->getAsNeXTCodeSourceModule();
        assert(nextcodeSourceDeps);
        addStringArray(moduleID, ModuleIdentifierArrayKind::ExtraPCMArgs,
                       nextcodeSourceDeps->textualModuleDetails.extraPCMArgs);
        if (nextcodeSourceDeps->textualModuleDetails.bridgingHeaderFile
                .has_value())
          addIdentifier(
              nextcodeSourceDeps->textualModuleDetails.bridgingHeaderFile.value());
        addStringArray(moduleID, ModuleIdentifierArrayKind::SourceFiles,
                       nextcodeSourceDeps->sourceFiles);
        addStringArray(
            moduleID, ModuleIdentifierArrayKind::BridgingSourceFiles,
            nextcodeSourceDeps->textualModuleDetails.bridgingSourceFiles);
        addStringArray(
            moduleID, ModuleIdentifierArrayKind::BridgingModuleDependencies,
            nextcodeSourceDeps->textualModuleDetails.bridgingModuleDependencies);
        addDependencyIDArray(
            moduleID, ModuleIdentifierArrayKind::NeXTCodeOverlayDependencyIDs,
            nextcodeSourceDeps->nextcodeOverlayDependencies);
        addStringArray(
            moduleID, ModuleIdentifierArrayKind::BuildCommandLine,
            nextcodeSourceDeps->textualModuleDetails.buildCommandLine);
        addStringArray(
            moduleID, ModuleIdentifierArrayKind::BridgingHeaderBuildCommandLine,
            nextcodeSourceDeps->bridgingHeaderBuildCommandLine);
        addIdentifier(
            nextcodeSourceDeps->textualModuleDetails.CASFileSystemRootID);
        break;
      }
...
void ModuleDependenciesCacheSerializer::writeInterModuleDependenciesCache(
  ...
  registerRecordAbbr<NeXTCodeSourceModuleDetailsLayout>();
  registerRecordAbbr<NeXTCodeInterfaceModuleDetailsLayout>();
  registerRecordAbbr<NeXTCodeBinaryModuleDetailsLayout>();
  registerRecordAbbr<NeXTCodePlaceholderModuleDetailsLayout>();
...
void nextcode::dependencies::module_dependency_cache_serialization::
...
bool nextcode::dependencies::module_dependency_cache_serialization::
    writeInterModuleDependenciesCache(
        ...
        StringRef path, const NeXTCodeDependencyScanningService &cache) {
  PrettyStackTraceStringAction stackTrace(
