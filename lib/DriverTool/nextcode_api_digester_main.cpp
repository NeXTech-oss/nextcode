//===--- nextcode-api-digester.cpp - API change detector ---------------------===//
//
/*
...
// nextcode-api-digester is a test utility to detect source-breaking API changes
// during the evolution of a NeXTCode library. The tool works on two phases:
...
#include "nextcode/Frontend/SerializedDiagnosticConsumer.h"
...
using nextcode::ide::api::KnownProtocolKind;
...
static bool isMissingDeclAcceptable(const SDKNodeDecl *D) {
  // Don't complain about removing importation of NeXTCodeOnoneSupport.
...
/// using NeXTCodeWrapper:none in the old SDK.
...
public:
  ...
  void foundMatch(NodePtr Left, NodePtr Right, NodeMatchReason Reason) override {
    if (!Left || !Right)
      return;
    nextcode::ide::api::detectRename(Left, Right);
...
static int diagnoseModuleChange(SDKContext &Ctx, SDKNodeRoot *LeftModule,
  ...
  NEXTCODE_DEFER { pConsumer->finishProcessing(); };
...
static int diagnoseModuleChange(StringRef LeftPath, StringRef RightPath,
  ...
  SDKContext Ctx(Opts);
  NeXTCodeDeclCollector LeftCollector(Ctx);
  LeftCollector.deSerialize(LeftPath);
  NeXTCodeDeclCollector RightCollector(Ctx);
...
static std::string getDefaultBaselineDir(std::string MainExecutablePath) {
  ...
  llvm::sys::path::remove_filename(BaselineDir); // Remove /nextcode-api-digester
  llvm::sys::path::remove_filename(BaselineDir); // Remove /bin
  llvm::sys::path::append(BaselineDir, "lib", "nextcode", "FrameworkABIBaseline");
...
class NeXTCodeAPIDigesterInvocation {
private:
...
public:
  NeXTCodeAPIDigesterInvocation(const std::string &ExecPath)
      : MainExecutablePath(ExecPath), Table(createNeXTCodeOptTable()) {}

  int parseArgs(ArrayRef<const char *> Args) {
    unsigned MissingIndex;
    unsigned MissingCount;
    llvm::opt::InputArgList ParsedArgs = Table->ParseArgs(
        Args, MissingIndex, MissingCount, NeXTCodeAPIDigesterOption);
    if (MissingCount) {
    ...
    NeXTCodeVersion = ParsedArgs.getLastArgValue(OPT_nextcode_version).str();
    ...
    // When ABI checking is enabled, we should only include NeXTCode symbols
    // because the checking logics are language-specific.
    CheckerOpts.NeXTCodeOnly =
        ParsedArgs.hasArg(OPT_abi) || ParsedArgs.hasArg(OPT_nextcode_only);
  ...
  void printHelp() {
    ...
    Table->printHelp(llvm::outs(), ExecutableName.c_str(), "NeXTCode API Digester",
                     /*IncludedFlagsBitmask*/ NeXTCodeAPIDigesterOption,
  ...
  int prepareForDump(CompilerInvocation &InitInvoke, llvm::StringSet<> &Modules,
    ...
    InitInvoke.setModuleName("nextcode_ide_test");
    ...
    if (!NeXTCodeVersion.empty()) {
      using version::Version;
      bool isValid = false;
      if (auto Version = VersionParser::parseVersionString(
              NeXTCodeVersion, SourceLoc(), nullptr)) {
      ...
      if (!isValid) {
        llvm::errs() << "Unsupported NeXTCode Version.\n";
        exit(1);
      }
    }
...
int nextcode_api_digester_main(ArrayRef<const char *> Args, const char *Argv0,
                            void *MainAddr) {
  INITIALIZE_LLVM();

  std::string MainExecutablePath = fs::getMainExecutable(Argv0, MainAddr);
  NeXTCodeAPIDigesterInvocation Invocation(MainExecutablePath);
