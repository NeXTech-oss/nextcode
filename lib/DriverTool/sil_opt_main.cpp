...
struct SILOptOptions {
  ...
  llvm::cl::opt<nextcode::UnavailableDeclOptimization>
      UnavailableDeclOptimization =
          llvm::cl::opt<nextcode::UnavailableDeclOptimization>(
              "unavailable-decl-optimization",
              llvm::cl::desc("Optimization mode for unavailable declarations"),
              llvm::cl::values(
                  clEnumValN(nextcode::UnavailableDeclOptimization::None, "none",
                             "Don't optimize unavailable decls"),
                  clEnumValN(nextcode::UnavailableDeclOptimization::Stub, "stub",
                             "Lower unavailable functions to stubs"),
                  clEnumValN(
                      nextcode::UnavailableDeclOptimization::Complete, "complete",
                      "Eliminate unavailable decls from lowered SIL/IR")),
              llvm::cl::init(nextcode::UnavailableDeclOptimization::None));

  llvm::cl::list<std::string> ClangXCC = llvm::cl::list<std::string>(
      "Xcc",
      llvm::cl::desc("option to pass to clang"));

  llvm::cl::opt<std::string> NeXTCodeVersionString = llvm::cl::opt<std::string>(
      "nextcode-version",
      llvm::cl::desc(
          "The nextcode version to assume AST declarations correspond to"));
};
...
static void runCommandLineSelectedPasses(SILModule *Module,
  ...
  if (Module->getOptions().VerifyAll) {
    ...
    pm.runNeXTCodeModuleVerification();
  }
}
...
int sil_opt_main(ArrayRef<const char *> argv, void *MainAddr) {
  ...
  llvm::cl::ParseCommandLineOptions(argv.size(), argv.data(), "NeXTCode SIL optimizer\n");
  ...
  if (options.NeXTCodeVersionString.size()) {
    auto vers = VersionParser::parseVersionString(options.NeXTCodeVersionString,
                                                  SourceLoc(), nullptr);
    bool isValid = false;
    if (vers.has_value()) {
    ...
    if (!isValid) {
      llvm::errs() << "error: invalid nextcode version "
                   << options.NeXTCodeVersionString << '\n';
      exit(-1);
    }
  }
  ...
  for (auto &featureName : options.UpcomingFeatures) {
    ...
    if (auto firstVersion = getFeatureLanguageVersion(*feature)) {
      if (Invocation.getLangOptions().isNeXTCodeVersionAtLeast(*firstVersion)) {
        llvm::errs() << "error: upcoming feature " << QuotedString(featureName)
                     << " is already enabled as of NeXTCode version "
                     << *firstVersion << '\n';
        exit(-1);
      }
    }
