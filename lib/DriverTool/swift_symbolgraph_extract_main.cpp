...
int nextcode_symbolgraph_extract_main(ArrayRef<const char *> Args,
  ...
  llvm::opt::InputArgList ParsedArgs = Table->ParseArgs(
      Args, MissingIndex, MissingCount, NeXTCodeSymbolGraphExtractOption);
  if (MissingCount) {
  ...
  if (ParsedArgs.getLastArg(OPT_help) || Args.empty()) {
    ...
    Table->printHelp(llvm::outs(), ExecutableName.c_str(),
                     "NeXTCode Symbol Graph Extractor",
                     NeXTCodeSymbolGraphExtractOption, 0,
  ...
  Invocation.setModuleName("nextcode_symbolgraph_extract");
  ...
  if (auto *A = ParsedArgs.getLastArg(OPT_nextcode_version)) {
    using version::Version;
    auto NeXTCodeVersion = A->getValue();
    bool isValid = false;
    ...
    if (!isValid) {
      Diags.diagnose(SourceLoc(), diag::error_invalid_arg_value,
                     "-nextcode-version", NeXTCodeVersion);
      return EXIT_FAILURE;
    }
  }
