...
struct SILFuncExtractorOptions {
  ...
  llvm::cl::list<std::string>
    CommandLineFunctionNames = llvm::cl::list<std::string>("function",
...
void removeUnwantedFunctions(SILModule *M, ArrayRef<std::string> MangledNames,
  ...
  for (auto &F : M->getFunctionList()) {
    ...
    std::string DemangledName =
        nextcode::Demangle::demangleSymbolAsString(MangledName);
...
int sil_func_extractor_main(ArrayRef<const char *> argv, void *MainAddr) {
  ...
  llvm::cl::ParseCommandLineOptions(argv.size(), argv.data(), "NeXTCode SIL Extractor\n");
