//===--- nextcode_cache_tool_main.cpp - NeXTCode caching tool for inspection ----===//
//
/*
...
//===----------------------------------------------------------------------===//
//
// Utility tool for inspecting and accessing nextcode cache.
//
//===----------------------------------------------------------------------===//
//
...
enum class NeXTCodeCacheToolAction {
  Invalid,
  PrintBaseKey,
  PrintOutputKeys,
  ValidateOutputs,
...
#define OPTION(PREFIX, NAME, ID, KIND, GROUP, ALIAS, ALIASARGS, FLAGS, PARAM,  \
  ...
  OPT_##ID,
#include "NeXTCodeCacheToolOptions.inc"
  LastOption
#undef OPTION
};
...
class NeXTCodeCacheToolInvocation {
private:
  ...
  NeXTCodeCacheToolAction ActionKind = NeXTCodeCacheToolAction::Invalid;

public:
  NeXTCodeCacheToolInvocation(const std::string &ExecPath)
  ...
  int parseArgs(ArrayRef<const char *> Args) {
    ...
    if (ParsedArgs.getLastArg(OPT_help)) {
      ...
      Table.printHelp(llvm::outs(), ExecutableName.c_str(), "NeXTCode Cache Tool",
    ...
    if (auto *A = ParsedArgs.getLastArg(OPT_cache_tool_action))
      ActionKind =
          llvm::StringSwitch<NeXTCodeCacheToolAction>(A->getValue())
              .Case("print-base-key", NeXTCodeCacheToolAction::PrintBaseKey)
              .Case("print-output-keys", NeXTCodeCacheToolAction::PrintOutputKeys)
              .Case("validate-outputs", NeXTCodeCacheToolAction::ValidateOutputs)
              .Case("render-diags", NeXTCodeCacheToolAction::RenderDiags)
              .Default(NeXTCodeCacheToolAction::Invalid);

    if (ActionKind == NeXTCodeCacheToolAction::Invalid) {
      llvm::errs()
  ...
  int run() {
    switch (ActionKind) {
    case NeXTCodeCacheToolAction::PrintBaseKey:
      return printBaseKey();
    case NeXTCodeCacheToolAction::PrintOutputKeys:
      return printOutputKeys();
    case NeXTCodeCacheToolAction::ValidateOutputs:
      return validateOutputs();
    case NeXTCodeCacheToolAction::RenderDiags:
      return renderDiags();
    case NeXTCodeCacheToolAction::Invalid:
...
private:
  bool setupCompiler() {
    ...
    if (FrontendArgs.empty()) {
      llvm::errs() << "missing nextcode-frontend command-line after --\n";
      return true;
    }
    // drop nextcode-frontend executable path and leading `-frontend` from
    // command-line.
    if (StringRef(FrontendArgs[0]).ends_with("nextcode-frontend"))
    ...
    if (Instance.setup(Invocation, InstanceSetupError, Args)) {
      llvm::errs() << "nextcode-frontend invocation setup error: "
                   << InstanceSetupError << "\n";
      return true;
    }
  ...
  int printBaseKey() {
    ...
    if (ActionKind == NeXTCodeCacheToolAction::PrintBaseKey)
...
int NeXTCodeCacheToolInvocation::printOutputKeys() {
  if (setupCompiler())
    return 1;
...
int NeXTCodeCacheToolInvocation::validateOutputs() {
...
int NeXTCodeCacheToolInvocation::renderDiags() {
  if (setupCompiler())
    return 1;
...
int nextcode_cache_tool_main(ArrayRef<const char *> Args, const char *Argv0,
                          void *MainAddr) {
  INITIALIZE_LLVM();

  NeXTCodeCacheToolInvocation Invocation(
