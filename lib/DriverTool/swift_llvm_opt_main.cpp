...
/// \file
///
/// This is a simple reimplementation of opt that includes support for NeXTCode-
...
/// NeXTCode's LLVM pass pipeline by using the same pass pipeline building
...
struct NeXTCodeLLVMOptOptions {
  llvm::cl::opt<bool>
    Optimized = llvm::cl::opt<bool>("O", llvm::cl::desc("Optimization level O. Similar to nextcode -O"));
...
static llvm::CodeGenOpt::Level GetCodeGenOptLevel(const NeXTCodeLLVMOptOptions &options) {
...
getTargetMachine(llvm::Triple TheTriple, StringRef CPUStr,
                 ...
                 const NeXTCodeLLVMOptOptions &options) {
  std::string Error;
...
int nextcode_llvm_opt_main(ArrayRef<const char *> argv, void *MainAddr) {
  INITIALIZE_LLVM();

  NeXTCodeLLVMOptOptions options;

  llvm::cl::ParseCommandLineOptions(argv.size(), argv.data(), "NeXTCode LLVM optimizer\n");
  ...
  if (options.Optimized) {
    ...
    PB.registerPipelineParsingCallback(
                [ModuleTriple](StringRef Name, llvm::ModulePassManager &PM,
                  ...
                  if (Name == "nextcode-merge-functions") {
                    if (ModuleTriple.isArm64e())
                      PM.addPass(NeXTCodeMergeFunctionsPass(true, 0));
                    else
                      PM.addPass(NeXTCodeMergeFunctionsPass(false, 0));
                    return true;
                  }
                  return false;
                });
    PB.registerPipelineParsingCallback(
                [ModuleTriple](StringRef Name, llvm::FunctionPassManager &PM,
                   ArrayRef<llvm::PassBuilder::PipelineElement>) {
                  if (Name == "nextcode-llvm-arc-optimize") {
                      PM.addPass(NeXTCodeARCOptPass());
                    return true;
                  }
                  return false;
                });
    PB.registerPipelineParsingCallback(
                [ModuleTriple](StringRef Name, llvm::FunctionPassManager &PM,
                   ArrayRef<llvm::PassBuilder::PipelineElement>) {
                  if (Name == "nextcode-llvm-arc-contract") {
                      PM.addPass(NeXTCodeARCContractPass());
                    return true;
                  }
                  return false;
                });
