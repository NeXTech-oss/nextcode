//===--- LLVMMergeFunctions.cpp - Merge similar functions for nextcode -------===//
//
...
// Currently this is very NeXTCode specific in the sense that it's intended to
...
#define DEBUG_TYPE "nextcode-mergefunc"

STATISTIC(NumNeXTCodeFunctionsMerged, "Number of functions merged");
STATISTIC(NumNeXTCodeThunksWritten, "Number of thunks generated");

static cl::opt<unsigned> NumFunctionsForSanityCheck(
    "nextcodemergefunc-sanity",
    cl::desc("How many functions in module could be used for "
             "NeXTCodeMergeFunctions pass sanity check. "
             "'0' disables this check. Works only with '-debug' key."),
    cl::init(0), cl::Hidden);

static cl::opt<unsigned> FunctionMergeThreshold(
    "nextcodemergefunc-threshold",
...
class NeXTCodeFunctionComparator : FunctionComparator {
public:
  NeXTCodeFunctionComparator(const Function *F1, const Function *F2,
...
int NeXTCodeFunctionComparator::
...
int NeXTCodeFunctionComparator::compareIgnoringConsts() {
  beginCompare();
...
/// NeXTCodeMergeFunctions finds functions which only differ by constants in
...
///
class NeXTCodeMergeFunctions {
public:
  NeXTCodeMergeFunctions() : FnTree(FunctionNodeCmp(&GlobalNumbers)) {}

  NeXTCodeMergeFunctions(bool ptrAuthEnabled, unsigned ptrAuthKey)
...
private:
  ...
  public:
    ...
    bool operator()(const EquivalenceClass &LHS, const EquivalenceClass &RHS) const {
      ...
      NeXTCodeFunctionComparator FCmp(LHS.First->F, RHS.First->F, GlobalNumbers);
...
class LegacyNeXTCodeMergeFunctions : public ModulePass {
public:
  static char ID;
  NeXTCodeMergeFunctions impl;

  LegacyNeXTCodeMergeFunctions() : ModulePass(ID) {}

  LegacyNeXTCodeMergeFunctions(bool ptrAuthEnabled, unsigned ptrAuthKey)
...
char LegacyNeXTCodeMergeFunctions::ID = 0;
INITIALIZE_PASS_BEGIN(LegacyNeXTCodeMergeFunctions, "nextcode-merge-functions",
                      "NeXTCode merge function pass", false, false)
INITIALIZE_PASS_END(LegacyNeXTCodeMergeFunctions, "nextcode-merge-functions",
                    "NeXTCode merge function pass", false, false)

llvm::ModulePass *
nextcode::createLegacyNeXTCodeMergeFunctionsPass(bool ptrAuthEnabled,
                                           unsigned ptrAuthKey) {
  initializeLegacyNeXTCodeMergeFunctionsPass(
      *llvm::PassRegistry::getPassRegistry());
  return new LegacyNeXTCodeMergeFunctions(ptrAuthEnabled, ptrAuthKey);
}

bool NeXTCodeMergeFunctions::doSanityCheck(std::vector<WeakTrackingVH> &Worklist) {
  if (const unsigned Max = NumFunctionsForSanityCheck) {
    ...
    for (std::vector<WeakTrackingVH>::iterator I = Worklist.begin(),
      ...
      for (std::vector<WeakTrackingVH>::iterator J = I; J != E && j < Max;
        ...
        int Res1 = NeXTCodeFunctionComparator(F1, F2, &GlobalNumbers).
                     compareIgnoringConsts();
        int Res2 = NeXTCodeFunctionComparator(F2, F1, &GlobalNumbers).
                     compareIgnoringConsts();
        ...
        for (std::vector<WeakTrackingVH>::iterator K = J; K != E && k < Max;
          ...
          int Res3 = NeXTCodeFunctionComparator(F1, F3, &GlobalNumbers).
                       compareIgnoringConsts();
          int Res4 = NeXTCodeFunctionComparator(F2, F3, &GlobalNumbers).
                       compareIgnoringConsts();
...
static bool isEligibleFunction(Function *F) {
  ...
  if (F->getCallingConv() == CallingConv::NeXTCodeTail)
    return false;
...
bool NeXTCodeMergeFunctions::runOnModule(Module &M) {
  ...
  if (!ptrAuthOptionsSet) {
    ...
    // If invoked from nextcode-llvm-opt, derive the options from the target triple.
...
void NeXTCodeMergeFunctions::updateUnhandledCalleeCount(FunctionEntry *FE,
                                                     int Delta) {
...
bool NeXTCodeMergeFunctions::tryMergeEquivalenceClass(FunctionEntry *FirstInClass) {
...
/// removed function to
NeXTCodeMergeFunctions::FunctionInfo NeXTCodeMergeFunctions::
...
bool NeXTCodeMergeFunctions::deriveParams(ParamInfos &Params,
                                       FunctionInfos &FInfos,
                                       unsigned maxParams) {
...
bool NeXTCodeMergeFunctions::numOperandsDiffer(FunctionInfos &FInfos) {
...
bool NeXTCodeMergeFunctions::constsDiffer(const FunctionInfos &FInfos,
                                       unsigned OpIdx) {
...
bool NeXTCodeMergeFunctions::tryMapToParameter(FunctionInfos &FInfos,
...
/// discriminator.
void NeXTCodeMergeFunctions::replaceCallWithAddedPtrAuth(CallInst *origCall,
                                    Value *newCallee,
                                    ConstantInt *discriminator) {
...
void NeXTCodeMergeFunctions::mergeWithParams(const FunctionInfos &FInfos,
                                          ParamInfos &Params) {
...
void NeXTCodeMergeFunctions::removeEquivalenceClassFromTree(FunctionEntry *FE) {
...
/// \p Params.
void NeXTCodeMergeFunctions::writeThunk(Function *ToFunc, Function *Thunk,
  ...
  bool isNeXTCodeTailCall =
   ToFunc->getCallingConv() == CallingConv::NeXTCodeTail &&
   Thunk->getCallingConv() == CallingConv::NeXTCodeTail;
  CI->setTailCallKind(
    isNeXTCodeTailCall ? llvm::CallInst::TCK_MustTail : llvm::CallInst::TCK_Tail);
...
bool NeXTCodeMergeFunctions::replaceDirectCallers(Function *Old, Function *New,
                                   const ParamInfos &Params, unsigned FuncIdx) {
...
PreservedAnalyses NeXTCodeMergeFunctionsPass::run(Module &M,
                                               ModuleAnalysisManager &AM) {
  NeXTCodeMergeFunctions helper(ptrAuthEnabled, ptrAuthKey);
