...
#define DEBUG_TYPE "nextcode-arc-contract"
...
using nextcode::NeXTCodeARCContract;
...
/// Pimpl implementation of NeXTCodeARCContractPass.
namespace {
...
class NeXTCodeARCContractImpl {
  /// Was a change made while running the optimization.
  bool Changed;

  /// NeXTCode RC Identity.
...
public:
  NeXTCodeARCContractImpl(Function &InF) : Changed(false), F(InF), B(F) {}
...
performRRNOptimization(DenseMap<Value *, LocalState> &PtrToLocalStateMap) {
  ...
  for (auto &P : PtrToLocalStateMap) {
    ...
    if (RetainList.size() > 1) {
      ...
      B.createRetainN(RC.getNeXTCodeRCIdentityRoot(O), RetainList.size(), RI);
    ...
    if (ReleaseList.size() > 1) {
      ...
      B.createReleaseN(RC.getNeXTCodeRCIdentityRoot(O), ReleaseList.size(), RI);
    ...
    if (UnknownObjectRetainList.size() > 1) {
      ...
      B.createUnknownObjectRetainN(RC.getNeXTCodeRCIdentityRoot(O),
    ...
    if (UnknownObjectReleaseList.size() > 1) {
      ...
      B.createUnknownObjectReleaseN(RC.getNeXTCodeRCIdentityRoot(O),
    ...
    if (BridgeRetainList.size() > 1) {
      ...
      auto *I = B.createBridgeRetainN(RC.getNeXTCodeRCIdentityRoot(O),
    ...
    if (BridgeReleaseList.size() > 1) {
      ...
      B.createBridgeReleaseN(RC.getNeXTCodeRCIdentityRoot(O),
...
bool NeXTCodeARCContractImpl::run() {
  ...
  for (BasicBlock &BB : F) {
    for (auto II = BB.begin(), IE = BB.end(); II != IE; ) {
      ...
      case RT_Retain: {
        ...
        auto *ArgVal = RC.getNeXTCodeRCIdentityRoot(CI->getArgOperand(0));
...
bool NeXTCodeARCContract::runOnFunction(Function &F) {
  return NeXTCodeARCContractImpl(F).run();
}

char NeXTCodeARCContract::ID = 0;
INITIALIZE_PASS_BEGIN(NeXTCodeARCContract, "nextcode-arc-contract",
                      "NeXTCode ARC contraction", false, false)
INITIALIZE_PASS_END(NeXTCodeARCContract,
                    "nextcode-arc-contract", "NeXTCode ARC contraction",
                    false, false)

llvm::FunctionPass *nextcode::createNeXTCodeARCContractPass() {
  initializeNeXTCodeARCContractPass(*llvm::PassRegistry::getPassRegistry());
  return new NeXTCodeARCContract();
}

void NeXTCodeARCContract::getAnalysisUsage(llvm::AnalysisUsage &AU) const {
  AU.setPreservesCFG();
}

llvm::PreservedAnalyses
NeXTCodeARCContractPass::run(llvm::Function &F,
                          llvm::FunctionAnalysisManager &AM) {
  bool changed = NeXTCodeARCContractImpl(F).run();
  if (!changed)
