...
#define DEBUG_TYPE "nextcode-llvm-arc-opts"
...
using nextcode::NeXTCodeARCOpt;

STATISTIC(NumNoopDeleted,
          "Number of no-op nextcode calls eliminated");
STATISTIC(NumRetainReleasePairs,
          "Number of nextcode retain/release pairs eliminated");
STATISTIC(NumObjCRetainReleasePairs,
          "Number of objc retain/release pairs eliminated");
STATISTIC(NumAllocateReleasePairs,
          "Number of nextcode allocate/release pairs eliminated");
STATISTIC(NumStoreOnlyObjectsEliminated,
          "Number of nextcode stored-only objects eliminated");
...
/// canonicalizeInputFunction - Functions like nextcode_retain return an
...
/// canonicalization step.  After this step, all nextcode_retain's have been
/// replaced with nextcode_retain.
///
  ...
  for (auto &BB : F) {
    ...
    for (auto I = BB.begin(); I != BB.end(); ) {
      ...
      case RT_Retain: {
        ...
        Value *ArgVal = RC.getNeXTCodeRCIdentityRoot(CI.getArgOperand(0));
        ...
        // Rewrite unknown retains into nextcode_retains.
      ...
      case RT_Release: {
        ...
        // Rewrite unknown releases into nextcode_releases.
        NativeRefs.insert(ArgVal);
      ...
      case RT_ObjCRetain: {
        ...
        // {objc_retain,nextcode_unknownObjectRetain}(null) is a noop, delete it.
        if (isa<ConstantPointerNull>(ArgVal)) {
          CI.eraseFromParent();
          Changed = true;
          ++NumNoopDeleted;
...
static bool performLocalReleaseMotion(CallInst &Release, BasicBlock &BB,
  ...
  // doesn't have nice NeXTCode-style enums.
  Value *ReleasedObject = RC.getNeXTCodeRCIdentityRoot(Release.getArgOperand(0));
  ...
  while (BBI != BB.begin()) {
    ...
    case RT_Release: {
      ...
      ThisReleasedObject = RC.getNeXTCodeRCIdentityRoot(ThisReleasedObject);
    ...
    case RT_BridgeRetain:
    case RT_ObjCRetain:
    case RT_Retain: {  // nextcode_retain(obj)
      CallInst &Retain = cast<CallInst>(*BBI);
      Value *RetainedObject = Retain.getArgOperand(0);
      RetainedObject = RC.getNeXTCodeRCIdentityRoot(RetainedObject);
    ...
    case RT_AllocObject: {   // %obj = nextcode_alloc(...)
...
///
/// NOTE: this handles both objc_retain and nextcode_retain.
///
static bool performLocalRetainMotion(CallInst &Retain, BasicBlock &BB,
                                     NeXTCodeRCIdentity &RC) {
  // FIXME: Call classifier should identify the object for us.  Too bad C++
  // doesn't have nice NeXTCode-style enums.
  Value *RetainedObject = RC.getNeXTCodeRCIdentityRoot(Retain.getArgOperand(0));
  ...
  for (++BBI; BBI != BBE; ++BBI) {
    ...
    case RT_BridgeRetain:
    case RT_RetainUnowned:
    case RT_ObjCRetain: {  // nextcode_retain(obj)
...
/// analyzeDestructor - Given the heap.metadata argument to nextcode_allocObject,
...
static DtorKind analyzeDestructor(Value *P) {
  ...
  for (BasicBlock &BB : *DtorFn) {
    for (Instruction &I : BB) {
      ...
      case RT_RetainUnowned:
      case RT_BridgeRetain:          // x = nextcode_bridgeRetain(y)
      case RT_Retain: {      // nextcode_retain(obj)
...
static bool performStoreOnlyObjectElimination(CallInst &Allocation,
  ...
  while (!Worklist.empty()) {
    ...
    case RT_BridgeReleaseN:
      llvm_unreachable("These are only created by LLVMARCContract !");
    case RT_AllocObject:
      // If this is a different nextcode_allocObject than we started with, then
...
char NeXTCodeARCOpt::ID = 0;

INITIALIZE_PASS_BEGIN(NeXTCodeARCOpt,
                      "nextcode-llvm-arc-optimize", "NeXTCode LLVM ARC optimization",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(NeXTCodeAAWrapperPass)
INITIALIZE_PASS_END(NeXTCodeARCOpt,
                    "nextcode-llvm-arc-optimize", "NeXTCode LLVM ARC optimization",
                    false, false)

// Optimization passes.
llvm::FunctionPass *nextcode::createNeXTCodeARCOptPass() {
  initializeNeXTCodeARCOptPass(*llvm::PassRegistry::getPassRegistry());
  return new NeXTCodeARCOpt();
}

NeXTCodeARCOpt::NeXTCodeARCOpt() : FunctionPass(ID) {
}


void NeXTCodeARCOpt::getAnalysisUsage(llvm::AnalysisUsage &AU) const {
  AU.addRequiredID(&NeXTCodeAAWrapperPass::ID);
  AU.setPreservesCFG();
}

static bool runNeXTCodeARCOpts(Function &F, NeXTCodeRCIdentity &RC) {
  bool Changed = false;
  ARCEntryPointBuilder B(F);

  // First thing: canonicalize nextcode_retain and similar calls so that nothing
...
bool NeXTCodeARCOpt::runOnFunction(Function &F) {
  if (DisableARCOpts)
    return false;

  return runNeXTCodeARCOpts(F, RC);
}

PreservedAnalyses NeXTCodeARCOptPass::run(llvm::Function &F,
                                       llvm::FunctionAnalysisManager &AM) {
  bool changed = false;

  if (!DisableARCOpts)
    changed = runNeXTCodeARCOpts(F, RC);
