...
//===----------------------------------------------------------------------===//
//
//  This file implements IR generation for value witnesses in NeXTCode.
//
...
static llvm::Constant *getNoOpVoidFunction(IRGenModule &IGM) {
  ...
  return IGM.getOrCreateHelperFunction("__nextcode_noop_void_return",
                                       IGM.VoidTy, argTys,
                                       [&](IRGenFunction &IGF) {
    IGF.Builder.CreateRetVoid();
  });
...
static llvm::Constant *getNoncopyableTrapFunction(IRGenModule &IGM) {
  return IGM.getOrCreateHelperFunction("__nextcode_cannot_copy_noncopyable_type",
                                       IGM.VoidTy, {},
                                       [&](IRGenFunction &IGF) {
...
static llvm::Constant *getAssignWithCopyStrongFunction(IRGenModule &IGM) {
  ...
  return IGM.getOrCreateHelperFunction("__nextcode_assignWithCopy_strong",
                                       ptrPtrTy, argTys,
                                       [&](IRGenFunction &IGF) {
...
static llvm::Constant *getAssignWithTakeStrongFunction(IRGenModule &IGM) {
  llvm::Type *ptrPtrTy = IGM.RefCountedPtrTy->getPointerTo();
  llvm::Type *argTys[] = { ptrPtrTy, ptrPtrTy, IGM.WitnessTablePtrTy };
  return IGM.getOrCreateHelperFunction("__nextcode_assignWithTake_strong",
                                       ptrPtrTy, argTys,
                                       [&](IRGenFunction &IGF) {
    auto it = IGF.CurFn->arg_begin();
    Address dest(&*(it++), IGM.RefCountedPtrTy, IGM.getPointerAlignment());
...
static llvm::Constant *getInitWithCopyStrongFunction(IRGenModule &IGM) {
  llvm::Type *ptrPtrTy = IGM.RefCountedPtrTy->getPointerTo();
  llvm::Type *argTys[] = { ptrPtrTy, ptrPtrTy, IGM.WitnessTablePtrTy };
  return IGM.getOrCreateHelperFunction("__nextcode_initWithCopy_strong",
                                       ptrPtrTy, argTys,
                                       [&](IRGenFunction &IGF) {
    auto it = IGF.CurFn->arg_begin();
    Address dest(&*(it++), IGM.RefCountedPtrTy, IGM.getPointerAlignment());
...
/// pointer from the first, and calls nextcode_release on it immediately.
static llvm::Constant *getDestroyStrongFunction(IRGenModule &IGM) {
  ...
  return IGM.getOrCreateHelperFunction(
      "__nextcode_destroy_strong", IGM.VoidTy, argTys, [&](IRGenFunction &IGF) {
...
bool irgen::layoutStringsEnabled(IRGenModule &IGM, bool diagnose) {
  auto moduleName = IGM.getNeXTCodeModule()->getRealName().str();
...
static void addValueWitness(IRGenModule &IGM, ConstantStructBuilder &B,
  ...
  case ValueWitness::Destroy:
    ...
    } else if (concreteTI.isSingleNeXTCodeRetainablePointer(ResilienceExpansion::Maximal)) {
...
Address TypeInfo::indexArray(IRGenFunction &IGF, Address base,
  ...
  // The stride of a NeXTCode type may not match its LLVM size. If we know we have
