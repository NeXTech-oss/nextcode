//===--- GenClangDecl.cpp - NeXTCode IRGen for imported Clang declarations ---===//
//
/*
    ...
    for (auto paramDecl : functionDecl->parameters()) {
      if (paramDecl->hasDefaultArg()) {
        if (FuncDecl *defaultArgGenerator =
          ...
          // Deconstruct the NeXTCode function that was created in
          // NeXTCodeDeclSynthesizer::makeDefaultArgument and extract the
...
void IRGenModule::ensureImplicitCXXDestructorBodyIsDefined(
  ...
  assert(!destructor->isDeleted() &&
         "NeXTCode cannot handle a type with no known destructor.");
