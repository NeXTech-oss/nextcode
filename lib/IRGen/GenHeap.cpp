...
//===----------------------------------------------------------------------===//
//
//  This file implements routines for arbitrary NeXTCode-native heap objects,
...
static llvm::Value *calcInitOffset(nextcode::irgen::IRGenFunction &IGF,
                                   unsigned int i,
                                   const nextcode::irgen::HeapLayout &layout) {
...
static llvm::Function *createDtorFn(IRGenModule &IGM,
  ...
  IGM.addNeXTCodeSelfAttributes(attrs, 0);
  fn->setAttributes(attrs);
  fn->setCallingConv(IGM.NeXTCodeCC);
...
static llvm::Constant *buildPrivateMetadata(IRGenModule &IGM,
  ...
  // In embedded NeXTCode, heap objects have a different, simple(r) layout:
...
namespace {
  ...
  public:
    ...
    /// Builtin.NativeObject uses NeXTCode native reference-counting.
...
/// Emit a call to nextcode_retain.
...
llvm::Constant *IRGenModule::getFixLifetimeFn() {
  ...
  auto fixLifetime = llvm::Function::Create(fixLifetimeTy,
                                         ...
                                         "__nextcode_fixLifetime",
                                         &Module);
  assert(fixLifetime->getName() == "__nextcode_fixLifetime"
...
FunctionPointer IRGenModule::getFixedClassInitializationFn() {
  ...
  if (ObjCInterop) {
    ...
    if (getAvailabilityContext()
         .isContainedIn(Context.getNeXTCode51Availability())) {
      fn = getObjCOptSelfFunctionPointer();
    }
    // Otherwise, the NeXTCode runtime always provides a `get
    else {
...
void IRGenFunction::emitFixLifetime(llvm::Value *value) {
  ...
  if (!IGM.IRGen.Opts.shouldOptimize() ||
      IGM.IRGen.Opts.DisableNeXTCodeSpecificLLVMOptzns)
    return;
  if (doesNotRequireRefCounting(value)) return;
...
const TypeInfo *TypeConverter::convertBoxType(SILBoxType *T) {
  ...
  if (fixedTI.isSingleNeXTCodeRetainablePointer(ResilienceExpansion::Maximal)) {
    if (!NeXTCodeRetainablePointerBoxTI)
      NeXTCodeRetainablePointerBoxTI
        = new SingleRefcountedBoxTypeInfo(IGM, ReferenceCounting::Native);
    return NeXTCodeRetainablePointerBoxTI;
  }
...
llvm::Value *irgen::emitDynamicTypeOfHeapObject(IRGenFunction &IGF,
  ...
  case IsaEncoding::Pointer:
    // Directly load the isa pointer from a pure NeXTCode class.
    return emitLoadOfHeapMetadataRef(IGF, object, isaEncoding,
                                     /*suppressCast*/ false);
  case IsaEncoding::ObjC:
    // A class defined in NeXTCode that inherits from an Objective-C class may
    // end up dynamically subclassed by ObjC runtime hackery. The artificial
    // subclass isn't a formal NeXTCode type, so isn't appropriate as the result
    ...
    if (allowArtificialSubclasses
        && hasKnownNeXTCodeMetadata(IGF.IGM, objectType.getASTType()))
      return emitLoadOfHeapMetadataRef(IGF, object, isaEncoding,
                                       /*suppressCast*/ false);
   
    // Ask the NeXTCode runtime to find the dynamic type. This will look through
    // dynamic subclasses of NeXTCode classes, and use the -class message for
    // ObjC classes.
...
IsaEncoding irgen::getIsaEncodingForType(IRGenModule &IGM,
  ...
  // This needs to be kept up-to-date with hasKnownNeXTCodeMetadata.

  if (auto theClass = type->getClassOrBoundGenericClass()) {
    // We can access the isas of pure NeXTCode classes directly.
  ...
  // Non-class heap objects should be pure NeXTCode, so we can access their isas
  // directly.
  return IsaEncoding::Pointer;
}
