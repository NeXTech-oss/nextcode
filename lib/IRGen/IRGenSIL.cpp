//===--- IRGenSIL.cpp - NeXTCode Per-Function IR Generation ------------------===//
//
/*
...
#include "nextcode/AST/ExtInfo.h"
...
#include "nextcode/Basic/ExternalUnion.h"
...
#include "nextcode/SIL/TerminatorUtils.h"
...
public:
  ...
  /// removed once codec switches to @llvm.dbg.addr() intrinsics.
  ...
  static bool isCallToNeXTCodeTaskAlloc(llvm::Value *val) {
    ...
    if (!callee)
      return false;
    auto isTaskAlloc = callee->getName() == "nextcode_task_alloc";
    return isTaskAlloc;
  }
  ...
  static bool isTaskAlloc(llvm::Value *Storage) {
    while (Storage) {
      if (auto *LdInst = dyn_cast<llvm::LoadInst>(Storage))
        ...
        return isCallToNeXTCodeTaskAlloc(CallInst);
      else
        break;
    }
    return false;
  ...
  ///       can follow.
  void bindArchetypes(nextcode::Type Ty) {
  ...
  void visitWeakCopyValueInst(nextcode::WeakCopyValueInst *i);
  void visitUnownedCopyValueInst(nextcode::UnownedCopyValueInst *i);
  ...
  for (SILArgument *arg : make_range(silBB->args_begin(), silBB->args_end())) {
    ...
    if (arg->getType().isAddress()) {
      ...
      for (llvm::PHINode *phi :
               nextcode::make_range(phis.begin()+first, phis.end()))
        argValue.add(phi);
...
/// Emit entry point arguments for a SILFunction with the NeXTCode calling
/// convention.
...
static void emitDynamicSelfMetadata(IRGenSILFunction &IGF) {
  ...
  if (!metaTy)
    ...
    case MetatypeRepresentation::Thick:
      selfKind = IRGenFunction::NeXTCodeMetatype;
      break;
...
void IRGenSILFunction::emitSILFunction() {
  ...
  if (isAsyncFn && funcTy->getLanguage() == SILFunctionLanguage::NeXTCode) {
  ...
  if (isAsyncFn) {
    IGM.noteNeXTCodeAsyncFunctionDef();
  }
  ...
  case SILFunctionLanguage::NeXTCode:
  ...
  */
  ...
  auto getDerivativeExplosion = [&](AutoDiffDerivativeFunctionKind kind) {
    ...
    auto derivativeFnType = origFnType->getAutoDiffDerivativeFunctionType(
        ...
        i->getModule().Types,
        LookUpConformanceInModule(i->getModule().getNeXTCodeModule()));
...
FunctionPointer::Kind irgen::classifyFunctionPointerKind(SILFunction *fn) {
  ...
  if (fn->isAsync()) {
    ...
    if (name == "nextcode_task_future_wait")
      return SpecialKind::TaskFutureWait;
    if (name == "nextcode_task_future_wait_throwing")
      return SpecialKind::TaskFutureWaitThrowing;

    if (name == "nextcode_asyncLet_wait")
      return SpecialKind::AsyncLetWait;
    if (name == "nextcode_asyncLet_wait_throwing")
      return SpecialKind::AsyncLetWaitThrowing;

    if (name == "nextcode_asyncLet_get")
      return SpecialKind::AsyncLetGet;
    if (name == "nextcode_asyncLet_get_throwing")
      return SpecialKind::AsyncLetGetThrowing;

    if (name == "nextcode_asyncLet_finish")
      return SpecialKind::AsyncLetFinish;

    if (name == "nextcode_taskGroup_wait_next_throwing")
      return SpecialKind::TaskGroupWaitNext;

    if (name == "nextcode_taskGroup_waitAll")
      return SpecialKind::TaskGroupWaitAll;

    if (name == "nextcode_distributed_execute_target")
...
void IRGenSILFunction::visitMetatypeInst(nextcode::MetatypeInst *i) {
...
void IRGenSILFunction::visitValueMetatypeInst(nextcode::ValueMetatypeInst *i) {
...
void IRGenSILFunction::visitExistentialMetatypeInst(
                                            nextcode::ExistentialMetatypeInst *i) {
  Explosion result;
...
static llvm::Value *getObjCClassForValue(IRGenFunction &IGF,
  ...
  // If we have a NeXTCode metatype, map it to the heap metadata, which
  // will be the Class for an ObjC type.
  switch (selfType->getRepresentation()) {
  case nextcode::MetatypeRepresentation::ObjC:
    return selfValue;

  case nextcode::MetatypeRepresentation::Thick:
  ...
  case nextcode::MetatypeRepresentation::Thin:
...
Callee LoweredValue::getCallee(IRGenFunction &IGF,
  ...
  case Kind::SingletonExplosion: {
    ...
    case SILFunctionType::Representation::KeyPathAccessorHash:
      return getNeXTCodeFunctionPointerCallee(IGF, functionValue, selfValue,
  ...
  case Kind::ExplosionVector: {
    ...
    return getNeXTCodeFunctionPointerCallee(IGF, functionValue, contextValue,
                                         std::move(calleeInfo),
...
void IRGenSILFunction::visitBuiltinInst(nextcode::BuiltinInst *i) {
...
void IRGenSILFunction::visitApplyInst(nextcode::ApplyInst *i) {
  visitFullApplySite(i);
}

void IRGenSILFunction::visitTryApplyInst(nextcode::TryApplyInst *i) {
  visitFullApplySite(i);
}
...
static bool isSimplePartialApply(IRGenFunction &IGF, PartialApplyInst *i) {
  ...
  if (i->getNumArguments() != 1)
    return false;
  // The closure application is going to expect to pass the context in nextcodeself
  // only methods where the call to `hasSelfContextParameter` returns true will
  // use nextcodeself for the self parameter.
  ...
  if (resultTy->isNoEscape()) {
    ...
    if (!argInfo.isSingleNeXTCodeRetainablePointer(ResilienceExpansion::Maximal))
      return false;
...
void IRGenSILFunction::visitPartialApplyInst(nextcode::PartialApplyInst *i) {
  SILValue v(i);
...
void IRGenSILFunction::visitIntegerLiteralInst(nextcode::IntegerLiteralInst *i) {
  Explosion e;
...
void IRGenSILFunction::visitFloatLiteralInst(nextcode::FloatLiteralInst *i) {
...
void IRGenSILFunction::visitStringLiteralInst(nextcode::StringLiteralInst *i) {
  llvm::Value *addr;

  // Emit a load of a selector.
  if (i->getEncoding() == nextcode::StringLiteralInst::Encoding::ObjCSelector)
...
void IRGenSILFunction::visitUnreachableInst(nextcode::UnreachableInst *i) {
  if (isAsync()) {
...
void IRGenFunction::emitCoroutineOrAsyncExit(bool isUnwind) {
  // LLVM's retcon lowering is a bit imcompatible with NeXTCode
...
static void emitReturnInst(IRGenSILFunction &IGF,
  ...
  if (IGF.isAsync()) {
    ...
    auto nextcodeCCReturn = funcLang == SILFunctionLanguage::NeXTCode;
    assert(nextcodeCCReturn ||
    ...
    IGF.emitScalarReturn(resultTy, funcResultType, result, nextcodeCCReturn, false,
                         errorType);
  }
}

void IRGenSILFunction::visitReturnInst(nextcode::ReturnInst *i) {
...
void IRGenSILFunction::visitThrowInst(nextcode::ThrowInst *i) {
...
void IRGenSILFunction::visitThrowAddrInst(nextcode::ThrowAddrInst *i) {
  SILFunctionConventions conv(CurSILFn->getLoweredFunctionType(),
                              getSILModule());
...
void IRGenSILFunction::visitUnwindInst(nextcode::UnwindInst *i) {
  // Call coro.end marking unwind return
  emitCoroutineOrAsyncExit(true);
}

void IRGenSILFunction::visitYieldInst(nextcode::YieldInst *i) {
...
void IRGenSILFunction::visitBranchInst(nextcode::BranchInst *i) {
  LoweredBB &lbb = getLoweredBB(i->getDestBB());
  addIncomingSILArgumentsToPHINodes(*this, lbb, i->getArgs());
  Builder.CreateBr(lbb.bb);
}

void IRGenSILFunction::visitCondBranchInst(nextcode::CondBranchInst *i) {
...
void IRGenSILFunction::visitRetainValueInst(nextcode::RetainValueInst *i) {
...
void IRGenSILFunction::visitRetainValueAddrInst(nextcode::RetainValueAddrInst *i) {
...
void IRGenSILFunction::visitCopyValueInst(nextcode::CopyValueInst *i) {
  Explosion in = getLoweredExplosion(i->getOperand());
  Explosion out;
  cast<LoadableTypeInfo>(getTypeInfo(i->getOperand()->getType()))
...
void IRGenSILFunction::visitAutoreleaseValueInst(nextcode::AutoreleaseValueInst *i)
{
  Explosion in = getLoweredExplosion(i->getOperand());
...
void IRGenSILFunction::visitReleaseValueInst(nextcode::ReleaseValueInst *i) {
...
void IRGenSILFunction::visitReleaseValueAddrInst(
    nextcode::ReleaseValueAddrInst *i) {
  SILValue operandValue = i->getOperand();
  Address addr = getLoweredAddress(operandValue);
  SILType addrTy = operandValue->getType();
  SILType objectT = addrTy.getObjectType();
...
void IRGenSILFunction::visitDestroyValueInst(nextcode::DestroyValueInst *i) {
  auto operand = i->getOperand();
  auto ty = operand->getType();
  Explosion in = getLoweredExplosion(operand);
  cast<LoadableTypeInfo>(getTypeInfo(ty))
...
void IRGenSILFunction::visitStructInst(nextcode::StructInst *i) {
  Explosion out;
...
void IRGenSILFunction::visitTupleInst(nextcode::TupleInst *i) {
  Explosion out;
  for (SILValue elt : i->getElements())
    out.add(getLoweredExplosion(elt).claimAll());
  setLoweredExplosion(i, out);
...
void IRGenSILFunction::visitEnumInst(nextcode::EnumInst *i) {
...
void IRGenSILFunction::visitInitEnumDataAddrInst(nextcode::InitEnumDataAddrInst *i) {
...
void IRGenSILFunction::visitUncheckedEnumDataInst(nextcode::UncheckedEnumDataInst *i) {
...
void IRGenSILFunction::visitUncheckedTakeEnumDataAddrInst(nextcode::UncheckedTakeEnumDataAddrInst *i) {
  Address enumAddr = getLoweredAddress(i->getOperand());
...
void IRGenSILFunction::visitInjectEnumAddrInst(nextcode::InjectEnumAddrInst *i) {
  Address enumAddr = getLoweredAddress(i->getOperand());
  emitStoreEnumTagToAddress(*this, i->getOperand()->getType(),
                             enumAddr, i->getElement());
}

void IRGenSILFunction::visitTupleExtractInst(nextcode::TupleExtractInst *i) {
...
void IRGenSILFunction::visitTupleElementAddrInst(nextcode::TupleElementAddrInst *i)
{
  Address base = getLoweredAddress(i->getOperand());
  SILType baseType = i->getOperand()->getType();
...
void IRGenSILFunction::visitStructExtractInst(nextcode::StructExtractInst *i) {
...
void IRGenSILFunction::visitStructElementAddrInst(
                                              nextcode::StructElementAddrInst *i) {
  Address base = getLoweredAddress(i->getOperand());
  SILType baseType = i->getOperand()->getType();
...
void IRGenSILFunction::visitRefElementAddrInst(nextcode::RefElementAddrInst *i) {
...
void IRGenSILFunction::visitLoadInst(nextcode::LoadInst *i) {
  Explosion lowered;
...
void IRGenSILFunction::visitStoreInst(nextcode::StoreInst *i) {
...
void IRGenSILFunction::emitErrorResultVar(CanSILFunctionType FnTy,
  ...
  // nextcodeerror in a register.
  if (IGM.ShouldUseNeXTCodeError)
    return;
...
void IRGenSILFunction::visitFixLifetimeInst(nextcode::FixLifetimeInst *i) {
...
void IRGenSILFunction::visitMarkDependenceInst(nextcode::MarkDependenceInst *i) {
...
void IRGenSILFunction::visitStrongRetainInst(nextcode::StrongRetainInst *i) {
  Explosion lowered = getLoweredExplosion(i->getOperand());
...
void IRGenSILFunction::visitStrongReleaseInst(nextcode::StrongReleaseInst *i) {
  Explosion lowered = getLoweredExplosion(i->getOperand());
  auto &ti = cast<ReferenceTypeInfo>(getTypeInfo(i->getOperand()->getType()));
...
#define NEVER_LOADABLE_CHECKED_REF_STORAGE(Name, name, ...) \
  ...
  void IRGenSILFunction::visitStore##Name##Inst(nextcode::Store##Name##Inst *i) { \
    Explosion source = getLoweredExplosion(i->getSrc()); \
    Address dest = getLoweredAddress(i->getDest()); \
...
#define ALWAYS_LOADABLE_CHECKED_REF_STORAGE(Name, name, ...)                   \
  void IRGenSILFunction::visitStrongRetain##Name##Inst(                        \
      nextcode::StrongRetain##Name##Inst *i) {                                    \
    Explosion lowered = getLoweredExplosion(i->getOperand());                  \
  ...
  void IRGenSILFunction::visit##Name##RetainInst(nextcode::Name##RetainInst *i) { \
    Explosion lowered = getLoweredExplosion(i->getOperand());                  \
    auto &ti = getReferentTypeInfo(*this, i->getOperand()->getType());         \
  ...
  void IRGenSILFunction::visitStrongCopy##Name##ValueInst(                     \
      nextcode::StrongCopy##Name##ValueInst *i) {                                 \
    Explosion in = getLoweredExplosion(i->getOperand());                       \
    auto silTy = i->getOperand()->getType();                                   \
    auto ty = cast<Name##StorageType>(silTy.getASTType());                     \
    auto isOptional = bool(ty.getReferentType()->getOptionalObjectType());     \
...
void IRGenSILFunction::visitIsUniqueInst(nextcode::IsUniqueInst *i) {
...
void IRGenSILFunction::visitIsEscapingClosureInst(
    nextcode::IsEscapingClosureInst *i) {
...
void IRGenSILFunction::emitDebugInfoForAllocStack(AllocStackInst *i,
  ...
  if (auto *BitCast = dyn_cast<llvm::BitCastInst>(addr)) {
    ...
    if (auto *Alloca = dyn_cast<llvm::AllocaInst>(Op0))
      ...
      addr = call;
      bool isTaskAlloc = isCallToNeXTCodeTaskAlloc(call);
      assert(isTaskAlloc && "expecting call to nextcode_task_alloc");
      (void)isTaskAlloc;
    }
  }
  ...
  assert(isa<llvm::AllocaInst>(addr) || isa<llvm::UndefValue>(addr) ||
         isa<llvm::IntrinsicInst>(addr) || isCallToNeXTCodeTaskAlloc(addr));

  auto Indirection = DirectValue;
  if (InCoroContext(*CurSILFn, *i))
    Indirection =
        isCallToNeXTCodeTaskAlloc(addr) ? CoroIndirectValue : CoroDirectValue;
...
void IRGenSILFunction::visitAllocStackInst(nextcode::AllocStackInst *i) {
...
void IRGenSILFunction::visitAllocPackInst(nextcode::AllocPackInst *i) {
...
void IRGenSILFunction::visitAllocRefInst(nextcode::AllocRefInst *i) {
...
void IRGenSILFunction::visitAllocRefDynamicInst(nextcode::AllocRefDynamicInst *i) {
  int StackAllocSize = -1;
  if (i->canAllocOnStack()) {
...
void IRGenSILFunction::visitDeallocStackInst(nextcode::DeallocStackInst *i) {
...
void IRGenSILFunction::visitDeallocPackInst(nextcode::DeallocPackInst *i) {
...
void IRGenSILFunction::visitDeallocRefInst(nextcode::DeallocRefInst *i) {
  // Lower the operand.
  Explosion self = getLoweredExplosion(i->getOperand());
  auto selfValue = self.claimNext();
  SILValue op = i->getOperand();
...
void IRGenSILFunction::visitDeallocPartialRefInst(nextcode::DeallocPartialRefInst *i) {
...
void IRGenSILFunction::visitDeallocBoxInst(nextcode::DeallocBoxInst *i) {
...
void IRGenSILFunction::visitAllocBoxInst(nextcode::AllocBoxInst *i) {
...
void IRGenSILFunction::visitProjectBoxInst(nextcode::ProjectBoxInst *i) {
  auto boxTy = i->getOperand()->getType().castTo<SILBoxType>();
...
void IRGenSILFunction::visitConvertFunctionInst(nextcode::ConvertFunctionInst *i) {
...
void IRGenSILFunction::visitConvertEscapeToNoEscapeInst(
    nextcode::ConvertEscapeToNoEscapeInst *i) {
...
void IRGenSILFunction::visitAddressToPointerInst(nextcode::AddressToPointerInst *i)
{
  Explosion to;
...
// Ignores the isStrict flag because NeXTCode TBAA is not lowered into LLVM IR.
void IRGenSILFunction::visitPointerToAddressInst(nextcode::PointerToAddressInst *i)
{
...
void IRGenSILFunction::visitUncheckedRefCastInst(
                                             nextcode::UncheckedRefCastInst *i) {
  auto &ti = getTypeInfo(i->getType());
...
void IRGenSILFunction::
visitUncheckedRefCastAddrInst(nextcode::UncheckedRefCastAddrInst *i) {
  Address dest = getLoweredAddress(i->getDest());
...
void IRGenSILFunction::visitUncheckedAddrCastInst(
                                             nextcode::UncheckedAddrCastInst *i) {
...
void IRGenSILFunction::visitUncheckedTrivialBitCastInst(
                                      nextcode::UncheckedTrivialBitCastInst *i) {
  Explosion in = getLoweredExplosion(i->getOperand());
  Explosion out;
...
void IRGenSILFunction::
visitUncheckedBitwiseCastInst(nextcode::UncheckedBitwiseCastInst *i) {
  Explosion in = getLoweredExplosion(i->getOperand());
  Explosion out;
...
void IRGenSILFunction::visitRefToRawPointerInst(
                                             nextcode::RefToRawPointerInst *i) {
  auto &ti = getTypeInfo(i->getType());
  emitPointerCastInst(*this, i->getOperand(), i, ti);
}

void IRGenSILFunction::visitRawPointerToRefInst(nextcode::RawPointerToRefInst *i) {
  auto &ti = getTypeInfo(i->getType());
  emitPointerCastInst(*this, i->getOperand(), i, ti);
}
...
void IRGenSILFunction::visit##KIND##Inst(nextcode::KIND##Inst *i) {  \
...
void IRGenSILFunction::visitThinToThickFunctionInst(
                                            nextcode::ThinToThickFunctionInst *i) {
...
void IRGenSILFunction::visitThickToObjCMetatypeInst(ThickToObjCMetatypeInst *i){
  Explosion from = getLoweredExplosion(i->getOperand());
  llvm::Value *nextcodeMeta = from.claimNext();
  // Claim any conformances.
  (void)from.claimAll();
  CanType instanceType(i->getType().castTo<AnyMetatypeType>().getInstanceType());
  Explosion to;
  llvm::Value *classPtr =
    emitClassHeapMetadataRefForMetatype(*this, nextcodeMeta, instanceType);
...
void IRGenSILFunction::visitUnconditionalCheckedCastInst(
                                       nextcode::UnconditionalCheckedCastInst *i) {
...
void IRGenSILFunction::visitRefToBridgeObjectInst(
                                              nextcode::RefToBridgeObjectInst *i) {
...
void IRGenSILFunction::visitBridgeObjectToRefInst(
                                              nextcode::BridgeObjectToRefInst *i) {
  Explosion boEx = getLoweredExplosion(i->getOperand());
...
void IRGenSILFunction::visitBridgeObjectToWordInst(
                                             nextcode::BridgeObjectToWordInst *i) {
  Explosion boEx = getLoweredExplosion(i->getOperand());
...
void IRGenSILFunction::visitUnconditionalCheckedCastAddrInst(
                                   nextcode::UnconditionalCheckedCastAddrInst *i) {
  Address dest = getLoweredAddress(i->getDest());
  Address src = getLoweredAddress(i->getSrc());
  emitCheckedCast(*this,
                  src, i->getSourceFormalType(),
...
void IRGenSILFunction::visitCheckedCastBranchInst(
                                              nextcode::CheckedCastBranchInst *i) {
...
void IRGenSILFunction::visitCheckedCastAddrBranchInst(
                                          nextcode::CheckedCastAddrBranchInst *i) {
  Address dest = getLoweredAddress(i->getDest());
  Address src = getLoweredAddress(i->getSrc());
...
void IRGenSILFunction::visitKeyPathInst(nextcode::KeyPathInst *I) {
...
void IRGenSILFunction::visitUpcastInst(nextcode::UpcastInst *i) {
...
void IRGenSILFunction::visitIndexAddrInst(nextcode::IndexAddrInst *i) {
...
void IRGenSILFunction::visitTailAddrInst(nextcode::TailAddrInst *i) {
  Address base = getLoweredAddress(i->getBase());
  Explosion indexValues = getLoweredExplosion(i->getIndex());
  llvm::Value *index = indexValues.claimNext();
...
void IRGenSILFunction::visitIndexRawPointerInst(nextcode::IndexRawPointerInst *i) {
...
void IRGenSILFunction::visitInitExistentialAddrInst(nextcode::InitExistentialAddrInst *i) {
...
void IRGenSILFunction::visitInitExistentialValueInst(
    nextcode::InitExistentialValueInst *i) {
...
void IRGenSILFunction::visitDeinitExistentialAddrInst(
                                              nextcode::DeinitExistentialAddrInst *i) {
  Address container = getLoweredAddress(i->getOperand());
...
void IRGenSILFunction::visitDeinitExistentialValueInst(
    nextcode::DeinitExistentialValueInst *i) {
  llvm_unreachable("unsupported instruction during IRGen");
}
...
void IRGenSILFunction::visitOpenPackElementInst(nextcode::OpenPackElementInst *i) {
...
void IRGenSILFunction::visitWitnessMethodInst(nextcode::WitnessMethodInst *i) {
  ...
  if (IGM.isResilient(conformance.getRequirement(), ResilienceExpansion::Maximal)) {
    ...
    shouldUseDispatchThunk = protoDecl->getModuleContext() != IGM.getNeXTCodeModule();
  }
...
void IRGenSILFunction::visitCopyAddrInst(nextcode::CopyAddrInst *i) {
...
void IRGenSILFunction::visitExplicitCopyAddrInst(
    nextcode::ExplicitCopyAddrInst *i) {
  SILType addrTy = i->getSrc()->getType();
  const TypeInfo &addrTI = getTypeInfo(addrTy);
  Address src = getLoweredAddress(i->getSrc());
  // See whether we have a deferred fixed-size buffer initialization.
...
// bind_memory and rebind_memory are no-ops because NeXTCode TBAA info is not
// lowered to LLVM IR TBAA, and the output token is ignored except for
// verification.
void IRGenSILFunction::visitBindMemoryInst(nextcode::BindMemoryInst *i) {
  LoweredValue &token = getUndefLoweredValue(i->getType());
  setLoweredValue(i, std::move(token));
}

void IRGenSILFunction::visitRebindMemoryInst(nextcode::RebindMemoryInst *i) {
  LoweredValue &token = getUndefLoweredValue(i->getType());
  setLoweredValue(i, std::move(token));
}

void IRGenSILFunction::visitDestroyAddrInst(nextcode::DestroyAddrInst *i) {
...
void IRGenSILFunction::visitCondFailInst(nextcode::CondFailInst *i) {
...
void IRGenSILFunction::visitIncrementProfilerCounterInst(
  ...
  // If we import profiling intrinsics from a nextcode module but profiling is
...
void IRGenSILFunction::visitSuperMethodInst(nextcode::SuperMethodInst *i) {
  ...
  if (IGM.hasResilientMetadata(classDecl, ResilienceExpansion::Maximal)) {
    ...
    auto &schema = methodType->isAsync()
                       ? getOptions().PointerAuth.AsyncNeXTCodeClassMethodPointers
                       : getOptions().PointerAuth.NeXTCodeClassMethodPointers;
    auto authInfo =
...
void IRGenSILFunction::visitObjCSuperMethodInst(nextcode::ObjCSuperMethodInst *i) {
  assert(i->getMember().isForeign);
...
void IRGenSILFunction::visitClassMethodInst(nextcode::ClassMethodInst *i) {
  assert(!i->getMember().isForeign);
  ...
  if (IGM.hasResilientMetadata(classDecl, ResilienceExpansion::Maximal)) {
    ...
    shouldUseDispatchThunk =
        classDecl->getModuleContext() != IGM.getNeXTCodeModule();
  }
  ...
  // For NeXTCode classes, get the method implementation from the vtable.
...
void IRGenSILFunction::visitObjCMethodInst(nextcode::ObjCMethodInst *i) {
