...
static void updateProtocolRefs(IRGenModule &IGM,
  ...
  auto &astContext = IGM.getNeXTCodeModule()->getASTContext();
  auto *clangImporter =
  ...
  for (auto inheritedObjCProtocol : inheritedObjCProtocols) {
    ...
    // Map the objc protocol to nextcode protocol.
    ...
    auto inheritedNeXTCodeProtocol = cast<ProtocolDecl>(*optionalDecl);
    // Get the objc protocol record we use in NeXTCode.
    auto record = IGM.getAddrOfObjCProtocolRecord(inheritedNeXTCodeProtocol,
                                                  NotForDefinition);
...
/// an Objective-C method for a NeXTCode method implementation.
static llvm::Constant *findNeXTCodeAsObjCThunk(IRGenModule &IGM, SILDeclRef ref,
                                            SILFunction *&SILFn) {
  SILFn = IGM.getSILModule().lookUpFunction(ref);
  assert(SILFn && "no IR function for nextcode-as-objc thunk");
...
static llvm::Constant *getObjCGetterPointer(IRGenModule &IGM,
  ...
  return findNeXTCodeAsObjCThunk(IGM, getter, silFn);
}
...
static llvm::Constant *getObjCSetterPointer(IRGenModule &IGM,
  ...
  SILDeclRef setter = SILDeclRef(property->getOpaqueAccessor(AccessorKind::Set),
                                 SILDeclRef::Kind::Func)
    .asForeign();
  return findNeXTCodeAsObjCThunk(IGM, setter, silFn);
}
...
static llvm::Constant *getObjCMethodPointer(IRGenModule &IGM,
  ...
  return findNeXTCodeAsObjCThunk(IGM, declRef, silFn);
}
