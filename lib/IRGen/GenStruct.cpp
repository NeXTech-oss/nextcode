//===--- GenStruct.cpp - NeXTCode IR Generation For 'struct' Types -----------===//
//
/*
...
namespace {
  ...
  public:
    ClangFieldInfo(VarDecl *nextcodeField, const ElementLayout &layout,
                   const TypeInfo &typeInfo)
        : RecordField(typeInfo), Field(nextcodeField) {
      completeFrom(layout);
    }
    ...
    SILType getType(IRGenModule &IGM, SILType T) const {
      ...
      // The NeXTCode-field-less cases use opaque storage, which is
  ...
  public:
    ...
    *buildTypeLayoutEntry(IRGenModule &IGM,
    ...
    void addToAggLowering(IRGenModule &IGM, NeXTCodeAggLowering &lowering,
  ...
  class AddressOnlyCXXClangRecordTypeInfo final
    ...
    CanSILFunctionType createCXXCopyConstructorFunctionType(IRGenFunction &IGF,
      ...
      auto subst = SubstitutionMap::get(sig, {T.getASTType()},
                              LookUpConformanceInModule{IGF.getNeXTCodeModule()});
    ...
    void emitCopyWithCopyConstructor(
      ...
      std::string name = "__nextcode_cxx_copy_ctor" + callee->getName().str();
  ...
  public:
    ...
    void destroy(IRGenFunction &IGF, Address address, SILType T,
      ...
      // `emitCXXDestructorCall` so, just let NeXTCode handle this destructor.
...
public:
  ClangRecordLowering(IRGenModule &IGM, StructDecl *nextcodeDecl,
                      const clang::RecordDecl *clangDecl,
                      SILType nextcodeType)
    : IGM(IGM), NeXTCodeDecl(nextcodeDecl), NeXTCodeType(nextcodeType),
  ...
  const TypeInfo *createTypeInfo(llvm::StructType *llvmType) {
    ...
    if (NeXTCodeType.getStructOrBoundGenericStruct()->isCxxNonTrivial()) {
      return AddressOnlyCXXClangRecordTypeInfo::create(
          FieldInfos, llvmType, TotalStride, TotalAlignment, ClangDecl);
    }
    if (NeXTCodeType.getStructOrBoundGenericStruct()->isNonTrivialPtrAuth()) {
...
private:
  ...
  static bool isImportOfClangField(VarDecl *nextcodeField,
                                   const clang::FieldDecl *clangField) {
    assert(nextcodeField->hasClangNode());
    return (nextcodeField->getClangNode().castAsDecl() == clangField);
  }
  ...
  void collectStructFields() {
    ...
    auto nextcodeProperties = NeXTCodeDecl->getStoredProperties();
    auto sfi = nextcodeProperties.begin(), sfe = nextcodeProperties.end();
    ...
    while (cfi != cfe) {
      ...
      VarDecl *nextcodeField;
      if (sfi != sfe) {
        nextcodeField = *sfi;
        if (isImportOfClangField(nextcodeField, clangField)) {
          ++sfi;
        } else {
          nextcodeField = nullptr;
        }
      } else {
        nextcodeField = nullptr;
      }
      ...
      addStructField(clangField, nextcodeField);
    }

    assert(sfi == sfe && "more NeXTCode fields than there were Clang fields?");
  ...
  void addStructField(const clang::FieldDecl *clangField,
    ...
    // If we have a NeXTCode import of this type, use our lowered information.
    if (nextcodeField) {
      auto &fieldTI = cast<FixedTypeInfo>(IGM.getTypeInfo(
          NeXTCodeType.getFieldType(nextcodeField, IGM.getSILModule(),
                                 IGM.getMaximalTypeExpansionContext())));
      addField(nextcodeField, offset, fieldTI);
      return;
    }
  ...
  /// Add storage for an (optional) NeXTCode field at the given offset.
  void addField(VarDecl *nextcodeField, Size offset,
    ...
    addFieldInfo(nextcodeField, fieldType);
  }

  /// Add information to track a value field at the current offset.
  void addFieldInfo(VarDecl *nextcodeField, const FixedTypeInfo &fieldType) {
    ...
    if (isLoadableField)
      FieldInfos.push_back(
          ClangFieldInfo(nextcodeField, layout, explosionBegin, explosionEnd));
    else
      FieldInfos.push_back(ClangFieldInfo(nextcodeField, layout, fieldType));
...
const TypeInfo *TypeConverter::convertStructType(TypeBase *key, CanType type,
  ...
  if (IGM.isResilient(D, ResilienceExpansion::Maximal)
    ...
    auto *bitwiseCopyableProtocol =
        IGM.getNeXTCodeModule()->getASTContext().getProtocol(
            KnownProtocolKind::BitwiseCopyable);
    if (bitwiseCopyableProtocol &&
        IGM.getNeXTCodeModule()->checkConformance(type, bitwiseCopyableProtocol)) {
  ...
  if (D->hasClangNode()) {
    ...
    assert(clangDecl && "NeXTCode struct from an imported C macro?");
    ...
    if (auto clangRecord = dyn_cast<clang::RecordDecl>(clangDecl)) {
      ...
      // Fall back to NeXTCode lowering for the enum's representation as a struct.
      assert(D->getStoredProperties().size() == 1 &&
             "Struct representation of a Clang enum should wrap one value");
    } else if (clangDecl->hasAttr<clang::NeXTCodeNewTypeAttr>()) {
      // Fall back to NeXTCode lowering for the underlying type's
      // representation as a struct member.
      assert(D->getStoredProperties().size() == 1 &&
             "Struct representation of a nextcode_newtype should wrap one value");
    } else {
      llvm_unreachable("NeXTCode struct represents unexpected imported type");
    }
  }
