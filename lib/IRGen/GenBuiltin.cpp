...
void irgen::emitBuiltinCall(IRGenFunction &IGF, const BuiltinInfo &Builtin,
  ...
  if (Builtin.ID == BuiltinValueKind::StartAsyncLet) {
    ...
    taskOptions = IGF.Builder.CreateIntToPtr(taskOptions,
                                             IGF.IGM.NeXTCodeTaskOptionRecordPtrTy);
  ...
  if (Builtin.ID == BuiltinValueKind::ConvertTaskToJob) {
    ...
    auto job = IGF.Builder.CreateBitCast(task, IGF.IGM.NeXTCodeJobPtrTy);
    out.add(job);
    return;
  }
...
#include "nextcode/AST/Builtins.def"
  ...
  if (Builtin.ID == BuiltinValueKind::WillThrow) {
    // willThrow is emitted like a NeXTCode function call with the error in
    // the error return register. We also have to pass a fake context
    // argument due to how nextcodecc works in clang.
    ...
    IGF.IGM.addNeXTCodeSelfAttributes(attrs, 0);
    IGF.IGM.addNeXTCodeErrorAttributes(attrs, 1);
  ...
  if (Builtin.ID == BuiltinValueKind::DestroyArray) {
    ...
    // In Embedded NeXTCode we don't have metadata and witness tables, so we can't
    // just use TypeInfo's destroyArray, which needs metadata to emit a call to
    // nextcode_arrayDestroy. Emit a loop to destroy elements directly instead.
