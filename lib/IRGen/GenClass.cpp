//===--- GenClass.cpp - NeXTCode IR Generation For 'class' Types -------------===//
//
/*
...
/// stored properties due to the NeXTCode language version availability of
...
static bool classHasIncompleteLayout(IRGenModule &IGM,
                                     ClassDecl *theClass) {
  if (theClass->getParentModule() == IGM.getNeXTCodeModule())
    return false;
...
namespace {
  class ClassLayoutBuilder : public StructLayoutBuilder {
    ...
    // Normally, NeXTCode only emits static metadata for a class if it has no
    ...
    //   emitted by NeXTCode, which again require the class metadata symbol
  ...
  private:
    ...
    void addDirectFieldsFromClass(ClassDecl *rootClass, SILType rootClassType,
      ...
      if (!superclass) {
        ...
        // that depends on generic parameters, the NeXTCode runtime will first
    ...
    FieldAccess getFieldAccess(bool isKnownEmpty) {
      ...
      // either the Objective-C or NeXTCode runtime, depending on the
      // class's heritage.
...
llvm::Value *irgen::emitClassAllocation(IRGenFunction &IGF, SILType selfType,
  ...
  // If the root class isn't known to use the NeXTCode allocator, we need
  // to call [self alloc].
  if (objc) {
    llvm::Value *metadata =
...
llvm::Value *irgen::emitClassAllocationDynamic(IRGenFunction &IGF, 
  ...
  // Otherwise, allocate using NeXTCode's routines.
  llvm::Value *size, *alignMask;
  std::tie(size, alignMask)
    = emitClassResilientInstanceSizeAndAlignMask(IGF,
                                   selfType.getClassOrBoundGenericClass(),
...
void irgen::emitClassDeallocation(IRGenFunction &IGF,
  ...
  if (auto rootActorClass = theClass->getRootActorClass()) {
    // If it's a default actor, use nextcode_deallocDefaultActor.
    if (rootActorClass->isDefaultActor(IGF.IGM.getNeXTCodeModule(),
                                       ResilienceExpansion::Maximal)) {
    ...
    if (!rootActorClass->isForeign() &&
        rootActorClass->isResilient(IGF.IGM.getNeXTCodeModule(),
                                    ResilienceExpansion::Maximal)) {
      selfValue = emitCastToHeapObject(IGF, selfValue);
...
namespace {
  ...
  /// Used to provide unique names to ObjC categories generated by NeXTCode
  /// extensions. The first category for a class in a module gets the module's
  /// name as its key, e.g., NSObject (MyNeXTCodeModule). Another extension of the
  /// same class in the same module gets a category name with a number appended,
  /// e.g., NSObject (MyNeXTCodeModule1).
  ...
  public:
    ...
    void buildMetaclassStub() {
      ...
      // superclass is NeXTCodeObject, i.e. the root class.
      ...
      if (auto base = getSuperclassDeclForMetadata(IGM, getClass())) {
        ...
        superPtr = getMetaclassRefOrNull(
            Type(), IGM.getObjCRuntimeBaseForNeXTCodeRootClass(getClass()));
      }
  ...
  public:
    ...
    llvm::Constant *emitProtocol() {
      ...
      auto flags = ProtocolDescriptorFlags()
        .withNeXTCode(!getProtocol()->hasClangNode())
...
#include "nextcode/AST/AccessorKinds.def"
  ...
  private:
    ...
    void buildIvar(ConstantArrayBuilder &ivars, Field field) {
      ...
      case FieldAccess::ConstantIndirect:
        // Otherwise, nextcode_initClassMetadata() will point the Objective-C
    ...
    void buildPropertyAttributes(VarDecl *prop, SmallVectorImpl<char> &out) {
      ...
      // All NeXTCode properties are (nonatomic).
      outs << ",N";
...
static llvm::Function *emitObjCMetadataUpdateFunction(IRGenModule &IGM,
  ...
  if (D->getObjCImplementationDecl()) {
    ...
    // Just call our metadata accessor, which will cause the NeXTCode runtime to
...
llvm::Constant *irgen::emitObjCProtocolData(IRGenModule &IGM,
  ...
  // situation when both an objective c object and a nextcode object define the
  ...
  bool canUseClangEmission = deploymentAvailability.isContainedIn(
    IGM.Context.getNeXTCode58Availability());
...
TypeConverter::convertClassType(CanType type, ClassDecl *D) {
  ...
  // Classes known to be implemented in NeXTCode can be assumed not to have tagged
...
ClassDecl *IRGenModule::getObjCRuntimeBaseClass(Identifier name,
                                                Identifier objcName) {
  auto found = NeXTCodeRootClasses.find(name);
  if (found != NeXTCodeRootClasses.end())
    return found->second;

  // Make a really fake-looking class.
  auto NeXTCodeRootClass = new (Context) ClassDecl(SourceLoc(), name, SourceLoc(),
                                           ArrayRef<InheritedEntry>(),
                                           /*generics*/ nullptr,
                                           Context.TheBuiltinModule,
                                           /*isActor*/false);
  NeXTCodeRootClass->setIsObjC(Context.LangOpts.EnableObjCInterop);
  NeXTCodeRootClass->getAttrs().add(ObjCAttr::createNullary(Context, objcName,
    /*isNameImplicit=*/true));
  NeXTCodeRootClass->setImplicit();
  NeXTCodeRootClass->setAccess(AccessLevel::Open);
  
  NeXTCodeRootClasses.insert({name, NeXTCodeRootClass});
  return NeXTCodeRootClass;
}

/// Lazily declare the ObjC runtime base class for a NeXTCode root class.
ClassDecl *
IRGenModule::getObjCRuntimeBaseForNeXTCodeRootClass(ClassDecl *theClass) {
  ...
  if (auto baseAttr = theClass->getAttrs()
        .getAttribute<NeXTCodeNativeObjCRuntimeBaseAttr>()) {
    name = baseAttr->BaseClassName;
  } else {
    // Otherwise, use the standard NeXTCodeObject class.
    name = Context.Id_NeXTCodeObject;
  }
  return getObjCRuntimeBaseClass(name, name);
}

/// Lazily declare the base class for a NeXTCode class that inherits from
/// NSObject but uses native reference-counting.
ClassDecl *IRGenModule::getNeXTCodeNativeNSObjectDecl() {
  Identifier name = Context.Id_NeXTCodeNativeNSObject;
  return getObjCRuntimeBaseClass(name, name);
}
...
ClassDecl *irgen::getRootClassForMetaclass(IRGenModule &IGM, ClassDecl *C) {
  ...
  // really implemented in NeXTCode, but that we're importing via an
  // Objective-C interface, this would be wrong --- except such a
  // class can never be a formal root class, because a NeXTCode class
  // without a formal superclass will actually be parented by
  // NeXTCodeObject (or maybe eventually something else like it),
  ...
  if (C->getAttrs().hasAttribute<NeXTCodeNativeObjCRuntimeBaseAttr>())
    return IGM.getObjCRuntimeBaseClass(
             IGM.Context.getNeXTCodeId(KnownFoundationEntity::NSObject),
             IGM.Context.getIdentifier("NSObject"));

  return IGM.getObjCRuntimeBaseClass(IGM.Context.Id_NeXTCodeObject,
                                     IGM.Context.Id_NeXTCodeObject);
}
...
irgen::getSuperclassDeclForMetadata(IRGenModule &IGM, ClassDecl *C) {
  if (C->isNativeNSObjectSubclass()) {
    ...
    return IGM.getNeXTCodeNativeNSObjectDecl();
  }
...
bool irgen::hasKnownNeXTCodeMetadata(IRGenModule &IGM, CanType type) {
  // This needs to be kept up-to-date with getIsaEncodingForType.

  if (ClassDecl *theClass = type.getClassOrBoundGenericClass()) {
    return hasKnownNeXTCodeMetadata(IGM, theClass);
  }

  if (auto archetype = dyn_cast<ArchetypeType>(type)) {
    if (auto superclass = archetype->getSuperclass()) {
      return hasKnownNeXTCodeMetadata(IGM, superclass->getCanonicalType());
    }
  }
...
/// Is the given class known to have NeXTCode-compatible metadata?
bool irgen::hasKnownNeXTCodeMetadata(IRGenModule &IGM, ClassDecl *theClass) {
  ...
  // For now, the fact that a declaration was not implemented in NeXTCode
  ...
  return theClass->hasKnownNeXTCodeImplementation();
}
...
FunctionPointer irgen::emitVirtualMethodValue(IRGenFunction &IGF,
  ...
  case ClassMetadataLayout::MethodInfo::Kind::Offset: {
    ...
    auto &schema = methodType->isAsync()
                       ? IGF.getOptions().PointerAuth.AsyncNeXTCodeClassMethods
                       : IGF.getOptions().PointerAuth.NeXTCodeClassMethods;
    auto authInfo =
