...
public:
  ...
  getTypeMetadataAccessFunctionSignature() {
    auto &ctx = IGM.getNeXTCodeModule()->getASTContext();
  ...
  getFunctionLoweredSignature(AbstractFunctionDecl *fd) {
    ...
    auto function = Lowering::SILGenModule(*silMod, declRef.getModuleContext())
                        .getFunction(declRef, nextcode::NotForDefinition);
    ...
    if (silFuncType->isAsync())
      return std::nullopt;
    if (silFuncType->getLanguage() != SILFunctionLanguage::NeXTCode)
      return std::nullopt;
  ...
  getMethodPointerAuthInfo(const AbstractFunctionDecl *funcDecl,
    ...
    if (funcDecl->hasAsync())
      return std::nullopt;
    const auto &schema = IGM.getOptions().PointerAuth.NeXTCodeClassMethods;
    if (!schema)
      return std::nullopt;
    auto discriminator =
  ...
  schema.enumerateComponents(
      [&](clang::CharUnits offset, clang::CharUnits end, llvm::Type *type) {
        auto primitiveType = getPrimitiveTypeFromLLVMType(
            owner.IGM.getNeXTCodeModule()->getASTContext(), type);
        if (!primitiveType) {
          hasError = true;
          return;
        }
