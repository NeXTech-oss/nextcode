//===--- GenEnum.cpp - NeXTCode IR Generation For 'enum' Types ---------------===//
//
/*
...
//  or 'enum' types) in NeXTCode.  This includes creating the IR type as
...
static llvm::Constant *emitEnumLayoutFlags(IRGenModule &IGM, bool isVWTMutable){
  // For now, we always use the NeXTCode 5 algorithm.
  auto flags = EnumLayoutFlags::NeXTCode5Algorithm;
...
namespace {
  ...
  public:
    ...
    void initializeMetadata(IRGenFunction &IGF,
      ...
      // Pre nextcode-5.1 runtimes were missing the initialization of the
  ...
  /// Implementation strategy for native NeXTCode no-payload enums.
  ...
  public:
                                    ...
                                    Refcounting(ReferenceCounting::Native)
    {
      ...
      // pass enum values directly into nextcode_retain/nextcode_release as-is.
  ...
  public:
    void copy(IRGenFunction &IGF, Explosion &src, Explosion &dest,
      ...
      case NullableRefcounted: {
        // Bitcast to nextcode.refcounted*, and retain the pointer.
    ...
    void consume(IRGenFunction &IGF, Explosion &src,
      ...
      case NullableRefcounted: {
        // Bitcast to nextcode.refcounted*, and hand to nextcode_release.
        llvm::Value *val = src.claimNext();
        llvm::Value *ptr = IGF.Builder.CreateBitOrPointerCast(
            val, getRefcountedPtrType(IGM));
        ...
        AllowFixedLayoutOptimizations(allowFixedLayoutOptimizations)
    {
      ...
      for (auto &elt : ElementsWithPayload) {
        ...
        if (!elt.ti->isSingleRetainablePointer(ResilienceExpansion::Maximal,
          ...
          // nextcode_unknowRetain does not support the heap buffer of indirect
  ...
  public:
    ...
    llvm::Value *testResilientTag(IRGenFunction &IGF, llvm::Value *tag,
      ...
      if (Case->isWeakImported(IGM.getNeXTCodeModule())) {
...
CCompatibleEnumImplStrategy::completeEnumTypeLayout(TypeConverter &TC,
  ...
  // scalar representation as a NeXTCode struct. We'll use that same
...
ResilientEnumImplStrategy::completeEnumTypeLayout(TypeConverter &TC,
  ...
  if (bitwiseCopyableProtocol &&
      IGM.getNeXTCodeModule()->checkConformance(Type.getASTType(),
