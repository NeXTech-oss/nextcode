...
#ifndef NEXTCODE_IRGEN_STRUCTLAYOUT_H
#define NEXTCODE_IRGEN_STRUCTLAYOUT_H
...
private:
  ...
  /// The nextcode type information for this element's layout.
  const TypeInfo *Type;
...
public:
  ...
  /// Add a nextcode heap header to the layout.  This must be the first
  ...
  /// thing added to the layout, following the NeXTCode heap header.
  void addDefaultActorHeader(ElementLayout &elt);
  /// Add the non-default distributed actor header to the layout.
  /// This must be the second thing added to the layout, following the NeXTCode heap header.
