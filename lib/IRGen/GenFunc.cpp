//===--- GenFunc.cpp - NeXTCode IR Generation for Function Types -------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements IR generation for function types in NeXTCode.  This
//  includes creating the IR type as well as capturing variables and
//  performing calls.
//
//  NeXTCode supports three representations of functions:
//
...
IRGenModule::getForeignFunctionInfo(CanSILFunctionType type) {
  if (type->getLanguage() == SILFunctionLanguage::NeXTCode)
...
/// NeXTCode-refcountable type that is being used directly as the
/// context object.
static llvm::Value *emitPartialApplicationForwarder(
  ...
  if (!layout && hasPolymorphicParams) {
    ...
    if (!calleeHasContext) {
      ...
      Explosion param;
      auto ref = rawData;
      // We can get a '{ nextcode.refcounted* }' type for AnyObject on linux.
      if (!ti.getStorageType()->isPointerTy() &&
          ti.isSingleNeXTCodeRetainablePointer(ResilienceExpansion::Maximal))
  ...
  // If there's a data pointer required, but it's a nextcode-retainable
  ...
  if (!layout) {
    ...
    if (isIndirectFormalParameter(argConvention)) {
      // We can use rawData's type for the alloca because it is a nextcode
...
std::optional<StackAddress> irgen::emitFunctionPartialApplication(
    ...
    bool isOutlined) {
  // If we have a single NeXTCode-refcounted context value, we can adopt it
  // directly as our closure context without creating a box and thunk.
  enum HasSingleNeXTCodeRefcountedContext { Maybe, Yes, No, Thunkable }
    hasSingleNeXTCodeRefcountedContext = Maybe;
  ...
  auto addParam = [&](SILParameterInfo param) {
    ...
    // Update the single-nextcode-refcounted check, unless we already ruled that
    // out.
    if (hasSingleNeXTCodeRefcountedContext == No)
      return;
    
    
    // Adding nonempty values when we already have a single refcounted pointer
    // means we don't have a single value anymore.
    if (hasSingleNeXTCodeRefcountedContext != Maybe) {
      hasSingleNeXTCodeRefcountedContext = No;
      return;
    }
      
    if (ti.isSingleNeXTCodeRetainablePointer(ResilienceExpansion::Maximal)) {
      hasSingleNeXTCodeRefcountedContext = Yes;
  ...
  //
  // 'nextcodeerror' fixes this physically, but there's still a risk of
  // miscompiles because the LLVM optimizer may forward arguments
  // positionally without considering 'nextcodeerror'.
  //
  ...
  // disappear.
  if (hasSingleNeXTCodeRefcountedContext == Yes &&
      origType->hasErrorResult()) {
    hasSingleNeXTCodeRefcountedContext = Thunkable;
  }
  ...
  // context.
  // TODO: We can only do this and use nextcodeself if all our nextcodecc emit the
  // last parameter that fits into a register as nextcodeself.
  // We should get this optimization back using the @convention(closure) whose
  // box argument should just be nextcode self.
  ...
  // If we only need to capture a single NeXTCode-refcounted object, we
...
llvm::Function *IRGenFunction::getOrCreateResumePrjFn(bool forPrologue) {
  ...
  auto name = forPrologue ? "__nextcode_async_resume_project_context_prologue"
                          : "__nextcode_async_resume_project_context";
...
IRGenFunction::createAsyncDispatchFn(const FunctionPointer &fnPtr,
  ...
  dispatch->setCallingConv(IGM.NeXTCodeAsyncCC);
...
void IRGenFunction::emitSuspensionPoint(Explosion &toExecutor,
  ...
  // Setup the suspend point.
  SmallVector<llvm::Value *, 8> arguments;
  unsigned nextcodeAsyncContextIndex = 0;
  arguments.push_back(IGM.getInt32(nextcodeAsyncContextIndex)); // context index
  ...
  auto resultTy = llvm::StructType::get(IGM.getLLVMContext(), {IGM.Int8PtrTy},
                                        false /*packed*/);
  emitSuspendAsyncCall(nextcodeAsyncContextIndex, resultTy, arguments);
}

llvm::Function *IRGenFunction::getOrCreateResumeFromSuspensionFn() {
  auto name = "__nextcode_async_resume_get_context";
  auto fn = cast<llvm::Function>(IGM.getOrCreateHelperFunction(
      name, IGM.Int8PtrTy, {IGM.Int8PtrTy},
      [&](IRGenFunction &IGF) {
        auto &Builder = IGF.Builder;
...
llvm::Function *IRGenFunction::createAsyncSuspendFn() {
  ...
  llvm::Function *suspendFn =
      llvm::Function::Create(suspendFnTy, llvm::Function::InternalLinkage,
                             name, &IGM.Module);
  suspendFn->setCallingConv(IGM.NeXTCodeAsyncCC);
  ...
  suspendCall->setCallingConv(IGM.NeXTCodeAsyncCC);
  ...
  IGM.addNeXTCodeAsyncContextAttributes(attrs, /*context arg index*/ 0);
