//===--- GenType.cpp - NeXTCode IR Generation For Types ----------------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements IR generation for types in NeXTCode.
//
//===----------------------------------------------------------------------===//
...
namespace {
  ...
  /// A TypeInfo implementation for opaque storage. NeXTCode will preserve any
...
// The following Apple platforms support backward deployment of NeXTCode
// code built with NeXTCode 5.0 running on an old Objective-C runtime
...
TypeConverter::TypeConverter(IRGenModule &IGM)
  ...
  // metadata update callback when realizing a NeXTCode class referenced from
  // Objective-C.
  ...
  auto fs =
      IGM.getNeXTCodeModule()->getASTContext().SourceMgr.getFileSystem();
  if (path.empty()) {
    ...
    auto archName = nextcode::getMajorArchitectureName(Triple);
...
const TypeInfo &IRGenModule::getNeXTCodeContextPtrTypeInfo() {
  return Types.getNeXTCodeContextPtrTypeInfo();
}

const TypeInfo &TypeConverter::getNeXTCodeContextPtrTypeInfo() {
  if (NeXTCodeContextPtrTI) return *NeXTCodeContextPtrTI;
  NeXTCodeContextPtrTI = createUnmanagedStorageType(IGM.NeXTCodeContextPtrTy,
                                                 ReferenceCounting::Unknown,
                                                 /*isOptional*/false);
  NeXTCodeContextPtrTI->NextConverted = FirstType;
  FirstType = NeXTCodeContextPtrTI;
  return *NeXTCodeContextPtrTI;
}
...
const LoadableTypeInfo &TypeConverter::getRawUnsafeContinuationTypeInfo() {
  ...
  // they can be used directly as NeXTCode function contexts.  Preserve this
...
#include "nextcode/AST/TypeNodes.def"
  ...
  case TypeKind::BuiltinNonDefaultDistributedActorStorage: {
    ...
    auto ty = llvm::StructType::create(IGM.getLLVMContext(),
                                 llvm::ArrayType::get(IGM.Int8PtrTy, numWords),
                                       "nextcode.nondefaultdistributedactor");
    auto size = IGM.getPointerSize() * numWords;
    auto align = Alignment(2 * IGM.getPointerAlignment().getValue());
    auto spareBits = SpareBitVector::getConstant(size.getValueInBits(), false);
    return new PrimitiveTypeInfo(ty, size, std::move(spareBits), align);
...
/// structural in the nextcode type system, but LLVM's type system
  ...
  if (auto structDecl = t.getStructOrBoundGenericStruct()) {
    ...
    if (allFields.size() == 1) {
      auto fieldTy = t.getFieldType(
          ...
          TypeExpansionContext(expansion, IGM.getNeXTCodeModule(),
...
static bool tryEmitDeinitCall(IRGenFunction &IGF,
  ...
  auto substitutions = ty->getContextSubstitutionMap(IGF.getNeXTCodeModule(),
                                                     nominal);
