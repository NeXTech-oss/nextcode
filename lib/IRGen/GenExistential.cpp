//===--- GenExistential.cpp - NeXTCode IR Generation for Existential Types ---===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements IR generation for existential types in NeXTCode.
//
//===----------------------------------------------------------------------===//
...
static void forEachProtocolWitnessTable(
  ...
  // Don't emit witness tables in embedded NeXTCode.
...
static llvm::Function *getAllocateBoxedOpaqueExistentialBufferFunction(
  ...
  // __nextcode_allocate_boxed_opaque_existential__N is the well-known function for
  ...
  llvm::raw_svector_ostream(fnName)
      << "__nextcode_allocate_boxed_opaque_existential_"
...
static llvm::Function *getDeallocateBoxedOpaqueExistentialBufferFunction(
  ...
  // __nextcode_deallocate_boxed_opaque_existential_N is the well-known function
  // for deallocating buffers in existential containers of types with N witness
  // tables.
  llvm::SmallString<40> fnName;
  llvm::raw_svector_ostream(fnName)
      << "__nextcode_deallocate_boxed_opaque_existential_"
      << existLayout.getNumTables();
...
getProjectBoxedOpaqueExistentialFunction(IRGenFunction &IGF,
  ...
  // __nextcode_project_boxed_opaque_existential_N is the well-known function for
  // projecting buffers in existential containers of types with N witness
  // tables.
  llvm::SmallString<40> fnName;
  llvm::raw_svector_ostream(fnName)
      << (accessKind == OpenedExistentialAccess::Immutable
              ? "__nextcode_project_boxed_opaque_existential_"
              : "__nextcode_mutable_project_boxed_opaque_existential_")
      << existLayout.getNumTables();
...
static llvm::Function *getAssignBoxedOpaqueExistentialBufferFunction(
  ...
  // __nextcode_assign_box_in_existentials_N is the well-known function for
  // assigning buffers in existential containers of types with N witness
  // tables.
  llvm::SmallString<40> fnName;
  llvm::raw_svector_ostream(fnName)
      << "__nextcode_assign_boxed_opaque_existential_"
      << existLayout.getNumTables();
...
static llvm::Function *getDestroyBoxedOpaqueExistentialBufferFunction(
  ...
  llvm::SmallString<40> fnName;
  llvm::raw_svector_ostream(fnName)
      << "__nextcode_destroy_boxed_opaque_existential_"
      << existLayout.getNumTables();
