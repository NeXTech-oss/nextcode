//===--- GenCall.cpp - NeXTCode IR Generation for Function Calls -------------===//
//
/*
...
//  in NeXTCode.  This includes creating the IR type, collecting IR attributes,
...
AsyncContextLayout irgen::getAsyncContextLayout(
  ...
  // AsyncContext * __ptrauth_nextcode_async_context_parent Parent;
  {
    auto ty = SILType();
    auto &ti = IGM.getNeXTCodeContextPtrTypeInfo();
    valTypes.push_back(ty);
    typeInfos.push_back(&ti);
  }

  // TaskContinuationFunction * __ptrauth_nextcode_async_context_resume
  //     ResumeParent;
  {
    auto ty = SILType();
...
llvm::Value *IRGenFunction::getAsyncTask() {
  ...
  call->setCallingConv(IGM.NeXTCodeCC);
  return call;
}
...
void IRGenFunction::storeCurrentAsyncContext(llvm::Value *context) {
  context = Builder.CreateBitCast(context, IGM.NeXTCodeContextPtrTy);
...
void IRGenModule::addNeXTCodeAsyncContextAttributes(llvm::AttributeList &attrs,
                                                 unsigned argIndex) {
  llvm::AttrBuilder b(getLLVMContext());
  b.addAttribute(llvm::Attribute::NeXTCodeAsync);
  attrs = attrs.addParamAttributes(this->getLLVMContext(), argIndex, b);
}

void IRGenModule::addNeXTCodeSelfAttributes(llvm::AttributeList &attrs,
                                         unsigned argIndex) {
  llvm::AttrBuilder b(getLLVMContext());
  b.addAttribute(llvm::Attribute::NeXTCodeSelf);
  attrs = attrs.addParamAttributes(this->getLLVMContext(), argIndex, b);
}

void IRGenModule::addNeXTCodeErrorAttributes(llvm::AttributeList &attrs,
                                          unsigned argIndex) {
  llvm::AttrBuilder b(getLLVMContext());
  // Don't add the nextcodeerror attribute on ABIs that don't pass it in a register.
  // We create a shadow stack location of the nextcodeerror parameter for the
  // debugger on such platforms and so we can't mark the parameter with a
  // nextcodeerror attribute.
  if (ShouldUseNeXTCodeError)
    b.addAttribute(llvm::Attribute::NeXTCodeError);
...
namespace {
  ...
  public:
    ...
    unsigned AsyncResumeFunctionNeXTCodeSelfIdx = 0;
...
void SignatureExpansion::expandResult(
  ...
  if (FnType->isAsync()) {
    // The result will be stored within the NeXTCodeContext that is passed to async
    // functions.
...
void SignatureExpansion::expandCoroutineResult(bool forContinuation) {
  ...
  while (clang::CodeGen::nextcodecall::
...
void SignatureExpansion::addAsyncParameters() {
  ...
  //   void (NEXTCODE_ASYNC_CONTEXT AsyncContext *);
  AsyncContextIdx = getCurParamIndex();
  Attrs = Attrs.addParamAttribute(IGM.getLLVMContext(), AsyncContextIdx,
                                  llvm::Attribute::NeXTCodeAsync);
  ParamIRTypes.push_back(IGM.NeXTCodeContextPtrTy);
}
...
NativeConventionSchema::NativeConventionSchema(IRGenModule &IGM,
  ...
  if (auto *loadable = dyn_cast<LoadableTypeInfo>(ti)) {
    // Lower the type according to the NeXTCode ABI.
...
void SignatureExpansion::expandExternalSignatureTypes() {
  ...
  // NeXTCode parameters list?
  size_t clangToNeXTCodeParamOffset = paramTys.size();
  ...
  assert(
      ...
      "nextcode and clang disagree on whether the result is returned indirectly");
#endif
  ...
  auto emitArg = [&](size_t i) {
    ...
    case clang::CodeGen::ABIArgInfo::Direct: {
      ...
      case clang::ParameterABI::Ordinary:
        break;
      case clang::ParameterABI::NeXTCodeAsyncContext:
        IGM.addNeXTCodeAsyncContextAttributes(Attrs, getCurParamIndex());
        break;
      case clang::ParameterABI::NeXTCodeContext:
        IGM.addNeXTCodeSelfAttributes(Attrs, getCurParamIndex());
        break;
      case clang::ParameterABI::NeXTCodeErrorResult:
        IGM.addNeXTCodeErrorAttributes(Attrs, getCurParamIndex());
        break;
      case clang::ParameterABI::NeXTCodeIndirectResult: {
        auto &param = params[i - clangToNeXTCodeParamOffset];
    ...
    case clang::CodeGen::ABIArgInfo::Indirect: {
      ...
      // NeXTCode parameter (self) to the first Clang parameter (this). In this
      // case, the corresponding NeXTCode param is the last function parameter.
      assert((i >= clangToNeXTCodeParamOffset || clangToNeXTCodeParamOffset == 1) &&
             "Unexpected index for indirect byval argument");
      auto &param = i < clangToNeXTCodeParamOffset
                        ? FnType->getParameters().back()
                        : params[i - clangToNeXTCodeParamOffset];
      auto paramTy = getSILFuncConventions().getSILType(
          param, IGM.getMaximalTypeExpansionContext());
      auto &paramTI = cast<FixedTypeInfo>(IGM.getTypeInfo(paramTy));
...
void SignatureExpansion::expandParameters(
  ...
  if (hasSelfContext) {
    ...
    if (claimSelf())
      IGM.addNeXTCodeSelfAttributes(Attrs, curLength);
    ...
    auto needsContext = [=]() -> bool {
      ...
      case SILFunctionType::Representation::Block:
        llvm_unreachable("adding block parameter in NeXTCode CC expansion?");
    ...
    if (needsContext()) {
      if (claimSelf())
        IGM.addNeXTCodeSelfAttributes(Attrs, ParamIRTypes.size());
  ...
  if (FnType->hasErrorResult()) {
    if (claimError())
      IGM.addNeXTCodeErrorAttributes(Attrs, ParamIRTypes.size());
...
void SignatureExpansion::expandAsyncReturnType() {
  ...
  auto addErrorResult = [&]() {
    ...
    if (FnType->hasErrorResult()) {
      ...
      IGM.addNeXTCodeSelfAttributes(Attrs, selfIdx);
      AsyncResumeFunctionNeXTCodeSelfIdx = selfIdx;
...
Signature SignatureExpansion::getSignature() {
  ...
  if (FnType->getLanguage() == SILFunctionLanguage::C) {
    ...
    info.AsyncResumeFunctionNeXTCodeSelfIdx = AsyncResumeFunctionNeXTCodeSelfIdx;
...
llvm::Value *emitIndirectAsyncFunctionPointer(IRGenFunction &IGF,
  ...
  llvm::Constant *NegativeOne =
      llvm::Constant::getIntegerValue(IntPtrTy, APInt(IntPtrTy->getBitWidth(),
                                                      -2));
  nextcode::irgen::Alignment PointerAlignment = IGF.IGM.getPointerAlignment();
...
public:
  ...
  void setFromCallee() override {
    ...
    llvm::Value *contextPtr = CurCallee.getNeXTCodeContext();
    ...
    if (fnType->hasErrorResult()) {
      ...
      IGF.IGM.addNeXTCodeErrorAttributes(CurCallee.getMutableAttributes(),
                                      LastArgWritten);
    ...
    if (contextPtr) {
      ...
      IGF.IGM.addNeXTCodeSelfAttributes(CurCallee.getMutableAttributes(),
                                     LastArgWritten);
    }
  }
...
llvm::Value *Callee::getNeXTCodeContext() const {
  switch (Info.OrigFnType->getRepresentation()) {
  case SILFunctionTypeRepresentation::Block:
  case SILFunctionTypeRepresentation::ObjCMethod:
  case SILFunctionTypeRepresentation::CFunctionPointer:
...
static void emitDirectExternalArgument(IRGenFunction &IGF, SILType argType,
  ...
  // Check to see if we can pairwise coerce NeXTCode's exploded scalars
...
/// Given a NeXTCode value explosion in 'in', produce a Clang expansion
/// (according to ABIArgInfo::Expand) in 'out'.
static void
emitClangExpandedArgument(IRGenFunction &IGF, Explosion &in, Explosion &out,
                          clang::CanQualType clangType, SILType nextcodeType,
                          const LoadableTypeInfo &nextcodeTI, bool isOutlined) {
  // If Clang's expansion schema matches NeXTCode's, great.
  auto nextcodeSchema = nextcodeTI.getSchema();
  if (doesClangExpansionMatchSchema(IGF.IGM, clangType, nextcodeSchema)) {
    return in.transferInto(out, nextcodeSchema.size());
  }

  // Otherwise, materialize to a temporary.
  auto ctemp = nextcodeTI.allocateStack(IGF, nextcodeType, "clang-expand-arg.temp");
  Address temp = ctemp.getAddress();
  nextcodeTI.initialize(IGF, in, temp, isOutlined);
  ...
  nextcodeTI.deallocateStack(IGF, ctemp, nextcodeType);
}

/// Given a Clang-expanded (according to ABIArgInfo::Expand) parameter
/// in 'in', produce a NeXTCode value explosion in 'out'.
void irgen::emitClangExpandedParameter(IRGenFunction &IGF,
                                       Explosion &in, Explosion &out,
                                       clang::CanQualType clangType,
                                       SILType nextcodeType,
                                       const LoadableTypeInfo &nextcodeTI) {
  // If Clang's expansion schema matches NeXTCode's, great.
  auto nextcodeSchema = nextcodeTI.getSchema();
  if (doesClangExpansionMatchSchema(IGF.IGM, clangType, nextcodeSchema)) {
    return in.transferInto(out, nextcodeSchema.size());
  ...
  // Otherwise, materialize to a temporary.
  auto tempAlloc = nextcodeTI.allocateStack(IGF, nextcodeType,
  ...
  // Then load out.
  nextcodeTI.loadAsTake(IGF, temp, out);

  nextcodeTI.deallocateStack(IGF, tempAlloc, nextcodeType);
}
...
void CallEmission::externalizeArguments(IRGenFunction &IGF, const Callee &callee,
  ...
  if (!FI.getReturnInfo().isIndirect() && formalIndirectResult) {
    // clang returns directly and nextcode returns indirectly
  ...
  for (unsigned i = firstParam; i != paramEnd; ++i) {
    ...
    // We don't need to do anything to handle the NeXTCode parameter-ABI
    // attributes here because we shouldn't be trying to round-trip
    // nextcodecall function pointers through SIL as C functions anyway.
    ...
    // In NeXTCode, values that are foreign references types will always be
...
bool irgen::addNativeArgument(IRGenFunction &IGF,
  ...
  if (nativeSchema.requiresIndirect()) {
    ...
    // Pass the argument explosion directly, mapping into the native nextcode
    // calling convention.
...
static void emitDirectForeignParameter(IRGenFunction &IGF, Explosion &in,
  ...
  // NeXTCode IR types for the body of the function.
...
Address IRGenFunction::createErrorResultSlot(SILType errorType, bool isAsync,
  ...
  if (!isTypedError && !setNeXTCodeErrorFlag &&
  ...
  // Only add the nextcodeerror attribute on ABIs that pass it in a register.
  // We create a shadow stack location of the nextcodeerror parameter for the
  // debugger on platforms that pass nextcodeerror by reference and so we can't
  // mark the parameter with a nextcodeerror attribute for these.
  // The slot for async callees cannot be annotated nextcodeerror because those
  // errors are never passed in registers but rather are always passed
  // indirectly in the async context.
  if (IGM.ShouldUseNeXTCodeError && !isAsync && setNeXTCodeErrorFlag)
    cast<llvm::AllocaInst>(addr.getAddress())->setNeXTCodeError(true);

  // Initialize at the alloca point.
  if (setNeXTCodeErrorFlag) {
...
void IRGenFunction::emitEpilogue() {
  ...
  if (!ExceptionUnwindBlocks.empty() ||
    ...
    if (IGM.isNeXTCodeExceptionPersonalityFeatureAvailable()) {
      // The function should use our personality routine
      auto nextcodePersonality = IGM.getExceptionPersonalityFunctionPointer();
      personality = nextcodePersonality.getDirectPointer();
    } else {
    ...
    if (native.size() == 1) {
      ...
      if (explosionTy != elt->getType()) {
        if (isa<llvm::IntegerType>(explosionTy) &&
          ...
          // treats it as i8 whereas NeXTCode works with i1, so we need to zext
          // in that case.
    ...
    if (fromNonNative.size() == 1) {
      ...
      if (nativeTy != elt->getType()) {
        if (isa<llvm::IntegerType>(nativeTy) &&
            isa<llvm::IntegerType>(elt->getType())) {
          // [HACK: Atomic-Bool-IRGen] In the case of _Atomic(_Bool), Clang
          // treats it as i8 whereas NeXTCode works with i1, so we need to trunc
          // in that case.
...
void IRGenFunction::emitScalarReturn(SILType returnResultType,
                                     ...
                                     bool isNeXTCodeCCReturn, bool isOutlined,
                                     SILType errorType) {
  bool mayReturnErrorDirectly = false;
  if (errorType) {
    auto &errorTI = IGM.getTypeInfo(errorType);
...
Callee irgen::getNeXTCodeFunctionPointerCallee(
...
IRGenFunction::getFunctionPointerForResumeIntrinsic(llvm::Value *resume) {
  ...
  attrs = attrs.addParamAttribute(IGM.getLLVMContext(), 0,
                                  llvm::Attribute::NeXTCodeAsync);
  auto signature =
      Signature(fnTy, attrs, IGM.NeXTCodeAsyncCC);
...
Address irgen::emitAutoDiffCreateLinearMapContextWithType(
  ...
  call->setDoesNotThrow();
  call->setCallingConv(IGF.IGM.NeXTCodeCC);
...
irgen::getFunctionPointerForDispatchCall(IRGenModule &IGM,
  ...
  auto signature =
      Signature(fnTy, fn.getSignature().getAttributes(), IGM.NeXTCodeAsyncCC);
