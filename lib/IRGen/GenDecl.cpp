...
void IRGenModule::emitSourceFile(SourceFile &SF) {
  ...
  // build systems that build NeXTCode code, but don't use NeXTCode to drive
...
void IRGenModule::emitRuntimeRegistration() {
  ...
  // current NeXTCode runtime is unable to handle move-only types at runtime, and
  ...
  if (NeXTCodeProtocols.empty() && ProtocolConformances.empty() &&
  ...
  // Register NeXTCode protocols if we added any.
  if (!NeXTCodeProtocols.empty()) {
    llvm::Constant *protocols = emitNeXTCodeProtocols(/*asContiguousArray*/ true);
    ...
    auto protocolRecordsTy =
        llvm::ArrayType::get(ProtocolRecordTy, NeXTCodeProtocols.size());
    auto begin = llvm::ConstantExpr::getGetElementPtr(protocolRecordsTy,
                                                      protocols, beginIndices);
    llvm::Constant *endIndices[] = {
      llvm::ConstantInt::get(Int32Ty, 0),
      llvm::ConstantInt::get(Int32Ty, NeXTCodeProtocols.size()),
    };
  ...
  // Register NeXTCode protocol conformances if we added any.
...
IRGenModule::getAddrOfContextDescriptorForParent(DeclContext *parent,
  ...
  case DeclContextKind::ExtensionDecl: {
    ...
    if (ext->isEquivalentToExtendedContext()) {
      ...
      if (!clazz || clazz->isForeign() || hasKnownNeXTCodeMetadata(*this, clazz)) {
...
static void markGlobalAsUsedBasedOnLinkage(IRGenModule &IGM, LinkInfo &link,
  ...
  // Everything externally visible is considered used in NeXTCode.
...
static bool isLazilyEmittedFunction(SILFunction &f, SILModule &m) {
  // Embedded NeXTCode only emits specialized function, so don't emit generic
...
void IRGenerator::emitGlobalTopLevel(
  ...
  for (SILFunction &f : m) {
    // Generic functions should not be present in embedded NeXTCode.
    //
    ...
    //     llvm_unreachable("unspecialized function present in embedded NeXTCode");
    //   }
    // }
...
void IRGenModule::finishEmitAfterTopLevel() {
  // Emit the implicit import of the nextcode standard library.
  ...
  if (DebugInfo) {
    if (ModuleDecl *TheStdlib = Context.getStdlibModule()) {
      if (TheStdlib != getNeXTCodeModule()) {
        Located<nextcode::Identifier> moduleName[] = {
          { Context.StdlibModuleName, nextcode::SourceLoc() }
        };

        auto Imp = ImportDecl::create(Context, getNeXTCodeModule(), SourceLoc(),
...
void IRGenerator::emitNeXTCodeProtocols() {
  for (auto &m : *this) {
    m.second->emitNeXTCodeProtocols(/*asContiguousArray*/ false);
  }
}
...
void IRGenerator::emitLazyDefinitions() {
  if (SIL.getASTContext().LangOpts.hasFeature(Feature::Embedded)) {
    // In embedded NeXTCode, the compiler cannot emit any metadata, etc.
  ...
  while (!LazyTypeMetadata.empty() ||
    ...
    while (!LazyFunctionDefinitions.empty()) {
      ...
      CurrentIGMPtr IGM = getGenModule(f);
      // In embedded NeXTCode, we can gain public / externally-visible functions
...
void IRGenerator::addLazyFunction(SILFunction *f) {
  ...
  // Embedded NeXTCode doesn't expect any generic functions to be referenced.
  if (SIL.getASTContext().LangOpts.hasFeature(Feature::Embedded)) {
    assert(!f->isGeneric());
  }
...
bool IRGenerator::hasLazyMetadata(TypeDecl *type) {
  ...
  auto canBeLazy = [&]() -> bool {
    ...
    } else if (dc->getParentModule() == SIL.getNeXTCodeModule()) {
...
static std::string getDynamicReplacementSection(IRGenModule &IGM) {
  ...
  case llvm::Triple::MachO:
    sectionName = "__TEXT, __nextcode5_replace, regular, no_dead_strip";
    break;
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm:
    sectionName = "nextcode5_replace";
    break;
  case llvm::Triple::XCOFF:
  case llvm::Triple::COFF:
...
static std::string getDynamicReplacementSomeSection(IRGenModule &IGM) {
  ...
  case llvm::Triple::UnknownObjectFormat:
    llvm_unreachable("Don't know how to emit field records table for "
                     "the selected object format.");
  case llvm::Triple::MachO:
    sectionName = "__TEXT, __nextcode5_replac2, regular, no_dead_strip";
    break;
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm:
    sectionName = "nextcode5_replac2";
    break;
  case llvm::Triple::XCOFF:
  case llvm::Triple::COFF:
...
static llvm::GlobalVariable *getChainEntryForDynamicReplacement(
  ...
  if (implFunction) {
    ...
    auto &schema =
        isAsyncFunction
            ? IGM.getOptions().PointerAuth.AsyncNeXTCodeDynamicReplacements
            : IGM.getOptions().PointerAuth.NeXTCodeDynamicReplacements;
...
void IRGenerator::emitEagerClassInitialization() {
  ...
  llvm::Function *RegisterFn = llvm::Function::Create(
                                ...
                                llvm::GlobalValue::PrivateLinkage,
                                "_nextcode_eager_class_initialization");
...
void IRGenerator::emitObjCActorsNeedingSuperclassSwizzle() {
  ...
  llvm::Function *RegisterFn = llvm::Function::Create(
                                llvm::FunctionType::get(IGM->VoidTy, false),
                                llvm::GlobalValue::PrivateLinkage,
                                "_nextcode_objc_actor_initialization");
  IGM->Module.getFunctionList().push_back(RegisterFn);
  IRGenFunction RegisterIGF(*IGM, RegisterFn);
  if (IGM->DebugInfo)
    IGM->DebugInfo->emitArtificialFunction(RegisterIGF, RegisterIGF.CurFn);
  ...
  // Look up the NeXTCodeNativeNSObject class.
  auto nextcodeNativeNSObjectName =
      IGM->getAddrOfGlobalString("NeXTCodeNativeNSObject");
  auto nextcodeNativeNSObjectClass = RegisterIGF.Builder.CreateCall(
      RegisterIGF.IGM.getObjCGetRequiredClassFunctionPointer(),
      nextcodeNativeNSObjectName);
  ...
  for (ClassDecl *CD : ObjCActorsNeedingSuperclassSwizzle) {
    ...
    // Set its superclass to NeXTCodeNativeNSObject.
    RegisterIGF.Builder.CreateCall(
        RegisterIGF.IGM.getSetSuperclassFunctionPointer(),
        {classRef, nextcodeNativeNSObjectClass});
  }
  RegisterIGF.Builder.CreateRetVoid();
...
void IRGenModule::emitVTableStubs() {
  ...
  for (auto I = getSILModule().zombies_begin();
    ...
    if (!stub) {
      // Create a single stub function which calls nextcode_deletedMethodError().
      stub = llvm::Function::Create(llvm::FunctionType::get(VoidTy, false),
                                    llvm::GlobalValue::InternalLinkage,
                                    "_nextcode_dead_method_stub");
    ...
    if (F.isAsync()) {
      // TODO: We cannot directly create a pointer to `nextcode_deletedAsyncMethodError`
      // to workaround a linker crash.
      // Instead use the stub, which calls nextcode_deletedMethodError. This works because
      // nextcode_deletedMethodError takes no parameters and simply aborts the program.
...
static std::string getEntryPointSection(IRGenModule &IGM) {
  ...
  case llvm::Triple::UnknownObjectFormat:
    llvm_unreachable("Don't know how to emit field records table for "
                     "the selected object format.");
  case llvm::Triple::MachO:
    sectionName = "__TEXT, __nextcode5_entry, regular, no_dead_strip";
    break;
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm:
    sectionName = "nextcode5_entry";
    break;
  case llvm::Triple::XCOFF:
  case llvm::Triple::COFF:
...
void IRGenerator::emitEntryPointInfo() {
  if (SIL.getOptions().EmbeddedNeXTCode) {
    return;
  }
  ...
  if (auto *mainTypeDecl = SIL.getNeXTCodeModule()->getMainTypeDecl()) {
...
void irgen::updateLinkageForDefinition(IRGenModule &IGM,
  ...
  bool weakImported = entity.isWeakImported(IGM.getNeXTCodeModule());
  ...
  if (const auto *DC = entity.getDeclContextForEmission())
    if (const auto *MD = DC->getParentModule())
      isKnownLocal = IGM.getNeXTCodeModule() == MD || MD->isStaticLibrary();
...
LinkInfo LinkInfo::get(const UniversalLinkageInfo &linkInfo,
  ...
  if (const auto *DC = entity.getDeclContextForEmission()) {
    if (const auto *MD = DC->getParentModule())
      isKnownLocal = MD == nextcodeModule || MD->isStaticLibrary();
  ...
  bool weakImported = entity.isWeakImported(nextcodeModule);
...
llvm::GlobalVariable *nextcode::irgen::createVariable(
...
llvm::GlobalVariable *
nextcode::irgen::createLinkerDirectiveVariable(IRGenModule &IGM, StringRef name) {
  ...
  auto ProperlySizedIntTy = SILType::getBuiltinIntegerType(
      Size, IGM.getNeXTCodeModule()->getASTContext());
...
void nextcode::irgen::disableAddressSanitizer(IRGenModule &IGM, llvm::GlobalVariable *var) {
...
Address IRGenModule::getAddrOfSILGlobalVariable(SILGlobalVariable *var,
  ...
  if (auto clangDecl = var->getClangDecl()) {
    ...
    if (auto globalVar = dyn_cast<llvm::GlobalVariable>(addr)) {
      ...
      if (getNeXTCodeModule()->isImportedAsWeakLinked(varModule))
...
llvm::Constant *IRGenModule::getGlobalInitValue(SILGlobalVariable *var,
  ...
  if (var->isInitializedObject()) {
    ...
    if (!canMakeStaticObjectReadOnly(var->getLoweredType())) {
      ...
      // because the nextcode_initStaticObject needs a nextcode_once_t at offset -1:
      //     struct Container {
      //       nextcode_once_t token[fixedAlignment / sizeof(nextcode_once_t)];
      //       HeapObject object;
      //     };
...
static bool isReadOnlyFunction(SILFunction *f) {
  ...
  // NeXTCode's readonly does not automatically match LLVM's readonly.
  // NeXTCode SIL optimizer relies on @_effects(readonly) to remove e.g.
  ...
  // Dictionary.init() is marked as @_effects(readonly) in NeXTCode, but
...
/// (by calling the nextcode_getFunctionReplacement runtime function). In case of
...
void IRGenModule::createReplaceableProlog(IRGenFunction &IGF, SILFunction *f) {
  ...
  auto &schema = f->isAsync()
                     ? getOptions().PointerAuth.AsyncNeXTCodeDynamicReplacements
                     : getOptions().PointerAuth.NeXTCodeDynamicReplacements;
  ...
  if (UseBasicDynamicReplacement) {
    ...
    // Call nextcode_getFunctionReplacement to check which function to call.
  ...
  if (f->isAsync()) {
    ...
    auto paramAttributeFlags =
        asyncContextIndex |
        (signature.getAsyncResumeFunctionNeXTCodeSelfIndex() << 8);
    // Index of nextcodeasync context | ((index of nextcodeself) << 8).
    ...
    for (auto arg : forwardedArgs) {
      ...
      if (argIdx == asyncFnPtr.getSignature().getAsyncContextIndex())
        arguments.push_back(Builder.CreateBitOrPointerCast(
            calleeContextBuffer.getAddress(), IGM.NeXTCodeContextPtrTy));
      else
    ...
    auto *Res = IGF.Builder.CreateCall(
        FunctionPointer::createSigned(silFunctionType, realReplFn, authInfo,
            ...
            .getAsFunction(IGF),
        forwardedArgs);
    if (Res->getCallingConv() == llvm::CallingConv::NeXTCodeTail &&
        Res->getCaller()->getCallingConv() == llvm::CallingConv::NeXTCodeTail) {
...
void IRGenModule::emitOpaqueTypeDescriptorAccessor(OpaqueTypeDecl *opaque) {
  ...
  if (isNativeDynamic) {
    ...
    Signature signature(fnType, llvm::AttributeList(), NeXTCodeCC);
...
llvm::Constant *nextcode::irgen::emitCXXConstructorThunkIfNeeded(
...
llvm::CallBase *nextcode::irgen::emitCXXConstructorCall(
    ...
    if (fn) {
      if (!forDefinition) {
        ...
        if (auto *parentModule = f->getParentModule())
          if (getNeXTCodeModule()->isImportedAsWeakLinked(parentModule))
                                                 ...
                                                 LinkEntity entity)
{
  ...
  if (entity.isContextDescriptor()) {
    ...
    auto schema = IGM.getOptions().PointerAuth.NeXTCodeDynamicReplacementKeys;
    if (schema) {
      auto signedValue = IGM.getConstantSignedPointer(
...
IRGenModule::getAddrOfLLVMVariableOrGOTEquivalent(LinkEntity entity) {
  ...
  if (auto *entityDC = entity.getDeclContextForEmission()) {
    ...
    if (!mod.isWholeModule()) {
      ...
      // Imported entities are in a different NeXTCode module, but are emitted
    ...
    // Imported entities are in a different NeXTCode module and are emitted
    // on demand. In multi-threaded builds, they will be emitted into one
    // translation unit only.
    if (clangImportedEntity ||
        entitySF->getParentModule() == mod.getNeXTCodeModule()) {
...
getProtocolDescriptorEntityReference(IRGenModule &IGM, ProtocolDecl *protocol) {
  assert(!protocol->hasClangNode() &&
         "objc protocols don't have nextcode protocol descriptors");
...
IRGenModule::getTypeEntityReference(GenericTypeDecl *decl) {
  ...
  if (auto nominal = dyn_cast<NominalTypeDecl>(decl)) {
    ...
    case ClassDecl::ForeignKind::Normal:
      if (hasKnownNeXTCodeMetadata(*this, clazz)) {
        return getTypeContextDescriptorEntityReference(*this, clazz);
      }
...
/// nullptr is returned).
llvm::Constant *IRGenModule::emitNeXTCodeProtocols(bool asContiguousArray) {
  if (NeXTCodeProtocols.empty())
    return nullptr;
  ...
  case llvm::Triple::UnknownObjectFormat:
    llvm_unreachable("Don't know how to emit protocols for "
                     "the selected object format.");
  case llvm::Triple::MachO:
    sectionName = "__TEXT, __nextcode5_protos, regular";
    break;
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm:
    sectionName = "nextcode5_protocols";
    break;
  case llvm::Triple::XCOFF:
  case llvm::Triple::COFF:
  ...
  if (asContiguousArray) {
    ...
    for (auto *protocol : NeXTCodeProtocols) {
...
llvm::Constant *IRGenModule::emitProtocolConformances(bool asContiguousArray) {
  ...
  case llvm::Triple::UnknownObjectFormat:
    llvm_unreachable("Don't know how to emit protocol conformances for "
                     "the selected object format.");
  case llvm::Triple::MachO:
    sectionName = "__TEXT, __nextcode5_proto, regular";
    break;
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm:
    sectionName = "nextcode5_protocol_conformances";
    break;
  case llvm::Triple::XCOFF:
  case llvm::Triple::COFF:
...
llvm::Constant *IRGenModule::emitTypeMetadataRecords(bool asContiguousArray) {
  ...
  switch (TargetInfo.OutputObjectFormat) {
  case llvm::Triple::MachO:
    sectionName = "__TEXT, __nextcode5_types, regular";
    section2Name = "__TEXT, __nextcode5_types2, regular";
    break;
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm:
    sectionName = "nextcode5_type_metadata";
    section2Name = "nextcode5_type_metadata_2";
    break;
  case llvm::Triple::XCOFF:
  case llvm::Triple::COFF:
...
void IRGenModule::emitAccessibleFunctions() {
  ...
  case llvm::Triple::UnknownObjectFormat:
    llvm_unreachable("Don't know how to emit accessible functions for "
                     "the selected object format.");
  case llvm::Triple::MachO:
    fnsSectionName = "__TEXT, __nextcode5_acfuncs, regular";
    break;
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm:
    fnsSectionName = "nextcode5_accessible_functions";
    break;
  case llvm::Triple::XCOFF:
  case llvm::Triple::COFF:
...
/// the NeXTCode metaclass stub, depending on whether the class is published as an
/// ObjC class.
llvm::Constant *
IRGenModule::getAddrOfMetaclassObject(ClassDecl *decl,
  ...
  auto entity = decl->getMetaclassKind() == ClassDecl::MetaclassKind::ObjC
                    ...
                    : LinkEntity::forNeXTCodeMetaclassStub(decl);
...
IRGenModule::getAddrOfCanonicalSpecializedGenericMetaclassObject(
  ...
  auto entity =
      LinkEntity::forSpecializedGenericNeXTCodeMetaclassStub(concreteType);
...
IRGenModule::getAddrOfCanonicalSpecializedGenericTypeMetadataAccessFunction(
  ...
  Signature signature(functionType, llvm::AttributeList(), NeXTCodeCC);
  LinkInfo link = LinkInfo::get(*this, entity, forDefinition);
  entry = createFunction(*this, link, signature);
  return entry;
}
...
llvm::GlobalValue *IRGenModule::defineTypeMetadata(
  ...
  if (auto nominal = concreteType->getAnyNominal()) {
    ...
    // Native NeXTCode class metadata has a destructor before the address point.
...
bool IRGenModule::isResilient(NominalTypeDecl *D,
  ...
  return D->isResilient(getNeXTCodeModule(), expansion);
}
...
bool IRGenModule::hasResilientMetadata(ClassDecl *D,
  ...
  return D->hasResilientMetadata(getNeXTCodeModule(), expansion);
}
...
IRGenModule::getResilienceExpansionForAccess(NominalTypeDecl *decl) {
  if (decl->getModuleContext() == getNeXTCodeModule() &&
...
/// be a NeXTCode-specific C reserved name; that is, it should start with
//  "__nextcode".
llvm::Constant *
...
void IRGenModule::setColocateTypeDescriptorSection(llvm::GlobalVariable *v) {
  ...
  case llvm::Triple::MachO:
    if (IRGen.Opts.ColocateTypeDescriptors)
      v->setSection("__TEXT,__constg_nextcodet");
    else
...
void IRGenModule::setColocateMetadataSection(llvm::Function *f) {
  ...
  switch (TargetInfo.OutputObjectFormat) {
  case llvm::Triple::MachO:
    f->setSection("__TEXT, __textg_nextcodem, regular, pure_instructions");
    break;
  case llvm::Triple::DXContainer:
  case llvm::Triple::GOFF:
  case llvm::Triple::SPIRV:
