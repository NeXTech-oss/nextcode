//===--- GenArchetype.cpp - NeXTCode IR Generation for Archetype Types -------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements IR generation for archetype types in NeXTCode.
//
//===----------------------------------------------------------------------===//
...
/// If ObjC interop is disabled, we can use NeXTCode refcounting entry
...
const TypeInfo *TypeConverter::convertArchetypeType(ArchetypeType *archetype) {
  ...
  if (layout && layout->isFixedSizeTrivial()) {
    ...
    auto ProperlySizedIntTy = SILType::getBuiltinIntegerType(
        size.getValueInBits(), IGM.getNeXTCodeModule()->getASTContext());
  ...
  // The protocol won't be present in nextcodeinterfaces from older SDKs.
  if (bitwiseCopyableProtocol && IGM.getNeXTCodeModule()->checkConformance(
...
getAddressOfOpaqueTypeDescriptor(IRGenFunction &IGF,
  ...
  if (!IGM.getOptions().shouldOptimize() &&
    ...
    desc->setCallingConv(IGM.NeXTCodeCC);
    return desc;
  }
