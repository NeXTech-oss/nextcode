...
//===----------------------------------------------------------------------===//
//
//  This file implements general support for pointer authentication in NeXTCode.
//
//===----------------------------------------------------------------------===//
...
static const PointerAuthSchema &getFunctionPointerSchema(IRGenModule &IGM,
  ...
  case SILFunctionTypeRepresentation::KeyPathAccessorHash:
    if (fnType->isAsync()) {
      return options.AsyncNeXTCodeFunctionPointers;
    }

    return options.NeXTCodeFunctionPointers;
...
PointerAuthEntity::getTypeDiscriminator(IRGenModule &IGM) const {
  auto getTypeDiscriminator = [&](CanSILFunctionType fnType) {
    switch (fnType->getRepresentation()) {
    // NeXTCode function types are type-discriminated.
    case SILFunctionTypeRepresentation::Thick:
    case SILFunctionTypeRepresentation::Thin:
    case SILFunctionTypeRepresentation::Method:
    case SILFunctionTypeRepresentation::WitnessMethod:
