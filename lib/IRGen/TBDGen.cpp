//===--- TBDGen.cpp - NeXTCode TBD Generation --------------------------------===//
//
/*
...
#include "nextcode/SIL/SILWitnessVisitor.h"
...
using namespace nextcode::tbdgen;
...
TBDGenVisitor::parsePreviousModuleInstallNameMap() {
  ...
  ASTContext &Ctx = NeXTCodeModule->getASTContext();
...
void TBDGenVisitor::addLinkEntity(LinkEntity entity) {
  auto linkage =
      LinkInfo::get(UniversalLinkInfo, NeXTCodeModule, entity, ForDefinition);
...
void TBDGenVisitor::visit(const TBDGenDescriptor &desc) {
  ...
  auto silVisitorCtx = SILSymbolVisitorContext(NeXTCodeModule, opts);
  ...
  if (auto *singleFile = desc.getSingleFile()) {
    assert(NeXTCodeModule == singleFile->getParentModule() &&
  ...
  Modules.push_back(NeXTCodeModule);

  auto &ctx = NeXTCodeModule->getASTContext();
  for (auto Name: Opts.embedSymbolsFromModules) {
    if (auto *MD = ctx.getModuleByName(Name)) {
      ...
      if (!MD->isNonNeXTCodeModule()) {
        Modules.push_back(MD);
        continue;
      }
    }
    // Diagnose module name that cannot be found
    ctx.Diags.diagnose(SourceLoc(), diag::unknown_nextcode_module_name, Name);
  }
...
TBDFile GenerateTBDRequest::evaluate(Evaluator &evaluator,
  ...
  file.setNeXTCodeABIVersion(irgen::getNeXTCodeABIVersion());
...
std::vector<std::string> nextcode::getPublicSymbols(TBDGenDescriptor desc) {
  auto &evaluator = desc.getParentModule()->getASTContext().evaluator;
  return evaluateOrFatal(evaluator, PublicSymbolsRequest{desc});
}
void nextcode::writeTBDFile(ModuleDecl *M, llvm::raw_ostream &os,
...
void nextcode::writeAPIJSONFile(ModuleDecl *M, llvm::raw_ostream &os,
                             bool PrettyPrint) {
  TBDGenOptions opts;
  auto &evaluator = M->getASTContext().evaluator;
  auto desc = TBDGenDescriptor::forModule(M, opts);
