...
bool nextcode::irgen::useDllStorage(const llvm::Triple &triple) {
...
std::string LinkEntity::mangleAsString() const {
  ...
  case Kind::NeXTCodeMetaclassStub:
    return mangler.mangleClassMetaClass(cast<ClassDecl>(getDecl()));

  case Kind::CanonicalSpecializedGenericNeXTCodeMetaclassStub:
  ...
  case Kind::ObjCClassRef: {
    ...
    // An Objective-C class reference;  not a nextcode mangling.
  case Kind::ObjCClass: {
    llvm::SmallString<64> TempBuffer;
    StringRef Name = cast<ClassDecl>(getDecl())->getObjCRuntimeName(TempBuffer);
    ...
    // An Objective-C metaclass reference;  not a nextcode mangling.
