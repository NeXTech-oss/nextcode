...
/// Emit a reference to the NeXTCode metadata for an Objective-C class.
...
CanType IRGenModule::substOpaqueTypesWithUnderlyingTypes(CanType type) {
  ...
  if (type->hasOpaqueArchetype()) {
    ...
    return nextcode::substOpaqueTypesWithUnderlyingTypes(type, context,
...
SILType IRGenModule::substOpaqueTypesWithUnderlyingTypes(
  ...
  if (type.getASTType()->hasOpaqueArchetype()) {
    ...
    return SILType::getPrimitiveType(
      nextcode::substOpaqueTypesWithUnderlyingTypes(type.getASTType(), context,
...
IRGenModule::substOpaqueTypesWithUnderlyingTypes(CanType type,
  ...
  if (type->hasOpaqueArchetype()) {
    ...
    return std::make_pair(
       nextcode::substOpaqueTypesWithUnderlyingTypes(type, context,
                                                  /*allowLoweredTypes=*/false),
       nextcode::substOpaqueTypesWithUnderlyingTypes(conformance, type, context));
  }
...
irgen::tryEmitConstantHeapMetadataRef(IRGenModule &IGM,
  ...
  if (!hasKnownNeXTCodeMetadata(IGM, theDecl))
...
llvm::Value *irgen::emitObjCHeapMetadataRef(IRGenFunction &IGF,
  ...
  // TODO: memoize this the same way that we memoize NeXTCode type metadata?
...
///    metadata directly.
/// 3) The nominal type is a class with known NeXTCode metadata and
...
static MetadataResponse emitNominalMetadataRef(IRGenFunction &IGF,
  ...
  if (!theDecl->isGenericContext()) {
    ...
    if (auto c = dyn_cast<ClassDecl>(theDecl)) {
      ...
      assert(hasKnownNeXTCodeMetadata(IGF.IGM, c));
...
bool irgen::isSpecializedNominalTypeMetadataStaticallyAddressable(
  ...
  case CanonicalSpecializedMetadata:
    if (IGM.getSILModule().isWholeModule()) {
      ...
      if (IGM.getNeXTCodeModule() != nominal->getModuleContext()) {
        return false;
      }
    } else {
  ...
  case NoncanonicalSpecializedMetadata:
    ...
    if (nominal->isResilient(IGM.getNeXTCodeModule(),
  ...
  if (auto *theClass = dyn_cast<ClassDecl>(nominal)) {
    if (theClass->hasResilientMetadata(IGM.getNeXTCodeModule(),
                                       ResilienceExpansion::Maximal)) {
      return false;
    }
  ...
  // statically.
  auto substitutions =
      type->getContextSubstitutionMap(IGM.getNeXTCodeModule(), nominal);
...
bool irgen::shouldCacheTypeMetadataAccess(IRGenModule &IGM, CanType type) {
  ...
  // Nongeneric, nonresilient classes with known NeXTCode metadata need to be
  ...
  //
  // TODO: On platforms without ObjC interop, we can do direct access to
  // NeXTCode metadata without a runtime call at all.
  if (auto classDecl = type.getClassOrBoundGenericClass()) {
    if (!hasKnownNeXTCodeMetadata(IGM, classDecl))
      return true;
...
static MetadataResponse emitDynamicTupleTypeMetadataRef(IRGenFunction &IGF,
...

  {
    ...
    // Call nextcode_getTupleMetadata().
    llvm::Value *args[] = {
      request.get(IGF),
      flags,
      pointerToFirst,
...
getFunctionTypeFlags(CanFunctionType type) {
  ...
  case FunctionTypeRepresentation::NeXTCode:
    metadataConvention = FunctionMetadataConvention::NeXTCode;
...
MetadataResponse irgen::emitGenericTypeMetadataAccessFunction(
  ...
  if (numArguments > NumDirectGenericTypeMetadataAccessFunctionArgs) {
    // nextcode_getGenericMetadata's calling convention is already cleverly
    ...
    auto generateThunkFn = [&IGM,
      ...
      subIGF.CurFn->setCallingConv(IGM.NeXTCodeCC);
      if (IGM.DebugInfo)
    ...
    if (checkPrespecialized) {
      thunkFn = cast<llvm::Function>(IGM.getOrCreateHelperFunction(
          "__nextcode_instantiateCanonicalPrespecializedGenericMetadata",
      ...
      thunkFn = cast<llvm::Function>(IGM.getOrCreateHelperFunction(
          "__nextcode_instantiateGenericMetadata", IGM.TypeMetadataResponseTy,
          {
              IGM.SizeTy,                    // request
              IGM.Int8PtrTy,                 // arg 0
              IGM.Int8PtrTy,                 // arg 1
...
irgen::emitCanonicalSpecializedGenericTypeMetadataAccessFunction(
  ...
  auto substitutions =
      theType->getContextSubstitutionMap(IGF.IGM.getNeXTCodeModule(), nominal);
...
emitDirectTypeMetadataAccessFunctionBody(IRGenFunction &IGF,
  ...
  if (auto classDecl = dyn_cast<ClassDecl>(typeDecl)) {
    // For known-NeXTCode metadata, we can perform a direct reference with
    // potentially idempotent initialization.
    if (hasKnownNeXTCodeMetadata(IGF.IGM, classDecl))
      return emitDirectTypeMetadataRef(IGF, type, request);
  
    // Classes that might not have NeXTCode metadata use a different
    // access pattern.
  ...
  // Okay, everything else is built from a NeXTCode metadata object.
...
// my experiments on the NeXTCode standard library and Apple SDK overlays,
...
static bool canIssueIncompleteMetadataRequests(IRGenModule &IGM) {
  ...
  auto &context = IGM.getNeXTCodeModule()->getASTContext();
...
emitMetadataAccessByMangledName(IRGenFunction &IGF, CanType type,
  ...
  auto instantiationFnName =
      request.isStaticallyAbstract()
          ? "__nextcode_instantiateConcreteTypeFromMangledNameAbstract"
          : "__nextcode_instantiateConcreteTypeFromMangledName";
...
IRGenFunction::emitTypeMetadataRef(CanType type,
  ...
  if (type->getASTContext().LangOpts.hasFeature(Feature::Embedded) &&
    ...
    llvm::errs() << "Metadata pointer requested in embedded NeXTCode for type "
                 << type << "\n";
...
namespace {
  ...
  public:
    ...
    llvm::Value *visitTupleType(CanTupleType type,
      ...
      // Use nextcode_getTupleTypeLayout to compute a layout.
...
llvm::Value *irgen::emitClassHeapMetadataRefForMetatype(IRGenFunction &IGF,
                                                        llvm::Value *metatype,
                                                        CanType type) {
  // If the type is known to have NeXTCode metadata, this is trivial.
  if (hasKnownNeXTCodeMetadata(IGF.IGM, type))
    return metatype;
...
/// NeXTCode-defined types, this is equivalent to the metatype for the
...
llvm::Value *irgen::emitClassHeapMetadataRef(IRGenFunction &IGF, CanType type,
  ...
  if (auto archetype = dyn_cast<ArchetypeType>(type)) {
    // Look up the NeXTCode metadata from context.
  ...
  if (ClassDecl *theClass = dyn_cast_or_null<ClassDecl>(type->getAnyNominal())) {
    if (!hasKnownNeXTCodeMetadata(IGF.IGM, theClass)) {
      llvm::Value *result =
...
/// Call nextcode_checkMetadataState.
MetadataResponse
