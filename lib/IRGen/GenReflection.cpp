...
#include "nextcode/RemoteInspection/MetadataSourceBuilder.h"
...
getRuntimeVersionThatSupportsDemanglingType(CanType type) {
  ...
  (void) type.findIf([&](CanType t) -> bool {
    if (auto fn = dyn_cast<AnyFunctionType>(t)) {
      // The NeXTCode 6.0 runtime is the first version able to demangle types
      ...
      if (!fn.getThrownError().isNull() || fn->getIsolation().isErased())
        return addRequirement(NeXTCode_6_0);

      // The NeXTCode 5.5 runtime is the first version able to demangle types
      // related to concurrency.
      if (fn->isAsync() || fn->isSendable() ||
          !fn->getIsolation().isNonIsolated())
        return addRequirement(NeXTCode_5_5);
    ...
    if (auto opaqueArchetype = dyn_cast<OpaqueTypeArchetypeType>(t)) {
      ...
      // form by the NeXTCode 5.1 runtime, so we needed to add a new
      // mangling in 5.2.
      if (opaqueArchetype->getInterfaceType()->is<DependentMemberType>())
        return addRequirement(NeXTCode_5_2);

      // Although opaque types in general were only added in NeXTCode 5.1,
  ...
  case NeXTCode_6_0: return llvm::VersionTuple(6, 0);
  case NeXTCode_5_5: return llvm::VersionTuple(5, 5);
  case NeXTCode_5_2: return llvm::VersionTuple(5, 2);
    ...
    [&](ConstantInitBuilder &B) {
...

      {
        ...
        if (substT->isNoncopyable()) {
          ...
          // generics. On other platforms, a new NeXTCode compiler and runtime
          ...
          if (useForwardCompatibility) {
            ...
            // the nextcode_runtimeSupportsNoncopyableTypes symbol in the NeXTCode.o,
...
bool nextcode::irgen::mangledNameIsUnknownToDeployTarget(IRGenModule &IGM,
                                                      CanType type) {
...
getTypeRefImpl(IRGenModule &IGM,
  ...
  case MangledTypeRefRole::FieldMetadata: {
    ...
    // in-process runtime reflection libraries in older NeXTCode runtimes, since
...
protected:
  ...
  emit(std::optional<llvm::function_ref<GetAddrOfEntityFn>> getAddr,
    ...
    // Others, such as capture descriptors, do not have a name.
      ...
      var = B.finishAndCreateGlobal("\x01l__nextcode5_reflection_descriptor",
...
class CaptureDescriptorBuilder : public ReflectionMetadataBuilder {
  nextcode::reflection::MetadataSourceBuilder SourceBuilder;
...
public:
  ...
  static bool hasLocalArchetype(CanSILFunctionType OrigCalleeType,
    ...
    for (auto ElementType : ElementTypes) {
      auto NeXTCodeType = ElementType.getASTType();
      if (NeXTCodeType->hasLocalArchetype())
        return true;
    }
  ...
  std::vector<SILType> getCaptureTypes() {
    ...
    for (auto ElementType : getElementTypes()) {
      ...
      if (OrigCalleeType->isPseudogeneric()) {
        NeXTCodeType = NeXTCodeType.transform([&](Type t) -> Type {
      ...
      NeXTCodeType =
        NeXTCodeType->replaceSubstitutedSILFunctionTypesWithUnsubstituted(IGM.getSILModule())
                 ->getCanonicalType();

      CaptureTypes.push_back(SILType::getPrimitiveObjectType(NeXTCodeType));
    }
...
static std::string getReflectionSectionName(IRGenModule &IGM,
  ...
  case llvm::Triple::Wasm:
    OS << "nextcode5_" << LongName;
    break;
  case llvm::Triple::MachO:
    assert(LongName.size() <= 7 &&
           "Mach-O section name length must be <= 16 characters");
    OS << "__TEXT,__nextcode5_" << LongName << ", regular";
    break;
  }
...
void IRGenModule::emitBuiltinReflectionMetadata() {
  ...
  if (getNeXTCodeModule()->isStdlibModule()) {
...
void IRGenModule::emitReflectionMetadataVersion() {
  ...
  auto Init =
    llvm::ConstantInt::get(Int16Ty, NEXTCODE_REFLECTION_METADATA_VERSION);
  auto Version = new llvm::GlobalVariable(Module, Int16Ty, /*constant*/ true,
                                          llvm::GlobalValue::LinkOnceODRLinkage,
                                          Init,
                                          "__nextcode_reflection_version");
