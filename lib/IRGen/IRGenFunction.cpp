//===--- IRGenFunction.cpp - NeXTCode Per-Function IR Generation -------------===//
//
/*
...
ModuleDecl *IRGenFunction::getNeXTCodeModule() const {
  return IGM.getNeXTCodeModule();
}
...
llvm::Value *IRGenFunction::emitAllocRawCall(llvm::Value *size,
                                             ...
                                             const llvm::Twine &name) {
  // For now, all we have is nextcode_slowAlloc.
...
llvm::Value *IRGenFunction::emitAllocObjectCall(llvm::Value *metadata,
                                                ...
                                                llvm::Value *alignMask,
                                                const llvm::Twine &name) {
  // For now, all we have is nextcode_allocObject.
...
void IRGenFunction::emitDeallocRawCall(llvm::Value *pointer,
                                       llvm::Value *size,
                                       llvm::Value *alignMask) {
  // For now, all we have is nextcode_slowDealloc.
...
void IRGenFunction::emitTSanInoutAccessCall(llvm::Value *address) {
  ...
  // access.
  const unsigned kExternalTagNeXTCodeModifyingAccess = 0x1;
  llvm::Value *tagValue =
      llvm::ConstantInt::get(IGM.SizeTy, kExternalTagNeXTCodeModifyingAccess);
...
static llvm::Value *emitLoadOfResumeContextFromTask(IRGenFunction &IGF,
                                                    llvm::Value *task) {
  // Task.ResumeContext is at field index 8 within NeXTCodeTaskTy. The offset comes
  ...
  auto addr = Address(task, IGF.IGM.NeXTCodeTaskTy, IGF.IGM.getPointerAlignment());
...
void IRGenFunction::emitGetAsyncContinuation(SILType resumeTy,
  ...
  // context before calling nextcode_continuation_init:
  ...
  llvm::Value *asyncContextValue =
    Builder.CreateBitCast(getAsyncContext(), IGM.NeXTCodeContextPtrTy);
  ...
  // Call the nextcode_continuation_init runtime function to initialize
  ...
  task->setCallingConv(IGM.NeXTCodeCC);
...
void IRGenFunction::emitAwaitAsyncContinuation(
  ...
  // Call nextcode_continuation_await to check whether the continuation
  ...
  // nextcode_continuation_await, emit the old inline sequence.  This can
...
void IRGenFunction::emitResumeAsyncContinuationReturning(
  ...
  continuation = Builder.CreateBitCast(continuation, IGM.NeXTCodeTaskPtrTy);
...
void IRGenFunction::emitClearSensitive(Address address, llvm::Value *size) {
  // If our deployment target doesn't contain the new nextcode_clearSensitive,
