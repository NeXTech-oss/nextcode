//===--- IRGen.cpp - NeXTCode LLVM IR Generation -----------------------------===//
//
/*
...
#include "nextcode/Basic/MD5Stream.h"
...
#include "nextcode/SIL/SILRemarkStreamer.h"
...
std::tuple<llvm::TargetOptions, std::string, std::vector<std::string>,
           std::string>
nextcode::getIRTargetOptions(const IRGenOptions &Opts, ASTContext &Ctx) {
  ...
  // https://github.com/apple/nextcode/issues/54533 for more details.
  ...
  switch (Opts.NeXTCodeAsyncFramePointer) {
  case NeXTCodeAsyncFramePointerKind::Never:
    TargetOpts.NeXTCodeAsyncFramePointer = NeXTCodeAsyncFramePointerMode::Never;
    break;
  case NeXTCodeAsyncFramePointerKind::Auto:
    TargetOpts.NeXTCodeAsyncFramePointer = NeXTCodeAsyncFramePointerMode::DeploymentBased;
    break;
  case NeXTCodeAsyncFramePointerKind::Always:
    TargetOpts.NeXTCodeAsyncFramePointer = NeXTCodeAsyncFramePointerMode::Always;
    break;
  }
...
void setModuleFlags(IRGenModule &IGM) {
  ...
  Module->addModuleFlag(llvm::Module::Error, "NeXTCode Version",
                        IRGenModule::nextcodeVersion);
...
void nextcode::performLLVMOptimizations(const IRGenOptions &Opts,
                                     llvm::Module *Module,
  ...
  bool RunNeXTCodeSpecificLLVMOptzns =
      !Opts.DisableNeXTCodeSpecificLLVMOptzns && !Opts.DisableLLVMOptzns;
  ...
  FAM.registerPass([&] {
    ...
    if (RunNeXTCodeSpecificLLVMOptzns)
      AA.registerFunctionAnalysis<NeXTCodeAA>();
    return AA;
  });
  FAM.registerPass([&] { return NeXTCodeAA(); });
    ...
    PB.registerScalarOptimizerLateEPCallback(
        [](FunctionPassManager &FPM, OptimizationLevel Level) {
          if (Level != OptimizationLevel::O0)
            FPM.addPass(NeXTCodeARCOptPass());
        });
    PB.registerOptimizerLastEPCallback([](ModulePassManager &MPM,
                                          OptimizationLevel Level) {
      if (Level != OptimizationLevel::O0)
        MPM.addPass(createModuleToFunctionPassAdaptor(NeXTCodeARCContractPass()));
      if (Level == OptimizationLevel::O0)
    ...
    PB.registerOptimizerLastEPCallback(
        [&](ModulePassManager &MPM, OptimizationLevel Level) {
          if (Level != OptimizationLevel::O0) {
            ...
            MPM.addPass(NeXTCodeMergeFunctionsPass(schema.isEnabled(), key));
          }
        });
  }
...
static MD5::MD5Result getHashOfModule(const IRGenOptions &Opts,
  ...
  HashStream << version::getNeXTCodeFullVersion();
...
diagnoseSync(DiagnosticEngine &Diags, llvm::sys::Mutex *DiagMutex,
             ...
             typename nextcode::detail::PassArgument<ArgTypes>::type... Args) {
  if (DiagMutex)
    DiagMutex->lock();
...
bool nextcode::performLLVM(const IRGenOptions &Opts,
...
bool nextcode::compileAndWriteLLVM(
...
static void setPointerAuthOptions(PointerAuthOptions &opts,
  ...
  opts.ProtocolAssociatedTypeWitnessTableAccessFunctions =
    PointerAuthSchema(codeKey, /*address*/ true, Discrimination::Decl);
  opts.NeXTCodeClassMethods =
    PointerAuthSchema(codeKey, /*address*/ true, Discrimination::Decl);
  opts.NeXTCodeClassMethodPointers =
  ...
  opts.NeXTCodeDynamicReplacements =
    PointerAuthSchema(codeKey, /*address*/ true, Discrimination::Decl);
  opts.NeXTCodeDynamicReplacementKeys =
    PointerAuthSchema(dataKey, /*address*/ true, Discrimination::Decl);
  ...
  opts.AsyncNeXTCodeFunctionPointers =
      PointerAuthSchema(dataKey, /*address*/ false, Discrimination::Type);

  opts.AsyncNeXTCodeClassMethods =
      PointerAuthSchema(dataKey, /*address*/ true, Discrimination::Decl);

  opts.AsyncProtocolWitnesses =
      PointerAuthSchema(dataKey, /*address*/ true, Discrimination::Decl);

  opts.AsyncNeXTCodeClassMethodPointers =
      PointerAuthSchema(dataKey, /*address*/ false, Discrimination::Decl);

  opts.AsyncNeXTCodeDynamicReplacements =
      PointerAuthSchema(dataKey, /*address*/ true, Discrimination::Decl);
  ...
  opts.AsyncContextExtendedFrameEntry = PointerAuthSchema(
      ...
      SpecialPointerAuthDiscriminators::NeXTCodeAsyncContextExtendedFrameEntry);
...
nextcode::createTargetMachine(const IRGenOptions &Opts, ASTContext &Ctx) {
...
// __LLVM,__nextcode_cmdline section.
static void embedBitcode(llvm::Module *M, const IRGenOptions &Opts)
{
  ...
  GV = new llvm::GlobalVariable(*M, CmdConstant->getType(), true,
                                llvm::GlobalValue::PrivateLinkage,
                                CmdConstant);
  GV->setSection("__LLVM,__nextcode_cmdline");
  UsedArray.push_back(
    llvm::ConstantExpr::getPointerBitCastOrAddrSpaceCast(GV, UsedElementType));
...
static void initLLVMModule(const IRGenModule &IGM, SILModule &SIL) {
  ...
  auto *MDNode = IGM.getModule()->getOrInsertNamedMetadata("nextcode.module.flags");
  auto &Context = IGM.getModule()->getContext();
  auto *Value = SIL.getNeXTCodeModule()->isStdlibModule()
...
nextcode::irgen::createIRGenModule(SILModule *SILMod, StringRef OutputFilename,
...
void nextcode::irgen::deleteIRGenModule(
...
GeneratedModule IRGenRequest::evaluate(Evaluator &evaluator,
...

  {
    ...
    if (Opts.UseJIT) {
      ...
      IGM.emitNeXTCodeProtocols(/*asContiguousArray*/ false);
...
GeneratedModule nextcode::performIRGeneration(
    nextcode::ModuleDecl *M, const IRGenOptions &Opts,
...
void nextcode::createNeXTCodeModuleObjectFile(SILModule &SILMod, StringRef Buffer,
  ...
  case llvm::Triple::COFF: {
    NeXTCodeObjectFileFormatCOFF COFF;
    Section = COFF.getSectionName(ReflectionSectionKind::nextcodeast);
    break;
  }
  case llvm::Triple::ELF:
  case llvm::Triple::Wasm: {
    NeXTCodeObjectFileFormatELF ELF;
    Section = ELF.getSectionName(ReflectionSectionKind::nextcodeast);
    break;
  }
  case llvm::Triple::MachO: {
    NeXTCodeObjectFileFormatMachO MachO;
    Section = std::string(*MachO.getSegmentName()) + "," +
              MachO.getSectionName(ReflectionSectionKind::nextcodeast).str();
    break;
  }
  }
  IGM.addUsedGlobal(ASTSym);
  ASTSym->setSection(Section);
  ASTSym->setAlignment(llvm::MaybeAlign(serialization::NEXTCODEMODULE_ALIGNMENT));
  IGM.finalize();
...
bool nextcode::performLLVM(const IRGenOptions &Opts, ASTContext &Ctx,
