//===--- GenCast.cpp - NeXTCode IR Generation for dynamic casts --------------===//
//
/*
...
/// Compute the flags to pass to nextcode_dynamicCast.
...
llvm::Value *irgen::emitClassDowncast(IRGenFunction &IGF, llvm::Value *from,
  ...
  // If the destination type is known to have a NeXTCode-compatible
  // implementation, use the most specific entrypoint.
  if (destClass && destClass->hasKnownNeXTCodeImplementation()) {
...
void irgen::emitMetatypeDowncast(IRGenFunction &IGF,
  ...
  case MetatypeRepresentation::Thick: {
    // Get the NeXTCode metadata for the type we're checking.
...
void irgen::emitScalarExistentialDowncast(
  ...
  if (!objcProtos.empty()) {
    ...
    // casting a NeXTCode metatype or ObjC object.
    FunctionPointer castFn;
    switch (mode) {
    case CheckedCastMode::Unconditional:
      castFn =
  ...
  // Get the NeXTCode type metadata for the type.
...
void irgen::emitScalarCheckedCast(IRGenFunction &IGF,
  ...
  if (auto sourceMetatype = sourceLoweredType.getAs<AnyMetatypeType>()) {
    ...
    if (object == nullptr) {
      // Obj-C cast routine failed, use nextcode_dynamicCast instead
...
/// `nextcode_dynamicCastClass`.
/// It also avoids a call to the metadata accessor of the class (which calls
/// `nextcode_getInitializedObjCClass`). For comparing the metadata pointers it's
