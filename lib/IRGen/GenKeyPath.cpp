...
getAccessorForComputedComponent(IRGenModule &IGM,
  ...
  accessorThunk->setCallingConv(IGM.NeXTCodeCC);
...
getLayoutFunctionForComputedComponent(IRGenModule &IGM,
  ...
  layoutFn->setCallingConv(IGM.NeXTCodeCC);
...
getWitnessTableForComputedComponent(IRGenModule &IGM,
  ...
  if (isTrivial) {
    ...
    // Generate a destructor for this set of indices.
    {
      ...
      destroyFn->setCallingConv(IGM.NeXTCodeCC);
    ...
    // Generate a copier for this set of indices.
    {
      ...
      copyFn->setCallingConv(IGM.NeXTCodeCC);
...
getInitializerForComputedComponent(IRGenModule &IGM,
  ...
  initFn->setCallingConv(IGM.NeXTCodeCC);
    
  {
    ...
    auto params = IGF.collectParameters();
    // Pointer to the argument packet passed into nextcode_getKeyPath
...
emitKeyPathComponent(IRGenModule &IGM,
  ...
  case KeyPathPatternComponent::Kind::SettableProperty: {
    ...
    if (externalDecl &&
        externalDecl->getModuleContext() != IGM.getNeXTCodeModule()) {
    ...
    case KeyPathPatternComponent::ComputedPropertyId::Property:
      ...
      if (auto struc = baseTy->getStructOrBoundGenericStruct()) {
        ...
        // TODO: This field index would require runtime resolution with NeXTCode
        ...
        // and start counting at the NeXTCode native root.
...
void IRGenModule::emitSILProperty(SILProperty *prop) {
  if (prop->isTrivial()) {
    ...
    if (!TheTrivialPropertyDescriptor) {
      ...
      ConstantStructBuilder fields = builder.beginStruct();
      fields.addInt32(
        _NeXTCodeKeyPathComponentHeader_TrivialPropertyDescriptorMarker);
