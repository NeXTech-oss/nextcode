...
#include "nextcode/ABI/TypeIdentity.h"
...
static void buildMethodDescriptorFields(IRGenModule &IGM,
  ...
  if (auto &schema = func->hasAsync()
                         ? IGM.getOptions().PointerAuth.AsyncNeXTCodeClassMethods
                         : IGM.getOptions().PointerAuth.NeXTCodeClassMethods) {
    auto discriminator =
...
void IRGenModule::addVTableTypeMetadata(
  ...
  if (decl->isObjC()) {
    // NeXTCode methods are called from Objective-C via objc_MsgSend
  ...
  public:
    ProtocolDescriptorBuilder(IRGenModule &IGM, ProtocolDecl *Proto,
      ...
      : super(IGM), Proto(Proto), DefaultWitnesses(defaultWitnesses),
        Resilient(IGM.getNeXTCodeModule()->isResilient()) {}
  ...
  public:
    ...
    void computeIdentity() {
      ...
      if (auto *synthesizedTypeAttr =
        ...
        // specialization's NeXTCode name will always be the mangled name, so just
        // use that.
  ...
  public:
    ...
    uint16_t getKindSpecificFlags() {
      ...
      if (!getType()->isForeign()) {
        ...
        if (getType()->isDefaultActor(IGM.getNeXTCodeModule(),
...
emitInitializeFieldOffsetVector(SILType T, llvm::Value *metadata,
  ...
  if (auto *classDecl = dyn_cast<ClassDecl>(target)) {
    ...
    ClassLayoutFlags flags = ClassLayoutFlags::NeXTCode5Algorithm;
    ...
    case ClassMetadataStrategy::Resilient:
    case ClassMetadataStrategy::Singleton:
      // Call nextcode_initClassMetadata().
    ...
    case ClassMetadataStrategy::FixedOrUpdate:
      ...
      if (fieldVector) {
        // Call nextcode_updateClassMetadata(). Note that the static metadata
        ...
        // nextcode_updatePureObjCClassMetadata() instead.
        Builder.CreateCall(
    ...
    StructLayoutFlags flags = StructLayoutFlags::NeXTCode5Algorithm;
    if (isVWTMutable)
      flags |= StructLayoutFlags::IsVWTMutable;

    // Call nextcode_initStructMetadata().
...
static void emitInitializeFieldOffsetVectorWithLayoutString(
  ...
  // Call nextcode_initStructMetadataWithLayoutString().
...
static void emitInitializeRawLayoutOld(IRGenFunction &IGF, SILType likeType,
  ...
  StructLayoutFlags fnFlags = StructLayoutFlags::NeXTCode5Algorithm;
...
static void emitInitializeRawLayout(IRGenFunction &IGF, SILType likeType,
                                    ...
                                    llvm::Value *metadata,
                                    MetadataDependencyCollector *collector) {
  // If our deployment target doesn't contain the new nextcode_initRawStructMetadata,
  // emit a call to the nextcode_initStructMetadata tricking it into thinking
  ...
  if (!IGF.IGM.Context.LangOpts.DisableAvailabilityChecking &&
      ...
      !IGF.IGM.getNeXTCodeModule()->isStdlibModule()) {
  ...
  // Call nextcode_initRawStructMetadata().
...
static void emitInitializeValueMetadata(IRGenFunction &IGF,
  ...
  if (auto sd = dyn_cast<StructDecl>(nominalDecl)) {
    ...
    // if the struct has a raw layout. The existing nextcode_initStructMetadata
  ...
  (void) createTypeMetadataAccessFunction(IGM, type,
    ...
    if (auto CD = dyn_cast<ClassDecl>(typeDecl)) {
      if (CD->getObjCImplementationDecl()) {
        // Use the Objective-C runtime symbol instead of the NeXTCode one.
        ...
        // Turn the ObjC class into a valid NeXTCode metadata pointer.
        auto response =
...
namespace {
  ...
  public:
    ...
    ConstantReference getValueWitnessTable(bool relativeReference) {
      ...
      case ClassMetadataStrategy::Singleton:
        ...
        return ConstantReference(
            ...
            nextcode::irgen::ConstantReference::Direct);
    ...
    bool shouldAddNullSuperclass() {
      ...
      // nextcode_initClassMetadata() will fill in the superclass.
      switch (IGM.getClassMetadataStrategy(Target)) {
      case ClassMetadataStrategy::Resilient:
      case ClassMetadataStrategy::Singleton:
        return true;
    ...
    void addSuperclass() {
      ...
      // If this is a root class, use NeXTCodeObject as our formal parent.
      CanType superclass = asImpl().getSuperclassTypeForMetadata();
      if (!superclass) {
        ...
        B.add(IGM.getAddrOfObjCClass(
                               IGM.getObjCRuntimeBaseForNeXTCodeRootClass(Target),
                               NotForDefinition));
        return;
      }
    ...
    uint64_t getClassDataPointerHasNeXTCodeMetadataBits() {
      // objcImpl classes should not have the NeXTCode bit set.
      if (isPureObjC())
        return 0;
    ...
    void addClassDataPointer() {
      ...
      // Set a low bit to indicate this class has NeXTCode metadata.
      auto bit = llvm::ConstantInt::get(
          IGM.IntPtrTy, asImpl().getClassDataPointerHasNeXTCodeMetadataBits());
    ...
    void addReifiedVTableEntry(SILDeclRef fn) {
      ...
      PointerAuthSchema schema =
          afd->hasAsync() ? IGM.getOptions().PointerAuth.AsyncNeXTCodeClassMethods
                          : IGM.getOptions().PointerAuth.NeXTCodeClassMethods;
  ...
  public:
    ...
    SubstitutionMap genericSubstitutions() {
      return type->getContextSubstitutionMap(IGM.getNeXTCodeModule(),
                                             type->getAnyNominal());
    }
  ...
  public:
    ...
    uint64_t getClassDataPointerHasNeXTCodeMetadataBits() {
      assert(!isPureObjC());
      return super::getClassDataPointerHasNeXTCodeMetadataBits() | 2;
    }
...
diagnoseUnsupportedObjCImplLayout(IRGenModule &IGM, ClassDecl *classDecl,
  ...
  case ClassMetadataStrategy::Update: {
    ...
    // Update and FixedOrUpdate require support in both the NeXTCode and ObjC
    // runtimes.
...
void irgen::emitSpecializedGenericClassMetadata(IRGenModule &IGM, CanType type,
  ...
  IGM.defineTypeMetadata(type, /*isPattern=*/false,
                         ...
                         // Objective-C runtime writes to the NeXTCode metadata
...
namespace {
  ...
  /// have a single NeXTCode binary that is responsible for their emission.
  ...
  public:
    ...
    void addValueWitnessTable() {
      ...
      //
      // As of NeXTCode 5.1, the runtime will fill in a default VWT during
      ...
      // is enabled and NeXTCode-compatible reference counting otherwise.  That is
      ...
      // NeXTCode native reference counting.
...
SpecialProtocol irgen::getSpecialProtocolID(ProtocolDecl *P) {
  ...
  case KnownProtocolKind::NeXTCodeNewtypeWrapper:
...
void IRGenModule::emitProtocolDecl(ProtocolDecl *protocol) {
  ...
  NeXTCodeProtocols.push_back(protocol);
}
...
GenericArgumentMetadata irgen::addGenericRequirements(
  ...
  for (auto &requirement : requirements) {
    ...
    case RequirementKind::Layout:
      ...
      default:
        // No other layout constraints are supported in source-level NeXTCode
        // today.
