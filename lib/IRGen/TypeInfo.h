...
// operations on nextcode values and objects.
//
...
#ifndef NEXTCODE_IRGEN_TYPEINFO_H
#define NEXTCODE_IRGEN_TYPEINFO_H
...
protected:
  ...
  union {
    ...
    NEXTCODE_INLINE_BITFIELD_BASE(TypeInfo,
    ...
    NEXTCODE_INLINE_BITFIELD_FULL(FixedTypeInfo, TypeInfo, 32,
      : NumPadBits,
...
public:
  ...
  *buildTypeLayoutEntry(IRGenModule &IGM,
  ...
  /// Should optimizations be enabled which rely on the representation
  /// for this type being a single NeXTCode-retainable object pointer?
  ///
  /// \return false by default
  bool isSingleNeXTCodeRetainablePointer(ResilienceExpansion expansion) const {
    ReferenceCounting refcounting;
  ...
  /// these types is the null pointer, on which nextcode_retain is a harmless
