...
void DistributedAccessor::decodeArguments(const ArgumentDecoderInfo &decoder,
  ...
  // Cast type buffer to `nextcode.type**`
  argumentTypes =
...
void DistributedAccessor::decodeArgument(unsigned argumentIdx,
  ...
  std::unique_ptr<CallEmission> emission =
      getCallEmission(IGF, callee.getNeXTCodeContext(), std::move(callee));
  ...
  // indirect result buffer as `nextcode.opaque*`
...
void DistributedAccessor::emitReturn(llvm::Value *errorValue) {
  ...
  // nextcode_release freed memory, which will be a no-op, however that also would
...
void DistributedAccessor::emit() {
  ...
  // `nextcode.type**` that holds the argument types that correspond to values.
...
ArgumentDecoderInfo DistributedAccessor::findArgumentDecoder(
  ...
  if (auto classDecl = dyn_cast<ClassDecl>(decoderDecl)) {
    ...
    usesDispatchThunk =
        getMethodDispatch(decodeFn) == nextcode::MethodDispatch::Class &&
