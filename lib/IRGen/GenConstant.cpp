//===--- GenConstant.cpp - NeXTCode IR Generation For Constants --------------===//
//
/*
...
void insertPadding(SmallVectorImpl<Explosion> &elements,
                   llvm::StructType *sTy) {
  // fill in any gaps, which are the explicit padding that codec inserts.
  ...
  if (IGM.canMakeStaticObjectReadOnly(OI->getType())) {
    if (!IGM.codeImmortalRefCount) {
      if (IGM.Context.LangOpts.hasFeature(Feature::Embedded)) {
        ...
        IGM.codeImmortalRefCount = llvm::ConstantInt::get(IGM.IntPtrTy, -1);
      } else {
        IGM.codeImmortalRefCount = llvm::ConstantExpr::getPtrToInt(
            new llvm::GlobalVariable(IGM.Module, IGM.Int8Ty,
              /*constant*/ true, llvm::GlobalValue::ExternalLinkage,
              /*initializer*/ nullptr, "_nextcodeImmortalRefCount"),
            IGM.IntPtrTy);
      }
    }
    if (!IGM.codeStaticArrayMetadata) {
      ...
      if (IGM.Context.LangOpts.hasFeature(Feature::Embedded)) {
        LinkEntity entity = LinkEntity::forTypeMetadata(classTy, TypeMetadataAddress::AddressPoint,
                                                        /*forceShared=*/ true);
        // In embedded nextcode, the metadata for the array buffer class only needs to be very minimal:
