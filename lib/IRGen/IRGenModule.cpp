//===--- IRGenModule.cpp - NeXTCode Global LLVM IR Generation ----------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements IR generation for global declarations in NeXTCode.
//
//===----------------------------------------------------------------------===//

#include "nextcode/AST/ASTContext.h"
#include "nextcode/AST/Availability.h"
...
#include "nextcode/Basic/LLVMExtras.h"
...
#include "nextcode/Runtime/RuntimeFnWrappersGen.h"
...
static void checkPointerAuthWitnessDiscriminator(IRGenModule &IGM, ArrayRef<StringRef> declPath, uint16_t expected) {
  ...
  auto decl = lookupSimple(IGM.getNeXTCodeModule(), declPath);
...
static void checkPointerAuthAssociatedTypeDiscriminator(IRGenModule &IGM, ArrayRef<StringRef> declPath, uint16_t expected) {
  ...
  auto decl = dyn_cast_or_null<AssociatedTypeDecl>(lookupSimple(IGM.getNeXTCodeModule(), declPath));
  assert(decl && "decl not found");
...
static void sanityCheckStdlib(IRGenModule &IGM) {
  if (!IGM.getNeXTCodeModule()->isStdlibModule()) return;

  // Only run the soundness check when we're building the real stdlib.
  if (!lookupSimple(IGM.getNeXTCodeModule(), { "String" })) return;
...
IRGenModule::IRGenModule(IRGenerator &irgen,
    ...
    : LLVMContext(new llvm::LLVMContext()), IRGen(irgen),
      ...
      TargetInfo(NeXTCodeTargetInfo::get(*this)), DebugInfo(nullptr),
  ...
  RefCountedStructTy =
    llvm::StructType::create(getLLVMContext(), "nextcode.refcounted");
...
#define CHECKED_REF_STORAGE(Name, name, ...)                                   \
  Name##ReferenceStructTy =                                                    \
      createStructType(*this, "nextcode." #name, {RefCountedPtrTy});              \
...
#include "nextcode/AST/ReferenceStorage.def"
  ...
  // structure.
  TypeMetadataStructTy = createStructType(*this, "nextcode.type", {
  ...
  TypeMetadataResponseTy = createStructType(*this, "nextcode.metadata_response", {
    TypeMetadataPtrTy,
    SizeTy
  });
  ...
  FullTypeLayoutTy = createStructType(*this, "nextcode.full_type_layout", {
    SizeTy, // size
    SizeTy, // flags
    SizeTy, // alignment
  ...
  TypeLayoutTy = createStructType(*this, "nextcode.type_layout", {
    SizeTy, // size
    SizeTy, // stride
    Int32Ty, // flags
  ...
  // metadata records.
  ProtocolDescriptorStructTy = createStructType(*this, "nextcode.protocol", {
    Int8PtrTy,              // objc isa
    Int8PtrTy,              // name
  ...
  ProtocolRequirementStructTy =
      createStructType(*this, "nextcode.protocol_requirement", {
    Int32Ty,                // flags
    RelativeAddressTy,      // default implementation
  });
  
  // A tuple type metadata record has a couple extra fields.
  auto tupleElementTy = createStructType(*this, "nextcode.tuple_element_type", {
    TypeMetadataPtrTy,      // Metadata *Type;
    Int32Ty                 // int32_t Offset;
  });
  TupleTypeMetadataTy = createStructType(
      *this, "nextcode.tuple_type",
      {
  ...
  FullTypeMetadataStructTy = createStructType(*this, "nextcode.full_type", {
    Int8PtrTy,
    WitnessTablePtrTy,
  ...
  FullForeignTypeMetadataStructTy = createStructType(*this, "nextcode.full_foreign_type", {
    WitnessTablePtrTy,
    TypeMetadataStructTy
  });
  ...
  FullExistentialTypeMetadataStructTy = createStructType(*this, "nextcode.full_existential_type", {
    WitnessTablePtrTy,
    TypeMetadataStructTy
  });
  ...
  FullHeapMetadataStructTy =
                  createStructType(*this, "nextcode.full_heapmetadata", {
    Int8PtrTy,
    dtorPtrTy,
    WitnessTablePtrTy,
    TypeMetadataStructTy
  ...
  FullBoxMetadataStructTy =
                  createStructType(*this, "nextcode.full_boxmetadata", {
    dtorPtrTy,
    WitnessTablePtrTy,
    TypeMetadataStructTy,
    Int32Ty,
  ...
  FunctionPairTy = createStructType(*this, "nextcode.function", {
    FunctionPtrTy,
    RefCountedPtrTy,
  });

  OpaqueTy = llvm::StructType::create(getLLVMContext(), "nextcode.opaque");
  OpaquePtrTy = OpaqueTy->getPointerTo(DefaultAS);
  NoEscapeFunctionPairTy = createStructType(*this, "nextcode.noescape.function", {
    FunctionPtrTy,
    OpaquePtrTy,
  });

  ProtocolRecordTy =
    createStructType(*this, "nextcode.protocolref", {
      RelativeAddressTy
    });
  ProtocolRecordPtrTy = ProtocolRecordTy->getPointerTo();

  ProtocolConformanceDescriptorTy
    = createStructType(*this, "nextcode.protocol_conformance_descriptor", {
      RelativeAddressTy,
      RelativeAddressTy,
      RelativeAddressTy,
      Int32Ty
  ...
  TypeContextDescriptorTy
    = llvm::StructType::create(getLLVMContext(), "nextcode.type_descriptor");
  ...
  MethodDescriptorStructTy
    = createStructType(*this, "nextcode.method_descriptor", {
      Int32Ty,
      RelativeAddressTy,
    });

  MethodOverrideDescriptorStructTy
    = createStructType(*this, "nextcode.method_override_descriptor", {
      RelativeAddressTy,
      RelativeAddressTy,
      RelativeAddressTy
    });
  ...
  TypeMetadataRecordTy
    = createStructType(*this, "nextcode.type_metadata_record", {
      RelativeAddressTy
    });
  ...
  FieldDescriptorTy
    = llvm::StructType::create(getLLVMContext(), "nextcode.field_descriptor");
  ...
  BridgeObjectPtrTy = llvm::StructType::create(getLLVMContext(), "nextcode.bridge")
                ->getPointerTo(DefaultAS);
  ...
  auto ErrorStructTy = llvm::StructType::create(getLLVMContext(), "nextcode.error");
  ...
  DefaultCC = NEXTCODE_DEFAULT_LLVM_CC;

  bool isNeXTCodeCCSupported =
    clangASTContext.getTargetInfo().checkCallingConvention(clang::CC_NeXTCode)
    == clang::TargetInfo::CCCR_OK;
  if (isNeXTCodeCCSupported) {
    NeXTCodeCC = llvm::CallingConv::NeXTCode;
  } else {
    NeXTCodeCC = DefaultCC;
  }

  bool isAsyncCCSupported =
    clangASTContext.getTargetInfo().checkCallingConvention(clang::CC_NeXTCodeAsync)
    == clang::TargetInfo::CCCR_OK;
  if (isAsyncCCSupported) {
    NeXTCodeAsyncCC = llvm::CallingConv::NeXTCodeTail;
    AsyncTailCallKind = llvm::CallInst::TCK_MustTail;
  } else {
    NeXTCodeAsyncCC = NeXTCodeCC;
  ...
  if (TargetInfo.OutputObjectFormat == llvm::Triple::Wasm) {
    ...
    // arguments for `nextcodeself` and `nextcodeerror`. If there is `nextcodeself` but
    // is no `nextcodeerror` in a nextcodecc function or invocation, then LLVM adds
    // dummy `nextcodeerror` parameter or argument. To count up how many dummy
    // arguments should be added, we need to mark it as `nextcodeerror` even though
    // it's not in register.
    ShouldUseNeXTCodeError = true;
  } else if (!isNeXTCodeCCSupported) {
    ShouldUseNeXTCodeError = false;
  } else {
    ShouldUseNeXTCodeError =
        clang::CodeGen::nextcodecall::isNeXTCodeErrorLoweredInRegister(
            ClangCodeGen->CGM());
  }
  ...
  DynamicReplacementLinkEntryTy =
      llvm::StructType::create(getLLVMContext(), "nextcode.dyn_repl_link_entry");
  ...
  DynamicReplacementKeyTy = createStructType(*this, "nextcode.dyn_repl_key",
                                             {RelativeAddressTy, Int32Ty});

  AccessibleFunctionRecordTy =
      createStructType(*this, "nextcode.accessible_function",
                       {RelativeAddressTy, RelativeAddressTy, RelativeAddressTy,
                        RelativeAddressTy, Int32Ty});

  AsyncFunctionPointerTy = createStructType(*this, "nextcode.async_func_pointer",
                                            {RelativeAddressTy, Int32Ty}, true);
  NeXTCodeContextTy = llvm::StructType::create(getLLVMContext(), "nextcode.context");
  NeXTCodeContextPtrTy = NeXTCodeContextTy->getPointerTo(DefaultAS);

  // This must match the definition of class AsyncTask in nextcode/ABI/Task.h.
  NeXTCodeTaskTy = createStructType(*this, "nextcode.task", {
    ...
    NeXTCodeContextPtrTy,    // Task.ResumeContext
    IntPtrTy              // Task.Status
  });

  AsyncFunctionPointerPtrTy = AsyncFunctionPointerTy->getPointerTo(DefaultAS);
  NeXTCodeTaskPtrTy = NeXTCodeTaskTy->getPointerTo(DefaultAS);
  NeXTCodeAsyncLetPtrTy = Int8PtrTy; // we pass it opaquely (AsyncLet*)
  NeXTCodeTaskOptionRecordTy =
    llvm::StructType::create(getLLVMContext(), "nextcode.task_option");
  NeXTCodeTaskOptionRecordPtrTy = NeXTCodeTaskOptionRecordTy->getPointerTo(DefaultAS);
  NeXTCodeTaskOptionRecordTy->setBody({
    SizeTy,                     // Flags
    NeXTCodeTaskOptionRecordPtrTy, // Parent
  });
  NeXTCodeTaskGroupPtrTy = Int8PtrTy; // we pass it opaquely (TaskGroup*)
  NeXTCodeTaskGroupTaskOptionRecordTy = createStructType(
      *this, "nextcode.task_group_task_option", {
    NeXTCodeTaskOptionRecordTy,    // Base option record
    NeXTCodeTaskGroupPtrTy,        // Task group
  });
  NeXTCodeResultTypeInfoTaskOptionRecordTy = createStructType(
      *this, "nextcode.result_type_info_task_option", {
    NeXTCodeTaskOptionRecordTy,    // Base option record
    SizeTy,
    SizeTy,
    Int8PtrTy,
  ...
  NeXTCodeExecutorTy = createStructType(*this, "nextcode.executor", {
    ExecutorFirstTy,      // identity
    ExecutorSecondTy,     // implementation
  });
  NeXTCodeInitialSerialExecutorTaskOptionRecordTy =
      createStructType(*this, "nextcode.serial_executor_task_option", {
    NeXTCodeTaskOptionRecordTy, // Base option record
    NeXTCodeExecutorTy,         // Executor
  });
  NeXTCodeInitialTaskExecutorUnownedPreferenceTaskOptionRecordTy =
      createStructType(*this, "nextcode.task_executor_task_option", {
    NeXTCodeTaskOptionRecordTy, // Base option record
    NeXTCodeExecutorTy,         // Executor
  });
  NeXTCodeInitialTaskExecutorOwnedPreferenceTaskOptionRecordTy =
      createStructType(*this, "nextcode.task_executor_owned_task_option", {
    NeXTCodeTaskOptionRecordTy, // Base option record
    NeXTCodeExecutorTy,         // Executor
  });
  NeXTCodeJobTy = createStructType(*this, "nextcode.job", {
    RefCountedStructTy,   // object header
  ...
  NeXTCodeJobPtrTy = NeXTCodeJobTy->getPointerTo(DefaultAS);

  // using TaskContinuationFunction =
  //   NEXTCODE_CC(nextcode) void (NEXTCODE_ASYNC_CONTEXT AsyncContext *);
  TaskContinuationFunctionTy = llvm::FunctionType::get(
      VoidTy, {NeXTCodeContextPtrTy}, /*isVarArg*/ false);
  TaskContinuationFunctionPtrTy = TaskContinuationFunctionTy->getPointerTo();

  NeXTCodeContextTy->setBody({
    NeXTCodeContextPtrTy,             // Parent
    TaskContinuationFunctionPtrTy, // ResumeParent
  });

  AsyncTaskAndContextTy = createStructType(
      *this, "nextcode.async_task_and_context",
      { NeXTCodeTaskPtrTy, NeXTCodeContextPtrTy });

  if (Context.LangOpts.isConcurrencyModelTaskToThread()) {
    ContinuationAsyncContextTy = createStructType(
        *this, "nextcode.continuation_context",
        {NeXTCodeContextTy,       // AsyncContext header
         SizeTy,               // flags
         SizeTy,               // await synchronization
         ErrorPtrTy,           // error result pointer
         OpaquePtrTy,          // normal result address
         NeXTCodeExecutorTy,      // resume to executor
  ...
  DifferentiabilityWitnessTy = createStructType(
      *this, "nextcode.differentiability_witness", {Int8PtrTy, Int8PtrTy});
}
...
namespace RuntimeConstants {
  ...
  RuntimeAvailability DynamicReplacementAvailability(ASTContext &Context) {
    auto featureAvailability = Context.getNeXTCode51Availability();
    if (!isDeploymentAvailabilityContainedIn(Context, featureAvailability)) {
  ...
  RuntimeAvailability TaskRunInlineAvailability(ASTContext &context) {
    ...
    // nextcode_task_run_inline is only available under task-to-thread execution
    // model.
    return RuntimeAvailability::ConditionallyAvailable;
  }
...
bool isStandardLibrary(const llvm::Module &M) {
  if (auto *Flags = M.getNamedMetadata("nextcode.module.flags")) {
...
llvm::FunctionType *nextcode::getRuntimeFnType(llvm::Module &Module,
...
llvm::Constant *nextcode::getRuntimeFn(
  ...
  if (auto fn = dyn_cast<llvm::Function>(fnptr)) {
    ...
    // Add nextcodeself and nextcodeerror attributes only when nextcode_willThrow
    // nextcode_willThrow is defined in RuntimeFunctions.def, but due to the
    // DSL limitation, arguments attributes are not set.
    // On the other hand, caller of `nextcode_willThrow` assumes that it's attributed
    // with `nextcodeself` and `nextcodeerror`.
    ...
    if (functionName == "nextcode_willThrow") {
      assert(IGM && "IGM is required for nextcode_willThrow.");
      fn->addParamAttr(0, Attribute::AttrKind::NeXTCodeSelf);
      if (IGM->ShouldUseNeXTCodeError) {
        fn->addParamAttr(1, Attribute::AttrKind::NeXTCodeError);
      }
    }
  }
...
llvm::Constant *IRGenModule::getDeletedAsyncMethodErrorAsyncFunctionPointer() {
  return getAddrOfLLVMVariableOrGOTEquivalent(
      LinkEntity::forKnownAsyncFunctionPointer("nextcode_deletedAsyncMethodError")).getValue();
}
...
llvm::Constant *IRGenModule::getObjCEmptyVTablePtr() {
  ...
  // didn't guarantee _objc_empty_vtable to be nil, but NeXTCode doesn't
...
Address IRGenModule::getAddrOfObjCISAMask() {
  ...
  if (!ObjCISAMaskPtr) {
    ObjCISAMaskPtr = Module.getOrInsertGlobal("nextcode_isaMask", IntPtrTy);
    ApplyIRLinkage(IRLinkage::ExternalImport)
...
ModuleDecl *IRGenModule::getNeXTCodeModule() const {
  return IRGen.SIL.getNeXTCodeModule();
}
...
bool nextcode::irgen::shouldRemoveTargetFeature(StringRef feature) {
...
StringRef
nextcode::irgen::encodeForceLoadSymbolName(llvm::SmallVectorImpl<char> &buf,
                                        StringRef name) {
  llvm::raw_svector_ostream os{buf};
  os << "_nextcode_FORCE_LOAD_$";
  appendEncodedName(os, name);
  return os.str();
}
...
static bool
doesTargetAutolinkUsingAutolinkExtract(const NeXTCodeTargetInfo &TargetInfo,
...
struct AutolinkKind {
  ...
  static AutolinkKind create(const NeXTCodeTargetInfo &TargetInfo,
...
StringRef AutolinkKind::getSectionNameMetadata() {
  ...
  case AutolinkKind::NeXTCodeAutoLinkExtract:
...
void AutolinkKind::collectEntriesFromLibraries(
  ...
  case AutolinkKind::NeXTCodeAutoLinkExtract: {
    ...
    for (LinkLibrary linkLib : AutolinkEntries) {
      ...
      case LibraryKind::Framework:
        // Frameworks are not supported with NeXTCode Autolink Extract.
        continue;
      }
      llvm_unreachable("Unhandled LibraryKind in switch.");
    }
  ...
  case AutolinkKind::NeXTCodeAutoLinkExtract: {
    ...
    auto var =
        new llvm::GlobalVariable(*IGM.getModule(), EntriesConstant->getType(),
                                 ...
                                 EntriesConstant, "_nextcode1_autolink_entries");
...
AutolinkKind AutolinkKind::create(const NeXTCodeTargetInfo &TargetInfo,
                                  llvm::Triple Triple,
  ...
  if (doesTargetAutolinkUsingAutolinkExtract(TargetInfo, Triple)) {
    return AutolinkKind::NeXTCodeAutoLinkExtract;
  }
...
void IRGenModule::emitAutolinkInfo() {
  ...
  nextcode::SmallSetVector<llvm::MDNode *, 4> Entries;
...
void emitNeXTCodeVersionNumberIntoModule(llvm::Module *Module) {
  ...
  uint8_t Major, Minor;
  std::tie(Major, Minor) = version::getNeXTCodeNumericVersion();
  uint32_t Value =
      (Major << 24) | (Minor << 16) | (IRGenModule::nextcodeVersion << 8);
...
void IRGenModule::cleanupClangCodeGenMetadata() {
  ...
  emitNeXTCodeVersionNumberIntoModule(&Module);
}

bool IRGenModule::finalize() {
  const char *ModuleHashVarName = "llvm.code_module_hash";
  if (IRGen.Opts.OutputKind == IRGenOutputKind::ObjectFile &&
    ...
    case llvm::Triple::MachO:
      ...
      ModuleHash->setSection("__LLVM,__nextcode_modhash");
      break;
  ...
  // Finalize NeXTCode debug info before running Clang codegen, because it may
  ...
  emitNeXTCodeAsyncExtendedFrameInfoWeakRef();
  emitAutolinkInfo();
  emitGlobalLists();
...
// In embedded nextcode features are available independent of deployment and
...
#include "nextcode/AST/FeatureAvailability.def"
...
bool IRGenModule::shouldPrespecializeGenericMetadata() {
  ...
  auto &context = getNeXTCodeModule()->getASTContext();
  auto deploymentAvailability =
      AvailabilityContext::forDeploymentTarget(context);
  return IRGen.Opts.PrespecializeGenericMetadata &&
...
bool IRGenModule::canUseObjCSymbolicReferences() {
  ...
  return isObjCSymbolicReferencesFeatureAvailable(
    getNeXTCodeModule()->getASTContext()
  );
}
...
ClassDecl *IRGenModule::getStaticArrayStorageDecl() {
  SmallVector<ValueDecl *, 1> results;
  Context.lookupInNeXTCodeModule("__StaticArrayStorage", results);
...
uint32_t nextcode::irgen::getNeXTCodeABIVersion() {
  return IRGenModule::nextcodeVersion;
}
...
void IRGenModule::emitNeXTCodeAsyncExtendedFrameInfoWeakRef() {
  if (!hasNeXTCodeAsyncFunctionDef || extendedFramePointerFlagsWeakRef)
    return;
  if (IRGen.Opts.NeXTCodeAsyncFramePointer !=
      NeXTCodeAsyncFramePointerKind::Auto)
    return;
  if (isConcurrencyAvailable())
    return;

  // Emit a weak reference to the `nextcode_async_extendedFramePointerFlags` symbol
  // needed by NeXTCode async functions.
  auto symbolName = "nextcode_async_extendedFramePointerFlags";
...
/// num-threads > 0.
bool nextcode::writeEmptyOutputFilesFor(
  ...
  for (auto fileName : ParallelOutputFilenames) {
    ...
    emitNeXTCodeVersionNumberIntoModule(llvmModule);

    nextcode::performLLVM(IRGenOpts, const_cast<ASTContext&>(Context),
                       llvmModule, fileName);
  }
  return false;
}
