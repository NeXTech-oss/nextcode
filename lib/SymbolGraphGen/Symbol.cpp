...
#include "nextcode/SymbolGraphGen/DocumentationCategory.h"
...
std::pair<StringRef, StringRef> Symbol::getKind(const Decl *D) {
  ...
  switch (D->getKind()) {
  case nextcode::DeclKind::Class:
    return {"nextcode.class", "Class"};
  case nextcode::DeclKind::Struct:
    return {"nextcode.struct", "Structure"};
  case nextcode::DeclKind::Enum:
    return {"nextcode.enum", "Enumeration"};
  case nextcode::DeclKind::EnumElement:
    return {"nextcode.enum.case", "Case"};
  case nextcode::DeclKind::Protocol:
    return {"nextcode.protocol", "Protocol"};
  case nextcode::DeclKind::Constructor:
    return {"nextcode.init", "Initializer"};
  case nextcode::DeclKind::Destructor:
    return {"nextcode.deinit", "Deinitializer"};
  case nextcode::DeclKind::Func: {
    const auto *VD = cast<ValueDecl>(D);

    if (VD->isOperator())
      return {"nextcode.func.op", "Operator"};
    if (VD->isStatic())
      return {"nextcode.type.method", "Type Method"};
    if (VD->getDeclContext()->getSelfNominalTypeDecl())
      return {"nextcode.method", "Instance Method"};
    return {"nextcode.func", "Function"};
  }
  case nextcode::DeclKind::Param: LLVM_FALLTHROUGH;
  case nextcode::DeclKind::Var: {
    const auto *VD = cast<ValueDecl>(D);

    if (VD->isStatic())
      return {"nextcode.type.property", "Type Property"};
    if (VD->getDeclContext()->getSelfNominalTypeDecl())
      return {"nextcode.property", "Instance Property"};
    return {"nextcode.var", "Global Variable"};
  }
  case nextcode::DeclKind::Subscript: {
    const auto *VD = cast<ValueDecl>(D);

    if (VD->isStatic())
      return {"nextcode.type.subscript", "Type Subscript"};
    return {"nextcode.subscript", "Instance Subscript"};
  }
  case nextcode::DeclKind::TypeAlias:
    return {"nextcode.typealias", "Type Alias"};
  case nextcode::DeclKind::AssociatedType:
    return {"nextcode.associatedtype", "Associated Type"};
  case nextcode::DeclKind::Extension:
    return {"nextcode.extension", "Extension"};
  case nextcode::DeclKind::Macro:
    return {"nextcode.macro", "Macro"};
  default:
...
void Symbol::serializeIdentifier(llvm::json::OStream &OS) const {
  OS.attributeObject("identifier", [&](){
    ...
    OS.attribute("interfaceLanguage", "nextcode");
  });
}
...
void Symbol::serializeFunctionSignature(llvm::json::OStream &OS) const {
  auto serializeParameterList = [&](const nextcode::ParameterList *ParamList) {
    if (ParamList->size()) {
...
static SubstitutionMap getSubMapForDecl(const ValueDecl *D, Type BaseType) {
  ...
  // in the substitution).
  nextcode::DeclContext *DC;
  if (isa<nextcode::ExtensionDecl>(D))
    DC = cast<nextcode::ExtensionDecl>(D)->getExtendedNominal();
  else
    DC = D->getInnermostDeclContext()->getInnermostTypeContext();

  nextcode::ModuleDecl *M = DC->getParentModule();
  if (isa<nextcode::NominalTypeDecl>(D) || isa<nextcode::ExtensionDecl>(D)) {
    return BaseType->getContextSubstitutionMap(M, DC);
  }

  const nextcode::ValueDecl *SubTarget = D;
  if (isa<nextcode::ParamDecl>(D)) {
    auto *DC = D->getDeclContext();
    if (auto *FD = dyn_cast<nextcode::AbstractFunctionDecl>(DC))
      SubTarget = FD;
  }
  return BaseType->getMemberSubstitutionMap(M, SubTarget);
}

void Symbol::serializeNeXTCodeGenericMixin(llvm::json::OStream &OS) const {
  ...
  if (const auto *GC = D->getAsGenericContext()) {
    if (const auto Generics = GC->getGenericSignature()) {
      ...
      OS.attributeObject("nextcodeGenerics", [&](){
      ...
      }); // end nextcodeGenerics:
    }
  }
}

void Symbol::serializeNeXTCodeExtensionMixin(llvm::json::OStream &OS) const {
  if (const auto *ED = dyn_cast<ExtensionDecl>(D)) {
    ::serialize(ED, OS);
  } else if (const auto *VD = dyn_cast<ValueDecl>(D)) {
...
void Symbol::serialize(llvm::json::OStream &OS) const {
  OS.object([&](){
    ...
    serializeNeXTCodeGenericMixin(OS);
    serializeNeXTCodeExtensionMixin(OS);
...
nextcode::DeclName Symbol::getName(const Decl *D) const {
  if (const auto *ED = dyn_cast<ExtensionDecl>(D)) {
