...
void SymbolGraph::recordMemberRelationship(Symbol S) {
  ...
  switch (DC->getContextKind()) {
    ...
    case nextcode::DeclContextKind::EnumElementDecl:
      /*
      ...
      return recordEdge(S,
                        ...
                        RelationshipKind::MemberOf());
    case nextcode::DeclContextKind::AbstractClosureExpr:
    case nextcode::DeclContextKind::SerializedAbstractClosure:
    case nextcode::DeclContextKind::Initializer:
    case nextcode::DeclContextKind::TopLevelCodeDecl:
    case nextcode::DeclContextKind::SerializedTopLevelCodeDecl:
    case nextcode::DeclContextKind::SubscriptDecl:
    case nextcode::DeclContextKind::AbstractFunctionDecl:
    case nextcode::DeclContextKind::Package:
    case nextcode::DeclContextKind::Module:
    case nextcode::DeclContextKind::FileUnit:
    case nextcode::DeclContextKind::MacroDecl:
      break;
  }
}
    ...
    OS.attributeObject("metadata", [&](){
      {
        ...
        llvm::VersionTuple FormatVersion(NEXTCODE_SYMBOLGRAPH_FORMAT_MAJOR,
                                         NEXTCODE_SYMBOLGRAPH_FORMAT_MINOR,
                                         NEXTCODE_SYMBOLGRAPH_FORMAT_PATCH);
        symbolgraphgen::serialize(FormatVersion, OS);
      } // end formatVersion:

      auto VersionString = version::getNeXTCodeFullVersion();
...
bool SymbolGraph::isImplicitlyPrivate(const Decl *D,
  ...
  if (const auto *VD = dyn_cast<ValueDecl>(D)) {
    ...
    // Clang module does not prevent internal NeXTCode symbols from being emitted when
  ...
  .Case("nextcode_" #C_TYPE "2", true) \
  .Case("nextcode_" #C_TYPE "3", true) \
  .Case("nextcode_" #C_TYPE "4", true)
  #include "nextcode/ClangImporter/SIMDMappedTypes.def"
    .Case("NEXTCODE_TYPEDEFS", true)
    .Case("char16_t", true)
    .Case("char32_t", true)
    .Default(false);
