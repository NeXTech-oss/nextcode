...
void ClangValueTypePrinter::printMetadataAccessAsVariable(
  ...
  printer.printNeXTCodeTypeMetadataAccessFunctionCall(metadataFuncName,
                                                   genericRequirements);
  os << ";\n";
}
...
static void addCppExtensionsToStdlibType(const NominalTypeDecl *typeDecl,
  ...
  if (typeDecl == typeDecl->getASTContext().getStringDecl()) {
    ...
    cPrologueOS << "struct nextcode_interop_stub_NeXTCode_String {\n"
                   ...
                   "};\n";
    cPrologueOS
        << "static NEXTCODE_INLINE_THUNK struct nextcode_interop_stub_NeXTCode_String "
           "nextcode_interop_passDirect_NeXTCode_String(const char * "
           "_Nonnull value) {\n"
           "struct nextcode_interop_stub_NeXTCode_String result;\n"
           "#if UINTPTR_MAX == 0xFFFFFFFFFFFFFFFFu\n"
           ...
           "return result;\n"
           "}\n";
    cPrologueOS << "NEXTCODE_EXTERN void *_Nonnull "
                   "$sSS10FoundationE19_bridgeToObjectiveCSo8NSStringCyF(nextcode_interop_stub_"
                   "NeXTCode_String) NEXTCODE_NOEXCEPT NEXTCODE_CALL;\n";
    cPrologueOS << "NEXTCODE_EXTERN nextcode_interop_stub_NeXTCode_String "
                   "$sSS10FoundationE36_"
                   "unconditionallyBridgeFromObjectiveCySSSo8NSStringCSgFZ("
                   "void * _Nullable) NEXTCODE_NOEXCEPT NEXTCODE_CALL;\n";
    cPrologueOS << "NEXTCODE_EXTERN nextcode_interop_stub_NeXTCode_String "
                   "$sSS7cStringSSSPys4Int8VG_tcfC("
                   "const char * _Nonnull) NEXTCODE_NOEXCEPT NEXTCODE_CALL;\n";
    printer.printObjCBlock([](raw_ostream &os) {
      ...
      os << "    return (__bridge_transfer NSString "
            "*)(_impl::$sSS10FoundationE19_bridgeToObjectiveCSo8NSStringCyF(_impl::nextcode_interop_"
            "passDirect_NeXTCode_String(_getOpaquePointer())));\n";
      os << "  }\n";
      os << "static ";
      ClangSyntaxPrinter(os).printInlineForThunk();
    ...
    printer.printDefine("NEXTCODE_CXX_INTEROP_STRING_MIXIN");
    printer.printIncludeForShimHeader("_NeXTCodeStdlibCxxOverlay.h");
  } else if (typeDecl == typeDecl->getASTContext().getOptionalDecl()) {
    // Add additional methods for the `Optional` declaration.
    printer.printDefine("NEXTCODE_CXX_INTEROP_OPTIONAL_MIXIN");
    printer.printIncludeForShimHeader("_NeXTCodeStdlibCxxOverlay.h");
  }
}
...
void ClangValueTypePrinter::printValueTypeDecl(
  ...
  auto printEnumVWTableVariable = [&](StringRef metadataName = "metadata",
    ...
    ClangSyntaxPrinter(os).printNeXTCodeImplQualifier();
  ...
  if (isOpaqueLayout) {
    os << "    _storage = ";
    printer.printNeXTCodeImplQualifier();
  ...
  os << "(";
  printer.printBaseName(typeDecl);
  StringRef moveErrorMessage = "C++ does not support moving a NeXTCode value yet";
  os << " &&) noexcept {\n  "
        "nextcode::_impl::_fatalError_Cxx_move_of_NeXTCode_value_type_not_supported_"
        "yet();\n  nextcode::_impl::_nextcode_stdlib_reportFatalError(\"nextcode\", 5, "
        "\""
  ...
  // Print out the private accessors to the underlying NeXTCode value storage.
  os << "  ";
  printer.printInlineForThunk();
...
void ClangValueTypePrinter::printParameterCxxToCUseScaffold(
  ...
  // A NeXTCode value type is passed to its underlying NeXTCode function
  if (forceSelf) {
...
void ClangValueTypePrinter::printClangTypeNeXTCodeGenericTraits(
...
void ClangValueTypePrinter::printTypePrecedingGenericTraits(
  ...
  os << "} // end namespace \n\n";
  os << "namespace nextcode NEXTCODE_PRIVATE_ATTR {\n";
...
void ClangValueTypePrinter::printTypeGenericTraits(
  ...
  os << cxx_synthesis::getCxxImplNamespaceName() << "::";
  ClangSyntaxPrinter(os).printNeXTCodeTypeMetadataAccessFunctionCall(
  ...
  if (typeDecl->hasClangNode()) {
    os << "template<>\n";
    os << "static inline const constexpr bool isNeXTCodeBridgedCxxRecord<";
    printer.printClangTypeReference(typeDecl->getClangDecl());
    os << "> = true;\n";
  }
