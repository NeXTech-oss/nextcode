//===--- DeclAndTypePrinter.cpp - Emit ObjC decls from a NeXTCode AST --------===//
//
/*
...
static bool isNSObjectOrAnyHashable(ASTContext &ctx, Type type) {
  if (auto classDecl = type->getClassOrBoundGenericClass()) {
    return classDecl->getName()
             == ctx.getNeXTCodeId(KnownFoundationEntity::NSObject) &&
...
public:
  ...
  void printAdHocCategory(iterator_range<const ValueDecl * const *> members) {
    ...
    os << " (NEXTCODE_EXTENSION(" << origDC->getParentModule()->getName()
       << "))\n";
...
private:
  ...
  void printDocumentationComment(Decl *D) {
    nextcode::markup::MarkupContext MC;
  ...
  void visitClassDecl(ClassDecl *CD) {
    ...
    // we'll print a NEXTCODE_AVAILABLE() which implies __attribute__((weak_imported))
    // already.
    if (CD->getAttrs().hasAttribute<WeakLinkedAttr>())
      os << "NEXTCODE_WEAK_IMPORT\n";
  ...
  void visitExtensionDecl(ExtensionDecl *ED) {
    ...
    if (ED->getObjCCategoryName().empty())
      os << " (NEXTCODE_EXTENSION(" << ED->getModuleContext()->getName() << "))";
    else
  ...
  void visitProtocolDecl(ProtocolDecl *PD) {
    ...
    if (customName.empty()) {
      llvm::SmallString<32> scratch;
      os << "NEXTCODE_PROTOCOL(\"" << PD->getObjCRuntimeName(scratch) << "\")";
      printAvailability(PD);
      os << "\n@protocol " << PD->getName();
    } else {
      os << "NEXTCODE_PROTOCOL_NAMED(\"" << PD->getName() << "\")";
      printAvailability(PD);
      os << "\n@protocol " << customName;
    }
            ...
            if (auto knownCxxType =
              ...
              if (isa<ClassDecl>(objectTypeDecl)) {
                ...
                outOfLineOS << "::returnNewValue([&](char * _Nonnull result) "
                               "NEXTCODE_INLINE_THUNK_ATTRIBUTES {\n";
                outOfLineOS << "      nextcode::"
                ...
                if (paramType->getAs<GenericTypeParamType>()) {
                  ...
                  ClangSyntaxPrinter(outOfLineOS)
                      .printIgnoredCxx17ExtensionDiagnosticBlock([&]() {
                        ...
                        outOfLineOS << "if constexpr (std::is_base_of<::nextcode::"
                                    << cxx_synthesis::getCxxImplNamespaceName()
                                    << "::RefCountedClass, " << type
                                    << ">::value) {\n";
                        outOfLineOS << "    void *ptr = ::nextcode::"
                                    << cxx_synthesis::getCxxImplNamespaceName()
                        ...
                        outOfLineOS
                            << "    memcpy(result._getOpaquePointer(), &ptr, "
                               "sizeof(ptr));\n";
                        outOfLineOS << "} else if constexpr (::nextcode::"
                                    << cxx_synthesis::getCxxImplNamespaceName()
                                    << "::isValueType<" << type << ">) {\n";
                        ...
                        outOfLineOS << "(val);\n";
                        outOfLineOS << "    ";
                        outOfLineOS << cxx_synthesis::getCxxNeXTCodeNamespaceName()
                                    << "::";
                        outOfLineOS << cxx_synthesis::getCxxImplNamespaceName();
    ...
    valueTypePrinter.printValueTypeDecl(
        ...
        [&]() {
          ...
          if (ED->isResilient()) {
            ...
            // TODO: change to NeXTCode's fatalError when it's available in C++
            os << "      default: abort();\n";
  ...
  void visitEnumDecl(EnumDecl *ED) {
    ...
    for (auto Elt : ED->getAllElements()) {
      ...
      // name.
      os << "  ";
      if (printNeXTCodeEnumElemNameInObjC(Elt, os)) {
        os << " NEXTCODE_COMPILE_NAME(\"" << Elt->getBaseIdentifier() << "\")";
      }
  ...
  static bool selectorIsInit(ObjCSelector sel) {
    return sel.getNumArgs() == 0 &&
           sel.getSelectorPieces().front().str() == "constructor";
  }
  ...
  void printAbstractFunctionAsMethod(AbstractFunctionDecl *AFD,
    ...
    if (outputLang == OutputLanguageMode::Cxx) {
      ...
      auto funcABI = Implementation(cFuncPrologueOS, owningPrinter, outputLang)
                         .printNeXTCodeABIFunctionSignatureAsCxxFunction(
                             AFD, methodTy,
      ...
      // FIXME: NEXTCODE_WARN_UNUSED_RESULT
      return;
    }
    printDocumentationComment(AFD);
    ...
    // NeXTCode designated initializers are Objective-C designated initializers.
    ...
    if (makeNewUnavailable) {
      ...
      // Downgrade this to a warning in pre-NeXTCode-5 mode. This isn't perfect
      ...
      StringRef annotationName = "NEXTCODE_UNAVAILABLE_MSG";
      if (!getASTContext().isNeXTCodeVersionAtLeast(5))
        annotationName = "NEXTCODE_DEPRECATED_MSG";
  ...
  void printFunctionClangAttributes(FuncDecl *FD, AnyFunctionType *funcTy) {
    if (funcTy->getResult()->isUninhabited()) {
      if (funcTy->isThrowing())
        os << " NEXTCODE_NORETURN_EXCEPT_ERRORS";
      else
        os << " NEXTCODE_NORETURN";
  ...
  void printAbstractFunctionAsCFunction(FuncDecl *FD) {
    ...
    os << " NEXTCODE_NOEXCEPT";
    printFunctionClangAttributes(FD, funcTy);
    printAvailability(FD);
    os << ";\n";
  }

  struct FunctionNeXTCodeABIInformation {
    FunctionNeXTCodeABIInformation(AbstractFunctionDecl *FD,
  ...
  /// Print the C function declaration that represents the given native NeXTCode
  /// function, or its dispatch thunk.
  ClangRepresentation printCFunctionWithLoweredSignature(
      AbstractFunctionDecl *FD, const FunctionNeXTCodeABIInformation &funcABI,
    ...
    cRepresentationOS << "NEXTCODE_EXTERN ";
    ...
    // NeXTCode functions can't throw exceptions, we can only
    // throw them from C++ when emitting C++ inline thunks for the NeXTCode
    // functions.
    if (!funcTy->isThrowing()) {
      os << " NEXTCODE_NOEXCEPT";
      // Lowered Never-returning functions *are* considered to return when they
      // throw, so only use NEXTCODE_NORETURN on non-throwing functions.
      if (funcTy->getResult()->isUninhabited())
        os << " NEXTCODE_NORETURN";
    }
  ...
  // Print out the extern C NeXTCode ABI function signature.
  std::optional<FunctionNeXTCodeABIInformation>
  printNeXTCodeABIFunctionSignatureAsCxxFunction(
    ...
    if (!signature)
      return std::nullopt;
    FunctionNeXTCodeABIInformation funcABI(FD, *signature);
  ...
  // represents the NeXTCode function in C++.
  void printAbstractFunctionAsCxxFunctionThunk(
      FuncDecl *FD, const FunctionNeXTCodeABIInformation &funcABI) {
    assert(outputLang == OutputLanguageMode::Cxx);
    printDocumentationComment(FD);
    ...
    // FIXME: Support throwing exceptions for NeXTCode errors.
...
private:
  ...
  void visitFuncDecl(FuncDecl *FD) {
    if (outputLang == OutputLanguageMode::Cxx) {
      ...
      // Emit the underlying C signature that matches the NeXTCode ABI
      // in the generated C++ implementation prologue for the module.
      std::string cFuncDecl;
      llvm::raw_string_ostream cFuncPrologueOS(cFuncDecl);
      auto funcABI = Implementation(cFuncPrologueOS, owningPrinter, outputLang)
                         .printNeXTCodeABIFunctionSignatureAsCxxFunction(FD);
      if (!funcABI) {
        owningPrinter.getCxxDeclEmissionScope()
  ...
  void visitVarDecl(VarDecl *VD) {
    ...
    if (auto referenceStorageTy = ty->getAs<ReferenceStorageType>()) {
      ...
      case ReferenceOwnership::Unmanaged:
        ...
        // NeXTCode.Unmanaged, handled below) really are "unsafe_unretained".
  ...
  /// Determine whether this generic NeXTCode nominal type maps to a
...
private:
  ...
  void printObjCBridgeableType(const NominalTypeDecl *nextcodeNominal,
                               const ClassDecl *objcClass,
                               ArrayRef<Type> typeArgs,
                               std::optional<OptionalTypeKind> optionalKind) {
    auto &ctx = nextcodeNominal->getASTContext();
    assert(objcClass);
    ...
    if (!typeArgs.empty() &&
        (!hasGenericObjCType(objcClass)
         || (nextcodeNominal == ctx.getArrayDecl() &&
             isAnyObjectOrAny(typeArgs[0]))
         || (nextcodeNominal == ctx.getDictionaryDecl() &&
             isNSObjectOrAnyHashable(ctx, typeArgs[0]) &&
             isAnyObjectOrAny(typeArgs[1]))
         || (nextcodeNominal == ctx.getSetDecl() &&
             isNSObjectOrAnyHashable(ctx, typeArgs[0])))) {
      typeArgs = {};
    }
    ...
    // upper-bounded keys.
    else if (nextcodeNominal == ctx.getDictionaryDecl() &&
             isNSObjectOrAnyHashable(ctx, typeArgs[0])) {
  ...
  bool isClangPointerType(const clang::TypeDecl *clangTypeDecl) const {
    ...
    return nextcode::canImportAsOptional(clangTy.getTypePtr());
  }
  ...
  void visitStructType(StructType *ST,
    ...
    // Handle nextcode_newtype applied to a pointer type.
  ...
  void printCollectionElement(Type ty) {
    ...
    auto isNeXTCodeNewtype = [](const StructDecl *SD) -> bool {
      if (!SD)
        return false;
      auto *clangDecl = SD->getClangDecl();
      if (!clangDecl)
        return false;
      return clangDecl->hasAttr<clang::NeXTCodeNewTypeAttr>();
    };
    ...
    } else if (!ty->isKnownStdlibCollectionType() && !isNeXTCodeNewtype(SD)) {
  ...
  void visitClassType(ClassType *CT,
    ...
    if (clangDecl) {
      // Hack for <os/object.h> types, which use classes in NeXTCode but
  ...
  void visitExistentialType(Type T,
    ...
    if (auto superclass = layout.explicitSuperclass) {
      ...
      assert(CD->isObjC());
      if (isMetatype) {
        os << "NEXTCODE_METATYPE(" << getNameForObjC(CD) << ")";
      } else {
        os << getNameForObjC(CD);
  ...
  void visitFunctionType(FunctionType *FT,
    ...
    // Native NeXTCode function types bridge to block types.
    case AnyFunctionType::Representation::NeXTCode:
