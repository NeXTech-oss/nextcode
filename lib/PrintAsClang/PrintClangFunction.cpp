...
// native NeXTCode function/method.
...
public:
  ...
  void printInoutTypeModifier() {
    os << (languageMode == nextcode::OutputLanguageMode::Cxx ? " &"
                                                          : " * _Nonnull");
  }
...
static bool printDirectReturnOrParamCType(
  ...
  StringRef stubTypeName =
      isResultType ? "nextcode_interop_returnStub_" : "nextcode_interop_passStub_";
  ...
  auto printStub = [&](raw_ostream &os, StringRef stubName) {
    ...
    os << "// Stub struct to be used to pass/return values to/from NeXTCode "
          "functions.\n";
    os << "struct " << stubName << " {\n";
    ...
    if (isResultType) {
      // Emit a stub that returns a value directly from nextcodecc function.
      os << "static NEXTCODE_C_INLINE_THUNK void nextcode_interop_returnDirect_"
         << minimalStubName;
      ...
      // Emit a stub that is used to pass value type directly to nextcodecc
      // function.
      os << "static NEXTCODE_C_INLINE_THUNK struct " << stubName
         << " nextcode_interop_passDirect_" << minimalStubName;
...
/// Make adjustments to the NeXTCode parameter name in generated C++, to
...
ClangRepresentation DeclAndTypeClangFunctionPrinter::printFunctionSignature(
  ...
  if (FD->hasThrows() && outputLang == OutputLanguageMode::Cxx)
    functionSignatureOS << "nextcode::ThrowingResult<";
    ...
    signature.visitParameterList(
        [&](const LoweredFunctionSignature::IndirectResultValue
          ...
          if (indirectResult.hasSRet())
            functionSignatureOS << "NEXTCODE_INDIRECT_RESULT ";
        ...
        [&](const LoweredFunctionSignature::IndirectParameter &param) {
          ...
          if (param.getParamDecl().isSelfParameter())
            functionSignatureOS << "NEXTCODE_CONTEXT ";
          bool isConst =
        ...
        [&](const LoweredFunctionSignature::ContextParameter &) {
          emitNewParam();
          functionSignatureOS << "NEXTCODE_CONTEXT void * _Nonnull _ctx";
        },
        [&](const LoweredFunctionSignature::ErrorResultValue &) {
          emitNewParam();
          functionSignatureOS
              << "NEXTCODE_ERROR_RESULT void * _Nullable * _Nullable _error";
        });
    if (needsComma == false)
      ...
      if ((isa<StructDecl>(decl) || isa<EnumDecl>(decl))) {
        if (!directTypeEncoding.empty())
          os << cxx_synthesis::getCxxImplNamespaceName()
             << "::nextcode_interop_passDirect_" << directTypeEncoding << '(';
        if (decl->hasClangNode()) {
            if (!directTypeEncoding.empty())
...
void DeclAndTypeClangFunctionPrinter::printGenericReturnSequence(
  ...
  ClangSyntaxPrinter(os).printIgnoredCxx17ExtensionDiagnosticBlock([&]() {
    os << "  if constexpr (std::is_base_of<::nextcode::"
    ...
    os << ";\n";
    os << "  return ::nextcode::" << cxx_synthesis::getCxxImplNamespaceName()
       << "::implClassFor<" << resultTyName
       << ">::type::makeRetained(returnValue);\n";
    os << "  } else if constexpr (::nextcode::"
    ...
    os << "  } else if constexpr (::nextcode::"
       ...
       << "::isNeXTCodeBridgedCxxRecord<" << resultTyName << ">) {\n";
    if (!initializeWithTakeFromValue) {
...
void DeclAndTypeClangFunctionPrinter::printCxxThunkBody(
    const AbstractFunctionDecl *FD, const LoweredFunctionSignature &signature,
    StringRef nextcodeSymbolName, const NominalTypeDecl *typeDeclContext,
  ...
  if (dispatchInfo && !isStaticMethod) {
    ...
    case DispatchKindTy::IndirectVTableRelativeOffset:
      os << "void ***selfPtr_ = reinterpret_cast<void ***>( "
            "::nextcode::_impl::_impl_RefCountedClass::getOpaquePointer(*this));"
            "\n";
      ...
      if (auto ptrAuthDisc = dispatchInfo->getPointerAuthDiscriminator())
        os << " __ptrauth_nextcode_class_method_pointer(" << ptrAuthDisc->value
           << ')';
      os << " func;\n";
      os << "};\n";
    ...
    case DispatchKindTy::Thunk:
      nextcodeSymbolName = dispatchInfo->getThunkSymbolName();
      break;
    }
  }
  ...
  // Check if we need to copy any parameters that are consumed by NeXTCode,
  // to ensure that NeXTCode does not destroy the value that's owned by C++.
  // FIXME: Support non-copyable types here as well between C++ -> NeXTCode.
  ...
  auto printCallToCFunc = [&](std::optional<StringRef> additionalParam) {
    if (indirectFunctionVar)
      ...
      os << cxx_synthesis::getCxxImplNamespaceName() << "::" << nextcodeSymbolName;
    os << '(';
    ...
    if (auto *decl = resultTy->getNominalOrBoundGenericNominal()) {
      auto valueTypeReturnThunker = [&](StringRef resultPointerName) {
        if (auto directResultType = signature.getDirectResultType()) {
          ...
          os << cxx_synthesis::getCxxImplNamespaceName()
             << "::nextcode_interop_returnDirect_" << typeEncoding << '('
             << resultPointerName << ", ";
  ...
  if (hasThrows) {
    ...
    os << "    throw (nextcode::Error(opaqueError));\n";
    os << "#else\n";
    if (resultTy->isVoid()) {
      os << "    return nextcode::Expected<void>(nextcode::Error(opaqueError));\n";
      os << "#endif\n";
          ...
          *directResultType, resultTy, moduleContext, os, cPrologueOS,
          typeMapping, interopContext, [&]() {
            ...
            auto s = printClangFunctionReturnType(
                ...
                OutputLanguageMode::Cxx);
            os << ">(nextcode::Error(opaqueError));\n";
            os << "#endif\n";

            // Return the function result value if it doesn't throw.
            if (!resultTy->isVoid() && hasThrows) {
              os << "\n";
              os << "  return NEXTCODE_RETURN_THUNK(";
...
void DeclAndTypeClangFunctionPrinter::printCxxMethod(
    ...
    const LoweredFunctionSignature &signature, StringRef nextcodeSymbolName,
  ...
  printCxxThunkBody(FD, signature, nextcodeSymbolName, typeDeclContext,
...
void DeclAndTypeClangFunctionPrinter::printCxxPropertyAccessorMethod(
  ...
  os << " {\n";
  // FIXME: should it be objTy for resultTy?
  printCxxThunkBody(accessor, signature, nextcodeSymbolName, typeDeclContext,
...
void DeclAndTypeClangFunctionPrinter::printCxxSubscriptAccessorMethod(
  ...
  // FIXME: should it be objTy for resultTy?
  printCxxThunkBody(
      accessor, signature, nextcodeSymbolName, typeDeclContext,
...
ClangRepresentation DeclAndTypeClangFunctionPrinter::getTypeRepresentation(
    PrimitiveTypeMapping &typeMapping,
    NeXTCodeToClangInteropContext &interopContext, DeclAndTypePrinter &declPrinter,
