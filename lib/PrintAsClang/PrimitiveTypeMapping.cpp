...
#define MAP(NEXTCODE_NAME, CLANG_REPR, NEEDS_NULLABILITY)                         \
  mappedTypeNames[{ctx.StdlibModuleName, ctx.getIdentifier(#NEXTCODE_NAME)}] = {  \
      CLANG_REPR, std::optional<StringRef>(CLANG_REPR),                        \
      std::optional<StringRef>(CLANG_REPR), NEEDS_NULLABILITY}
#define MAP_C(NEXTCODE_NAME, OBJC_REPR, C_REPR, NEEDS_NULLABILITY)                \
  mappedTypeNames[{ctx.StdlibModuleName, ctx.getIdentifier(#NEXTCODE_NAME)}] = {  \
      OBJC_REPR, std::optional<StringRef>(C_REPR),                             \
      std::optional<StringRef>(C_REPR), NEEDS_NULLABILITY}
#define MAP_CXX(NEXTCODE_NAME, OBJC_REPR, C_REPR, CXX_REPR, NEEDS_NULLABILITY)    \
  mappedTypeNames[{ctx.StdlibModuleName, ctx.getIdentifier(#NEXTCODE_NAME)}] = {  \
      OBJC_REPR, std::optional<StringRef>(C_REPR),                             \
  ...
  MAP_CXX(Int, "NSInteger", "ptrdiff_t", "nextcode::Int", false);
  MAP_CXX(UInt, "NSUInteger", "size_t", "nextcode::UInt", false);
  ...
  mappedTypeNames[{ID_ObjectiveC, ctx.getIdentifier(nextcode::getNeXTCodeName(
...
#include "nextcode/ClangImporter/SIMDMappedTypes.def"
  static_assert(NEXTCODE_MAX_IMPORTED_SIMD_ELEMENTS == 4,
