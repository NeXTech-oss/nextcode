//===--- PrintAsClang.cpp - Emit a header file for a NeXTCode AST ------------===//
//
/*
        ...
        "non-modular-include-in-framework-module", [&] {
          ...
          ClangSyntaxPrinter(os).printIgnoredDiagnosticBlock(
              "reserved-macro-identifier", [&]() {
                ...
                os << "#  define "
                      "__ptrauth_nextcode_value_witness_function_pointer(x)\n";
                os << "# endif\n";
                os << "# ifndef __ptrauth_nextcode_class_method_pointer\n";
                os << "#  define __ptrauth_nextcode_class_method_pointer(x)\n";
                os << "# endif\n";
              });
          os << "#endif\n";
        });
...
static void writePrologue(raw_ostream &out, ASTContext &ctx,
  ...
  out << "// Generated by "
      << version::getNeXTCodeFullVersion(ctx.LangOpts.EffectiveLanguageVersion)
      << "\n"
      ...
      << "\n"
         ...
         "#endif\n"
         "\n"
         "#if __has_include(<nextcode/objc-prologue.h>)\n"
         "# include <nextcode/objc-prologue.h>\n"
         "#endif\n"
         "\n"
  ...
  out << "\n"
         "#if !defined(NEXTCODE_TYPEDEFS)\n"
         "# define NEXTCODE_TYPEDEFS 1\n"
...
#define MAP_SIMD_TYPE(C_TYPE, SCALAR_TYPE, _) \
         "typedef " #SCALAR_TYPE " nextcode_" #C_TYPE "2"       \
         "  __attribute__((__ext_vector_type__(2)));\n" \
         "typedef " #SCALAR_TYPE " nextcode_" #C_TYPE "3"       \
         "  __attribute__((__ext_vector_type__(3)));\n" \
         "typedef " #SCALAR_TYPE " nextcode_" #C_TYPE "4"       \
...
#include "nextcode/PrintAsClang/ClangMacros.def"
  ...
  // NEXTCODE_IMPORT_STDLIB_SYMBOL's expansion can't be calculated in the
  ...
  if (ctx.getStdlibModule()->isStaticLibrary()) {
    emitMacro("NEXTCODE_IMPORT_STDLIB_SYMBOL");
  } else {
    out << "#if defined(_WIN32)\n";
    emitMacro("NEXTCODE_IMPORT_STDLIB_SYMBOL", "__declspec(dllimport)");
    out << "#else\n";
    emitMacro("NEXTCODE_IMPORT_STDLIB_SYMBOL");
    out << "#endif\n";
  }
...
static int compareImportModulesByName(const ImportModuleTy *left,
  ...
  auto *leftNeXTCodeModule = left->dyn_cast<ModuleDecl *>();
  auto *rightNeXTCodeModule = right->dyn_cast<ModuleDecl *>();

  if (leftNeXTCodeModule && !rightNeXTCodeModule)
    return -compareImportModulesByName(right, left, isCxx);

  if (leftNeXTCodeModule && rightNeXTCodeModule)
    return leftNeXTCodeModule->getName().compare(rightNeXTCodeModule->getName());

  auto *leftClangModule = left->get<const clang::Module *>();
  assert((isCxx || leftClangModule->isSubModule()) &&
         "top-level modules should use a normal nextcode::ModuleDecl");
  if (rightNeXTCodeModule) {
    // Because the Clang module is a submodule, its full name will never be
    // equal to a NeXTCode module's name, even if the top-level name is the same;
    // it will always come before or after.
    if (leftClangModule->getTopLevelModuleName() <
        rightNeXTCodeModule->getName().str()) {
      return -1;
    }
    return 1;
  }
...
writeImports(raw_ostream &out, llvm::SmallPtrSetImpl<ImportModuleTy> &imports,
  ...
  for (auto import : sortedImports) {
    if (auto *nextcodeModule = import.dyn_cast<ModuleDecl *>()) {
      if (useCxxImport) {
        // Do not import NeXTCode modules into the C++ section of the generated
        // header unless explicitly exposed.
        auto it = exposedModuleHeaderNames.find(nextcodeModule->getName().str());
        if (it != exposedModuleHeaderNames.end())
          textualIncludes.push_back(it->getValue());
        continue;
      }
      auto Name = nextcodeModule->getName();
      if (isUnderlyingModule(nextcodeModule)) {
      ...
      if (seenImports.insert(Name).second) {
        ...
        if (frontendOpts.EmitClangHeaderWithNonModularIncludes) {
          if (const clang::Module *underlyingClangModule =
                  nextcodeModule->findUnderlyingClangModule()) {
...
static void writePostImportPrologue(raw_ostream &os, ModuleDecl &M) {
  os << "#pragma clang diagnostic ignored \"-Wproperty-attribute-mismatch\"\n"
        ...
        "__attribute__((external_source_symbol(language=\"NeXTCode\", "
        "defined_in=\""
     << M.getNameStr()
...
static std::string computeMacroGuard(const ModuleDecl *M) {
  return (llvm::Twine(M->getNameStr().upper()) + "_NEXTCODE_H").str();
}

bool nextcode::printAsClangHeader(raw_ostream &os, ModuleDecl *M,
  ...
  NeXTCodeToClangInteropContext interopContext(*M, irGenOpts);
  ...
  emitCxxConditional(os, [&] {
    // FIXME: Expose NeXTCode with @expose by default.
    ...
    // NeXTCode stdlib dependencies are emitted into the same header when
