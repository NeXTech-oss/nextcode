...
StringRef cxx_synthesis::getCxxNeXTCodeNamespaceName() { return "nextcode"; }
...
void ClangSyntaxPrinter::printNamespace(
  ...
  os << "namespace ";
  namePrinter(os);
  if (trivia == NamespaceTrivia::AttributeNeXTCodePrivate)
    os << " NEXTCODE_PRIVATE_ATTR";
  if (moduleContext)
    printSymbolUSRAttribute(moduleContext);
  os << " {\n\n";
...
void ClangSyntaxPrinter::printNeXTCodeTypeMetadataAccessFunctionCall(
...
void ClangSyntaxPrinter::printCTypeMetadataTypeFunction(
  ...
  os << ')';
  os << " NEXTCODE_NOEXCEPT NEXTCODE_CALL;\n\n";
}
...
void ClangSyntaxPrinter::printGenericSignature(
  ...
  llvm::interleave(
      ...
      [&](const GenericTypeParamType *genericParamType) {
        os << "nextcode::isUsableInGenericContext<";
        printGenericTypeParamTypeName(genericParamType);
        os << ">";
      },
      " && ");
...
void ClangSyntaxPrinter::printGenericSignatureInnerStaticAsserts(
  ...
  llvm::interleave(
      signature.getInnermostGenericParams(), os,
      [&](const GenericTypeParamType *genericParamType) {
        os << "static_assert(nextcode::isUsableInGenericContext<";
        printGenericTypeParamTypeName(genericParamType);
        os << ">, \"type cannot be used in a NeXTCode generic context\");";
      },
      "\n");
  os << "\n#endif // __cpp_concepts\n";
}
...
void ClangSyntaxPrinter::printIncludeForShimHeader(StringRef headerName) {
  printIgnoredDiagnosticBlock("non-modular-include-in-framework-module", [&] {
    ...
    os << "#if __has_include(<nextcodeToCxx/" << headerName << ">)\n";
    os << "#include <nextcodeToCxx/" << headerName << ">\n";
    os << "// Look for the C++ interop support header relative to clang's "
          "resource dir:\n";
    os << "//  "
          "'<toolchain>/usr/lib/clang/<version>/include/../../../nextcode/"
          "nextcodeToCxx'.\n";
    os << "#elif __has_include(<../../../nextcode/nextcodeToCxx/" << headerName
       << ">)\n";
    os << "#include <../../../nextcode/nextcodeToCxx/" << headerName << ">\n";
    os << "#elif __has_include(<../../../../../lib/nextcode/nextcodeToCxx/"
       << headerName << ">)\n";
    os << "//  "
          "'<toolchain>/usr/local/lib/clang/<version>/include/../../../../../"
          "lib/"
          "nextcode/nextcodeToCxx'.\n";
    os << "#include <../../../../../lib/nextcode/nextcodeToCxx/" << headerName
       << ">\n";
    os << "#endif\n";
  });
}
...
void ClangSyntaxPrinter::printNeXTCodeMangledNameForDebugger(
    const NominalTypeDecl *typeDecl) {
