...
public:
  ModuleWriter(raw_ostream &os, raw_ostream &prologueOS,
               ...
               NeXTCodeToClangInteropContext &interopContext, AccessLevel access,
  ...
  void emitReferencedClangTypeMetadata(const TypeDecl *typeDecl) {
    ...
    if (it.second)
      ClangValueTypePrinter::printClangTypeNeXTCodeGenericTraits(os, typeDecl, &M,
                                                              printer);
  }
  ...
  void write() {
    ...
    // already have in `nextcode::` namespace. Also skip `Error` protocol from
    ...
    for (const auto *vd : removedVDList) {
      ...
      // Emit an unavailable stub for a NeXTCode type.
...
void nextcode::printModuleContentsAsObjC(
    raw_ostream &os, llvm::SmallPtrSetImpl<ImportModuleTy> &imports,
    ModuleDecl &M, NeXTCodeToClangInteropContext &interopContext) {
...
EmittedClangHeaderDependencyInfo nextcode::printModuleContentsAsCxx(
  ...
  // Define the `NEXTCODE_SYMBOL` macro.
  os << "#ifdef NEXTCODE_SYMBOL\n";
  os << "#undef NEXTCODE_SYMBOL\n";
  os << "#endif\n";
  os << "#define NEXTCODE_SYMBOL(usrValue) NEXTCODE_SYMBOL_MODULE_USR(\"";
  ...
  if (M.isStdlibModule()) {
    ...
    os << "#ifndef NEXTCODE_CXX_INTEROP_HIDE_STL_OVERLAY\n";
    os << "#include <string>\n";
    os << "#endif\n";
    os << "#include <new>\n";
    ...
    // Ignore typos in NeXTCode stdlib doc comments.
    os << "#pragma clang diagnostic push\n";
    os << "#pragma clang diagnostic ignored \"-Wdocumentation\"\n";
  }

  os << "#ifndef NEXTCODE_PRINTED_CORE\n";
  os << "#define NEXTCODE_PRINTED_CORE\n";
  printNeXTCodeToClangCoreScaffold(interopContext, M.getASTContext(),
  ...
  ClangSyntaxPrinter(os).printNamespace(
      ...
      ClangSyntaxPrinter::NamespaceTrivia::AttributeNeXTCodePrivate, &M);
