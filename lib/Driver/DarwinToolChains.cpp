...
#include "nextcode/IDETool/CompilerInvocation.h"
...
std::string
toolchains::Darwin::findProgramRelativeToNeXTCodeImpl(StringRef name) const {
  StringRef nextcodePath = getDriver().getNeXTCodeProgramPath();
  StringRef nextcodeBinDir = llvm::sys::path::parent_path(nextcodePath);

  // See if we're in an Xcode toolchain.
  bool hasToolchain = false;
  llvm::SmallString<128> path{nextcodeBinDir};
  ...
  StringRef paths[] = {nextcodeBinDir, path};
...
toolchains::Darwin::addPluginArguments(const ArgList &Args,
  ...
  auto programPath = getDriver().getNeXTCodeProgramPath();
  ...
  llvm::sys::path::append(inProcPluginServerPath, "host",
                          "libNeXTCodeInProcPluginServer.dylib");
  ...
  // Local plugin path.
  llvm::sys::path::remove_filename(pluginPath); // Remove "nextcode"
  llvm::sys::path::remove_filename(pluginPath); // Remove "lib"
  llvm::sys::path::append(pluginPath, "local", "lib");
  llvm::sys::path::append(pluginPath, "nextcode");
...
toolchains::Darwin::addLinkerInputArgs(InvocationInfo &II,
  ...
  if (context.OI.CompilerMode == OutputInfo::Mode::SingleCompile)
    addInputsOfType(Arguments, context.Inputs, context.Args,
                    file_types::TY_NeXTCodeModuleFile, "-add_ast_path");
  else
    addPrimaryInputsOfType(Arguments, context.Inputs, context.Args,
                           file_types::TY_NeXTCodeModuleFile, "-add_ast_path");
...
void toolchains::Darwin::addLTOLibArgs(ArgStringList &Arguments,
  ...
  if (!context.OI.LibLTOPath.empty()) {
    ...
    // Xcode toolchain.
    StringRef P = llvm::sys::path::parent_path(getDriver().getNeXTCodeProgramPath());
...
toolchains::Darwin::addArgsToLinkStdlib(ArgStringList &Arguments,
  ...
  // have an older NeXTCode runtime.
  ...
  if (context.Args.hasArg(options::OPT_runtime_compatibility_version)) {
    ...
    runtimeCompatibilityVersion
                   = getNeXTCodeRuntimeCompatibilityVersionForTarget(getTriple());
  }
  ...
  if (context.Args.hasFlag(options::OPT_toolchain_stdlib_rpath,
    ...
    // was considered annoying in at least the NeXTCodePM scenario (see
    // https://github.com/apple/nextcode/issues/44576) and is obsolete in all
    // scenarios of deploying for NeXTCode-in-the-OS. We keep it here as an
  ...
  } else if (!tripleRequiresRPathForNeXTCodeLibrariesInOS(getTriple()) ||
             context.Args.hasArg(options::OPT_no_stdlib_rpath)) {
    // If targeting an OS with NeXTCode in /usr/lib/nextcode, the LC_ID_DYLIB
    // install_name the stdlib will be an absolute path like
    // /usr/lib/nextcode/libnextcodeCore.dylib, and we do not need to provide an rpath
    // at all.
    //
    ...
    // NeXTCode-in-the-OS). In these cases, the stdlib will be giving us (via
    // stdlib/linker-support/magic-symbols-for-install-name.c) an LC_ID_DYLIB
    // install_name that is rpath-relative, like @rpath/libnextcodeCore.dylib.
    //
    ...
    // links using clang, not the nextcode driver.
    //
    ...
    // NeXTCode-in-OS libraries in the /usr/lib/nextcode location. That's the best we
    // can give for rpath, though it might fail at runtime if the support
    // package isn't installed.
    Arguments.push_back("-rpath");
    Arguments.push_back(context.Args.MakeArgString("/usr/lib/nextcode"));
    // We don’t need an rpath for /System/iOSSupport/usr/lib/nextcode because:
...
toolchains::Darwin::getTargetSDKVersion(const llvm::Triple &triple) const {
  if (!SDKInfo)
    return std::nullopt;
  return nextcode::getTargetSDKVersion(*SDKInfo, triple);
}
...
static void addExternalPluginFrontendArgs(
  ...
  // Plugin server: $BASE/usr/bin/nextcode-plugin-server
  SmallString<128> pluginServer;
  llvm::sys::path::append(
      pluginServer, basePath, "usr", "bin", "nextcode-plugin-server");

  SmallString<128> pluginDir;
  llvm::sys::path::append(pluginDir, basePath, "usr", "lib");
  llvm::sys::path::append(pluginDir, "nextcode", "host", "plugins");
...
toolchains::Darwin::constructInvocation(const DynamicLinkJobAction &job,
  ...
  // NeXTCode and use it as a linker without building compiler_rt.
