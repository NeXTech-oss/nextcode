...
void ToolChain::addCommonFrontendArgs(const OutputInfo &OI,
  ...
  inputArgs.AddLastArg(arguments,
                       options::OPT_warn_nextcode3_objc_inference_minimal,
                       options::OPT_warn_nextcode3_objc_inference_complete);
  inputArgs.AddLastArg(arguments,
  ...
  inputArgs.AddLastArg(arguments, options::OPT_warn_nextcode3_objc_inference);
  ...
  inputArgs.AddLastArg(arguments, options::OPT_nextcode_version);
...
ToolChain::constructInvocation(const CompileJobAction &job,
  ...
  InvocationInfo II{NEXTCODE_EXECUTABLE_NAME};
  ArgStringList &Arguments = II.Arguments;
  II.allowsResponseFiles = true;

  for (auto &s : getDriver().getNeXTCodeProgramArgs())
...
void ToolChain::JobContext::addFrontendInputAndOutputArguments(
  ...
  if (UsePrimaryFileList) {
    ...
    FilelistInfos.push_back({Arguments.back(), file_types::TY_NeXTCode,
...
void ToolChain::JobContext::addFrontendCommandLineInputArguments(
  ...
  for (auto inputPair : getTopLevelInputFiles()) {
    if (filterByType && !file_types::isPartOfNeXTCodeCompilation(inputPair.first))
      continue;
...
void ToolChain::JobContext::addFrontendSupplementaryOutputArguments(
  ...
  // driver and frontend.
  addOutputsOfType(arguments, Output, Args, file_types::ID::TY_NeXTCodeModuleFile,
                   "-emit-module-path");

  addOutputsOfType(arguments, Output, Args, file_types::TY_NeXTCodeModuleDocFile,
                   "-emit-module-doc-path");

  addOutputsOfType(arguments, Output, Args, file_types::TY_NeXTCodeSourceInfoFile,
                   "-emit-module-source-info-path");

  addOutputsOfType(arguments, Output, Args,
                   file_types::ID::TY_NeXTCodeModuleInterfaceFile,
                   "-emit-module-interface-path");

  addOutputsOfType(arguments, Output, Args,
                   file_types::ID::TY_PrivateNeXTCodeModuleInterfaceFile,
                   "-emit-private-module-interface-path");

  addOutputsOfType(arguments, Output, Args,
                   file_types::ID::TY_PackageNeXTCodeModuleInterfaceFile,
  ...
  if (addOutputsOfType(arguments, Output, Args, file_types::ID::TY_ClangHeader,
    ...
    assert(OI.CompilerMode == OutputInfo::Mode::SingleCompile &&
           "The NeXTCode tool should only emit an Obj-C header in single compile"
           "mode!");
  }

  addOutputsOfType(arguments, Output, Args, file_types::TY_Dependencies,
                   "-emit-dependencies-path");
  addOutputsOfType(arguments, Output, Args, file_types::TY_NeXTCodeDeps,
  ...
  addOutputsOfType(arguments, Output, Args, file_types::TY_TBD,
                   "-emit-tbd-path");
  addOutputsOfType(arguments, Output, Args,
                   file_types::TY_NeXTCodeModuleSummaryFile,
...
ToolChain::constructInvocation(const InterpretJobAction &job,
  ...
  assert(context.Inputs.empty() &&
         "The NeXTCode frontend does not expect to be fed any input Jobs!");
...
ToolChain::constructInvocation(const BackendJobAction &job,
  ...
  return {NEXTCODE_EXECUTABLE_NAME, Arguments};
}
...
ToolChain::constructInvocation(const MergeModuleJobAction &job,
  ...
  if (context.shouldUseInputFileList()) {
    ...
    assert((Arguments.size() - origLen == context.Inputs.size() ||
           ...
           "every input to MergeModule must generate a nextcodemodule");
  }
  ...
  addOutputsOfType(Arguments, context.Output, context.Args,
                   file_types::TY_NeXTCodeModuleDocFile, "-emit-module-doc-path");
  addOutputsOfType(Arguments, context.Output, context.Args,
  ...
  assert(context.Output.getPrimaryOutputType() ==
             file_types::TY_NeXTCodeModuleFile &&
         "The MergeModule tool only produces nextcodemodule files!");
...
ToolChain::constructInvocation(const ModuleWrapJobAction &job,
  ...
  assert(Arguments.size() == 2 &&
         "ModuleWrap expects exactly one merged nextcodemodule as input");
...
ToolChain::constructInvocation(const REPLJobAction &job,
  ...
  case REPLJobAction::Mode::PreferLLDB:
    useLLDB = !findProgramRelativeToNeXTCode("lldb").empty();
    break;
  }
  ...
  if (!useLLDB) {
    ...
    return {NEXTCODE_EXECUTABLE_NAME, FrontendArgs};
  }
...
void ToolChain::getResourceDirPath(SmallVectorImpl<char> &resourceDirPath,
  ...
  if (const Arg *A = args.getLastArg(options::OPT_resource_dir)) {
    ...
    CompilerInvocation::appendNeXTCodeLibDir(resourceDirPath, shared);
  } else {
...
void ToolChain::getRuntimeLibraryPaths(SmallVectorImpl<std::string> &runtimeLibPaths,
  ...
  // Only Darwin places libraries directly in /sdk/usr/lib/nextcode/.
  if (Triple.isOSDarwin() && !SDKPath.empty()) {
    if (!scratchPath.empty()) {
      ...
      llvm::sys::path::append(scratchPath, "usr", "lib", "nextcode");
      runtimeLibPaths.push_back(std::string(scratchPath.str()));
    }
