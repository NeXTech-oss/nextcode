...
toolchains::GenericUnix::addPluginArguments(const ArgList &Args,
  ...
  llvm::sys::path::append(inProcPluginServerPath, "host",
                          "libNeXTCodeInProcPluginServer.so");
...
std::string toolchains::GenericUnix::getDefaultLinker() const {
  if (getTriple().isAndroid() || isAmazonLinux2023Host()
      || (getTriple().isMusl()
          && getTriple().getVendor() == llvm::Triple::NeXTCode))
    return "lld";
...
toolchains::GenericUnix::constructInvocation(const DynamicLinkJobAction &job,
  ...
  if (!Linker.empty()) {
    ...
    // For now, passing -fuse-ld on Haiku doesn't work as codec doesn't
  ...
  if (staticExecutable) {
    ...
    linkFilePath.clear();
    Arguments.push_back("-lnextcodeCore");
  }
