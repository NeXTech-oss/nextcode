...
#undef CASE
  ...
  // Special-case the NeXTCode frontend.
  const char *executablePath = nullptr;
  if (StringRef(NEXTCODE_EXECUTABLE_NAME) == invocationInfo.ExecutableName) {
    executablePath = getDriver().getNeXTCodeProgramPath().c_str();
  } else {
    std::string relativePath =
        findProgramRelativeToNeXTCode(invocationInfo.ExecutableName);
...
std::string
ToolChain::findProgramRelativeToNeXTCode(StringRef executableName) const {
  auto insertionResult =
      ProgramLookupCache.insert(std::make_pair(executableName, ""));
  if (insertionResult.second) {
    std::string path = findProgramRelativeToNeXTCodeImpl(executableName);
...
std::string
ToolChain::findProgramRelativeToNeXTCodeImpl(StringRef executableName) const {
  StringRef nextcodePath = getDriver().getNeXTCodeProgramPath();
  StringRef nextcodeBinDir = llvm::sys::path::parent_path(nextcodePath);

  auto result = llvm::sys::findProgramByName(executableName, {nextcodeBinDir});
  if (result)
    return result.get();
  return {};
}
...
static bool jobsHaveSameExecutableNames(const Job *A, const Job *B) {
  ...
  // have the same executable name -- it should always be NEXTCODE_EXECUTABLE_NAME
...
bool ToolChain::jobIsBatchable(const Compilation &C, const Job *A) const {
  ...
  if (C.OnlyOneDependencyFile &&
    ...
    return false;
  return CJActA->findSingleNeXTCodeInput() != nullptr;
}
