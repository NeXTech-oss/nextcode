...
#include "nextcode/Basic/OutputFileMap.h"
...
Driver::Driver(StringRef DriverExecutable,
  ...
  : Opts(createNeXTCodeOptTable()), Diags(Diags),
...
void Driver::parseDriverKind(ArrayRef<const char *> Args) {
  ...
  std::optional<DriverKind> Kind =
      llvm::StringSwitch<std::optional<DriverKind>>(DriverName)
          .Case("nextcode", DriverKind::Interactive)
          .Case("codec", DriverKind::Batch)
          .Case("nextcode-legacy-driver", DriverKind::Interactive)
          .Case("codec-legacy-driver", DriverKind::Batch)
          ...
          .Case("nextcode-dependency-tool", DriverKind::NeXTCodeDependencyTool)
          .Case("nextcode-llvm-opt", DriverKind::NeXTCodeLLVMOpt)
          .Case("nextcode-autolink-extract", DriverKind::AutolinkExtract)
          .Case("nextcode-indent", DriverKind::NeXTCodeIndent)
          .Case("nextcode-symbolgraph-extract", DriverKind::SymbolGraph)
          .Case("nextcode-api-digester", DriverKind::APIDigester)
          .Case("nextcode-cache-tool", DriverKind::CacheTool)
          .Case("nextcode-parse-test", DriverKind::ParseTest)
          .Default(std::nullopt);
...
static void validateDebugInfoArgs(DiagnosticEngine &diags,
  ...
  if (args.hasArg(options::OPT_verify_debug_info)) {
    Arg *debugOpt = args.getLastArg(nextcode::options::OPT_g_Group);
    if (!debugOpt || debugOpt->getOption().matches(nextcode::options::OPT_gnone)) {
...
createStatsReporter(const llvm::opt::InputArgList *ArgList,
  ...
  return std::make_unique<UnifiedStatsReporter>("nextcode-driver",
                                                 OI.ModuleName,
                                                 InputName,
                                                 DefaultTargetTriple,
                                                 OutputType,
    ...
    for (const Arg *A : *ArgList)
      if (A->getOption().hasFlag(UnsupportedFlag))
        Diags.diagnose(SourceLoc(), diag::error_unsupported_option,
            ...
            UnsupportedFlag == options::NoBatchOption ? "nextcode" : "codec");
...
void Driver::buildInputs(const ToolChain &TC,
  ...
  for (Arg *A : Args) {
    if (A->getOption().getKind() == Option::InputClass) {
      ...
      if (Value == "-") {
        // By default, treat stdin as NeXTCode input.
        Ty = file_types::TY_NeXTCode;
      } else {
...
static void diagnoseOutputModeArg(DiagnosticEngine &diags, const Arg *arg,
  ...
  case options::OPT_i:
    diags.diagnose(SourceLoc(), diag::error_i_mode,
                   isInteractiveDriver ? driverName : "nextcode");
    break;
...
void Driver::buildOutputInfo(const ToolChain &TC, const DerivedArgList &Args,
  ...
  if (!OutputModeArg) {
    if (Args.hasArg(options::OPT_emit_module, options::OPT_emit_module_path)) {
      OI.CompilerOutputType = file_types::TY_NeXTCodeModuleFile;
...

  {
    if (const Arg *A = Args.getLastArg(options::OPT_sdk)) {
      ...
      if (TC.getTriple().isMacOSX()) {
        ...
        // This will prefer the SDK alongside the NeXTCode found by "xcrun nextcode".
...
void Driver::buildActions(SmallVectorImpl<const Action *> &TopLevelActions,
  ...
  case OutputInfo::Mode::StandardCompile: {
    ...
    for (const InputPair &Input : Inputs) {
      ...
      case file_types::TY_SIB: {
        ...
        assert(file_types::isPartOfNeXTCodeCompilation(InputType));
        if (Args.hasArg(options::OPT_embed_bitcode)) {
  ...
  case OutputInfo::Mode::SingleCompile: {
    ...
    if (Args.hasArg(options::OPT_embed_bitcode)) {
      ...
      for (const InputPair &Input : Inputs) {
        file_types::ID InputType = Input.first;
        if (!file_types::isPartOfNeXTCodeCompilation(InputType)) {
          HandledHere = false;
          break;
        }
      }
  ...
  if (OI.ShouldGenerateModule &&
    ...
    // so we generate a single nextcodemodule as output.
  ...
  if (OI.shouldLink() && !AllLinkerInputs.empty()) {
    ...
    for (const Action *A : AllLinkerInputs)
      if (A->getType() == file_types::TY_Object) {
        // Shared objects on ELF platforms don't have a nextcode1_autolink_entries
...
bool Driver::handleImmediateArgs(const ArgList &Args, const ToolChain &TC) {
  ...
  if (Args.hasArg(options::OPT_print_target_info)) {
    ...
    std::string executable = getNeXTCodeProgramPath();

    // FIXME(https://github.com/apple/nextcode/issues/54554): This bypasses mechanisms like -v and -###.
    sys::TaskQueue queue;
...
static StringRef getOutputFilename(Compilation &C,
  ...
  if (isa<MergeModuleJobAction>(JA)) {
    auto optFilename = getOutputFilenameFromPathArgOrAsTopLevel(
        OI, Args, options::OPT_emit_module_path, file_types::TY_NeXTCodeModuleFile,
...
Job *Driver::buildJobsForAction(Compilation &C, const JobAction *JA,
  ...
  if (OI.ShouldGenerateModule && isa<CompileJobAction>(JA))
    chooseNeXTCodeModuleOutputPath(C, OutputMap, workingDirectory, Output.get());

  if (OI.ShouldGenerateModule &&
      (isa<CompileJobAction>(JA) || isa<MergeModuleJobAction>(JA))) {
    chooseNeXTCodeModuleDocOutputPath(C, OutputMap, workingDirectory, Output.get());
    if (!C.getArgs().hasArg(options::OPT_avoid_emit_module_source_info)) {
      chooseNeXTCodeSourceInfoOutputPath(C, OutputMap, workingDirectory,
                                      Output.get());
    }
  }
...
void Driver::chooseNeXTCodeModuleOutputPath(Compilation &C,
                                         const TypeToPathMap *OutputMap,
                                         StringRef workingDirectory,
                                         CommandOutput *Output) const {

  if (hasExistingAdditionalOutput(*Output, file_types::TY_NeXTCodeModuleFile))
    return;

  StringRef OFMModuleOutputPath;
  if (OutputMap) {
    auto iter = OutputMap->find(file_types::TY_NeXTCodeModuleFile);
    if (iter != OutputMap->end())
  ...
  using file_types::TY_NeXTCodeModuleFile;

  if (!OFMModuleOutputPath.empty()) {
    // Prefer a path from the OutputMap.
    Output->setAdditionalOutputForType(TY_NeXTCodeModuleFile, OFMModuleOutputPath);
    ...
    Output->setAdditionalOutputForType(TY_NeXTCodeModuleFile, A->getValue());
  } else if (Output->getPrimaryOutputType() == TY_NeXTCodeModuleFile) {
    ...
    llvm::sys::path::replace_extension(
        Path, file_types::getExtension(TY_NeXTCodeModuleFile));
    Output->setAdditionalOutputForType(TY_NeXTCodeModuleFile, Path);
...
static void chooseModuleAuxiliaryOutputFilePath(
  ...
  if (!OFMOutputPath.empty()) {
    ...
    auto ModulePath = Output->getAnyOutputForType(file_types::TY_NeXTCodeModuleFile);
...
void Driver::chooseNeXTCodeSourceInfoOutputPath(Compilation &C,
                                             const TypeToPathMap *OutputMap,
                                             StringRef workingDirectory,
                                             CommandOutput *Output) const {
  chooseModuleAuxiliaryOutputFilePath(C, OutputMap, workingDirectory, Output,
                                      file_types::TY_NeXTCodeSourceInfoFile,
                                      /*shouldUseProjectFolder*/true,
                                      options::OPT_emit_module_source_info_path);
}

void Driver::chooseNeXTCodeModuleDocOutputPath(Compilation &C,
                                            const TypeToPathMap *OutputMap,
                                            StringRef workingDirectory,
                                            CommandOutput *Output) const {
  chooseModuleAuxiliaryOutputFilePath(C, OutputMap, workingDirectory, Output,
                                      file_types::TY_NeXTCodeModuleDocFile);
}
...
void Driver::chooseModuleInterfacePath(Compilation &C, const JobAction *JA,
  ...
  case OutputInfo::Mode::Immediate:
  case OutputInfo::Mode::REPL:
    llvm_unreachable("these modes aren't usable with 'codec'");
  }

  auto pathOpt = options::OPT_emit_module_interface_path;
  if (fileType == file_types::TY_PrivateNeXTCodeModuleInterfaceFile)
    pathOpt = options::OPT_emit_private_module_interface_path;
  else if (fileType == file_types::TY_PackageNeXTCodeModuleInterfaceFile)
...
void Driver::chooseModuleSummaryPath(Compilation &C,
  ...
  addAuxiliaryOutput(C, *Output, file_types::TY_NeXTCodeModuleSummaryFile,
...
void Driver::chooseLoadedModuleTracePath(Compilation &C,
  ...
  auto tracePathEnvVar = getenv("NEXTCODE_LOADED_MODULE_TRACE_FILE");
...
void Driver::printVersion(const ToolChain &TC, raw_ostream &OS) const {
  OS << version::getNeXTCodeFullVersion(
...
void Driver::printHelp(bool ShowHidden) const {
  ...
  case DriverKind::NeXTCodeDependencyTool:
  case DriverKind::NeXTCodeLLVMOpt:
  case DriverKind::AutolinkExtract:
  case DriverKind::NeXTCodeIndent:
  ...
  getOpts().printHelp(llvm::outs(), Name.c_str(), "NeXTCode compiler",
                      IncludedFlagsBitmask, ExcludedFlagsBitmask,
                      /*ShowAllAliases*/false);

  // These strings match the descriptions found in the corresponding nextcodepm 
  // help pages
  if (driverKind == DriverKind::Interactive) {
    llvm::outs() << "\nSEE ALSO - PACKAGE MANAGER COMMANDS: \n"
        "\t\"nextcode build\" Build sources into binary products \n"
        "\t\"nextcode package\" Perform operations on NeXTCode packages \n"
        "\t\"nextcode run\" Build and run an executable product \n"
        "\t\"nextcode test\" Build and run tests \n";
  } else {
    llvm::outs() << "\nSEE ALSO: nextcode build, nextcode run, nextcode package, " 
                    "nextcode test \n";
  }
