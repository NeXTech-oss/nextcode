...
//===----------------------------------------------------------------------===//
//
// This file defines the native NeXTCode ownership transfer conventions
...
#include "nextcode/SIL/AbstractionPatternGenerators.h"
...
static bool isClangTypeMoreIndirectThanSubstType(TypeConverter &TC,
  ...
  if (clangTy->isPointerType()
    ...
    // nextcode_newtypes are always passed directly
    if (auto typedefTy = clangTy->getAs<clang::TypedefType>()) {
      if (typedefTy->getDecl()->getAttr<clang::NeXTCodeNewTypeAttr>())
        return false;
    }
...
static bool isFormallyPassedIndirectly(TypeConverter &TC,
                                       ...
                                       CanType substType,
                                       const TypeLowering &substTL) {
  // If this is a native NeXTCode class that's passed directly to C/C++, treat it
  // as indirect.
  ...
  // If the C type of the argument is a const pointer, but the NeXTCode type
...
private:
  ...
  void visitTopLevelParams(AbstractionPattern origType,
    ...
    // pretend that there is a self parameter in the position NeXTCode
  ...
  bool maybeAddForeignSelfParameter() {
    ...
    if (ForeignSelf) {
      // This is a "self", but it's not a NeXTCode self, we handle it differently.
...
static CanSILFunctionType getSILFunctionType(
  ...
  if (substFnInterfaceType->getExtInfo().isAsync() && !foreignInfo.async) {
    assert(!origType.isForeign()
           && "using native NeXTCode async for foreign type!");
    isAsync = true;
  }
  ...
  // `throws` or if the abstraction pattern specifies a NeXTCode function type
  ...
  if (isThrowing && !foreignInfo.error &&
      !foreignInfo.async) {
    assert(!origType.isForeign()
           && "using native NeXTCode error convention for foreign type!");
...
/// The default NeXTCode conventions.
...
/// The default conventions for NeXTCode initializing constructors.
///
...
/// The default conventions for NeXTCode setter accessories.
///
...
CanSILFunctionType nextcode::getNativeSILFunctionType(
    TypeConverter &TC, TypeExpansionContext context,
...
buildThunkSignature(SILFunction *fn,
  ...
  auto *mod = fn->getModule().getNeXTCodeModule();
...
CanSILFunctionType nextcode::buildSILFunctionThunkType(
...
/// to leave to chance. NeXTCode functions starting with "constructor" are always
...
#include "nextcode/AST/AccessorKinds.def"
  ...
  /// Currently IRGen wraps alloc/init methods into NeXTCode constructors
  /// with NeXTCode conventions.
  case SILDeclRef::Kind::Allocator:
...
public:
  ...
  ResultConvention getResult(const TypeLowering &tl) const override {
    ...
    if (type->hasRetainablePointerRepresentation() ||
        (type->getNeXTCodeNewtypeUnderlyingType() && !tl.isTrivial()) ||
...
static CanSILFunctionType getUncachedSILFunctionTypeForConstant(
  ...
  // If we have a clang decl associated with the NeXTCode decl, derive its
  ...
  // The type of the native-to-foreign thunk for a nextcode closure.
...
TypeConverter::getLoweredFormalTypes(SILDeclRef constant,
  ...
  auto innerExtInfo =
    fnType->getExtInfo().withRepresentation(FunctionTypeRepresentation::NeXTCode);
...
static bool areABICompatibleParamsOrReturns(SILType a, SILType b,
  ...
  for (unsigned i : indices(aElements)) {
    ...
    if (inFunction) {
      ...
      auto *currentModule = inFunction->getModule().getNeXTCodeModule();
