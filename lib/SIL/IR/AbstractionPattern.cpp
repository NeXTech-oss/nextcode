...
TypeConverter::getAbstractionPattern(VarDecl *var, bool isNonObjC) {
  ...
  auto nextcodeType = sig.getReducedType(interfaceType);

  if (isNonObjC)
    return AbstractionPattern(sig, nextcodeType);

  if (auto clangDecl = var->getClangDecl()) {
    auto clangType = getClangType(clangDecl);
    auto contextType = var->getDeclContext()->mapTypeIntoContext(nextcodeType);
    nextcodeType =
        getLoweredBridgedType(AbstractionPattern(sig, nextcodeType, clangType),
                              contextType, getClangDeclBridgeability(clangDecl),
                              SILFunctionTypeRepresentation::CFunctionPointer,
                              TypeConverter::ForMemory)
            ->getCanonicalType();
    return AbstractionPattern(sig, nextcodeType, clangType);
  }
...
AbstractionPattern::getObjCMethodAsyncCompletionHandlerType(
  ...
  case Kind::ObjCMethodType: {
    ...
    return AbstractionPattern(patternSig,
                              nextcodeCompletionHandlerType, callbackParamTy);
  }
  case Kind::Opaque:
  case Kind::OpaqueFunction:
  case Kind::OpaqueDerivativeFunction:
  ...
  case Kind::CurriedCXXMethodType:
  case Kind::ObjCCompletionHandlerArgumentsType:
    nextcode_unreachable("not appropriate for this kind");
  }
...
CanType AbstractionPattern::getObjCMethodAsyncCompletionHandlerForeignType(
  ...
  // Bridge the NeXTCode completion handler type back to its
