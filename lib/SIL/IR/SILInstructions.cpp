...
#include "nextcode/Basic/AssertImplements.h"
...
AllocVectorInst *AllocVectorInst::create(SILDebugLocation Loc, SILValue capacity,
  ...
  auto size = totalSizeToAlloc<nextcode::Operand>(1 + typeDependentOperands.size());
...
AllocVectorInst *AllocVectorInst::createInInitializer(SILDebugLocation Loc,
                                                      ...
                                                      SILType elementType,
                                                      SILModule &M) {
  auto size = totalSizeToAlloc<nextcode::Operand>(1);
...
AllocPackInst *AllocPackInst::create(SILDebugLocation loc,
  ...
  auto size = totalSizeToAlloc<nextcode::Operand>(allOperands.size());
...
AllocRefInst *AllocRefInst::create(SILDebugLocation Loc, SILFunction &F,
  ...
  auto Size = totalSizeToAlloc<nextcode::Operand, SILType>(AllOperands.size(),
                                                        ElementTypes.size());
...
AllocBoxInst::create(SILDebugLocation Loc, CanSILBoxType BoxType,
  ...
  auto Sz = totalSizeToAlloc<nextcode::Operand, char>(TypeDependentOperands.size(),
                                                   Var ? Var->Name.size() : 0);
...
AllocExistentialBoxInst *AllocExistentialBoxInst::create(
  ...
  auto Size = totalSizeToAlloc<nextcode::Operand>(TypeDependentOperands.size());
...
BuiltinInst *BuiltinInst::create(SILDebugLocation Loc, Identifier Name,
                                 ...
                                 SILModule &M) {
  auto Size = totalSizeToAlloc<nextcode::Operand>(Args.size());
...
bool nextcode::doesApplyCalleeHaveSemantics(SILValue callee, StringRef semantics) {
...
SILType DifferentiableFunctionExtractInst::getExtracteeType(
  ...
  auto resultFnTy = originalFnTy->getAutoDiffDerivativeFunctionType(
      ...
      LookUpConformanceInModule(module.getNeXTCodeModule()));
...
SILType DifferentiabilityWitnessFunctionInst::getDifferentiabilityWitnessType(
  ...
  if (auto derivativeKind = witnessKind.getAsDerivativeFunctionKind()) {
    ...
    auto diffFnTy = fnTy->getAutoDiffDerivativeFunctionType(
        ...
        LookUpConformanceInModule(module.getNeXTCodeModule()), witnessCanGenSig,
...
StringLiteralInst::StringLiteralInst(SILDebugLocation Loc, StringRef Text,
  ...
  // It is undefined behavior to feed ill-formed UTF-8 into `NeXTCode.String`;
...
StringRef nextcode::getSILAccessKindName(SILAccessKind kind) {
  switch (kind) {
  case SILAccessKind::Init: return "constructor";
...
StringRef nextcode::getSILAccessEnforcementName(SILAccessEnforcement enforcement) {
  switch (enforcement) {
...
MarkFunctionEscapeInst::create(SILDebugLocation Loc,
  ...
  auto Size = totalSizeToAlloc<nextcode::Operand>(Elements.size());
...
UncheckedRefCastAddrInst::create(SILDebugLocation Loc, SILValue src,
  ...
  unsigned size =
      totalSizeToAlloc<nextcode::Operand>(2 + allOperands.size());
...
BorrowedFromInst *BorrowedFromInst::create(SILDebugLocation DebugLoc, SILValue borrowedValue,
  ...
  auto Size = totalSizeToAlloc<nextcode::Operand>(enclosingValues.size() + 1);
  auto Buffer = M.allocateInst(Size, alignof(StructInst));
...
TupleAddrConstructorInst *TupleAddrConstructorInst::create(
  ...
  auto Size = totalSizeToAlloc<nextcode::Operand>(Elements.size() + 1);
...
unsigned nextcode::getNumFieldsInNominal(NominalTypeDecl *decl) {
  unsigned count = 0;
...
VarDecl *nextcode::getIndexedField(NominalTypeDecl *decl, unsigned index) {
...
YieldInst *YieldInst::create(SILDebugLocation loc,
                             ...
                             SILFunction &F) {
  auto Size = totalSizeToAlloc<nextcode::Operand>(yieldedValues.size());
...
CondBranchInst::create(SILDebugLocation Loc, SILValue Condition,
  ...
  auto Size = totalSizeToAlloc<nextcode::Operand>(Args.size() + NumFixedOpers);
...
SwitchValueInst *SwitchValueInst::create(
  ...
  auto size = totalSizeToAlloc<nextcode::Operand, SILSuccessor>(numCases + 1,
                                                             numSuccessors);
...
SelectEnumInstBase<SELECT_ENUM_INST, BaseTy>::createSelectEnum(
  ...
  auto Size = SELECT_ENUM_INST::template
    totalSizeToAlloc<nextcode::Operand, EnumElementDecl*>(CaseValues.size() + 1,
                                                       CaseDecls.size());
...
ObjCMethodInst::create(SILDebugLocation DebugLoc, SILValue Operand,
  ...
  unsigned size =
      totalSizeToAlloc<nextcode::Operand>(1 + TypeDependentOperands.size());
...
InitExistentialMetatypeInst *InitExistentialMetatypeInst::create(
  ...
  unsigned size = totalSizeToAlloc<nextcode::Operand, ProtocolConformanceRef>(
...
OpenedExistentialAccess nextcode::getOpenedExistentialAccessFor(AccessKind access) {
  switch (access) {
...
PackLengthInst *PackLengthInst::create(SILFunction &F,
  ...
  size_t size =
    totalSizeToAlloc<nextcode::Operand>(typeDependentOperands.size());
  void *buffer =
...
UncheckedRefCastInst::create(SILDebugLocation DebugLoc, SILValue Operand,
  ...
  unsigned size = totalSizeToAlloc<nextcode::Operand>(1);
...
CheckedCastBranchInst *CheckedCastBranchInst::create(
  ...
  collectTypeDependentOperands(TypeDependentOperands, F, DestFormalTy);
  unsigned size =
      totalSizeToAlloc<nextcode::Operand>(3 + TypeDependentOperands.size());
