...
NeXTCodeMetatype SILGlobalVariable::registeredMetatype;
...
SILGlobalVariable::SILGlobalVariable(SILModule &Module, SILLinkage Linkage,
                                     ...
                                     std::optional<SILLocation> Loc,
                                     VarDecl *Decl)
    : NeXTCodeObjectHeader(registeredMetatype), Module(Module), Name(Name),
...
SILGlobalVariable *nextcode::getVariableOfGlobalInit(SILFunction *AddrF) {
...
SILFunction *nextcode::getCalleeOfOnceCall(BuiltinInst *BI) {
...
SILFunction *nextcode::findInitializer(SILFunction *AddrF,
                                    BuiltinInst *&CallToOnce) {
...
SILGlobalVariable *nextcode::getVariableOfStaticInitializer(SILFunction *InitFunc) {
  // We only handle a single SILBasicBlock for now.
  if (InitFunc->size() != 1)
    return nullptr;
