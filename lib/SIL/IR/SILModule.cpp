...
class SILModule::SerializationCallback final
  ...
  template <class T> void updateLinkage(T *decl) {
    ...
    case SILLinkage::PublicNonABI:
      ...
      // translation units in the same NeXTCode module.
...
SILModule::SILModule(llvm::PointerUnion<FileUnit *, ModuleDecl *> context,
  ...
  TheNeXTCodeModule = AssociatedDeclContext->getParentModule();
...
ASTContext &SILModule::getASTContext() const {
  return TheNeXTCodeModule->getASTContext();
}
...
// TODO: We should have an "isNoReturn" bit on NeXTCode's BuiltinInfo, but for
...
bool SILModule::isOptimizedOnoneSupportModule() const {
  return getOptions().shouldOptimize() &&
         getNeXTCodeModule()->isOnoneSupportModule();
}
...
bool SILModule::isStdlibModule() const {
  return TheNeXTCodeModule->isStdlibModule();
}
void SILModule::performOnceForPrespecializedImportedExtensions(
  ...
  // No prespecitalizations in embedded NeXTCode
  if (getASTContext().LangOpts.hasFeature(Feature::Embedded))
    return;

  SmallVector<ModuleDecl *, 8> importedModules;
  // Add the NeXTCode module.
  if (!isStdlibModule()) {
    auto *NeXTCodeStdlib = getASTContext().getStdlibModule(true);
    if (NeXTCodeStdlib)
      importedModules.push_back(NeXTCodeStdlib);
  }

  // Add explicitly imported modules.
  SmallVector<Decl *, 32> topLevelDecls;
  getNeXTCodeModule()->getTopLevelDecls(topLevelDecls);
  for (const Decl *D : topLevelDecls) {
    if (auto importDecl = dyn_cast<ImportDecl>(D)) {
      if (!importDecl->getModule() ||
          importDecl->getModule()->isNonNeXTCodeModule())
        continue;
...
SILLinkage nextcode::getDeclSILLinkage(const ValueDecl *decl) {
...
void nextcode::simple_display(llvm::raw_ostream &out, const SILModule *M) {
  if (!M) {
    out << "(null)";
    return;
  }
  out << "SIL for ";
  simple_display(out, M->getNeXTCodeModule());
}

SourceLoc nextcode::extractNearestSourceLoc(const SILModule *M) {
  if (!M)
    return SourceLoc();
  return extractNearestSourceLoc(M->getNeXTCodeModule());
}
