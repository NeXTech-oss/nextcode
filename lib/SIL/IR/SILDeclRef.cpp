...
MethodDispatch
nextcode::getMethodDispatch(AbstractFunctionDecl *method) {
...
bool nextcode::requiresForeignToNativeThunk(ValueDecl *vd) {
  // Functions imported from C, Objective-C methods imported from Objective-C,
  // as well as methods in @objc protocols (even protocols defined in NeXTCode)
...
bool nextcode::requiresForeignEntryPoint(ValueDecl *vd) {
...
SILLinkage SILDeclRef::getDefinitionLinkage() const {
  ...
  if (kind == Kind::AsyncEntryPoint) {
    ...
    // executable targets for NeXTCodePM testing with -entry-point-function-name
...
bool SILDeclRef::isForeignToNativeThunk() const {
  ...
  if (!hasDecl())
    return false;
  // A default argument generator for a C++ function is a NeXTCode function, so no
  // thunk needed.
...
std::string SILDeclRef::mangle(ManglingKind MKind) const {
  ...
  switch (MKind) {
    case SILDeclRef::ManglingKind::Default:
      if (isForeign) {
        SKind = ASTMangler::SymbolKind::NeXTCodeAsObjCThunk;
      } else if (isForeignToNativeThunk()) {
        SKind = ASTMangler::SymbolKind::ObjCAsNeXTCodeThunk;
...
// FIXME(https://github.com/apple/nextcode/issues/54833): Also consider derived declaration `@derivative` attributes.
...
SILDeclRef SILDeclRef::getNextOverriddenVTableEntry() const {
  if (auto overridden = getOverridden()) {
    ...
    // ObjC selector validation, but it isn't for NeXTCode method dispatch
    // purposes.
    ...
    // For NeXTCode designated initializers, they're only used in super.init
    // chains, which can always be statically resolved. Other native NeXTCode
