  ...
  if (!Decl->hasName()) {
    ...
    bool shouldEscape = !Decl->getBaseName().isSpecial() &&
        llvm::StringSwitch<bool>(Decl->getBaseName().userFacingName())
            // FIXME: Represent "constructor" by a special name and remove this case
...
void SILType::print(raw_ostream &OS, const PrintOptions &PO) const {
  ...
  // Print other types as their NeXTCode representation.
  //
...
#include "nextcode/AST/ReferenceStorage.def"
  ...
  void visitWitnessMethodInst(WitnessMethodInst *WMI) {
    ...
    QualifiedSILTypeOptions.CurrentModule = WMI->getModule().getNeXTCodeModule();
...
void SILModule::dump(const char *FileName, bool Verbose,
  ...
  print(Ctx, getNeXTCodeModule(), PrintASTDecls);
}
...
void SILModule::print(SILPrintContext &PrintCtx, ModuleDecl *M,
      ...
      if ((isa<ValueDecl>(D) || isa<OperatorDecl>(D) ||
        ...
        if (auto importDecl = dyn_cast<ImportDecl>(D)) {
          ...
          if (importName == BUILTIN_NAME ||
              ...
              importName == NEXTCODE_SHIMS_NAME)
            continue;
        }
        D->print(OS, Options);
        OS << "\n\n";
  ...
  printSILLinearMapTypes(PrintCtx, getNeXTCodeModule());
