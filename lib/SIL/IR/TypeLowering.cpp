...
CaptureKind TypeConverter::getDeclCaptureKind(CapturedValue capture,
  ...
  // its boxed form so we can obey NeXTCode's capture reference semantics.
...
TypeConverter::computeLoweredRValueType(TypeExpansionContext forExpansion,
  ...
  public:
    ...
    CanType visitType(CanType substType) {
      ...
      // The NeXTCode type directly corresponds to the lowered type.
      auto underlyingTy =
...
TypeConverter::getProtocolDispatchStrategy(ProtocolDecl *P) {
  // ObjC protocols use ObjC method dispatch, and NeXTCode protocols
  // use witness tables.
  if (P->isObjC())
