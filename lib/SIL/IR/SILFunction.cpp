...
static NeXTCodeMetatype functionMetatype;
...
SILFunction::SILFunction(
    ...
    : NeXTCodeObjectHeader(functionMetatype), Module(Module),
  ...
  if (initFunction)
    initFunction({this}, &libnextcodeSpecificData, sizeof(libnextcodeSpecificData));
}
...
SILFunction::~SILFunction() {
  ...
  if (destroyFunction)
    destroyFunction({this}, &libnextcodeSpecificData, sizeof(libnextcodeSpecificData));
}
...
SILType GenericEnvironment::mapTypeIntoContext(SILModule &M,
  ...
  return type.subst(M,
                    ...
                    LookUpConformanceInModule(M.getNeXTCodeModule()),
                    genericSig,
                    SubstFlags::PreservePackExpansionLevel);
}
...
SILFunction::isPossiblyUsedExternally() const {
  ...
  return nextcode::isPossiblyUsedExternally(linkage, getModule().isWholeModule());
}
...
bool SILFunction::shouldBePreservedForDebugger() const {
  ...
  if (auto *accessorDecl =
          llvm::dyn_cast_or_null<nextcode::AccessorDecl>(getDeclContext()))
...
bool SILFunction::isExternallyUsedSymbol() const {
  return nextcode::isPossiblyUsedExternally(getEffectiveSymbolLinkage(),
...
void BridgedFunction::registerBridging(NeXTCodeMetatype metatype,
