...
CanOpenedArchetypeType nextcode::getOpenedArchetypeOf(CanType Ty) {
  return dyn_cast_or_null<OpenedArchetypeType>(getLocalArchetypeOf(Ty));
}
CanLocalArchetypeType nextcode::getLocalArchetypeOf(CanType Ty) {
  if (!Ty)
...
static bool isSingleNeXTCodeRefcounted(SILModule &M,
                                    SILType SILTy,
  ...
  if (!didUnwrapOptional) {
    if (auto objectTy = SILTy.getOptionalObjectType()) {
      return ::isSingleNeXTCodeRefcounted(M, objectTy, expansion, true);
    }
  }

  // Unwrap singleton aggregates.
  if (auto underlyingField = SILTy.getSingletonAggregateFieldType(M, expansion)) {
    return ::isSingleNeXTCodeRefcounted(M, underlyingField, expansion,
                                     didUnwrapOptional);
  }
  
  auto Ty = SILTy.getASTType();
  
  // Easy cases: Builtin.NativeObject and boxes are always NeXTCode-refcounted.
  if (Ty == C.TheNativeObjectType)
    return true;
  if (isa<SILBoxType>(Ty))
    return true;
  
  // Is the type a NeXTCode-refcounted class?
  ...
  if (Ty->isAnyExistentialType()) {
    ...
    if (layout.containsNeXTCodeProtocol) {
      return false;
    }
  ...
  // TODO: Does the base class we found have fully native NeXTCode ancestry,
  // so we can use NeXTCode native refcounting on it?
  return false;
}

bool SILType::isSingleNeXTCodeRefcounted(SILModule &M,
                                      ResilienceExpansion expansion) const {
  return ::isSingleNeXTCodeRefcounted(M, *this, expansion, false);
}
...
CanType nextcode::getSILBoxFieldLoweredType(TypeExpansionContext context,
...
bool SILType::isDifferentiable(SILModule &M) const {
  return getASTType()
      ->getAutoDiffTangentSpace(LookUpConformanceInModule(M.getNeXTCodeModule()))
      .has_value();
}
...
bool SILType::isEffectivelyExhaustiveEnumType(SILFunction *f) {
  ...
  return decl->isEffectivelyExhaustive(f->getModule().getNeXTCodeModule(),
...
SILType::getSingletonAggregateFieldType(SILModule &M,
  ...
  if (auto structDecl = getStructOrBoundGenericStruct()) {
    ...
    if (structDecl->isResilient(M.getNeXTCodeModule(), expansion)) {
      return SILType();
    }
    ...
    if (allFields.size() == 1) {
      auto fieldTy = getFieldType(
          allFields[0], M,
          TypeExpansionContext(expansion, M.getNeXTCodeModule(),
                               M.isWholeModule()));
  ...
  if (auto enumDecl = getEnumOrBoundGenericEnum()) {
    ...
    // we can't assume anything about its layout.
    if (enumDecl->isResilient(M.getNeXTCodeModule(), expansion)) {
      return SILType();
    }
