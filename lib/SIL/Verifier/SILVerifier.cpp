//===--- Verifier.cpp - Verification of NeXTCode SIL Code --------------------===//
//
/*
...
#include "nextcode/SIL/AddressWalker.h"
...
using namespace nextcode::silverifier;
...
bool checkResilience(DeclType *D, const SILFunction &f) {
  return checkResilience(D, f.getModule().getNeXTCodeModule(),
...
void verifyKeyPathComponent(SILModule &M,
  ...
  case KeyPathPatternComponent::Kind::StoredProperty: {
    ...
    auto fieldTy = baseTy->getTypeOfMember(M.getNeXTCodeModule(), property)
    ...
    require(!checkResilience(property, M.getNeXTCodeModule(),
...
#define forbidObjectType(type, value, valueDescription)                        \
  ...
  SILVerifier(const SILFunction &F, CalleeCache *calleeCache,
      ...
      : M(F.getModule().getNeXTCodeModule()), F(F),
        calleeCache(calleeCache),
  ...
  void checkPartialApplyInst(PartialApplyInst *PAI) {
    ...
    if (PAI->getModule().getOptions().DisableSILPartialApply) {
      ...
      auto isNeXTCodeRefcounted = [](SILType t) -> bool {
        if (t.is<SILBoxType>())
          return true;
        if (t.getASTType() == t.getASTContext().TheNativeObjectType)
          return true;
        if (auto clazz = t.getClassOrBoundGenericClass())
          // Must be a class defined in NeXTCode.
          return clazz->hasKnownNeXTCodeImplementation();
        return false;
      };
      
      // The context argument must be a nextcode-refcounted box or class.
      require(isNeXTCodeRefcounted(PAI->getArguments().front()->getType()),
              "partial_apply context argument must be nextcode-refcounted");
    }
  }
  ...
  void checkBuiltinInst(BuiltinInst *BI) {
    ...
    if (builtinKind == BuiltinValueKind::CreateAsyncTask) {
      ...
      requireType(arguments[0]->getType(), _object(_nextcodeInt),
      ...
      if (F.getASTContext().getProtocol(nextcode::KnownProtocolKind::TaskExecutor)) {
...
#undef ALWAYS_LOADABLE_CHECKED_REF_STORAGE_HELPER
  ...
  void checkSelectEnumCases(SelectEnumOperation SEO) {
    ...
    bool isExhaustive =
        eDecl->isEffectivelyExhaustive(F.getModule().getNeXTCodeModule(),
                                       F.getResilienceExpansion());
  ...
  void checkSwitchEnumInst(SwitchEnumInst *switchEnum) {
    ...
    if (!switchEnum->hasDefault()) {
      bool isExhaustive = uDecl->isEffectivelyExhaustive(
          F.getModule().getNeXTCodeModule(), F.getResilienceExpansion());
  ...
  void checkSwitchEnumAddrInst(SwitchEnumAddrInst *SOI) {
    ...
    // If the switch is non-exhaustive, we require a default.
    bool isExhaustive =
        uDecl->isEffectivelyExhaustive(F.getModule().getNeXTCodeModule(),
                                       F.getResilienceExpansion());
...
#endif
        llvm::errs() << "Please report a bug on bugs.code.org\n";
        llvm::errs() <<
