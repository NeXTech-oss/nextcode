...
bool nextcode::calleesAreStaticallyKnowable(SILModule &module, SILDeclRef decl) {
  if (decl.isForeign)
    return false;
...
/// Are the callees that could be called through Decl statically
/// knowable based on the Decl and the compilation mode?
bool nextcode::calleesAreStaticallyKnowable(SILModule &module, ValueDecl *vd) {
...
CalleeList CalleeCache::getDestructors(SILType type, bool isExactType) const {
  ...
  if (isExactType || classDecl->isFinal()) {
    ...
    // In embedded NeXTCode, we need the specialized destructor, not the generic
    // one.
