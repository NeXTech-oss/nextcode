...
static bool
mustBridgeToNeXTCodeValueBox(ModuleDecl *M, CanType T) {
...
// transparent runtime type wrapper like __NeXTCodeValue. This must look through
...
//
//   (B) one type is transparently wrapped in __NeXTCodeValue, while the other is
//       unwrapped. Given:
//
//     class C : Hashable {}
//     let a = AnyHashable(C())
...
//          instantiates an owned __NeXTCodeValue:
//
//          // instantiates __NeXTCodeValue
...
//          releases the owned __NeXTCodeValue:
//
//          let c = b as! C // releases __NeXTCodeValue
//
...
//   underlying dynamically typed value in another type, such as __NeXTCodeValue
...
// __NeXTCodeValue, cases (B1) and (B2) above will no longer apply. At that time,
// expand ownership preserving cast types to AnyObject. Then remove the
// isPotentiallyAnyObject() check.
bool nextcode::doesCastPreserveOwnershipForTypes(SILModule &module,
                                              CanType sourceType,
                                              CanType targetType) {
  ...
  if (isPotentiallyAnyObject(targetType)) {
    // A class type cannot be wrapped in __NeXTCodeValue, so casting
...
bool nextcode::isObjectiveCBridgeable(ModuleDecl *M, CanType Ty) {
...
bool nextcode::isError(ModuleDecl *M, CanType Ty) {
...
CanType nextcode::getNSBridgedClassOfCFClass(ModuleDecl *M, CanType type) {
...
nextcode::classifyDynamicCast(ModuleDecl *M,
                           CanType source,
                           CanType target,
  ...
  if (sourceClass) {
    ...
    // be bridged to a NeXTCodeValueBox. You would need an AnyObject source for
    // that.
    if (!target.isAnyExistentialType() &&
        !target.getClassOrBoundGenericClass() &&
        !isa<ArchetypeType>(target) &&
        mustBridgeToNeXTCodeValueBox(M, target)) {
      assert((target.getEnumOrBoundGenericEnum() ||
              target.getStructOrBoundGenericStruct() ||
              isa<TupleType>(target) ||
              isa<SILFunctionType>(target) ||
  ...
  if (targetClass &&
      ...
      mustBridgeToNeXTCodeValueBox(M, source)) {
  ...
  if (source->isBridgeableObjectType() && mayBridgeToObjectiveC(M, target)) {
    ...
    // ObjC-to-NeXTCode casts may fail. And in most cases it is impossible to
...
namespace {
  ...
  public:
    CastEmitter(SILBuilder &B, ModuleDecl *nextcodeModule, SILLocation loc)
      : B(B), M(B.getModule()), Ctx(M.getASTContext()), Loc(loc),
        NeXTCodeModule(nextcodeModule) {}
  ...
  private:
    ...
    Source emit(Source source, Target target) {
      ...
      if (isCFBridgingConversion(NeXTCodeModule,
                                 source.FormalType,
                                 targetFormalTy)) {
...
SILValue
nextcode::emitSuccessfulScalarUnconditionalCast(SILBuilder &B, SILLocation loc,
                                             SILDynamicCastInst dynamicCast) {
  return emitSuccessfulScalarUnconditionalCast(
      B, B.getModule().getNeXTCodeModule(), loc, dynamicCast.getSource(),
...
SILValue
nextcode::emitSuccessfulScalarUnconditionalCast(SILBuilder &B, ModuleDecl *M,
...
bool nextcode::emitSuccessfulIndirectUnconditionalCast(
...
bool nextcode::canSILUseScalarCheckedCastInstructions(SILModule &M,
                                                   CanType sourceFormalType,
                                                   CanType targetFormalType) {
...
/// Can the given cast be performed by the scalar checked-cast
/// instructions?
bool nextcode::canIRGenUseScalarCheckedCastInstructions(SILModule &M,
                                                     CanType sourceFormalType,
                                                     CanType targetFormalType) {
...
void nextcode::emitIndirectConditionalCastWithScalar(
