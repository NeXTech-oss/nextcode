...
#include "nextcode/Basic/GraphNodeWorklist.h"
#include "nextcode/Basic/SmallPtrSetVector.h"
...
bool nextcode::findPointerEscape(SILValue original) {
...
bool nextcode::canOpcodeForwardInnerGuaranteedValues(SILValue value) {
...
bool nextcode::canOpcodeForwardInnerGuaranteedValues(Operand *use) {
...
bool nextcode::canOpcodeForwardOwnedValues(SILValue value) {
  if (auto *inst = value->getDefiningInstructionOrTerminator()) {
    if (auto fwdOp = ForwardingOperation(inst)) {
      return fwdOp.preservesOwnership() &&
...
bool nextcode::canOpcodeForwardOwnedValues(Operand *use) {
  if (auto fwdOp = ForwardingOperation(use->getUser()))
    return fwdOp.preservesOwnership() &&
           !fwdOp.canForwardGuaranteedCompatibleValuesOnly();
  return false;
...
bool nextcode::computeIsReborrow(SILArgument *arg) {
...
bool nextcode::computeIsScoped(SILArgument *arg) {
...
bool nextcode::computeIsGuaranteedForwarding(SILValue value) {
...
BorrowedFromInst *nextcode::getBorrowedFromUser(SILValue v) {
...
SILValue nextcode::lookThroughBorrowedFromUser(SILValue v) {
  if (BorrowedFromInst *bfi = getBorrowedFromUser(v))
    return bfi;
  return v;
}

SILValue nextcode::lookThroughBorrowedFromDef(SILValue v) {
...
bool nextcode::findInnerTransitiveGuaranteedUses(
...
bool nextcode::findExtendedUsesOfSimpleBorrowedValue(
...
bool nextcode::findUsesOfSimpleValue(SILValue value,
...
bool nextcode::visitGuaranteedForwardingPhisForSSAValue(
...
bool nextcode::findTransitiveGuaranteedUses(
    SILValue guaranteedValue, SmallVectorImpl<Operand *> &usePoints,
...
bool nextcode::getAllBorrowIntroducingValues(SILValue inputValue,
...
BorrowedValue nextcode::getSingleBorrowIntroducingValue(SILValue inputValue) {
  if (inputValue->getOwnershipKind() != OwnershipKind::Guaranteed)
    return {};
...
bool nextcode::getAllOwnedValueIntroducers(
...
OwnedValueIntroducer nextcode::getSingleOwnedValueIntroducer(SILValue inputValue) {
  if (inputValue->getOwnershipKind() != OwnershipKind::Owned)
    return {};
...
void nextcode::visitExtendedReborrowPhiBaseValuePairs(
...
void nextcode::visitExtendedGuaranteedForwardingPhiBaseValuePairs(
...
bool nextcode::visitForwardedGuaranteedOperands(
...
bool nextcode::visitEnclosingDefs(SILValue value,
                               function_ref<bool(SILValue)> visitor) {
...
bool nextcode::visitBorrowIntroducers(SILValue value,
                                   function_ref<bool(SILValue)> visitor) {
  if (isa<SILUndef>(value))
    return true;
  return FindEnclosingDefs(value->getFunction())
...
///      
bool nextcode::isInnerAdjacentPhi(SILArgument *innerPhiVal,
...
/// returns true.
bool nextcode::visitInnerAdjacentPhis(SILArgument *phi,
...
void nextcode::visitTransitiveEndBorrows(
    SILValue value,
...
bool nextcode::isNestedLexicalBeginBorrow(BeginBorrowInst *bbi) {
...
bool nextcode::isRedundantMoveValue(MoveValueInst *mvi) {
