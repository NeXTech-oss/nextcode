...
SILValue nextcode::lookThroughOwnershipInsts(SILValue v) {
  while (true) {
    switch (v->getKind()) {
    default:
      return v;
...
bool nextcode::visitNonOwnershipUses(SILValue value,
...
SILValue nextcode::lookThroughCopyValueInsts(SILValue val) {
  while (auto *cvi =
...
SILValue nextcode::getUnderlyingObject(SILValue v) {
  while (true) {
...
SILValue nextcode::stripSinglePredecessorArgs(SILValue V) {
  while (true) {
...
SILValue nextcode::stripCastsWithoutMarkDependence(SILValue v) {
  while (true) {
...
SILValue nextcode::stripCasts(SILValue v) {
  while (true) {
    v = stripSinglePredecessorArgs(v);
    if (auto *svi = dyn_cast<SingleValueInstruction>(v)) {
      if (isIdentityPreservingRefCast(svi) ||
...
SILValue nextcode::stripUpCasts(SILValue v) {
...
SILValue nextcode::stripClassCasts(SILValue v) {
  while (true) {
    if (auto *ui = dyn_cast<UpcastInst>(v)) {
      v = ui->getOperand();
      continue;
...
SILValue nextcode::stripAddressProjections(SILValue V) {
  while (true) {
    V = stripSinglePredecessorArgs(V);
...
SILValue nextcode::lookThroughAddressToAddressProjections(SILValue v) {
  while (true) {
    v = stripSinglePredecessorArgs(v);
...
SILValue nextcode::stripValueProjections(SILValue V) {
  while (true) {
    V = stripSinglePredecessorArgs(V);
...
SILValue nextcode::stripIndexingInsts(SILValue V) {
  while (true) {
...
SILValue nextcode::stripExpectIntrinsic(SILValue V) {
...
SILValue nextcode::stripBorrow(SILValue V) {
...
SingleValueInstruction *nextcode::getSingleValueCopyOrCast(SILInstruction *I) {
...
bool nextcode::isBeginScopeMarker(SILInstruction *user) {
  switch (user->getKind()) {
  default:
    return false;
  case SILInstructionKind::BeginAccessInst:
...
bool nextcode::isEndOfScopeMarker(SILInstruction *user) {
  switch (user->getKind()) {
  default:
    return false;
...
bool nextcode::mayCheckRefCount(SILInstruction *User) {
  return isa<IsUniqueInst>(User) || isa<IsEscapingClosureInst>(User) ||
         isa<BeginCOWMutationInst>(User);
}

bool nextcode::isSanitizerInstrumentation(SILInstruction *Instruction) {
...
bool nextcode::isInstrumentation(SILInstruction *Instruction) {
...
SILValue nextcode::isPartialApplyOfReabstractionThunk(PartialApplyInst *PAI) {
...
bool nextcode::onlyUsedByAssignByWrapper(PartialApplyInst *PAI) {
...
bool nextcode::onlyUsedByAssignOrInit(PartialApplyInst *PAI) {
...
static RuntimeEffect metadataEffect(SILType ty) {
  ...
  if (cl && !cl->hasKnownNeXTCodeImplementation())
    return RuntimeEffect::MetaData | RuntimeEffect::ObjectiveC;
  return RuntimeEffect::MetaData;
}

RuntimeEffect nextcode::getRuntimeEffect(SILInstruction *inst, SILType &impactType) {
...
/// NeXTCode closures that invoking the block will call. The StoredClosures may not
...
void nextcode::findClosuresForFunctionValue(
...
bool PolymorphicBuiltinSpecializedOverloadInfo::init(
  ...
  NEXTCODE_DEFER { isInitialized = true; };
...
SILValue
nextcode::getStaticOverloadForSpecializedPolymorphicBuiltin(BuiltinInst *bi) {
...
bool nextcode::visitExplodedTupleType(SILType inputType,
...
bool nextcode::visitExplodedTupleValue(
    SILValue inputValue,
