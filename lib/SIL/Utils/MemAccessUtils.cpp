...
SILValue nextcode::getTypedAccessAddress(SILValue address) {
...
SILValue nextcode::getAccessScope(SILValue address) {
  assert(address->getType().isAddress());
...
SILValue nextcode::getAccessBase(SILValue address) {
...
bool nextcode::isLetAddress(SILValue address) {
...
bool nextcode::mayAccessPointer(SILInstruction *instruction) {
...
bool nextcode::mayLoadWeakOrUnowned(SILInstruction *instruction) {
  assert(!FullApplySite::isa(instruction) && !isa<EndApplyInst>(instruction) &&
         !isa<AbortApplyInst>(instruction));
  if (isa<BuiltinInst>(instruction)) {
...
bool nextcode::maySynchronize(SILInstruction *instruction) {
  assert(!FullApplySite::isa(instruction) && !isa<EndApplyInst>(instruction) &&
         !isa<AbortApplyInst>(instruction));
  if (isa<BuiltinInst>(instruction)) {
    return instruction->mayReadOrWriteMemory();
...
bool nextcode::mayBeDeinitBarrierNotConsideringSideEffects(SILInstruction *instruction) {
...
bool nextcode::isIdentityPreservingRefCast(SingleValueInstruction *svi) {
...
bool nextcode::isIdentityAndOwnershipPreservingRefCast(
...
SILValue nextcode::findReferenceRoot(SILValue ref) {
...
SILValue nextcode::findOwnershipReferenceRoot(SILValue ref) {
  while (auto *svi = dyn_cast<SingleValueInstruction>(ref)) {
...
void nextcode::findGuaranteedReferenceRoots(SILValue referenceValue,
...
SILValue nextcode::findOwnershipReferenceAggregate(SILValue ref) {
  SILValue root = ref;
  while(true) {
...
bool nextcode::visitProductLeafAccessPathNodes(
...
bool nextcode::visitAccessPathUses(AccessUseVisitor &visitor,
                                AccessPath accessPath, SILFunction *function) {
  return AccessPathDefUseTraversal(visitor, accessPath, function).visitUses();
}

bool nextcode::visitAccessPathBaseUses(AccessUseVisitor &visitor,
                                    AccessPathWithBase accessPathWithBase,
                                    SILFunction *function) {
...
bool nextcode::visitAccessStorageUses(AccessUseVisitor &visitor,
                                   AccessStorage storage,
                                   SILFunction *function) {
...
bool nextcode::memInstMustInitialize(Operand *memOper) {
...
Operand *
nextcode::getSingleInitAllocStackUse(AllocStackInst *asi,
...
bool nextcode::isAddressForLocalInitOnly(SILValue sourceAddr) {
...
// module.
bool nextcode::isExternalGlobalAddressor(ApplyInst *AI) {
...
bool nextcode::isUnsafePointerExtraction(StructExtractInst *SEI) {
...
void nextcode::checkSwitchEnumBlockArg(SILPhiArgument *arg) {
...
bool nextcode::isPossibleFormalAccessStorage(const AccessStorage &storage,
                                          SILFunction *F) {
...
SILBasicBlock::iterator nextcode::removeBeginAccess(BeginAccessInst *beginAccess) {
...
void nextcode::visitAccessedAddress(SILInstruction *I,
                                 llvm::function_ref<void(Operand *)> visitor) {
