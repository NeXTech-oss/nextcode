//===--- NameLookup.cpp - NeXTCode Name Lookup Routines ----------------------===//
//
/*
...
using namespace nextcode::namelookup;
...
bool nextcode::removeOverriddenDecls(SmallVectorImpl<ValueDecl*> &decls) {
  if (decls.size() < 2)
    return false;
  ...
  for (auto decl : decls) {
    ...
    while (auto overrides = decl->getOverriddenDecl()) {
      ...
      // visibility than initializers written in NeXTCode classes, we can
...
static void recordShadowedDeclsAfterTypeMatch(
  ...
  for (unsigned firstIdx : indices(decls)) {
    ...
    for (unsigned secondIdx : range(firstIdx + 1, decls.size())) {
      ...
      // NeXTCode 4 compatibility hack: Don't shadow properties defined in
      // extensions of generic types with properties defined elsewhere.
      // This is due to the fact that in NeXTCode 4, we only gave custom overload
      // types to properties in extensions of generic types, otherwise we
      // used the null type.
      if (!ctx.isNeXTCodeVersionAtLeast(5) && isa<ValueDecl>(firstDecl)) {
      ...
      // library module.
      if (auto nextcodeModule = ctx.getStdlibModule()) {
        if ((firstModule == nextcodeModule) != (secondModule == nextcodeModule)) {
          // If the second module is the standard library module, the second
          // declaration is shadowed by the first.
          if (secondModule == nextcodeModule) {
            shadowed.insert(secondDecl);
            continue;
          }
      ...
      // treated as being ambiguous with NeXTCodeNIO's Data.withUnsafeBytes
...
bool nextcode::removeShadowedDecls(SmallVectorImpl<ValueDecl *> &decls,
                                const DeclContext *dc) {
  return removeShadowedDeclsImpl<ValueDecl *>(decls, dc);
}

bool nextcode::removeShadowedDecls(TinyPtrVector<OperatorDecl *> &decls,
                                const DeclContext *dc) {
#ifndef NDEBUG
...
bool nextcode::removeShadowedDecls(TinyPtrVector<PrecedenceGroupDecl *> &decls,
                                const DeclContext *dc) {
...
SelfBounds nextcode::getSelfBoundsFromWhereClause(
...
SelfBounds
nextcode::getSelfBoundsFromGenericSignature(const ExtensionDecl *extDecl) {
...
/// for fast retrieval.
class nextcode::MemberLookupTable : public ASTAllocated<nextcode::MemberLookupTable> {
...
class nextcode::ObjCMethodLookupTable
...
static bool shouldLoadMembersImmediately(ExtensionDecl *ext) {
  ...
  // Historically, NeXTCode forced these extensions to load their members
  // immediately, bypassing the module's NeXTCodeLookupTable. Using the
  // NeXTCodeLookupTable *ought* to work the same, but in practice it sometimes
  ...
  if (!Table.isLazilyComplete(name.getBaseName())) {
    ...
    if (isa_and_nonnull<clang::NamespaceDecl>(decl->getClangDecl())) {
      ...
      if (!allFound.empty()) {
        ...
        if (known != Table.end()) {
          auto nextcodeLookupResult = maybeFilterOutUnwantedDecls(
              known->second, name, includeAttrImplements,
              excludeMacroExpansions);
          for (auto foundNeXTCodeDecl : nextcodeLookupResult) {
            allFound.push_back(foundNeXTCodeDecl);
          }
        }
        return allFound;
      }
...
shouldDiagnoseConflict(NominalTypeDecl *ty, AbstractFunctionDecl *newDecl,
  ...
  // bridging header? Some code bases implement ObjC methods in NeXTCode even
...
resolveTypeDeclsToNominal(Evaluator &evaluator,
  ...
  for (auto typeDecl : typeDecls) {
    ...
    if (auto typealias = dyn_cast<TypeAliasDecl>(typeDecl)) {
      ...
      // Recognize NeXTCode.AnyObject directly.
...
CollectedOpaqueReprs nextcode::collectOpaqueTypeReprs(TypeRepr *r, ASTContext &ctx,
                                                   DeclContext *d) {
...
void nextcode::getDirectlyInheritedNominalTypeDecls(
    llvm::PointerUnion<const TypeDecl *, const ExtensionDecl *> decl,
...
nextcode::getDirectlyInheritedNominalTypeDecls(
    llvm::PointerUnion<const TypeDecl *, const ExtensionDecl *> decl,
    InvertibleProtocolSet &inverses, bool &anyObject) {
...
void nextcode::simple_display(llvm::raw_ostream &out, NLKind kind) {
  switch (kind) {
...
void nextcode::simple_display(llvm::raw_ostream &out, NLOptions options) {
