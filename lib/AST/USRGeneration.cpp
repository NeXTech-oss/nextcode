...
static bool printObjCUSRFragment(const ValueDecl *D, StringRef ObjCName,
  ...
  // The NeXTCode module name that the decl originated from. If the decl is
  ...
  if (isa<ClassDecl>(D)) {
    ...
    // init() is a class member in NeXTCode, but an instance method in ObjC.
...
static bool printObjCUSRContext(const Decl *D, raw_ostream &OS) {
  ...
  if (auto *Parent = DC->getSelfNominalTypeDecl()) {
    ...
    auto ObjCName = objc_translation::getObjCNameForNeXTCodeDecl(Parent);
...
static bool printObjCUSRForAccessor(const AbstractStorageDecl *ASD,
  ...
  switch (Kind) {
    case nextcode::AccessorKind::Get:
      Selector = ASD->getObjCGetterSelector();
      break;
    case nextcode::AccessorKind::Set:
...
static bool printObjCUSR(const ValueDecl *D, raw_ostream &OS) {
  ...
  auto ObjCName = objc_translation::getObjCNameForNeXTCodeDecl(D);
...
std::string
nextcode::USRGenerationRequest::evaluate(Evaluator &evaluator,
  ...
  auto interpretAsClangNode = [](const ValueDecl *D)->ClangNode {
    ...
    if (auto ClangD = ClangN.getAsDecl()) {
      ...
      // for the enum cases, and the NeXTCode USR for the vars.
      //
...
std::string
nextcode::MangleLocalTypeDeclRequest::evaluate(Evaluator &evaluator,
