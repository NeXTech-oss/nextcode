...
Type nextcode::Demangle::getTypeForMangling(ASTContext &ctx,
                                         StringRef mangling,
...
TypeDecl *nextcode::Demangle::getTypeDeclForMangling(ASTContext &ctx,
                                                  StringRef mangling,
                                                  GenericSignature genericSig) {
  Demangle::Context Dem;
  auto node = Dem.demangleSymbolAsNode(mangling);
...
TypeDecl *nextcode::Demangle::getTypeDeclForUSR(ASTContext &ctx,
                                             StringRef usr,
                                             GenericSignature genericSig) {
...
Type ASTBuilder::decodeMangledType(NodePointer node, bool forRequirement) {
  return nextcode::Demangle::decodeMangledType(*this, node, forRequirement)
      .getType();
}
...
Type ASTBuilder::createFunctionType(
  ...
  case FunctionMetadataConvention::NeXTCode:
    representation = FunctionTypeRepresentation::NeXTCode;
    break;
...
Type ASTBuilder::createImplFunctionType(
  ...
  #define SIMPLE_CASE(CASE) \
      case ImplFunctionDifferentiabilityKind::CASE: \
        diffKind = nextcode::DifferentiabilityKind::CASE; break;
  SIMPLE_CASE(NonDifferentiable)
  SIMPLE_CASE(Forward)
  SIMPLE_CASE(Reverse)
  SIMPLE_CASE(Normal)
...
Type ASTBuilder::createSILBoxTypeWithLayout(
  ...
  if (!genericTypeParams.empty()) {
    ...
    signature = nextcode::buildGenericSignature(Ctx,
                                             signature,
                                             genericTypeParams,
...
ModuleDecl *ASTBuilder::findModule(NodePointer node) {
  ...
  // mangled names. But don't touch anything under the NeXTCode stdlib's
  // umbrella.
...
CanGenericSignature ASTBuilder::demangleGenericSignature(
  ...
  NEXTCODE_DEFER { popGenericParams(); };
...
ASTBuilder::findTypeDecl(DeclContext *dc,
  ...
  // try the _Concurrency module, which is also mangled into the NeXTCode module.
