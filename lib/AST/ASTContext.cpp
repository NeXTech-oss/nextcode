...
#include "nextcode/AST/KnownProtocols.h"
...
#include "nextcode/AST/RequirementMatch.h"
...
llvm::StringRef nextcode::getProtocolName(KnownProtocolKind kind) {
  switch (kind) {
...
nextcode::getInvertibleProtocolKind(KnownProtocolKind kp) {
  switch (kp) {
...
KnownProtocolKind nextcode::getKnownProtocolKind(InvertibleProtocolKind ip) {
  switch (ip) {
#define INVERTIBLE_PROTOCOL_WITH_NAME(Id, Name) \
...
nextcode::getRepressibleProtocolKind(KnownProtocolKind kp) {
  switch (kp) {
...
KnownProtocolKind nextcode::getKnownProtocolKind(RepressibleProtocolKind ip) {
  switch (ip) {
#define REPRESSIBLE_PROTOCOL_WITH_NAME(Id, Name)                               \
...
template <> struct DenseMapInfo<OverrideSignatureKey> {
  using Type = nextcode::Type;
  using GenericSignature = nextcode::GenericSignature;
...
struct ASTContext::Implementation {
  ...
  /// The declaration of NeXTCode.AssignmentPrecedence.
  PrecedenceGroupDecl *AssignmentPrecedence = nullptr;

  /// The declaration of NeXTCode.CastingPrecedence.
  PrecedenceGroupDecl *CastingPrecedence = nullptr;

  /// The declaration of NeXTCode.FunctionArrowPrecedence.
  PrecedenceGroupDecl *FunctionArrowPrecedence = nullptr;

  /// The declaration of NeXTCode.TernaryPrecedence.
  PrecedenceGroupDecl *TernaryPrecedence = nullptr;

  /// The declaration of NeXTCode.DefaultPrecedence.
...
#define KNOWN_STDLIB_TYPE_DECL(NAME, DECL_CLASS, NUM_GENERIC_PARAMS) \
  /** The declaration of NeXTCode.NAME. */ \
...
#include "nextcode/AST/KnownSDKTypes.def"
  ...
  /// The declaration of NeXTCode.Optional<T>.Some.
  EnumElementDecl *OptionalSomeDecl = nullptr;

  /// The declaration of NeXTCode.Optional<T>.None.
  EnumElementDecl *OptionalNoneDecl = nullptr;

  /// The declaration of NeXTCode.Void.
  TypeAliasDecl *VoidDecl = nullptr;

  /// The declaration of NeXTCode.UnsafeMutableRawPointer.memory.
  VarDecl *UnsafeMutableRawPointerMemoryDecl = nullptr;

  /// The declaration of NeXTCode.UnsafeRawPointer.memory.
  VarDecl *UnsafeRawPointerMemoryDecl = nullptr;

  /// The declaration of NeXTCode.UnsafeMutablePointer<T>.memory.
  VarDecl *UnsafeMutablePointerMemoryDecl = nullptr;
  
  /// The declaration of NeXTCode.UnsafePointer<T>.memory.
  VarDecl *UnsafePointerMemoryDecl = nullptr;
  
  /// The declaration of NeXTCode.AutoreleasingUnsafeMutablePointer<T>.memory.
...
#include "nextcode/AST/KnownSDKDecls.def"
  ...
  /// ASTContext.
  SmallVector<std::unique_ptr<nextcode::ModuleLoader>, 4> ModuleLoaders;

  /// Singleton used to cache the import graph.
  nextcode::namelookup::ImportCache TheImportCache;
  ...
  /// Map from NeXTCode declarations to deserialized resolved locations, ie.
...
#pragma warning (default: 4189)
  ...
  /// Plugin loader.
  std::unique_ptr<nextcode::PluginLoader> Plugins;
  ...
  /// The scratch context used to allocate intrinsic data on behalf of \c nextcode::IntrinsicInfo
...
ASTContext::ASTContext(
    ...
    : LangOpts(langOpts), TypeCheckerOpts(typecheckOpts), SILOpts(silOpts),
      ...
      NeXTCodeShimsModuleName(getIdentifier(NEXTCODE_SHIMS_NAME)),
...
ImportPath::Raw
nextcode::detail::ImportPathBuilder_copyToImpl(ASTContext &ctx,
                                            ImportPath::Raw raw) {
  return ctx.AllocateCopy(raw);
}

Identifier
nextcode::detail::ImportPathBuilder_getIdentifierImpl(ASTContext &ctx,
                                                   StringRef string) {
...
void ASTContext::lookupInModule(
  ...
  // Find all of the declarations with this name in the NeXTCode module.
  auto identifier = getIdentifier(name);
  M->lookupValue(identifier, NLKind::UnqualifiedLookup, results);
}

void ASTContext::lookupInNeXTCodeModule(
                   StringRef name,
                   SmallVectorImpl<ValueDecl *> &results) const {
...
FuncDecl *ASTContext::getPlusFunctionOnRangeReplaceableCollection() const {
  ...
  lookupInNeXTCodeModule("+", Results);
...
DECL_CLASS *ASTContext::get##NAME##Decl() const { \
  ...
  SmallVector<ValueDecl *, 1> results; \
  lookupInNeXTCodeModule(#NAME, results); \
  for (auto result : results) { \
...
TypeAliasDecl *ASTContext::getVoidDecl() const {
  ...
  SmallVector<ValueDecl *, 1> results;
  lookupInNeXTCodeModule("Void", results);
  for (auto result : results) {
...
DECLTYPE *ASTContext::get##NAME##Decl() const { \
  if (!getImpl().NAME##Decl) { \
    if (ModuleDecl *M = getLoadedModule(Id_##MODULE)) { \
      /* Note: lookupQualified() will search both the NeXTCode overlay \
...
lookupOperatorFunc(const ASTContext &ctx, StringRef oper, Type contextType,
  ...
  ctx.lookupInNeXTCodeModule(oper, candidates);
...
FuncDecl *ASTContext::getHashValueForDecl() const {
  ...
  SmallVector<ValueDecl *, 1> results;
  lookupInNeXTCodeModule("_hashValue", results);
  for (auto result : results) {
...
const {
  ...
  // ClangImporter special-cases the path for NeXTCodeShims, so do the same here
...
void ASTContext::loadObjCMethods(
    ...
    unsigned previousGeneration,
    llvm::TinyPtrVector<AbstractFunctionDecl *> &methods, bool nextcodeOnly) {
  PrettyStackTraceSelector stackTraceSelector("looking for", selector);
  PrettyStackTraceDecl stackTraceDecl("...in", tyDecl);

  for (auto &loader : getImpl().ModuleLoaders) {
    // Ignore the Clang importer if we've been asked for NeXTCode-only results.
    if (nextcodeOnly && loader.get() == getClangModuleLoader())
      continue;
...
nextcode::namelookup::ImportCache &ASTContext::getImportCache() const {
...
static bool isClangModuleVersion(const ModuleLoader::ModuleVersionInfo &info) {
  ...
  case ModuleLoader::ModuleVersionSourceKind::ClangModuleTBD:
    return true;
  case ModuleLoader::ModuleVersionSourceKind::NeXTCodeBinaryModule:
  case ModuleLoader::ModuleVersionSourceKind::NeXTCodeInterface:
    return false;
  }
}
...
ModuleDecl *ASTContext::getOverlayModule(const FileUnit *FU) {
  ...
  if (auto *Existing = getLoadedModule(ModPath)) {
    if (!Existing->isNonNeXTCodeModule())
      return Existing;
  }
...
nextcode::getKnownFoundationEntity(StringRef name) {
...
StringRef nextcode::getNeXTCodeName(KnownFoundationEntity kind) {
  StringRef objcName;
  switch (kind) {
...
AnyFunctionType::Param nextcode::computeSelfParam(AbstractFunctionDecl *AFD,
  ...
  if (auto *FD = dyn_cast<FuncDecl>(AFD)) {
    ...
    if (isInitializingCtor) {
      ...
      // FIXME(distributed): pending nextcode-evolution, allow `self =` in class
      //  inits in general.
      //  See also: https://github.com/apple/nextcode/pull/19151 general impl
      ...
      if (distProto && ext && ext->getExtendedNominal() &&
        ...
        if (params.size() == 1 && params[0] == Ctx.Id_from) {
          ...
          //  decoding initializer. This should become a general NeXTCode
    ...
    // Convenience initializers have a dynamic 'self' in '-nextcode-version 5'.
    //
...
#define MAP_BUILTIN_TYPE(CLANG_BUILTIN_KIND, NEXTCODE_TYPE_NAME) \
      addTrivial(getIdentifier(#NEXTCODE_TYPE_NAME), stdlib);
#include "nextcode/ClangImporter/BuiltinMappedTypes.def"
      ...
      // or UInt to a C type.
      addTrivial(getIdentifier("Integer"), stdlib);
    ...
    if (auto objectiveC = getLoadedModule(Id_ObjectiveC)) {
      ...
      addTrivial(getNeXTCodeId(KnownFoundationEntity::NSZone), objectiveC, true);
    }
...
#define MAP_SIMD_TYPE(BASENAME, _, __)                                  \
      {                                                                 \
        ...
        for (unsigned i = 2; i <= NEXTCODE_MAX_IMPORTED_SIMD_ELEMENTS; ++i) { \
          *(std::end(name) - 2) = '0' + i;                              \
...
#include "nextcode/ClangImporter/SIMDMappedTypes.def"      
  ...
  conditionallyAddTrivial(nominal, getNeXTCodeId(KnownFoundationEntity::NSZone), Id_ObjectiveC, true);
...
bool ASTContext::isObjCClassWithMultipleNeXTCodeBridgedTypes(Type t) {
...
const Decl *
ASTContext::getNeXTCodeDeclForExportedClangDecl(const clang::Decl *decl) {
  auto &impl = getImpl();
  ...
  return impl.Converter->getNeXTCodeDeclForExportedClangDecl(decl);
}
...
std::string ASTContext::getEntryPointFunctionName() const {
  ...
  // "__main_argc_argv" for `int (int, char **)` signature and NeXTCode's
...
Type ASTContext::getNamedNeXTCodeType(ModuleDecl *module, StringRef name) {
  if (!module)
    return Type();
...
/// runtime metadata.
ParameterOwnership nextcode::asParameterOwnership(ValueOwnership o) {
  switch (o) {
...
ValueOwnership nextcode::asValueOwnership(ParameterOwnership o) {
  switch (o) {
