//===---- FineGrainedDependencyFormat.cpp - reading and writing nextcodedeps -===//
//
/*
...
public:
  ...
  bool readFineGrainedDependencyGraphFromNeXTCodeModule(SourceFileDepGraph &g);
};
...
bool Deserializer::readFineGrainedDependencyGraph(SourceFileDepGraph &g,
  ...
  case Purpose::ForNeXTCodeDeps:
    ...
    if (enterTopLevelBlock())
      return true;
    LLVM_FALLTHROUGH;
  case Purpose::ForNeXTCodeModule:
    // N.B. Incremental metadata embedded in nextcodemodule files does not have
...
bool nextcode::fine_grained_dependencies::readFineGrainedDependencyGraph(
    llvm::MemoryBuffer &buffer, SourceFileDepGraph &g) {
  Deserializer deserializer(buffer.getMemBufferRef());
  return deserializer.readFineGrainedDependencyGraph(g, Purpose::ForNeXTCodeDeps);
}
...
void nextcode::fine_grained_dependencies::writeFineGrainedDependencyGraph(
...
bool nextcode::fine_grained_dependencies::writeFineGrainedDependencyGraphToPath(
  ...
  return withOutputPath(diags, backend, path, [&](llvm::raw_ostream &out) {
    ...
    writeFineGrainedDependencyGraph(Writer, g, Purpose::ForNeXTCodeDeps);
...
bool nextcode::fine_grained_dependencies::
    readFineGrainedDependencyGraphFromNeXTCodeModule(llvm::MemoryBuffer &buffer,
                                                  SourceFileDepGraph &g) {
  Deserializer deserializer(buffer.getMemBufferRef());
  return deserializer.readFineGrainedDependencyGraphFromNeXTCodeModule(g);
}

bool Deserializer::readFineGrainedDependencyGraphFromNeXTCodeModule(
    SourceFileDepGraph &g) {
  ...
  while (!Cursor.AtEndOfStream()) {
    ...
    case INCREMENTAL_INFORMATION_BLOCK_ID: {
      ...
      if (readFineGrainedDependencyGraph(g, Purpose::ForNeXTCodeModule)) {
        break;
      }
