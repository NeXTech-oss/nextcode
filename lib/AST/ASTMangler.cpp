//===--- ASTMangler.cpp - NeXTCode AST symbol mangling -----------------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements declaration name mangling in NeXTCode.
//
//===----------------------------------------------------------------------===//
...
using namespace nextcode::Mangle;
...
void ASTMangler::appendSymbolKind(SymbolKind SKind) {
  switch (SKind) {
    ...
    case SymbolKind::NeXTCodeAsObjCThunk: return appendOperator("To");
    case SymbolKind::ObjCAsNeXTCodeThunk: return appendOperator("TO");
...
/// If the declaration is an @objc protocol defined in NeXTCode and the
...
getOverriddenNeXTCodeProtocolObjCName(const ValueDecl *decl,
...
void ASTMangler::appendDeclName(const ValueDecl *decl, DeclBaseName name) {
  ...
  if (synthesizedTypeAttr) {
    assert(!isDigit(synthesizedTypeAttr->originalTypeName[0]) &&
           "synthesized type's original name must be a valid NeXTCode identifier");
    ...
    switch (decl->getAttrs().getUnaryOperatorKind()) {
      ...
      case UnaryOperatorKind::None:
        ...
        break;
               ...
               getOverriddenNeXTCodeProtocolObjCName(decl, UseObjCRuntimeNames)) {
    // @objc NeXTCode protocols should be mangled as Objective-C protocols,
...
ASTMangler::getSpecialManglingContext(const ValueDecl *decl,
  ...
  // If @objc NeXTCode protocols should be mangled as Objective-C protocols,
  // they are defined in the Objective-C context.
  if (getOverriddenNeXTCodeProtocolObjCName(decl, useObjCProtocolNames))
    return ASTMangler::ObjCContext;
  ...
  // ClangImporterContext, even if an __attribute__((nextcode_name())) nests them
  // inside a NeXTCode type syntactically.
...
void ASTMangler::appendModule(const ModuleDecl *module,
  ...
  // _Concurrency and nextcode-syntax (which adds "Compiler" as a prefix when
  // building nextcode-syntax as part of the compiler).
  ...
  if ((RespectOriginallyDefinedIn ||
       (module->getName().str() != NEXTCODE_CONCURRENCY_NAME &&
        !module->getABIName().str().starts_with(
            NEXTCODE_MODULE_ABI_NAME_PREFIX))) &&
      module->getABIName() != module->getName())
    ModName = module->getABIName().str();

  // Try the special 'nextcode' substitution.
...
void ASTMangler::appendAnyGenericType(const GenericTypeDecl *decl,
  ...
  auto tryAppendClangName = [this, decl]() -> bool {
    ...
    if (UseObjCRuntimeNames && interface) {
      ...
      // imported as a NeXTCode decl with `__CxxTemplateInst...` name.
      ...
      // name of the NeXTCode decl which does include template arguments.
      appendIdentifier(nominal->getName().str());
    } else {
    ...
    // We continue to mangle "class" the standard NeXTCode way because it feels
...
bool ASTMangler::tryAppendStandardSubstitution(const GenericTypeDecl *decl) {
  // Bail out if our parent isn't the nextcode standard library.
...
void ASTMangler::appendProtocolConformanceRef(
  ...
  // Builtin conformances are always from the NeXTCode module.
