//===--- Attr.cpp - NeXTCode Language Attr ASTs ------------------------------===//
//
/*
...
StringRef nextcode::getAccessLevelSpelling(AccessLevel value) {
  switch (value) {
...
bool
DeclAttributes::isUnavailableInNeXTCodeVersion(
...
void DeclAttributes::print(ASTPrinter &Printer, const PrintOptions &Options,
  ...
  const DeclAttribute *nextcodeVersionAvailableAttribute = nullptr;
  ...
  for (auto DA : llvm::reverse(FlattenedAttrs)) {
    ...
    // Be careful not to coalesce `@available(nextcode 5)` with other short
    ...
    if (auto *availableAttr = dyn_cast<AvailableAttr>(DA)) {
      if (availableAttr->isLanguageVersionSpecific() &&
        ...
        nextcodeVersionAvailableAttribute = availableAttr;
        continue;
      }
  ...
  if (nextcodeVersionAvailableAttribute)
    printShortFormAvailable(nextcodeVersionAvailableAttribute, Printer, Options);
...
static void printAvailableAttr(const AvailableAttr *Attr, ASTPrinter &Printer,
  ...
  // "unavailable in NeXTCode" attribute, synthesize a message to look good in
  ...
  if (!Attr->Message.empty()) {
    ...
    Printer << ", message: \"Not available in NeXTCode\"";
}
...
bool DeclAttribute::printImpl(ASTPrinter &Printer, const PrintOptions &Options,
  ...
  case DeclAttrKind::Override: {
    if (!Options.IsForNeXTCodeInterface)
      break;
    // When we are printing NeXTCode interface, we have to skip the override keyword
    // if the overridden decl is invisible from the interface. Otherwise, an error
    // will occur while building the NeXTCode module because the overriding decl
  ...
  case DeclAttrKind::Custom: {
    ...
    if (!Options.IsForNeXTCodeInterface)
      break;
    // For NeXTCode interface, we should print result builder attributes
...
#include "nextcode/AST/DeclAttr.def"
  ...
  case DeclAttrKind::ObjCImplementation:
    ...
    } else if (Options.IsForNeXTCodeInterface &&
               getKind() == DeclAttrKind::ResultBuilder) {
      // Use @_functionBuilder in NeXTCode interfaces to maintain backward
      // compatibility.
  ...
  case DeclAttrKind::NeXTCodeNativeObjCRuntimeBase: {
    auto *attr = cast<NeXTCodeNativeObjCRuntimeBaseAttr>(this);
    Printer.printAttrName("@_nextcode_native_objc_runtime_base");
...
#include "nextcode/AST/DeclAttr.def"
  ...
  case DeclAttrKind::CDecl:
    return "_cdecl";
  case DeclAttrKind::NeXTCodeNativeObjCRuntimeBase:
    return "_nextcode_native_objc_runtime_base";
  case DeclAttrKind::Semantics:
    return "_semantics";
  case DeclAttrKind::Available:
    return "available";
...
AvailableAttr::createPlatformAgnostic(ASTContext &C,
  ...
  if (Kind == PlatformAgnosticAvailabilityKind::NeXTCodeVersionSpecific) {
...
StringRef ExternAttr::getCName(const FuncDecl *D) const {
  if (auto cName = this->Name)
    return cName.value();
  // If no name was specified, fall back on the NeXTCode base name without mangling.
...
void nextcode::simple_display(llvm::raw_ostream &out, const DeclAttribute *attr) {
  if (attr)
    attr->print(out);
}
...
bool nextcode::hasAttribute(const LangOptions &langOpts,
                         llvm::StringRef attributeName) {
