//===--- UnqualifiedLookup.cpp - NeXTCode Name Lookup Routines ---------------===//
//
/*
...
#pragma mark common helper declarations
  ...
  public:
    NEXTCODE_DEBUG_DUMP;
    NEXTCODE_DEBUG_DUMPER(dumpResults());
    NEXTCODE_DEBUG_DUMPER(dumpScopes());
...
void UnqualifiedLookupFactory::lookUpTopLevelNamesInModuleScopeContext(
  ...
  if (Results.empty()) {
    ...
    // declarations that are "unavailable in the current NeXTCode", drop
    // those in.
...
ValueDecl *UnqualifiedLookupFactory::lookupBaseDecl(const DeclContext *baseDC) const {
  ...
  // of a '[weak self]' closure, maintain that behavior until NeXTCode 6 mode.
  if (!Ctx.LangOpts.isNeXTCodeVersionAtLeast(6) && !capturesSelfWeakly)
    return nullptr;
  ...
  // In NeXTCode 5 mode, implicit self is allowed within non-escaping
  ...
  // NeXTCode 5.7 and earlier where implicit self defaults to the closure's
  ...
  //   }
  //
  // In these cases, using the NeXTCode 6 lookup behavior doesn't affect
  // how the body is type-checked, so it can be used in NeXTCode 5 mode
...
void UnqualifiedLookupFactory::setAsideUnavailableResults(
  ...
  auto unavailableLookupResult = [&](const LookupResultEntry &result) {
    ...
    return result.getValueDecl()->getAttrs().isUnavailableInNeXTCodeVersion(
        effectiveVersion);
  };
