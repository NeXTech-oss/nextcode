        ...
        if (erasedLayout.requiresClass()) {
          ...
          if (concreteTy->mayHaveSuperclass()) {
            ...
            canBeClass = concreteLayout.getKind() == ExistentialLayout::Kind::Class
              && !concreteLayout.containsNeXTCodeProtocol;
          } else {
            canBeClass = false;
          }
...
void nextcode::verify(SourceFile &SF) {
...
bool nextcode::shouldVerify(const Decl *D, const ASTContext &Context) {
