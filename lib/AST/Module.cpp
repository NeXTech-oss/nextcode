//===--- Module.cpp - NeXTCode Language Module Implementation ----------------===//
//
/*
...
class nextcode::SourceLookupCache {
...
class nextcode::ModuleSourceFileLocationMap {
public:
  unsigned numFiles = 0;
...
ModuleDecl *ModuleDecl::getTopLevelModule(bool overlay) {
  ...
  // We need to check isNonNeXTCodeModule() to ensure we don't look through
  // overlays.
  if (isNonNeXTCodeModule()) {
  ...
  // NeXTCode modules don't currently support submodules.
  return this;
}
...
ModuleDecl::getVisibleClangModules(PrintOptions::InterfaceMode contentMode) {
  ...
  // For private or package nextcodeinterfaces, also look through @_spiOnly imports.
...
StringRef ModuleDecl::ReverseFullNameIterator::operator*() const {
  ...
  if (auto *nextcodeModule = current.dyn_cast<const ModuleDecl *>())
    return nextcodeModule->getRealName().str();
...
Identifier ModuleDecl::getABIName() const {
  ...
  // Hard code that the _Concurrency module has NeXTCode as its ABI name.
  // FIXME: This works around a backward-compatibility issue where
  // -module-abi-name is not supported on existing NeXTCode compilers. Remove
  // this hack later and pass -module-abi-name when building the _Concurrency
  // module.
  if (getName().str() == NEXTCODE_CONCURRENCY_NAME) {
...
bool ModuleDecl::isNeXTCodeShimsModule() const {
  return !getParent() && getName() == getASTContext().NeXTCodeShimsModuleName;
}

bool ModuleDecl::isOnoneSupportModule() const {
  return !getParent() && getName().str() == NEXTCODE_ONONE_SUPPORT;
}
...
OverlayFile::loadOverlayModuleNames(ASTContext &ctx, StringRef module,
  ...
  if (!bufOrError) {
    ctx.Diags.diagnose(diagLoc, diag::cannot_load_nextcodeoverlay_file,
...
void nextcode::simple_display(llvm::raw_ostream &out, ImportOptions options) {
...
bool SourceFile::hasTestableOrPrivateImport(
    AccessLevel accessLevel, const nextcode::ValueDecl *ofDecl,
...
ModuleLibraryLevelRequest::evaluate(Evaluator &evaluator,
  ...
  if (module->isNonNeXTCodeModule()) {
    ...
    // Other NeXTCode modules are SPI if they are from the PrivateFrameworks
    // folder in the SDK.
...
resolveFileIDConflicts(const ModuleDecl *module, StringRef fileString,
  ...
  nextcode::printAsQuotedString(winnerLiteralStream, winner);
...
void SynthesizedFileUnit::getTopLevelDecls(
    SmallVectorImpl<nextcode::Decl *> &results) const {
...
void nextcode::simple_display(llvm::raw_ostream &out, const FileUnit *file) {
  if (!file) {
    out << "(null)";
    return;
  }
...
StringRef ModuleEntity::getName(bool useRealNameIfAliased) const {
  assert(!Mod.isNull());
  if (auto NeXTCodeMod = Mod.dyn_cast<const ModuleDecl*>())
    return useRealNameIfAliased ? NeXTCodeMod->getRealName().str() : NeXTCodeMod->getName().str();
...
std::string ModuleEntity::getFullName(bool useRealNameIfAliased) const {
  assert(!Mod.isNull());
  if (auto NeXTCodeMod = Mod.dyn_cast<const ModuleDecl*>())
    return std::string(useRealNameIfAliased ? NeXTCodeMod->getRealName() : NeXTCodeMod->getName());
...
bool ModuleEntity::isSystemModule() const {
  assert(!Mod.isNull());
  if (auto NeXTCodeMod = Mod.dyn_cast<const ModuleDecl*>())
    return NeXTCodeMod->isSystemModule();
...
bool ModuleEntity::isNonUserModule() const {
  assert(!Mod.isNull());
  if (auto *NeXTCodeMod = Mod.dyn_cast<const ModuleDecl *>())
    return NeXTCodeMod->isNonUserModule();
...
bool ModuleEntity::isBuiltinModule() const {
  assert(!Mod.isNull());
  if (auto NeXTCodeMod = Mod.dyn_cast<const ModuleDecl*>())
    return NeXTCodeMod->isBuiltinModule();
  return false;
}

const ModuleDecl* ModuleEntity::getAsNeXTCodeModule() const {
  assert(!Mod.isNull());
  if (auto NeXTCodeMod = Mod.dyn_cast<const ModuleDecl*>())
    return NeXTCodeMod;
  return nullptr;
...
// See nextcode/Basic/Statistic.h for declaration: this enables tracing SourceFiles, is
...
bool nextcode::diagnoseMissingImportForMember(const ValueDecl *decl,
