...
ASTContext &GenericSignature::getASTContext(
                                    ArrayRef<GenericTypeParamType *> params,
                                    ArrayRef<nextcode::Requirement> requirements) {
...
void nextcode::simple_display(raw_ostream &out, GenericSignature sig) {
  if (sig)
    sig->print(out);
  else
    out << "NULL";
...
int nextcode::compareAssociatedTypes(AssociatedTypeDecl *assocType1,
...
int nextcode::compareDependentTypes(Type type1, Type type2) {
...
void nextcode::validateGenericSignature(ASTContext &context,
...
void nextcode::validateGenericSignaturesInModule(ModuleDecl *module) {
...
GenericSignature
nextcode::buildGenericSignature(ASTContext &ctx,
