...
#include "nextcode/Parse/Lexer.h" // bad dependency
...
#define EDUCATIONAL_NOTES(DIAG, ...)                                           \
  static constexpr const char *const DIAG##_educationalNotes[] = {__VA_ARGS__, \
                                                                  nullptr};
#include "nextcode/AST/EducationalNotes.def"
...
InFlightDiagnostic &
InFlightDiagnostic::limitBehaviorUntilNeXTCodeVersion(
  ...
  if (!Engine->languageVersion.isVersionAtLeast(majorVersion)) {
    ...
    // in a message that this will become an error in a later NeXTCode
    ...
    if (limit >= DiagnosticBehavior::Warning)
      wrapIn(diag::error_in_future_nextcode_version, majorVersion);
  ...
  if (majorVersion == 6) {
    if (auto stats = Engine->statsReporter) {
      ++stats->getFrontendCounters().NumNeXTCode6Errors;
    }
  }
...
InFlightDiagnostic &
InFlightDiagnostic::warnUntilNeXTCodeVersion(unsigned majorVersion) {
  return limitBehaviorUntilNeXTCodeVersion(DiagnosticBehavior::Warning,
                                        majorVersion);
}

InFlightDiagnostic &
InFlightDiagnostic::warnInNeXTCodeInterface(const DeclContext *context) {
...
static bool isInterestingTypealias(Type type) {
  ...
  // The 'NeXTCode.AnyObject' typealias is not 'interesting'.
...
void nextcode::printClangDeclName(const clang::NamedDecl *ND,
                               llvm::raw_ostream &os) {
...
llvm::cl::opt<bool> AssertOnError("nextcode-diagnostics-assert-on-error",
                                  llvm::cl::init(false));
// A special option only for compiler writers that causes Diagnostics to assert
// when a warning diagnostic is emitted. Intended for use in the debugger.
llvm::cl::opt<bool> AssertOnWarning("nextcode-diagnostics-assert-on-warning",
                                    llvm::cl::init(false));
...
DeclName
nextcode::getGeneratedSourceInfoMacroName(const GeneratedSourceInfo &info) {
