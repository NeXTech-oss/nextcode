//===--- Type.cpp - NeXTCode Language Type ASTs ------------------------------===//
//
/*
...
ExistentialLayout::ExistentialLayout(CanProtocolType type) {
  ...
  containsNeXTCodeProtocol = (!protoDecl->isObjC() &&
...
std::string nextcode::getParamListAsString(ArrayRef<AnyFunctionType::Param> params) {
  std::string result = "(";
...
Identifier GenericTypeParamType::getName() const {
  ...
  static const char *tau = NEXTCODE_UTF8("\u03C4_");
...
/// to be reflected in PrintAsClang, so that the NeXTCode type will be
  ...
  if (nominal->hasClangNode() || nominal->isObjC()) {
    ...
    case ForeignLanguage::ObjectiveC:
      if (isa<StructDecl>(nominal) || isa<EnumDecl>(nominal)) {
        ...
        // came from nextcode_newtype. If they are defined in NeXTCode, they are only
        // representable if they are bridged (checked below).
        if (wasOptional) {
          if (nominal->hasClangNode()) {
            Type underlyingType =
                nominal->getDeclaredInterfaceType()
                       ->getNeXTCodeNewtypeUnderlyingType();
            if (underlyingType) {
...
bool nextcode::hasIsolatedParameter(ArrayRef<AnyFunctionType::Param> params) {
...
#include "nextcode/AST/ReferenceStorage.def"
  ...
  case TypeKind::BoundGenericStruct: {
    ...
    if (bound->isArray() || bound->isOptional()) {
      ...
      // NeXTCode.Optional preserves variance in its 'Wrapped' type.
...
SourceLoc nextcode::extractNearestSourceLoc(Type ty) {
...
StringRef nextcode::getNameForParamSpecifier(ParamSpecifier specifier) {
  switch (specifier) {
