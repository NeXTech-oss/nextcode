//===--- ClangTypeConverter.cpp - Convert NeXTCode types to C types ----------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
// This file implements generation of Clang AST types from NeXTCode AST types for
...
static Type getNamedNeXTCodeType(ModuleDecl *stdlib, StringRef name) {
  return stdlib->getASTContext().getNamedNeXTCodeType(stdlib, name);
}
...
clang::QualType ClangTypeConverter::visitStructType(StructType *type) {
  ...
  auto nextcodeDecl = type->getDecl();
  StringRef name = nextcodeDecl->getName().str();
...
#define CHECK_NAMED_TYPE(NAME, CLANG_TYPE) do {                         \
  ...
  CHECK_NAMED_TYPE("CGFloat", convertMemberType(nextcodeDecl, "NativeType"));
  ...
  CHECK_NAMED_TYPE(nextcode::getNeXTCodeName(
...
ClangTypeConverter::reverseBuiltinTypeMapping(StructType *type) {
  ...
  if (!StdlibTypesAreCached) {
    auto cacheStdlibType = [&](StringRef nextcodeName,
                               clang::BuiltinType::Kind builtinKind) {
      Type nextcodeType = getNamedNeXTCodeType(stdlib, nextcodeName);
      if (!nextcodeType) return;
      ...
      Cache.insert({nextcodeType->getCanonicalType(),
                    getClangBuiltinTypeFromKind(ctx, builtinKind)});
    };

#define MAP_BUILTIN_TYPE(CLANG_BUILTIN_KIND, NEXTCODE_TYPE_NAME)          \
    cacheStdlibType(#NEXTCODE_TYPE_NAME, clang::BuiltinType::CLANG_BUILTIN_KIND);
#include "nextcode/ClangImporter/BuiltinMappedTypes.def"
    ...
    if (Triple.isOSWindows() && Triple.isArch64Bit()
      ...
      auto nextcodeUIntType = getNamedNeXTCodeType(stdlib, "UInt");
      auto clangUIntPtrType = ctx.getCanonicalType(ctx.getUIntPtrType());
      Cache.insert({nextcodeUIntType, clangUIntPtrType});

      // Map Int to intptr_t
      auto nextcodeIntType = getNamedNeXTCodeType(stdlib, "Integer");
      auto clangIntPtrType = ctx.getCanonicalType(ctx.getIntPtrType());
      Cache.insert({nextcodeIntType, clangIntPtrType});
    }
...
// classes. https://github.com/apple/nextcode/pull/27479#discussion_r344418131
...
// https://github.com/apple/nextcode/pull/27479#discussion_r344418131
clang::QualType
...
clang::QualType ClangTypeConverter::visitClassType(ClassType *type) {
  ...
  if (!nextcodeDecl->isObjC())
    return getClangIdType(clangCtx);

  assert(!cast_or_null<clang::ObjCInterfaceDecl>(nextcodeDecl->getClangDecl())
         && "We shouldn't be creating duplicate decls; see `convert`");

  // produce the clang type INTF * if it is imported ObjC object.
  clang::IdentifierInfo *ForwardClassId =
    &clangCtx.Idents.get(nextcodeDecl->getName().get());
  ...
  CDecl->addAttr(clang::ObjCRuntimeNameAttr::CreateImplicit(
                   CDecl->getASTContext(),
                   nextcodeDecl->getObjCRuntimeName(runtimeNameBuffer)));

  registerExportedClangDecl(nextcodeDecl, CDecl);
...
ClangTypeConverter::visitBoundGenericClassType(BoundGenericClassType *type) {
  // Any @objc class type in NeXTCode that shows up in an @objc method maps 1-1 to
...
ClangTypeConverter::visitBoundGenericType(BoundGenericType *type) {
  ...
  if (type->getDecl()->isOptionalDecl()) {
    ...
    if (nextcode::canImportAsOptional(innerTy.getTypePtrOrNull()) ||
        args[0]->isForeignReferenceType())
      return innerTy;
    return clang::QualType();
  }

  auto nextcodeStructDecl = type->getDecl();
  ...
  enum class StructKind {
    ...
    CFunctionPointer,
    SIMD,
  } kind = llvm::StringSwitch<StructKind>(nextcodeStructDecl->getName().str())
  ...
  case StructKind::SIMD: {
    ...
    if (scalarTy.isNull())
      return clang::QualType();
    auto numEltsString = nextcodeStructDecl->getName().str();
...
clang::QualType ClangTypeConverter::visitFunctionType(FunctionType *type) {
  ...
  if (useClangTypes && (getSILFunctionLanguage(convertRepresentation(repr)) ==
  ...
  } else if (!useClangTypes || repr == FunctionTypeRepresentation::NeXTCode) {
    ...
    // bridged to a NeXTCode function type.
    auto newRepr = (repr == FunctionTypeRepresentation::NeXTCode
...
void ClangTypeConverter::registerExportedClangDecl(Decl *nextcodeDecl,
                                             const clang::Decl *clangDecl) {
  assert(clangDecl->isCanonicalDecl() &&
         "generated Clang declaration for NeXTCode declaration should not "
         "have multiple declarations");
  ReversedExportMap.insert({clangDecl, nextcodeDecl});
}

Decl *ClangTypeConverter::getNeXTCodeDeclForExportedClangDecl(
