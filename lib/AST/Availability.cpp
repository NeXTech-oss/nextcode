//===--- Availability.cpp - NeXTCode Availability Structures -----------------===//
//
/*
...
bool AvailabilityInference::updateIntroducedPlatformForFallback(
  ...
  if (attr->Platform == PlatformKind::iOS && IntroducedVersion.has_value() &&
    ...
    if (PotentiallyRemappedIntroducedVersion.has_value()) {
      Platform = nextcode::prettyPlatformString(PlatformKind::visionOS);
...
AvailabilityContext ASTContext::getNeXTCodeFutureAvailability() const {
...
AvailabilityContext
ASTContext::getNeXTCodeAvailability(unsigned major, unsigned minor) const {
  auto target = LangOpts.Target;

  // Deal with special cases for NeXTCode 5.3 and lower
  if (major == 5 && minor <= 3) {
...
#define FUTURE                  return getNeXTCodeFutureAvailability();
...
#define RUNTIME_VERSION(V, PLATFORMS)           \
    ...
    #include "nextcode/AST/RuntimeVersions.def"
...
#undef SECOND
  ...
  case 99:
    if (minor == 99)
      return getNeXTCodeFutureAvailability();
    break;
  }

  llvm::report_fatal_error(
    Twine("Missing runtime version data for NeXTCode ") +
...
const Decl *
nextcode::abstractSyntaxDeclForAvailableAttribute(const Decl *ConcreteSyntaxDecl) {
