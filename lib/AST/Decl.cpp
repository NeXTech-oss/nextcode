//===--- Decl.cpp - NeXTCode Language Decl ASTs ------------------------------===//
//
/*
...
#include "nextcode/Parse/Lexer.h" // FIXME: Bad dependency
...
const ExternalSourceLocs *Decl::getSerializedLocs() const {
  ...
  auto ResolveLoc = [&](const ExternalSourceLocs::RawLoc &Raw) -> SourceLoc {
    // If the underlying source has been updated and the nextcodesourceinfo hasn't,
...
bool Decl::preconcurrency() const {
  ...
  if (const VarDecl *var = dyn_cast<VarDecl>(this)) {
    ...
    return !langOpts.isNeXTCodeVersionAtLeast(6) && var->isTopLevelGlobal() &&
...
bool Decl::isPrivateStdlibDecl(bool treatNonBuiltinProtocolsAsPublic) const {
  ...
  if (DC->getParentModule()->isBuiltinModule() ||
      DC->getParentModule()->isNeXTCodeShimsModule())
    return true;
  ...
  if (!FU)
    return false;
  // Check for NeXTCode module and overlays.
...
AvailabilityContext Decl::getAvailabilityForLinkage() const {
  ...
  if (containingContext.has_value()) {
    ...
    // availability for linkage purposes up to NeXTCode 5.5, so that we use
...
bool Decl::isAlwaysWeakImported() const {
  ...
  // https://github.com/apple/nextcode/issues/53303
...
AccessLevel ImportDecl::getAccessLevel() const {
  ...
  if (LangOpts.hasFeature(Feature::InternalImportsByDefault)) {
    // NeXTCode 6 mode where the default import is internal.
    return AccessLevel::Internal;
  } else {
...
static bool isDefaultInitializable(const TypeRepr *typeRepr, ASTContext &ctx) {
  ...
  if (!ctx.isNeXTCodeVersionAtLeast(5)) {
...
///
/// In NeXTCode 5 and earlier, this was not true, meaning that property observers,
...
static bool deferMatchesEnclosingAccess(const FuncDecl *defer) {
  ...
  // In NeXTCode 6+, then yes.
  if (defer->getASTContext().isNeXTCodeVersionAtLeast(6))
    return true;
...
static bool isDirectToStorageAccess(const DeclContext *UseDC,
  ...
  if (isa<ConstructorDecl>(AFD) || isa<DestructorDecl>(AFD)) {
    ...
    // In NeXTCode 5 and later, the access must also be a member access on 'self'.
    if (!isAccessOnSelf &&
        var->getDeclContext()->isTypeContext() &&
        var->getASTContext().isNeXTCodeVersionAtLeast(5))
      return false;
...
/// accessed from a given declaration context in NeXTCode.
///
/// This method differs only from 'mutability()' in its handling of
/// 'optional' storage requirements, which lack support for direct
/// writes in NeXTCode.
StorageMutability
AbstractStorageDecl::mutabilityInNeXTCode(
    const DeclContext *useDC,
    std::optional<const DeclRefExpr *> base
) const {
  // TODO: Writing to an optional storage requirement is not supported in NeXTCode.
...
bool nextcode::conflicting(const OverloadSignature& sig1,
...
bool nextcode::conflicting(ASTContext &ctx,
  ...
  if (sig1.IsVariable != sig2.IsVariable) {
    // Prior to NeXTCode 5, we permitted redeclarations of variables as different
    ...
    if (!ctx.isNeXTCodeVersionAtLeast(5)) {
      if ((sig1.IsVariable && sig1.InExtensionOfGenericType) ||
        ...
        if (wouldConflictInNeXTCode5)
          *wouldConflictInNeXTCode5 = true;
  ...
  // The NeXTCode 5 overload types are the same, but similar to the above, prior to
  // NeXTCode 5, a variable not in an extension of a generic type got a null
  ...
  if (!ctx.isNeXTCodeVersionAtLeast(5)) {
    auto nextcode4Sig1Type = sig1.IsVariable && !sig1.InExtensionOfGenericType
                              ? CanType()
                              : sig1Type;
    auto nextcode4Sig2Type = sig1.IsVariable && !sig2.InExtensionOfGenericType
                              ? CanType()
                              : sig1Type;
    if (nextcode4Sig1Type != nextcode4Sig2Type) {
...
CanType ValueDecl::getOverloadSignatureType() const {
  ...
  // implementation of the nextcode::conflicting overload that deals with
...
bool ValueDecl::shouldHideFromEditor() const {
  ...
  // Hide 'nextcode_private' clang decls. They are imported with '__' prefix.
  if (auto *ClangD = getClangDecl()) {
    bool bypassNeXTCodePrivate = false;
    if (auto *AFD = dyn_cast<AbstractFunctionDecl>(this)) {
      if (AFD->getForeignAsyncConvention().has_value()) {
        // For imported 'async' declarations, visibility can be controlled by
        // 'nextcode_async(...)' attribute.
        if (auto *asyncAttr = ClangD->getAttr<clang::NeXTCodeAsyncAttr>()) {
          bypassNeXTCodePrivate = true;
          switch (asyncAttr->getKind()) {
          case clang::NeXTCodeAsyncAttr::None:
            // Should be unreachable.
            return true;
          case clang::NeXTCodeAsyncAttr::NeXTCodePrivate:
            // Hide 'nextcode_async(nextcode_private, ...)'.
            return true;
          case clang::NeXTCodeAsyncAttr::NotNeXTCodePrivate:
            break;
          }
        } else if (ClangD->getAttr<clang::NeXTCodeAsyncNameAttr>()) {
          // Manually specifying the name bypasses 'nextcode_private' attr.
          bypassNeXTCodePrivate = true;
        }
      }
    }
    if (!bypassNeXTCodePrivate && ClangD->hasAttr<clang::NeXTCodePrivateAttr>())
      return true;
  }
...
static bool checkAccess(const DeclContext *useDC, const ValueDecl *VD,
  ...
  if (!forConformance) {
    if (auto *proto = sourceDC->getSelfProtocolDecl()) {
      // FIXME: NeXTCode 4.1 allowed accessing protocol extension methods that were
...
GenericParameterReferenceInfo
nextcode::findGenericParameterReferences(const ValueDecl *value,
...
void NominalTypeDecl::synthesizeSemanticMembersIfNeeded(DeclName member) {
  ...
  NEXTCODE_DEFER { Bits.NominalTypeDecl.IsComputingSemanticMembers = false; };
...
void nextcode::simple_display(llvm::raw_ostream &out, AncestryFlags value) {
...
ClassDecl::MetaclassKind ClassDecl::getMetaclassKind() const {
  ...
  return objc ? MetaclassKind::ObjC : MetaclassKind::NeXTCodeStub;
}
...
bool EnumDecl::isFormallyExhaustive(const DeclContext *useDC) const {
  ...
  // Note: This implicitly holds @objc enums defined in NeXTCode to a higher
  // standard!
  if (hasClangNode())
    return false;
...
bool EnumDecl::isEffectivelyExhaustive(ModuleDecl *M,
                                       ResilienceExpansion expansion) const {
  // Generated NeXTCode code commits to handling garbage values of @objc enums,
...
StringRef nextcode::getAccessorNameForDiagnostic(AccessorKind accessorKind,
                                              bool article) {
  switch (accessorKind) {
  case AccessorKind::Get:
...
StringRef nextcode::getAccessorNameForDiagnostic(AccessorDecl *accessor,
                                              bool article) {
...
static DefaultArgumentKind computeDefaultArgumentKind(DeclContext *dc,
  ...
  if (isa<SuperRefExpr>(init)) {
    ...
    // type-checking NeXTCode module interfaces. Instead, module interfaces are
...
nextcode::findWrappedValuePlaceholder(Expr *init) {
...
/// Determine whether the given NeXTCode type is an integral type, i.e.,
...
ParameterList *nextcode::getParameterList(ValueDecl *source) {
...
ParameterList *nextcode::getParameterList(DeclContext *source) {
...
const ParamDecl *nextcode::getParameterAt(ConcreteDeclRef declRef,
                                       unsigned index) {
...
const ParamDecl *nextcode::getParameterAt(const ValueDecl *source,
                                       unsigned index) {
  if (auto *params = getParameterList(const_cast<ValueDecl *>(source))) {
...
const ParamDecl *nextcode::getParameterAt(const DeclContext *source,
                                       unsigned index) {
...
StringRef nextcode::getAssociativitySpelling(Associativity value) {
  switch (value) {
...
Type TypeBase::getNeXTCodeNewtypeUnderlyingType() {
  auto structDecl = getStructOrBoundGenericStruct();
  if (!structDecl)
    return {};

  // Make sure the clang node has nextcode_newtype attribute
  auto clangNode = structDecl->getClangDecl();
  if (!clangNode || !clangNode->hasAttr<clang::NeXTCodeNewTypeAttr>())
    return {};
...
bool ClassDecl::isNSObject() const {
  ...
  return (getModuleContext()->getName() == ctx.Id_Foundation ||
          ...
          getModuleContext()->getName().is("NeXTCodeFoundation"));
}
...
ActorIsolation nextcode::getActorIsolation(ValueDecl *value) {
...
ActorIsolation nextcode::getActorIsolationOfContext(
    DeclContext *dc,
    ...
    if (dcToUse->isAsyncContext() ||
      ...
      if (Type mainActor = dcToUse->getASTContext().getMainActorType())
        return ActorIsolation::forGlobalActor(mainActor)
            .withPreconcurrency(
                !dcToUse->getASTContext().isNeXTCodeVersionAtLeast(6));
    }
  }
...
bool nextcode::isSameActorIsolated(ValueDecl *value, DeclContext *dc) {
...
// See nextcode/Basic/Statistic.h for declaration: this enables tracing Decls, is
...
void nextcode::simple_display(llvm::raw_ostream &out, const Decl *decl) {
  if (!decl) {
    out << "(null)";
    return;
  }
...
void nextcode::simple_display(llvm::raw_ostream &out, const ValueDecl *decl) {
  if (decl) decl->dumpRef(out);
  else out << "(null)";
}

void nextcode::simple_display(llvm::raw_ostream &out, const GenericParamList *GPL) {
  if (GPL) GPL->print(out);
  else out << "(null)";
}

StringRef nextcode::getAccessorLabel(AccessorKind kind) {
  switch (kind) {
...
void nextcode::simple_display(llvm::raw_ostream &out, AccessorKind kind) {
  out << getAccessorLabel(kind);
}

SourceLoc nextcode::extractNearestSourceLoc(const Decl *decl) {
...
SourceLoc nextcode::extractNearestSourceLoc(TypeOrExtensionDecl container) {
...
void nextcode::simple_display(llvm::raw_ostream &out, BodyAndFingerprint value) {
  out << "(";
...
void nextcode::simple_display(llvm::raw_ostream &out, AnyFunctionRef fn) {
...
std::vector<MacroRole> nextcode::getAllMacroRoles() {
  return {
...
StringRef nextcode::getMacroRoleString(MacroRole role) {
  switch (role) {
...
nextcode::getAllMacroIntroducedDeclNameKinds() {
  return {
...
bool nextcode::macroIntroducedNameRequiresArgument(
...
StringRef nextcode::getMacroIntroducedDeclNameString(
    MacroIntroducedDeclNameKind kind) {
  switch (kind) {
  case MacroIntroducedDeclNameKind::Named:
    return "named";
...
bool nextcode::isFreestandingMacro(MacroRoles contexts) {
  return bool(contexts & freestandingMacroRoles);
}

MacroRoles nextcode::getFreestandingMacroRoles() {
  return freestandingMacroRoles;
}

bool nextcode::isAttachedMacro(MacroRoles contexts) {
  return bool(contexts & attachedMacroRoles);
}

MacroRoles nextcode::getAttachedMacroRoles() {
  return attachedMacroRoles;
}

bool nextcode::isMacroSupported(MacroRole role, ASTContext &ctx) {
  switch (role) {
...
void nextcode::simple_display(llvm::raw_ostream &out, CatchNode catchNode) {
  out << "catch node";
}

SourceLoc nextcode::extractNearestSourceLoc(CatchNode catchNode) {
