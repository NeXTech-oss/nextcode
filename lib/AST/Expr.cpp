//===--- Expr.cpp - NeXTCode Language Expression ASTs ------------------------===//
//
/*
...
#include "nextcode/AST/Decl.h" // FIXME: Bad dependency
...
StringRef nextcode::getFunctionRefKindStr(FunctionRefKind refKind) {
  switch (refKind) {
...
APInt BuiltinIntegerWidth::parse(StringRef text, unsigned radix, bool negate,
  ...
  // NeXTCode doesn't treat a leading zero as signifying octal, but
...
ActorIsolation
nextcode::__AbstractClosureExpr_getActorIsolation(AbstractClosureExpr *CE) {
...
void nextcode::simple_display(llvm::raw_ostream &out, const ClosureExpr *CE) {
  if (!CE) {
    out << "(null)";
    return;
  }
...
SourceLoc nextcode::extractNearestSourceLoc(const ClosureExpr *expr) {
  return expr->getLoc();
}

SourceLoc nextcode::extractNearestSourceLoc(const DefaultArgumentExpr *expr) {
  return expr->getLoc();
}

SourceLoc nextcode::extractNearestSourceLoc(const MacroExpansionExpr *expr) {
  return expr->getLoc();
}

// See nextcode/Basic/Statistic.h for declaration: this enables tracing Exprs, is
...
void AbstractClosureExpr::getIsolationCrossing(
  ...
  for (auto pair : llvm::enumerate(getCaptureInfo().getCaptures())) {
    ...
    auto declIsolation = nextcode::getActorIsolation(capture.getDecl());
