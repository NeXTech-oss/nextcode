...
#define NEXTCODE_TYPEID_ZONE TypeChecker
#define NEXTCODE_TYPEID_HEADER "nextcode/AST/TypeCheckerTypeIDZone.def"
...
void nextcode::simple_display(llvm::raw_ostream &out, ASTContext *ctx) {
  out << "(AST Context)";
}
...
void nextcode::simple_display(llvm::raw_ostream &out, ASTNode node) {
  if (node) {
    node.dump(out);
  } else {
    out << "null";
...
void nextcode::simple_display(llvm::raw_ostream &out, Type type) {
  if (type)
    type.print(out);
  else
    out << "null";
...
void nextcode::simple_display(llvm::raw_ostream &out, const TypeRepr *TyR) {
  if (TyR)
    TyR->print(out);
  else
    out << "null";
...
void nextcode::simple_display(llvm::raw_ostream &out, const TypeLoc source) {
  out << "(";
...
void nextcode::simple_display(llvm::raw_ostream &os, PropertyWrapperMutability m) {
...
void nextcode::simple_display(llvm::raw_ostream &out, PropertyWrapperLValueness l) {
...
void nextcode::simple_display(llvm::raw_ostream &out, Initializer *init) {
...
SourceLoc nextcode::extractNearestSourceLoc(const TypeRepr *repr) {
  if (!repr)
    return SourceLoc();
  return repr->getLoc();
}
...
void nextcode::simple_display(llvm::raw_ostream &out, CustomAttrTypeKind value) {
  switch (value) {
...
bool nextcode::areTypesEqual(Type type1, Type type2) {
  if (!type1 || !type2)
    return !type1 && !type2;
...
void nextcode::simple_display(llvm::raw_ostream &out, MacroRoles roles) {
  out << "macro-roles";
}

bool nextcode::operator==(MacroRoles lhs, MacroRoles rhs) {
  return lhs.containsOnly(rhs);
}

llvm::hash_code nextcode::hash_value(MacroRoles roles) {
  return roles.toRaw();
}
