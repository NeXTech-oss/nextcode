//===--- ASTDumper.cpp - NeXTCode Language AST Dumper ------------------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
//  This file implements dumping for the NeXTCode ASTs.
//
//===----------------------------------------------------------------------===//
...
static StringRef getDumpString(ImportKind value) {
  ...
  case ImportKind::Func: return "function";
  }
...
static StringRef getDumpString(ForeignErrorConvention::IsOwned_t owned) {
  switch (owned) {
  case nextcode::ForeignErrorConvention::IsNotOwned:
    return "unowned";
  case nextcode::ForeignErrorConvention::IsOwned:
    return "owned";
  }
...
namespace {
  ...
  public:
    ...
    void visitModuleDecl(ModuleDecl *MD, StringRef label) {
      ...
      printFlag(MD->isNonNeXTCodeModule(), "non_nextcode");
      printFoot();
    }
...
void nextcode::printContext(raw_ostream &os, DeclContext *dc) {
...
StringRef nextcode::getAccessorKindString(AccessorKind value) {
  switch (value) {
#define ACCESSOR(ID)
