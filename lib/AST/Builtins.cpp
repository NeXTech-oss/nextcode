//===--- Builtins.cpp - NeXTCode Language Builtin ASTs -----------------------===//
//
/*
...
#include "nextcode/AST/ASTSynthesis.h"
...
Type nextcode::getBuiltinType(ASTContext &Context, StringRef Name) {
...
StringRef nextcode::getBuiltinBaseName(ASTContext &C, StringRef Name,
...
Type nextcode::getAsyncTaskAndContextType(ASTContext &ctx) {
...
static ValueDecl *getCreateTask(ASTContext &ctx, Identifier id) {
  auto taskExecutorIsAvailable =
      ctx.getProtocol(nextcode::KnownProtocolKind::TaskExecutor) != nullptr;

  return getBuiltinFunction(
      ctx, id, _thin, _generics(_unrestricted, _conformsToDefaults(0)),
      _parameters(
          _label("flags", _nextcodeInt),
...
static ValueDecl *getOnceOperation(ASTContext &Context,
  ...
  nextcode::CanType ContextTy = Context.TheRawPointerType;
  ...
  SmallVector<nextcode::Type, 3> ArgTypes = {HandleTy, BlockTy};
  if (withContext) {
...
bool nextcode::canBuiltinBeOverloadedForType(BuiltinValueKind ID, Type Ty) {
...
llvm::Intrinsic::ID nextcode::getLLVMIntrinsicID(StringRef InName) {
  using namespace llvm;

  // NeXTCode intrinsic names start with int_.
...
llvm::Intrinsic::ID
nextcode::getLLVMIntrinsicIDForBuiltinWithOverflow(BuiltinValueKind ID) {
  switch (ID) {
    default: break;
...
Type IntrinsicTypeDecoder::decodeImmediate() {
  ...
  case IITDescriptor::AArch64Svcount:
    // These types cannot be expressed in nextcode yet.
    return Type();
...
static bool
getNeXTCodeFunctionTypeForIntrinsic(llvm::Intrinsic::ID ID,
  ...
  // Decode the intrinsic's LLVM IR type, and map it to nextcode builtin types.
  ...
  // Translate LLVM function attributes to NeXTCode function attributes.
  IntrinsicInfo II;
  II.ID = ID;
...
llvm::AtomicOrdering nextcode::decodeLLVMAtomicOrdering(StringRef O) {
  using namespace llvm;
...
ValueDecl *nextcode::getBuiltinValueDecl(ASTContext &Context, Identifier Id) {
  ...
  // If this is the name of an LLVM intrinsic, cons up a nextcode function with a
  // type that matches the IR types.
  if (llvm::Intrinsic::ID ID = getLLVMIntrinsicID(OperationName)) {
    SmallVector<Type, 8> ArgElts;
    Type ResultTy;
    if (getNeXTCodeFunctionTypeForIntrinsic(ID, Types, Context, ArgElts, ResultTy))
      return getBuiltinFunction(Id, ArgElts, ResultTy);
  }
...
StringRef nextcode::getBuiltinName(BuiltinValueKind ID) {
  switch (ID) {
...
bool nextcode::isPolymorphicBuiltin(BuiltinValueKind id) {
  switch (id) {
  case BuiltinValueKind::None:
    llvm_unreachable("no builtin kind");
#define BUILTIN(Id, Name, Attrs)                                               \
