...
public:
  UsedDeclEnumerator(const SourceFile *SF, StringRef nextcodeDeps)
      : SF(SF), nextcodeDeps(nextcodeDeps),
        sourceFileImplementation(DependencyKey::createKeyForWholeSourceFile(
            DeclAspect::implementation, nextcodeDeps)) {}
...
void FrontendSourceFileDepGraphFactory::addAllUsedDecls() {
  UsedDeclEnumerator(SF, nextcodeDeps)
      .enumerateAllUses(
          [&](const DependencyKey &def, const DependencyKey &use) {
            addAUsedDecl(def, use);
          });
  ExternalDependencyEnumerator(depTracker, nextcodeDeps)
