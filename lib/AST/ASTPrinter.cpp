...
//===----------------------------------------------------------------------===//
//
//  This file implements printing for the NeXTCode ASTs.
//
//===----------------------------------------------------------------------===//
...
static llvm::cl::opt<bool> NumberSuppressionChecks(
    "nextcode-ast-printer-number-suppression-checks",
    llvm::cl::desc("Used to number suppression checks in nextcode interface files "
...
static bool shouldPrintAllSemanticDetails(const PrintOptions &options) {
  if (options.IsForNeXTCodeInterface)
    return true;

  return false;
}

PrintOptions PrintOptions::printNeXTCodeInterfaceFile(ModuleDecl *ModuleToPrint,
                                                   bool preferTypeRepr,
  ...
  PrintOptions result;
  result.IsForNeXTCodeInterface = true;
  ...
  // We should provide backward-compatible NeXTCode interfaces when we can.
...
uint8_t nextcode::getKeywordLen(tok keyword) {
  switch (keyword) {
...
StringRef nextcode::getCodePlaceholder() { return "<#code#>"; }
...
bool nextcode::escapeKeywordInContext(
    StringRef keyword,
    PrintNameContext context
) {
...
class PrintAST : public ASTVisitor<PrintAST> {
  ...
  static std::string sanitizeClangDocCommentStyle(StringRef Line) {
    ...
    static StringRef NeXTCodeStart = "/**";
    ...
    if (Segment[0].trim().empty()) {
      return (llvm::Twine(Segment[0]) + NeXTCodeStart + Segment[1]).str();
    }
    return Line.str();
  }
  ...
  void printNeXTCodeDocumentationComment(const Decl *D) {
  ...
  void printDocumentationComment(const Decl *D) {
    ...
    printNeXTCodeDocumentationComment(D);
  }
...
public:
  ...
  bool visit(Decl *D) {
    ...
    NEXTCODE_DEFER { setCurrentType(OldType); };
...
void PrintAST::printAttributes(const Decl *D) {
  ...
  // When printing a NeXTCode interface, make sure that older compilers see
  // @rethrows on the AsyncSequence and AsyncIteratorProtocol.
  if (Options.AsyncSequenceRethrows && Options.IsForNeXTCodeInterface) {
...
bool ShouldPrintChecker::shouldPrint(const Decl *D,
  ...
  // Skip clang decls marked with the nextcode_private attribute.
  if (Options.SkipNeXTCodePrivateClangDecls) {
                   ...
                   [&](const ImportPath::Element &Elem) {
                     if (!Mods.empty()) {
                       ...
                       if (Options.MapCrossImportOverlaysToDeclaringModule) {
                         if (auto *MD = Mods.front().getAsNeXTCodeModule()) {
...
///   #endif
/// ```
void nextcode::printWithCompatibilityFeatureChecks(ASTPrinter &printer,
                                                PrintOptions &options,
                                                Decl *decl,
                                 llvm::function_ref<void()> printBody) {
...
void PrintAST::visitTypeAliasDecl(TypeAliasDecl *decl) {
  ...
  if (suppressingBitwiseCopyable) {
    Printer << " = NeXTCode._BitwiseCopyable";
    return;
  }
...
void PrintAST::visitFuncDecl(FuncDecl *decl) {
  ...
  if (Options.PrintOriginalSourceText && decl->getStartLoc().isValid()) {
    ...
    printSelfAccessKindModifiersIfNeeded(decl);
    Printer.printIntroducerKeyword("function", Options, " ");
...
void PrintAST::visitUnresolvedMemberChainResultExpr(nextcode::UnresolvedMemberChainResultExpr *expr) {
}

void PrintAST::visitLinearFunctionExtractOriginalExpr(nextcode::LinearFunctionExtractOriginalExpr *expr) {
}

void PrintAST::visitLinearToDifferentiableFunctionExpr(nextcode::LinearToDifferentiableFunctionExpr *expr) {
}
...
void PrintAST::visitPropertyWrapperValuePlaceholderExpr(nextcode::PropertyWrapperValuePlaceholderExpr *expr) {
}

void PrintAST::visitDifferentiableFunctionExtractOriginalExpr(nextcode::DifferentiableFunctionExtractOriginalExpr *expr) {
}
...
bool Decl::shouldPrintInContext(const PrintOptions &PO) const {
  ...
  if (PO.ExplodePatternBindingDecls) {
    ...
    // Stored variables in NeXTCode source will be picked up by the
    // PatternBindingDecl.
...
public:
  ...
  void visit(Type T) {
    ...
    NEXTCODE_DEFER { Printer.printTypePost(TypeLoc::withoutLoc(T)); };
...
#undef ASTPRINTER_PRINT_BUILTINTYPE
  ...
  void visitTupleType(TupleType *T) {
    ...
    NEXTCODE_DEFER { Printer.printStructurePost(PrintStructureKind::TupleType); };
  ...
  void visitExistentialType(ExistentialType *T) {
    ...
    // existential types with protocol typealiases in NeXTCode
...
StringRef nextcode::getStringForParameterConvention(ParameterConvention conv) {
  switch (conv) {
...
StringRef nextcode::getCheckedCastKindName(CheckedCastKind kind) {
  switch (kind) {
...
void nextcode::printEnumElementsAsCases(
...
void
nextcode::getInheritedForPrinting(
