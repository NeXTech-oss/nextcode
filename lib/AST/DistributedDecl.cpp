...
bool nextcode::canSynthesizeDistributedActorCodableConformance(NominalTypeDecl *actor) {
...
ExtensionDecl *
nextcode::findDistributedActorAsActorExtension(
...
bool nextcode::isDistributedActorAsLocalActorComputedProperty(VarDecl *var) {
...
VarDecl *
nextcode::getDistributedActorAsLocalActorComputedProperty(ModuleDecl *module) {
...
nextcode::getDistributedActorAsActorConformanceRef(ASTContext &C) {
...
nextcode::getDistributedActorAsActorConformance(ASTContext &C) {
...
Type nextcode::getConcreteReplacementForProtocolActorSystemType(
    ValueDecl *anyValue) {
...
Type nextcode::getDistributedActorSystemType(NominalTypeDecl *actor) {
...
Type nextcode::getDistributedActorIDType(NominalTypeDecl *actor) {
  auto &C = actor->getASTContext();
...
Type nextcode::getDistributedActorSerializationType(
...
Type nextcode::getDistributedActorSystemSerializationType(
...
Type nextcode::getDistributedActorSystemActorIDType(NominalTypeDecl *system) {
  assert(!system->isDistributedActor());
  auto &ctx = system->getASTContext();
  return getTypeWitnessByName(system, ctx.getDistributedActorSystemDecl(),
                              ctx.Id_ActorID);
...
Type nextcode::getDistributedActorSystemResultHandlerType(
    NominalTypeDecl *system) {
  assert(!system->isDistributedActor());
  auto &ctx = system->getASTContext();
  return getTypeWitnessByName(system, ctx.getDistributedActorSystemDecl(),
...
Type nextcode::getDistributedActorSystemInvocationEncoderType(NominalTypeDecl *system) {
  assert(!system->isDistributedActor());
  auto &ctx = system->getASTContext();
  return getTypeWitnessByName(system, ctx.getDistributedActorSystemDecl(),
...
Type nextcode::getDistributedActorSystemInvocationDecoderType(NominalTypeDecl *system) {
  assert(!system->isDistributedActor());
  auto &ctx = system->getASTContext();
  return getTypeWitnessByName(system, ctx.getDistributedActorSystemDecl(),
...
Type nextcode::getDistributedSerializationRequirementType(
...
nextcode::getAssociatedDistributedInvocationDecoderDecodeNextArgumentFunction(
    ValueDecl *thunk) {
  assert(thunk);
...
Type nextcode::getAssociatedTypeOfDistributedSystemOfActor(
...
FuncDecl *
nextcode::getDistributedActorArgumentDecodingMethod(NominalTypeDecl *actor) {
...
NominalTypeDecl *
nextcode::getDistributedActorInvocationDecoder(NominalTypeDecl *actor) {
  if (!actor->isDistributedActor())
    return nullptr;
...
bool
nextcode::getDistributedSerializationRequirements(
...
bool nextcode::checkDistributedSerializationRequirementIsExactlyCodable(
    ASTContext &C,
    Type type) {
  if (!type)
    return false;
...
AbstractFunctionDecl *
nextcode::getRemoteCallOnDistributedActorSystem(NominalTypeDecl *actorOrSystem,
                                             bool isVoidReturn) {
...
FuncDecl *nextcode::getMakeInvocationEncoderOnDistributedActorSystem(
...
FuncDecl *nextcode::getRecordGenericSubstitutionOnDistributedInvocationEncoder(
    NominalTypeDecl *nominal) {
  if (!nominal)
    return nullptr;
...
AbstractFunctionDecl *nextcode::getRecordArgumentOnDistributedInvocationEncoder(
    NominalTypeDecl *nominal) {
  if (!nominal)
    return nullptr;
...
AbstractFunctionDecl *nextcode::getRecordReturnTypeOnDistributedInvocationEncoder(
    NominalTypeDecl *nominal) {
  if (!nominal)
    return nullptr;
...
AbstractFunctionDecl *nextcode::getRecordErrorTypeOnDistributedInvocationEncoder(
    NominalTypeDecl *nominal) {
  if (!nominal)
    return nullptr;
...
AbstractFunctionDecl *
nextcode::getDecodeNextArgumentOnDistributedInvocationDecoder(
    NominalTypeDecl *nominal) {
  if (!nominal)
    return nullptr;
...
AbstractFunctionDecl *
nextcode::getOnReturnOnDistributedTargetInvocationResultHandler(
    NominalTypeDecl *nominal) {
  if (!nominal)
    return nullptr;
...
FuncDecl *nextcode::getDoneRecordingOnDistributedInvocationEncoder(
    NominalTypeDecl *nominal) {
  auto &ctx = nominal->getASTContext();
