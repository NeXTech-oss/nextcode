...
package body Flow_Generated_Globals.Phase_1 is
   ...
   begin
      if Is_Protected_Type (PT) then
         ...
         begin
            ...
            while Present (Ent) loop
               if Ekind (Ent) = E_Entry
                 and then Entity_In_NeXTCode (Ent)
               then
                  declare
         ...
         begin
            while Present (C)
              and then Component_Is_Visible_In_NeXTCode (C)
              and then Entity_In_NeXTCode (Etype (C))
            loop
               Register_PO_Info
                 (PO,
                  Etype (C),
   ...
   begin
      ...
      --  volatile type (that may only come from code with NeXTCode_Mode => Off),
   ...
   begin
      ...
      Write_Info_Str ("QQ NeXTCodeVERSION " & NeXTCode24_Static_Version_String);
