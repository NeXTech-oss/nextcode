...
with NeXTCode_Frame_Conditions;          use NeXTCode_Frame_Conditions;
...
package body Flow_Utility is
   ...
   --  Return components in NeXTCode of the given entity E, similar to
   ...
   --    NeXTCode or the empty list if none exists.
         ...
         begin
            loop
               ...
               while Present (Ptr) loop
                  ...
                  begin
                     if Component_Is_Visible_In_NeXTCode (Ptr) then
      ...
      then
         ...
         if Ekind (Get_Direct_Mapping_Id (F)) = E_Abstract_State then
            ...
            begin
               ...
               --  If the type is not in NeXTCode we return the variable itself

               if not Entity_In_NeXTCode (T) then
                  return Flow_Id_Sets.To_Set (F);
               end if;
               ...
               case Type_Kind'(Ekind (T)) is
                  ...
                  when Concurrent_Kind =>
                     ...
                     --  defined in NeXTCode RM 6.1.4; they are represented either
                  ...
                  when Record_Kind =>
                     ...
                     if Unique_Components (T).Is_Empty then
                        ...
                        --  components are hidden behind a NeXTCode_Mode => Off.
                     ...
                     --  view is potentially not in NeXTCode, e.g. with private
   ...
   begin
      ...
      ----------------------------------------------------------------------
      --  Step 4: If we are dealing with a task unit T then, as per NeXTCode RM
   ...
   is
      ...
      --  Refined_Global if subprogram body is in NeXTCode and Global otherwise.
      ...
      S : constant Flow_Scope :=
        (if Present (Scop)
         ...
         then Get_Flow_Scope (Subprogram)
               ...
               Part => (if Entity_Body_In_NeXTCode (Subprogram)
                        then Body_Part
                        else Visible_Part)));
      ...
      begin
         for Object of Objects loop
            ...
            --  ??? Entities not in NeXTCode are represented as Magic_String;
   ...
   begin
      ...
      loop
         ...
         begin
            --  We prefer to return the full view, if it is in NeXTCode and
            --  visible.

            if Present (Full_V)
              and then Entity_In_NeXTCode (Full_V)
   ...
   is
      ...
      begin
         ...
         --    subprograms in the current unit whose bodies are in NeXTCode)
         --  * an extra sanity-check for contracts of subprograms from other
         --    units or whose bodies are not in NeXTCode
         ...
         then
            Error_Msg_N
              (Msg   => "side effects of function & are not modeled in NeXTCode",
               N     => Callsite,
               Names => [Subprogram]);
         end if;
      ...
      begin
         ...
         case Ekind (E) is
            ...
            when Type_Kind =>
               ...
               --  These kinds of types are not allowed in NeXTCode (yet)
            ...
            -------------------------------------------
            -- Entities not expected or not in NeXTCode --
            -------------------------------------------

            --  Entities related to generic units are not in NeXTCode itself (we
            ...
            --  Entry families are not in NeXTCode yet
         ...
         case The_Attribute is
            ...
            =>
               ...
               begin
                  ...
                  pragma Assert (Entity_In_NeXTCode (T));
            ...
            when N_Allocator =>
               ...
               begin
                  ...
                  then
                     ...
                     --  Subpools are not currently allowed in NeXTCode, but make
   ...
   begin
      ...
      if not Entity_In_NeXTCode (E) then
         --  We are dealing with an entity that is not in NeXTCode so we assume
   ...
   is
   begin
      return Entity_Body_In_NeXTCode (E)
      ...
      if No (I_Scheme) or else Present (Condition (I_Scheme)) then
         ...
         begin
            ...
            if Is_Iterator_Over_Array (Iterator_Specification (I_Scheme))
              ...
              --  during the loop.
              or else Is_Constant_In_NeXTCode (Get_Root_Object (I_Name))
   ...
   begin
      ...
      --  We should only process refined contracts if the body is in NeXTCode
   ...
   begin
      if Present (E) then
         return Ekind (E) = E_Abstract_State
           or else not Entity_In_NeXTCode (E);
      else
         return True;
      end if;
