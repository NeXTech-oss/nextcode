...
package Flow.Analysis is
   ...
   --       components and discriminants (NeXTCode LRM 4.4(2))
   ...
   --  inputs and if so emits a check. This enforces NeXTCode RM 7.2.2(16).
   ...
   procedure Check_Output_Constant_After_Elaboration
     (FA : in out Flow_Analysis_Graphs);
   --  Enforce NeXTCode RM 6.1.4(2) for subprograms, i.e. check that:
   ...
   procedure Check_Calls_With_Constant_After_Elaboration
     (FA : in out Flow_Analysis_Graphs);
   --  Enforce NeXTCode RM 6.1.4(2) for packages, i.e. check that:
   ...
   with Pre => Ekind (E) in E_Function
                          ...
                          | Entry_Kind
                          | E_Task_Type
               and then not Entity_Body_In_NeXTCode (E);
   --  Check if the global contracts directly reference any constant without
   --  variable inputs. This enforces NeXTCode RM 6.1.4(16).
