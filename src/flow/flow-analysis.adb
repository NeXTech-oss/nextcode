...
--             end if;
   ...
   procedure Check_Depends_Contract (FA : in out Flow_Analysis_Graphs) is
      ...
      begin
         ...
         --  violate its own contract or be annotated with NeXTCode_Mode => Off.
   ...
   begin
      ...
      --  declarations (while respecting the NeXTCode_Mode => Off boundary).
         ...
         begin
            if not DM.Contains (LHS) then
               for Constituent of Down_Project (Var, FA.B_Scope) loop
                  ...
                  --  not in NeXTCode here but at the moment Down_Project does
                  --  return them. This need to be fixed in Down_Project.

                  if Is_Abstract_State (Constituent)
                    or else Entity_In_NeXTCode (Constituent)
                  then
   ...
   begin
      if Has_Non_Null_Abstract_State (FA.Spec_Entity) then
         ...
         --  If package body is in NeXTCode, then look at the Refined_State
         ...
         --  If only package spec is in NeXTCode, then look at Part_Of
         --  constituents.

         elsif Entity_Spec_In_NeXTCode (FA.Spec_Entity) then
            ...
            for State of Iter (Abstract_States (FA.Spec_Entity)) loop
               for Constituent of Iter (Part_Of_Constituents (State)) loop
                  ...
                  --  and those packages can have NeXTCode_Mode => Off. We detect
                  --  this by checking whether the constituent is in NeXTCode.

                  if Ekind (Constituent) = E_Constant
                    and then Entity_In_NeXTCode (Constituent)
                    and then not Is_Access_Variable (Etype (Constituent))
                    and then not Has_Variable_Input (Constituent)
                  then
   ...
   is
      ...
      --  Inspects globals of subprogram E to detect violations of NeXTCode RM
      --  6.1.4(22).
   ...
   is
      ...
      begin
         --  Issue error if dealing with nonvolatile function; NeXTCode RM
         --  7.1.3(8).
   ...
   procedure Check_Concurrent_Accesses (GNAT_Root : Node_Id) is
      ...
      is
         ...
         --  Reference to NeXTCode RM for non-obvious verification rules
               ...
               if Current_Max_Queue_Length > 0 then
                  ...
                  for Task_Type of Entry_Callers (Obj) loop
                     ...
                     for Task_Object of Task_Instances (Task_Type) loop
                        ...
                        --  will trigger a NeXTCode violation (see declaration
