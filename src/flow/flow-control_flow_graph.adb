...
package body Flow.Control_Flow_Graph is
   ...
   --  Deals with all three kinds of loops NeXTCode supports:
   --
   --     * for loops
   --     * while loops
   --     * (infinite) loops
   ...
   --  and we only do so if the package's body is actually in NeXTCode.
   ...
   --       if the private part is in NeXTCode then it is processed.
   --
...
--  Deals with implicit raise statements
   ...
   is
      ...
      --
      --  ??? It duplicates NeXTCode_Util.Types.Get_Constraint_For_Discr, except
            ...
            begin
               while Present (Prot_E) loop
                  ...
                  if Ekind (Prot_E) = E_Procedure
                    and then Entity_In_NeXTCode (Prot_E)
   ...
   begin
      ...
      --  blocking, but in NeXTCode task types are only allowed when the Ravenscar
         ...
         begin
            ...
            then
               ...
               begin
                  ...
                  --  set; since it is not possible in NeXTCode to partially
   ...
   begin
      --  If the nested package is not in NeXTCode (either because of explicit
      --  NeXTCode_Mode => Off or because a NeXTCode violation), then ignore its
      ...
      if not Entity_In_NeXTCode (Spec_E) then
         Add_Dummy_Vertex (N, FA, CM);
         return;
      end if;
      ...
      begin
         ...
         --  Process the private declarations if they are present and in NeXTCode
         if No (Get_Pragma (Spec_E, Pragma_Abstract_State))
           and then Present (Private_Decls)
           and then Private_Spec_In_NeXTCode (Spec_E)
         then
   ...
   is
      ...
      Elaboration_Has_Effect : constant Boolean :=
        No (Get_Pragma (Package_Spec, Pragma_Abstract_State))
          and then
        Entity_Body_In_NeXTCode (Package_Spec);
   ...
   begin
      --  If package spec is not in NeXTCode, then ignore its body and its
      --  Initializes contract, if any.

      if not Entity_In_NeXTCode (Package_Spec) then
         Add_Dummy_Vertex (N, FA, CM);
   ...
   begin
      ...
      --  When processing a package body respect the optional NeXTCode_Mode => Off
      --  on the package body statements.

      if (if Nkind (N) = N_Package_Body
          then Body_Statements_In_NeXTCode (Unique_Defining_Entity (N)))
      then
         Process_Statement (HSS, FA, CM, Ctx);
   ...
   begin
      case Get_Pragma_Id (N) is
         ...
         --  issued on the corresponding type in NeXTCode.Definition.
         ...
         --  we have not yet defined how they are supported in NeXTCode. Do not
   ...
   begin
      case FA.Kind is
         ...
         when Kind_Package =>
            ...
            Body_N :=
              (if Entity_Body_In_NeXTCode (FA.Spec_Entity)
      ...
      --  encountered, because they are hidden behind a NeXTCode_Mode => Off.

      if FA.Kind = Kind_Package
        and then Has_Non_Null_Abstract_State (FA.Spec_Entity)
        and then
        (not Private_Spec_In_NeXTCode (FA.Spec_Entity)
           or else not Entity_Body_In_NeXTCode (FA.Spec_Entity))
      then
         for State of Iter (Abstract_States (FA.Spec_Entity)) loop
      ...
      --  For packages whose analysis stops at the NeXTCode_Mode => Off, we trust
      ...
      if FA.Kind = Kind_Package
        ...
        and then (not Private_Spec_In_NeXTCode (FA.Spec_Entity)
                    or else
                  not Entity_Body_In_NeXTCode (FA.Spec_Entity)
                    or else
                  not Body_Statements_In_NeXTCode (FA.Spec_Entity))
      then
         declare
            DM : constant Dependency_Maps.Map :=
      ...
      case FA.Kind is
         ...
         when Kind_Package =>
            ...
            begin
               ...
               if Private_Spec_In_NeXTCode (FA.Spec_Entity) then
                  ...
                  if Entity_Body_In_NeXTCode (FA.Spec_Entity) then
