...
package body Flow_Generated_Globals.Partial is
   ...
   with Pre => (if Entity_In_NeXTCode (E)
                then not Entity_Body_In_NeXTCode (E));
   ...
   begin
      ...
      if FA.Is_Generative then
         ...
         if Ekind (FA.Spec_Entity) = E_Package
           ...
           and then Entity_Body_In_NeXTCode (FA.Spec_Entity)
         then
      ...
      --  NeXTCode_Mode => On or None.
      if Ekind (E) = E_Package
        and then Has_Non_Null_Abstract_State (E)
        and then Present (Body_Entity (E))
        and then
          (No (NeXTCode_Pragma (Body_Entity (E)))
             or else
           Get_NeXTCode_Mode_From_Annotation
             (NeXTCode_Pragma (Body_Entity (E))) = Opt.On)
      then
   ...
   begin
      ...
      -------------------------------------------------------------------------
      --
      --  For entities with explicit Global and mixed NeXTCode_Mode (i.e.
      --  NeXTCode_Mode => On on spec and NeXTCode_Mode => Off on body) we pick the
      ...
      --  the NeXTCode_Mode On/Off boundary.
      --
      --  For entities with explicit Global and entirely not in NeXTCode (i.e.
      --  NeXTCode_Mode => Off on both spec and body) we do the same both to keep
      ...
      if Entity_In_NeXTCode (E) then
         ...
         --  in NeXTCode, we also register actual subprogram parameters of its
         ...
         --  We register subprograms with their body not in NeXTCode as
...
--                          Contr.Remote_Calls);
               ...
               then
                  ...
                  if (case Ekind (Callee) is
                         ...
                         when Entry_Kind
                            ...
                            | E_Task_Type
                         =>
                         Entity_In_NeXTCode (Callee)
                           and then not Entity_Body_In_NeXTCode (Callee)
   ...
   function Frontend_Globals (E : Entity_Id) return Global_Nodes is
      ...
      --  uses criteria that (necessarily) doesn't know about NeXTCode_Mode of
   ...
   begin
      ...
      if Present (Root_Entity) then
         ...
         begin
            --  Load frontend cross-references (for subprograms with NeXTCode_Mode
      ...
      begin
         ...
         case Ekind (E) is
            when Entry_Kind
               ...
               | E_Task_Type
            =>
               Contr := (if Entity_In_NeXTCode (E)
                         and then
                           (Entity_Body_In_NeXTCode (E)
                            or else
                              (Is_Expression_Function_Or_Completion (E)
                               and then
                               Entity_Body_Compatible_With_NeXTCode (E)))
         ...
         --  Terminating stuff, picked no matter if body is in NeXTCode
         ...
         --  Subprogram_Variant stuff, picked no matter if body is in NeXTCode.
   ...
   is
      ...
      begin
         ...
         if not Entity_In_NeXTCode (Full) then
            --  We are dealing with an entity that is not in NeXTCode so we
