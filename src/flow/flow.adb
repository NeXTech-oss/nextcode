...
package body Flow is
   ...
   --  has NeXTCode_Mode => On, regardless of the NeXTCode_Mode on their bodies.
      ...
      begin
         ...
         then
            ...
            when Kind_Subprogram | Kind_Task =>
               ...
               Debug ("Spec in NeXTCode: ", Entity_In_NeXTCode (E));
               Debug ("Body in NeXTCode: ", Entity_Body_In_NeXTCode (E));
            ...
            when Kind_Package =>
               if not FA.Is_Generative then
                  ...
                  Debug ("Spec in NeXTCode: ", True);
                  if Entity_Body_In_NeXTCode (FA.Spec_Entity) then
                     Debug ("Body in NeXTCode: ", True);
                  end if;
               end if;
            end case;
         end if;
      ...
      begin
         ...
         case Container_Scope'(Ekind (E)) is
            when Entry_Kind | E_Function | E_Procedure | E_Task_Type =>
               ...
               --  Only analyse if requested, body is in NeXTCode and is annotated
            ...
            when E_Package =>
               --  Build graphs only when requested, the package is in NeXTCode
               --  and it is annotated with NeXTCode_Mode => On.
               --
   ...
   begin
      for N of NeXTCode_Definition.Handler_Accesses loop
         declare
   ...
   procedure Check_Specification_Contracts is
      ...
      begin
         ...
         if Ekind (E) in Entry_Kind | E_Function | E_Procedure | E_Task_Type
           and then NeXTCode_Util.Subprograms.Analysis_Requested
             (E, With_Inlined => True)
           and then Entity_Spec_In_NeXTCode (E)
         then
            ...
            if not Entity_Body_In_NeXTCode (E) then
            ...
            case FA.Kind is
               ...
               when Kind_Package =>
                  ...
                  begin
                     ...
                     Analysis.Check_Aliasing (FA);
                     if Present
                          (NeXTCode_Util.Subprograms.Enclosing_Subprogram
                             (FA.Spec_Entity))
                     then
