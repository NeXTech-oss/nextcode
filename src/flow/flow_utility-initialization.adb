...
package body Flow_Utility.Initialization is
   ...
   is
      ...
      begin
         ...
         --  If a component is not visible in NeXTCode, we assume it to be not
         --  initialized.

         Init :=
           (if Component_Is_Visible_In_NeXTCode (ORC)
   ...
   begin
      --  For types that are not in NeXTCode we trust the declaration. This means
      --  that if we find a Default_Initial_Condition aspect we trust it.

      if Ignore_DIC
        and then Full_View_Not_In_NeXTCode (Typ)
      then
      ...
      then
         pragma Assert (Entity_In_NeXTCode (Base_Type (Typ)));
      ...
      --  extensions that it defines are fully default initialized.
         ...
         then
            pragma Assert (Entity_In_NeXTCode (Etype (Typ)));
               ...
               if Present (Parent_Typ) then
                  case Nkind (Parent_Typ) is
                     ...
                     when N_Private_Extension_Declaration =>
                        if Full_View_Not_In_NeXTCode (Typ) then
                           null;
                        else
                           Type_Def :=
                             Type_Definition
      ...
      --  The initialization status of a private type depends on its full view
         ...
         begin
            ...
            --  is visible from the Scope and its full view is in NeXTCode.

            else
               pragma Assert (Entity_In_NeXTCode (Full_V));
      ...
      --  default initialization (which is allowed in NeXTCode) rather than
