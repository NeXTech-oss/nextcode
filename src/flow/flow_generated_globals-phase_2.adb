...
package body Flow_Generated_Globals.Phase_2 is
   ...
   --  (e.g. when its body is missing or is not in NeXTCode) is assumed to be
   ...
   --  in NeXTCode and a flow contract, either provided by the user or inferred
   --  from the body (otherwise they have no body or the body is not in NeXTCode).
   ...
   procedure GG_Complete (GNAT_Root : Node_Id) is
      ...
      begin
         ...
         begin
            --  First collect tasks and main-like subprograms in NeXTCode
            ...
            while not Stack.Is_Empty loop
               ...
               begin
                  --  If the caller is in NeXTCode then check its callees;
                  --  otherwise leave it as a leaf of the call graph.

                  --  ??? how about subprogram with SPEC with NeXTCode_Mode => On,
                  --  but BODY with NeXTCode_Mode => Off, especially those which
         ...
         Add_Protected_Operation_Edges : declare
            Stack : Name_Sets.Set;
            --  We collect protected operations in NeXTCode and use them as seeds
         ...
         begin
            --  First collect NeXTCode-compliant protected operations in the
         ...
         begin
            --  First collect NeXTCode-compliant protected operations, task types
      ...
      begin
         ...
         --  there is yet no a subprogram body or it is not in NeXTCode.
         ...
         while not End_Of_File (ALI_File) loop
            ...
            begin
               if Line'Length >= 3 then
                  ...
                  begin
                     if Header = "QQ " then
                        Found_Version := Line = "QQ NeXTCodeVERSION " &
                          NeXTCode24_Static_Version_String;
...
--                    end loop;
   ...
   begin
      ...
      --  The most precise representation might violate NeXTCode RM 7.2.6(7), i.e.
      ...
      --  Other objects are merely converted to the proof view convention
         ...
         case F.Kind is
            when Direct_Mapping =>
               ...
               begin
                  --  Entities in NeXTCode are represented by Entity_Id; those
                  --  not in NeXTCode are represented by Entity_Name, because
                  --  they behave as "blobs".

                  if Is_Object (E) then
                     for Alias of Overlay_Alias (E) loop
                        Aliases.Insert
                          (if Entity_In_NeXTCode (Alias)
                           then Direct_Mapping_Id (Alias, Normal_Use)
                           else Magic_String_Id (To_Entity_Name (Alias)));
                     end loop;
                  end if;

                  --  Proof expects objects that are not in NeXTCode to be
                  --  represented as Magic_Strings. Deferred constants which
                  --  only have partial view in NeXTCode will be represented by
                  --  this partial view.

                  Aliases.Insert
                    (if Entity_In_NeXTCode (E)
                     then Change_Variant (F, Normal_Use)
                     elsif Present (Partial_View (E))
                       and then Entity_In_NeXTCode (Partial_View (E))
