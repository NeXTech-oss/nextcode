...
package NeXTCode_Util.Subprograms is
   ...
   --  exception, and it is implemented in NeXTCode, then you will run into
   ...
   --
   --  C) If the procedure loops forever but is not in NeXTCode and we have
   ...
   --     of a private type, which is not supported in NeXTCode)
   ...
   --  @param E any entity
   --  @return True iff E is an allocating function (NeXTCode RM 4.8)
   ...
   --  @return True iff E is volatile for internal calls, see NeXTCode RM 7.1.2
   ...
   --  (abstract states with invisible constituents and entities not in NeXTCode).
   ...
   --     considered to be dispatching in NeXTCode, because the Retysp of its
