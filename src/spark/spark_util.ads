...
package NeXTCode_Util is
   ...
   subtype N_Ignored_In_NeXTCode is Node_Kind with
     Predicate =>
       N_Ignored_In_NeXTCode in
   ...
   --  A subtype for our special NeXTCode volatility aspects
   ...
   --  We store this additional information in extra tables during the NeXTCode
   ...
   --  in NeXTCode, which can be either V or its partial view if the full view of
   --  V is not in NeXTCode.
   ...
   --  convenient to the NeXTCode backend).
   ...
   function In_NeXTCode_Library_Unit (N : Node_Or_Entity_Id) return Boolean;
   --  Return True if the source unit for N is in the NeXTCode library, ie. if
   --  the unit name starts with "spark-" and ends with ".code" or ".code".
   ...
   --  True if E is an object declared in an expression with actions. In NeXTCode,
   --  this should correspond to a declare expression.

   function Component_Is_Visible_In_NeXTCode (E : Entity_Id) return Boolean
   ...
   --     NeXTCode) or the full view of the enclosing record is in NeXTCode.
   ...
   function Is_Constant_In_NeXTCode (E : Object_Kind_Id) return Boolean;
   --  Return True if E is a constant in NeXTCode (ie. a constant which is not
   ...
   --  @return Return True if E is visible in NeXTCode. A discriminant might not
   ...
   --  However, the front end rejects these two cases. For the NeXTCode back end,
   ...
   package Local_CFG is
      ...
      --  relying on Ada+NeXTCode assumptions that make them side-effect free.
