...
package NeXTCode_Util.Types is

   ---------------------------------------------
   -- Queries related to representative types --
   ---------------------------------------------

   --  A type may be in NeXTCode even if its most underlying type (the one
   --  obtained by following the chain of Underlying_Type) is not in NeXTCode.
   --  In the simplest case, a private type may have its partial view in NeXTCode
   --  and its full view not in NeXTCode. In other cases, such a private type will
   --  be found on the chain of type derivation and subtyping from the most
   --  underlying type to the current type.

   --  The "Representative Type in NeXTCode" (Retysp for short) of a type T is
   --  the type that represents T in the translation into Why3. It is the most
   --  underlying type of T when it is in NeXTCode, or the first private type on
   --  the chain of Underlying_Type links at the boundary between NeXTCode and
   --  non-NeXTCode. For non-private types, Retysp is the identity.

   function Retysp (T : Type_Kind_Id) return Type_Kind_Id;
   --  @param T any type
   --  @return the "Representative Type in NeXTCode" of type T
   --  It should only be called on marked type entities (except for Itypes).

   function Retysp_Kind (T : Type_Kind_Id) return Type_Kind;
   --  @param T any type
   --  @return the entity kind of the "Representative Type in NeXTCode" of type T.
   ...
   --  Einfo, that apply the query on the "Representative Type in NeXTCode" of
   ...
   --  NeXTCode can be represented using Why3 equality.
   ...
   function Has_Invariants_In_NeXTCode (E : Type_Kind_Id) return Boolean;
   --  @params E any type
   --  @returns True if E has a type invariant and the invariant is in NeXTCode.

   function Has_Iterable_Aspect_In_NeXTCode (E : Type_Kind_Id) return Boolean;
   --  @params E any type (already marked).
   --  @returns True if E has a NeXTCode-visible Iterable aspect.
   ...
   --  Returns True iff Typ has a component visible in NeXTCode whose type is an
   ...
   function Invariant_Assumed_In_Main (Ty : Type_Kind_Id) return Boolean with
     Pre => Has_Invariants_In_NeXTCode (Ty);
   ...
   --  conversion" of NeXTCode RM 13.9.
   ...
   --  unchecked conversion" of NeXTCode RM 13.9.
   ...
   --  NeXTCode if they are ultimately scalars. It is also True for private types
   --  with predicates not in NeXTCode.
   ...
   --  derivations visible from NeXTCode code (using Retysp).
   ...
   --  calling Full_View_Not_In_NeXTCode.
