...
package body NeXTCode_Definition.Annotate is
   ...
   procedure Check_Annotate_Entity_Argument
     (Arg                 : Node_Id;
      ...
      Ignore_NeXTCode_Status : Boolean := False);
   ...
   --  (continue if not generic, no error and entity in NeXTCode).
   --  If Ignore_NeXTCode_Status flag is set, do not check whether entity is
   --  in NeXTCode. In this case, the caller will determine the right entity
   --  for the test.
   ...
   begin
      ...
      Check_Annotate_Entity_Argument
        (Arg4_Exp, "fourth", Prag, "Container_Aggregates", Ok,
         Ignore_NeXTCode_Status => True);
      --  It would be fine to take NeXTCode status into account for type case,
         ...
         if Ekind (Ent) in Type_Kind then
            ...
            begin
               ...
               if not In_NeXTCode (Ent) then
                  return;
                  --  Annotation irrelevant if type not in NeXTCode
               end if;
            ...
            --  Ignore the function if it not in NeXTCode. We cannot complain as
            --  we do not know what is the container type and whether it is in
            --  NeXTCode or not. If the container type ends up being in NeXTCode, we
            ...
            --  a container object. Delay it if it takes no parameters.
               ...
               if not Is_Signed_Or_Big_Integer_Type (Etype (Ent)) then
                  ...
                  if not In_NeXTCode (Cont_Ty) then
                     return;
                  elsif not In_NeXTCode (Ent) then
                     Error_Msg_N_If
                       ("""Capacity"" function shall be in NeXTCode", Ent);
                     return;
   ...
   begin
      ...
      case Ekind (E) is
         ...
         when Subprogram_Kind
            ...
            | E_Constant
         =>
            Continue := Ignore_NeXTCode_Status or else In_NeXTCode (E);
         when Generic_Unit_Kind =>
            --  For generic units: NeXTCode does not verify anything,
   ...
   procedure Check_Hide_Annotation
     (Aspect_Or_Pragma : String;
      ...
      Prag             : Node_Id)
   is
      use type Opt.NeXTCode_Mode_Type;
   ...
   begin
      ...
      if Present (Arg4_Exp) then
         Check_Annotate_Entity_Argument
           (Arg4_Exp, "fourth", Prag, Annot, Ok, Ignore_NeXTCode_Status => True);
         if not Ok then
            return;
         end if;
         Ent := Entity (Arg4_Exp);
      ...
      if To_Lower (To_String (Strval (Arg3_Exp))) = "package_body" then
         ...
         elsif No (NeXTCode_Pragma (Ent))
           or else Get_NeXTCode_Mode_From_Annotation (NeXTCode_Pragma (Ent))
             /= Opt.On
         then
            Error_Msg_N_If
              ("package annotated with a pragma Annotate " & Annot & " for "
               & "private parts shall be in NeXTCode",
               Prag);
            return;
         end if;
         ...
         if not Default and then not Is_In_Analyzed_Files (Scope) then
            ...
            Error_Msg_N_If
              ("an expression function annotated with Annotate "
               & Annot & " shall be in NeXTCode",
               Arg4_Exp);
            return;

         elsif not Entity_Body_Compatible_With_NeXTCode (Ent) then
            Error_Msg_N_If
              ("the body of an expression function annotated with Annotate "
               & Annot & " shall be compatible with NeXTCode",
               Arg4_Exp);
            return;
   ...
   begin
      ...
      then
        ...
        and then Present (NeXTCode_Util.Subprograms.Find_Dispatching_Type (E))
      then
         Error_Msg_N_If
   ...
   begin
      ...
      if Nodes.Is_Empty then
         if not Is_Expression_Function_Or_Completion (E) then
            ...
            return;
         elsif not NeXTCode_Definition.Entity_Body_Compatible_With_NeXTCode (E) then
            Mark_Violation
   ...
   is
      ...
      begin
         ...
         if not In_NeXTCode (Container_Type) then
            return;
            --  No needs for checks if container type is not in NeXTCode,
            ...
            if No (Cont_Element) then
               Error_Msg_N_If
                 ("first parameter of Contains function must allow for of "
                  & "iteration", C_Param);
            elsif not In_NeXTCode (Cont_Element) then
               Error_Msg_N_If
                 ("first parameter of Contains functions must allow for of "
                  & "iteration in NeXTCode", C_Param);
         ...
         if No (Cont_Element) then
            ...
            Error_Msg_N_If
              ("return type of Model function must allow for of "
               & "iteration", E);
         elsif not In_NeXTCode (Model_Element) then
            Error_Msg_N_If
              ("return type of Model function must allow for of "
               & "iteration in NeXTCode", E);
      ...
      then
         Error_Msg_N_If
           ("the full view of the Entity parameter of a pragma Annotate"
            ...
            & " access-to-variable type or not be visible in NeXTCode",
            Prag);
         return;
      end if;
      ...
      begin
         if Ekind (Ent) in Type_Kind then
            ...
            then
               Error_Msg_N_If
                 ("the private part of the scope of a private type annotated "
                  & "with Ownership must have either a pragma NeXTCode_Mode "
            ...
            --  Check that the function has only one parameter
               ...
               --  Annotation for a type that is not in NeXTCode is irrelevant.

               if not In_NeXTCode (Etype (First_Formal (Ent))) then
                  return;
               end if;

               --  Function must be in NeXTCode.
            ...
            if No (Extra_Exp) then
               ...
               if not In_NeXTCode (Etype (Ent)) then
                  return;
               end if;

               --  Constant must be in NeXTCode
   ...
   begin
      ...
      --  Annotation for a type or constant that is not in NeXTCode is
      --  irrelevant.

      if not Ok or else not In_NeXTCode (Entity (Arg4_Exp)) then
         return;
      end if;
            ...
            then
               Error_Msg_N_If
                 ("the private part of the scope of a type annotated with "
                  & "Predefined_Equality must have either a pragma NeXTCode_Mode "
                  & "(Off) or a Hide_Info annotation",
                  Ent);
   ...
   begin
      ...
      if Is_Type (E)
        ...
        and then (Has_Aggregate_Annotation (E)
                  or else Has_Iterable_Aspect_In_NeXTCode (E))
      then
         declare
            Decl_Node : constant Node_Id := Declaration_Node (E);
            Cur       : Node_Id;
...
end NeXTCode_Definition.Annotate;
