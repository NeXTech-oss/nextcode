...
package body NeXTCode_Util is
   ...
   function Can_Be_Deeply_Updated (Variable : Entity_Id) return Boolean is
     (declare
      ...
      begin
         Is_Deep (Ty)
         and then not Is_Constant_In_NeXTCode (Variable)
         and then
   ...
   begin
      Specific_Tagged_Types.Insert
        (E,
         (if Is_Full_View (V)
            and then Full_View_Not_In_NeXTCode (Partial_View (V))
          then Partial_View (V)
          else V));
   ...
   begin
      while Present (P) loop
         ...
         case Nkind (P) is
            ...
            --  Deep aggregate cannot be the prefix of an expression (NeXTCode RM
   ...
   begin
      ...
      then
         ...
         --  Direct_Mapping_Id; maybe it should be done once, in NeXTCode_Rewrite.
   ...
   -----------------------------------
   -- Component_Is_Visible_In_NeXTCode --
   -----------------------------------

   function Component_Is_Visible_In_NeXTCode (E : Entity_Id) return Boolean is
   ...
   begin
      ...
      --  Hidden discriminants are only in NeXTCode if Ty's full view is in NeXTCode
      ...
      --  * the full view of this type is in NeXTCode.
         ...
         begin
            ...
            --  by going only through NeXTCode derivations.
            ...
            loop
               ...
               --  full view is in NeXTCode. Otherwise it is the ancestor of the
               --  partial view.

               pragma Assert (Entity_Marked (Full_Rec));
               Rec := (if Entity_In_NeXTCode (Full_Rec) then Etype (Full_Rec)
                       else Etype (Rec));
            ...
            return Entity_In_NeXTCode (Full_Orig);
         end;
      end if;
   end Component_Is_Visible_In_NeXTCode;
   ...
   ---------------------------
   -- In_NeXTCode_Library_Unit --
   ---------------------------

   function In_NeXTCode_Library_Unit (N : Node_Or_Entity_Id) return Boolean is
   ...
   --------------------------
   -- Is_Constant_In_NeXTCode --
   --------------------------

   function Is_Constant_In_NeXTCode (E : Object_Kind_Id) return Boolean is
   begin
      case Ekind (E) is
         when E_In_Parameter =>
   ...
   begin
      ...
      --  NeXTCode_Mode => Off.
        ...
        and then (No (NeXTCode_Aux_Pragma (Scope (Obj)))
                  or else Get_NeXTCode_Mode_From_Annotation
                    (NeXTCode_Aux_Pragma (Scope (Obj))) /= Opt.Off)
      then
   ...
   begin
      --  In general E might not be in NeXTCode (e.g. if it came from the front
      ...
      return Present (Root)
        and then
          (not Is_Deep (Etype (Root))
           or else (Is_Constant_In_NeXTCode (Root)
   ...
   package body Local_CFG is
      ...
      procedure Cache_Graph_Of_Body (Enclosing_Body : Node_Id) is
         ...
         is
            ...
            --  label exits (only forward gotos are allowed in NeXTCode).
      ...
      begin
         ...
         begin
            return Result : constant Boolean :=
              ...
              and then Is_Anonymous_Access_Object_Type (T)
              and then not Is_Access_Constant (T)
              and then not Is_Constant_In_NeXTCode (V)
   ...
   begin
      ...
      --  allowed in NeXTCode. Search for it in Root by name.
            ...
            --  the subprogram.
               ...
               pragma Assert
                 (not Is_Expression_Function_Or_Completion (Caller)
                  and then Entity_Body_In_NeXTCode (Caller));
   ...
   is
      ...
      --  In details, invariant positioning constraint from NeXTCode's language
            ...
            when N_Object_Declaration =>
               ...
               begin
                  return ((not Nested and then Ekind (Obj) = E_Constant)
                          or else Is_Constant_In_NeXTCode (Obj))
   ...
   begin
      ...
      --  states are given); however, respect the NeXTCode_Mode barrier.
      --
      --  Note: objects declared behind a NeXTCode_Mode => Off barrier might
      ...
      elsif Private_Spec_In_NeXTCode (E) then
         Traverse_Declarations (Private_Declarations (Pkg_Spec));

         if Entity_Body_In_NeXTCode (E) then
