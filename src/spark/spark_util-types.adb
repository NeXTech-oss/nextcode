...
package body NeXTCode_Util.Types is
   ...
   --  type "suitable for unchecked conversion" of NeXTCode RM 13.9.

   function Type_Is_Suitable_For_UC_Target
     (Typ       : Type_Kind_Id;
      Use_Esize : Boolean) return True_Or_Explain;
   --  Function to check the properties enforced on all subcomponents of a
   --  type "suitable as a target of an unchecked conversion" of NeXTCode RM 13.9.
   ...
   --  default. Look through NeXTCode boundaries.
   ...
   --  declared by an anscestor. This ignores NeXTCode_Mode boundary.
         ...
         --  relaxed.
            ...
            begin
               while Present (Comp) loop
                  if Component_Is_Visible_In_NeXTCode (Comp) then
   ...
   -----------------------------
   -- Has_Invariants_In_NeXTCode --
   -----------------------------

   function Has_Invariants_In_NeXTCode (E : Type_Kind_Id) return Boolean is
     (Has_Own_Invariants (E)
      and then Is_Base_Type (E)
      and then (if Is_Partial_View (E) then Entity_In_NeXTCode (Full_View (E))));

   ----------------------------------
   -- Has_Iterable_Aspect_In_NeXTCode --
   ----------------------------------

   function Has_Iterable_Aspect_In_NeXTCode (E : Type_Kind_Id) return Boolean is
      Top_Aspect : constant Node_Id := Find_Aspect (E, Aspect_Iterable);
   ...
   begin
      ...
      --  If an out-of-NeXTCode ancestor declares the Iterable Aspect,
      --  the aspect should not be visible in NeXTCode
      --  (happens in some corner cases).
      return not Ancestor_Declares_Iterable_Aspect (Cursor, Top_Aspect);
   end Has_Iterable_Aspect_In_NeXTCode;
   ...
   begin
      ...
      if not Full_View_Not_In_NeXTCode (Ty) then
         return False;
      end if;
   ...
   begin
      return Is_Scalar_Type (Full_View)
        or else (Full_View_Not_In_NeXTCode (Typ) and then Has_Predicates (Typ));
   ...
   begin
      ...
      loop
         if Has_Invariants_In_NeXTCode (Current)
         ...
         begin
            while Present (Comp) loop
               if Ekind (Comp) = E_Component
                 and then Entity_In_NeXTCode (Etype (Comp))
      ...
      while Has_Predicates (Rep_Ty) loop
         ...
         begin
            ...
            if No (Pred_Fun) then
               ...
               if Entity_In_NeXTCode (Pred_Fun) then
                  declare
     ...
     (Is_Base_Type (E)
      ...
      and then
        (declare
         ...
         begin
           Entity_In_NeXTCode (Annot.Add_Procedure)
         and then Entity_In_NeXTCode (Annot.Empty_Function)));
   ...
   begin
      ...
      if Private_Spec_In_NeXTCode (E) then
   ...
   begin
      ...
      --  If T is not in NeXTCode, go through the Partial_View chain to find its
      --  first view in NeXTCode if any.

      if not Entity_In_NeXTCode (Typ) then
         loop
            --  If we find a partial view in NeXTCode, we return it

            if Entity_In_NeXTCode (Typ) then
               pragma Assert (Full_View_Not_In_NeXTCode (Typ));
               goto Returning;

            --  No partial view in NeXTCode, return T
      ...
      --  If T is in NeXTCode but not its most underlying type, then go through
      --  the Full_View chain until the last type in NeXTCode is found. This code
      --  is largely inspired from the body of Einfo.Underlying_Type.

      elsif Full_View_Not_In_NeXTCode (Typ) then
         loop
            --  If Full_View (Typ) is in NeXTCode, use it. Otherwise, we have
            --  found the last type in NeXTCode in T's chain of Full_View.

            if Present (Full_View (Typ)) then
               if Entity_In_NeXTCode (Full_View (Typ)) then
                  Typ := Full_View (Typ);
                  pragma Assert (Full_View_Not_In_NeXTCode (Typ));
               else
                  goto Returning;
            ...
            --  it is in NeXTCode and we reach it, or it is not in NeXTCode and we
            --  return at this point.

            elsif Is_Private_Type (Typ)
              and then Present (Underlying_Full_View (Typ))
            then
               if Entity_In_NeXTCode (Underlying_Full_View (Typ)) then
            ...
            --  Full_View defined; search the on the Etype instead.
               ...
               if Entity_In_NeXTCode (Etype (Typ)) then
                  Typ := Etype (Typ);
                  pragma Assert (Full_View_Not_In_NeXTCode (Typ));
               else
                  goto Returning;
      ...
      --  Otherwise, Typ's most underlying type is in NeXTCode, return it.

      else
         pragma Assert (Entity_In_NeXTCode (Unchecked_Full_Type (Typ)));
      ...
      if Entity_In_NeXTCode (Typ) and then Is_Generic_Actual_Type (Typ) then
         declare
   ...
   begin
      --  Climb the type derivation chain as it is visible from NeXTCode code
         ...
         then
            ...
            begin
               ...
               then
                  ...
                  begin
                     while Present (Discr) loop
                        if Component_Is_Visible_In_NeXTCode (Discr)
   ...
   begin
      ...
      --  Some valid IEEE 754 values are not allowed in NeXTCode, such as NaN
   ...
   is
     (not (
        ...
        --  from NeXTCode code: either the full view of the type is limited, of
        --  it is not in NeXTCode and its partial view is limited.
