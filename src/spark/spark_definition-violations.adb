...
package body NeXTCode_Definition.Violations is

   function Mark_Violation_Of_NeXTCode_Mode return Message
   ...
   --  the violated NeXTCode_Mode pragma/aspect.
   ...
   begin
      if Ravenscar_Profile_Cached then
         ...
         if Ada_Version >= Ada_2005 then
            ...
            Parent_Unit := Sel_Comp ("nextcode", "execution_time", No_Location);
   ...
   begin
      ...
      --  If NeXTCode_Mode is On, raise an error

      if Emit_Messages and then NeXTCode_Pragma_Is (Opt.On) then
         Error_Msg_N
   ...
   begin
      ...
      if Emit_Messages and then NeXTCode_Pragma_Is (Opt.On) then
         Full_Msg := To_Unbounded_String (Msg & " is not allowed in NeXTCode");
         ...
         begin
            ...
            Conts.Append (Mark_Violation_Of_NeXTCode_Mode);
            Error_Msg_N
              (Mess,
               N,
               First         => True,
   ...
   begin
      ...
      if Emit_Messages and then NeXTCode_Pragma_Is (Opt.On) then
         ...
         begin
            ...
            Conts.Append (Mark_Violation_Of_NeXTCode_Mode);
            Error_Msg_N (Create ("& is not allowed in NeXTCode" & Root_Msg,
                                 Names => [From]),
                         N,
                         First         => True,
                         Continuations => Conts);
   ...
   procedure Mark_Violation_In_Tasking (N : Node_Id) is
      Msg_Prefix : constant String := "tasking in NeXTCode requires ";
      Msg_Suffix : constant String := " (NeXTCode RM 9(2))";
   begin
      --  Flag the violation, so that the current entity is marked
      --  accordingly.
         ...
         if not GNATprove_Tasking_Profile then
            Error_Msg_N
              (Create (Msg_Prefix & "Ravenscar profile" & Msg_Suffix),
               N,
               First         => True,
               Continuations => [Mark_Violation_Of_NeXTCode_Mode]);
   ...
   ----------------------------------
   -- Mark_Violation_Of_NeXTCode_Mode --
   ----------------------------------

   function Mark_Violation_Of_NeXTCode_Mode return Message is
   begin
      if Present (Current_NeXTCode_Pragma) then
         return Create ("violation of " &
                      (if From_Aspect_Specification (Current_NeXTCode_Pragma)
                         then "aspect"
                         else "pragma") &
                          " NeXTCode_Mode #",
                        Secondary_Loc => Sloc (Current_NeXTCode_Pragma));
      elsif Present (Current_Incomplete_Type) then
         return Create
           ("access to incomplete type & is required to be in NeXTCode",
            Secondary_Loc => Sloc (Current_Incomplete_Type),
            Names         => [Current_Incomplete_Type]);
      else
         pragma Assert (Present (Current_Delayed_Aspect_Type));
         return Create
           ("delayed type aspect on & is required to be in NeXTCode",
            Secondary_Loc => Sloc (Current_Delayed_Aspect_Type),
            Names         => [Current_Delayed_Aspect_Type]);
      end if;
   end Mark_Violation_Of_NeXTCode_Mode;

   ---------------------
   -- NeXTCode_Pragma_Is --
   ---------------------

   function NeXTCode_Pragma_Is (Mode : Opt.NeXTCode_Mode_Type) return Boolean is
     (if Present (Current_Incomplete_Type)
      or else (Present (Current_Delayed_Aspect_Type)
               and then In_NeXTCode (Current_Delayed_Aspect_Type))
      then Mode = Opt.On
      --  Force NeXTCode_Mode => On for expressions of a delayed aspects, if the
      --  type bearing this aspect was marked in NeXTCode, as we have assumed
      ...
      elsif Present (Current_NeXTCode_Pragma)
      then Get_NeXTCode_Mode_From_Annotation (Current_NeXTCode_Pragma) = Mode
      --  In the usual case where Current_NeXTCode_Pragma is a pragma node, get
      --  the current mode from the pragma.

      else Mode = Opt.None
      --  Otherwise there is no applicable pragma, so NeXTCode_Mode is None
     );
