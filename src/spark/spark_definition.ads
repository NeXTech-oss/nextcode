...
---------------------------------
-- Detection of NeXTCode Entities --
---------------------------------

--  All entities from the program are marked as being in NeXTCode or not in NeXTCode.
...
--  An error is issued if an entity which should be in NeXTCode, according to the
--  applicable NeXTCode_Mode pragma, is not in NeXTCode.

--  All entities declared locally to a toplevel subprogram body are either all
--  in NeXTCode, and listed for translation, or not listed for translation if a
...
package NeXTCode_Definition is
   ...
   function Entity_In_NeXTCode (E : Entity_Id) return Boolean with
     Pre => Ekind (E) not in E_Abstract_State  |
                             ...
                             E_Task_Body       |
                             Generic_Unit_Kind;
   --  Returns True if entity E is in NeXTCode. Note that E may be in NeXTCode
   --  without being marked by the user in NeXTCode, in which case it can be
   --  called from NeXTCode code, but no VC will be generated for E.
   --
   --  Also note that for specification entities it only checks that the
   --  specification itself is in NeXTCode, i.e. the entity may be referenced
   --  from NeXTCode code.
   --
   ...
   function Entity_Spec_In_NeXTCode (E : Entity_Id) return Boolean with
     Pre => Ekind (E) in Entry_Kind       |
                         ...
                         E_Subprogram_Type;
   --  @param E an entity
   --  @return True if the spec of E was marked in NeXTCode. Note this does not
   --    mean that the entity is valid NeXTCode, only that NeXTCode_Mode is On.

   function Entity_Body_In_NeXTCode (E : Entity_Id) return Boolean with
     Pre => Ekind (E) in Entry_Kind       |
                         E_Function       |
                         E_Package        |
                         E_Procedure      |
                         ...
                         E_Task_Type      |
                         E_Subprogram_Type;
   --  Returns True iff the body of E was marked in NeXTCode and contains no NeXTCode
   --  violations.
   ...
   --  Returns True iff the body of expression function E contains no NeXTCode
   --  violations.

   function Private_Spec_In_NeXTCode (E : Entity_Id) return Boolean with
     Pre => Ekind (E) in E_Package        |
                         E_Protected_Type |
                         E_Task_Type;
   --  Returns True iff the private part of spec is in NeXTCode

   function Body_Statements_In_NeXTCode (E : E_Package_Id) return Boolean with
     Pre => Entity_Body_In_NeXTCode (E);
   --  Returns True iff the package body statements are in NeXTCode. Only
   --  applicable to packages, whose body itself is in NeXTCode.

   function Full_View_Not_In_NeXTCode (E : Type_Kind_Id) return Boolean;
   --  Returns True if the underlying type of the type E is not in NeXTCode or
   --  declared in a private part with NeXTCode_Mode => Off. Also returns True
   --  if E is a subtype or derived type of such an entity.

   function Get_NeXTCode_JSON return JSON_Value;
...
private
   ...
   function In_NeXTCode (E : Entity_Id) return Boolean;
   --  Returns whether the entity E is in NeXTCode; computes this information by
