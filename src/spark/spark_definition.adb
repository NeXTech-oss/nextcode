...
with NeXTCode_Atree.Entities;
with NeXTCode_Util;                     use NeXTCode_Util;
with NeXTCode_Definition.Annotate;      use NeXTCode_Definition.Annotate;
with NeXTCode_Definition.Violations;    use NeXTCode_Definition.Violations;
with NeXTCode_Util.Hardcoded;           use NeXTCode_Util.Hardcoded;
with NeXTCode_Util.Subprograms;         use NeXTCode_Util.Subprograms;
...
package body NeXTCode_Definition is

   -----------------------------------------
   -- Marking of Entities in NeXTCode or Not --
   -----------------------------------------

   --  This pass detects which entities are in NeXTCode and which are not, based
   --  on the presence of NeXTCode_Mode pragmas in the source, and the violations
   --  of NeXTCode restrictions. Entities that are not in NeXTCode may still be
   --  translated in Why, although differently than entities in NeXTCode
   --  (variables not in NeXTCode are still declared in case they appear in Global
   --  contracts).
   ...
   --  NeXTCode or not. We remember which entities are being marked, to avoid
   ...
   --  Any violation of NeXTCode rules results in the current toplevel subprogram
   --  (unit subprogram, or subprogram only contained in packages all the
   --  way to the unit level) to be not in NeXTCode, as well as everything it
   --  defines locally.

   --  An error is raised if an entity that has a corresponding NeXTCode_Mode
   --  pragma of On is determined to be not in NeXTCode.

   --  Each entity is added to the list of entities Entity_List. The
   --  translation will depend on the status (in NeXTCode or not) of each entity,
   --  and on where the entity is declared (in the current unit or not).

   --  A subprogram spec can be in NeXTCode even if its body is not in NeXTCode.
   ...
   --  Private types are always in NeXTCode (except currently record (sub)type
   --  with private part), even if the underlying type is not in NeXTCode. This
   --  allows operations which do not depend on the underlying type to be in
   --  NeXTCode, which is the case in client code that does not have access to the
   ...
   --  full view are treated specially, so that they are in NeXTCode only if the
   --  most underlying type is in NeXTCode. This most underlying type is the last
   ...
   --  Partial views of deferred constants may be in NeXTCode even if their full
   --  view is not in NeXTCode. This is the case if the type of the constant is
   --  in NeXTCode, while its initializing expression is not.
   ...
   function NeXTCode_Pragma_Of_Entity (E : Entity_Id) return Node_Id;
   --  Return NeXTCode_Pragma that applies to entity E. This function is basically
   --  the same as Einfo.NeXTCode_Pragma, but it is more general because it will
   --  work for any entity.
   --
   --  NeXTCode_Pragma cannot be directly specified for types nor declare blocks
   --  but comes from the most immediate scope where pragma NeXTCode_Mode can be
   --  attached. Then, for NeXTCode_Pragma coming from package entity (either body
   ...
   --  non-package entities in NeXTCode and packages with explicit NeXTCode_Mode =>
   ...
   Entities_In_NeXTCode : Hashed_Node_Sets.Set;
   --  Entities in NeXTCode. An entity is added to this set if, after marking,
   --  no violations where attached to the corresponding scope. Standard
   --  entities are individually added to this set.

   Bodies_In_NeXTCode : Hashed_Node_Sets.Set;
   --  Unique defining entities whose body is marked in NeXTCode; for kinds of
   --  entities in this set see the contract of Entity_Body_In_NeXTCode.

   Bodies_Compatible_With_NeXTCode : Hashed_Node_Sets.Set;
   --  Unique defining entities for expression functions whose body does not
   --  contain NeXTCode violations. Entities that are in this set and not in
   --  Bodies_In_NeXTCode are expression functions that are compatible with
   --  NeXTCode and subject to NeXTCode_Mode of Auto. Thus, their body should not
   ...
   --  NeXTCode_Mode of Auto.

   Full_Views_Not_In_NeXTCode : Hashed_Node_Sets.Set;
   --  Registers type entities in NeXTCode whose full view was declared in a
   --  private part with NeXTCode_Mode => Off or a subtype or derived type of such
   --  an entity.
   ...
   --  NeXTCode_Mode entity if there is one or to their type entity in discovery
   --  mode.
   ...
   function Entity_In_NeXTCode (E : Entity_Id) return Boolean
     renames Entities_In_NeXTCode.Contains;

   function Entity_Marked (E : Entity_Id) return Boolean
     renames Entity_Set.Contains;

   function Entity_Body_In_NeXTCode (E : Entity_Id) return Boolean
     renames Bodies_In_NeXTCode.Contains;

   function Entity_Body_Compatible_With_NeXTCode
     (E : E_Function_Id)
      return Boolean
   is (Bodies_Compatible_With_NeXTCode.Contains (E));

   function Full_View_Not_In_NeXTCode (E : Type_Kind_Id) return Boolean is
     (Full_Views_Not_In_NeXTCode.Contains (E));
   ...
   --  Return True if node Alloc is a valid allocating context (NeXTCode RM 4.8).
   ...
   --  Return True if there is a type with a type invariant visible in NeXTCode
   ...
   ------------------------------
   -- Body_Statements_In_NeXTCode --
   ------------------------------

   function Body_Statements_In_NeXTCode (E : E_Package_Id) return Boolean is
      Prag : constant Node_Id :=
        NeXTCode_Aux_Pragma (Defining_Entity (Package_Body (E)));
   begin
      return
        (if Present (Prag) then Get_NeXTCode_Mode_From_Annotation (Prag) /= Off);
   end Body_Statements_In_NeXTCode;

   --------------------------
   -- Entity_Spec_In_NeXTCode --
   --------------------------

   function Entity_Spec_In_NeXTCode (E : Entity_Id) return Boolean is
      Prag : constant Node_Id := NeXTCode_Pragma (E);
   begin
      return
        Present (Prag) and then Get_NeXTCode_Mode_From_Annotation (Prag) = Opt.On;
   end Entity_Spec_In_NeXTCode;

   ---------------------------
   -- Private_Spec_In_NeXTCode --
   ---------------------------

   function Private_Spec_In_NeXTCode (E : Entity_Id) return Boolean is
   begin
      return
        Entity_Spec_In_NeXTCode (E) and then
        Get_NeXTCode_Mode_From_Annotation (NeXTCode_Aux_Pragma (E)) /= Off;
   end Private_Spec_In_NeXTCode;
   ...
   --  their refined variant). We want to detect constants not in NeXTCode, even
   --  if they only appear in the flow contracts, to handle them as having no
   --  variable input.

   function Most_Underlying_Type_In_NeXTCode (Id : Type_Kind_Id) return Boolean;
   --  Mark the Retysp of Id and check that it is not completely private

   function Retysp_In_NeXTCode (E : Type_Kind_Id) return Boolean
     with Post => (if not Retysp_In_NeXTCode'Result then not Entity_In_NeXTCode (E));
   --  Returns whether the representive type of the entity E is in NeXTCode;
   --  computes this information by calling Mark_Entity, which is very cheap.
   --  Theoretically, it is equivalent to In_NeXTCode (Retyps (E)) except that
   ...
   --  detected, E is added to the Entities_In_NeXTCode.
   ...
   --  that declarations of constants in actions are possibly marked in NeXTCode.
   ...
   procedure Mark_Type_With_Relaxed_Init
     (N   : Node_Id;
      Ty  : Type_Kind_Id;
      Own : Boolean := False)
     with Pre => Entity_In_NeXTCode (Ty);
   ...
   begin
      if not Is_Access_Type (Root_Retysp (Expected_Type)) then
      ...
      elsif not Most_Underlying_Type_In_NeXTCode (Actual_Type) then
         ...
         --  designated types are in NeXTCode (we need to check this here as
         --  marking of the designated type of recursive access types might be
         --  deferred).

         if In_NeXTCode (Actual_Des_Ty)
           and then In_NeXTCode (Expected_Des_Ty)
               ...
               if Nkind (N) = N_Procedure_Call_Statement then
                  ...
                  begin
                     ...
                     while Present (Formal) loop
                        if not Is_Constant_In_NeXTCode (Formal) then
                           goto Not_Lemma;
                        end if;
                        Next_Formal (Formal);
                     end loop;
   ...
   begin
      --  NeXTCode RM 3.10(4): If the target of an assignment operation is an
               ...
               then
                  Mark_Violation
                    ("borrow or observe of a non-traversal function call",
                     Root_Expr,
                     SRM_Reference => "NeXTCode RM 3.10(4)");
      ...
      --  which case the borrower is constant.
        ...
        and then Is_Constant_In_NeXTCode (Root)
        and then
   ...
   begin
      if not Use_Predefined_Equality_For_Type (Ty) then
         Eq := Ultimate_Alias
           (NeXTCode_Util.Types.Get_User_Defined_Eq (Base_Type (Ty)));

         Mark_Entity (Eq);
         if not Entity_In_NeXTCode (Eq) then
            Mark_Violation
              (Msg & " whose primitive equality is not in NeXTCode",
               N);
   ...
   begin
      while Present (Decl) loop
         if Nkind (Decl) = N_Private_Type_Declaration
           and then In_NeXTCode (Defining_Identifier (Decl))
           and then Has_Invariants_In_NeXTCode (Defining_Identifier (Decl))
         then
            return True;
         end if;
         Next (Decl);
   ...
   function Get_NeXTCode_JSON return JSON_Value is
      NeXTCode_Status_JSON : constant JSON_Value := Create_Object;
   ...
   begin
      ...
      --  NeXTCode. However, Entity_Tree does not contain protected types (maybe
      --  it should?) while we want to generate NeXTCode status for them (maybe
      --  we should not?).
      ...
      for E of Entity_List loop
         ...
         then
            declare
               ...
               NeXTCode_Status : constant NeXTCode_Mode_Status :=
                 (if Entity_Body_In_NeXTCode (E)
                  then All_In_NeXTCode
                  elsif Entity_Spec_In_NeXTCode (E)
                  then
                    (if Ekind (E) = E_Package and then No (Package_Body (E))
                     ...
                     then All_In_NeXTCode
                     else Spec_Only_In_NeXTCode)
                  else Not_In_NeXTCode);
            begin
               Set_Field (NeXTCode_Status_JSON, To_Key (V),
                          To_JSON (NeXTCode_Status));
            end;

         elsif Is_Type (E)
           and then Entity_In_NeXTCode (E)
           and then E = Retysp (E)
           and then Analysis_Requested (E, With_Inlined => True)
           and then
         ...
         then
            ...
            declare
               V            : constant Subp_Type :=
                 Entity_To_Subp_Assumption (E);
               NeXTCode_Status : constant NeXTCode_Mode_Status :=
                 (if Full_View_Not_In_NeXTCode (E)
                  then Spec_Only_In_NeXTCode
                  else All_In_NeXTCode);
            begin
               Set_Field (NeXTCode_Status_JSON, To_Key (V),
                          To_JSON (NeXTCode_Status));
            end;
      ...
      return NeXTCode_Status_JSON;
   end Get_NeXTCode_JSON;
   ...
   function In_NeXTCode (E : Entity_Id) return Boolean is
   begin
      ...
      Mark_Entity (E);
      return Entities_In_NeXTCode.Contains (E);
   end In_NeXTCode;
   ...
   begin
      ...
      then
         ...
         --  If present, the type of N should be in NeXTCode. This also allows
      ...
      case Nkind (N) is
         ...
         when N_Aggregate =>
            ...
            if Is_Container_Aggregate (N) then
               ...
               if not Has_Aggregate_Annotation (Etype (N)) then
                  ...
                  begin
                     if not In_NeXTCode (Annot.Empty_Function) then
                        Mark_Violation (N, From => Annot.Empty_Function);
                     elsif not In_NeXTCode (Annot.Add_Procedure) then
            ...
            elsif not Most_Underlying_Type_In_NeXTCode (Etype (N)) then
               Mark_Violation (N, From => Etype (N));
            else
               if Is_Array_Type (Etype (N)) then
         ...
         when N_Allocator =>
            ...
            elsif Retysp_In_NeXTCode (Etype (N))
            ...
            then
               ...
               --  have an uninitialized allocator.
                  ...
                  begin
                     ...
                     elsif not In_NeXTCode (Typ) then
         ...
         when N_Assignment_Statement =>
            ...
            begin
               ...
               elsif Is_Constant_In_NeXTCode (Get_Root_Object (Var)) then
               ...
               --  NeXTCode RM 3.10(8): If the type of the target is an anonymous
               ...
               --  observers as for borrowers. To be seen if the NeXTCode RM
                  ...
                  then
                     Mark_Violation
                       ((if Is_Access_Constant (Etype (Var))
                        ...
                        & " as a root",
                        Expr,
                        SRM_Reference => "NeXTCode RM 3.10(8)");
                  end if;

               --  If we are performing a move operation, check that we are
               --  moving a path.

               elsif Retysp_In_NeXTCode (Etype (Var))
         ...
         when N_Explicit_Dereference =>
            if not Most_Underlying_Type_In_NeXTCode (Etype (Prefix (N))) then
         ...
         when N_Extension_Aggregate =>
            if not Most_Underlying_Type_In_NeXTCode (Etype (N)) then
               Mark_Violation (N, From => Etype (N));
            ...
            then
               ...
               --  The second case is not currently supported in NeXTCode.
         ...
         when N_Iterator_Specification =>
            ...
            --  Mark the name first, since Has_Iterable_In_NeXTCode
            ...
            if Has_Iterable_Aspect_In_NeXTCode (Etype (Name (N))) then
            ...
            then
               ...
               --  other forms of iteration are not allowed in NeXTCode.

               Mark_Violation ("iterator specification", N,
                               SRM_Reference => "NeXTCode RM 5.5.2");
            end if;
         ...
         when N_Null =>
            if not Retysp_In_NeXTCode (Etype (N))
         ...
         --  by zero. No condition should be present in NeXTCode code, but accept
         ...
         when N_Selected_Component =>
            ...
            begin
               if No (Search_Component_By_Name (Prefix_Type, Selector)) then
                  if NeXTCode_Pragma_Is (Opt.On) then
                     Error_Msg_N
            ...
            --  most underlying one) to see whether the access is in NeXTCode. An
            --  exception is the access to discrimants to a private type whose
            --  full view is not in NeXTCode.

            if not Retysp_In_NeXTCode (Etype (Prefix (N))) then
               Mark_Violation (N, From  => Etype (Prefix (N)));
            end if;
         ...
         when N_Subprogram_Declaration =>
            ...
            --  corresponding tagged view is in a NeXTCode-invisible section.
            ...
            begin
               if Is_Predicate_Function (E) then
                  ...
                  declare
                     ...
                     Prag : constant Node_Id :=
                       NeXTCode_Pragma_Of_Entity (View);
                  begin
                     if Is_In_Hidden_Private (View)
                       or else
                         (Present (Prag)
                          and then Get_NeXTCode_Mode_From_Annotation (Prag) = Off)
                     then
                        Do_Mark := False;
                     end if;
                  end;
         ...
         =>
            ...
            --  rest of the checks on NeXTCode restrictions.
            ...
            if Has_Array_Type (Etype (N)) then
               ...
               declare
                  ...
                  --  NeXTCode representation of the type of source expression
               ...
               Scalar_Conversion : declare
                  ...
                  From_Modular_128 : constant Boolean :=
                    Has_Modular_Integer_Type (From_Type)
                      and then NeXTCode_Atree.Entities.Modular_Size
                        (Retysp (From_Type))
                     ...
                     --  avoid pulling inherited fields.
                        ...
                        begin
                           while Present (Comp) loop
                              if Component_Is_Visible_In_NeXTCode (Comp)
                                and then No
         ...
         =>
            --  Pick NeXTCode_Mode from the concurrent type definition

            declare
               Save_NeXTCode_Pragma : constant Opt_N_Pragma_Id :=
                 Current_NeXTCode_Pragma;
               E                 : constant Type_Kind_Id :=
                 Defining_Entity (N);
            begin
               Current_NeXTCode_Pragma := NeXTCode_Pragma (E);
               Mark_Entity (E);

               Mark_Concurrent_Type_Declaration (N);

               Current_NeXTCode_Pragma := Save_NeXTCode_Pragma;
            end;
         ...
         when N_Protected_Body
            | N_Task_Body
         =>
            if Is_NeXTCode_Tasking_Configuration then
               case Nkind (N) is
                  when N_Protected_Body =>
               ...
               declare
                  ...
                  begin
                     ...
                     --  NeXTCode, mark the choice and the prefix.
                        ...
                        if not Retysp_In_NeXTCode (Pref_Ty) then
                           Mark_Violation (Choice, From => Pref_Ty);
                           return;
                        end if;

                        if Nkind (Choice) = N_Indexed_Component then
                           if not Most_Underlying_Type_In_NeXTCode (Pref_Ty)
                           then
   ...
   begin
      ...
      declare
         ...
         E_Is_Constant   : constant Boolean := Is_Constant_In_NeXTCode (E);
      begin
         ...
         if not Supported_Alias then
            ...
            begin
               ...
               --  atomic. This partly addresses assumptions NeXTCode_EXTERNAL.
               ...
               --  NeXTCode_EXTERNAL and NeXTCode_ALIASING_ADDRESS.
               ...
               --  This addresses assumption NeXTCode_EXTERNAL_VALID.
         ...
         --  update. Check that we only accept cases we can handle that way.
            ...
            --  mutable or both are constant for NeXTCode.

            if E_Is_Constant /=
              (Is_Constant_In_NeXTCode (Aliased_Object)
   ...
   begin
      --  This case statement must agree with the table specified in NeXTCode RM
      ...
      case Attr_Id is
         ...
         --  the attributes marked "Yes" in NeXTCode RM 15.2 for which we support
         --  non-static values.
         ...
         =>
            ...
            if not Retysp_In_NeXTCode (Etype (P))
            ...
            then
              ...
              and then NeXTCode_Pragma_Is (Opt.On)
         ...
         --  These are the attributes marked "Warn" in NeXTCode RM 15.2.
         ...
         when Attribute_Initialized =>
            if not Retysp_In_NeXTCode (Etype (P)) then
         ...
         --  or attribute definition clause (NeXTCode RM 15.2).
         ...
         --  Check NeXTCode RM 3.10(13) regarding 'Old and 'Loop_Entry on access
         --  types.
                  ...
                  then
                     ...
                     Mark_Violation
                       ("prefix of """ & Astring & """ introducing aliasing",
                        P, SRM_Reference => "NeXTCode RM 3.10(13)",
               ...
               then
                  ...
                  begin
                     ...
                     if (Is_Anonymous_Access_Object_Type (Etype (Root))
                         or else not Is_Constant_In_NeXTCode (Root)
   ...
   begin
      --  Call is in NeXTCode only if the subprogram called is in NeXTCode.
      --
      ...
      if Nkind (N) in N_Op_Eq | N_Op_Ne
        and then Retysp_In_NeXTCode (Etype (Left_Opnd (N)))
      ...
      if Emit_Warning_Info_Messages
        ...
        --  are only noise, because a user sets the strict NeXTCode mode precisely
   ...
   procedure Mark_Call (N : Node_Id) is
      ...
      begin
         ...
         then
            ...
            --  to an instance of Unchecked_Conversion. (NeXTCode RM 7.1.3(9)).
               ...
               Mark_Violation
                 (Msg           =>
                  ...
                  SRM_Reference => "NeXTCode RM 7.1.3(9)");
            end if;
         end if;
         ...
         if not Is_Constant_In_NeXTCode (Formal) then
            ...
            begin
               ...
               elsif Is_Constant_In_NeXTCode (Get_Root_Object (Actual)) then
                  Mark_Violation
         ...
         if In_NeXTCode (Etype (Formal)) then
   ...
   begin
      ...
      if Nkind (Name (N)) = N_Explicit_Dereference then
         ...
         --  Make sure they are never called from NeXTCode code.
      ...
      --  [right-hand side] expression of an assignment statement. (NeXTCode
      --  RM 6.1.11(4))
      ...
      if not In_NeXTCode (E) then
         if Is_Subprogram (E) then
            declare
               ...
               --  If the body is marked as NeXTCode_Mode(Off), then having
               --  a separate declaration could allow marking it as
               --  NeXTCode_Mode(On) so that it can be called from NeXTCode code. Do
               ...
               Prag_N : constant Node_Id :=
                 (if Present (Body_N) then
                    NeXTCode_Pragma_Of_Entity (Subprogram_Body_Entity (E))
                  else Empty);
               Cont_Msg : constant String :=
                 (if Present (Prag_N)
                    and then Sloc (Spec_N) = Sloc (Body_N)
                    and then Get_NeXTCode_Mode_From_Annotation (Prag_N) = Opt.Off
                  then
                     "separate subprogram declaration from subprogram body to "
                     & "allow calling it from NeXTCode code"
                  else "");
            begin
      ...
      --  Warn about assumptions, but only when the NeXTCode_Mode is On

      elsif Emit_Warning_Info_Messages and then NeXTCode_Pragma_Is (Opt.On) then
   ...
   begin
      ...
      --  should not affect the NeXTCode status of the entities in the compilation
      ...
      --  Mark entities in NeXTCode or not
      ...
      loop
         ...
         --  Mark delayed type aspects
            ...
            --  If no NeXTCode_Mode is set for the type, we only mark delayed
            --  aspects for types which have been found to be in NeXTCode. In this
            --  case, every violation is considered an error as we can't easily
            --  backtrack the type to be out of NeXTCode.
            ...
            begin
               ...
               --  marked NeXTCode_Mode(On)...

               if Nkind (Delayed_Mapping) = N_Pragma then
                  Current_NeXTCode_Pragma := Delayed_Mapping;
                  Mark_Delayed_Aspect := True;

               --  Or if the type entity has been found to be in NeXTCode. In this
               --  case (scope not marked NeXTCode_Mode(On)), the type entity was
               --  stored as value in the Delayed_Type_Aspects map.

               elsif Retysp_In_NeXTCode (Delayed_Mapping) then
                  Current_NeXTCode_Pragma := Empty;
                  Mark_Delayed_Aspect := True;
         ...
         --  are in NeXTCode (otherwise an error is raised). Also populate
            ...
            begin
               if Entity_In_NeXTCode (E) then
                  ...
                  begin
                     ...
                     --  Get the appropriate NeXTCode pragma for the access type

                     Current_NeXTCode_Pragma := NeXTCode_Pragma_Of_Entity (E);

                     --  As the access type has already been found to be in
                     --  NeXTCode, force the reporting of errors by setting the
                     --  Current_Incomplete_Type.

                     if not NeXTCode_Pragma_Is (Opt.On) then
                        Current_Incomplete_Type := E;
                        Current_NeXTCode_Pragma := Empty;
                     end if;

                     if not Retysp_In_NeXTCode (Des_Ty) then
   ...
   begin
      --  We enforce NeXTCode RM 4.3(1) for which the box symbol, <>, shall not
                     ...
                     then
                        Mark_Violation
                          ("box notation without default or relaxed "
                           & "initialization",
                           Choice,
                           SRM_Reference => "NeXTCode RM 4.3(1)");
   ...
   begin
      ...
      --  shall be a library-level declaration (NeXTCode RM 7.1.3(3)).
      ...
      if Present (Type_Def) then
         ...
         declare
            Save_NeXTCode_Pragma : constant Node_Id := Current_NeXTCode_Pragma;

         begin
            Current_NeXTCode_Pragma := NeXTCode_Aux_Pragma (E);
            if not NeXTCode_Pragma_Is (Opt.Off) then
   ...
   procedure Mark_Entity (E : Entity_Id) is
      ...
      begin
         ...
         --  type as encapsulator must be (NeXTCode RM 9.4):
         ...
         then
            Mark_Violation ("not fully initialized part of " &
                            ...
                            Def, SRM_Reference => "NeXTCode RM 9.4");
         end if;

         --  The object is in NeXTCode if-and-only-if its type is in NeXTCode and
         --  its initialization expression, if any, is in NeXTCode.

         --  If the object's nominal and actual types are not in NeXTCode, then
         --  the expression can't be in NeXTCode, so we skip it to limit the
         --  number of error messages.

         if not Retysp_In_NeXTCode (T) then
         ...
         --  block statement (NeXTCode RM 3.10(5)).
               ...
               if not Is_Local_Context (Scop) then
                  Mark_Violation
                    ("object of anonymous access not declared "
                     ...
                     N, SRM_Reference => "NeXTCode RM 3.10(5)");
               end if;
            end;
         ...
         if Is_Effectively_Volatile (E) then
            ...
            --  A ghost type or object shall not be effectively volatile (NeXTCode
            --  RM 6.9(7)).
            if Is_Ghost_Entity (E) then
               Mark_Violation
                 ("volatile ghost object", N,
                  SRM_Reference => "NeXTCode RM 6.9(7)");

            --  The declaration of an effectively volatile stand-alone
            --  object or type shall be a library-level declaration
            --  (NeXTCode RM 7.1.3(3)). A return object introduced by
            ...
            --  with its type (NeXTCode RM 7.1.3(2)).
      ...
      begin
         --  If T is not in NeXTCode, E is not in NeXTCode. If T is a limited view
         ...
         elsif not Retysp_In_NeXTCode (T) then
            Mark_Violation (E, From => T);

         --  A discriminant or a loop parameter shall not be effectively
         --  volatile (NeXTCode RM 7.1.3(4)).
      ...
      procedure Mark_Subprogram_Entity (E : Callable_Kind_Id) is
         ...
         begin
            ...
            --  effectively volatile type (NeXTCode RM 7.1.3(8)).
            ...
            --  Check for the first formal to be in NeXTCode before calling
            ...
            if Is_Anonymous_Access_Object_Type (Etype (Id))
              and then
                (No (First_Formal (Id))
                 or else Retysp_In_NeXTCode (Etype (First_Formal (Id))))
                     ...
                     then
                        Mark_Violation
                          ("ghost function as user-defined equality"
                           ...
                           SRM_Reference => "NeXTCode RM 6.9(22)");
            ...
            while Present (Formal) loop
               ...
               --  of an effectively volatile type (NeXTCode RM 7.1.3(8)). Do
            ...
            --  If the result type of a subprogram is not in NeXTCode, then the
            --  subprogram is not in NeXTCode. If the result types is a limited
            ...
            elsif not Retysp_In_NeXTCode (Etype (Id)) then
               ...
               begin
                  ...
                  if not Globals.Outputs.Is_Empty then
                     ...
                     loop
                        ...
                        --  global inputs (NeXTCode RM 7.1.3(7)).
         ...
         begin
            while Present (Formal) loop
               if not In_NeXTCode (Formal) then
         ...
         is
            Disp_Typ : constant Opt_Type_Kind_Id :=
              NeXTCode_Util.Subprograms.Find_Dispatching_Type (Spec_Id);
      ...
      begin
         ...
         if Is_Protected_Operation (E)
           and then not Is_NeXTCode_Tasking_Configuration
         then
         ...
         --  expressions, which are not supported in NeXTCode.

         if not Is_Hardcoded_Entity (E) then
            Mark_Subprogram_Contracts;
         end if;

         --  Plain preconditions cannot be used in NeXTCode on dispatching
         --  subprograms. The reason for that is that otherwise the dynamic
         --  semantics of Ada combined with the verification of Liskov
         --  Substitution Principle in NeXTCode force Pre and Pre'Class to be
         ...
         --  subprogram in NeXTCode (dispatching on this subprogram is forbidden,
         --  and deriving such a type is also forbidden).

         if Is_Dispatching_Operation (E) then
            declare
               Typ      : constant Opt_Type_Kind_Id :=
                 NeXTCode_Util.Subprograms.Find_Dispatching_Type (E);
                  ...
                  then
                     Mark_Violation
                       ("plain precondition on dispatching subprogram",
                        Pre,
                        SRM_Reference => "NeXTCode RM 6.1.1(2)",
      ...
      procedure Mark_Type_Entity (E : Type_Kind_Id) is
         ...
         --  NeXTCode_Mode => Off or in the hidden private part of a withed unit.
         ...
         procedure Check_Not_Inheriting_Overriding_From_NeXTCode_Off
         ...
         ----------------------------------------------------
         -- Check_Not_Inheriting_Overriding_From_NeXTCode_Off --
         ----------------------------------------------------

         procedure Check_Not_Inheriting_Overriding_From_NeXTCode_Off is
            ...
            then
               ...
               for Prim of Iter (Direct_Primitive_Operations (E)) loop
                  ...
                  begin
                     ...
                     --  crossing NeXTCode boundaries arbitrarily, so that is
                     --  unsuitable.
                     ...
                     --  NeXTCode_Mode Off) is the last to declare an overriding
         ...
         end Check_Not_Inheriting_Overriding_From_NeXTCode_Off;
         ...
         begin
            ...
            return
              (Present (NeXTCode_Aux_Pragma (Defining_Entity (Pack_Decl)))
               and then Get_NeXTCode_Mode_From_Annotation
                 (NeXTCode_Aux_Pragma (Defining_Entity (Pack_Decl))) = Off)
      ...
      begin
         ...
         --  Controlled types are not allowed in NeXTCode
         ...
         --  be translated in NeXTCode. We add the entity of their full views to
         ...
         --  the full view might contain NeXTCode violations, but the partial view
         ...
         if not Is_Nouveau_Type (E)
           ...
           and then not Retysp_In_NeXTCode (Etype (E))
         ...
         --  as NeXTCode_Mode On, and the private part of the package is
         --  marked as NeXTCode_Mode Off. This is also used later during
         --  generation of Why.
         ...
         begin
            if Anc_Subt /= Etype (E)
              and then not Retysp_In_NeXTCode (Anc_Subt)
            then
            ...
            then
               ...
               declare
                  Delayed_Mapping : constant Node_Id :=
                    (if Present (Current_NeXTCode_Pragma)
                     then Current_NeXTCode_Pragma
                     else E);
               begin
         ...
         then
            Mark_Violation
              ("discriminant on derived type",
               Parent (E),
               SRM_Reference => "NeXTCode RM 3.7(2)");
         end if;
         ...
         then
            ...
            begin
               while Present (Disc) loop
                  ...
                  --  Check that the type of the discriminant is in NeXTCode

                  if not In_NeXTCode (Etype (Disc)) then
                     Mark_Violation (Disc, From => Etype (Disc));
                  end if;

                  --  A discriminant or a loop parameter shall not be
                  --  effectively volatile (NeXTCode RM 7.1.3(4)).
                  ...
                  --  specified in NeXTCode RM 3.10

                  if Has_Access_Type (Etype (Disc)) then
                     Mark_Violation ("access discriminant", Disc);
                  end if;

                  --  Check that the default expression is in NeXTCode

                  Mark_Default_Expression (Disc);

                  --  Check that the discriminant constraint is in NeXTCode
         ...
         --  define the type in Why3, if it happens to be in NeXTCode. Hence the
         ...
         then
            ...
            --  has NeXTCode_Mode => Off, we do not need to mark its underlying
            ...
            --  Incomplete types which are marked have no visible full view
               ...
               begin
                  if not In_NeXTCode (Utype)
                    or else Full_View_Not_In_NeXTCode (Utype)
                  then
                     Full_Views_Not_In_NeXTCode.Insert (E);
                     Discard_Underlying_Type (E);
                  end if;
         ...
         if Has_Own_Invariants (E) then
            ...
            --  Classwide invariants are not in NeXTCode

            if Has_Inheritable_Invariants (E) then
               Mark_Violation
                 ("classwide invariant", E,
                  SRM_Reference => "NeXTCode RM 7.3.2(2)");

            --  Partial invariants are not allowed in NeXTCode
               ...
               Mark_Violation
                 ("type invariant on effectively volatile type",
                  E, SRM_Reference => "NeXTCode RM 7.3.2(4)");
            ...
            then
               ...
               declare
                  ...
                  --  NeXTCode_Util as it may not have been constructed yet.
               ...
               begin
                  ...
                  --  new RM wording in NeXTCode or new syntax in Ada (see
                  --  P826-030).
                  ...
                  --  calls to root primitives (see NeXTCode RM 7.3.2(8) and
         ...
         --  cannot have a predicate (NeXTCode RM 3.2.4(3)). Here, we do not try
         --  to distinguish the case where the predicate is inherited from a
         --  parent whose full view is not in NeXTCode.
         ...
         then
            Mark_Violation
              ("subtype predicate on effectively volatile type for reading",
               E, SRM_Reference => "NeXTCode RM 3.2.4(3)");
         end if;
         ...
         begin
            if Present (Decl)
              ...
              and then Entity_In_NeXTCode (Full)
         ...
         --  tagged cannot be derived (NeXTCode RM 3.4(1)).
               ...
               then
                  Mark_Violation
                    ("deriving & from & declared as untagged private", E,
                     ...
                     SRM_Reference => "NeXTCode RM 3.4(1)",
                     Root_Cause_Msg =>
         ...
         --  (see NeXTCode RM 7.3.2(8) and test
         ...
         then
            ...
            begin
               while Present (Comp) loop
                  if Component_Is_Visible_In_NeXTCode (Comp)
                    and then In_NeXTCode (Etype (Comp))
         ...
         --  type shall be a library-level declaration (NeXTCode RM 7.1.3(3)).
         ...
         if Is_Array_Type (E) then
            ...
            begin
               ...
               --  volatility with the composite type (NeXTCode RM 7.1.3(6)).
               if Is_Effectively_Volatile (E) then
                  Check_Volatility_Compatibility
                    (Component_Type (E), E,
               ...
               --  Check that all index types are in NeXTCode

               while Present (Index) loop
                  if not In_NeXTCode (Etype (Index)) then
                     Mark_Violation (E, From => Etype (Index));
                  end if;
                  Next_Index (Index);
               end loop;

               --  Check that component type is in NeXTCode

               if not In_NeXTCode (Component_Typ) then
         ...
         --  Most discrete and floating-point types are in NeXTCode
            ...
            then
               ...
               begin
                  pragma Assert
                    (In_NeXTCode (Etype (Low))
                     and then In_NeXTCode (Etype (High))
                     and then not Has_Fixed_Point_Type (Etype (Low))
                     and then not Has_Fixed_Point_Type (Etype (High)));
               end;
            end if;

            --  Check that the range of the type is in NeXTCode
            ...
            --  Class wide types with a non NeXTCode root are not in NeXTCode.
            --  Remark that the violation is always redundant for classwide
            --  types implicitely declared on code with NeXTCode_Mode => On.
            ...
            if Present (Full_View (E))
              and then Entity_In_NeXTCode (Full_View (E))
            ...
            --  NeXTCode_Mode, we cannot support it in NeXTCode. Indeed, we
            ...
            then
               declare
                  Scop     : constant Entity_Id := Scope (E);
                  Prag     : constant Node_Id := NeXTCode_Pragma (Scop);
                  Aux_Prag : constant Node_Id := NeXTCode_Aux_Pragma (Scop);
                  Rep      : Node_Id := First_Rep_Item (Full_View (E));
                  Found    : Boolean := False;
                  Full     : Boolean := False;
               ...
               begin
                  ...
                  --  NeXTCode_Mode or if the private part of the enclosing
                  --  package is hidden.
                  ...
                  if (Present (Prag)
                      ...
                      and then Get_NeXTCode_Mode_From_Annotation (Prag) /=
                        Get_NeXTCode_Mode_From_Annotation (Aux_Prag))
                        ...
                        begin
                           if Found and then Full /= N_Full then
                              if Has_Hidden_Private_Part (Scop) then
                                 ...
                                 Mark_Unsupported
                                   (Lim_Predicate_With_Different_NeXTCode_Mode,
                                    E);
                              end if;
                              exit;
                           end if;
            ...
            --  private part with NeXTCode_Mode Off, all (public) primitive
            ...
            --  type defined under a part with NeXTCode_Mode Off, which leads to
            ...
            if Ekind (E) = E_Record_Subtype
              and then not In_NeXTCode (Base_Type (E))
            then
            ...
            --  not have a discriminated part (NeXTCode RM 7.1.3(5)).
            if Is_Effectively_Volatile (E) then
               if Has_Discriminants (E) then
            ...
            --  with respect to volatility with the composite type (NeXTCode
            --  RM 7.1.3(6)).
            else
               declare
            ...
            --  a component type which yields synchronized objects (NeXTCode RM
            --  9.5).
            ...
            --  A ghost type shall not have a task or protected part (NeXTCode RM
            --  6.9(21)).
            if Is_Ghost_Entity (E) then
               declare
                  Comp : Opt_E_Component_Id := First_Component (E);
            ...
            --  Components of a record type should be in NeXTCode for the record
            --  type to be in NeXTCode.
               ...
               begin
                  while Present (Comp) loop
                     ...
                     if not Is_Tag (Comp)
                       ...
                       --  NeXTCode_Mode => Off.
                       and then Component_Is_Visible_In_NeXTCode (Comp)
                     then
                        Comp_Type := Etype (Comp);

                        if not In_NeXTCode (Comp_Type) then
                           Mark_Violation (Comp, From => Comp_Type);
                        else

                           --  Tagged extensions cannot have owning components
                           --  in NeXTCode nor components with relaxed init.
                     ...
                     if Enclosing_Dynamic_Scope (Etype (E)) /= Scop then
                        Mark_Violation
                          ("local derived type from non-local parent",
                           E,
                           SRM_Reference => "NeXTCode RM 3.9.1(1)");
                     end if;
            ...
            --  A record type may have a type with full_view not in NeXTCode as an
            --  etype. In this case, the whole type has fullview not in NeXTCode.

            if Full_View_Not_In_NeXTCode (Etype (E)) then
               Full_Views_Not_In_NeXTCode.Insert (E);
            end if;
            ...
            begin
               ...
               --  with the access type (NeXTCode RM 7.1.3(6)).
               Check_Volatility_Compatibility
               ...
               --  Storage_Pool is not in NeXTCode
               ...
               --  Storage_Size is not in NeXTCode
               ...
               --  Store the type in the Incomplete_Type map to be marked later
                  ...
                  --  NeXTCode_mode to avoid crashes if they are out of NeXTCode
                  --  later.

                  if Is_Declared_In_Private (Des_Ty)
                    and then No (NeXTCode_Pragma_Of_Entity (Des_Ty))
                  then
                     Mark_Violation (E, From => Des_Ty);
                  else
               ...
               --  full view is not in NeXTCode.
                 ...
                 and then not Most_Underlying_Type_In_NeXTCode (Base_Retysp (E))
               then
               ...
               elsif not Retysp_In_NeXTCode (Des_Ty) then
                  Mark_Violation (E, From => Des_Ty);
               end if;
            end;
            ...
            if not Is_NeXTCode_Tasking_Configuration then
               Mark_Violation_In_Tasking (E);

            --  To know whether the fullview of a protected type with no
            --  NeXTCode_Mode is in NeXTCode, we need to mark its components.
            ...
            then
               ...
               begin
                  ...
                  if Present (Type_Def) then
                     ...
                     declare
                        Save_NeXTCode_Pragma : constant Node_Id :=
                          Current_NeXTCode_Pragma;
                  ...
                  if Ekind (E) = E_Protected_Type
                    and then not NeXTCode_Pragma_Is (Opt.Off)
                  then
                     declare
                     ...
                     begin
                        while Present (Comp) loop
                           ...
                           if In_NeXTCode (Etype (Comp)) then
                        ...
                        if Is_Single_Concurrent_Type (E) then
                           ...
                           loop
                              ...
                              if Is_Object (Part)
                                and then Retysp_In_NeXTCode (Etype (Part))
                                and then Is_Anonymous_Access_Object_Type
                                  (Retysp (Etype (Part)))
                              then
                                 Mark_Violation
                        ...
                        --  view of the type is forced to be NeXTCode. Violations
                        --  found during marking of the private part are not
                        --  reverted.

                        if NeXTCode_Pragma_Is (Opt.On) then
                           Fullview_In_NeXTCode := True;

                           --  If a violation has been found while marking the
                           --  private components of the protected type, then
                           --  its full view is not in NeXTCode. The type itself
                           --  can still be in NeXTCode if no NeXTCode_Mode has been
                           --  specified.

                        else
                           pragma Assert (NeXTCode_Pragma_Is (Opt.None));

                           Fullview_In_NeXTCode := not Violation_Detected;
                           Violation_Detected := Save_Violation_Detected;
                        end if;
                     end;

                  --  The full view of a task is in NeXTCode

                  else
                     Fullview_In_NeXTCode := Is_Task_Type (E);
                  end if;

                  Current_NeXTCode_Pragma := Save_NeXTCode_Pragma;

                  --  If the protected type is in NeXTCode but not its full view,
                  --  store it in Full_Views_Not_In_NeXTCode.

                  if not Violation_Detected and then not Fullview_In_NeXTCode then
                     Full_Views_Not_In_NeXTCode.Insert (E);
                  end if;
               end;
   ...
   begin
      ...
      if Ekind (E) in E_Protected_Type | E_Task_Type then
         ...
         --  NeXTCode_Implicit_Load in Analyze_Protected_Type_Declaration and
      ...
      --  If the entity is declared in the scope of NeXTCode_Mode => Off, then do
      --  not consider whether it could be in NeXTCode or not. Restore NeXTCode_Mode
      --  pragma before returning.

      if NeXTCode_Pragma_Is (Opt.Off) then
         ...
         if Emit_Messages then
            Add_Violation_Root_Cause
              (E, Msg => "entity declared with NeXTCode_Mode Off");
         end if;
      ...
      --  For recursive references, start with marking the entity in NeXTCode

      Entities_In_NeXTCode.Include (E);
      ...
      --  NeXTCode_Mode On, and the private part of the package is marked
      --  as NeXTCode_Mode Off. This is also used later during generation of Why.
      ...
      --  If a violation was detected, remove E from the set of NeXTCode entities
      ...
      --  Otherwise, add entity to appropriate list
         ...
         case Ekind (E) is
            ...
            when E_Protected_Type | E_Task_Type =>
               ...
               --  refinement is not hidden behind a NeXTCode_Mode => Off) or
   ...
   begin
      --  NeXTCode RM 3.10(6): return statement of traversal function

      if Is_Traversal_Function (Subp) then
         Mark_Violation
           ("extended return applying to a traversal function",
            N,
            SRM_Reference => "NeXTCode RM 3.10(6)");
      end if;
      ...
      case Ekind (E) is
         when Object_Kind =>
            if Ekind (E) in E_Variable | E_Constant | Formal_Kind then
               ...
               --  non-interfering context (NeXTCode RM 7.1.3(9)).
            ...
            --  Record components and discriminants are in NeXTCode if they are
            --  visible in the representative type of their scope. Do not
            --  report a violation if the type itself is not NeXTCode, as the
            --  violation will already have been reported.
               ...
               begin
                  if not Retysp_In_NeXTCode (Ty)
                    or else not Component_Is_Visible_In_NeXTCode (E)
                  then
         ...
         --  and families are not supported in NeXTCode (yet), so we should never
   ...
   begin
      if Present (Condition (N)) then
         ...
         --  The loop parameter shall be added to the entities in NeXTCode
         declare
   ...
   begin
      if In_NeXTCode (E) then
         pragma Assert (In_NeXTCode (Etype (E)));
      else
         Mark_Violation (N, From => E);
      end if;
   ...
   procedure Mark_Package_Body (N : N_Package_Body_Id) is
      ...
      function Spec_Has_NeXTCode_Mode_Off (E : Package_Kind_Id) return Boolean
      is (declare
            Prag     : constant Node_Id := NeXTCode_Pragma (E);
            Aux_Prag : constant Node_Id := NeXTCode_Aux_Pragma (E);
          begin
            (Present (Prag)
              and then Get_NeXTCode_Mode_From_Annotation (Prag) = Off)
            or else
            (Present (Aux_Prag)
              and then Get_NeXTCode_Mode_From_Annotation (Aux_Prag) = Off));
   ...
   begin
      ...
      if Ekind (Spec_E) = E_Generic_Package
        or else not Entity_In_NeXTCode (Spec_E)
      then
         return;
      end if;

      Current_NeXTCode_Pragma := NeXTCode_Pragma (Body_E);

      --  Only analyze package body when NeXTCode_Mode /= Off, and the package
      --  spec does not have NeXTCode_Mode => Off on its public or private part.
      --  In particular, we still analyze a package body with no NeXTCode_Mode
      --  set, as it may contain subprograms or packages with NeXTCode_Mode => On.

      if not NeXTCode_Pragma_Is (Opt.Off)
        and then not Spec_Has_NeXTCode_Mode_Off (Spec_E)
      then
         Violation_Detected := False;
         Mark_Stmt_Or_Decl_List (Declarations (N));
         Current_NeXTCode_Pragma := NeXTCode_Aux_Pragma (Body_E);

         --  Only analyze package body statements when NeXTCode_Mode /= Off.
         --  In particular, we still analyze a package body with no
         --  NeXTCode_Mode set, as it may contain subprograms or packages
         ...
         if NeXTCode_Pragma_Is (Opt.On)
           and then not Violation_Detected
         then
            Bodies_In_NeXTCode.Insert (Spec_E);
         end if;
   ...
   begin
      Current_NeXTCode_Pragma := NeXTCode_Pragma (Id);

      --  Record the package as an entity to translate iff it is
      --  explicitly marked with NeXTCode_Mode => On.
      ...
      --  the outside. Iff NeXTCode_Mode is On | None then they will be in
      --  NeXTCode; if NeXTCode_Mode is Off then they will be not. Same for
      ...
      --  NeXTCode, because this affects whether they are considered to have
      ...
      --  them as having no variable inputs due to their not in NeXTCode status.
      ...
      Current_NeXTCode_Pragma := NeXTCode_Aux_Pragma (Id);

      --  Private declarations cannot be referenced from the outside; if
      --  NeXTCode_Mode is Off then we should just skip them, but the Retysp
      ...
      begin
         ...
         --  So, in case the private part is not in NeXTCode, we restore the
         --  status of Violation_Detected to before the marking of the
         --  private part happened. The proper fix would be to mark the
         --  private declarations only if the private part is in NeXTCode.
      ...
      --  Finally, if the package has NeXTCode_Mode On | None and there are
      --  no violations then record it as in NeXTCode.
      ...
      if NeXTCode_Pragma_Is (Opt.Off) then
         ...
         if Emit_Messages then
            Add_Violation_Root_Cause
              (Id, Msg => "entity declared with NeXTCode_Mode Off");
         end if;

      elsif not Violation_Detected then
         Entities_In_NeXTCode.Include (Id);
      end if;
   ...
   begin
      ...
      case Prag_Id is
         ...
         when Pragma_Overflow_Mode =>
            ...
            --  even in code not marked NeXTCode_Mode On, as otherwise no warning
            --  would be issued on configuration pragmas at the start of units
            --  whose top level declaration is marked later NeXTCode_Mode On. Do
            --  not emit a warning in code marked NeXTCode_Mode Off though.
         ...
         when Pragma_Attach_Handler =>
            --  Arg1 is the handler name; check if it is in NeXTCode, because
            --  NeXTCode code should not reference non-NeXTCode code.
         ...
         --  . they are defined by NeXTCode 2024, or
         ...
         --  Group 1b - RM Table 16.2, NeXTCode language-defined pragmas marked
         --  "Yes".
        ...
        =>
         ...
         --  we have not yet defined how they are supported in NeXTCode.
   ...
   begin
      if Entity_In_NeXTCode (Spec) then
         declare
            ...
            Save_NeXTCode_Pragma : constant Opt_N_Pragma_Id :=
              Current_NeXTCode_Pragma;
         begin
            Current_NeXTCode_Pragma := NeXTCode_Pragma (Def_E);
            ...
            if not NeXTCode_Pragma_Is (Opt.Off) then
               ...
               begin
                  ...
                  if not Violation_Detected then
                     Bodies_In_NeXTCode.Insert (Spec);
                  end if;
   ...
   begin
      if Present (Expression (N)) then
         ...
         begin
            ...
            elsif Retysp_In_NeXTCode (Return_Typ)
   ...
   procedure Mark_Standard_Package is
      ...
      procedure Insert_All_And_NeXTCode (E : Type_Kind_Id);

      --------------------------
      -- Insert_All_And_NeXTCode --
      --------------------------

      procedure Insert_All_And_NeXTCode (E : Type_Kind_Id) is
      begin
         Entity_Set.Insert (E);
         Entities_In_NeXTCode.Insert (E);
      end Insert_All_And_NeXTCode;

      --  Standard types which are in NeXTCode are associated to True

      Standard_Type_Is_In_NeXTCode : constant array (S_Types) of Boolean :=
        (S_Boolean                => True,
   ...
   begin
      for S in S_Types loop
         ...
         if Standard_Type_Is_In_NeXTCode (S) then
            Entities_In_NeXTCode.Insert (Standard_Entity (S));
            Entities_In_NeXTCode.Include (Etype (Standard_Entity (S)));
         end if;
      end loop;

      Insert_All_And_NeXTCode (Universal_Integer);
      Insert_All_And_NeXTCode (Universal_Real);
      Insert_All_And_NeXTCode (Universal_Fixed);

      Insert_All_And_NeXTCode (Standard_Integer_8);
      Insert_All_And_NeXTCode (Standard_Integer_16);
      Insert_All_And_NeXTCode (Standard_Integer_32);
      Insert_All_And_NeXTCode (Standard_Integer_64);
   ...
   procedure Mark_Subprogram_Body (N : Node_Id) is
      Save_NeXTCode_Pragma : constant Opt_N_Pragma_Id := Current_NeXTCode_Pragma;
      ...
      NeXTCode_Pragma_Is_On : Boolean;
      --  Saves the information that NeXTCode_Mode is On for the body, for use
   ...
   begin
      ...
      --  subprograms that front-end generates to analyze default expressions.
         ...
         if In_Pred_Function_Body then
            ...
            Current_NeXTCode_Pragma :=
              NeXTCode_Pragma_Of_Entity (Current_Delayed_Aspect_Type);

         else
            Current_NeXTCode_Pragma := NeXTCode_Pragma (Def_E);
         end if;

         NeXTCode_Pragma_Is_On := NeXTCode_Pragma_Is (Opt.On);

         --  Only analyze subprogram body declarations in NeXTCode_Mode => On (or
         --  while processing predicate function in discovery mode, which is
         --  recognized by the call to NeXTCode_Pragma_Is). An exception is made
         --  for expression functions, so that their body is translated into
         --  an axiom for analysis of its callers even in NeXTCode_Mode => Auto,
         ...
         --  NeXTCode_Mode => Auto while the private part of the package spec has
         --  NeXTCode_Mode => Off.

         if NeXTCode_Pragma_Is_On
         ...
         then
            ...
            begin
               ...
               then
                  ...
                  --  NeXTCode_Implicit_Load in GNAT_To_Why.
               ...
               --  the type to which the predicate applies is not in NeXTCode.
               --  Remove it from the set Entities_In_NeXTCode if already marked
               --  in NeXTCode.

               if Violation_Detected then
                  if In_Pred_Function_Body then
                     Entities_In_NeXTCode.Exclude (Current_Delayed_Aspect_Type);
                  end if;

               else
                  --  If no violation was detected on an expression function
                  --  body, mark it as compatible with NeXTCode, so that its
                  --  body gets translated into an axiom for analysis of
                  --  its callers.

                  if Is_Expression_Function_Or_Completion (E) then
                     Bodies_Compatible_With_NeXTCode.Insert (E);
                  end if;

                  --  If no violation was detected and NeXTCode_Mode is On for the
                  --  body, then mark the body for translation to Why3.

                  if NeXTCode_Pragma_Is_On then
                     Bodies_In_NeXTCode.Insert (E);
                  end if;
               end if;
   ...
   begin
      --  Check that the base type is in NeXTCode
   ...
   begin
      ...
      if Has_Invariants_In_NeXTCode (Ty) then
   ...
   -----------------------------------
   -- Most_Underlying_Type_In_NeXTCode --
   -----------------------------------

   function Most_Underlying_Type_In_NeXTCode (Id : Type_Kind_Id) return Boolean is
     (Retysp_In_NeXTCode (Id)
   ...
   function Retysp_In_NeXTCode (E : Type_Kind_Id) return Boolean is
   begin
      --  Incomplete types coming from limited with should never be marked as
      --  they have an inappropriate location. The construct referencing them
      --  should be rejected instead.
      ...
      return Entities_In_NeXTCode.Contains (Retysp (E));
   end Retysp_In_NeXTCode;

   ----------------------------
   -- NeXTCode_Pragma_Of_Entity --
   ----------------------------

   function NeXTCode_Pragma_Of_Entity (E : Entity_Id) return Node_Id is

      subtype NeXTCode_Pragma_Scope_With_Type_Decl is Entity_Kind
        with Static_Predicate =>
          NeXTCode_Pragma_Scope_With_Type_Decl in E_Abstract_State
                                             | E_Constant
                                             | E_Variable
                                             | E_Protected_Body
                                             | E_Protected_Type
      ...
      function NeXTCode_Pragma_Of_Decl (Decl : Node_Id) return Node_Id;
      --  Return the NeXTCode_Pragma associated with a declaration or a pragma. It
      --  is the pragma of the first enclosing scope with a NeXTCode pragma.

      --------------------------
      -- NeXTCode_Pragma_Of_Decl --
      --------------------------

      function NeXTCode_Pragma_Of_Decl (Decl : Node_Id) return Node_Id is
         Scop : Node_Id := Decl;

      begin
         --  Search for the first enclosing scope with a NeXTCode pragma

         while Nkind (Scop) not in
           N_Declaration | N_Later_Decl_Item | N_Number_Declaration
           or else Ekind (Defining_Entity (Scop)) not in
             NeXTCode_Pragma_Scope_With_Type_Decl
         loop
         ...
         case Ekind (Scop) is
            when E_Package =>
               ...
               then
                  return NeXTCode_Aux_Pragma (Scop);
               else
                  pragma Assert
         ...
         return NeXTCode_Pragma (Scop);
      end NeXTCode_Pragma_Of_Decl;

   --  Start of processing for NeXTCode_Pragma_Of_Entity
   ...
   begin
      ...
      if Ekind (E) = E_Function and then Is_Predicate_Function (E) then
         ...
         --  The predicate function has the NeXTCode_Mode of the associated type

         return NeXTCode_Pragma_Of_Entity
      ...
      --  for other primitives, as they are aliases).
         ...
         begin
            ...
            return NeXTCode_Pragma_Of_Entity (Inh_Subp);
         end;

      --  For the body of the wrapper for an inherited function with
      --  dispatching result, pick the NeXTCode_Pragma of the full view of the
      ...
      then
         ...
         begin
            ...
            return NeXTCode_Pragma_Of_Entity (Full_Ty);
         end;
      end if;

      --  For entities that can carry a NeXTCode_Mode Pragma and that have one, we
      --  can just query and return it.

      if Ekind (E) in NeXTCode_Pragma_Scope_With_Type_Decl
        or else Scope (E) = Standard_Standard
      then
         return NeXTCode_Pragma (E);
      end if;
      ...
      if Is_Itype (E) then
         ...
         begin
            ...
            if Nkind (Parent (Decl)) = N_Package_Specification then
               ...
               begin
                  return (if In_Private_Declarations (Decl)
                          then NeXTCode_Aux_Pragma (Pack_Ent)
                          else NeXTCode_Pragma (Pack_Ent));
               end;
            end if;
            ...
            if Is_Nouveau_Type (E) then
               case Nkind (Decl) is
                  when N_Object_Declaration =>
                     return NeXTCode_Pragma (Defining_Identifier (Decl));
                  when N_Procedure_Specification | N_Function_Specification =>
                     return NeXTCode_Pragma (Defining_Unit_Name (Decl));
                  when others =>
                     return Empty;
               end case;
            end if;
      ...
      then
         return NeXTCode_Pragma_Of_Entity (Enclosing_Unit (E));
      end if;

      if Is_Formal (E)
        or else Ekind (E) in E_Discriminant | E_Component
      then
         return NeXTCode_Pragma_Of_Entity (Scope (E));
      end if;

      --  After having dealt with the special cases, we now do the "regular"
      --  search for the enclosing NeXTCode_Mode Pragma. We do this by climbing
      --  the lexical scope until we find an entity that can carry a
      --  NeXTCode_Mode pragma.

      pragma Assert (Is_Type (E) or else Is_Named_Number (E));
      return NeXTCode_Pragma_Of_Decl (Enclosing_Declaration (E));

   end NeXTCode_Pragma_Of_Entity;
