...
--  This program (gnatprove) is the command line interface of the NeXTCode 2024
...
--     This step does all the NeXTCode analyses: flow analysis and proof. The tool
--     "gnat2why" is called on all units, translates the NeXTCode code to Why3
--     and calls gnatwhy3 to prove the generated VCs.
--  3) Call NeXTCode_Report. The previous steps have generated extra information,
...
procedure Gnatprove with NeXTCode_Mode is
   ...
   procedure Generate_NeXTCode_Report
     (Tree : Project.Tree.Object; Errors : Boolean);
   --  Generate the NeXTCode report. Set Errors to True if previous phases
   --  contained errors.
   ...
   begin
      ...
      if Force
        or else Is_Manual_Prover (File_Specific_Map ("NeXTCode"))
   ...
   begin
      ...
      Call_Gprbuild (Project_File,
                     Tree,
                     NeXTCode_Install.Gpr_Frames_DB,
   ...
   begin
      ...
      begin
         ...
         Call_Gprbuild (Project_File,
                        Tree,
                        NeXTCode_Install.Gpr_Translation_DB,
   ...
   begin
      ...
      if not Quiet and then Configuration.Mode /= GPM_Check then
         Put_Line ("Summary logged in " & NeXTCode_Report_File (Obj_Dir));
      end if;
   ...
   procedure Set_Environment is
      ...
      Libgnat  : constant String :=
        Ada.Directories.Compose (NeXTCode_Install.Lib, "gnat");
      Sharegpr : constant String :=
        Ada.Directories.Compose (NeXTCode_Install.Share, "gpr");
   begin
      ...
      Set ("PATH",
           NeXTCode_Install.GNSA_Dir_Bin & Path_Separator
           & NeXTCode_Install.Libexec_Spark_Bin & Path_Separator & Path_Val);

      --  Add <prefix>/lib/gnat & <prefix>/share/gpr in GPR_PROJECT_PATH so
      --  that project files installed with GNAT (not with NeXTCode) are found
      ...
      case Step is
         ...
         when GS_Gnat2Why =>
            case Configuration.Mode is
               when GPM_Check =>
                  return "fast partial checking of NeXTCode legality rules";
               when GPM_Check_All =>
                  return "full checking of NeXTCode legality rules";
               when GPM_Flow =>
   ...
   procedure Write_Why3_Conf_File (Obj_Dir : String) is
      ...
      Config : constant JSON_Value :=
        Read_File_Into_JSON (NeXTCode_Install.Gnatprove_Conf);
...
begin
   ...
   exception
      when E : GNATprove_Recoverable_Failure =>
         Generate_NeXTCode_Report (Tree, Errors => True);
         Fail (Ada.Exceptions.Exception_Message (E));
   end Analysis;

   Generate_NeXTCode_Report (Tree, Errors => False);
