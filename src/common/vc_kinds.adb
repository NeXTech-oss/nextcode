...
package body VC_Kinds is
   ...
   function Description (Kind : Unsupported_Kind) return String is
     (case Kind is
         ...
         when Lim_Inherited_Prim_From_NeXTCode_Off =>
            "a subprogram which is inherited, not overriden, from an ancestor"
          & " declared in the private part of a package with NeXTCode_Mode Off",
         ...
         when Lim_Predicate_With_Different_NeXTCode_Mode =>
           "a private type whose full view is not in NeXTCode annotated with two"
   ...
   function From_JSON (V : JSON_Value) return NeXTCode_Mode_Status is
      S : constant String := Get (V);
   begin
      return
        (if S = "all" then All_In_NeXTCode
         elsif S = "spec" then Spec_Only_In_NeXTCode
         elsif S = "no" then Not_In_NeXTCode
               ...
               for Index in 1 .. Length (JS_Array_Elts) loop
                  ...
                  begin
                     declare
                        ...
                        --  And, on NeXTCode input, non simple value cannot
                        --  be produced.
   ...
   function To_JSON (Status : NeXTCode_Mode_Status) return JSON_Value is
      S : constant String :=
        (case Status is
            when All_In_NeXTCode => "all",
            when Spec_Only_In_NeXTCode => "spec",
            when Not_In_NeXTCode => "no");
   begin
      return Create (S);
   end To_JSON;
