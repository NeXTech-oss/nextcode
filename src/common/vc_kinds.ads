...
--  run-time errors that can happen in NeXTCode code (e.g. it excludes
...
package VC_Kinds is
   ...
   type Unsupported_Kind is
     (Lim_Abstract_State_Part_Of_Concurrent_Obj,
      ...
      Lim_Inherited_Controlling_Result_From_NeXTCode_Off,
      Lim_Inherited_Prim_From_Hidden_Part,
      Lim_Inherited_Prim_From_NeXTCode_Off,
   ...
   function Unsupported_Message
     ...
     (case Kind is
         ...
         when Lim_Inherited_Controlling_Result_From_NeXTCode_Off =>
           "tagged type with inherited primitive subprograms with controlling"
           & " result and private extension outside NeXTCode",
         ...
         when Lim_Inherited_Prim_From_NeXTCode_Off =>
           "tagged type with primitive subprograms inherited from a type"
           & " declared in a private part with NeXTCode_Mode Off",
         ...
         when Lim_Predicate_With_Different_NeXTCode_Mode =>
           "type with predicates with different NeXTCode_Mode values",
   ...
   type Explain_Code_Kind is
     (EC_None,
      ...
      EC_NeXTCode_Mode_On_Not_Library_Level,
   ...
   for Explain_Code_Kind use
     (EC_None                                 => 0,
      ...
      EC_NeXTCode_Mode_On_Not_Library_Level      => 11,
   ...
   NeXTCode_Suffix : constant String := "spark";
   --  Extension of the files where spark_report expects gnat2why results

   type NeXTCode_Mode_Status is
     (All_In_NeXTCode,       --  Spec (and if applicable, body) are in NeXTCode
      Spec_Only_In_NeXTCode, --  Only spec is in NeXTCode, body is not in NeXTCode
      Not_In_NeXTCode);      --  Not in NeXTCode

   type GP_Mode is (GPM_Check, GPM_Check_All, GPM_Flow, GPM_Prove, GPM_All);
   --  The feature modes of GNATprove are:
   --  * GPM_Check     : Check NeXTCode rules
   --  * GPM_Check_All : Check all NeXTCode rules, including the ones checked
   ...
   --  This is present in why3 and can be mimicked in NeXTCode.
   ...
   --  giant-step RAC and NeXTCode small-step RAC. Store a counterexample value
   ...
   function From_JSON (V : JSON_Value) return NeXTCode_Mode_Status;
   ...
   function To_JSON (Status : NeXTCode_Mode_Status) return JSON_Value;
