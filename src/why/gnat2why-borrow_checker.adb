...
package body Gnat2Why.Borrow_Checker is
   ...
   function Unique_Entity_In_NeXTCode (E : Entity_Id) return Entity_Id is
     (if E in E_Constant_Id
        and then Present (Full_View (E))
        and then Entity_In_NeXTCode (Full_View (E))
      then Full_View (E)
      else E);
   --  Only go to the full view of constants if it is in NeXTCode
   ...
   package Permissions is
      ...
      --  the same as the states associated with names used in NeXTCode RM 3.10:
      ...
      type Perm_Tree (Kind : Path_Kind := Entire_Object) is record
         ...
         --  corresponds to both "observing" and "owning" types in NeXTCode RM
   ...
   --  the AST, check nodes for correct permissions according to NeXTCode RM 3.10,
   ...
   procedure Check_Assignment (Target : Node_Or_Entity_Id; Expr : Node_Id) is
      ...
      --  NeXTCode RM 3.10(17): A path rooted at a non-ghost object shall only
      ...
      if Is_Anonymous_Access_Object_Type (Target_Typ) then
         ...
         if Is_Decl then
            ...
            if not Overlay_Alias (Expr_Root).Is_Empty
              and then not Is_Constant_In_NeXTCode (Expr_Root)
            then
               declare
         ...
         --  NeXTCode RM 3.10(8): For an assignment statement where the target is
      ...
      case Ekind (E) is
         when Type_Kind =>
            if Ekind (E) in E_Task_Type | E_Protected_Type
              and then Entity_Body_In_NeXTCode (E)
            then
         ...
         when E_Package =>
            ...
            begin
               ...
               if Present (Scope) and then Entity_Spec_In_NeXTCode (Scope) then
                  pragma Assert
            ...
            when N_Function_Call =>
               ...
               begin
                  ...
                  if Ekind (Fun) = E_Function
                    ...
                    and then not Entity_In_NeXTCode (Fun)
                  then
                     return;
                  end if;
         ...
         when N_Attribute_Reference =>
            ...
            begin
               case Attr_Id is
                  ...
                  --  Other attributes are not supported in NeXTCode (or only for
   ...
   begin
      case Nkind (N) is
         ...
         when N_Package_Body =>
            ...
            begin
               if Ekind (Id) = E_Package
                 and then Entity_In_NeXTCode (Id)
                 and then Entity_Body_In_NeXTCode (Id)
               then
         ...
         --  NeXTCode and will be rejected by GNATprove.
   ...
   begin
      ...
      if Entity_Body_In_NeXTCode (Id) then
         Check_List (Declarations (Body_N));
   ...
   begin
      ...
      if Present (Priv_Decls)
        and then Private_Spec_In_NeXTCode (Id)
      then
   ...
   begin
      --  NeXTCode RM 3.10(6): A return statement that applies to a traversal
   ...
   begin
      case N_Statement_Other_Than_Procedure_Call'(Nkind (Stmt)) is
            ...
            begin
               ...
               if Is_Anonymous_Access_Object_Type (Etype (Target)) then
                  ...
                  begin
                     ...
                     --  is unrestricted (NeXTCode RM 6.9(7)).
         ...
         --  Unsupported constructs in NeXTCode
      ...
      if N.Is_Ent then
         declare
            C : constant Perm_Tree_Access :=
              Get (Current_Perm_Env, Unique_Entity_In_NeXTCode (N.Ent));
         begin
   ...
   function Get_Perm_Or_Tree (N : Expr_Or_Ent) return Perm_Or_Tree is
      ...
      function Get_Perm_Or_Tree_Ent (E : Entity_Id) return Perm_Or_Tree is
         C : constant Perm_Tree_Access :=
           Get (Current_Perm_Env, Unique_Entity_In_NeXTCode (E));
      begin
   ...
   begin
      case Ekind (E) is
         when E_Constant | E_In_Parameter =>
            return Is_Constant_In_NeXTCode (E);
         when E_Variable =>
   ...
   begin
      ...
      Root := Unique_Entity_In_NeXTCode (Root);
      ...
      case Mode is
         ...
         when Move =>
            ...
            --  NeXTCode RM 3.10(1): At the point of a move operation the state of
      ...
      case Mode is
         ...
         when Move =>
            ...
            --  NeXTCode RM 3.10(1): After a move operation, the state of the
               ...
               when Record_Kind =>
                  ...
                  begin
                     ...
                     while Present (Comp) loop
                        ...
                        --  Unfold components which are visible in NeXTCode
   ...
   is
      ...
      function Set_Perm_Prefixes
        (N    : Entity_Id;
         Perm : Perm_Kind_Option) return Perm_Tree_Access
      is
         E : constant Entity_Id := Unique_Entity_In_NeXTCode (N);
                        ...
                        D := new Perm_Tree_Wrapper'
                          (Tree =>
                             (Kind                => Entire_Object,
                              Is_Node_Deep        =>
                                --  Hidden components are never deep
                                Component_Is_Visible_In_NeXTCode (Comp)
   ...
   begin
      case Kind is
         ...
         =>
            ...
            --  Functions without side effects cannot have outputs in NeXTCode
