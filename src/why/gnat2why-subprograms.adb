...
package body Gnat2Why.Subprograms is
                     ...
                     case F.Kind is
                        when Direct_Mapping =>
                           ...
                           begin
                              ...
                              --  Only consider objects that are in NeXTCode.
   ...
   begin
      ...
      if Private_Spec_In_NeXTCode (Ty) then
   ...
   begin
      ...
      for I in Func_Why_Binders'Range loop
         ...
         --  protected type is in NeXTCode. Otherwise, self will have the type
         ...
         if Func_Why_Binders (I).Kind = Concurrent_Self then
            ...
            begin
               if Entity_In_NeXTCode (Ada_Type) then
                  declare
           ...
           and then Entity_In_NeXTCode (Formal)
         then
   ...
   begin
      ...
      if Present (Init_Cond)
        and then (No (Body_N) or else Entity_Body_In_NeXTCode (E))
      then
         declare
            Expr : constant Node_Id :=
      ...
      if No (Body_N) or else Entity_Body_In_NeXTCode (E) then
         declare
      ...
      if Present (Priv_Decls)
        and then Private_Spec_In_NeXTCode (E)
      then
         Why_Spec :=
         ...
         if Present (Enclosing_Subp) then
            ...
            begin
               ...
               if Is_Entry (Enclosing_Subp)
                 and then Entity_Body_In_NeXTCode (Enclosing_Subp)
               then
                  declare
                     Params  : constant Transformation_Params :=
   ...
   is
      ...
      with Pre => Entity_Body_In_NeXTCode (E);
         ...
         Need_Check : constant Boolean :=
           (Present (Retrieve_Inline_Annotation (E)) and then
              (not Is_Expression_Function_Or_Completion (E)
               or else not Entity_Body_Compatible_With_NeXTCode (E)))
   ...
   begin
      ...
      --  useful both for verifying the body of E when it is in NeXTCode, and for
      ...
      --  Regular subprogram with body in NeXTCode

      elsif Entity_Body_In_NeXTCode (E) then
         declare
            Body_N     : constant Node_Id := Get_Body (E);
      ...
      declare
         ...
         Warn_Post : constant W_Prog_Id :=
           ...
           (if not Gnat2Why_Args.Proof_Warnings
              ...
              --  or if the body is in NeXTCode
              or else Entity_Body_In_NeXTCode (E)
              --  or if warnings are suppressed.
              or else Opt.Warning_Mode = Opt.Suppress
            then +Void
   ...
   is
      Ty            : constant Entity_Id :=
        Base_Retysp (NeXTCode_Util.Subprograms.Find_Dispatching_Type (E));
   ...
   begin
      ...
      for Descendant of Descendants loop
         ...
         if Entity_In_NeXTCode (Descendant_E) then
               ...
               declare
                  ...
                  Eq_Expr   : constant W_Pred_Id :=
                    (New_Call (Name => Why_Eq,
                               ...
                               Typ  => EW_Bool_Type));
               begin
                  if Entity_Body_In_NeXTCode (E) and then Has_Refinement (E)
                  then
                     Refined_Post :=
   ...
   begin
      ...
      --  If the entity's body is not in NeXTCode, if it is inlined for proof, or
      --  if it is a volatile function or a function with side effects, do not
      --  generate axiom.

      if not Entity_Body_Compatible_With_NeXTCode (E)
