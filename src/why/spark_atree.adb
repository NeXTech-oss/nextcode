...
package body NeXTCode_Atree is
   ...
   begin
      ...
      or else
        (Sinfo.Nodes.Nkind (N) = N_Type_Conversion
         ...
         and then Einfo.Entities.Ekind (NeXTCode_Util.Get_Formal_From_Actual (N))
   ...
   function Get_Range_Check_Info
     (N : Node_Id) return NeXTCode_Util.Scalar_Check_Kind
   is
      Typ : constant Entity_Id := NeXTCode_Util.Types.Retysp (Etype (N));
   ...
   begin
      ...
      if Nkind (Par) = N_Indexed_Component then
         return NeXTCode_Util.RCK_Index;
      ...
      then
         return NeXTCode_Util.RCK_Length;

      --  Otherwise, this is a range check

      else
         return NeXTCode_Util.RCK_Range;
      end if;
