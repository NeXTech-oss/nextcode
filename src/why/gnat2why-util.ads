...
package Gnat2Why.Util is
   ...
   --     a record type. E should be a "Representative Type in NeXTCode".
   --  @return the number of regular fields in the record representing E into
   --     Why3, which contains:
   --     - One field per component of E visible in NeXTCode
   --       (use Component_Is_Visible_In_NeXTCode)
   ...
   --     DIC, if its full view is in NeXTCode, and if its DIC mentions the
   ...
   function Has_Init_Wrapper (Typ : Type_Kind_Id) return Boolean is
     (NeXTCode_Definition.Has_Relaxed_Init (Typ)
   ...
   --    a private type is in NeXTCode, that one is returned instead of the
   --    private type.
   ...
   function Get_Called_Entity_For_Proof (N : Node_Id) return Entity_Id with
     Pre  => Nkind (N) in N_Subprogram_Call
                        ...
                        | N_Op;
   --  Call NeXTCode_Atree.Get_Called_Entity and use the Specialized_Call_Params
