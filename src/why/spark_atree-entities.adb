...
with Sem_Util;
with Sem_Prag;
with NeXTCode_Util.Types;
with Interfaces;

package body NeXTCode_Atree.Entities is
   ...
   is
     (Einfo.Entities.Default_Aspect_Component_Value
        (NeXTCode_Util.Types.Base_Retysp (Typ)));
   ...
   function Designates_Incomplete_Type (E : Access_Kind_Id) return Boolean is
      Des_Ty : constant Type_Kind_Id :=
        Einfo.Entities.Directly_Designated_Type
          (NeXTCode_Util.Types.Base_Retysp (E));
   ...
   begin
      return NeXTCode_Util.Types.Acts_As_Incomplete_Type (Des_Ty);
   ...
   function Has_Default_Aspect (Typ : Type_Kind_Id) return Boolean is
     (Einfo.Entities.Has_Default_Aspect (NeXTCode_Util.Types.Base_Retysp (Typ)));
   ...
   begin
      ...
      --  not hidden in NeXTCode. It can happen for subtypes of private types
      --  whose full view has discriminants but is not in NeXTCode and whose
      ...
      begin
         return Present (Discr)
           and then NeXTCode_Util.Is_Not_Hidden_Discriminant (Discr);
      end;
   ...
   is
     (Einfo.Entities.Is_Dispatching_Operation (Subp)
       and then Present (NeXTCode_Util.Subprograms.Find_Dispatching_Type (Subp)));
   ...
   function Is_Unchecked_Union (E : Type_Kind_Id) return Boolean is
     (Einfo.Entities.Is_Unchecked_Union (NeXTCode_Util.Types.Base_Retysp (E)));
   ...
   function Modulus (Typ : Modular_Integer_Kind_Id) return Uint is
     (Einfo.Entities.Modulus (NeXTCode_Util.Types.Base_Retysp (Typ)));
   ...
   is (Einfo.Entities.Non_Binary_Modulus (NeXTCode_Util.Types.Base_Retysp (Typ)));
