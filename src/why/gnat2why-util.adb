...
package body Gnat2Why.Util is
   ...
   package body Ada_Ent_To_Why is
      ...
      --  entity. For constants whose full view is in NeXTCode, use the full
      --  view.
   ...
   function Get_Container_In_Iterator_Specification
     (N : Node_Id)
      return Node_Id
   is
      Iter : constant Node_Id := NeXTCode_Atree.Name (N);
   begin
      return (Iter);
   ...
   begin
      ...
      --  NeXTCode/Ada or due to its represententation in Why.
      ...
      case Ekind (E) is
         ...
         when E_Constant =>
            ...
            elsif NeXTCode_Definition.Is_Loop_Entity (E)
              and then not NeXTCode_Definition.Is_Actions_Entity (E)
            then
            ...
            --  designated object.
               ...
               return not Is_Constant_In_NeXTCode (E);
            end if;
   ...
   is (Ekind (Entity (N)) = E_Function
       and then Full_View_Not_In_NeXTCode (Etype (Right_Opnd (N)))
       and then (if Nkind (N) in N_Binary_Op then
                    Full_View_Not_In_NeXTCode (Etype (Left_Opnd (N)))));
   ...
   begin
      return May_Need_DIC_Checking (E)
        ...
        --  in NeXTCode. If the Restysp is not a private type, then the full view
        --  is necessarily in NeXTCode.
   ...
   function Type_Needs_Dynamic_Invariant
     (T              : Type_Kind_Id;
      ...
      begin
         loop
            if Has_Invariants_In_NeXTCode (Current) then
               return True;
            end if;
   ...
   begin
      return Gnat2Why_Args.Proof_Generate_Guards
        ...
        and then No (NeXTCode_Definition.Annotate.Retrieve_Inline_Annotation (E))

        --  Functions from predefined units should be safe

        and then not Is_Ignored_Internal (E)

        --  Same for the NeXTCode library

        and then not In_NeXTCode_Library_Unit (E)
