...
package body Why.Gen.Binders is
   ...
   is
      Typ : constant W_Type_Id :=
        (if Entity_In_NeXTCode (Ty) then Type_Of_Node (Ty)
   ...
   is
      Use_Ty : constant Entity_Id :=
        (if not In_Fun_Decl
         ...
         and then Entity_In_NeXTCode (Actual_Subtype (E))
   ...
   begin
      ...
      --  If E is in NeXTCode, decide whether it should be split into multiple
      --  objects in Why3 or not.

      elsif Entity_In_NeXTCode (Ty)
