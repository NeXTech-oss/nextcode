...
package body Gnat2Why.Tables is
   ...
   begin
      ...
      --  If the type has private fields that are not visible in NeXTCode, add the
   ...
   begin
      ...
      then
         ...
         begin
            ...
            if Full_View_Not_In_NeXTCode (E) or else Is_Task_Type (E) then
               Needs_Private_Part := True;
            else
               Field := First_Component (E);
               while Present (Field) loop
                  pragma Assert (Component_Is_Visible_In_NeXTCode (Field));
                  Comp_Info (Position).Variant_Info.Insert
                    (Field,
                     Component_Info'(others => Types.Empty));
            ...
            then
               ...
               loop
                  if Entity_In_NeXTCode (Part) then
