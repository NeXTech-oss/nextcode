...
package body Gnat2Why.Expr.Loops.Inv is
                  ...
                  while Has_Element (Prev) loop
                     ...
                     begin
                        exit when Nkind (D) not in
                          N_Declaration | N_Ignored_In_NeXTCode;
   ...
   begin
      ...
      then
         Write_Expr
           (New_Write     => NeXTCode_Atree.Name (Call),
            Loop_Writes   => Loop_Writes,
            After_Inv     => After_Inv,
            Relevant_Path => Relevant_Path);
   ...
   begin
      case Nkind (N) is
         when N_Assignment_Statement =>
            declare
               Lvalue   : constant Entity_Id := NeXTCode_Atree.Name (N);
               Rvalue   : constant Node_Id   := NeXTCode_Atree.Expression (N);
