...
with NeXTCode_Register;                  use NeXTCode_Register;
with NeXTCode_Rewrite;                   use NeXTCode_Rewrite;
...
package body Gnat2Why.Driver is
   ...
   with Pre => (if Ekind (E) = E_Package
                then Entity_Spec_In_NeXTCode (E)
                else Entity_In_NeXTCode (E));
   ...
   --  units (and thus only known by name), or not in NeXTCode (thus ignored by
   ...
   --  Perform NeXTCode access legality checking
   ...
   --  For all calls from a NeXTCode subprogram to another, register assumptions
   ...
   --  Objects not in NeXTCode but still translated to Why; we get them from the
   ...
   begin
      ...
      case Ekind (E) is
         ...
         =>
            if Is_Translated_Subprogram (E) then
               if Is_Expression_Function_Or_Completion (E)
                 and then Entity_Body_Compatible_With_NeXTCode (E)
               then
         ...
         when Type_Kind =>
            pragma Assert (Entity_In_NeXTCode (E));
   ...
   procedure Create_JSON_File (Progress    : Analysis_Progress;
      ...
      File_Name : constant String :=
        Ada.Directories.Compose
          (Name      => Unit_Name,
           Extension => VC_Kinds.NeXTCode_Suffix);
      Full : constant JSON_Value := Create_Object;
   begin
      Set_Field (Full, "spark", Get_NeXTCode_JSON);
      ...
      case Ekind (E) is
         ...
         when Type_Kind =>
            ...
            if Ekind (E) in E_Protected_Type | E_Task_Type
              and then Entity_Spec_In_NeXTCode (E)
            ...
            elsif Entity_Spec_In_NeXTCode (Enclosing_Unit (E))
              and then E = Retysp (E)
            then
   ...
   begin
      ...
      if Is_Generic_Unit (E) then
         ...
         --  NeXTCode expansion in the frontend should be applied to generic
         --  units as well.
      ...
      if Gnat2Why_Args.Global_Gen_Mode then
         ...
         --  Mark the current compilation unit as "in NeXTCode / not in NeXTCode"
         ...
         --  NeXTCode status of expressions in their initial values, so must be
         --  done after marking.
         ...
         --  main entity in the compilation unit is not marked in NeXTCode. It may
         --  still be that an enclosed package/subprogram is marked in NeXTCode.
         ...
         --  is neither marked in NeXTCode nor out of NeXTCode.
         ...
         --  neither marked in NeXTCode nor out of NeXTCode.

         declare
            Root : constant Entity_Id := Defining_Entity (Unit (GNAT_Root));
         begin
            if Gnat2Why_Args.Limit_Units
              and then No (NeXTCode_Pragma (Root))
            then
               Error_Msg_N
                 ("NeXTCode_Mode not applied to this compilation unit",
                  GNAT_Root,
                  Info_Kind,
                  Continuations =>
                    ["only enclosed declarations with NeXTCode_Mode"
                     & " will be analyzed"]);
            end if;
         end;
         ...
         --  Check NeXTCode rules for pointer aliasing. This is only activated on
         --  NeXTCode code.
      ...
      for E of Entities_To_Translate loop
         case Ekind (E) is
            when Entry_Kind
               ...
               | E_Task_Type
            =>
               --  Packages have always a "body" in the NeXTCode meaning, that is,
               ...
               if Analysis_Requested (E, With_Inlined => True)
                 ...
                 and then (if Ekind (E) /= E_Package
                           then Entity_Body_In_NeXTCode (E))
               then
      ...
      begin
         ...
         then
            ...
            is
               ...
               --  address for example) or if all calls are in non-NeXTCode code,
   ...
   begin
      ...
      Translated_Object_Names.Insert
        (To_Entity_Name (NeXTCode_Xrefs.Name_Of_Heap_Variable));
   ...
   begin
      ...
      case Ekind (E) is
         when Type_Kind =>
            ...
            --  the one with the most information that's still in NeXTCode.

            if Retysp (E) = E then
               --  Partial views of private types should not be
               --  translated if the underlying type is not in NeXTCode,
         ...
         when Object_Kind =>
            ...
            --  Constants and variables are translated differently
               ...
               if Ekind (E) = E_Constant then
                  if Is_Partial_View (E) then
                     ...
                     if not Entity_In_NeXTCode (Full_View (E)) then
            ...
            --  partial view bacause the full view may not be in NeXTCode.
