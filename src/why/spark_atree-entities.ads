...
with Sinfo.Nodes;
with NeXTCode_Util;
with NeXTCode_Util.Subprograms;

package NeXTCode_Atree.Entities is
   ...
   --  tagged dispatching type (NeXTCode_Util.Subprograms.Find_Dispatching_Type
   --  returns True).
   ...
   --  on the result of NeXTCode_Util.Nth_Index_Type which may return a string
   --  literal subtype.
   ...
   with
     ...
     Post => NeXTCode_Util.Is_Not_Hidden_Discriminant (First_Discriminant'Result);
   ...
   procedure Next_Discriminant (Discr : in out Opt_E_Discriminant_Id)
     with Post => (if Present (Discr) then
                     NeXTCode_Util.Is_Not_Hidden_Discriminant (Discr));
   ...
   function Enclosing_Type (Obj : Entity_Id) return Type_Kind_Id with
     Pre  => Ekind (Obj) in
       E_Discriminant | E_Component | E_Constant | E_In_Parameter
        or else NeXTCode_Util.Subprograms.Is_Protected_Operation (Obj),
   ...
   function Full_View (Obj : E_Constant_Id) return E_Constant_Id
     with Pre => NeXTCode_Util.Is_Partial_View (Obj);
