...
package body Gnat2Why.Types is
   ...
   procedure Generate_Type_Completion (E : Type_Kind_Id) is
      ...
      procedure Create_Type_Invariant
        (Th : Theory_UC;
         E  : Type_Kind_Id)
        with Pre => Has_Invariants_In_NeXTCode (E);
      ...
      begin
         ...
         if not Use_Predefined_Equality_For_Type (E)
           and then Entity_In_NeXTCode (Eq)
         then
            User_Th :=
              Open_Theory
   ...
   procedure Generate_VCs_For_Type (E : Type_Kind_Id) is
      ...
      S_Ty            : constant Entity_Id :=
        (if Present (First_Subtype (E))
         and then Entity_In_NeXTCode (First_Subtype (E))
      ...
      begin
         case Ekind (E) is
            ...
            when Incomplete_Or_Private_Kind =>
               pragma Assert (Full_View_Not_In_NeXTCode (E));
               Declare_Ada_Record (Th, E);
            ...
            Th :=
              Open_Theory
                (WF_Context, E_Module (E, Type_Representative),
                 ...
                 & """" & Get_Name_String (Chars (E)) & """"
                 & (if NeXTCode_Atree.Sloc (E) > 0 then
                      " defined at " & Build_Location_String (Sloc (E))
                   else "")
                 & ", created in " & GNAT.Source_Info.Enclosing_Entity);
