...
struct nextcode::ide::api::SDKNodeInitInfo {
  SDKContext &Ctx;
  DeclKind DKind;
  AccessorKind AccKind;
  SourceLoc Loc;
#define KEY_STRING(X, Y) StringRef X;
#include "nextcode/IDE/DigesterEnums.def"
...
#include "nextcode/IDE/DigesterEnums.def"
  ...
  nextcode::ReferenceOwnership ReferenceOwnership = ReferenceOwnership::Strong;
...
bool nextcode::ide::api::hasValidParentPtr(SDKNodeKind kind) {
  switch(kind) {
...
SDKNodeDecl::SDKNodeDecl(SDKNodeInitInfo Info, SDKNodeKind Kind)
      : SDKNode(Info, Kind), DKind(Info.DKind), Usr(Info.Usr),
        ...
        introVersions({Info.IntromacOS, Info.IntroiOS, Info.IntrotvOS,
                       Info.IntrowatchOS, Info.Intronextcode}),
...
bool SDKNodeDeclType::isConformingTo(nextcode::ide::api::KnownProtocolKind Kind) const {
  switch (Kind) {
...
#include "nextcode/IDE/DigesterEnums.def"
      case KeyKind::KK_ownership:
        Info.ReferenceOwnership =
            nextcode::ReferenceOwnership(getAsInt(Pair.getValue()));
        assert(Info.ReferenceOwnership != nextcode::ReferenceOwnership::Strong &&
               "Strong is implied.");
        break;
...
void nextcode::ide::api::nodeSetDifference(ArrayRef<SDKNode*> Left,
...
SDKNodeInitInfo::SDKNodeInitInfo(SDKContext &Ctx, Decl *D):
      ...
      Intronextcode(Ctx.getLanguageIntroVersion(D)),
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructTypeNode(Type T, TypeInitInfo Info) {
  if (Ctx.checkingABI()) {
    ...
    if (T->hasOpaqueArchetype()) {
      ...
      // function where the function body is present in the nextcode module file,
...
std::vector<SDKNode*> nextcode::ide::api::
NeXTCodeDeclCollector::createParameterNodes(ParameterList *PL) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructFunctionNode(FuncDecl* FD,
                                          SDKNodeKind Kind) {
...
SDKNode* nextcode::ide::api::
NeXTCodeDeclCollector::constructInitNode(ConstructorDecl *CD) {
...
bool nextcode::ide::api::
SDKContext::shouldIgnore(Decl *D, const Decl* Parent) const {
  // Exclude all clang nodes if we're comparing NeXTCode decls specifically.
  // FIXME: isFromClang also excludes NeXTCode decls with @objc. We should allow those.
  if (Opts.NeXTCodeOnly && isFromClang(D)) {
    return true;
  }
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructTypeDeclNode(NominalTypeDecl *NTD) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructExternalExtensionNode(NominalTypeDecl *NTD,
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructVarNode(ValueDecl *VD) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructTypeAliasNode(TypeAliasDecl *TAD) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructMacroNode(MacroDecl *MD) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructAssociatedTypeNode(AssociatedTypeDecl *ATD) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructSubscriptDeclNode(SubscriptDecl *SD) {
...
NeXTCodeDeclCollector::addMembersToRoot(SDKNode *Root, IterableDeclContext *Context) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructTypeWitnessNode(AssociatedTypeDecl *Assoc,
                                             Type Ty) {
...
SDKNode *nextcode::ide::api::
NeXTCodeDeclCollector::constructConformanceNode(ProtocolConformance *Conform) {
  if (Ctx.checkingABI())
...
void nextcode::ide::api::
NeXTCodeDeclCollector::addConformancesToTypeDecl(SDKNodeDeclType *Root,
                                              NominalTypeDecl *NTD) {
...
void NeXTCodeDeclCollector::printTopLevelNames() {
  for (auto &Node : RootNode->getChildren()) {
    llvm::outs() << Node->getKind() << ": " << Node->getName() << '\n';
  }
}
NeXTCodeDeclCollector::NeXTCodeDeclCollector(SDKContext &Ctx,
  ...
  for (auto M: Modules) {
    ...
    nextcode::getTopLevelDeclsForDisplay(M, Decls);
    for (auto D : Decls) {
...
SDKNode *NeXTCodeDeclCollector::constructOperatorDeclNode(OperatorDecl *OD) {
  return SDKNodeInitInfo(Ctx, OD).createSDKNode(SDKNodeKind::DeclOperator);
}

void NeXTCodeDeclCollector::processDecl(Decl *D) {
...
void NeXTCodeDeclCollector::processValueDecl(ValueDecl *VD) {
  if (auto FD = dyn_cast<FuncDecl>(VD)) {
...
void NeXTCodeDeclCollector::foundDecl(ValueDecl *VD, DeclVisibilityKind Reason,
                                   DynamicLookupInfo) {
  if (VD->getClangMacro()) {
...
void SDKNodeDecl::jsonize(json::Output &out) {
  ...
  output(out, KeyKind::KK_intro_nextcode, introVersions.code);
...
namespace nextcode {
namespace json {
// In the namespace of nextcode::json, we define several functions so that the
...
template <> struct nextcode::json::ObjectTraits<ConstExprInfo> {
...
// Serialize the content of all roots to a given file using JSON format.
void NeXTCodeDeclCollector::serialize(llvm::raw_ostream &os) {
  NeXTCodeDeclCollector::serialize(os, RootNode, PayLoad());
}

SDKNodeRoot *
nextcode::ide::api::getEmptySDKNodeRoot(SDKContext &SDKCtx) {
  NeXTCodeDeclCollector Collector(SDKCtx);
  return Collector.getSDKRoot();
}

SDKNodeRoot*
nextcode::ide::api::getSDKNodeRoot(SDKContext &SDKCtx,
  ...
  NEXTCODE_DEFER { CI.getDiags().removeConsumer(PrintDiags); };
  ...
  if (!Stdlib) {
    llvm::errs() << "Failed to load NeXTCode stdlib\n";
    return nullptr;
  }
  ...
  NeXTCodeDeclCollector Collector(SDKCtx, Modules);
  return Collector.getSDKRoot();
}

void nextcode::ide::api::dumpSDKRoot(SDKNodeRoot *Root, PayLoad load,
                                  llvm::raw_ostream &os) {
  assert(Root);
  auto Opts = Root->getSDKContext().getOpts();
  if (Opts.Verbose)
    llvm::errs() << "Dumping SDK...\n";
  NeXTCodeDeclCollector::serialize(os, Root, load);
}

void nextcode::ide::api::dumpSDKRoot(SDKNodeRoot *Root, llvm::raw_ostream &os) {
  dumpSDKRoot(Root, PayLoad(), os);
}

int nextcode::ide::api::dumpSDKContent(const CompilerInvocation &InitInvoke,
                                    const llvm::StringSet<> &ModuleNames,
...
int nextcode::ide::api::deserializeSDKDump(StringRef dumpPath, StringRef OutputPath,
    CheckerOptions Opts) {
  std::error_code EC;
  ...
  NeXTCodeDeclCollector Collector(Ctx);
  Collector.deSerialize(dumpPath);
  Collector.serialize(FS);
  return 0;
}

void nextcode::ide::api::dumpModuleContent(ModuleDecl *MD, llvm::raw_ostream &os,
                                        bool ABI, bool Empty) {
  CheckerOptions opts;
  opts.ABI = ABI;
  opts.NeXTCodeOnly = true;
  ...
  NeXTCodeDeclCollector collector(ctx, modules);
...
int nextcode::ide::api::findDeclUsr(StringRef dumpPath, CheckerOptions Opts) {
  std::error_code EC;
  if (!fs::exists(dumpPath)) {
    llvm::errs() << dumpPath << " does not exist\n";
    return 1;
...
void nextcode::ide::api::SDKNodeDeclType::diagnose(SDKNode *Right) {
...
void nextcode::ide::api::SDKNodeDeclAbstractFunc::diagnose(SDKNode *Right) {
  SDKNodeDecl::diagnose(Right);
...
void nextcode::ide::api::SDKNodeDeclFunction::diagnose(SDKNode *Right) {
...
void nextcode::ide::api::detectRename(SDKNode *L, SDKNode *R) {
...
void nextcode::ide::api::SDKNodeDecl::diagnose(SDKNode *Right) {
  ...
  if (!isOwnershipEquivalent(getReferenceOwnership(),
    ...
    auto getOwnershipDescription = [&](nextcode::ReferenceOwnership O) {
...
void nextcode::ide::api::SDKNodeDeclOperator::diagnose(SDKNode *Right) {
  SDKNodeDecl::diagnose(Right);
...
void nextcode::ide::api::SDKNodeDeclVar::diagnose(SDKNode *Right) {
  SDKNodeDecl::diagnose(Right);
...
void nextcode::ide::api::SDKNodeType::diagnose(SDKNode *Right) {
  SDKNode::diagnose(Right);
...
void nextcode::ide::api::SDKNodeTypeFunc::diagnose(SDKNode *Right) {
