//===--- TypeLowering.cpp - NeXTCode Type Lowering for Reflection ------------===//
//
/*
...
#include "nextcode/AST/ReferenceStorage.def"
    ...
    nextcode_unreachable("Bad TypeInfo kind");
  }
};
...
/// context pointer. The context is modeled exactly like a native NeXTCode
/// class reference.
...
public:
  ...
  const TypeInfo *visitAnyNominalTypeRef(const TypeRef *TR) {
    ...
    nextcode_unreachable("Unhandled FieldDescriptorKind in switch.");
  }
  ...
  const TypeInfo *visitFunctionTypeRef(const FunctionTypeRef *F) {
    ...
    nextcode_unreachable("Unhandled FunctionMetadataConvention in switch.");
  }
  ...
  const TypeInfo *visitMetatypeTypeRef(const MetatypeTypeRef *M) {
    ...
    nextcode_unreachable("Unhandled MetatypeRepresentation in switch.");
  }
