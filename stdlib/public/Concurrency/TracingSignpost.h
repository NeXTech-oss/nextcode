...
#ifndef NEXTCODE_CONCURRENCY_TRACINGSIGNPOST_H
#define NEXTCODE_CONCURRENCY_TRACINGSIGNPOST_H
...
#include "nextcode/Runtime/TracingCommon.h"
#include <inttypes.h>
#include <os/log.h>
...
// NeXTCode runtime build. Changes here must be considered carefully to avoid
...
#define NEXTCODE_LOG_ACTOR_LIFETIME_NAME "actor_lifetime"
#define NEXTCODE_LOG_ACTOR_DEALLOCATE_NAME "actor_deallocate"
#define NEXTCODE_LOG_ACTOR_ENQUEUE_NAME "actor_enqueue"
#define NEXTCODE_LOG_ACTOR_DEQUEUE_NAME "actor_dequeue"
#define NEXTCODE_LOG_ACTOR_STATE_CHANGED_NAME "actor_state_changed"
#define NEXTCODE_LOG_ACTOR_JOB_QUEUE_NAME "actor_job_queue"
#define NEXTCODE_LOG_TASK_LIFETIME_NAME "task_lifetime"
#define NEXTCODE_LOG_TASK_FLAGS_CHANGED_NAME "task_flags_changed"
#define NEXTCODE_LOG_TASK_STATUS_CHANGED_NAME "task_status_changed"
#define NEXTCODE_LOG_TASK_WAIT_NAME "task_wait"
#define NEXTCODE_LOG_TASK_CONTINUATION "task_continuation"
#define NEXTCODE_LOG_TASK_CONTINUATION_AWAIT "task_continuation_await"
#define NEXTCODE_LOG_JOB_ENQUEUE_GLOBAL_NAME "job_enqueue_global"
#define NEXTCODE_LOG_JOB_ENQUEUE_GLOBAL_WITH_DELAY_NAME                           \
  "job_enqueue_global_with_delay"
#define NEXTCODE_LOG_JOB_ENQUEUE_MAIN_EXECUTOR_NAME "job_enqueue_main_executor"
#define NEXTCODE_LOG_JOB_RUN_NAME "job_run"
...
extern nextcode::once_t LogsToken;
...
#define ENSURE_LOGS(...)                                                       \
  do {                                                                         \
    if (!runtime::trace::tracingReady())                                       \
      return __VA_ARGS__;                                                      \
    nextcode::once(LogsToken, setupLogs, nullptr);                                \
    if (!TracingEnabled)                                                       \
      return __VA_ARGS__;                                                      \
  } while (0)
...
inline void actor_create(HeapObject *actor) {
  ...
  // Do an explicit enabled check here to avoid the cost of nextcode_getTypeName in
  // the common case.
  if (!os_signpost_enabled(ActorLog))
    return;

  auto typeName = nextcode_getTypeName(nextcode_getObjectType(actor), true);

  auto id = os_signpost_id_make_with_pointer(ActorLog, actor);
  os_signpost_interval_begin(ActorLog, id, NEXTCODE_LOG_ACTOR_LIFETIME_NAME,
...
inline void actor_destroy(HeapObject *actor) {
  ENSURE_LOGS();
  auto id = os_signpost_id_make_with_pointer(ActorLog, actor);
  os_signpost_interval_end(ActorLog, id, NEXTCODE_LOG_ACTOR_LIFETIME_NAME,
                           "actor=%p", actor);
}
...
inline void actor_deallocate(HeapObject *actor) {
  ENSURE_LOGS();
  auto id = os_signpost_id_make_with_pointer(ActorLog, actor);
  os_signpost_event_emit(ActorLog, id, NEXTCODE_LOG_ACTOR_DEALLOCATE_NAME,
                         "actor=%p", actor);
}
...
inline void actor_enqueue(HeapObject *actor, Job *job) {
  if (AsyncTask *task = dyn_cast<AsyncTask>(job)) {
    ENSURE_LOGS();
    auto id = os_signpost_id_make_with_pointer(ActorLog, actor);
    os_signpost_event_emit(ActorLog, id, NEXTCODE_LOG_ACTOR_ENQUEUE_NAME,
...
inline void actor_dequeue(HeapObject *actor, Job *job) {
  if (AsyncTask *task = dyn_cast_or_null<AsyncTask>(job)) {
    ENSURE_LOGS();
    auto id = os_signpost_id_make_with_pointer(ActorLog, actor);
    os_signpost_event_emit(ActorLog, id, NEXTCODE_LOG_ACTOR_DEQUEUE_NAME,
                           "actor=%p task=%" PRIx64, actor, task->getTaskId());
  }
}
...
inline void actor_state_changed(HeapObject *actor, Job *firstJob, uint8_t state,
  ...
  ENSURE_LOGS();
  auto id = os_signpost_id_make_with_pointer(ActorLog, actor);
  os_signpost_event_emit(ActorLog, id, NEXTCODE_LOG_ACTOR_STATE_CHANGED_NAME,
...
inline void actor_note_job_queue(HeapObject *actor, Job *first,
  ...
  auto id = os_signpost_id_make_with_pointer(ActorLog, actor);
  os_signpost_event_emit(ActorLog, id, NEXTCODE_LOG_ACTOR_JOB_QUEUE_NAME,
...
inline void task_create(AsyncTask *task, AsyncTask *parent, TaskGroup *group,
  ...
  os_signpost_interval_begin(
      TaskLog, id, NEXTCODE_LOG_TASK_LIFETIME_NAME,
      "task=%" PRIx64
...
inline void task_destroy(AsyncTask *task) {
  ENSURE_LOGS();
  auto id = os_signpost_id_make_with_pointer(TaskLog, task);
  os_signpost_interval_end(TaskLog, id, NEXTCODE_LOG_TASK_LIFETIME_NAME,
...
inline void task_status_changed(AsyncTask *task, uint8_t maxPriority,
  ...
  os_signpost_event_emit(
      TaskLog, id, NEXTCODE_LOG_TASK_STATUS_CHANGED_NAME,
...
inline void task_flags_changed(AsyncTask *task, uint8_t jobPriority,
  ...
  auto id = os_signpost_id_make_with_pointer(TaskLog, task);
  os_signpost_event_emit(
      TaskLog, id, NEXTCODE_LOG_TASK_FLAGS_CHANGED_NAME,
...
inline void task_wait(AsyncTask *task, AsyncTask *waitingOn, uintptr_t status) {
  ...
  os_signpost_interval_begin(TaskLog, id, NEXTCODE_LOG_TASK_WAIT_NAME,
...
inline void task_resume(AsyncTask *task) {
  auto id = os_signpost_id_make_with_pointer(TaskLog, task);
  os_signpost_interval_end(TaskLog, id, NEXTCODE_LOG_TASK_WAIT_NAME,
                           "task=%" PRIx64, task->getTaskId());
}
...
inline void task_continuation_init(AsyncTask *task,
  ...
  os_signpost_interval_begin(TaskLog, id, NEXTCODE_LOG_TASK_CONTINUATION,
...
inline void task_continuation_await(ContinuationAsyncContext *context) {
  ENSURE_LOGS();
  auto id = os_signpost_id_make_with_pointer(TaskLog, context);
  os_signpost_event_emit(TaskLog, id, NEXTCODE_LOG_TASK_CONTINUATION_AWAIT,
                         "context=%p", context);
}
...
inline void task_continuation_resume(ContinuationAsyncContext *context,
  ...
  ENSURE_LOGS();
  auto id = os_signpost_id_make_with_pointer(TaskLog, context);
  os_signpost_interval_end(TaskLog, id, NEXTCODE_LOG_TASK_CONTINUATION,
...
inline void job_enqueue_global(Job *job) {
  if (AsyncTask *task = dyn_cast<AsyncTask>(job)) {
    ...
    os_signpost_event_emit(TaskLog, id, NEXTCODE_LOG_JOB_ENQUEUE_GLOBAL_NAME,
                           "task=%" PRIx64, task->getTaskId());
  }
}
...
inline void job_enqueue_global_with_delay(unsigned long long delay, Job *job) {
  if (AsyncTask *task = dyn_cast<AsyncTask>(job)) {
    ...
    auto id = os_signpost_id_make_with_pointer(TaskLog, job);
    os_signpost_event_emit(
        TaskLog, id, NEXTCODE_LOG_JOB_ENQUEUE_GLOBAL_WITH_DELAY_NAME,
...
inline void job_enqueue_main_executor(Job *job) {
  if (AsyncTask *task = dyn_cast<AsyncTask>(job)) {
    ...
    os_signpost_event_emit(TaskLog, id,
                           NEXTCODE_LOG_JOB_ENQUEUE_MAIN_EXECUTOR_NAME,
                           "task=%" PRIx64, task->getTaskId());
  }
}
...
inline job_run_info job_run_begin(Job *job) {
  ...
  if (AsyncTask *task = dyn_cast<AsyncTask>(job)) {
    ...
    os_signpost_interval_begin(TaskLog, handle, NEXTCODE_LOG_JOB_RUN_NAME,
                               "task=%" PRIx64, taskId);
...
inline void job_run_end(job_run_info info) {
  if (info.handle != OS_SIGNPOST_ID_INVALID) {
    ENSURE_LOGS();
    os_signpost_interval_end(TaskLog, info.handle, NEXTCODE_LOG_JOB_RUN_NAME,
