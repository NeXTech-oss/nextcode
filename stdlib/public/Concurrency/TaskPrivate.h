...
#ifndef NEXTCODE_CONCURRENCY_TASKPRIVATE_H
#define NEXTCODE_CONCURRENCY_TASKPRIVATE_H
...
#include "nextcode/Runtime/Error.h"
...
#define NEXTCODE_FATAL_ERROR nextcode_Concurrency_fatalError
#include "../runtime/StackAllocator.h"

namespace nextcode {

// Set to 1 to enable helpful debug spew to stderr
// If this is enabled, tests with `nextcode_task_debug_log` requirement can run.
#if 0
#define NEXTCODE_TASK_DEBUG_LOG_ENABLED 1
#define NEXTCODE_TASK_DEBUG_LOG(fmt, ...)                                         \
  fprintf(stderr, "[%#lx] [%s:%d](%s) " fmt "\n",                              \
          (unsigned long)Thread::current().platformThreadId(), __FILE__,       \
          __LINE__, __FUNCTION__, __VA_ARGS__)
#else
#define NEXTCODE_TASK_DEBUG_LOG_ENABLED 0
#define NEXTCODE_TASK_DEBUG_LOG(fmt, ...) (void)0
#endif
...
void *_nextcode_task_alloc_specific(AsyncTask *task, size_t size);

/// dellocate task-local memory on behalf of a specific task,
/// not necessarily the current one.  Generally this should only be
/// done on behalf of a child task.
void _nextcode_task_dealloc_specific(AsyncTask *task, void *ptr);
...
AsyncTask *_nextcode_task_clearCurrent();
/// Set the active task reference for the current thread.
AsyncTask *_nextcode_task_setCurrent(AsyncTask *newTask);
...
/// status record lock.
void _nextcode_taskGroup_cancelAllChildren(TaskGroup *group);
...
void _nextcode_taskGroup_detachChild(TaskGroup *group, AsyncTask *child);

/// Tell TSan about an acquiring load
inline void _nextcode_tsan_acquire(void *addr) {
  nextcode::tsan::acquire(addr);
}
/// Tell TSan about a releasing store
inline void _nextcode_tsan_release(void *addr) {
  nextcode::tsan::release(addr);
}
/// Tell TSan about a consuming load
inline void _nextcode_tsan_consume(void *addr) {
...
_nextcode_task_getDispatchQueueSerialExecutorWitnessTable() {
  extern SerialExecutorWitnessTable wtable
    NEXTCODE_ASM_LABEL_WITH_PREFIX("$ss17DispatchQueueShimCScfsWP");
  return &wtable;
}
#endif
...
void
nextcode_executor_escalate(SerialExecutorRef executor, AsyncTask *task, JobPriority newPriority);
...
///
///   @_silgen_name("nextcode_task_future_wait")
///   func _taskFutureGet<T>(_ task: Builtin.NativeObject) async -> T
///
///   @_silgen_name("nextcode_task_future_wait_throwing")
...
///
///   @_silgen_name("nextcode_asyncLet_waitThrowing")
///   func _asyncLetGetThrowing<T>(_ task: Builtin.RawPointer) async throws -> T
///
///   @_silgen_name("nextcode_taskGroup_wait_next_throwing")
...
public:
  ...
  void fillWithError(NeXTCodeError *error) {
    errorResult = error;
...
/// with the OS in order to have the intended side effects. On Darwin, NeXTCode
...
class alignas(2 * sizeof(void*)) ActiveTaskStatus {
  ...
  // include/nextcode/RemoteInspection/RuntimeInternals.h. Any changes to the layout here
...
public:
  ...
  /// without having first locked it; see nextcode_taskCancel.
...
struct AsyncTask::PrivateStorage {
  ...
  /// libnextcodeCore so that libnextcodeCore can control the layout of our initial
  /// state.
    ...
    while (true) {
      ...
      if (task->_private()._status().compare_exchange_weak(oldStatus, newStatus,
        ...
        if (oldStatus.isStoredPriorityEscalated()) {
          NEXTCODE_TASK_DEBUG_LOG("[Override] Reset override %#x on thread from task %p", oldStatus.getStoredPriority(), this);
          nextcode_dispatch_lock_override_end((qos_class_t) oldStatus.getStoredPriority());
        }
#endif
        break;
      }
  ...
  nextcode::atomic<ActiveTaskStatus> &_status() {
    return reinterpret_cast<nextcode::atomic<ActiveTaskStatus>&> (this->StatusStorage);
  }

  const nextcode::atomic<ActiveTaskStatus> &_status() const {
    return reinterpret_cast<const nextcode::atomic<ActiveTaskStatus>&> (this->StatusStorage);
  }
};
...
inline void AsyncTask::flagAsRunning() {
  ...
  if (!oldStatus.hasTaskDependency()) {
    NEXTCODE_TASK_DEBUG_LOG("%p->flagAsRunning() with no task dependency", this);
    ...
    while (true) {
      ...
      if (threadOverrideInfo.can_override && (maxTaskPriority > overrideFloor)) {
        NEXTCODE_TASK_DEBUG_LOG("[Override] Self-override thread with oq_floor %#x to match task %p's max priority %#x",
            overrideFloor, this, maxTaskPriority);

        (void) nextcode_dispatch_thread_override_self(maxTaskPriority);
      ...
      if (_private()._status().compare_exchange_weak(oldStatus, newStatus,
        ...
        nextcode_task_enterThreadLocalContext(
    ...
    assert(dependencyRecord != nullptr);
    NEXTCODE_TASK_DEBUG_LOG("[Dependency] %p->flagAsRunning() and remove dependencyRecord %p",
...
AsyncTask::flagAsAndEnqueueOnExecutor(SerialExecutorRef newExecutor) {
  ...
  if (!oldStatus.isRunning() && oldStatus.hasTaskDependency()) {
    ...
    NEXTCODE_TASK_DEBUG_LOG("[Dependency] %p->flagAsAndEnqueueOnExecutor() and update dependencyRecord %p",
      this, dependencyRecord);
    ...
    // they can't escalate.
    void *allocation = _nextcode_task_alloc_specific(this, sizeof(class TaskDependencyStatusRecord));
    ...
    NEXTCODE_TASK_DEBUG_LOG("[Dependency] %p->flagAsAndEnqueueOnExecutor() with dependencyRecord %p", this,
      dependencyRecord);
    ...
    if (oldStatus.isRunning()) {
      ...
      if (oldStatus.isStoredPriorityEscalated()) {
        NEXTCODE_TASK_DEBUG_LOG("[Override] Reset override %#x on thread from task %p",
          oldStatus.getStoredPriority(), this);
        nextcode_dispatch_lock_override_end((qos_class_t) oldStatus.getStoredPriority());
      }
#endif /* NEXTCODE_CONCURRENCY_ENABLE_PRIORITY_ESCALATION */
      nextcode_task_exitThreadLocalContext((char *)&_private().ExclusivityAccessSet[0]);
  ...
  nextcode_task_enqueue(this, newExecutor);
...
void AsyncTask::flagAsSuspended(TaskDependencyStatusRecord *dependencyStatusRecord) {
  NEXTCODE_TASK_DEBUG_LOG("[Dependency] %p->flagAsSuspended() with dependencyRecord %p", this,
...
inline void AsyncTask::destroyTaskDependency(TaskDependencyStatusRecord *dependencyRecord) {
  ...
  _nextcode_task_dealloc_specific(this, dependencyRecord);
...
inline void AsyncTask::flagAsSuspendedOnTask(AsyncTask *task) {
  ...
  NEXTCODE_TASK_DEBUG_LOG("[Dependency] Create a dependencyRecord %p for dependency on task %p", allocation, task);
...
inline void AsyncTask::flagAsSuspendedOnContinuation(ContinuationAsyncContext *context) {
  ...
  NEXTCODE_TASK_DEBUG_LOG("[Dependency] Create a dependencyRecord %p for dependency on continuation %p", allocation, context);
  _private().dependencyRecord = record;
...
inline void AsyncTask::flagAsSuspendedOnTaskGroup(TaskGroup *taskGroup) {
  ...
  NEXTCODE_TASK_DEBUG_LOG("[Dependency] Create a dependencyRecord %p for dependency on taskGroup %p", allocation, taskGroup);
  _private().dependencyRecord = record;
...
inline void AsyncTask::flagAsDestroyed() {
  NEXTCODE_TASK_DEBUG_LOG("task destroyed %p", this);
}
