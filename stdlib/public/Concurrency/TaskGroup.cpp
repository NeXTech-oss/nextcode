...
#if 0
#define NEXTCODE_TASK_GROUP_DEBUG_LOG_ENABLED 1
#define NEXTCODE_TASK_GROUP_DEBUG_LOG(group, fmt, ...)                     \
...
#define NEXTCODE_TASK_GROUP_DEBUG_LOG_0(group, fmt, ...)                   \
fprintf(stderr, "[%#lx] [%s:%d][group(%p)] (%s) " fmt "\n",             \
      ...
      __VA_ARGS__)
#else
#define NEXTCODE_TASK_GROUP_DEBUG_LOG_ENABLED 0
#define NEXTCODE_TASK_GROUP_DEBUG_LOG(group, fmt, ...) (void)0
#define NEXTCODE_TASK_GROUP_DEBUG_LOG_0(group, fmt, ...) (void)0
#endif
...
/// re-throw; If we threw the body task, we must nextcode_release the stored "first child error" (if it was present).
///
...
#define NEXTCODE_TASK_GROUP_BODY_THROWN_ERROR_WINS 1
...
public:
  ...
  enum class ReadyStatus : uintptr_t {
    ...
    /// A raw NeXTCodeError is stored in the item's storage, rather than a Task with an Error inside.
    ///
  ...
  struct PollResult {
    ...
    ///
    /// # Important: nextcode_release
    /// If if a task is returned here, the task MUST be nextcode_released
    ...
    static PollResult getError(NeXTCodeError *error) {
      assert(error);
      return PollResult{
  ...
  struct ReadyQueueItem {
    ...
    NeXTCodeError *getRawError(DiscardingTaskGroup *group) const {
      ...
      return reinterpret_cast<NeXTCodeError *>(storage & ~statusMask);
    }
    ...
    static ReadyQueueItem getRawError(DiscardingTaskGroup *group, NeXTCodeError *error) {
      assert(group && "only a discarding task group uses raw errors in the ready queue");
      return ReadyQueueItem{
...
protected:
#if NEXTCODE_STDLIB_SINGLE_THREADED_CONCURRENCY || NEXTCODE_CONCURRENCY_TASK_TO_THREAD_MODEL
  // Synchronization is simple here. In a single threaded mode, all nextcode tasks
...
public:
  ...
  void waitAll(NeXTCodeError* bodyError, AsyncTask *waitingTask,
                     OpaqueValue *resultPointer, NEXTCODE_ASYNC_CONTEXT AsyncContext *callerContext,
...
struct TaskGroupStatus {
  ...
  static void reportPendingTaskOverflow(TaskGroupBase* group, TaskGroupStatus status) {
    ...
    if (_nextcode_shouldReportFatalErrorsToDebugger()) {
      ...
      _nextcode_reportToDebugger(RuntimeErrorFlagFatal, message, &details);
    }
#endif
...
#endif
#if defined(NEXTCODE_STDLIB_HAS_ASL)
...
AsyncTask *TaskGroupBase::claimWaitingTask() {
  ...
  if (!waitQueue.compare_exchange_strong(waitingTask, nullptr)) {
    nextcode_Concurrency_fatalError(0, "Failed to claim waitingTask!");
  }
  return waitingTask;
}
...
TaskGroupStatus TaskGroupBase::statusAddPendingTaskAssumeRelaxed(bool unconditionally) {
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "addPending, after: %s", s.to_string(this).c_str());
  return s;
}
...
static void nextcode_taskGroup_initializeImpl(TaskGroup *group, const Metadata *T) {
  nextcode_taskGroup_initializeWithFlags(0, group, T);
}

// Initializes into the preallocated _group an actual instance.
NEXTCODE_CC(nextcode)
static void nextcode_taskGroup_initializeWithFlagsImpl(size_t rawGroupFlags,
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG_0(group, "create group, from task:%p; flags: isDiscardingResults=%d",
  ...
  assert(record->getKind() == nextcode::TaskStatusRecordKind::TaskGroup);
...
#else
  nextcode_unreachable("task groups not supported yet in embedded NeXTCode");
#endif
}
...
void TaskGroup::addChildTask(AsyncTask *child) {
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "attach child task = %p", child);
...
void TaskGroup::removeChildTask(AsyncTask *child) {
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "detach child task = %p", child);
...
NEXTCODE_CC(nextcode)
static void nextcode_taskGroup_destroyImpl(TaskGroup *group) {
  asBaseImpl(group)->destroy();
}

void AccumulatingTaskGroup::destroy() {
#if NEXTCODE_TASK_GROUP_DEBUG_LOG_ENABLED
  if (!this->isEmpty()) {
    auto status = this->statusLoadRelaxed();
    NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "destroy, tasks .ready = %d, .pending = %llu",
                         status.readyTasks(this), status.pendingTasks(this));
  } else {
    NEXTCODE_TASK_DEBUG_LOG("destroying task group = %p", this);
  }
#endif
  ...
  // This is done in NeXTCode's withTaskGroup function explicitly.
...
void DiscardingTaskGroup::destroy() {
  ...
  if (!this->isEmpty()) {
    ...
    NEXTCODE_TASK_DEBUG_LOG("destroying discarding task group = %p", this);
  }
#endif
  // Verify using the group's status that indeed we're expected to be empty
  assert(this->isEmpty() && "Attempted to destroy non-empty task group!");
...
static void fillGroupNextResult(TaskFutureWaitAsyncContext *context,
  ...
  case PollStatus::Error: {
    auto error = reinterpret_cast<NeXTCodeError *>(result.storage);
...
void DiscardingTaskGroup::enqueueCompletedTask(AsyncTask *completedTask, bool hadErrorResult) {
  if (!readyQueue.isEmpty()) {
    NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "discard task, we already have an error stored, completedTask:%p",
                               completedTask);
  }

  if (hadErrorResult) {
    // we only store the FIRST error in discardResults mode
    NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "store first error, completedTask:%p", completedTask);
    // continue handling as usual, which will perform the enqueue
  } else {
    NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "discard successful result, %p", completedTask);
...
void AccumulatingTaskGroup::offer(AsyncTask *completedTask, AsyncContext *context) {
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "offer, completedTask:%p, status:%s",
                             completedTask,
                             statusString().c_str());
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "ready: %d, pending: %llu",
...
void DiscardingTaskGroup::offer(AsyncTask *completedTask, AsyncContext *context) {
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "offer, completedTask:%p, error:%d, status:%s",
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "offer, complete, status afterComplete:%s", afterComplete.to_string(this).c_str());
  ...
  if (hadErrorResult) {
    ...
    if (afterComplete.hasWaitingTask() && afterComplete.pendingTasks(this) == 0) {
      ...
      if (readyQueue.dequeue(readyErrorItem)) {
        ...
        _nextcode_taskGroup_detachChild(asAbstract(this), completedTask);

        switch (readyErrorItem.getStatus()) {
        case ReadyStatus::RawError: {
          NEXTCODE_TASK_GROUP_DEBUG_LOG(
      ...
      NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "offer, enqueue child task:%p", completedTask);
      enqueueCompletedTask(completedTask, hadErrorResult);
      return unlock();
    } else {
      NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "offer, complete, discard child task:%p", completedTask);
      _nextcode_taskGroup_detachChild(asAbstract(this), completedTask);
      return unlock();
    }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunreachable-code"
    ...
    nextcode_unreachable("expected to early return from when handling offer of last task in group");
#pragma clang diagnostic pop
  }
  ...
  if (afterComplete.hasWaitingTask() && afterComplete.pendingTasks(this) == 0) {
    ...
    NEXTCODE_TASK_GROUP_DEBUG_LOG(this,
...
TaskGroupBase::PreparedWaitingTask TaskGroupBase::prepareWaitingTaskWithTask(
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "resume, waitingTask = %p, completedTask = %p, alreadyDecremented:%d, error:%d",
  ...
  assert(assumed.hasWaitingTask());
        ...
        if (isDiscardingResults() && hadErrorResult && taskWasRetained) {
          ...
          nextcode_release(completedTask);
        }
...
static void
task_group_wait_resume_adapter(NEXTCODE_ASYNC_CONTEXT AsyncContext *_context) {
...
NEXTCODE_CC(nextcodeasync) static void workaround_function_nextcode_taskGroup_wait_next_throwingImpl(
...
__attribute__((noinline))
NEXTCODE_CC(nextcodeasync) static void workaround_function_nextcode_taskGroup_waitAllImpl(
    OpaqueValue *result, NEXTCODE_ASYNC_CONTEXT AsyncContext *callerContext,
    TaskGroup *_group,
...
NEXTCODE_CC(nextcodeasync)
static void nextcode_taskGroup_wait_next_throwingImpl(
    OpaqueValue *resultPointer, NEXTCODE_ASYNC_CONTEXT AsyncContext *callerContext,
    TaskGroup *_group,
    ThrowingTaskFutureWaitContinuationFunction *resumeFunction,
    AsyncContext *rawContext) {
  ...
  assert(group && "nextcode_taskGroup_wait_next_throwing was passed context without group!");

  PollResult polled = group->poll(waitingTask);
  switch (polled.status) {
  case PollStatus::MustWait:
    NEXTCODE_TASK_DEBUG_LOG("poll group = %p, no ready tasks, waiting task = %p",
                         group, waitingTask);
    // The waiting task has been queued on the channel,
    // there were pending tasks so it will be woken up eventually.
#ifdef __ARM_ARCH_7K__
    return workaround_function_nextcode_taskGroup_wait_next_throwingImpl(
  ...
  case PollStatus::Error:
  case PollStatus::Success:
    NEXTCODE_TASK_GROUP_DEBUG_LOG(group, "poll, task = %p, ready task available = %p",
                         waitingTask, polled.retainedTask);
    fillGroupNextResult(context, polled);
    if (auto completedTask = polled.retainedTask) {
      // Remove the child from the task group's running tasks list.
      _nextcode_taskGroup_detachChild(asAbstract(group), completedTask);
...
PollResult AccumulatingTaskGroup::poll(AsyncTask *waitingTask) {
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "poll, waitingTask:%p", waitingTask);
  lock();
...
reevaluate_if_taskgroup_has_results:;
  ...
  if (assumed.isEmpty(this)) {
    NEXTCODE_TASK_DEBUG_LOG("poll group = %p, group is empty, no pending tasks", this);
  ...
  if (assumed.readyTasks(this)) {
    NEXTCODE_TASK_DEBUG_LOG("poll group = %p, tasks .ready = %d, .pending = %llu",
    ...
    if (status.compare_exchange_strong(
      ...
      // be nextcode_release'd; we kept it alive while it was in the readyQueue by
      ...
      switch (item.getStatus()) {
        case ReadyStatus::Success:
          ...
          _nextcode_tsan_acquire(static_cast<Job *>(result.retainedTask));
          unlock();
          return result;
        ...
        case ReadyStatus::RawError:
          nextcode_Concurrency_fatalError(0, "accumulating task group should never use raw-errors!");
      }
      nextcode_Concurrency_fatalError(0, "must return result when status compare-and-swap was successful");
  ...
  while (true) {
    ...
    NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "WATCH OUT, SET WAITER ONTO waitQueue.head = %p", waitQueue.load(std::memory_order_relaxed));
    if (waitQueue.compare_exchange_weak(
      ...
      NEXTCODE_TASK_DEBUG_LOG("[RunInline] Switching away from running %p to now running %p", oldTask, childTask);
      ...
      // executor
      nextcode_job_run(childTask, SerialExecutorRef::generic());
      haveRunOneChildTaskInline = true;

      NEXTCODE_TASK_DEBUG_LOG("[RunInline] Switching back from running %p to now running %p", childTask, oldTask);
...
NEXTCODE_CC(nextcodeasync)
static void nextcode_taskGroup_waitAllImpl(
    OpaqueValue *resultPointer, NEXTCODE_ASYNC_CONTEXT AsyncContext *callerContext,
    TaskGroup *_group,
    NeXTCodeError *bodyError,
    ThrowingTaskFutureWaitContinuationFunction *resumeFunction,
...
void TaskGroupBase::waitAll(NeXTCodeError* bodyError, AsyncTask *waitingTask,
                                  OpaqueValue *resultPointer, NEXTCODE_ASYNC_CONTEXT AsyncContext *callerContext,
                                  ThrowingTaskFutureWaitContinuationFunction *resumeFunction,
                                  AsyncContext *rawContext) {
  lock();
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "waitAll, bodyError = %p, status = %s", bodyError, statusString().c_str());
  ...
  NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "waitAll, status = %s", assumed.to_string(this).c_str());
  ...
  if (assumed.isEmpty(this)) {
    ...
    NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "waitAll, early return, no pending tasks, bodyError:%p, status = %s",
                               bodyError,  assumed.to_string(this).c_str());
    // No tasks in flight, we know no tasks were submitted before this poll
    // was issued, and if we parked here we'd potentially never be woken up.

#if NEXTCODE_TASK_GROUP_BODY_THROWN_ERROR_WINS
    if (bodyError) {
      fillGroupNextErrorResult(context, bodyError);
    } else {
      fillGroupNextResult(context, result);
    }
#else // so, not NEXTCODE_TASK_GROUP_BODY_THROWN_ERROR_WINS
    fillGroupNextResult(context, polled);
#endif // NEXTCODE_TASK_GROUP_BODY_THROWN_ERROR_WINS
  ...
  while (true) {
    ...
    if (waitQueue.compare_exchange_weak(
      ...
      NEXTCODE_TASK_GROUP_DEBUG_LOG(this, "waitAll, marked waiting status = %s", statusString().c_str());
      ...
      // there were pending tasks so it will be woken up eventually.
#ifdef __ARM_ARCH_7K__
      workaround_function_nextcode_taskGroup_waitAllImpl(
...
NEXTCODE_CC(nextcode)
static bool nextcode_taskGroup_isEmptyImpl(TaskGroup *group) {
  return asBaseImpl(group)->isEmpty();
}

NEXTCODE_CC(nextcode)
static bool nextcode_taskGroup_isCancelledImpl(TaskGroup *group) {
...
NEXTCODE_CC(nextcode)
static void nextcode_taskGroup_cancelAllImpl(TaskGroup *group) {
  asBaseImpl(group)->cancelAll();
}

bool TaskGroupBase::cancelAll() {
  NEXTCODE_TASK_DEBUG_LOG("cancel all tasks in group = %p", this);
  ...
  _nextcode_taskGroup_cancelAllChildren(asAbstract(this));

  return true;
}

NEXTCODE_CC(nextcode)
static void nextcode_task_cancel_group_child_tasksImpl(TaskGroup *group) {
  ...
  _nextcode_taskGroup_cancelAllChildren(group);
}
...
/// status record lock.
void nextcode::_nextcode_taskGroup_cancelAllChildren(TaskGroup *group) {
  ...
  for (auto childTask: group->getTaskRecord()->children())
    nextcode_task_cancel(childTask);
}
...
NEXTCODE_CC(nextcode)
static bool nextcode_taskGroup_addPendingImpl(TaskGroup *_group, bool unconditionally) {
  auto group = asBaseImpl(_group);
  auto assumed = group->statusAddPendingTaskAssumeRelaxed(unconditionally);
  NEXTCODE_TASK_DEBUG_LOG("add pending %s to group(%p), tasks pending = %d",
