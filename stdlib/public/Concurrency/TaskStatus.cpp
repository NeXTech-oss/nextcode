...
#include "nextcode/Runtime/AtomicWaitQueue.h"
...
static bool waitForStatusRecordUnlockIfNotSelfLocked(AsyncTask *task,
  ...
  while (true) {
    ...
    bool waited = waiter.tryReloadAndWait([&]() -> StatusRecordLockRecord* {
      ...
      status = task->_private()._status().load(NEXTCODE_MEMORY_ORDER_CONSUME);
      _nextcode_tsan_consume(task);
...
static void waitForStatusRecordUnlock(AsyncTask *task,
  ...
  while (true) {
    ...
    bool waited = waiter.tryReloadAndWait([&]() -> StatusRecordLockRecord* {
      ...
      auto record = cast<StatusRecordLockRecord>(status.getInnermostRecord());
      if (record->isStatusRecordLockedBySelf()) {
        nextcode_Concurrency_fatalError(0, "Waiting on a status record lock that is owned by self");
      }
      return record;
    });
    if (!waited)
...
static bool withStatusRecordLock(AsyncTask *task, ActiveTaskStatus status,
  ...
  while (true) {
    if (status.isStatusRecordLocked() &&
      ...
      NEXTCODE_TASK_DEBUG_LOG("[StatusRecordLock] Lock for task %p is already owned by thread", task);
      break;
    }
    ...
    if (task->_private()._status().compare_exchange_weak(status, newStatus,
      ...
      _nextcode_tsan_acquire(task);
      _nextcode_tsan_release(task);
...
bool nextcode::addStatusRecord(AsyncTask *task, TaskStatusRecord *newRecord,
    ActiveTaskStatus& oldStatus,
    llvm::function_ref<bool(ActiveTaskStatus, ActiveTaskStatus&)> shouldAddRecord) {

  NEXTCODE_TASK_DEBUG_LOG("Adding %p record to task %p", newRecord, task);
  while (true) {
...
NEXTCODE_CC(nextcode)
bool nextcode::addStatusRecordToSelf(TaskStatusRecord *record,
     llvm::function_ref<bool(ActiveTaskStatus, ActiveTaskStatus&)> testAddRecord) {
  return addStatusRecord(nextcode_task_getCurrent(), record, testAddRecord);
}

NEXTCODE_CC(nextcode)
bool nextcode::addStatusRecordToSelf(TaskStatusRecord *record, ActiveTaskStatus &status,
     llvm::function_ref<bool(ActiveTaskStatus, ActiveTaskStatus&)> testAddRecord) {
  return addStatusRecord(nextcode_task_getCurrent(), record, status, testAddRecord);
}
...
NEXTCODE_CC(nextcode)
void nextcode::removeStatusRecord(AsyncTask *task, TaskStatusRecord *record,
     ActiveTaskStatus& oldStatus,
     llvm::function_ref<void(ActiveTaskStatus, ActiveTaskStatus&)>fn) {

  NEXTCODE_TASK_DEBUG_LOG("remove status record = %p, from task = %p",
                       record, task);
...
// modify some flags in the ActiveTaskStatus at the same time.
NEXTCODE_CC(nextcode)
void nextcode::removeStatusRecordWhere(
     AsyncTask *task,
     ActiveTaskStatus& oldStatus,
  ...
  NEXTCODE_TASK_DEBUG_LOG("remove status record where(), from task = %p",
                       task);
...
NEXTCODE_CC(nextcode)
void nextcode::removeStatusRecordFromSelf(TaskStatusRecord *record, ActiveTaskStatus &status,
     llvm::function_ref<void(ActiveTaskStatus, ActiveTaskStatus&)>fn) {
  return removeStatusRecord(nextcode_task_getCurrent(), record, status, fn);
}

NEXTCODE_CC(nextcode)
void nextcode::removeStatusRecordFromSelf(TaskStatusRecord *record,
     llvm::function_ref<void(ActiveTaskStatus, ActiveTaskStatus&)>fn) {
  return removeStatusRecord(nextcode_task_getCurrent(), record, fn);
}

NEXTCODE_CC(nextcode)
void nextcode::updateStatusRecord(AsyncTask *task, TaskStatusRecord *record,
     llvm::function_ref<void()>updateRecord,
     ActiveTaskStatus& status,
     llvm::function_ref<void(ActiveTaskStatus, ActiveTaskStatus&)>fn) {

  NEXTCODE_TASK_DEBUG_LOG("Updating status record %p of task %p", record, task);
...
NEXTCODE_CC(nextcode)
static bool nextcode_task_hasTaskGroupStatusRecordImpl() {
...
nextcode_task_getPreferredTaskExecutorImpl() {
  if (auto task = nextcode_task_getCurrent()) {
...
nextcode_task_pushTaskExecutorPreferenceImpl(TaskExecutorRef taskExecutor) {
  auto task = nextcode_task_getCurrent();
  if (!task) {
  ...
  void *allocation = _nextcode_task_alloc_specific(
  ...
  NEXTCODE_TASK_DEBUG_LOG("[TaskExecutorPreference] Create task executor "
...
NEXTCODE_CC(nextcode)
static void nextcode_task_popTaskExecutorPreferenceImpl(
    TaskExecutorPreferenceStatusRecord *record) {
  NEXTCODE_TASK_DEBUG_LOG("[TaskExecutorPreference] Remove task executor "
                       "preference record %p from task:%p",
                       record, nextcode_task_getCurrent());
...
void AsyncTask::pushInitialTaskExecutorPreference(
  ...
  NEXTCODE_TASK_DEBUG_LOG("[InitialTaskExecutorPreference] Create a task "
                       "preference record %p for task:%p",
                       record, this);
...
void AsyncTask::dropInitialTaskExecutorPreferenceRecord() {
  NEXTCODE_TASK_DEBUG_LOG("[InitialTaskExecutorPreference] Drop initial task "
  ...
  withStatusRecordLock(this, [&](ActiveTaskStatus status) {
    for (auto r : status.records()) {
      if (r->getKind() == TaskStatusRecordKind::TaskExecutorPreference) {
        ...
        removeStatusRecordLocked(status, record);
        _nextcode_task_dealloc_specific(this, record);
        return;
      }
    }
  ...
  nextcode_release(executorIdentityToRelease);
}
...
NEXTCODE_CC(nextcode)
void nextcode::updateNewChildWithParentAndGroupState(AsyncTask *child,
  ...
  // Avoids the extra logic in `nextcode_task_cancel` and `nextcode_task_escalate`
...
NEXTCODE_CC(nextcode)
static void nextcode_taskGroup_attachChildImpl(TaskGroup *group,
                                            AsyncTask *child) {
  ...
  assert(parent == nextcode_task_getCurrent());
...
void nextcode::_nextcode_taskGroup_detachChild(TaskGroup *group,
                                         AsyncTask *child) {
...
static void performCancellationAction(TaskStatusRecord *record) {
  ...
  case TaskStatusRecordKind::TaskGroup: {
    ...
    _nextcode_taskGroup_cancelAllChildren(groupRecord->getGroup());
    return;
  }
...
NEXTCODE_CC(nextcode)
static void nextcode_task_cancelImpl(AsyncTask *task) {
  NEXTCODE_TASK_DEBUG_LOG("cancel task = %p", task);
  ...
  while (true) {
    ...
    if (task->_private()._status().compare_exchange_weak(oldStatus, newStatus,
            /*success*/ NEXTCODE_MEMORY_ORDER_CONSUME,
            /*failure*/ std::memory_order_relaxed)) {
      _nextcode_tsan_consume(task);
      break;
    }
...
static void performEscalationAction(TaskStatusRecord *record,
  ...
  case TaskStatusRecordKind::ChildTask: {
    auto childRecord = cast<ChildTaskStatusRecord>(record);
    for (AsyncTask *child: childRecord->children())
      nextcode_task_escalate(child, newPriority);
    return;
  }
  case TaskStatusRecordKind::TaskGroup: {
  ...
  case TaskStatusRecordKind::TaskDependency: {
    ...
    NEXTCODE_TASK_DEBUG_LOG("[Dependency] Escalating a task dependency record %p to %#x",
                    record, newPriority);
...
NEXTCODE_CC(nextcode)
JobPriority
static nextcode_task_escalateImpl(AsyncTask *task, JobPriority newPriority) {

  NEXTCODE_TASK_DEBUG_LOG("Escalating %p to %#zx priority", task, newPriority);
  auto oldStatus = task->_private()._status().load(std::memory_order_relaxed);
  auto newStatus = oldStatus;
  ...
  while (true) {
    ...
    if (oldStatus.getStoredPriority() >= newPriority) {
      NEXTCODE_TASK_DEBUG_LOG("Task is already at %#zx priority", oldStatus.getStoredPriority());
    ...
    if (oldStatus.isRunning() || oldStatus.isEnqueued()) {
      ...
      NEXTCODE_TASK_DEBUG_LOG("Escalated a task %p which had completed, do nothing", task);
      return oldStatus.getStoredPriority();
    } else {
      // Task is suspended.
  ...
  if (newStatus.isRunning()) {
    ...
    NEXTCODE_TASK_DEBUG_LOG("[Override] Escalating %p which is running on %#x to %#x", task, newStatus.currentExecutionLockOwner(), newPriority);
    nextcode_dispatch_lock_override_start_with_debounce(executionLock, newStatus.currentExecutionLockOwner(), (qos_class_t) newPriority);
#endif
    ...
    //  priority inversion
    NEXTCODE_TASK_DEBUG_LOG("[Override] Escalating %p which is enqueued", task);
  ...
  NEXTCODE_TASK_DEBUG_LOG("[Override] Escalating %p which is suspended to %#x", task, newPriority);
...
void TaskDependencyStatusRecord::performEscalationAction(JobPriority newPriority) {
  switch (this->DependencyKind) {
    case WaitingOnTask:
      NEXTCODE_TASK_DEBUG_LOG("[Dependency] Escalate dependent task %p noted in %p record",
        this->DependentOn.Task, this);
      nextcode_task_escalate(this->DependentOn.Task, newPriority);
      break;
    case WaitingOnContinuation:
      ...
      NEXTCODE_TASK_DEBUG_LOG("[Dependency] Escalate dependent continuation %p noted in %p record -- do nothing",
    ...
    case WaitingOnTaskGroup:
      ...
      NEXTCODE_TASK_DEBUG_LOG("[Dependency] Escalate dependent taskgroup %p noted in %p record -- do nothing",
        this->DependentOn.TaskGroup, this);
      break;
    case EnqueuedOnExecutor:
      NEXTCODE_TASK_DEBUG_LOG("[Dependency] Escalate dependent executor %p noted in %p record",
        this->DependentOn.Executor, this);
      nextcode_executor_escalate(this->DependentOn.Executor, this->WaitingTask, newPriority);
      break;
  }
}
