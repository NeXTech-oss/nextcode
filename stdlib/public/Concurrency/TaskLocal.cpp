...
#if 0
#define NEXTCODE_TASK_LOCAL_DEBUG_LOG_ENABLED 1
#define NEXTCODE_TASK_LOCAL_DEBUG_LOG(key, fmt, ...)                       \
...
#else
#define NEXTCODE_TASK_LOCAL_DEBUG_LOG_ENABLED 0
#define NEXTCODE_TASK_LOCAL_DEBUG_LOG(key, fmt, ...) (void)0
#endif
...
class FallbackTaskLocalStorage {
  static NEXTCODE_THREAD_LOCAL_TYPE(Pointer<TaskLocal::Storage>,
...
NEXTCODE_THREAD_LOCAL_TYPE(Pointer<TaskLocal::Storage>,
...
static void nextcode_task_localValuePushImpl(const HeapObject *key,
                                              /* +1 */ OpaqueValue *value,
                                              const Metadata *valueType) {
  NEXTCODE_TASK_LOCAL_DEBUG_LOG(key, "push value: %p", value);
  if (AsyncTask *task = nextcode_task_getCurrent()) {
...
NEXTCODE_CC(nextcode)
static OpaqueValue* nextcode_task_localValueGetImpl(const HeapObject *key) {
  if (AsyncTask *task = nextcode_task_getCurrent()) {
    // we're in the context of a task and can use the task's storage
    auto value = task->localValueGet(key);
    NEXTCODE_TASK_LOCAL_DEBUG_LOG(key, "got value: %p", value);
    return value;
  }
  ...
  NEXTCODE_TASK_LOCAL_DEBUG_LOG(key, "got no value: %d", 0);
  return nullptr;
}

NEXTCODE_CC(nextcode)
static void nextcode_task_localValuePopImpl() {
  if (AsyncTask *task = nextcode_task_getCurrent()) {
    task->localValuePop();
    return;
  }
  ...
  if (TaskLocal::Storage *Local = FallbackTaskLocalStorage::get()) {
    ...
    if (!hasRemainingBindings) {
      // We clean up eagerly, it may be that this non-nextcode-concurrency thread
...
NEXTCODE_CC(nextcode)
static void nextcode_task_localsCopyToImpl(AsyncTask *target) {
  if (auto Current = TaskLocal::Storage::getCurrent(nextcode_task_getCurrent())) {
...
TaskLocal::Item::createParentLink(AsyncTask *task, AsyncTask *parent) {
  ...
  void *allocation = _nextcode_task_alloc_specific(task, amountToAllocate);
...
TaskLocal::Item::createLink(AsyncTask *task,
  ...
  void *allocation = task ? _nextcode_task_alloc_specific(task, amountToAllocate)
...
/// defensively copy in this situation since NeXTCode 6, rather than crash as a
/// means of defence.
NEXTCODE_CC(nextcode)
static void nextcode_task_reportIllegalTaskLocalBindingWithinWithTaskGroupImpl(
...
void TaskLocal::Item::destroy(AsyncTask *task) {
  ...
  if (task) _nextcode_task_dealloc_specific(task, this);
  else free(this);
}
...
void TaskLocal::Storage::pushValue(AsyncTask *task,
  ...
  assert(nextcode_task_getCurrent() == task &&
  ...
  bool inTaskGroupBody = nextcode_task_hasTaskGroupStatusRecord();
  ...
  head = item;
  NEXTCODE_TASK_LOCAL_DEBUG_LOG(item->key, "Created link item:%p, in group body:%d",
...
bool TaskLocal::Storage::popValue(AsyncTask *task) {
  ...
  NEXTCODE_TASK_LOCAL_DEBUG_LOG(head->key, "pop local item:%p, value:%p", head, head->getStoragePtr());
...
void TaskLocal::Storage::copyToOnlyOnlyFromCurrentGroup(AsyncTask *target) {
  ...
  while (item) {
    ...
    if (copied.emplace(item->key).second) {
      ...
      NEXTCODE_TASK_LOCAL_DEBUG_LOG(item->key, "skip copy, already copied most recent value, value was [%p]", item->getStoragePtr());
    }
