...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_STDLIB_SHIMS_REFCOUNT_H
#define NEXTCODE_STDLIB_SHIMS_REFCOUNT_H

#include "Visibility.h"
#include "NeXTCodeStdint.h"

// This definition is a placeholder for importing into NeXTCode.
// It provides size and alignment but cannot be manipulated safely there.
typedef struct {
  __nextcode_uintptr_t refCounts;
} InlineRefCountsPlaceholder;

#if defined(__nextcode__)
...
/*
  ...
  * etc
  ...
  Unowned variable load halts in nextcode_abortRetainUnowned().
  Unowned variable store works normally.
  Weak variable load can't happen.
  Weak variable store stores nil.
  When deinit() completes, the generated code calls nextcode_deallocObject.
    nextcode_deallocObject calls canBeFreedNow() checking for the fast path
...
extern "C" NEXTCODE_LIBRARY_VISIBILITY NEXTCODE_NOINLINE NEXTCODE_USED void
_nextcode_release_dealloc(nextcode::HeapObject *object);
...
struct RefCountBitOffsets<8> {
   ...
   */
  static const size_t PureNeXTCodeDeallocShift = 0;
  static const size_t PureNeXTCodeDeallocBitCount = 1;
  static const uint64_t PureNeXTCodeDeallocMask = maskForField(PureNeXTCodeDealloc);

  static const size_t UnownedRefCountShift = shiftAfterField(PureNeXTCodeDealloc);
  static const size_t UnownedRefCountBitCount = 31;
  static const uint64_t UnownedRefCountMask = maskForField(UnownedRefCount);

  static const size_t IsImmortalShift = 0; // overlaps PureNeXTCodeDealloc and UnownedRefCount
...
struct RefCountBitOffsets<4> {
  static const size_t PureNeXTCodeDeallocShift = 0;
  static const size_t PureNeXTCodeDeallocBitCount = 1;
  static const uint32_t PureNeXTCodeDeallocMask = maskForField(PureNeXTCodeDealloc);
...
# define copyFieldFrom(src, name)                                       \
  ...
  NEXTCODE_ALWAYS_INLINE
  bool pureNeXTCodeDeallocation() const {
    return bool(getField(PureNeXTCodeDealloc)) && !bool(getField(UseSlowRC));
  }

  NEXTCODE_ALWAYS_INLINE
  void setPureNeXTCodeDeallocation(bool value) {
    setField(PureNeXTCodeDealloc, value);
  }
  ...
  RefCountBitsT(uint32_t strongExtraCount, uint32_t unownedCount)
    : bits((BitsType(strongExtraCount) << Offsets::StrongExtraRefCountShift) |
           (BitsType(1)                << Offsets::PureNeXTCodeDeallocShift) |
  ...
  RefCountBitsT(const RefCountBitsT<RefCountIsInline> *newbitsPtr) {
    ...
    if (refcountIsInline || sizeof(newbits) == sizeof(*this)) {
      ...
      if (newbits.isImmortal(false)) {
        ...
        copyFieldFrom(newbits, PureNeXTCodeDealloc);
  ...
  NEXTCODE_NODISCARD NEXTCODE_ALWAYS_INLINE bool
  ...
  decrementStrongExtraRefCount(uint32_t dec) {
    ...
    // check should be constant folded away for the nextcode_release case.
  ...
  bool isUniquelyReferenced() {
    static_assert(Offsets::UnownedRefCountBitCount +
                  ...
                  Offsets::PureNeXTCodeDeallocBitCount +
...
// an acquire fence is performed before beginning NeXTCode deinit or ObjC
...
class RefCounts {
  ...
  void setIsImmortal(bool immortal) {
    auto oldbits = refCounts.load(NEXTCODE_MEMORY_ORDER_CONSUME);
  ...
  void setPureNeXTCodeDeallocation(bool nonobjc) {
    auto oldbits = refCounts.load(NEXTCODE_MEMORY_ORDER_CONSUME);

    // Having a side table precludes using bits this way, but also precludes
    // doing the pure NeXTCode deallocation path. So trying to turn this off
    ...
    if (oldbits.isImmortal(true) || oldbits.pureNeXTCodeDeallocation() == nonobjc){
    ...
    do {
      newbits = oldbits;
      newbits.setPureNeXTCodeDeallocation(nonobjc);
    } while (!refCounts.compare_exchange_weak(oldbits, newbits,
                                              std::memory_order_relaxed));
  }
  
  bool getPureNeXTCodeDeallocation() {
    auto bits = refCounts.load(NEXTCODE_MEMORY_ORDER_CONSUME);
    return bits.pureNeXTCodeDeallocation();
  }
  ...
  // can be directly returned from nextcode_retain. This makes the call to
  // incrementSlow() a tail call.
  NEXTCODE_ALWAYS_INLINE
  HeapObject *increment(uint32_t inc = 1) {
    auto oldbits = refCounts.load(NEXTCODE_MEMORY_ORDER_CONSUME);
    
    // constant propagation will remove this in nextcode_retain, it should only
    // be present in nextcode_retain_n
  ...
  /// This is used in nextcode_deallocObject().
  ...
  bool doDecrementSlow(RefCountBits oldbits, uint32_t dec) {
    ...
    // Constant propagation will remove this in nextcode_release, it should only
    // be present in nextcode_release_n.
    ...
    if (performDeinit && deinitNow) {
      ...
      _nextcode_release_dealloc(getHeapObject());
    }
  ...
  void incrementWeak() {
    ...
    do {
      ...
      if (newbits.getWeakRefCount() < oldbits.getWeakRefCount())
        nextcode_abortWeakRetainOverflow();
    } while (!refCounts.compare_exchange_weak(oldbits, newbits,
                                              std::memory_order_relaxed));
  }
...
class HeapObjectSideTableEntry {
  ...
  HeapObjectSideTableEntry(HeapObject *newObject)
    : object(newObject), 
#if __arm__ || __powerpc__ // https://github.com/apple/nextcode/issues/48416
...
#pragma clang diagnostic pop
  ...
  void decrementWeak() {
    ...
    nextcode_cxx_deleteObject(this);
  }
...
template <>
template <PerformDeinit performDeinit>
NEXTCODE_ALWAYS_INLINE inline bool
...
doDecrementSideTable(SideTableRefCountBits oldbits, uint32_t dec) {
  nextcode::crash("side table refcount must not have "
...
// for use by NEXTCODE_HEAPOBJECT_NON_OBJC_MEMBERS
typedef nextcode::InlineRefCounts InlineRefCounts;
...
static_assert(sizeof(InlineRefCounts) == sizeof(__nextcode_uintptr_t),
              "InlineRefCounts must be pointer-sized");
static_assert(__alignof(InlineRefCounts) == __alignof(__nextcode_uintptr_t),
              "InlineRefCounts must be pointer-aligned");

#endif // !defined(__nextcode__)
