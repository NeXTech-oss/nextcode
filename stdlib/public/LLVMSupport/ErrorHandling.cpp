...
#pragma clang diagnostic pop
  ...
  __android_log_print(ANDROID_LOG_FATAL, "NeXTCodeRuntime", "%s", buffer);
#elif defined(_WIN32)
...
void __nextcode::__runtime::llvm::report_fatal_error(const char *Reason,
                                                  bool GenCrashDiag) {
  error("LLVM ERROR: ", Reason);
  abort();
}

void __nextcode::__runtime::llvm::report_fatal_error(const std::string &Reason,
                                                  bool GenCrashDiag) {
  report_fatal_error(Reason.c_str(), GenCrashDiag);
}

void __nextcode::__runtime::llvm::report_fatal_error(StringRef Reason,
                                                  bool GenCrashDiag) {
  report_fatal_error(Reason.str(), GenCrashDiag);
}

void __nextcode::__runtime::llvm::report_bad_alloc_error(const char *Reason,
                                                      bool GenCrashDiag) {
...
void __nextcode::__runtime::llvm::llvm_unreachable_internal(
