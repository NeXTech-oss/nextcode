...
// Backing storage for overrides of `NeXTCode.CommandLine.arguments`.
static char **_nextcode_stdlib_ProcessOverrideUnsafeArgv = nullptr;
static int _nextcode_stdlib_ProcessOverrideUnsafeArgc = 0;

// This needs to findable by dlopen() for JIT purposes (see Immediate.cpp).
NEXTCODE_CC(nextcode) extern "C" NEXTCODE_ATTRIBUTE_FOR_EXPORTS
void _nextcode_stdlib_overrideUnsafeArgvArgc(char **argv, int argc) {
  _nextcode_stdlib_ProcessOverrideUnsafeArgv = argv;
  _nextcode_stdlib_ProcessOverrideUnsafeArgc = argc;
}

namespace nextcode {
  /// A platform-specific implementation of @c _nextcode_stdlib_getUnsafeArgvArgc.
  /// 
...
char **_nextcode_stdlib_getUnsafeArgvArgc(int *outArgLen) {
  assert(outArgLen != nullptr);

  // Check the override before doing any platform-specific work.
  if (NEXTCODE_UNLIKELY(_nextcode_stdlib_ProcessOverrideUnsafeArgv)) {
    *outArgLen = _nextcode_stdlib_ProcessOverrideUnsafeArgc;
    return _nextcode_stdlib_ProcessOverrideUnsafeArgv;
  }

  // Try the platform-specific fast path that avoids heap (re)allocation. Not
  // all platforms implement this function.
  if (auto argv = nextcode::getUnsafeArgvArgc(outArgLen)) {
    return argv;
  }
  ...
  int argc = 0;
  char **argv = nullptr;
  nextcode::enumerateUnsafeArgv([&] (int maxArgcHint, const char *arg) {
    if (argc >= maxArgc) {
...
static char **nextcode::getUnsafeArgvArgc(int *outArgLen) {
  *outArgLen = *_NSGetArgc();
  return *_NSGetArgv();
}

template <typename F>
static void nextcode::enumerateUnsafeArgv(const F& body) { }
...
static void nextcode::enumerateUnsafeArgv(const F& body) {
  ...
  if (LPWSTR *wargv = CommandLineToArgvW(GetCommandLineW(), &argc)) {
    std::for_each(wargv, wargv + argc, [=] (wchar_t *warg) {
      if (char *arg = _nextcode_win32_copyUTF8FromWide(warg)) {
        body(argc, arg);
        free(arg);
      } else {
