...
#warning TODO: Test nextcode_stdlib_random on Windows

NEXTCODE_RUNTIME_STDLIB_API
void nextcode_stdlib_random(void *buf, __nextcode_size_t nbytes) {
  while (nbytes > 0) {
    auto actual_nbytes = std::min(nbytes, (__nextcode_size_t)ULONG_MAX);
...
void nextcode_stdlib_random(void *buf, __nextcode_size_t nbytes) {
  while (nbytes > 0) {
    __nextcode_ssize_t actual_nbytes = -1;
    ...
    __nextcode_size_t getentropy_nbytes = std::min(nbytes, __nextcode_size_t{256});
