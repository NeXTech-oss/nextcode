...
static inline __nextcode_uint32_t scramble(__nextcode_uint32_t scalar) {
  scalar *= 0xCC9E2D51;
...
static __nextcode_uint32_t hash(__nextcode_uint32_t scalar, __nextcode_uint32_t level,
                             __nextcode_uint32_t seed) {
  __nextcode_uint32_t hash = seed;
...
__nextcode_intptr_t _nextcode_stdlib_getMphIdx(__nextcode_uint32_t scalar,
  ...
  __nextcode_intptr_t resultIdx = 0;
  ...
  for (int i = 0; i != levels; i += 1) {
    ...
    auto idx = (__nextcode_uint64_t) hash(scalar, sizes[i], i);

    auto word = bitArray[idx / 64];
    auto mask = (__nextcode_uint64_t) 1 << (idx % 64);
...
__nextcode_intptr_t _nextcode_stdlib_getScalarBitArrayIdx(__nextcode_uint32_t scalar,
  ...
  if ((__nextcode_uint64_t) idx > quickLookSize - 1) {
    return INTPTR_MAX;
  }
  ...
  // one of them.
  if ((quickLook & ((__nextcode_uint64_t) 1 << chunkBit)) == 0) {
    return INTPTR_MAX;
  }
  ...
  if ((chunkWord & ((__nextcode_uint64_t) 1 << scalarSpecificBit)) == 0) {
    return INTPTR_MAX;
  }
