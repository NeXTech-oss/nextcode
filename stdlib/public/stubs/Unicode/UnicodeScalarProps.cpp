...
__nextcode_uint64_t _nextcode_stdlib_getBinaryProperties(__nextcode_uint32_t scalar) {
  ...
  while (upperBoundIndex >= lowerBoundIndex) {
    ...
    if (scalar >= lowerBoundScalar && scalar <= upperBoundScalar) {
      return  _nextcode_stdlib_scalar_binProps_data[dataIndex];
    }
...
__nextcode_uint8_t _nextcode_stdlib_getNumericType(__nextcode_uint32_t scalar) {
  ...
  while (upperBoundIndex >= lowerBoundIndex) {
    ...
    auto entry = _nextcode_stdlib_numeric_type[idx];

    auto lowerBoundScalar = (entry << 11) >> 11;
    auto rangeCount = (entry << 3) >> 24;
    auto upperBoundScalar = lowerBoundScalar + rangeCount;

    auto numericType = (__nextcode_uint8_t)(entry >> 29);
...
double _nextcode_stdlib_getNumericValue(__nextcode_uint32_t scalar) {
  ...
  __nextcode_intptr_t scalarIdx = _nextcode_stdlib_getMphIdx(scalar, levelCount,
                                                  _nextcode_stdlib_numeric_values_keys,
                                                  _nextcode_stdlib_numeric_values_ranks,
                                                  _nextcode_stdlib_numeric_values_sizes);

  auto valueIdx = _nextcode_stdlib_numeric_values_indices[scalarIdx];
  return _nextcode_stdlib_numeric_values[valueIdx];
#endif
}
...
const char *_nextcode_stdlib_getNameAlias(__nextcode_uint32_t scalar) {
  ...
  auto dataIdx = _nextcode_stdlib_getScalarBitArrayIdx(scalar,
                                                    _nextcode_stdlib_nameAlias,
                                                  _nextcode_stdlib_nameAlias_ranks);

  if (dataIdx == INTPTR_MAX) {
    return nullptr;
  }

  return _nextcode_stdlib_nameAlias_data[dataIdx];
#endif
}
...
__nextcode_int32_t _nextcode_stdlib_getMapping(__nextcode_uint32_t scalar,
...
#else
  auto dataIdx = _nextcode_stdlib_getScalarBitArrayIdx(scalar,
                                                    _nextcode_stdlib_mappings,
                                                  _nextcode_stdlib_mappings_ranks);

  if (dataIdx == INTPTR_MAX) {
    return 0;
  }

  auto mappings = _nextcode_stdlib_mappings_data_indices[dataIdx];

  __nextcode_uint8_t mappingIdx;
  ...
  return _nextcode_stdlib_mappings_data[mappingIdx];
#endif
}
...
#else
  auto dataIdx = _nextcode_stdlib_getScalarBitArrayIdx(scalar,
                                                 _nextcode_stdlib_special_mappings,
                                          _nextcode_stdlib_special_mappings_ranks);

  if (dataIdx == INTPTR_MAX) {
    return nullptr;
  }

  auto index = _nextcode_stdlib_special_mappings_data_indices[dataIdx];

  auto uppercase = _nextcode_stdlib_special_mappings_data + index;
...
__nextcode_intptr_t _nextcode_stdlib_getScalarName(__nextcode_uint32_t scalar,
  ...
  nextcode::nextcode_abortDisabledUnicodeSupport();
#else
  auto setOffset = _nextcode_stdlib_names_scalar_sets[scalar >> 7];
  ...
  auto scalarOffset = _nextcode_stdlib_names_scalars[scalarIndex];
  ...
  __nextcode_uint32_t nextScalarOffset = 0;
  ...
  if (scalarIndex != NAMES_SCALARS_MAX_INDEX) {
    ...
    while (nextScalarOffset == 0) {
      nextScalarOffset = _nextcode_stdlib_names_scalars[scalarIndex + i];
      i += 1;
    }
  } else {
  ...
  for (__nextcode_uint32_t i = 0; i < nameSize; i += 1) {
    __nextcode_uint16_t wordIndex = (__nextcode_uint16_t) _nextcode_stdlib_names[
      scalarOffset + i
    ];
    ...
    if (wordIndex == 0xFF) {
      i += 1;
      auto firstPart = _nextcode_stdlib_names[scalarOffset + i];
      wordIndex = firstPart;

      i += 1;
      auto secondPart = _nextcode_stdlib_names[scalarOffset + i];
      wordIndex |= secondPart << 8;
    }

    auto wordOffset = _nextcode_stdlib_word_indices[wordIndex];

    auto word = _nextcode_stdlib_words + wordOffset;
...
__nextcode_uint16_t _nextcode_stdlib_getAge(__nextcode_uint32_t scalar) {
  ...
  while (upperBoundIndex >= lowerBoundIndex) {
    ...
    auto entry = _nextcode_stdlib_ages[idx];

    auto lowerBoundScalar = (entry << 43) >> 43;
    auto rangeCount = entry >> 32;
    auto upperBoundScalar = lowerBoundScalar + rangeCount;

    auto ageIdx = (__nextcode_uint8_t)((entry << 32) >> 32 >> 21);

    if (scalar >= lowerBoundScalar && scalar <= upperBoundScalar) {
      return _nextcode_stdlib_ages_data[ageIdx];
    }
...
__nextcode_uint8_t _nextcode_stdlib_getGeneralCategory(__nextcode_uint32_t scalar) {
  ...
  while (upperBoundIndex >= lowerBoundIndex) {
    ...
    auto entry = _nextcode_stdlib_generalCategory[idx];

    auto lowerBoundScalar = (entry << 43) >> 43;
    auto rangeCount = entry >> 32;
    auto upperBoundScalar = lowerBoundScalar + rangeCount;

    auto generalCategory = (__nextcode_uint8_t)((entry << 32) >> 32 >> 21);
...
__nextcode_uint8_t _nextcode_stdlib_getScript(__nextcode_uint32_t scalar) {
  ...
  while (upperBoundIndex >= lowerBoundIndex) {
    ...
    auto entry = _nextcode_stdlib_scripts[index];
    ...
    // distance to the next element.
    if (index != endIndex - 1) {
      auto nextEntry = _nextcode_stdlib_scripts[index + 1];
...
NEXTCODE_RUNTIME_STDLIB_INTERNAL
const __nextcode_uint8_t *_nextcode_stdlib_getScriptExtensions(__nextcode_uint32_t scalar,
                                                       __nextcode_uint8_t *count) {
#if !NEXTCODE_STDLIB_ENABLE_UNICODE_DATA
  nextcode::nextcode_abortDisabledUnicodeSupport();
#else
  auto dataIdx = _nextcode_stdlib_getScalarBitArrayIdx(scalar,
                                                _nextcode_stdlib_script_extensions,
                                         _nextcode_stdlib_script_extensions_ranks);
  ...
  auto scalarDataIdx = _nextcode_stdlib_script_extensions_data_indices[dataIdx];
  *count = scalarDataIdx >> 11;
  
  return _nextcode_stdlib_script_extensions_data + (scalarDataIdx & 0x7FF);
#endif
}
...
void _nextcode_stdlib_getCaseMapping(__nextcode_uint32_t scalar,
  ...
  nextcode::nextcode_abortDisabledUnicodeSupport();
#else
  auto mphIdx = _nextcode_stdlib_getMphIdx(scalar, CASE_FOLD_LEVEL_COUNT,
                                        _nextcode_stdlib_case_keys,
                                        _nextcode_stdlib_case_ranks,
                                        _nextcode_stdlib_case_sizes);
  
  auto caseValue = _nextcode_stdlib_case[mphIdx];
  __nextcode_uint32_t hashedScalar = (caseValue << 43) >> 43;
  ...
  if ((caseValue & ((__nextcode_uint64_t)(0x1) << 63)) == 0) {
    auto distance = (__nextcode_int32_t)((caseValue << 1) >> 22);
    auto mappedScalar = (__nextcode_uint32_t)((__nextcode_int32_t)(scalar) - distance);
  ...
  auto fullMphIdx = _nextcode_stdlib_getMphIdx(scalar, CASE_FULL_FOLD_LEVEL_COUNT,
                                            _nextcode_stdlib_case_full_keys,
                                            _nextcode_stdlib_case_full_ranks,
                                            _nextcode_stdlib_case_full_sizes);
  
  auto fullCaseValue = _nextcode_stdlib_case_full[fullMphIdx];

  // Count is either 2 or 3.
  auto count = fullCaseValue >> 62;

  for (__nextcode_uint64_t i = 0; i != count; i += 1) {
    auto distance = (__nextcode_int32_t)(fullCaseValue & 0xFFFF);
