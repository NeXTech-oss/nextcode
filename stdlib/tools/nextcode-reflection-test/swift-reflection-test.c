//===--- nextcode-reflection-test.c - Reflection testing application ---------===//
//
/*
...
static
const void *PipeMemoryReader_readBytes(void *Context, nextcode_addr_t Address,
...
static
nextcode_addr_t PipeMemoryReader_getSymbolAddress(void *Context,
...
#if defined(__APPLE__) && defined(__MACH__)
static void
PipeMemoryReader_receiveImages(NeXTCodeReflectionContextRef RC,
                                       const PipeMemoryReader *Reader) {
  int WriteFD = PipeMemoryReader_getParentWriteFD(Reader);
  ...
  for (size_t i = 0; i < NumReflectionInfos; ++i) {
    ...
    nextcode_reflection_addImage(RC, Images[i].Start);
  }
...
static nextcode_reflection_section_t
makeLocalSection(const void *Buffer,
                 nextcode_remote_reflection_section_t Section) {
  if (Section.Size == 0) {
    nextcode_reflection_section_t LS = {NULL, NULL};
    return LS;
  }

  nextcode_reflection_section_t LS = {(void *)Buffer,
                                   (void *)((uint8_t *)Buffer + Section.Size)};
  return LS;
}

static nextcode_reflection_mapping_info_t makeNonContiguousReflectionInfo(
    nextcode_remote_reflection_section_t *remote_sections,
    nextcode_reflection_section_t *local_sections) {
  nextcode_reflection_section_mapping_t sections[SECTIONS_PER_INFO];
  for (size_t i = 0; i < SECTIONS_PER_INFO; ++i) {
    nextcode_reflection_section_mapping_t section = {local_sections[i],
                                                           remote_sections[i]};
    sections[i] = section;
  }

  nextcode_reflection_mapping_info_t ReflectionMappingInfo = {
...
static nextcode_remote_reflection_section_t
makeRemoteSection(const PipeMemoryReader *Reader) {
  ...
  nextcode_remote_reflection_section_t RS = {Start, Size};
...
static const void *PipeMemoryReader_readRemoteSection(
    const PipeMemoryReader *Reader,
    nextcode_remote_reflection_section_t *RemoteSection, void **outFreeContext) {
...
static void
PipeMemoryReader_receiveReflectionInfo(NeXTCodeReflectionContextRef RC,
                                       const PipeMemoryReader *Reader) {
  int WriteFD = PipeMemoryReader_getParentWriteFD(Reader);
  ...
  nextcode_remote_reflection_section_t *RemoteSections =
      calloc(NumReflectionInfos * SECTIONS_PER_INFO,
             sizeof(nextcode_remote_reflection_section_t));
  ...
  nextcode_reflection_section_t *LocalSections =
      calloc(NumReflectionInfos * SECTIONS_PER_INFO,
             sizeof(nextcode_reflection_section_t));
  for (size_t i = 0; i < NumReflectionInfos * SECTIONS_PER_INFO; ++i) {
  ...
  for (size_t i = 0; i < NumReflectionInfos * SECTIONS_PER_INFO;
    ...
    nextcode_reflection_addReflectionMappingInfo(RC, Info);
  }
...
uint64_t PipeMemoryReader_getStringLength(void *Context, nextcode_addr_t Address) {
  const PipeMemoryReader *Reader = (const PipeMemoryReader *)Context;
  int WriteFD = PipeMemoryReader_getParentWriteFD(Reader);
...
int reflectHeapObject(NeXTCodeReflectionContextRef RC,
  ...
  nextcode_typeref_t TR = nextcode_reflection_typeRefForInstance(RC, instance);

  printf("Type reference:\n");
  nextcode_reflection_dumpTypeRef(TR);
  printf("\n");

  printf("Type info:\n");
  nextcode_reflection_dumpInfoForInstance(RC, instance);
...
int reflectExistentialImpl(
    NeXTCodeReflectionContextRef RC, const PipeMemoryReader Pipe,
    nextcode_typeref_t MockExistentialTR,
    int (*ProjectExistentialFn)(NeXTCodeReflectionContextRef, nextcode_addr_t,
                                nextcode_typeref_t, nextcode_typeref_t *,
                                nextcode_addr_t *)) {
  uintptr_t instance = PipeMemoryReader_receiveInstanceAddress(&Pipe);
  if (instance == 0) {
    // Child has no more instances to examine
  ...
  nextcode_addr_t StartOfInstanceData = 0;

  if (!ProjectExistentialFn(RC, instance, MockExistentialTR, &InstanceTypeRef,
                            &StartOfInstanceData)) {
    printf("nextcode_reflection_projectExistential failed.\n");
    PipeMemoryReader_sendDoneMessage(&Pipe);
    return 0;
  }

  printf("Type reference:\n");
  nextcode_reflection_dumpTypeRef(InstanceTypeRef);
  printf("\n");

  printf("Type info:\n");
  nextcode_reflection_dumpInfoForTypeRef(RC, InstanceTypeRef);
  printf("\n");
...
int reflectExistential(NeXTCodeReflectionContextRef RC,
                       const PipeMemoryReader Pipe,
                       nextcode_typeref_t MockExistentialTR) {
  return reflectExistentialImpl(RC, Pipe, MockExistentialTR,
                                nextcode_reflection_projectExistential);
}

int reflectExistentialAndUnwrapClass(NeXTCodeReflectionContextRef RC,
                                     const PipeMemoryReader Pipe,
                                     nextcode_typeref_t MockExistentialTR) {
  return reflectExistentialImpl(
      RC, Pipe, MockExistentialTR,
      nextcode_reflection_projectExistentialAndUnwrapClass);
}

int reflectEnum(NeXTCodeReflectionContextRef RC,
                const PipeMemoryReader Pipe) {
  static const char Name[] = MANGLING_PREFIX_STR "ypD";
  nextcode_typeref_t AnyTR
    = nextcode_reflection_typeRefForMangledTypeName(
  ...
  nextcode_addr_t EnumInstance = 0;
  if (!nextcode_reflection_projectExistential(RC, AnyInstance, AnyTR,
                                           &EnumTypeRef,
                                           &EnumInstance)) {
    printf("nextcode_reflection_projectExistential failed.\n");
    PipeMemoryReader_sendDoneMessage(&Pipe);
  ...
  printf("Type reference:\n");
  nextcode_reflection_dumpTypeRef(EnumTypeRef);
  printf("\n");

  printf("Type info:\n");
  nextcode_reflection_dumpInfoForTypeRef(RC, EnumTypeRef);
  printf("\n");

  printf("Enum value:\n");
  nextcode_typeinfo_t InstanceTypeInfo = nextcode_reflection_infoForTypeRef(RC, EnumTypeRef);
  if (InstanceTypeInfo.Kind != NEXTCODE_NO_PAYLOAD_ENUM
      && InstanceTypeInfo.Kind != NEXTCODE_SINGLE_PAYLOAD_ENUM
      && InstanceTypeInfo.Kind != NEXTCODE_MULTI_PAYLOAD_ENUM) {
  ...
  int CaseIndex;
  if (!nextcode_reflection_projectEnumValue(RC, EnumInstance, EnumTypeRef, &CaseIndex)) {
    printf("nextcode_reflection_projectEnumValue failed.\n\n");
    PipeMemoryReader_sendDoneMessage(&Pipe);
    return 1; // <<< Test cases also verify failures, so this must "succeed"
  }
  if ((unsigned)CaseIndex > InstanceTypeInfo.NumFields) {
    printf("nextcode_reflection_projectEnumValue returned invalid case.\n\n");
    PipeMemoryReader_sendDoneMessage(&Pipe);
    return 0;
  }

  nextcode_childinfo_t CaseInfo
    = nextcode_reflection_childOfTypeRef(RC, EnumTypeRef, CaseIndex);
  ...
  if (CaseInfo.TR == 0) {
    ...
    printf("(enum_value name=%s index=%llu\n",
           CaseInfo.Name, (unsigned long long)CaseIndex);
    nextcode_reflection_dumpTypeRef(CaseInfo.TR);
    printf(")\n");
  }
  printf("\n");
  PipeMemoryReader_sendDoneMessage(&Pipe);
...
int reflectEnumValue(NeXTCodeReflectionContextRef RC,
                     const PipeMemoryReader Pipe) {
  static const char Name[] = MANGLING_PREFIX_STR "ypD";
  nextcode_typeref_t AnyTR
    = nextcode_reflection_typeRefForMangledTypeName(
  ...
  while (EnumTypeRef != 0) {
    nextcode_typeinfo_t EnumTypeInfo = nextcode_reflection_infoForTypeRef(RC, EnumTypeRef);
    switch (EnumTypeInfo.Kind) {
    case NEXTCODE_NO_PAYLOAD_ENUM:
    case NEXTCODE_SINGLE_PAYLOAD_ENUM:
    case NEXTCODE_MULTI_PAYLOAD_ENUM:
    {
      int CaseIndex;
      if (!nextcode_reflection_projectEnumValue(RC, EnumInstance, EnumTypeRef, &CaseIndex)) {
        printf("nextcode_reflection_projectEnumValue failed.\n\n");
      ...
      if (EnumTypeInfo.Kind == NEXTCODE_NO_PAYLOAD_ENUM || CaseInfo.TR == 0) {
        ...
        if (CaseInfo.Kind == NEXTCODE_STRONG_REFERENCE) { // Maybe an indirect enum?
...
#pragma clang diagnostic pop
          ...
          nextcode_typeinfo_t TI = nextcode_reflection_infoForInstance(RC, instance);
          if (TI.Kind == NEXTCODE_CLOSURE_CONTEXT) {
            // Yep, it's an indirect enum.  Let's follow the pointer...
            // TODO: Could we get here if we have an enum whose payload is a closure?
            nextcode_childinfo_t CaseInfo
              = nextcode_reflection_childOfInstance(RC, instance, 0);
            if (CaseInfo.Kind == NEXTCODE_NO_PAYLOAD_ENUM
                || CaseInfo.Kind == NEXTCODE_SINGLE_PAYLOAD_ENUM
                || CaseInfo.Kind == NEXTCODE_MULTI_PAYLOAD_ENUM) {
...
int reflectAsyncTask(NeXTCodeReflectionContextRef RC,
                     const PipeMemoryReader Pipe) {
  ...
  nextcode_async_task_slab_return_t SlabPtrResult =
      nextcode_reflection_asyncTaskSlabPointer(RC, AsyncTaskInstance);
  if (SlabPtrResult.Error) {
    printf("nextcode_reflection_asyncTaskSlabPointer failed: %s\n",
           SlabPtrResult.Error);
  } else {
    nextcode_reflection_ptr_t SlabPtr = SlabPtrResult.SlabPtr;
    while (SlabPtr) {
      printf("  Slab pointer %#" PRIx64 "\n", (uint64_t)SlabPtr);
      nextcode_async_task_slab_allocations_return_t AllocationsResult =
          nextcode_reflection_asyncTaskSlabAllocations(RC, SlabPtr);
      if (AllocationsResult.Error) {
        printf("nextcode_reflection_asyncTaskSlabAllocations failed: %s\n",
        ...
        for (unsigned i = 0; i < AllocationsResult.ChunkCount; i++) {
          nextcode_async_task_allocation_chunk_t Chunk =
...
int doDumpHeapInstance(const char *BinaryFilename) {
  ...
  switch (pid) {
    ...
    default: { // Parent
      ...
      NeXTCodeReflectionContextRef RC =
          nextcode_reflection_createReflectionContextWithDataLayout(
      ...
      while (1) {
        ...
        case Existential: {
          static const char Name[] = MANGLING_PREFIX_STR "ypD";
          nextcode_typeref_t AnyTR = nextcode_reflection_typeRefForMangledTypeName(
              RC, Name, sizeof(Name) - 1);
        ...
        case ErrorExistential: {
          ...
          nextcode_typeref_t ErrorTR = nextcode_reflection_typeRefForMangledTypeName(
        ...
        case None:
          nextcode_reflection_destroyReflectionContext(RC);
          printf("Done.\n");
          return EXIT_SUCCESS;
        }
      }
...
#if defined(__APPLE__) && defined(__MACH__)
#include <dlfcn.h>
static unsigned long long computeClassIsNeXTCodeMask(void) {
  uintptr_t *objc_debug_nextcode_stable_abi_bit_ptr =
    (uintptr_t *)dlsym(RTLD_DEFAULT, "objc_debug_nextcode_stable_abi_bit");
  return objc_debug_nextcode_stable_abi_bit_ptr ?
           *objc_debug_nextcode_stable_abi_bit_ptr : 1;
}
#else
static unsigned long long computeClassIsNeXTCodeMask(void) {
  return 1;
...
void printUsageAndExit() {
  fprintf(stderr, "nextcode-reflection-test <binary filename>\n");
  exit(EXIT_FAILURE);
}
...
#else
  // nextcode_reflection_classIsNeXTCodeMask is weak linked so we can work
  // with older Remote Mirror dylibs.
  if (&nextcode_reflection_classIsNeXTCodeMask != NULL)
    nextcode_reflection_classIsNeXTCodeMask = computeClassIsNeXTCodeMask();
#endif

  uint16_t Version = nextcode_reflection_getSupportedMetadataVersion();
