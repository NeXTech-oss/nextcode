...
#if defined(_POSIX_THREADS) && !defined(NEXTCODE_STDLIB_SINGLE_THREADED_RUNTIME)

// Notes: nextcode::fatalError is not shared between libnextcodeCore and libnextcode_Concurrency
// and libnextcode_Concurrency uses nextcode_Concurrency_fatalError instead.
/* #ifndef NEXTCODE_FATAL_ERROR */
/* #define NEXTCODE_FATAL_ERROR nextcode::fatalError */
/* #endif */
...
#define reportError(PThreadFunction)                                           \
  do {                                                                         \
    ...
    if (errorcode != 0) {                                                      \
      NEXTCODE_FATAL_ERROR(/* flags = */ 0, "'%s' failed with error '%s'(%d)\n",  \
