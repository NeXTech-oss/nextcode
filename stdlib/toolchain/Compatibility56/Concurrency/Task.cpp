...
FutureFragment::Status AsyncTask::waitFuture(AsyncTask *waitingTask,
                                             ...
                                             OpaqueValue *result) {
  NEXTCODE_TASK_DEBUG_LOG("compat 56 task task %p", this);
  ...
  while (true) {
    ...
    if (waitingStatus.getStoredPriority() > escalatedPriority) {
      nextcode_task_escalateBackdeploy56(this, waitingStatus.getStoredPriority());
...
void NEXTCODE_CC(nextcodeasync) nextcode::nextcode56override_nextcode_task_future_wait(
                                            OpaqueValue *result,
...
//===--- nextcode_task_future_wait_throwing ----------------------------------===//
...
void NEXTCODE_CC(nextcodeasync) nextcode::nextcode56override_nextcode_task_future_wait_throwing(
                                            OpaqueValue *result,
                                            NEXTCODE_ASYNC_CONTEXT AsyncContext *callerContext,
                                            AsyncTask *task,
...
nextcode::nextcode56override_nextcode_task_create_common(
