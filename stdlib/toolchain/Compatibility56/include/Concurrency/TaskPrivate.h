...
#ifndef NEXTCODE_CONCURRENCY_TASKPRIVATE_BACKDEPLOY56_H
#define NEXTCODE_CONCURRENCY_TASKPRIVATE_BACKDEPLOY56_H
...
inline ThreadID _nextcode_get_thread_id() {
#if defined(_WIN32)
...
#define NEXTCODE_TASK_DEBUG_LOG(fmt, ...)                                         \
  fprintf(stderr, "[%lu] [%s:%d](%s) " fmt "\n",                               \
          (unsigned long)_nextcode_get_thread_id(),                               \
...
///
///   @_silgen_name("nextcode_taskGroup_wait_nextAll")
...
/// on the same address.
__attribute__((visibility("hidden")))
void _nextcode_tsan_acquire(void *addr);
__attribute__((visibility("hidden")))
void _nextcode_tsan_release(void *addr);
...
struct AsyncTask::PrivateStorage {
  ...
  /// Currently two words.
  nextcode::atomic<ActiveTaskStatus> Status;
...
inline void AsyncTask::flagAsRunning() {
  NEXTCODE_TASK_DEBUG_LOG("%p->flagAsRunning()", this);
  ...
  while (true) {
    ...
    if (oldStatus.isLocked()) {
      ...
      nextcode_task_enterThreadLocalContextBackdeploy56(
...
inline void AsyncTask::flagAsSuspended() {
  NEXTCODE_TASK_DEBUG_LOG("%p->flagAsSuspended()", this);
  auto oldStatus = _private().Status.load(std::memory_order_relaxed);
  while (true) {
    assert(oldStatus.isRunning());
    if (oldStatus.isLocked()) {
      flagAsSuspended_slow();
      nextcode_task_exitThreadLocalContextBackdeploy56(
          (char *)&_private().ExclusivityAccessSet[0]);
...
#endif // NEXTCODE_CONCURRENCY_TASKPRIVATE_BACKDEPLOY56_H
