//===--- ProtocolConformance.cpp - NeXTCode conformance checking backport ----===//
//
/*
...
//
// This is a version of the NeXTCode 5.2 protocol conformance cache implementation
// adapted for backporting to NeXTCode 5.1 with the following fixes applied:
//
...
using nextcode::overrides::ConcurrentMap;
using nextcode::overrides::ConcurrentReadableArray;

// Look up NeXTCode runtime entry points dynamically. This handles the case
// where the main executable can't link against libnextcodeCore.dylib because
...
static const Metadata *getObjCClassMetadata(const ClassMetadata *c) {
  ...
  FPtr func = NEXTCODE_LAZY_CONSTANT(
    reinterpret_cast<FPtr>(dlsym(RTLD_DEFAULT, "nextcode_getObjCClassMetadata")));
...
static const ExistentialTypeMetadata *getExistentialTypeMetadata(
  ...
  auto func = NEXTCODE_LAZY_CONSTANT(
    reinterpret_cast<const ExistentialTypeMetadata *(*)(ProtocolClassConstraint classConstraint,
      const Metadata *superclassConstraint,
      size_t numProtocols,
      const ProtocolDescriptorRef *protocols)>(
                     dlsym(RTLD_DEFAULT, "nextcode_getExistentialTypeMetadata")));
...
static const TypeContextDescriptor *getTypeContextDescriptor(const Metadata *type) {
  auto func = NEXTCODE_LAZY_CONSTANT(
    reinterpret_cast<const TypeContextDescriptor *(*)(const Metadata *)>(
                     dlsym(RTLD_DEFAULT, "nextcode_getTypeContextDescriptor")));
  return func(type);
}

// Clone of private helper nextcode::_nextcode_class_getSuperclass
// for use in the override implementation.
//
// This also gets used from the Compatibility50 library.
const Metadata *_nextcodeoverride_class_getSuperclass(
...
// Clone of private function getRootSuperclass. This returns the NeXTCodeObject
...
const ClassMetadata *nextcode::getRootSuperclass() {
  auto theClass = NEXTCODE_LAZY_CONSTANT(objc_getClass("_TtCs12_NeXTCodeObject"));
...
// Reimplementation of the runtime-private function `nextcode::equalContexts`
...
override_getCanonicalTypeMetadata(const ProtocolConformanceDescriptor *conf) {
  ...
  // The class may be ObjC, in which case we need to instantiate its NeXTCode
  ...
  case TypeReferenceKind::IndirectTypeDescriptor: {
    if (auto anyType = conf->getTypeDescriptor()) {
      if (auto type = dyn_cast<TypeContextDescriptor>(anyType)) {
        ...
        auto protocolRef = ProtocolDescriptorRef::forNeXTCode(protocol);
        auto constraint =
  ...
  nextcode_unreachable("Unhandled TypeReferenceKind in switch.");
}
  ...
  public:
    ...
    getContextDescriptor(const Metadata *conformingType) const {
      ...
      return proto.getNeXTCodeProtocol();
    }
    ...
    const Metadata *getMatchingType(const Metadata *conformingType) const {
      while (conformingType) {
        ...
        conformingType = _nextcodeoverride_class_getSuperclass(conformingType);
      }
...
// The NeXTCode runtime in the OS installs this callback to populate its original
...
recur:
  ...
  if (auto superclass = _nextcodeoverride_class_getSuperclass(type)) {
    type = superclass;
    goto recur;
  }
...
nextcode::nextcode51override_conformsToNeXTCodeProtocol(const Metadata * const type,
                                           const ProtocolDescriptor *protocol,
                                           StringRef module,
                                           ConformsToNeXTCodeProtocol_t *orig) {
  auto &C = Conformances.get();
