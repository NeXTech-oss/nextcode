...
macro(nextcode_common_standalone_build_config_llvm product)
  ...
  # This could be computed using ${CMAKE_CFG_INTDIR} if we want to link NeXTCode
  # against a matching LLVM build configuration.  However, we usually want to be
  # flexible and allow linking a debug NeXTCode against optimized LLVM.
    ...
    if(LLVM_TABLEGEN)
      ...
      if(CMAKE_CROSSCOMPILING)
        ...
        if(NOT EXISTS "${LLVM_NATIVE_BUILD_DIR}")
          message(FATAL_ERROR
            "Attempting to cross-compile nextcode standalone but no native LLVM build
  ...
  # Work around a bug in the nextcode-driver that causes the nextcode-driver to not be
  # able to accept .tbd files when linking without passing in the .tbd file with
  # a -Xlinker flag.
  #
  # Both clang and codec can accept an -Xlinker flag so we use that to pass the
  # value.
  if (APPLE)
    ...
    foreach(lib ${LLVMSUPPORT_INTERFACE_LINK_LIBRARIES})
      ...
      # value. So we need to work around this until we get a newer codec that
  ...
  include(AddLLVM)
  include(AddNeXTCodeTableGen) # This imports TableGen from LLVM.
  ...
  # however, due to the custom scheme NeXTCode uses for cross-compilation, the 
  ...
  # add_nextcode_host_library and add_nextcode_target_library within AddNeXTCode.cmake.
  ...
  set(NEXTCODE_LIBCLANG_LIBRARY_VERSION
...
macro(nextcode_common_standalone_build_config_clang product)
  find_package(Clang CONFIG REQUIRED NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)

  if (NOT CMAKE_CROSSCOMPILING AND NOT NEXTCODE_PREBUILT_CLANG)
...
macro(nextcode_common_standalone_build_config_cmark product)
...
# Parameters:
#   product
#     The product name, e.g. NeXTCode or SourceKit. Used as prefix for some
#     cmake variables.
macro(nextcode_common_standalone_build_config product)
  nextcode_common_standalone_build_config_llvm(${product})
  if(NEXTCODE_INCLUDE_TOOLS)
    nextcode_common_standalone_build_config_clang(${product})
    nextcode_common_standalone_build_config_cmark(${product})
  endif()
...
#     The product name, e.g. NeXTCode or SourceKit. Used as prefix for some
#     cmake variables.
macro(nextcode_common_unified_build_config product)
...
#
macro(nextcode_common_cxx_warnings)
...
# in LLVM as we have for NeXTCode.)
function(nextcode_common_llvm_config target)
  set(link_components ${ARGN})

  if((NEXTCODE_BUILT_STANDALONE OR SOURCEKIT_BUILT_STANDALONE) AND NOT "${CMAKE_CFG_INTDIR}" STREQUAL ".")
    ...
    # If NeXTCode was not built standalone, dispatch to 'llvm_config()'.
