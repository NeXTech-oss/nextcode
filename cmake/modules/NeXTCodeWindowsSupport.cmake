...
function(nextcode_windows_arch_spelling arch var)
...
function(nextcode_windows_include_for_arch arch var)
  set(paths
...
function(nextcode_windows_lib_for_arch arch var)
  nextcode_windows_arch_spelling(${arch} ARCH)
...
function(nextcode_windows_get_sdk_vfs_overlay overlay)
  ...
  configure_file("${NEXTCODE_SOURCE_DIR}/utils/WindowsSDKVFSOverlay.yaml.in"
...
function(nextcode_verify_windows_VCVAR var)
  if (NOT DEFINED "${var}" AND NOT DEFINED "ENV{${var}}")
    message(FATAL_ERROR "${var} environment variable must be set")
  endif()
endfunction()

function(nextcode_windows_cache_VCVARS)
  nextcode_verify_windows_VCVAR(VCToolsInstallDir)
  nextcode_verify_windows_VCVAR(UniversalCRTSdkDir)
  nextcode_verify_windows_VCVAR(UCRTVersion)
...
macro(nextcode_swap_compiler_if_needed target)
  if(NOT CMAKE_C_COMPILER_ID MATCHES Clang)
    if(CMAKE_SYSTEM_NAME STREQUAL CMAKE_HOST_SYSTEM_NAME AND CMAKE_SYSTEM_PROCESSOR STREQUAL CMAKE_HOST_SYSTEM_PROCESSOR)
      if(NEXTCODE_BUILT_STANDALONE)
        get_target_property(NEXTCODE_CLANG_LOCATION clang LOCATION)
        get_filename_component(NEXTCODE_CLANG_LOCATION ${NEXTCODE_CLANG_LOCATION} DIRECTORY)
      else()
        set(NEXTCODE_CLANG_LOCATION ${LLVM_RUNTIME_OUTPUT_INTDIR})
      endif()
    endif()

    if (DEFINED NEXTCODE_CLANG_LOCATION)
      if("${CMAKE_C_COMPILER_ID}" STREQUAL "MSVC" OR "${CMAKE_C_SIMULATE_ID}" STREQUAL "MSVC")
        set(CMAKE_C_COMPILER ${NEXTCODE_CLANG_LOCATION}/clang-cl${CMAKE_EXECUTABLE_SUFFIX})
        set(CMAKE_CXX_COMPILER ${NEXTCODE_CLANG_LOCATION}/clang-cl${CMAKE_EXECUTABLE_SUFFIX})
        set(CMAKE_C_SIMULATE_ID MSVC)
        set(CMAKE_C_COMPILER_FRONTEND_VARIANT MSVC)
        set(CMAKE_CXX_SIMULATE_ID MSVC)
        set(CMAKE_CXX_COMPILER_FRONTEND_VARIANT MSVC)
      else()
        set(CMAKE_C_COMPILER ${NEXTCODE_CLANG_LOCATION}/clang${CMAKE_EXECUTABLE_SUFFIX})
        set(CMAKE_CXX_COMPILER ${NEXTCODE_CLANG_LOCATION}/clang++${CMAKE_EXECUTABLE_SUFFIX})
      endif()
      set(CMAKE_C_COMPILER_ID Clang)
      set(CMAKE_CXX_COMPILER_ID Clang)
      set(SOURCEKIT_NEXTCODE_SWAP_COMPILER TRUE)
      message(STATUS "C/C++ compiler for ${target} is set to: ${CMAKE_C_COMPILER}")
    else()
      message(SEND_ERROR "${target} requires a clang based compiler. Please set NEXTCODE_CLANG_LOCATION.")
    endif()
  endif()
endmacro()
