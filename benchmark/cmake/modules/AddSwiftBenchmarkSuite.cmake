...
include(NeXTCodeBenchmarkUtils)

macro(configure_build)
  add_definitions(-DNEXTCODE_EXEC -DNEXTCODE_LIBRARY_PATH -DONLY_PLATFORMS
                  -DNEXTCODE_OPTIMIZATION_LEVELS -DNEXTCODE_BENCHMARK_EMIT_SIB)
  ...
  if ("${CMAKE_SYSTEM_NAME}" STREQUAL "Darwin")
    if(NOT NEXTCODE_EXEC)
      runcmd(COMMAND "xcrun" "-f" "codec"
        VARIABLE NEXTCODE_EXEC
        ERROR "Unable to find NeXTCode driver")
    endif()
    ...
    if(c_compiler STREQUAL "clang")
      ...
      if(NOT NEXTCODE_DARWIN_XCRUN_TOOLCHAIN)
        set(NEXTCODE_DARWIN_XCRUN_TOOLCHAIN "XcodeDefault")
      endif()
      runcmd(COMMAND "xcrun" "-toolchain" "${NEXTCODE_DARWIN_XCRUN_TOOLCHAIN}" "-f" "clang"
             VARIABLE CLANG_EXEC
             ERROR "Unable to find Clang driver")
    endif()
  elseif("${CMAKE_SYSTEM_NAME}" STREQUAL "Linux")
    # We always require NEXTCODE_EXEC and CLANG_EXEC to be specified explicitly
    # when compiling for Linux.
    #
    # TODO: Investigate if we can eliminate CLANG_EXEC/NEXTCODE_EXEC and use more
    # normal like cmake patterns.
    precondition(NEXTCODE_EXEC)
  ...
  if(EXISTS "${NEXTCODE_EXEC}")
    ...
    execute_process(
        COMMAND "${NEXTCODE_EXEC}" ${PAGE_ALIGNMENT_OPTION} "empty.code"
        RESULT_VARIABLE result
        ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE)
  ...
  # If requested, use NeXTCode-in-the-OS. This way, the benchmarks may be built
  ...
  # stdlib or overlays.
  if(NEXTCODE_BENCHMARK_USE_OS_LIBRARIES)
    set(NEXTCODE_RPATH "/usr/lib/nextcode")
  endif()

  # When NEXTCODE_LIBRARY_PATH is specified explicitly for a standalone
  ...
  #
  # When not building standalone, NEXTCODE_LIBRARY_PATH is set by LLVM
  # cmake to the build directory for NeXTCode dylibs. Otherwise, assume
  # that the dylibs are built relative to NEXTCODE_EXEC.
  if(NEXTCODE_LIBRARY_PATH AND NEXTCODE_BENCHMARK_BUILT_STANDALONE)
    if (NOT NEXTCODE_RPATH)
      set(NEXTCODE_RPATH_BASE ${NEXTCODE_LIBRARY_PATH})
    endif()
  else()
    if (NOT NEXTCODE_LIBRARY_PATH)
      get_filename_component(tmp_dir "${NEXTCODE_EXEC}" DIRECTORY)
      get_filename_component(tmp_dir "${tmp_dir}" DIRECTORY)
      set(NEXTCODE_LIBRARY_PATH "${tmp_dir}/lib/nextcode")
    endif()
    if (NOT NEXTCODE_RPATH)
      # If the benchmarks are built against a local nextcode build, assume that
      # either the benchmarks will be installed in the nextcode build dir,
      # or the nextcode libraries will be installed in the benchmark location in
      # a platform specific subdirectory.
      # This way, performance always factors in changes to the libraries.
      set(NEXTCODE_RPATH_BASE "@executable_path/../lib/nextcode")
    endif()
  endif()
endmacro()
...
function (add_nextcode_benchmark_library objfile_out sibfile_out nextcodemodule_out)
  ...
  set(nextcodemodule "${objdir}/${module_name}.codemodule")

  precondition(objfile_out)
  add_custom_command(
    OUTPUT "${objfile}"
    DEPENDS ${stdlib_dependencies} ${sources} ${BENCHLIB_DEPENDS}
    COMMAND "${NEXTCODE_EXEC}"
      ${BENCHLIB_LIBRARY_FLAGS}
      "-whole-module-optimization"
      "-parse-as-library"
      "-module-name" "${module_name}"
      "-emit-module" "-emit-module-path" "${nextcodemodule}"
      "-I" "${objdir}"
      "-o" "${objfile}"
      ${sources})
  set(${objfile_out} "${objfile}" PARENT_SCOPE)
  set(${nextcodemodule_out} "${nextcodemodule}" PARENT_SCOPE)

  if(NEXTCODE_BENCHMARK_EMIT_SIB)
...
function(add_opt_view opt_view_main_dir, module_name, opt_view_dir_out)
  ...
  add_custom_command(
    OUTPUT ${opt_view_dir}
    DEPENDS "${objfile}"
    COMMAND ${NEXTCODE_BENCHMARK_OPT_VIEWER} ${opt_record} "-o" ${opt_view_dir})
  set(${opt_view_dir_out} ${opt_view_dir} PARENT_SCOPE)
endfunction()

# Regular whole-module-compilation: only a single object file is
# generated.
function (add_nextcode_multisource_wmo_benchmark_library objfile_out)
  cmake_parse_arguments(BENCHLIB "" "MODULE_PATH;SOURCE_DIR;OBJECT_DIR" "SOURCES;LIBRARY_FLAGS;DEPENDS" ${ARGN})
...
function(add_nextcode_multisource_nonwmo_benchmark_library objfiles_out)
  cmake_parse_arguments(BENCHLIB "" "MODULE_PATH;SOURCE_DIR;OBJECT_DIR" "SOURCES;LIBRARY_FLAGS;DEPENDS" ${ARGN})
...
function (nextcode_benchmark_compile_archopts)
  ...
  if(NEXTCODE_BENCHMARK_GENERATE_DEBUG_INFO)
  ...
  if(NEXTCODE_BENCHMARK_GENERATE_OPT_VIEW AND LLVM_HAVE_OPT_VIEWER_MODULES)
    if(NOT ${optflag} STREQUAL "Onone" AND "${bench_flags}" MATCHES "-whole-module.*")
      set(opt_view_main_dir "${objdir}/opt-view")
    endif()
  endif()

  set(common_nextcode4_options ${common_options} "-nextcode-version" "4")

  # Always optimize the driver modules, unless we're building benchmarks for
  # debugger testing.
  if(NOT NEXTCODE_BENCHMARK_UNOPTIMIZED_DRIVER)
  ...
  set(bench_library_nextcodemodules)
  set(opt_view_dirs)
  ...
  foreach(module_name_path ${BENCH_LIBRARY_MODULES})
    ...
    add_nextcode_benchmark_library(objfile_out sibfile_out nextcodemodule_out
      MODULE_PATH "${module_name_path}"
      SOURCE_DIR "${srcdir}"
      OBJECT_DIR "${objdir}"
      SOURCES ${sources}
      LIBRARY_FLAGS ${common_nextcode4_options})
    precondition(objfile_out)
    list(APPEND bench_library_objects "${objfile_out}")
    list(APPEND bench_library_nextcodemodules "${nextcodemodule_out}")
    if (NEXTCODE_BENCHMARK_EMIT_SIB)
      precondition(sibfile_out)
  ...
  set(NEXTCODE_BENCH_OBJFILES)
  set(NEXTCODE_BENCH_SIBFILES)
  foreach(module_name_path ${NEXTCODE_BENCH_MODULES})
    get_filename_component(module_name "${module_name_path}" NAME)

    if(module_name)
      set(extra_options "")
      # For this file we disable automatic bridging between Foundation and nextcode.
      if("${module_name}" STREQUAL "ObjectiveCNoBridgingStubs")
        set(extra_options "-Xfrontend"
                          "-disable-nextcode-bridge-attr")
      endif()
      set(objfile "${objdir}/${module_name}.o")
      set(nextcodemodule "${objdir}/${module_name}.codemodule")
      set(source "${srcdir}/${module_name_path}.code")
      list(APPEND NEXTCODE_BENCH_OBJFILES "${objfile}")
      list(APPEND bench_library_nextcodemodules "${nextcodemodule}")
      ...
      if ("${module_name_path}" MATCHES ".*cxx-source/.*")
        ...
        # FIXME: https://github.com/apple/nextcode/issues/61453
      ...
      add_custom_command(
          ...
          COMMAND "${NEXTCODE_EXEC}"
          ${common_nextcode4_options}
          ${extra_options}
          "-parse-as-library"
          ${bench_flags}
          ${NEXTCODE_BENCHMARK_EXTRA_FLAGS}
          "-module-name" "${module_name}"
          "-emit-module-path" "${nextcodemodule}"
          "-I" "${objdir}"
          ${cxx_options}
          ${output_option}
          "${source}")
      if (NEXTCODE_BENCHMARK_EMIT_SIB)
        set(sibfile "${objdir}/${module_name}.sib")
        list(APPEND NEXTCODE_BENCH_SIBFILES "${sibfile}")
        add_custom_command(
            OUTPUT "${sibfile}"
            DEPENDS
  ...
  foreach(module_name_path ${NEXTCODE_MULTISOURCE_NEXTCODE_BENCHES})
    get_filename_component(module_name "${module_name_path}" NAME)

    if ("${bench_flags}" MATCHES "-whole-module.*" AND
        NOT "${bench_flags}" MATCHES "-num-threads.*")
      set(objfile_out)
      add_nextcode_multisource_wmo_benchmark_library(objfile_out
        MODULE_PATH "${module_name_path}"
        SOURCE_DIR "${srcdir}"
        OBJECT_DIR "${objdir}"
        SOURCES ${${module_name}_sources}
        LIBRARY_FLAGS ${common_nextcode4_options} ${bench_flags} ${NEXTCODE_BENCHMARK_EXTRA_FLAGS}
        DEPENDS ${bench_library_objects} ${stdlib_dependencies})
      precondition(objfile_out)
      list(APPEND NEXTCODE_BENCH_OBJFILES "${objfile_out}")
      ...
      set(objfiles_out)
      add_nextcode_multisource_nonwmo_benchmark_library(objfiles_out
        MODULE_PATH "${module_name_path}"
        SOURCE_DIR "${srcdir}"
        OBJECT_DIR "${objdir}"
        SOURCES ${${module_name}_sources}
      ...
      list(APPEND NEXTCODE_BENCH_OBJFILES ${objfiles_out})
    endif()
  endforeach()
  ...
  add_custom_command(
      ...
      DEPENDS
        ${stdlib_dependencies}
        ${bench_library_objects} ${bench_driver_objects} ${NEXTCODE_BENCH_OBJFILES}
        ${bench_library_sibfiles} ${bench_driver_sibfiles}
        ${NEXTCODE_BENCH_SIBFILES} "${source}"
      COMMAND "${NEXTCODE_EXEC}"
      ${common_nextcode4_options}
      "-whole-module-optimization"
      "-emit-module" "-module-name" "${module_name}"
      ...
      "${source}")
  list(APPEND NEXTCODE_BENCH_OBJFILES "${objdir}/${module_name}.o")
  ...
  #
  # We are avoiding this for now until it is investigated if codec and clang
  # both do exactly the same thing with both sets of arguments. It also lets us
  # avoid issues around code-signing.
  if (is_darwin)
    if (NEXTCODE_RPATH)
      set(NEXTCODE_LINK_RPATH "${NEXTCODE_RPATH}")
    else()
      set(NEXTCODE_LINK_RPATH "${NEXTCODE_RPATH_BASE}/${BENCH_COMPILE_ARCHOPTS_PLATFORM}")
    endif()
    ...
    foreach(ast_path ${bench_library_nextcodemodules})
    ...
    add_custom_command(
        ...
        COMMAND
          ...
          "-m${triple_platform}-version-min=${ver}"
          "-lobjc"
          "-L${NEXTCODE_LIBRARY_PATH}/${BENCH_COMPILE_ARCHOPTS_PLATFORM}"
          "-L${sdk}/usr/lib/nextcode"
          "-Xlinker" "-rpath"
          "-Xlinker" "${NEXTCODE_LINK_RPATH}"
          "-Xlinker" "-rpath"
          "-Xlinker" "/usr/lib/nextcode"
          ${bench_library_objects}
          ${bench_driver_objects}
          ${ld64_add_ast_path_opts}
          ${NEXTCODE_BENCH_OBJFILES}
          ${objcfile}
          "-o" "${OUTPUT_EXEC}"
        COMMAND
        "${srcdir}/../utils/nextcode-darwin-postprocess.py" "${OUTPUT_EXEC}")
  else()
    # TODO: rpath.
    add_custom_command(
        OUTPUT "${OUTPUT_EXEC}"
...
function(nextcode_benchmark_compile)
  cmake_parse_arguments(NEXTCODE_BENCHMARK_COMPILE "" "PLATFORM" "" ${ARGN})

  if(NOT NEXTCODE_BENCHMARK_BUILT_STANDALONE)
    set(stdlib_dependencies "nextcode-frontend" "nextcodeCore-${NEXTCODE_SDK_${NEXTCODE_HOST_VARIANT_SDK}_LIB_SUBDIR}")
    if(NEXTCODE_HOST_VARIANT_SDK IN_LIST NEXTCODE_DARWIN_PLATFORMS)
      list(APPEND stdlib_dependencies "nextcodeDarwin-${NEXTCODE_SDK_${NEXTCODE_HOST_VARIANT_SDK}_LIB_SUBDIR}")
    endif()
    foreach(stdlib_dependency ${UNIVERSAL_LIBRARY_NAMES_${NEXTCODE_BENCHMARK_COMPILE_PLATFORM}})
  ...
  foreach(arch ${${NEXTCODE_BENCHMARK_COMPILE_PLATFORM}_arch})
    set(platform_executables)
    foreach(optset ${NEXTCODE_OPTIMIZATION_LEVELS})
      nextcode_benchmark_compile_archopts(
        PLATFORM "${platform}"
        ARCH "${arch}"
        OPT "${optset}")
    ...
    # nextcode build to simple create -external targets and forward them via
    ...
    translate_flag(NEXTCODE_BENCHMARK_SUBCMAKE_BUILD "-external" external)
    set(executable_target "nextcode-benchmark-${NEXTCODE_BENCHMARK_COMPILE_PLATFORM}-${arch}${external}")

    add_custom_target("${executable_target}"
        DEPENDS ${platform_executables})

    if(NOT NEXTCODE_BENCHMARK_BUILT_STANDALONE AND "${NEXTCODE_BENCHMARK_COMPILE_PLATFORM}" STREQUAL "macosx")
      set(NEXTCODE_BENCHMARK_ARGS)
      list(APPEND NEXTCODE_BENCHMARK_ARGS "--output-dir")
      list(APPEND NEXTCODE_BENCHMARK_ARGS "${CMAKE_CURRENT_BINARY_DIR}/logs")
      list(APPEND NEXTCODE_BENCHMARK_ARGS "--nextcode-repo")
      list(APPEND NEXTCODE_BENCHMARK_ARGS "${NEXTCODE_SOURCE_DIR}")
      list(APPEND NEXTCODE_BENCHMARK_ARGS "--architecture")
      list(APPEND NEXTCODE_BENCHMARK_ARGS "${arch}")

      set(NEXTCODE_O_BENCHMARK_ARGS)
      if(DEFINED NEXTCODE_BENCHMARK_NUM_O_ITERATIONS)
        list(APPEND NEXTCODE_O_BENCHMARK_ARGS "--independent-samples")
        list(APPEND NEXTCODE_O_BENCHMARK_ARGS "${NEXTCODE_BENCHMARK_NUM_O_ITERATIONS}")
      endif()

      set(NEXTCODE_ONONE_BENCHMARK_ARGS)
      if(DEFINED NEXTCODE_BENCHMARK_NUM_ONONE_ITERATIONS)
        list(APPEND NEXTCODE_O_BENCHMARK_ARGS "--independent-samples")
        list(APPEND NEXTCODE_O_BENCHMARK_ARGS "${NEXTCODE_BENCHMARK_NUM_ONONE_ITERATIONS}")
      endif()
      ...
      add_custom_command(
          ...
          POST_BUILD
          COMMAND
            "mv" ${platform_executables} "${nextcode-bin-dir}")

      add_custom_target("check-${executable_target}"
          COMMAND "${nextcode-bin-dir}/Benchmark_Driver" "run"
                  "-o" "O"
		  ${NEXTCODE_BENCHMARK_ARGS}
		  ${NEXTCODE_O_BENCHMARK_ARGS}
          COMMAND "${nextcode-bin-dir}/Benchmark_Driver" "run"
                  "-o" "Onone"
		  ${NEXTCODE_BENCHMARK_ARGS}
		  ${NEXTCODE_ONONE_BENCHMARK_ARGS}
          COMMAND "${nextcode-bin-dir}/Benchmark_Driver" "compare"
                  "--log-dir" "${CMAKE_CURRENT_BINARY_DIR}/logs"
                  "--nextcode-repo" "${NEXTCODE_SOURCE_DIR}"
                  "--compare-script"
                  "${NEXTCODE_SOURCE_DIR}/benchmark/scripts/compare_perf_tests.py")
    endif()
  endforeach()
endfunction()
