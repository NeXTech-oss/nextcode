//===------------ NeXTCodeCaching.cpp - NeXTCode Compiler -----------------------===//
//
/*
...
//===----------------------------------------------------------------------===//
//
// Implementation of the caching APIs in libNeXTCodeScan
//
//===----------------------------------------------------------------------===//
...
#include "nextcode/Frontend/DiagnosticHelper.h"
...
namespace {
/// Helper class to manage CAS/Caching from libNeXTCodeScan C APIs.
class NeXTCodeScanCAS {
public:
  llvm::cas::ObjectStore &getCAS() const { return *CAS; }
  llvm::cas::ActionCache &getCache() const { return *Cache; }

  // Construct NeXTCodeScanCAS.
  static llvm::Expected<NeXTCodeScanCAS *>
  createNeXTCodeScanCAS(llvm::StringRef Path);

  static llvm::Expected<NeXTCodeScanCAS *>
  createNeXTCodeScanCAS(clang::CASOptions &CASOpts);

private:
  NeXTCodeScanCAS(std::shared_ptr<llvm::cas::ObjectStore> CAS,
...
struct NeXTCodeCachedCompilationHandle {
  NeXTCodeCachedCompilationHandle(llvm::cas::ObjectRef Key,
                               llvm::cas::ObjectRef Output,
                               clang::cas::CompileJobCacheResult &&Result,
                               unsigned InputIndex, NeXTCodeScanCAS &CAS)
      : Key(Key), Output(Output), InputIndex(InputIndex), Result(Result),
        DB(CAS) {}
  NeXTCodeCachedCompilationHandle(llvm::cas::ObjectRef Key,
                               llvm::cas::ObjectRef Output,
                               nextcode::cas::CompileJobCacheResult &&Result,
                               unsigned InputIndex, NeXTCodeScanCAS &CAS)
      : Key(Key), Output(Output), InputIndex(InputIndex), Result(Result),
        DB(CAS) {}
  ...
  llvm::cas::ObjectRef Output;
  unsigned InputIndex;
  std::variant<nextcode::cas::CompileJobCacheResult,
               clang::cas::CompileJobCacheResult>
      Result;
  NeXTCodeScanCAS &DB;
};

struct NeXTCodeCachedOutputHandle {
  llvm::cas::ObjectRef Ref;
  nextcode::file_types::ID Kind;
  llvm::cas::ObjectStore &CAS;
  std::optional<llvm::cas::ObjectProxy> LoadedProxy;

  NeXTCodeCachedOutputHandle(llvm::cas::ObjectRef Ref, nextcode::file_types::ID Kind,
                          llvm::cas::ObjectStore &CAS)
      : Ref(Ref), Kind(Kind), CAS(CAS) {}
};

struct NeXTCodeScanReplayInstance {
  nextcode::CompilerInvocation Invocation;
  llvm::BumpPtrAllocator StringAlloc;
  llvm::SmallVector<const char *> Args;
};

struct NeXTCodeCachedReplayResult {
  llvm::SmallVector<char> outMsg;
  llvm::SmallVector<char> errMsg;
  llvm::raw_svector_ostream outOS;
  llvm::raw_svector_ostream errOS;

  NeXTCodeCachedReplayResult() : outOS(outMsg), errOS(errMsg) {}
};
} // namespace

DEFINE_SIMPLE_CONVERSION_FUNCTIONS(clang::CASOptions, nextcodescan_cas_options_t)
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(NeXTCodeScanCAS, nextcodescan_cas_t)
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(NeXTCodeCachedCompilationHandle,
                                   nextcodescan_cached_compilation_t)
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(NeXTCodeCachedOutputHandle,
                                   nextcodescan_cached_output_t)
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(NeXTCodeScanReplayInstance,
                                   nextcodescan_cache_replay_instance_t)
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(NeXTCodeCachedReplayResult,
                                   nextcodescan_cache_replay_result_t)

//=== CAS Functions ----------------------------------------------------------//

nextcodescan_cas_options_t nextcodescan_cas_options_create() {
  clang::CASOptions *CASOpts = new clang::CASOptions();
  return wrap(CASOpts);
}

void nextcodescan_cas_options_dispose(nextcodescan_cas_options_t options) {
  delete unwrap(options);
}

void nextcodescan_cas_options_set_ondisk_path(nextcodescan_cas_options_t options,
                                           const char *path) {
  unwrap(options)->CASPath = path;
}

void nextcodescan_cas_options_set_plugin_path(nextcodescan_cas_options_t options,
                                           const char *path) {
  unwrap(options)->PluginPath = path;
}

bool nextcodescan_cas_options_set_plugin_option(nextcodescan_cas_options_t options,
                                             const char *name,
                                             const char *value,
...
nextcodescan_cas_t
nextcodescan_cas_create_from_options(nextcodescan_cas_options_t options,
                                  nextcodescan_string_ref_t *error) {
  clang::CASOptions *opts = unwrap(options);
  auto cas = NeXTCodeScanCAS::createNeXTCodeScanCAS(*opts);
  if (!cas) {
    *error =
        nextcode::c_string_utils::create_clone(toString(cas.takeError()).c_str());
    return nullptr;
  }
  return wrap(*cas);
}

void nextcodescan_cas_dispose(nextcodescan_cas_t cas) { delete unwrap(cas); }

nextcodescan_string_ref_t nextcodescan_cas_store(nextcodescan_cas_t cas, uint8_t *data,
                                           unsigned size,
                                           nextcodescan_string_ref_t *error) {
  auto failure = [&](llvm::Error E) {
    *error =
        nextcode::c_string_utils::create_clone(toString(std::move(E)).c_str());
  ...
  *error = nextcode::c_string_utils::create_null();
  return nextcode::c_string_utils::create_clone(
      CAS.getID(*Result).toString().c_str());
}

int64_t nextcodescan_cas_get_ondisk_size(nextcodescan_cas_t cas,
                                      nextcodescan_string_ref_t *error) {
  auto &CAS = unwrap(cas)->getCAS();
...
bool
nextcodescan_cas_set_ondisk_size_limit(nextcodescan_cas_t cas, int64_t size_limit,
    nextcodescan_string_ref_t *error) {
  if (size_limit < 0) {
    *error = nextcode::c_string_utils::create_clone(
        "invalid size limit passing to nextcodescan_cas_set_ondisk_size_limit");
    return true;
  }
  auto &CAS = unwrap(cas)->getCAS();
...
bool nextcodescan_cas_prune_ondisk_data(nextcodescan_cas_t cas,
                                     nextcodescan_string_ref_t *error) {
  auto &CAS = unwrap(cas)->getCAS();
...
/// the parameter. Return nextcode-frontend arguments in an ArrayRef, which has the
/// first "-frontend" option dropped if needed.
static llvm::ArrayRef<const char *>
expandNeXTCodeInvocation(int argc, const char **argv, llvm::StringSaver &Saver,
                      llvm::SmallVectorImpl<const char *> &ArgsStorage) {
  ArgsStorage.reserve(argc);
  for (int i = 0; i < argc; ++i)
    ArgsStorage.push_back(Saver.save(argv[i]).data());
  nextcode::driver::ExpandResponseFilesWithRetry(Saver, ArgsStorage);
...
computeCacheKey(llvm::cas::ObjectStore &CAS, llvm::ArrayRef<const char *> Args,
  ...
  auto BaseKey = nextcode::createCompileJobBaseCacheKey(CAS, Args);
  if (!BaseKey)
    return BaseKey.takeError();

  // Parse the arguments to figure out the index for the input.
  nextcode::CompilerInvocation Invocation;
  nextcode::SourceManager SourceMgr;
  nextcode::DiagnosticEngine Diags(SourceMgr);
  ...
  std::string MainExecutablePath = llvm::sys::fs::getMainExecutable(
      "nextcode-frontend", (void *)nextcodescan_cache_replay_compilation);
  ...
  auto computeKey = [&](unsigned Index) -> llvm::Expected<std::string> {
    auto Key = nextcode::createCompileJobCacheKeyForOutput(CAS, *BaseKey, Index);
    if (!Key)
...
computeCacheKeyFromIndex(llvm::cas::ObjectStore &CAS,
  ...
  auto Key =
      nextcode::createCompileJobCacheKeyForOutput(CAS, *BaseKey, InputIndex);
  if (!Key)
    return Key.takeError();
  return CAS.getID(*Key).toString();
}
...
nextcodescan_cache_compute_key(nextcodescan_cas_t cas, int argc, const char **argv,
  ...
  auto Args = expandNeXTCodeInvocation(argc, argv, Saver, ArgsStorage);

  auto ID = computeCacheKey(unwrap(cas)->getCAS(), Args, input);
  if (!ID) {
    *error =
        nextcode::c_string_utils::create_clone(toString(ID.takeError()).c_str());
    return nextcode::c_string_utils::create_null();
  }
  *error = nextcode::c_string_utils::create_null();
  return nextcode::c_string_utils::create_clone(ID->c_str());
}
...
static nextcodescan_string_ref_t createNonOwningString(llvm::StringRef Str) {
  if (Str.empty())
    return nextcode::c_string_utils::create_null();

  nextcodescan_string_ref_t Result;
...
static llvm::Expected<NeXTCodeCachedCompilationHandle *>
createCachedCompilation(NeXTCodeScanCAS &CAS, const llvm::cas::CASID &ID,
  ...
  {
    nextcode::cas::CompileJobResultSchema Schema(CAS.getCAS());
    if (Schema.isRootNode(*Proxy)) {
      auto Result = Schema.load(Proxy->getRef());
      if (!Result)
        return Result.takeError();
      return new NeXTCodeCachedCompilationHandle(KeyProxy->getRef(), *Ref,
...
void nextcodescan_cache_query_async(
  ...
  auto notfound = [ctx, callback]() {
    return callback(ctx, nullptr, nextcode::c_string_utils::create_null());
  };
  auto success = [ctx, callback](nextcodescan_cached_compilation_t comp) {
    return callback(ctx, comp, nextcode::c_string_utils::create_null());
  };
...
unsigned nextcodescan_cached_compilation_get_num_outputs(
    nextcodescan_cached_compilation_t id) {
  auto *Comp = unwrap(id);
  if (auto *Result =
          std::get_if<nextcode::cas::CompileJobCacheResult>(&Comp->Result))
...
nextcodescan_cached_output_t
nextcodescan_cached_compilation_get_output(nextcodescan_cached_compilation_t id,
                                        unsigned idx) {
  auto *Comp = unwrap(id);
  if (auto *Result =
      std::get_if<nextcode::cas::CompileJobCacheResult>(&Comp->Result))
    return wrap(new NeXTCodeCachedOutputHandle(Result->getOutput(idx).Object,
                                            Result->getOutput(idx).Kind,
                                            Comp->DB.getCAS()));

  auto *Result = std::get_if<clang::cas::CompileJobCacheResult>(&Comp->Result);
  assert(Result && "unexpected variant");
  nextcode::file_types::ID Kind = nextcode::file_types::TY_INVALID;
  switch (Result->getOutput(idx).Kind) {
  case clang::cas::CompileJobCacheResult::OutputKind::MainOutput:
    Kind = nextcode::file_types::TY_ClangModuleFile;
    break;
  case clang::cas::CompileJobCacheResult::OutputKind::Dependencies:
    Kind = nextcode::file_types::TY_Dependencies;
    break;
  case clang::cas::CompileJobCacheResult::OutputKind::SerializedDiagnostics:
    Kind = nextcode::file_types::TY_CachedDiagnostics;
    break;
  }
  assert(Kind != nextcode::file_types::TY_INVALID && "Invalid kind");
  return wrap(new NeXTCodeCachedOutputHandle(Result->getOutput(idx).Object, Kind,
                                          Comp->DB.getCAS()));
}

bool nextcodescan_cached_compilation_is_uncacheable(
    nextcodescan_cached_compilation_t id) {
  // Currently, all compilations are cacheable.
  return false;
}

void nextcodescan_cached_compilation_dispose(nextcodescan_cached_compilation_t id) {
  delete unwrap(id);
}

bool nextcodescan_cached_output_load(nextcodescan_cached_output_t output,
                                  nextcodescan_string_ref_t *error) {
...
class AsyncObjectLoader
  ...
  void *Ctx;
  void (*Callback)(void *Ctx, bool, nextcodescan_string_ref_t);
  llvm::cas::ObjectStore &CAS;

  // The output handle to update after load if set.
  NeXTCodeCachedOutputHandle *OutputHandle;
...
public:
  AsyncObjectLoader(void *Ctx,
                    void (*Callback)(void *Ctx, bool, nextcodescan_string_ref_t),
                    llvm::cas::ObjectStore &CAS,
                    NeXTCodeCachedOutputHandle *Handle = nullptr)
  ...
  void visit(llvm::cas::ObjectRef Ref, bool IsRootNode) {
    ...
    CAS.getProxyAsync(
        Ref, [This, IsRootNode](
          ...
          NEXTCODE_DEFER { This->finishedNode(); };
          if (!Obj) {
  ...
  void finishedNode() {
    ...
    if (MissingNode)
      return Callback(Ctx, false, nextcode::c_string_utils::create_null());

    if (OutputHandle)
      OutputHandle->LoadedProxy = RootObj;
    return Callback(Ctx, true, nextcode::c_string_utils::create_null());
  }
...
void nextcodescan_cached_output_load_async(
    nextcodescan_cached_output_t output, void *ctx,
...
void nextcodescan_cache_download_cas_object_async(
    nextcodescan_cas_t cas, const char *id, void *ctx,
    void (*callback)(void *ctx, bool success, nextcodescan_string_ref_t error),
    nextcodescan_cache_cancellation_token_t *token) {
  if (token)
    *token = nullptr;
...
bool nextcodescan_cached_output_is_materialized(nextcodescan_cached_output_t output) {
  auto *Out = unwrap(output);
  // Already loaded.
  if (Out->LoadedProxy)
    return true;
...
nextcodescan_string_ref_t
nextcodescan_cached_output_get_casid(nextcodescan_cached_output_t output) {
  auto *Out = unwrap(output);
  auto ID = Out->CAS.getID(Out->Ref);
  return nextcode::c_string_utils::create_clone(ID.toString().c_str());
}

void nextcodescan_cached_compilation_make_global_async(
    nextcodescan_cached_compilation_t comp, void *ctx,
  ...
  auto success = [ctx, callback]() {
    return callback(ctx, nextcode::c_string_utils::create_null());
  };
...
nextcodescan_string_ref_t
nextcodescan_cached_output_get_name(nextcodescan_cached_output_t output) {
  auto *Out = unwrap(output);
  return nextcode::c_string_utils::create_clone(
      nextcode::file_types::getTypeName(Out->Kind).str().c_str());
}

nextcodescan_cache_replay_instance_t
nextcodescan_cache_replay_instance_create(int argc, const char **argv,
                                       nextcodescan_string_ref_t *error) {
  auto *Instance = new NeXTCodeScanReplayInstance();
  llvm::SmallVector<const char *> Compilation;
  llvm::StringSaver Saver(Instance->StringAlloc);
  auto Args = expandNeXTCodeInvocation(argc, argv, Saver, Instance->Args);

  // Capture the diagnostics when creating invocation.
  std::string err_msg;
  llvm::raw_string_ostream OS(err_msg);
  nextcode::PrintingDiagnosticConsumer Diags(OS);
  nextcode::SourceManager SrcMgr;
  nextcode::DiagnosticEngine DE(SrcMgr);
  ...
  if (Instance->Invocation.parseArgs(Args, DE, nullptr, {},
                                     MainExecutablePath)) {
    delete Instance;
    *error = nextcode::c_string_utils::create_clone(err_msg.c_str());
    return nullptr;
  }
  ...
  // thread-safe to be called in libNeXTCodeScan. The replay instance should not be
...
void nextcodescan_cache_replay_instance_dispose(
    nextcodescan_cache_replay_instance_t instance) {
...
static llvm::Error replayCompilation(NeXTCodeScanReplayInstance &Instance,
                                     llvm::cas::ObjectStore &CAS,
                                     NeXTCodeCachedCompilationHandle &Comp,
  ...
  nextcode::cas::CachedResultLoader Loader(CAS, Comp.Output);
...
nextcodescan_cache_replay_result_t
nextcodescan_cache_replay_compilation(nextcodescan_cache_replay_instance_t instance,
                                   nextcodescan_cached_compilation_t comp,
                                   nextcodescan_string_ref_t *error) {
  NeXTCodeScanReplayInstance &Instance = *unwrap(instance);
  NeXTCodeCachedCompilationHandle &Comp = *unwrap(comp);
  NeXTCodeCachedReplayResult *Result = new NeXTCodeCachedReplayResult();

  if (auto err = replayCompilation(Instance, Comp.DB.getCAS(), Comp,
                                   Result->outOS, Result->errOS)) {
    *error =
        nextcode::c_string_utils::create_clone(toString(std::move(err)).c_str());
    delete Result;
    return nullptr;
  }
  *error = nextcode::c_string_utils::create_null();
...
nextcodescan_string_ref_t nextcodescan_cache_replay_result_get_stdout(
    nextcodescan_cache_replay_result_t result) {
  return createNonOwningString(unwrap(result)->outOS.str());
}

nextcodescan_string_ref_t nextcodescan_cache_replay_result_get_stderr(
    nextcodescan_cache_replay_result_t result) {
  return createNonOwningString(unwrap(result)->errOS.str());
}

void nextcodescan_cached_output_dispose(nextcodescan_cached_output_t out) {
  delete unwrap(out);
}

void nextcodescan_cache_replay_result_dispose(
    nextcodescan_cache_replay_result_t result) {
  delete unwrap(result);
}

// FIXME: implement cancellation.
void nextcodescan_cache_action_cancel(nextcodescan_cache_cancellation_token_t) {}
void nextcodescan_cache_cancellation_token_dispose(
    nextcodescan_cache_cancellation_token_t) {}

llvm::Expected<NeXTCodeScanCAS *>
NeXTCodeScanCAS::createNeXTCodeScanCAS(llvm::StringRef Path) {
  clang::CASOptions Opts;
  Opts.CASPath = Path;

  return createNeXTCodeScanCAS(Opts);
}

llvm::Expected<NeXTCodeScanCAS *>
NeXTCodeScanCAS::createNeXTCodeScanCAS(clang::CASOptions &CASOpts) {
  auto DB = CASOpts.getOrCreateDatabases();
  if (!DB)
    return DB.takeError();

  return new NeXTCodeScanCAS(std::move(DB->first), std::move(DB->second));
}
