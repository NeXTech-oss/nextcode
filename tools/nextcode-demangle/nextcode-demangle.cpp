//===--- nextcode-demangle.cpp - NeXTCode Demangler app -------------------------===//
//
/*
...
using namespace nextcode::Demangle;
...
static llvm::cl::opt<bool> DisplayStdlibModule(
    ...
    llvm::cl::desc("Qualify types originating from the NeXTCode standard library"),
    llvm::cl::Hidden);
...
static void demangle(llvm::raw_ostream &os, llvm::StringRef name,
                     ...
                     const nextcode::Demangle::DemangleOptions &options) {
  bool hadLeadingUnderscore = false;
  if (name.starts_with("__")) {
    hadLeadingUnderscore = true;
    name = name.substr(1);
  }
  nextcode::Demangle::NodePointer pointer = DCtx.demangleSymbolAsNode(name);
  ...
  if (RemangleMode) {
    ...
    if (!pointer || !(name.starts_with(MANGLING_PREFIX_STR) ||
      ...
      auto mangling = nextcode::Demangle::mangleNode(pointer);
      ...
      unsigned prefixLen = nextcode::Demangle::getManglingPrefixLength(remangled);
      assert(prefixLen > 0);
      ...
      if (!name.starts_with(remangled.substr(0, prefixLen))) {
        unsigned namePrefixLen =
          nextcode::Demangle::getManglingPrefixLength(name);
    ...
    if (pointer) {
      auto mangling = nextcode::Demangle::mangleNodeOld(pointer);
      if (!mangling.isSuccess()) {
        llvm::errs() << "Error: (" << mangling.error().code << ":"
                     << mangling.error().line << ") unable to re-mangle "
                     << name << '\n';
  ...
  if (!TreeOnly) {
    ...
    std::string string = nextcode::Demangle::nodeToString(pointer, options);
    if (!CompactMode)
      os << name << " ---> ";

    if (Classify) {
      std::string Classifications;
      if (!nextcode::Demangle::isNeXTCodeSymbol(name))
        Classifications += 'N';
      ...
      if (pointer && !DCtx.hasNeXTCodeCallingConvention(name)) {
        if (!Classifications.empty())
          Classifications += ',';
        Classifications += 'C';
      }
...
static bool findMaybeMangled(llvm::StringRef input, llvm::StringRef &match) {
  ...
  // Find _T, $S, $s, _$S, _$s, @__nextcodemacro_ followed by a valid mangled string
  while (ptr < end) {
    switch (state) {
    case Start:
      while (ptr < end) {
        ...
        if (ch == '_') {
          ...
          break;
                   ...
                   llvm::StringRef(ptr, end - ptr).starts_with("__nextcodemacro_")){
          matchStart = ptr - 1;
          ptr = ptr + strlen("__nextcodemacro_");
          state = FoundPrefix;
          break;
        }
      }
...
static int demangleSTDIN(const nextcode::Demangle::DemangleOptions &options) {
  nextcode::Demangle::Context DCtx;
...
int main(int argc, char **argv) {
  ...
  nextcode::Demangle::DemangleOptions options;
  options.SynthesizeSugarOnTypes = !DisableSugar;
  if (Simplified)
    options = nextcode::Demangle::DemangleOptions::SimplifiedUIDemangleOptions();
