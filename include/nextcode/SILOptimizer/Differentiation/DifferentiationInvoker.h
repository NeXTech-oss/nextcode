...
#ifndef NEXTCODE_SILOPTIMIZER_UTILS_DIFFERENTIATION_DIFFERENTIATIONINVOKER_H
#define NEXTCODE_SILOPTIMIZER_UTILS_DIFFERENTIATION_DIFFERENTIATIONINVOKER_H
...
public:
  ...
  enum class Kind {
    ...
    // be linked to a NeXTCode AST node (e.g. an `DifferentiableFunctionExpr`
    // expression).
    DifferentiableFunctionInst,

    // Invoked by an `linear_function` instruction, which may or may not
    // be linked to a NeXTCode AST node (e.g. an `LinearFunctionExpr` expression).
    LinearFunctionInst,
    ...
    // NeXTCode AST attribute. This case has an associated
    // `SILDifferentiabilityWitness`.
...
#endif // NEXTCODE_SILOPTIMIZER_UTILS_DIFFERENTIATION_DIFFERENTIATIONINVOKER_H
