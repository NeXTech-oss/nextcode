//===--- PassManager.h  - NeXTCode Pass Manager --------------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_SILOPTIMIZER_PASSMANAGER_PASSMANAGER_H
#define NEXTCODE_SILOPTIMIZER_PASSMANAGER_PASSMANAGER_H
...
/// Utility class to invoke NeXTCode passes.
class NeXTCodePassInvocation {
  ...
  NeXTCodePassInvocation *nestedNeXTCodePassInvocation = nullptr;
...
public:
  NeXTCodePassInvocation(SILPassManager *passManager, SILFunction *function,
                         SILCombiner *silCombiner) :
    passManager(passManager), function(function), silCombiner(silCombiner) {}

  NeXTCodePassInvocation(SILPassManager *passManager, SILTransform *transform,
                      SILFunction *function) :
    passManager(passManager), transform(transform), function(function) {}

  NeXTCodePassInvocation(SILPassManager *passManager) :
  ...
  /// The top-level API to erase an instruction, called from the NeXTCode pass.
  ...
  NeXTCodePassInvocation *initializeNestedNeXTCodePassInvocation(SILFunction *newFunction) {
    assert(!nestedNeXTCodePassInvocation && "Nested NeXTCode pass invocation already initialized");
    nestedNeXTCodePassInvocation = new NeXTCodePassInvocation(passManager, transform, newFunction);
    return nestedNeXTCodePassInvocation;
  }

  void deinitializeNestedNeXTCodePassInvocation() {
    assert(nestedNeXTCodePassInvocation && "Nested NeXTCode pass invocation not initialized");
    nestedNeXTCodePassInvocation->endTransformFunction();
    delete nestedNeXTCodePassInvocation;
    nestedNeXTCodePassInvocation = nullptr;
  }
};
...
public:
  ...
  NeXTCodePassInvocation *getNeXTCodePassInvocation() {
  ...
  /// Runs the SIL verifier which is implemented in the NeXTCodeCompilerSources.
  void runNeXTCodeFunctionVerification(SILFunction *f);

  /// Runs the SIL verifier which is implemented in the NeXTCodeCompilerSources.
  void runNeXTCodeModuleVerification();
...
inline void NeXTCodePassInvocation::
