...
#ifndef NEXTCODE_SILOPTIMIZER_ANALYSIS_ALIASANALYSIS_H
#define NEXTCODE_SILOPTIMIZER_ANALYSIS_ALIASANALYSIS_H
...
///
/// This wrapper sits above the NeXTCodeCompilerSource implementation of
...
private:
  ...
  friend struct ::llvm::DenseMapInfo<nextcode::AliasAnalysis::AliasCacheKey>;
...
namespace llvm {
template <> struct DenseMapInfo<nextcode::AliasAnalysis::AliasCacheKey> {
  using AliasCacheKey = nextcode::AliasAnalysis::AliasCacheKey;

  static inline AliasCacheKey getEmptyKey() {
    return {DenseMapInfo<nextcode::SILValue>::getEmptyKey(), nextcode::SILValue(),
            nullptr, nullptr};
  }
  static inline AliasCacheKey getTombstoneKey() {
    return {DenseMapInfo<nextcode::SILValue>::getTombstoneKey(), nextcode::SILValue(),
            nullptr, nullptr};
  }
  static unsigned getHashValue(const AliasCacheKey Val) {
    unsigned H = 0;
    H ^= DenseMapInfo<nextcode::SILValue>::getHashValue(Val.V1);
    H ^= DenseMapInfo<nextcode::SILValue>::getHashValue(Val.V2);
