...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_SILOPTIMIZER_ANALYSIS_ACCESSED_STORAGE_ANALYSIS_H
#define NEXTCODE_SILOPTIMIZER_ANALYSIS_ACCESSED_STORAGE_ANALYSIS_H

#include "nextcode/SIL/MemAccessUtils.h"
#include "nextcode/SIL/SILFunction.h"
#include "nextcode/SIL/SILInstruction.h"
#include "nextcode/SILOptimizer/Analysis/BottomUpIPAnalysis.h"
...
template <> struct DenseMapInfo<nextcode::StorageAccessInfo> {
  static nextcode::StorageAccessInfo getEmptyKey() {
    auto key = DenseMapInfo<nextcode::AccessStorage>::getEmptyKey();
    return static_cast<nextcode::StorageAccessInfo &>(key);
  }

  static nextcode::StorageAccessInfo getTombstoneKey() {
    auto key = DenseMapInfo<nextcode::AccessStorage>::getTombstoneKey();
    return static_cast<nextcode::StorageAccessInfo &>(key);
  }
  static unsigned getHashValue(nextcode::StorageAccessInfo storage) {
    return DenseMapInfo<nextcode::AccessStorage>::getHashValue(storage);
  }
  static bool isEqual(nextcode::StorageAccessInfo LHS,
                      nextcode::StorageAccessInfo RHS) {
    return DenseMapInfo<nextcode::AccessStorage>::isEqual(LHS, RHS);
  }
};
}
...
#endif // NEXTCODE_SILOPTIMIZER_ANALYSIS_ACCESSED_STORAGE_ANALYSIS_H
