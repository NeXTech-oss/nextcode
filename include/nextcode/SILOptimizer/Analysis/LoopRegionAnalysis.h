...
#ifndef NEXTCODE_SILOPTIMIZER_ANALYSIS_LOOPNESTANALYSIS_H
#define NEXTCODE_SILOPTIMIZER_ANALYSIS_LOOPNESTANALYSIS_H
...
raw_ostream &operator<<(raw_ostream &os, nextcode::LoopRegion &LR);
raw_ostream &operator<<(raw_ostream &os, nextcode::LoopRegion::SuccessorID &S);

template <> struct DenseMapInfo<nextcode::LoopRegion::SuccessorID> {
  using Type = nextcode::LoopRegion::SuccessorID;
  ...
  static unsigned getHashValue(const nextcode::LoopRegion::SuccessorID Val) {
    return DenseMapInfo<unsigned>::getHashValue(Val.asInt());
  }
  static bool isEqual(const nextcode::LoopRegion::SuccessorID LHS,
                      const nextcode::LoopRegion::SuccessorID RHS) {
    return LHS == RHS;
  }
};
