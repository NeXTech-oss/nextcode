...
//===----------------------------------------------------------------------===//
//
#ifndef NEXTCODE_MARKUP_AST_H
#define NEXTCODE_MARKUP_AST_H
...
/// The basic structure of a doc comment attached to a NeXTCode
/// declaration.
struct CommentParts {
  std::optional<const Paragraph *> Brief;
  ArrayRef<const MarkupASTNode *> BodyNodes;
  ArrayRef<ParamField *> ParamFields;
  std::optional<const nextcode::markup::ReturnsField *> ReturnsField;
  std::optional<const nextcode::markup::ThrowsField *> ThrowsField;
  llvm::SmallSetVector<StringRef, 8> Tags;
  std::optional<const nextcode::markup::LocalizationKeyField *>
