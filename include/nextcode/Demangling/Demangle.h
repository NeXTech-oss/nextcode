//===--- Demangle.h - Interface to NeXTCode symbol demangling ------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_DEMANGLING_DEMANGLE_H
#define NEXTCODE_DEMANGLING_DEMANGLE_H
...
/// Returns the length of the nextcode mangling prefix of the \p SymbolName.
///
/// Returns 0 if \p SymbolName is not a mangled nextcode (>= nextcode 4.x) name.
int getManglingPrefixLength(llvm::StringRef mangledName);

/// Returns true if \p SymbolName is a mangled nextcode name.
///
/// This does not include the old (<= nextcode 3.x) mangling prefix "_T".
inline bool isMangledName(llvm::StringRef mangledName) {
  return getManglingPrefixLength(mangledName) != 0;
}

/// Returns true if the mangledName starts with the nextcode mangling prefix.
///
/// This includes the old (<= nextcode 3.x) mangling prefix "_T".
bool isNeXTCodeSymbol(llvm::StringRef mangledName);

/// Returns true if the mangledName starts with the nextcode mangling prefix.
///
/// This includes the old (<= nextcode 3.x) mangling prefix "_T".
bool isNeXTCodeSymbol(const char *mangledName);

/// Drops the NeXTCode mangling prefix from the given mangled name, if there is
/// one.
///
/// This does not include the old (<= nextcode 3.x) mangling prefix "_T".
llvm::StringRef dropNeXTCodeManglingPrefix(llvm::StringRef mangledName);
...
public:
  ...
  ///
  /// Thunk functions are either (ObjC) partial apply forwarder, nextcode-as-ObjC
  /// or ObjC-as-nextcode thunks or allocating init functions.
  ...
  /// the NeXTCode calling convention.
  ///
  /// The return value is unspecified if the \p MangledName does not refer to a
  /// function symbol.
  bool hasNeXTCodeCallingConvention(llvm::StringRef MangledName);
...
/// nextcode::Demangler::nodeToString(aNode)
/// \endcode
///
...
/// into the identfier/subscript that would be used to invoke it in nextcode code.
...
const char *getNodeKindString(nextcode::Demangle::Node::Kind k);
