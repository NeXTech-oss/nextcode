...
//===----------------------------------------------------------------------===//
//
// An implementation of a stable hashing for NeXTCode.
//
// Derived from the reference implementation for SipHash 2-4:
//   https://github.com/veorq/SipHash
//
// With inline buffering derived from the hash implementation in the NeXTCode
// Standard Library.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_BASIC_STABLEHASHER_H
#define NEXTCODE_BASIC_STABLEHASHER_H
...
/// \c nextcode::StableHasher::Combiner<T>
///
///    template <typename T>
///    struct nextcode::StableHasher::Combiner<std::optional<T>> {
///      static void combine(StableHasher &hasher, const std::optional<T> &O) {
///        if (!O.has_value()) {
///          hasher.combine(0);
///        } else {
///          hasher.combine(1);
///          nextcode::StableHasher::Combiner<T>::combine(hasher, O.value());
///        }
///      }
///    };
///
...
#endif // NEXTCODE_BASIC_STABLEHASHER_H
