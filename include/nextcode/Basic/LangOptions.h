...
#ifndef NEXTCODE_BASIC_LANGOPTIONS_H
#define NEXTCODE_BASIC_LANGOPTIONS_H

#include "nextcode/AST/DiagnosticsFrontend.h"
#include "nextcode/Basic/Feature.h"
#include "nextcode/Basic/FixedBitSet.h"
#include "nextcode/Basic/FunctionBodySkipping.h"
#include "nextcode/Basic/LLVM.h"
#include "nextcode/Basic/PlaygroundOption.h"
...
#include "nextcode/AST/PlatformConditionKinds.def"
  };
  ...
  enum class UnavailableDeclOptimization : uint8_t {
    ...
    /// pre-NeXTCode 5.9 compiler may depend on linkable symbols associated with
  ...
  public:
    ...
    /// NeXTCode runtime version to compile for.
    ...
    /// to the NeXTCode language version.
    ...
    /// when importing NeXTCode modules that enable C++ interoperability.
    bool RequireCxxInteropToImportCxxInteropModule = true;

    // Workaround for bug when building NeXTCodeCompilerSources (rdar://128013193)
    bool CxxInteropUseOpaquePointerForMoveOnly = false;

    /// On Darwin platforms, use the pre-stable ABI's mark bit for NeXTCode
    ...
    /// it gets disabled when compiling the NeXTCode core stdlib.
    ...
    /// immediately, bypassing their NeXTCodeLookupTables. This emulates an
    ...
    bool DisableImplicitConcurrencyModuleImport =
        !NEXTCODE_IMPLICIT_CONCURRENCY_IMPORT;
    ...
    bool DisableImplicitBacktracingModuleImport =
        !NEXTCODE_IMPLICIT_BACKTRACING_IMPORT;
    ...
    // NeXTCode into ObjC. If false, will use unchecked continuations instead.
    ...
    /// errors when deserializing from a binary nextcodemodule file.
    ///
    ...
    bool EnableDeserializationSafety =
      ::getenv("NEXTCODE_ENABLE_DESERIALIZATION_SAFETY");
    ...
    /// When using fine-grained dependencies, emit dot files for every nextcodedeps
    /// file.
    ...
    /// Load nextcodemodule files in memory as volatile and avoid mmap.
  ...
  public:
    ...
    enum class Modes : uint8_t {
      /// Set up Clang for importing modules into NeXTCode and generating IR from
      /// NeXTCode code.
      Normal,
    ...
    /// diagnostic printer as well as being passed to NeXTCode's diagnostic engine.
    ...
    /// If true ignore the nextcode bridged attribute.
    bool DisableNeXTCodeBridgeAttr = false;

    /// When set, don't look for or load overlays.
    bool DisableOverlayModules = false;

    /// When set, import SPI_AVAILABLE symbols with NeXTCode SPI attributes.
    ...
    /// When building a PCM, rely on the NeXTCode frontend's command-line -Xcc flags
    ...
    /// Whether the dependency scanner should construct all nextcode-frontend
    ...
    /// For a nextcode module dependency, interface build command generation must
    /// inherit
    ...
    getReducedExtraArgsForNeXTCodeModuleDependency() const;
  };

} // end namespace nextcode

#endif // NEXTCODE_BASIC_LANGOPTIONS_H
