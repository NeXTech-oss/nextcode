...
#ifndef NEXTCODE_BASIC_SOURCELOC_H
#define NEXTCODE_BASIC_SOURCELOC_H
...
/// SourceLoc in nextcode is just an SMLoc.  We define it as a different type
/// (instead of as a typedef) just to remove the "getFromPointer" methods and
/// enforce purity in the NeXTCode codebase.
class SourceLoc {
...
public:
  ...
  NEXTCODE_DEBUG_DUMPER(dump(const SourceManager &SM));
...
/// SourceRange in nextcode is a pair of locations.  However, note that the end
...
template <> struct DenseMapInfo<nextcode::SourceLoc> {
  static nextcode::SourceLoc getEmptyKey() {
    return nextcode::SourceLoc(
        SMLoc::getFromPointer(DenseMapInfo<const char *>::getEmptyKey()));
  }

  static nextcode::SourceLoc getTombstoneKey() {
  ...
  static unsigned getHashValue(const nextcode::SourceLoc &Val) {
    return DenseMapInfo<const void *>::getHashValue(
        Val.getOpaquePointerValue());
  }

  static bool isEqual(const nextcode::SourceLoc &LHS,
                      const nextcode::SourceLoc &RHS) {
    return LHS == RHS;
  }
};

template <> struct DenseMapInfo<nextcode::SourceRange> {
  static nextcode::SourceRange getEmptyKey() {
    return nextcode::SourceRange(nextcode::SourceLoc(
        SMLoc::getFromPointer(DenseMapInfo<const char *>::getEmptyKey())));
  }

  static nextcode::SourceRange getTombstoneKey() {
    // Make this different from empty key. See for context:
    // http://lists.llvm.org/pipermail/llvm-dev/2015-July/088744.html
    return nextcode::SourceRange(nextcode::SourceLoc(
        SMLoc::getFromPointer(DenseMapInfo<const char *>::getTombstoneKey())));
  ...
  static unsigned getHashValue(const nextcode::SourceRange &Val) {
    return hash_combine(Val.Start.getOpaquePointerValue(),
                        Val.End.getOpaquePointerValue());
  }

  static bool isEqual(const nextcode::SourceRange &LHS,
                      const nextcode::SourceRange &RHS) {
    return LHS == RHS;
  }
};
} // namespace llvm

#endif // NEXTCODE_BASIC_SOURCELOC_H
