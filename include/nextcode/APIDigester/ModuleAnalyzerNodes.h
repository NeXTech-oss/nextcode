...
//===----------------------------------------------------------------------===//
//
//  Describing nodes from a nextcodemodule file to detect ABI/API breakages.
//
//===----------------------------------------------------------------------===//

#ifndef __NEXTCODE_ABI_DIGESTER_MODULE_NODES_H__
#define __NEXTCODE_ABI_DIGESTER_MODULE_NODES_H__
...
#include "nextcode/Basic/ColorUtils.h"
...
public:
  ...
  nextcode::ReferenceOwnership getReferenceOwnership() const {
    return nextcode::ReferenceOwnership(ReferenceOwnership);
  }
  ...
  void emitDiag(SourceLoc Loc,
    ...
    // Don't emit objc decls if we care about nextcode exclusively
    if (Ctx.getOpts().NeXTCodeOnly) {
      if (isObjc())
        return;
    }
...
class NeXTCodeDeclCollector: public VisibleDeclConsumer {
  SDKContext &Ctx;
  SDKNode *RootNode;
...
public:
  ...
  NeXTCodeDeclCollector(SDKContext &Ctx, ArrayRef<ModuleDecl*> modules = {});
...
int dumpNeXTCodeModules(const CompilerInvocation &InitInvoke,
