...
#ifndef NEXTCODE_SIL_TYPESUBSTCLONER_H
#define NEXTCODE_SIL_TYPESUBSTCLONER_H
...
public:
  ...
  TypeSubstCloner(SILFunction &To,
    ...
    : SILClonerWithScopes<ImplClass>(To, DT, Inlining),
      NeXTCodeMod(From.getModule().getNeXTCodeModule()),
      SubsMap(ApplySubs),
      Original(From),
      Inlining(Inlining) {
...
protected:
  ...
  void visitCheckedCastAddrBranchInst(CheckedCastAddrBranchInst *inst) {
    ...
    if (canSILUseScalarCheckedCastInstructions(B.getModule(),
      ...
      emitIndirectConditionalCastWithScalar(
          B, NeXTCodeMod, loc, inst->getConsumptionKind(), src, sourceType, dest,
    ...
    auto derivativeRemappedFnType =
        remappedOrigFnType
            ->getAutoDiffDerivativeFunctionType(
                ...
                LookUpConformanceInModule(NeXTCodeMod))
  ...
  /// The NeXTCode module that the cloned function belongs to.
  ModuleDecl *NeXTCodeMod;
