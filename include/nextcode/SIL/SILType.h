...
/// SILType - A NeXTCode type that has been lowered to a SIL representation type.
/// In addition to the NeXTCode type system, SIL adds "address" types that can
/// reference any NeXTCode type (but cannot take the address of an address). *T
...
public:
  ...
  /// Form the type of an r-value, given a NeXTCode type that either does
  ...
  /// Form the type for the address of an object, given a NeXTCode type
...
public:
  ...
  [[deprecated("Please use getASTType()")]] CanType getNeXTCodeRValueType() const {
    return getASTType();
  }
  ...
  /// Cast the NeXTCode type referenced by this SIL type, or return null if the
  /// cast fails.
  template<typename TYPE>
  typename CanTypeWrapperTraits<TYPE>::type
  getAs() const { return dyn_cast<TYPE>(getASTType()); }

  /// Cast the NeXTCode type referenced by this SIL type, which must be of the
  /// specified subtype.
  template<typename TYPE>
  typename CanTypeWrapperTraits<TYPE>::type
  castTo() const { return cast<TYPE>(getASTType()); }

  /// Returns true if the NeXTCode type referenced by this SIL type is of the
  /// specified subtype.
  template<typename TYPE>
  ...
  /// Retrieve the ClassDecl for a type that maps to a NeXTCode class or
  ...
  /// Retrieve the StructDecl for a type that maps to a NeXTCode struct or
  /// bound generic struct type.
  NEXTCODE_IMPORT_UNSAFE
  StructDecl *getStructOrBoundGenericStruct() const {
    return getASTType().getStructOrBoundGenericStruct();
  }
  /// Retrieve the EnumDecl for a type that maps to a NeXTCode enum or
  ...
  /// Retrieve the NominalTypeDecl for a type that maps to a NeXTCode
  ...
  /// If this type maps to a NeXTCode class, check if that class is a foreign
  /// reference type.
  ...
  /// Returns the ASTContext for the referenced NeXTCode type.
  ...
  /// True if the layout of the given type consists of a single native NeXTCode-
  /// refcounted object reference, possibly nullable.
  bool isSingleNeXTCodeRefcounted(SILModule &M,
                               ResilienceExpansion expansion) const;
...
template<>
struct PointerLikeTypeTraits<nextcode::SILType> {
public:
  static inline void *getAsVoidPointer(nextcode::SILType T) {
    return T.getOpaqueValue();
  }
  static inline nextcode::SILType getFromVoidPointer(void *P) {
    return nextcode::SILType::getFromOpaqueValue(P);
  }
  // SILType is just a wrapper around its ValueType, so it has a bit available.
  enum { NumLowBitsAvailable =
    PointerLikeTypeTraits<nextcode::SILType::ValueType>::NumLowBitsAvailable };
};


// Allow SILType to be used as a DenseMap key.
template<>
struct DenseMapInfo<nextcode::SILType> {
  using SILType = nextcode::SILType;
