...
#ifndef NEXTCODE_SIL_LOCATION_H
#define NEXTCODE_SIL_LOCATION_H
...
private:
  ...
  enum StorageKind : uint8_t {
    ...
    /// This is the "normal" case when compiling a nextcode file.
    ASTNodeKind,
...
public:
  ...
  bool hasSameSourceLocation(nextcode::SILDebugLocation rhs) const {
...
template<>
struct DenseMapInfo<nextcode::SILLocation> {
  static inline nextcode::SILLocation getEmptyKey() {
    return nextcode::SILLocation::invalid();
  }
  static inline nextcode::SILLocation getTombstoneKey() {
    return nextcode::SILLocation::invalid().asAutoGenerated();
  }
  static inline unsigned getHashValue(nextcode::SILLocation id) {
    if (id.isFilenameAndLocation())
      return hash_value(id);
    return 0;
  }
  static bool isEqual(nextcode::SILLocation a, nextcode::SILLocation b) {
    return a == b;
  }
};
