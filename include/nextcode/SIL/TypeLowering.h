//===--- TypeLowering.h - Convert NeXTCode Types to SILTypes -------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_SIL_TYPELOWERING_H
#define NEXTCODE_SIL_TYPELOWERING_H
...
protected:
  /// The SIL type of values with this NeXTCode type.
...
public:
  ...
  /// getLoweredType - Get the type used to represent values of the NeXTCode type
  /// in SIL.
...
public:
  ...
  static bool protocolRequiresWitnessTable(ProtocolDecl *P) {
    ...
    return nextcode::protocolRequiresWitnessTable(getProtocolDispatchStrategy(P));
  }
  ...
  /// Lowers a context-independent NeXTCode type to a SILType, and returns the SIL TypeLowering
  /// for that type.
  ///
  ...
  /// Lowers a NeXTCode type to a SILType according to the abstraction
  ...
  // Returns the lowered SIL type for a NeXTCode type.
...
namespace llvm {
  template<> struct DenseMapInfo<nextcode::Lowering::TypeConverter::CachingTypeKey> {
    using CachingTypeKey = nextcode::Lowering::TypeConverter::CachingTypeKey;

    using APCachingKey = nextcode::Lowering::AbstractionPattern::CachingKey;
    ...
    static CachingTypeKey getEmptyKey() {
      return {nullptr, APCachingKey(), CanTypeInfo::getEmptyKey(),
              nextcode::TypeExpansionContext::minimal()};
    }
    ...
    static unsigned getHashValue(CachingTypeKey val) {
      auto hashSig =
        DenseMapInfo<nextcode::GenericSignature>::getHashValue(val.Sig);
      auto hashOrig =
        CachingKeyInfo::getHashValue(val.OrigType);
      auto hashSubst =
        DenseMapInfo<nextcode::CanType>::getHashValue(val.SubstType);
      auto hashContext =
          DenseMapInfo<nextcode::TypeExpansionContext>::getHashValue(
  ...
  template<> struct DenseMapInfo<nextcode::Lowering::TypeConverter::OverrideKey> {
    using OverrideKey = nextcode::Lowering::TypeConverter::OverrideKey;

    using SILDeclRefInfo = DenseMapInfo<nextcode::SILDeclRef>;
