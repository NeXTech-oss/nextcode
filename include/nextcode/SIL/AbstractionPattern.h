...
#ifndef NEXTCODE_SIL_ABSTRACTIONPATTERN_H
#define NEXTCODE_SIL_ABSTRACTIONPATTERN_H

#include "nextcode/Basic/IndexedViewRange.h"
...
///
/// The representation of values in NeXTCode can vary according to how
...
/// would force NeXTCode to adopt a very inefficient representation for
/// abstractable values.
///
...
/// NeXTCode's implementation model, these functions don't have to be
...
///
/// 1. Give all types in NeXTCode a common representation.  The generic
...
class AbstractionPattern {
  enum class Kind {
    ...
    /// an API that has been imported into NeXTCode as async,
    ...
    /// A NeXTCode function whose parameters and results are opaque. This is
    ...
    OpaqueFunction,
    /// A NeXTCode function whose parameters are opaque and whose result is the
  ...
  void initNeXTCodeType(SubstitutionMap subs,
  ...
  void initClangType(SubstitutionMap subs, CanGenericSignature signature,
    ...
    initNeXTCodeType(subs, signature, origType, kind);
    ClangType = clangType;
  }
...
public:
  ...
  explicit AbstractionPattern(CanGenericSignature signature, CanType origType) {
    initNeXTCodeType(SubstitutionMap(), signature, origType);
  }
  explicit AbstractionPattern(SubstitutionMap subs, CanType origType) {
    initNeXTCodeType(subs, subs.getGenericSignature().getCanonicalSignature(),
                  origType);
  }
  explicit AbstractionPattern(SubstitutionMap subs, CanGenericSignature sig,
                              CanType origType) {
    initNeXTCodeType(subs, sig, origType);
  }
...
public:
  ...
  static AbstractionPattern getDiscard(SubstitutionMap subs,
                                       ...
                                       CanType origType) {
    AbstractionPattern pattern;
    pattern.initNeXTCodeType(subs, signature, origType, Kind::Discard);
    return pattern;
  }
...
public:
  ...
  /// a Clang or NeXTCode type.  That is, it cannot be a tuple or an ObjC
  /// method type.
  ...
  /// Return the NeXTCode type which provides structure for this
  /// abstraction pattern.
  ///
  ...
  /// is not possible in NeXTCode's ABI to support this without some sort
  ...
  AbstractionPattern getObjCMethodAsyncCompletionHandlerType(
                                     CanType nextcodeCompletionHandlerType) const;
