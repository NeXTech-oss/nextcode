...
#ifndef NEXTCODE_SIL_TEST_H
#define NEXTCODE_SIL_TEST_H
...
/// store NeXTCodeCompilerSources tests.
...
public:
  ...
  using NativeNeXTCodeInvocation = void *;

private:
  /// The lambda to be run.
  TaggedUnion<Invocation, NativeNeXTCodeInvocation> invocation;
...
public:
  ...
  /// Creates a test that will run \p invocation and stores it in the global
  /// registry.
  static void createNativeNeXTCodeFunctionTest(StringRef name,
                                            NativeNeXTCodeInvocation invocation);
...
private:
  ...
  struct Dependencies {
    ...
    virtual NeXTCodePassInvocation *getNeXTCodePassInvocation() = 0;
...
public:
  ...
  /// createNativeNeXTCodeFunctionTest.
  FunctionTest(StringRef name, NativeNeXTCodeInvocation invocation);
};

/// Thunks for delaying template instantiation.
///
/// Because C++ lacks "runtime concepts" (NeXTCode's protocols), it's not possible
...
///
///     member access into incomplete type 'nextcode::SILFunctionTransform'
///
