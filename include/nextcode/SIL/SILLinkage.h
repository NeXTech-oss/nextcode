...
#ifndef NEXTCODE_SIL_SILLINKAGE_H
#define NEXTCODE_SIL_SILLINKAGE_H
...
///
/// Note that a NeXTCode module is not the same thing as a SILModule.
...
enum class SILLinkage : uint8_t {
  /// This object definition is visible to multiple NeXTCode modules (and
  ...
  /// This object definition is visible only to the current NeXTCode
  ...
  /// This object definition is visible only within a single NeXTCode
  ...
  /// available to the current NeXTCode module at runtime.  If this
  ...
  /// will be defined by the current NeXTCode module at runtime.
  ///
...
///
/// After the nextcodemodule file is written, the IsSerialized flag is cleared from
...
inline SILLinkage addExternalToLinkage(SILLinkage linkage) {
  ...
  case SILLinkage::PackageNonABI:
    ...
    // same NeXTCode module, so we treat it as hidden here.
