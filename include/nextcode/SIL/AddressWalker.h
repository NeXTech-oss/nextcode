...
#ifndef NEXTCODE_SIL_ADDRESSWALKER_H
#define NEXTCODE_SIL_ADDRESSWALKER_H

#include "nextcode/Basic/Defer.h"
#include "nextcode/SIL/AddressUseKind.h"
...
protected:
  ...
  void meet(AddressUseKind other) {
    ...
    result = nextcode::meet(result, other);
  }
...
TransitiveAddressWalker<Impl>::walk(SILValue projectedAddress) && {
  ...
  NEXTCODE_DEFER { didInvalidate = true; };
