...
//===----------------------------------------------------------------------===//
//
// This file defines the high-level BasicBlocks used for NeXTCode SIL code.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_SIL_BASICBLOCK_H
#define NEXTCODE_SIL_BASICBLOCK_H

#include "nextcode/Basic/Compiler.h"
#include "nextcode/Basic/Range.h"
#include "nextcode/Basic/NeXTCodeObjectHeader.h"
#include "nextcode/SIL/SILArgumentArrayRef.h"
...
public NeXTCodeObjectHeader {
  ...
  static NeXTCodeMetatype registeredMetatype;
...
public:
  static void registerBridgedMetatype(NeXTCodeMetatype metatype) {
...
template <>
struct ilist_traits<::nextcode::SILBasicBlock>
  : ilist_node_traits<::nextcode::SILBasicBlock> {
  using SelfTy = ilist_traits<::nextcode::SILBasicBlock>;
  using SILBasicBlock = ::nextcode::SILBasicBlock;
  using SILFunction = ::nextcode::SILFunction;
  using FunctionPtrTy = ::nextcode::NullablePtr<SILFunction>;

private:
  friend class ::nextcode::SILFunction;
...
template <> struct DenseMapInfo<nextcode::PhiOperand> {
  static nextcode::PhiOperand getEmptyKey() { return nextcode::PhiOperand(); }
  static nextcode::PhiOperand getTombstoneKey() {
    nextcode::PhiOperand phiOper;
    phiOper.predBlock =
        llvm::DenseMapInfo<nextcode::SILBasicBlock *>::getTombstoneKey();
    return phiOper;
  }
  static unsigned getHashValue(nextcode::PhiOperand phiOper) {
    return llvm::hash_combine(phiOper.predBlock, phiOper.argIndex);
  }
  static bool isEqual(nextcode::PhiOperand lhs, nextcode::PhiOperand rhs) {
    return lhs == rhs;
  }
};

template <> struct DenseMapInfo<nextcode::PhiValue> {
  static nextcode::PhiValue getEmptyKey() { return nextcode::PhiValue(); }
  static nextcode::PhiValue getTombstoneKey() {
    nextcode::PhiValue phiValue;
    phiValue.phiBlock =
        llvm::DenseMapInfo<nextcode::SILBasicBlock *>::getTombstoneKey();
    return phiValue;
  }
  static unsigned getHashValue(nextcode::PhiValue phiValue) {
    return llvm::hash_combine(phiValue.phiBlock, phiValue.argIndex);
  }
  static bool isEqual(nextcode::PhiValue lhs, nextcode::PhiValue rhs) {
    return lhs == rhs;
  }
};
