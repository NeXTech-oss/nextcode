...
#ifndef NEXTCODE_SIL_MEMACCESSUTILS_H
#define NEXTCODE_SIL_MEMACCESSUTILS_H

#include "nextcode/Basic/IndexTrie.h"
...
protected:
  ...
  union {
    ...
    // can be reduced.
    NEXTCODE_INLINE_BITFIELD_BASE(AccessRepresentation, 32,
    ...
    NEXTCODE_INLINE_BITFIELD_FULL(StorageAccessInfo, AccessRepresentation,
    ...
    NEXTCODE_INLINE_BITFIELD_FULL(AccessEnforcementOptsInfo,
    ...
    NEXTCODE_INLINE_BITFIELD(DomAccessStorage, AccessRepresentation, 1 + 1,
...
template <> struct DenseMapInfo<nextcode::AccessStorage> {
  static nextcode::AccessStorage getEmptyKey() {
    return nextcode::AccessStorage(nextcode::SILValue::getFromOpaqueValue(
                               llvm::DenseMapInfo<void *>::getEmptyKey()),
                           nextcode::AccessStorage::Unidentified);
  }

  static nextcode::AccessStorage getTombstoneKey() {
    return nextcode::AccessStorage(nextcode::SILValue::getFromOpaqueValue(
                               llvm::DenseMapInfo<void *>::getTombstoneKey()),
                           nextcode::AccessStorage::Unidentified);
  }

  static unsigned getHashValue(nextcode::AccessStorage storage) {
    switch (storage.getKind()) {
    case nextcode::AccessStorage::Unidentified:
      if (!storage)
        return DenseMapInfo<nextcode::SILValue>::getHashValue(nextcode::SILValue());
      LLVM_FALLTHROUGH;
    case nextcode::AccessStorage::Box:
    case nextcode::AccessStorage::Stack:
    case nextcode::AccessStorage::Nested:
    case nextcode::AccessStorage::Yield:
      return DenseMapInfo<nextcode::SILValue>::getHashValue(storage.getValue());
    case nextcode::AccessStorage::Argument:
      return storage.getParamIndex();
    case nextcode::AccessStorage::Global:
      return DenseMapInfo<void *>::getHashValue(storage.getGlobal());
    case nextcode::AccessStorage::Class:
      return llvm::hash_combine(storage.getObject(),
                                storage.getPropertyIndex());
    case nextcode::AccessStorage::Tail:
      return DenseMapInfo<nextcode::SILValue>::getHashValue(storage.getObject());
    }
    llvm_unreachable("Unhandled AccessStorageKind");
  }

  static bool isEqual(nextcode::AccessStorage LHS, nextcode::AccessStorage RHS) {
...
template <> struct DenseMapInfo<nextcode::AccessPath> {
  static inline nextcode::AccessPath getEmptyKey() {
    return nextcode::AccessPath(
        DenseMapInfo<nextcode::AccessStorage>::getEmptyKey(),
        nextcode::AccessPath::PathNode(
          DenseMapInfo<nextcode::IndexTrieNode *>::getEmptyKey()), 0);
  }
  static inline nextcode::AccessPath getTombstoneKey() {
    return nextcode::AccessPath(
        DenseMapInfo<nextcode::AccessStorage>::getTombstoneKey(),
        nextcode::AccessPath::PathNode(
          DenseMapInfo<nextcode::IndexTrieNode *>::getTombstoneKey()), 0);
  }
  static inline unsigned getHashValue(const nextcode::AccessPath &val) {
    return llvm::hash_combine(
        DenseMapInfo<nextcode::AccessStorage>::getHashValue(val.getStorage()),
        val.getPathNode().node);
  }
  static bool isEqual(const nextcode::AccessPath &lhs,
                      const nextcode::AccessPath &rhs) {
    return lhs == rhs;
  }
};
template <> struct DenseMapInfo<nextcode::AccessPathWithBase> {
  static inline nextcode::AccessPathWithBase getEmptyKey() {
    return nextcode::AccessPathWithBase(
        DenseMapInfo<nextcode::AccessPath>::getEmptyKey(),
        DenseMapInfo<nextcode::SILValue>::getEmptyKey());
  }
  static inline nextcode::AccessPathWithBase getTombstoneKey() {
    return nextcode::AccessPathWithBase(
        DenseMapInfo<nextcode::AccessPath>::getTombstoneKey(),
        DenseMapInfo<nextcode::SILValue>::getTombstoneKey());
  }
  static inline unsigned getHashValue(const nextcode::AccessPathWithBase &val) {
    return llvm::hash_combine(
        DenseMapInfo<nextcode::AccessPath>::getHashValue(val.accessPath),
        DenseMapInfo<nextcode::SILValue>::getHashValue(val.base));
  }
  static bool isEqual(const nextcode::AccessPathWithBase &lhs,
                      const nextcode::AccessPathWithBase &rhs) {
    return lhs == rhs;
  }
};
...
template<>
struct PointerLikeTypeTraits<nextcode::AccessPath::PathNode> {
  static inline void *getAsVoidPointer(nextcode::AccessPath::PathNode node) {
    return (void *)node.node;
  }
  static inline nextcode::AccessPath::PathNode getFromVoidPointer(void *pointer) {
    return nextcode::AccessPath::PathNode((nextcode::IndexTrieNode *)pointer);
  }
  enum { NumLowBitsAvailable =
         PointerLikeTypeTraits<nextcode::IndexTrieNode *>::NumLowBitsAvailable };
};
} // end namespace llvm
