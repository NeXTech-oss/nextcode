...
#ifndef NEXTCODE_SIL_SILDeclRef_H
#define NEXTCODE_SIL_SILDeclRef_H
...
/// A key for referencing a NeXTCode declaration in SIL.
///
...
struct SILDeclRef {
  ...
  /// Represents the "kind" of the SILDeclRef. For some NeXTCode decls there
  ...
  /// declaration to NeXTCode calling convention.
  bool isForeignToNativeThunk() const;

  /// True if the decl ref references a thunk from a natively NeXTCode declaration
  ...
  /// True if the referenced entity is emitted by NeXTCode on behalf of the Clang
  /// importer.
...
private:
  friend struct llvm::DenseMapInfo<nextcode::SILDeclRef>;
...
template<> struct DenseMapInfo<nextcode::SILDeclRef> {
  using SILDeclRef = nextcode::SILDeclRef;
  ...
  static unsigned getHashValue(nextcode::SILDeclRef Val) {
  ...
  static bool isEqual(nextcode::SILDeclRef const &LHS,
                      nextcode::SILDeclRef const &RHS) {
    return LHS == RHS;
  }
};
