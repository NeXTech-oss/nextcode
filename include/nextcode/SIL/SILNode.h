...
#ifndef NEXTCODE_SIL_SILNODE_H
#define NEXTCODE_SIL_SILNODE_H

#include "llvm/Support/Compiler.h"
#include "llvm/Support/PointerLikeTypeTraits.h"
#include "nextcode/Basic/LLVM.h"
#include "nextcode/Basic/NeXTCodeObjectHeader.h"
#include <type_traits>
...
class alignas(8) SILNode :
  // SILNode contains a nextcode object header for bridging with NeXTCode.
  // For details see NeXTCodeCompilerSources/README.md.
  public NeXTCodeObjectHeader {
public:
...
private:
  NeXTCodeMetatype getSILNodeMetatype(SILNodeKind kind);

protected:
  SILNode(SILNodeKind kind) : NeXTCodeObjectHeader(getSILNodeMetatype(kind)),
                              kind((uint8_t)kind),
...
template <class To>
struct cast_convert_val<To, nextcode::SILNode*, nextcode::SILNode*> {
  using ret_type = typename cast_retty<To, nextcode::SILNode*>::ret_type;
  static ret_type doit(nextcode::SILNode *node) {
    return nextcode::cast_from_SILNode<To>::doit(node);
  }
};
template <class To>
struct cast_convert_val<To, const nextcode::SILNode *, const nextcode::SILNode *> {
  using ret_type = typename cast_retty<To, const nextcode::SILNode*>::ret_type;
  static ret_type doit(const nextcode::SILNode *node) {
    return nextcode::cast_from_SILNode<To>::doit(const_cast<nextcode::SILNode*>(node));
  }
};
template <class To>
struct cast_convert_val<To, nextcode::SILInstruction*, nextcode::SILInstruction*> {
  using ret_type = typename cast_retty<To, nextcode::SILInstruction*>::ret_type;
  static ret_type doit(nextcode::SILInstruction *inst) {
    return nextcode::cast_from_SILInstruction<To>::doit(inst);
  }
};
template <class To>
struct cast_convert_val<To, const nextcode::SILInstruction *,
                            const nextcode::SILInstruction *> {
  using ret_type = typename cast_retty<To, const nextcode::SILInstruction*>::ret_type;
  static ret_type doit(const nextcode::SILInstruction *inst) {
    return nextcode::cast_from_SILInstruction<To>::
             doit(const_cast<nextcode::SILInstruction*>(inst));
  }
};

// We don't support casting from SILNode references yet.
template <class To, class From>
struct cast_convert_val<To, nextcode::SILNode, From>;
template <class To, class From>
struct cast_convert_val<To, const nextcode::SILNode, From>;

/// ValueBase * is always at least eight-byte aligned; make the three tag bits
/// available through PointerLikeTypeTraits.
template<>
struct PointerLikeTypeTraits<nextcode::SILNode *> {
public:
  static inline void *getAsVoidPointer(nextcode::SILNode *I) {
    return (void*)I;
  }
  static inline nextcode::SILNode *getFromVoidPointer(void *P) {
    return (nextcode::SILNode *)P;
  }
