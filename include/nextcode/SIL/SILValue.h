...
#ifndef NEXTCODE_SIL_SILVALUE_H
#define NEXTCODE_SIL_SILVALUE_H
...
struct OwnershipKind {
  enum innerty : uint8_t {
    ...
    /// between NeXTCode and Objective-C code.
    Unowned,
...
public:
  ...
  /// To ignore debug-info instructions use nextcode::onlyHaveDebugUses instead
  ...
  /// To ignore debug-info instructions use nextcode::getNonDebugUses instead
  /// (see comment in DebugUtils.h).
  inline use_range getUses() const;

  /// Returns true if this value has exactly one use.
  /// To ignore debug-info instructions use nextcode::hasOneNonDebugUse instead
  /// (see comment in DebugUtils.h).
  ...
  using UserRange =
      llvm::iterator_range<llvm::mapped_iterator<nextcode::ValueBaseUseIterator,
                                                 nextcode::ValueBase::UseToUser,
                                                 nextcode::SILInstruction *>>;
...
template<>
struct PointerLikeTypeTraits<nextcode::ValueBase *> {
public:
  static inline void *getAsVoidPointer(nextcode::ValueBase *I) {
    return (void*)I;
  }
  static inline nextcode::ValueBase *getFromVoidPointer(void *P) {
    return (nextcode::ValueBase *)P;
  }
...
namespace llvm {
  ...
  template<> struct simplify_type<const ::nextcode::SILValue> {
    using SimpleType = ::nextcode::ValueBase *;
    static SimpleType getSimplifiedValue(::nextcode::SILValue Val) {
      return Val;
    }
  };
  template<> struct simplify_type< ::nextcode::SILValue>
    : public simplify_type<const ::nextcode::SILValue> {};

  // Values hash just like pointers.
  template<> struct DenseMapInfo<nextcode::SILValue> {
    static nextcode::SILValue getEmptyKey() {
      return nextcode::SILValue::getFromOpaqueValue(
                                      llvm::DenseMapInfo<void*>::getEmptyKey());
    }
    static nextcode::SILValue getTombstoneKey() {
      return nextcode::SILValue::getFromOpaqueValue(
                                  llvm::DenseMapInfo<void*>::getTombstoneKey());
    }
    static unsigned getHashValue(nextcode::SILValue V) {
      return DenseMapInfo<nextcode::ValueBase *>::getHashValue(V);
    }
    static bool isEqual(nextcode::SILValue LHS, nextcode::SILValue RHS) {
      return LHS == RHS;
    }
  };

  /// SILValue is a PointerLikeType.
  template<> struct PointerLikeTypeTraits<::nextcode::SILValue> {
    using SILValue = ::nextcode::SILValue;
  public:
    ...
    enum { NumLowBitsAvailable = nextcode::SILValue::NumLowBitsAvailable };
  };

  /// A SILValue can be checked if a value is present, so we can use it with
  /// dyn_cast_or_null.
  template <>
  struct ValueIsPresent<nextcode::SILValue> {
    using SILValue = nextcode::SILValue;
