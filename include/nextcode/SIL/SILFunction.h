...
#ifndef NEXTCODE_SIL_SILFUNCTION_H
#define NEXTCODE_SIL_SILFUNCTION_H
...
private:
  void *libnextcodeSpecificData[4];
...
private:
  ...
  /// preserved and exported more widely than its NeXTCode linkage and usage
  /// would indicate.
...
/// \p callerSerializedKind The caller's SerializedKind.
  ...
  bool isAvailableExternally() const {
    return nextcode::isAvailableExternally(getLinkage());
  }
  ...
  /// function.  We have to store an owner (a NeXTCode declaration) instead of
...
template <>
struct ilist_traits<::nextcode::SILFunction> :
public ilist_node_traits<::nextcode::SILFunction> {
