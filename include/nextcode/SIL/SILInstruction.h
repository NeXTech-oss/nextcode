...
#ifndef NEXTCODE_SIL_INSTRUCTION_H
#define NEXTCODE_SIL_INSTRUCTION_H

#include "nextcode/AST/ActorIsolation.h"
...
template <> struct compute_node_options<::nextcode::SILInstruction> {
  struct type {
    typedef ::nextcode::SILInstruction value_type;
...
/// contents of a NeXTCode SILBasicBlock.
///
...
protected:
  friend class NeXTCodePassInvocation;
...
/// This represents the allocation of a heap box for a NeXTCode value of some type.
...
public:
  ...
  ///
  /// Warning: this enum must be in sync with the nextcode `MarkUninitializedInst.Kind`
  enum Kind {
    ...
    ///
    /// This is only used for NeXTCode 4 compatibility. In NeXTCode 5, cross-module
...
/// to a native nextcode object with strong reference count of 1.
class IsUniqueInst
...
template <>
struct ilist_traits<::nextcode::SILInstruction> :
  public ilist_node_traits<::nextcode::SILInstruction> {
  using SILInstruction = ::nextcode::SILInstruction;

private:
  nextcode::SILBasicBlock *getContainingBlock();
...
template <>
struct DenseMapInfo<nextcode::SILDebugVariable> {
  using KeyTy = nextcode::SILDebugVariable;
