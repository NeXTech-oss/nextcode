...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_SEMA_CSBINDINGS_H
#define NEXTCODE_SEMA_CSBINDINGS_H
...
template <>
struct DenseMapInfo<nextcode::constraints::inference::PotentialBinding> {
  using Binding = nextcode::constraints::inference::PotentialBinding;

  static Binding getEmptyKey() {
    return placeholderKey(llvm::DenseMapInfo<nextcode::TypeBase *>::getEmptyKey());
  }

  static Binding getTombstoneKey() {
    return placeholderKey(
        llvm::DenseMapInfo<nextcode::TypeBase *>::getTombstoneKey());
  }

  static unsigned getHashValue(const Binding &Val) {
    return DenseMapInfo<nextcode::Type>::getHashValue(
  ...
  static bool isEqual(const Binding &LHS, const Binding &RHS) {
    ...
    if (DenseMapInfo<nextcode::TypeBase *>::isEqual(lhsTy, rhsTy))
      return true;

    // If either side is empty or tombstone, let's use pointer equality.
    {
      auto emptyTy = llvm::DenseMapInfo<nextcode::TypeBase *>::getEmptyKey();
      auto tombstoneTy =
          llvm::DenseMapInfo<nextcode::TypeBase *>::getTombstoneKey();
...
private:
  static Binding placeholderKey(nextcode::Type type) {
...
public:
  nextcode::SmallSetVector<PotentialBinding, 4> Bindings;
...
#endif // NEXTCODE_SEMA_CSBINDINGS_H
