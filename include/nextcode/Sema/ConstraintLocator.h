...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_SEMA_CONSTRAINTLOCATOR_H
#define NEXTCODE_SEMA_CONSTRAINTLOCATOR_H
...
public:
  TupleType(Type type)
    ...
    assert(type->getDesugaredType()->is<nextcode::TupleType>());
  }
...
public:
  PackType(Type type)
    ...
    assert(type->getDesugaredType()->is<nextcode::PackType>());
  }
...
class LocatorPathElt::PackExpansionType final
    : public StoredPointerElement<nextcode::PackExpansionType> {
public:
  PackExpansionType(nextcode::PackExpansionType *openedPackExpansionTy)
      : StoredPointerElement(PathElementKind::PackExpansionType,
                             openedPackExpansionTy) {
    assert(openedPackExpansionTy);
  }

  nextcode::PackExpansionType *getOpenedType() const { return getStoredPointer(); }
...
#endif // LLVM_NEXTCODE_SEMA_CONSTRAINTLOCATOR_H
