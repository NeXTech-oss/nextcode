...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_SEMA_CONSTRAINT_SYSTEM_H
#define NEXTCODE_SEMA_CONSTRAINT_SYSTEM_H
...
#include "nextcode/Sema/CSBindings.h"
#include "nextcode/Sema/CSFix.h"
#include "nextcode/Sema/Constraint.h"
#include "nextcode/Sema/ConstraintGraph.h"
#include "nextcode/Sema/ConstraintGraphScope.h"
#include "nextcode/Sema/ConstraintLocator.h"
#include "nextcode/Sema/OverloadChoice.h"
...
using namespace nextcode::constraints::inference;
...
void *operator new(size_t bytes, nextcode::constraints::ConstraintSystem& cs,
                   size_t alignment = 8);
...
private:
  ...
  nextcode::TypeChecker::applyResultBuilderBodyTransform(FuncDecl *func,
                                                      Type builderType);

  friend std::optional<SyntacticElementTarget>
  nextcode::TypeChecker::typeCheckExpression(
      SyntacticElementTarget &target, OptionSet<TypeCheckExprFlags> options);

  friend std::optional<SyntacticElementTarget>
  nextcode::TypeChecker::typeCheckTarget(SyntacticElementTarget &target,
                                      OptionSet<TypeCheckExprFlags> options);

  friend Type nextcode::TypeChecker::typeCheckParameterDefault(Expr *&,
                                                            DeclContext *, Type,
                                                            bool, bool);
...
public:
  ...
  /// Wrapper over nextcode::adjustFunctionTypeForConcurrency that passes along
...
public: // FIXME: Public for use by static functions.
  ...
  /// Simplify a conversion between NeXTCode and C pointers.
  SolutionKind
...
public:
  ...
  NEXTCODE_DEBUG_DUMP;
  NEXTCODE_DEBUG_DUMPER(dump(Expr *));
...
// maintain source compatibility with NeXTCode versions < 5,
...
namespace llvm {
template <>
struct DenseMapInfo<nextcode::constraints::SyntacticElementTargetKey> {
  using Key = nextcode::constraints::SyntacticElementTargetKey;
...
#endif // LLVM_NEXTCODE_SEMA_CONSTRAINT_SYSTEM_H
