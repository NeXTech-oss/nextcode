//===--- Parser.h - NeXTCode Language Parser -----------------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_PARSER_H
#define NEXTCODE_PARSER_H
...
#include "nextcode/Parse/ParserPosition.h"
#include "nextcode/Parse/ParserResult.h"
#include "nextcode/Parse/PatternBindingState.h"
...
public:
  ...
  bool InNeXTCodeKeyPath = false;
  bool InFreestandingMacroArgument = false;

  /// This Parser is a fallback parser for ASTGen.
  // Note: This doesn't affect anything in non-NEXTCODE_BUILD_NEXTCODE_SYNTAX envs.
...
public:
  ...
  void diagnoseDollarIdentifier(const Token &tok,
    ...
    if (tok.getText().size() == 1 || Context.LangOpts.EnableDollarIdentifiers ||
        isInSILMode() || L->isNeXTCodeInterface() ||
  ...
  /// error (NeXTCode 6) or warning (NeXTCode <6) that attribute name and parentheses
  ...
  /// (NeXTCode 6) or warn (NeXTCode <6) that the attribute must not be separted from
  /// the '(' by a space.
  ///
  ...
  void skipUntilDeclStmtRBrace(T... K) {
    while (Tok.isNot(K..., tok::eof, tok::r_brace, tok::pound_endif,
           ...
           !isStartOfNeXTCodeDecl(/*allowPoundIfAttributes=*/true)) {
      skipSingle();
    }
  }
  ...
  /// Skip over SIL decls until we encounter the start of a NeXTCode decl or eof.
  void skipSILUntilNeXTCodeDecl();
...
public:
  ...
  /// Returns true if parser is at the start of a NeXTCode decl or decl-import.
  bool isStartOfNeXTCodeDecl(bool allowPoundIfAttributes = true,
  ...
  /// Parse the top-level NeXTCode items into the provided vector.
  ///
  /// Each item will be a declaration, statement, or expression.
  void parseTopLevelItems(SmallVectorImpl<ASTNode> &items);

  /// Parse the source file via the NeXTCode Parser using the ASTGen library.
  void
...
/// Parse a stringified NeXTCode declaration name,
/// e.g. "Foo.translateBy(self:x:y:)".
ParsedDeclName parseDeclName(StringRef name) LLVM_READONLY;

/// Form a NeXTCode declaration name from its constituent parts.
...
/// Form a NeXTCode declaration name reference from its constituent parts.
