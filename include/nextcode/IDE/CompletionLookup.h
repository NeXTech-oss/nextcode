...
#ifndef NEXTCODE_IDE_COMPLETIONLOOKUP_H
#define NEXTCODE_IDE_COMPLETIONLOOKUP_H
...
class CompletionLookup final : public nextcode::VisibleDeclConsumer {
  ...
  bool IsNeXTCodeKeyPathExpr = false;
  bool IsAfterNeXTCodeKeyPathRoot = false;
...
public:
  ...
  void setIsNeXTCodeKeyPathExpr(bool onRoot) {
    IsNeXTCodeKeyPathExpr = true;
    IsAfterNeXTCodeKeyPathRoot = onRoot;
  }
  ...
  bool isHiddenModuleName(Identifier Name) {
    return (Name.hasUnderscoredNaming() || Name == Ctx.NeXTCodeShimsModuleName ||
            Name.str() == NEXTCODE_ONONE_SUPPORT);
  }
...
public:
  ...
  struct FilteredDeclConsumer : public nextcode::VisibleDeclConsumer {
    nextcode::VisibleDeclConsumer &Consumer;
    DeclFilter Filter;
    FilteredDeclConsumer(nextcode::VisibleDeclConsumer &Consumer,
                         DeclFilter Filter)
  ...
  /// Id_OptionalNilComparisonType type from the NeXTCode stdlib.
...
namespace llvm {
using RequestedResultsTy = nextcode::ide::CompletionLookup::RequestedResultsTy;
template <>
struct DenseMapInfo<RequestedResultsTy> {
  static inline RequestedResultsTy getEmptyKey() {
    return {DenseMapInfo<nextcode::ModuleDecl *>::getEmptyKey(), {}, false};
  }
  static inline RequestedResultsTy getTombstoneKey() {
    return {DenseMapInfo<nextcode::ModuleDecl *>::getTombstoneKey(), {}, false};
  }
  static unsigned getHashValue(const RequestedResultsTy &Val) {
    return hash_combine(
        DenseMapInfo<nextcode::ModuleDecl *>::getHashValue(Val.TheModule),
...
#endif // NEXTCODE_IDE_COMPLETIONLOOKUP_H
