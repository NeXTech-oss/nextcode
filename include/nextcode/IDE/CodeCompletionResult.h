...
#ifndef NEXTCODE_IDE_CODECOMPLETION_RESULT_H
#define NEXTCODE_IDE_CODECOMPLETION_RESULT_H

#include "nextcode/Basic/StringExtras.h"
#include "nextcode/IDE/CodeCompletionResultType.h"
#include "nextcode/IDE/CodeCompletionString.h"
...
namespace llvm {
template <> struct DenseMapInfo<nextcode::ide::CodeCompletionKeywordKind> {
  using Kind = nextcode::ide::CodeCompletionKeywordKind;
...
template <> struct DenseMapInfo<nextcode::ide::CodeCompletionLiteralKind> {
  using Kind = nextcode::ide::CodeCompletionLiteralKind;
  static Kind getEmptyKey() { return Kind(~0u); }
  static Kind getTombstoneKey() { return Kind(~1u); }
  static unsigned getHashValue(const Kind &Val) { return unsigned(Val); }
  static bool isEqual(const Kind &LHS, const Kind &RHS) { return LHS == RHS; }
};
template <> struct DenseMapInfo<nextcode::ide::CodeCompletionDeclKind> {
  using Kind = nextcode::ide::CodeCompletionDeclKind;
  static Kind getEmptyKey() { return Kind(~0u); }
  static Kind getTombstoneKey() { return Kind(~1u); }
  static unsigned getHashValue(const Kind &Val) { return unsigned(Val); }
  static bool isEqual(const Kind &LHS, const Kind &RHS) { return LHS == RHS; }
...
#endif // NEXTCODE_IDE_CODECOMPLETION_RESULT_H
