...
#ifndef NEXTCODE_THREADING_THREADLOCALSTORAGE_H
#define NEXTCODE_THREADING_THREADLOCALSTORAGE_H
...
#if NEXTCODE_THREADING_USE_RESERVED_TLS_KEYS
  ...
  once(
      ...
      [](void *ctx) {
        ...
        if (!tls_init(pinfo->k, pinfo->d))
          nextcode::threading::fatal("tls_init_once() failed to set destructor");
      },
      (void *)&info);
}
...
#endif // NEXTCODE_THREADING_USE_RESERVED_TLS_KEYS
  ...
  once(
      ...
      [](void *ctx) {
        ...
        if (!tls_alloc(pinfo->k, pinfo->d))
          nextcode::threading::fatal("tls_alloc_once() failed to allocate key");
      },
      (void *)&info);
}
#endif // !NEXTCODE_THREADING_NONE
...
//
// - On platforms that define NEXTCODE_THREAD_LOCAL, an object of this type
//   is declared with NEXTCODE_THREAD_LOCAL.  This makes the object
//   itself thread-local, and no internal support is required.
//
//   Note that this includes platforms that don't support threading,
//   for which NEXTCODE_THREAD_LOCAL is empty; thread-local declarations
//   then create an ordinary global.
//
// - On platforms that don't define NEXTCODE_THREAD_LOCAL, we have to simulate
//   thread-local storage.  Fortunately, all of these platforms (at least
//   for now) support pthread_getspecific or similar.
#ifdef NEXTCODE_THREAD_LOCAL
template <class T>
class ThreadLocal {
...
#else
// A wrapper around a TLS key that is lazily initialized using nextcode::once.
...
/// NEXTCODE_THREAD_LOCAL_TYPE(TYPE, KEY) - Declare a variable
/// to be a thread-local variable.  The declaration must have static
/// storage duration; it may be prefixed with "static".
///
/// For example
///
///   static NEXTCODE_THREAD_LOCAL_TYPE(int, NEXTCODE_RESERVED_TLS_KEY_T_9) frobble;
///
...
#ifdef NEXTCODE_THREAD_LOCAL
#define NEXTCODE_THREAD_LOCAL_TYPE(TYPE, KEY)                                     \
  NEXTCODE_THREAD_LOCAL nextcode::ThreadLocal<TYPE>
#elif NEXTCODE_THREADING_USE_RESERVED_TLS_KEYS
#define NEXTCODE_THREAD_LOCAL_TYPE(TYPE, KEY)                                     \
  nextcode::ThreadLocal<TYPE, nextcode::ConstantThreadLocalKey<KEY>>
#else
#define NEXTCODE_THREAD_LOCAL_TYPE(TYPE, KEY)                                     \
  nextcode::ThreadLocal<TYPE, nextcode::ThreadLocalKey>
#endif

#endif // NEXTCODE_THREADING_THREADLOCALSTORAGE_H
