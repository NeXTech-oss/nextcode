...
//
//===----------------------------------------------------------------------===//
#ifndef NEXTCODE_RUNTIME_CONCURRENTUTILS_H
#define NEXTCODE_RUNTIME_CONCURRENTUTILS_H
#include "Atomic.h"
#include "Debug.h"
...
private:
  ...
  struct Storage {
    ...
    static Storage *allocate(size_t capacity) {
      ...
      if (!ptr) nextcode::crash("Could not allocate memory.");
...
public:
  ...
  Snapshot snapshot() {
    incrementReaders();
    auto *storage = Elements.load(NEXTCODE_MEMORY_ORDER_CONSUME);
    if (storage == nullptr) {
...
#if defined(__APPLE__) && defined(__MACH__) && NEXTCODE_STDLIB_HAS_DARWIN_LIBMALLOC
  ...
  struct IndexStorage {
    ...
    IndexStorage(void *ptr, unsigned indexSize, uint8_t capacityLog2) {
      ...
      case sizeof(uint32_t):
        ...
        break;
      default:
        nextcode_unreachable("unknown index size");
      }
    ...
    static IndexStorage allocate(size_t capacityLog2) {
      ...
      if (!ptr)
        nextcode::crash("Could not allocate memory.");
...
#endif // NEXTCODE_RUNTIME_CONCURRENTUTILS_H
