...
#ifndef NEXTCODE_RUNTIME_ATOMICWAITQUEUE_H
#define NEXTCODE_RUNTIME_ATOMICWAITQUEUE_H
...
class AtomicWaitQueue {
  ...
  void release_locked() {
    if (referenceCount == 1) {
      nextcode_cxx_deleteObject(&asImpl());
    } else {
      referenceCount--;
    }
  }
...
public:
  ...
  public:
    ...
    ~Worker() {
      ...
      if (CurrentQueue) {
        ...
        nextcode_cxx_deleteObject(CurrentQueue);
      }
    }
  ...
  private:
    ...
    static Impl *createNewQueue(Args &&...args) {
      auto queue = nextcode_cxx_newObject<Impl>(std::forward<Args>(args)...);
