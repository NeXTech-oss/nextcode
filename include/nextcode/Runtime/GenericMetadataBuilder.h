...
#ifndef NEXTCODE_RUNTIME_GENERIC_METADATA_BUILDER_H
#define NEXTCODE_RUNTIME_GENERIC_METADATA_BUILDER_H

#include "nextcode/ABI/Metadata.h"
#include "nextcode/Basic/MathUtils.h"
#include "nextcode/Demangling/TypeLookupError.h"
...
public:
  ...
  NEXTCODE_FORMAT(2, 3) BuilderError(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);

    char *string = nullptr;
    nextcode_vasprintf(&string, fmt, args);
    if (string)
      errorString = string;
    else
...
public:
  ...
  /// lookup.
  BuilderErrorOr(const nextcode::TypeLookupError &error) {
...
class GenericMetadataBuilder {
  ...
  // Convenience aliases for a bunch of NeXTCode metadata types we use.
  template <typename T>
  using FullMetadata = nextcode::FullMetadata<T>;
  template <typename T, template <typename> class U>
  using ConstTargetMetadataPointer = nextcode::ConstTargetMetadataPointer<T, U>;
  using ClassDescriptor = nextcode::TargetClassDescriptor<Runtime>;
  using ClassMetadataType = nextcode::TargetClassMetadataType<Runtime>;
  using EnumDescriptor = nextcode::TargetEnumDescriptor<Runtime>;
  using EnumMetadata = nextcode::TargetEnumMetadata<Runtime>;
  using EnumValueWitnessTable = nextcode::TargetEnumValueWitnessTable<Runtime>;
  using GenericMetadataPattern = nextcode::TargetGenericMetadataPattern<Runtime>;
  using GenericValueMetadataPattern =
      nextcode::TargetGenericValueMetadataPattern<Runtime>;
  using Metadata = nextcode::TargetMetadata<Runtime>;
  using StructDescriptor = nextcode::TargetStructDescriptor<Runtime>;
  using StructMetadata = nextcode::TargetStructMetadata<Runtime>;
  using TypeContextDescriptor = nextcode::TargetTypeContextDescriptor<Runtime>;
  using TypeLayout = nextcode::TypeLayout;
  using ValueMetadata = nextcode::TargetValueMetadata<Runtime>;
  using ValueTypeDescriptor = nextcode::TargetValueTypeDescriptor<Runtime>;
  using ValueWitnessFlags =
      nextcode::TargetValueWitnessFlags<typename Runtime::StoredSize>;
  using ValueWitnessTable = nextcode::TargetValueWitnessTable<Runtime>;

  // Start of member variables.
  ReaderWriter &readerWriter;

  // Various functions and witness tables needed from the NeXTCode runtime. These
...
public:
  ...
  GenericMetadataBuilder(ReaderWriter &readerWriter)
      : readerWriter(readerWriter),
        pod_copy(readerWriter.getSymbolPointer("_nextcode_pod_copy")),
        pod_destroy(readerWriter.getSymbolPointer("_nextcode_pod_destroy")),
        pod_direct_initializeBufferWithCopyOfBuffer(
            readerWriter.getSymbolPointer(
                "_nextcode_pod_direct_initializeBufferWithCopyOfBuffer")),
        pod_indirect_initializeBufferWithCopyOfBuffer(
            readerWriter.getSymbolPointer(
                "_nextcode_pod_indirect_initializeBufferWithCopyOfBuffer")),
  ...
  BuilderErrorOr<std::monostate> initializeValueMetadataFromPattern(
    ...
    if (pattern->hasExtraDataPattern()) {
      ...
      // [pre-5.3-extra-data-zeroing] Before NeXTCode 5.3, the runtime did not
  ...
  int32_t getGenericArgumentOffset(
    ...
    if (auto classDescription = llvm::dyn_cast<ClassDescriptor>(description))
      nextcode_unreachable("Classes not yet supported.");
  ...
  initializeGenericMetadata(WritableData<FullMetadata<Metadata>> metadataBuffer,
                            nextcode::Demangle::NodePointer metadataMangleNode) {
  ...
  static constexpr nextcode::TargetTypeLayout<Runtime>
  getInitialLayoutForValueType() {
    nextcode::TargetValueWitnessFlags<typename Runtime::StoredSize> flags{};
...
#include "nextcode/ABI/ValueWitness.def"
  ...
  /// metadata.
  BuilderErrorOr<std::monostate>
  installCommonValueWitnesses(const nextcode::TargetTypeLayout<Runtime> &layout,
  ...
  initializeStructMetadata(WritableData<FullMetadata<Metadata>> metadataBuffer,
    ...
    for (unsigned i = 0; i != fields.size(); ++i) {
      ...
      auto mangledTypeName = nextcode::Demangle::makeSymbolicMangledNameStringRef(
...
#endif // NEXTCODE_RUNTIME_GENERIC_METADATA_BUILDER_H
