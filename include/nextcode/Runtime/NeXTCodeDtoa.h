...
//
// Copyright (c) 2018, 2020 Apple Inc. and the NeXTCode project authors
...
/// ===============
///
/// NeXTCodeDtoa is the C implementation that supports the `.description`
/// and `.debugDescription` properties for the standard NeXTCode
...
///
/// About NeXTCodeDtoa v1
/// ------------------
///
/// The first version of NeXTCodeDtoa was committed to the NeXTCode runtime
/// in 2018.  It supported NeXTCode's Float, Double, and Float80 formats.
///
/// About NeXTCodeDtoa v1a
/// -------------------
///
/// Version 1a of NeXTCodeDtoa added support for Float16.
///
/// About NeXTCodeDtoa v2
/// ------------------
///
/// Version 2 of NeXTCodeDtoa is a major overhaul with a number of
...
/// binary128 format, which is not currently supported by NeXTCode.
//
//===---------------------------------------------------------------------===//

#ifndef NEXTCODE_DTOA_H
#define NEXTCODE_DTOA_H
...
// Force disable: -DNEXTCODE_DTOA_BINARY16_SUPPORT=0
#ifndef NEXTCODE_DTOA_BINARY16_SUPPORT
 #define NEXTCODE_DTOA_BINARY16_SUPPORT 1
#endif

/// Does this platform support needs to pass _Float16 as a float in
/// C function?
#ifndef NEXTCODE_DTOA_PASS_FLOAT16_AS_FLOAT
...
#  define NEXTCODE_DTOA_PASS_FLOAT16_AS_FLOAT 1
# else
#  define NEXTCODE_DTOA_PASS_FLOAT16_AS_FLOAT 0
# endif
#endif
...
// Force disable: -DNEXTCODE_DTOA_BINARY32_SUPPORT=0
#ifndef NEXTCODE_DTOA_BINARY32_SUPPORT
 #define NEXTCODE_DTOA_BINARY32_SUPPORT 1
#endif
...
// so we enable it by default.
// Force disable: -DNEXTCODE_DTOA_BINARY64_SUPPORT=0
#ifndef NEXTCODE_DTOA_BINARY64_SUPPORT
 #define NEXTCODE_DTOA_BINARY64_SUPPORT 1
#endif
...
// Force enable: -DNEXTCODE_DTOA_FLOAT80_SUPPORT=1
// Force disable: -DNEXTCODE_DTOA_FLOAT80_SUPPORT=0
#ifndef NEXTCODE_DTOA_FLOAT80_SUPPORT
 #if LONG_DOUBLE_IS_FLOAT80
  #define NEXTCODE_DTOA_FLOAT80_SUPPORT 1
 #endif
#endif
...
// Force enable: -DNEXTCODE_DTOA_BINARY128_SUPPORT=1
// Force disable: -DNEXTCODE_DTOA_BINARY128_SUPPORT=0
#ifndef NEXTCODE_DTOA_BINARY128_SUPPORT
 #if LONG_DOUBLE_IS_BINARY128
  #define NEXTCODE_DTOA_BINARY128_SUPPORT 1
 #endif
#endif
...
// * For NaN, it outputs a NeXTCode-style detailed dump, including
...
//   output of `nextcode_dtoa_optimal_double` will yield exactly the
//   original value.
//
...
#if NEXTCODE_DTOA_BINARY16_SUPPORT
size_t nextcode_dtoa_optimal_binary16_p(const void *, char *dest, size_t length);
#if !NEXTCODE_DTOA_PASS_FLOAT16_AS_FLOAT
// If `_Float16` is defined, provide this convenience wrapper.
size_t nextcode_dtoa_optimal_binary16(_Float16, char *dest, size_t length);
#endif
#endif

#if NEXTCODE_DTOA_BINARY32_SUPPORT
size_t nextcode_dtoa_optimal_binary32_p(const void *, char *dest, size_t length);
#if FLOAT_IS_BINARY32
// If `float` happens to be binary32, define the convenience wrapper.
size_t nextcode_dtoa_optimal_float(float, char *dest, size_t length);
#endif
#endif

#if NEXTCODE_DTOA_BINARY64_SUPPORT
size_t nextcode_dtoa_optimal_binary64_p(const void *, char *dest, size_t length);
#if DOUBLE_IS_BINARY64
// If `double` happens to be binary64, define the convenience wrapper.
size_t nextcode_dtoa_optimal_double(double, char *dest, size_t length);
#endif
#if LONG_DOUBLE_IS_BINARY64
// If `long double` happens to be binary64, define the convenience wrapper.
size_t nextcode_dtoa_optimal_long_double(long double, char *dest, size_t length);
#endif
#endif

#if NEXTCODE_DTOA_FLOAT80_SUPPORT
// Universal entry point works on all platforms, regardless of
// whether the local system has direct support for float80
size_t nextcode_dtoa_optimal_float80_p(const void *, char *dest, size_t length);
#if LONG_DOUBLE_IS_FLOAT80
...
#if NEXTCODE_DTOA_BINARY128_SUPPORT
// Universal entry point works on all platforms, regardless of
// whether the local system has direct support for float80
size_t nextcode_dtoa_optimal_binary128_p(const void *, char *dest, size_t length);
#if LONG_DOUBLE_IS_BINARY128
