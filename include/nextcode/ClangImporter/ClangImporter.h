...
/// Clang AST to ClangImporter to import the type into NeXTCode.
...
/// Class that imports Clang modules into NeXTCode, mapping directly
/// from Clang ASTs over to NeXTCode ASTs.
class ClangImporter final : public ClangModuleLoader {
  friend class ClangModuleUnit;
  friend class NeXTCodeDeclSynthesizer;
...
public:
  ...
  ///
  /// \param nextcodePCHHash A hash of NeXTCode's various options in a compiler
  ...
  create(ASTContext &ctx,
         std::string nextcodePCHHash = "", DependencyTracker *tracker = nullptr,
  ...
  ///
  /// \param AllowMemoryCache Affects only loading serialized NeXTCode modules,
  ...
  /// or in the NeXTCode module cache). Delegates to clang for everything except
  ...
  /// of a binary NeXTCode module.
  ///
  /// \param moduleID the name of the NeXTCode module whose dependency
  ...
  /// and match the SDK deployment target against which NeXTCode modules are also
  /// built.
  ///
  /// In this case, we must use the NeXTCode compiler's OS version triple when
  ...
  /// When `-clang-target` is set, they are configured with the NeXTCode
  ...
  /// Get clang import creation cc1 args for nextcode explicit module build.
  std::vector<std::string> getNeXTCodeExplicitModuleDirectCC1Args() const;

  /// If we already imported a given decl successfully, return the corresponding
  /// NeXTCode decl as an Optional<Decl *>, but if we previously tried and failed
  ...
  /// Dump NeXTCode lookup tables.
  void dumpNeXTCodeLookupTables() const override;
  ...
  NeXTCodeLookupTable *findLookupTable(const clang::Module *clangModule) override;
...
/// to allow using them from NeXTCode.
