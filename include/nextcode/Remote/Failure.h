...
#ifndef NEXTCODE_REMOTE_FAILURE_H
#define NEXTCODE_REMOTE_FAILURE_H

#include "nextcode/Remote/RemoteAddress.h"
...
#include "nextcode/Remote/FailureKinds.def"
  };
...
public:
  ...
  std::string render() const {
    ...
    while (const char *next = std::strchr(text, '%')) {
      ...
      // but nextcode::remote is supposed to be a minimal, header-only library.
      case ArgStorageKind::Address: {
        result += '0';
        result += 'x';
...
#endif // NEXTCODE_REMOTE_FAILURE_H
