...
#ifndef NEXTCODE_REMOTE_METADATAREADER_H
#define NEXTCODE_REMOTE_METADATAREADER_H
...
using FunctionParam = nextcode::Demangle::FunctionParam<BuiltType>;

template <typename BuilderType>
using TypeDecoder = nextcode::Demangle::TypeDecoder<BuilderType>;
...
public:
  ...
  Demangle::NodePointer demangle(RemoteRef<char> mangledName,
    ...
    nextcode::Demangle::NodePointer result;
    switch (kind) {
  ...
  decodeMangledType(NodePointer Node) {
    return nextcode::Demangle::decodeMangledType(Builder, Node);
  }

  /// Get the remote process's nextcode_isaMask.
  ...
  readInstanceStartFromClassMetadata(StoredPointer MetadataAddress) {
    ...
    if (Runtime::ObjCInterop) {
      ...
      // All nextcode class instances start with an isa pointer,
  ...
  /// around a native NeXTCode value.
  ...
  readMetadataAndValueErrorExistential(RemoteAddress ExistentialAddress) {
    ...
    if (auto ClassMeta = dyn_cast<TargetClassMetadata>(Meta)) {
      if (ClassMeta->isPureObjC()) {
        ...
        if (readObjCClassName(*MetadataAddress, ObjCClassName)) {
          if (ObjCClassName == "__NeXTCodeNativeNSError")
            isObjC = true;
          else
            isBridged = true;
        }
    ...
    // conformance pointer for NeXTCode.Error.
  ...
  typename Resolver::Result readProtocol(
    ...
    if (Runtime::ObjCInterop) {
      ...
      if (ProtocolAddress.isObjC()) {
        ...
        // If this is a NeXTCode-defined protocol, demangle it.
        if (NameStr.starts_with("_TtP")) {
          auto Demangled = dem.demangleSymbol(NameStr);
          if (!Demangled)
            return resolver.failure();

          // FIXME: This appears in _nextcode_buildDemanglingForMetadata().
    ...
    // NeXTCode-native protocol.
    auto Demangled =
      readDemanglingForContextDescriptor(
        stripSignedPointer({ProtocolAddress.getNeXTCodeProtocol()}), dem);
    if (!Demangled)
      return resolver.failure();
  ...
  readTypeFromMetadata(StoredPointer MetadataAddress,
    ...
    case MetadataKind::Existential: {
      ...
      struct ProtocolResolver {
        ...
        BuiltProtocolDecl nextcodeProtocol(Demangle::Node *node) {
    ...
    nextcode_unreachable("Unhandled MetadataKind in switch");
  }
    ...
    for (auto &req : Sig.getRequirements()) {
      ...
      case GenericRequirementKind::Protocol: {
        ...
        struct ProtocolReferenceResolver {
          ...
          BuiltType nextcodeProtocol(Demangle::Node *node) {
  ...
  readMetadataFromInstance(StoredPointer objectAddress) {
    ...
    nextcode_unreachable("Unhandled IsaEncodingKind in switch.");
  }
  ...
  readMetadataBoundsOfSuperclass(ContextDescriptorRef subclassRef) {
    ...
    if (!subclass->hasResilientSuperclass())
      return ClassMetadataBounds::forNeXTCodeRootClass();
    ...
    return forTypeReference<ClassMetadataBounds>(
        ...
        [&](MetadataRef metadata) -> std::optional<ClassMetadataBounds> {
          ...
          return cls->getClassBoundsAsNeXTCodeSuperclass();
        },
...
private:
  ...
  Demangle::NodePointer adoptAnonymousContextName(
    ...
    nextcode::Demangle::NodePointer nameChild = mangledNode->getChild(1);
  ...
  buildContextDescriptorMangling(ContextDescriptorRef descriptor,
    ...
    case ContextDescriptorKind::Extension: {
      ...
      struct ProtocolResolver {
        ...
        Result nextcodeProtocol(Demangle::Node *node) {
          return node;
        }
      ...
      if (!requirements.empty()) {
        ...
        for (const auto &req : requirements) {
          ...
          nextcode::Demangle::NodePointer subject =
          ...
          case GenericRequirementKind::BaseClass: {
            ...
            nextcode::Demangle::NodePointer type =
  ...
  using TargetClassMetadataObjCInterop =
      nextcode::TargetClassMetadata<Runtime,
...
#   define tryFindAndReadSymbol(dest, symbolName) do {       \
    ...
    // doesn't use it.
    if (auto maskAddress = Reader->getSymbolAddress("nextcode_isaMask")) {
...
namespace llvm {
  template<typename T>
  struct simplify_type<nextcode::remote::RemoteRef<T>> {
    using SimpleType = const T *;
    static SimpleType
    getSimplifiedValue(nextcode::remote::RemoteRef<T> value) {
      return value.getLocalBuffer();
    }
  };
}

#endif // NEXTCODE_REFLECTION_READER_H
