//===--- ReflectionContext.h - NeXTCode Type Reflection Context ----*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_REFLECTION_REFLECTIONCONTEXT_H
#define NEXTCODE_REFLECTION_REFLECTIONCONTEXT_H
...
#include "nextcode/ABI/Enum.h"
#include "nextcode/ABI/ObjectFile.h"
#include "nextcode/Concurrency/Actor.h"
#include "nextcode/Remote/MemoryReader.h"
#include "nextcode/Remote/MetadataReader.h"
#include "nextcode/RemoteInspection/DescriptorFinder.h"
#include "nextcode/RemoteInspection/GenericMetadataCacheEntry.h"
#include "nextcode/RemoteInspection/Records.h"
#include "nextcode/RemoteInspection/RuntimeInternals.h"
#include "nextcode/RemoteInspection/TypeLowering.h"
#include "nextcode/RemoteInspection/TypeRef.h"
...
// The NeXTCode runtime can be built in two ways: with or without
// NEXTCODE_CONCURRENCY_ENABLE_PRIORITY_ESCALATION enabled. In order to decode the
// lock used in a runtime with priority escalation enabled, we need inline
// functions from dispatch/nextcode_concurrency_private.h. If we don't have that
...
// combinations:
//
//      Runtime        | nextcode_concurrency_private.h | task running info
// --------------------+-----------------------------+------------------
...
#if __has_include(<dispatch/nextcode_concurrency_private.h>)
...
using nextcode::remote::MemoryReader;
using nextcode::remote::RemoteAddress;
...
public:
  ...
  std::optional<uint32_t> readMachOSections(
   ...
   // Find the load command offset.
    ...
    NeXTCodeObjectFileFormatMachO ObjectFileFormat;
  ...
  std::optional<uint32_t> readPECOFFSections(
    ...
    NeXTCodeObjectFileFormatCOFF ObjectFileFormat;
  ...
  std::optional<uint32_t> readELFSections(
    ...
    // `SHF_GNU_RETAIN` flag.  gold, presently, does not.  The NeXTCode
    ...
    // sections.  However, `nextcodert.o` contains declarations of the
    ...
    // (the retained sections will still exist because of `nextcodert.o`,
    ...
    NeXTCodeObjectFileFormatELF ObjectFileFormat;
  ...
  /// Adds an image using the FindSection closure to find the nextcode metadata
  ...
  std::optional<std::string> iterateConformances(
    ...
    std::string ConformancesPointerName =
        "_nextcode_debug_protocolConformanceStatePointer";
...
#include "../../../stdlib/public/runtime/MetadataAllocatorTags.def"
  ...
  std::optional<std::string> iterateMetadataAllocations(
    ...
    std::string IterationEnabledName =
        "_nextcode_debug_metadataAllocationIterationEnabled";
    std::string AllocationPoolPointerName =
        "_nextcode_debug_allocationPoolPointer";
  ...
  std::optional<std::string> iterateMetadataAllocationBacktraces(
    ...
    std::string BacktraceListName =
        "_nextcode_debug_metadataAllocationBacktraceList";
...
private:
  void setIsRunning(
    ...
    // the nextcode_concurrency_private.h header needed to decode the running
...
#else
    // The target runtime was built with priority escalation but we don't have
    // the nextcode_concurrency_private.h header needed to decode the lock.
    return {false, 0};
#endif
  }
  ...
  asyncTaskInfo(StoredPointer AsyncTaskPtr, unsigned ChildTaskLimit,
    ...
    nextcode::JobFlags JobFlags(AsyncTaskObj->Flags);
  ...
  void loadTargetPointers() {
    ...
    target_non_future_adapter =
        getFunc("_nextcode_concurrency_debug_non_future_adapter");
    target_future_adapter = getFunc("_nextcode_concurrency_debug_future_adapter");
    target_task_wait_throwing_resume_adapter =
        getFunc("_nextcode_concurrency_debug_task_wait_throwing_resume_adapter");
    target_task_future_wait_resume_adapter =
        getFunc("_nextcode_concurrency_debug_task_future_wait_resume_adapter");
    auto supportsPriorityEscalationAddr = getReader().getSymbolAddress(
        "_nextcode_concurrency_debug_supportsPriorityEscalation");
  ...
  bool isMetadataSourceReady(const MetadataSource *MS,
    ...
    nextcode_unreachable("Unhandled MetadataSourceKind in switch.");
  }
...
#endif // NEXTCODE_REFLECTION_REFLECTIONCONTEXT_H
