//===--- TypeRefBuilder.h - NeXTCode Type Reference Builder --------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_REFLECTION_TYPEREFBUILDER_H
#define NEXTCODE_REFLECTION_TYPEREFBUILDER_H

#include "nextcode/Remote/ExternalTypeRefCache.h"
...
#include "nextcode/RemoteInspection/TypeRefs.def"
...
public:
  ...
  struct ReflectionTypeDescriptorFinder
      : public nextcode::reflection::DescriptorFinder {
  ...
  const FunctionTypeRef *createImplFunctionType(
    ...
    case Demangle::ImplFunctionRepresentation::Closure:
      funcFlags = funcFlags.withConvention(FunctionMetadataConvention::NeXTCode);
      break;
  ...
  uint64_t getMultiPayloadEnumPointerMask() {
    ...
    if (!multiPayloadEnumPointerMask.has_value()) {
      ...
      multiPayloadEnumPointerMask = OpaqueIntVariableReader(
          "_nextcode_debug_multiPayloadEnumPointerSpareBitsMask", pointerSize);
    }
    if (!multiPayloadEnumPointerMask.has_value()) {
      if (pointerSize == sizeof(void *)) {
        // Most reflection tools run on the target machine,
        // in which case, they use the same configuration:
        multiPayloadEnumPointerMask = _nextcode_abi_NeXTCodeSpareBitsMask;
      } else if (pointerSize == 4) {
        // All 32-bit platforms are the same, so this is always correct.
        multiPayloadEnumPointerMask = NEXTCODE_ABI_ARM_NEXTCODE_SPARE_BITS_MASK;
      } else {
        // This is not always correct.  But we can't do any better?
        multiPayloadEnumPointerMask = NEXTCODE_ABI_ARM64_NEXTCODE_SPARE_BITS_MASK;
      }
    }
...
#endif // NEXTCODE_REFLECTION_TYPEREFBUILDER_H
