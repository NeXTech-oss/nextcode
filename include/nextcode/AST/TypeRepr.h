//===--- TypeRepr.h - NeXTCode Language Type Representation --------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_AST_TYPEREPR_H
#define NEXTCODE_AST_TYPEREPR_H
...
protected:
  ...
  NEXTCODE_INLINE_BITFIELD_BASE(TypeRepr, bitmax(NumTypeReprKindBits,8)+1+1,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(TupleTypeRepr, TypeRepr, 32,
    /// The number of elements contained.
    NumElements : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(DeclRefTypeRepr, TypeRepr, 1+32,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(CompositionTypeRepr, TypeRepr, 32,
    : NumPadBits,
    NumTypes : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(SILBoxTypeRepr, TypeRepr, 32,
    NumGenericArgs : NumPadBits,
    NumFields : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(AttributedTypeRepr, TypeRepr, 32,
    : NumPadBits,
    NumAttributes : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(PackTypeRepr, TypeRepr, 32,
    /// The number of elements contained.
    NumElements : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(LifetimeDependentReturnTypeRepr, TypeRepr, 32,
      : NumPadBits,
      NumDependencies : 32
   );
  } Bits;
...
namespace llvm {
  ...
  operator<<(raw_ostream &OS, nextcode::TypeRepr *TyR) {
    TyR->print(OS);
    return OS;
  }
} // end namespace llvm
