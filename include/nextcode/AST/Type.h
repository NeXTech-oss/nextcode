//===--- Type.h - Value objects for NeXTCode and SIL types ---------*- C++ -*-===//
//
/*
...
// NeXTCode and SIL types -- including tuple types, function types and more -- is
// defined in Types.h.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_TYPE_H
#define NEXTCODE_TYPE_H
...
/// Enumeration describing foreign languages to which NeXTCode may be
/// bridged.
...
namespace llvm {
  ...
  operator<<(raw_ostream &OS, nextcode::Type Ty) {
    Ty.print(OS);
    return OS;
  }

  // A Type casts like a TypeBase*.
  template<> struct simplify_type<const ::nextcode::Type> {
    typedef ::nextcode::TypeBase *SimpleType;
    static SimpleType getSimplifiedValue(const ::nextcode::Type &Val) {
      return Val.getPointer();
    }
  };
  template<> struct simplify_type< ::nextcode::Type>
    : public simplify_type<const ::nextcode::Type> {};
  
  // Type hashes just like pointers.
  template<> struct DenseMapInfo<nextcode::Type> {
    static nextcode::Type getEmptyKey() {
      return llvm::DenseMapInfo<nextcode::TypeBase*>::getEmptyKey();
    }
    static nextcode::Type getTombstoneKey() {
      return llvm::DenseMapInfo<nextcode::TypeBase*>::getTombstoneKey();
    }
    static unsigned getHashValue(nextcode::Type Val) {
      return DenseMapInfo<nextcode::TypeBase*>::getHashValue(Val.getPointer());
    }
    static bool isEqual(nextcode::Type LHS, nextcode::Type RHS) {
      return LHS.getPointer() == RHS.getPointer();
    }
  };
  template<> struct DenseMapInfo<nextcode::CanType>
    : public DenseMapInfo<nextcode::Type> {
    static nextcode::CanType getEmptyKey() {
      return nextcode::CanType(llvm::DenseMapInfo<nextcode::
                              TypeBase*>::getEmptyKey());
    }
    static nextcode::CanType getTombstoneKey() {
      return nextcode::CanType(llvm::DenseMapInfo<nextcode::
  ...
  template<>
  struct PointerLikeTypeTraits<nextcode::Type> {
  public:
    static inline void *getAsVoidPointer(nextcode::Type I) {
      return (void*)I.getPointer();
    }
    static inline nextcode::Type getFromVoidPointer(void *P) {
      return (nextcode::TypeBase*)P;
    }
    enum { NumLowBitsAvailable = nextcode::TypeAlignInBits };
  };
  
  template<>
  struct PointerLikeTypeTraits<nextcode::CanType> :
    public PointerLikeTypeTraits<nextcode::Type> {
  public:
    static inline nextcode::CanType getFromVoidPointer(void *P) {
      return nextcode::CanType((nextcode::TypeBase*)P);
    }
  };
} // end namespace llvm
