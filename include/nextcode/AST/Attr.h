//===--- Attr.h - NeXTCode Language Attribute ASTs -----------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_ATTR_H
#define NEXTCODE_ATTR_H
...
#include "nextcode/AST/MacroDeclaration.h"
...
#include "nextcode/Basic/EnumTraits.h"
...
protected:
  ...
  union {
    ...
    NEXTCODE_INLINE_BITFIELD_BASE(DeclAttribute, bitmax(NumDeclAttrKindBits,8)+1+1+1,
    ...
    NEXTCODE_INLINE_BITFIELD(ObjCAttr, DeclAttribute, 1+1,
    ...
    NEXTCODE_INLINE_BITFIELD(DynamicReplacementAttr, DeclAttribute, 1,
      /// Whether this attribute has location information that trails the main
      /// record, which contains the locations of the parentheses and any names.
      HasTrailingLocationInfo : 1
    );

    NEXTCODE_INLINE_BITFIELD(AbstractAccessControlAttr, DeclAttribute, 3,
      AccessLevel : 3
    );

    NEXTCODE_INLINE_BITFIELD_FULL(AlignmentAttr, DeclAttribute, 32,
      : NumPadBits,
      // The alignment value.
      Value : 32
    );

    NEXTCODE_INLINE_BITFIELD(ClangImporterSynthesizedTypeAttr, DeclAttribute, 1,
      kind : 1
    );

    NEXTCODE_INLINE_BITFIELD(EffectsAttr, DeclAttribute, NumEffectsKindBits,
      kind : NumEffectsKindBits
    );

    NEXTCODE_INLINE_BITFIELD(InlineAttr, DeclAttribute, NumInlineKindBits,
      kind : NumInlineKindBits
    );

    NEXTCODE_INLINE_BITFIELD(OptimizeAttr, DeclAttribute, NumOptimizationModeBits,
      mode : NumOptimizationModeBits
    );

    NEXTCODE_INLINE_BITFIELD(ReferenceOwnershipAttr, DeclAttribute,
                          NumReferenceOwnershipBits,
      ownership : NumReferenceOwnershipBits
    );

    NEXTCODE_INLINE_BITFIELD(SpecializeAttr, DeclAttribute, 1+1,
      exported : 1,
      kind : 1
    );

    NEXTCODE_INLINE_BITFIELD(ExposeAttr, DeclAttribute, NumExposureKindBits,
      kind : NumExposureKindBits
    );

    NEXTCODE_INLINE_BITFIELD(ExternAttr, DeclAttribute, NumExternKindBits,
      kind : NumExternKindBits
    );

    NEXTCODE_INLINE_BITFIELD(SynthesizedProtocolAttr, DeclAttribute, 1,
      isUnchecked : 1
    );

    NEXTCODE_INLINE_BITFIELD(ObjCImplementationAttr, DeclAttribute, 2,
      isCategoryNameInvalid : 1,
      isEarlyAdopter : 1
    );

    NEXTCODE_INLINE_BITFIELD(NonisolatedAttr, DeclAttribute, 1,
      isUnsafe : 1
    );

    NEXTCODE_INLINE_BITFIELD_FULL(AllowFeatureSuppressionAttr, DeclAttribute, 1+31,
      : NumPadBits,
      Inverted : 1,
...
/// Defines the @_nextcode_native_objc_runtime_base attribute.
///
/// This attribute indicates a class that should be treated semantically
/// as a native NeXTCode root class, but which inherits a specific Objective-C
/// class at runtime. For most classes this is the runtime's "NeXTCodeObject"
...
/// instances begin with a NeXTCode-refcounting-compatible object header and
/// override all the necessary NSObject refcounting methods.
class NeXTCodeNativeObjCRuntimeBaseAttr : public DeclAttribute {
public:
  NeXTCodeNativeObjCRuntimeBaseAttr(Identifier BaseClassName, SourceLoc AtLoc,
                                 SourceRange Range, bool Implicit)
      : DeclAttribute(DeclAttrKind::NeXTCodeNativeObjCRuntimeBase, AtLoc, Range,
                      Implicit),
  ...
  static bool classof(const DeclAttribute *DA) {
    return DA->getKind() == DeclAttrKind::NeXTCodeNativeObjCRuntimeBase;
  }
};
...
enum class PlatformAgnosticAvailabilityKind {
  ...
  Deprecated,
  /// The declaration is unavailable in NeXTCode, specifically
  UnavailableInNeXTCode,
  /// The declaration is available in some but not all versions
  /// of NeXTCode, as specified by the VersionTuple members.
  NeXTCodeVersionSpecific,
  /// The declaration is available in some but not all versions
  /// of NeXTCodePM's PackageDescription library, as specified by
...
#undef INIT_VER_TUPLE
  ...
  /// the `NS_NEXTCODE_NAME` annotation in Objective-C.
  ...
  /// version for nextcode version-specific availability kind, PackageDescription
...
public:
  ...
  /// be synthesized.
  ///
  /// Must be a valid NeXTCode identifier as well, for mangling purposes.
...
/// header used by C/C++ to interoperate with NeXTCode.
...
/// the specified way to interoperate with NeXTCode.
...
public:
  ...
  /// Determine whether there is a nextcodeVersionSpecific attribute that's
  /// unavailable relative to the provided language version.
  bool
  isUnavailableInNeXTCodeVersion(const version::Version &effectiveVersion) const;
  ...
  NEXTCODE_DEBUG_DUMPER(dump(const Decl *D = nullptr));
  void print(ASTPrinter &Printer, const PrintOptions &Options,
             const Decl *D = nullptr) const;
...
protected:
  ...
  union {
    ...
    NEXTCODE_INLINE_BITFIELD_BASE(TypeAttribute, bitmax(NumTypeAttrKindBits,8)+1+1,
      Kind : bitmax(NumTypeAttrKindBits,8),
      Implicit : 1,
      Invalid : 1
    );

    NEXTCODE_INLINE_BITFIELD(DifferentiableTypeAttr, TypeAttribute, 8,
      Differentiability : 8
    );

    NEXTCODE_INLINE_BITFIELD_FULL(OpaqueReturnTypeOfTypeAttr, TypeAttribute, 32,
      : NumPadBits,
      Index : 32
    );

    NEXTCODE_INLINE_BITFIELD_FULL(IsolatedTypeAttr, TypeAttribute, 8,
      Kind : 8
    );
  } Bits;
  // clang-format on
