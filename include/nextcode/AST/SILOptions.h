//===--- SILOptions.h - NeXTCode Language SILGen and SIL options ---*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_AST_SILOPTIONS_H
#define NEXTCODE_AST_SILOPTIONS_H
...
public:
  ...
  /// Are we building in embedded NeXTCode mode?
  bool EmbeddedNeXTCode = false;

  /// Are we building in embedded NeXTCode + -no-allocations?
  bool NoAllocations = false;

  /// Should we use the experimental NeXTCode based closure-specialization
  /// optimization pass instead of the existing C++ one.
  bool EnableExperimentalNeXTCodeBasedClosureSpecialization = false;
