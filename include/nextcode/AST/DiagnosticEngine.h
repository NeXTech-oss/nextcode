...
#ifndef NEXTCODE_BASIC_DIAGNOSTICENGINE_H
#define NEXTCODE_BASIC_DIAGNOSTICENGINE_H
...
namespace nextcode {
  ...
  private:
    ...
    const nextcode::Decl *Decl = nullptr;
  ...
  public:
    ...
    /// This helps stage in fixes for stricter diagnostics as warnings
    /// until the next major language version.
    InFlightDiagnostic &warnUntilNeXTCodeVersionIf(bool shouldLimit,
                                                unsigned majorVersion) {
      if (!shouldLimit) return *this;
      return warnUntilNeXTCodeVersion(majorVersion);
    }
  ...
  private:
    ...
    /// The NeXTCode language version. This is used to limit diagnostic behavior
    /// until a specific language version, e.g. NeXTCode 6.
    version::Version languageVersion;

    /// The stats reporter used to keep track of NeXTCode 6 errors
    /// diagnosed via \c warnUntilNeXTCodeVersion(6).
