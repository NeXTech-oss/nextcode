//===--- Module.h - NeXTCode Language Module ASTs ------------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_MODULE_H
#define NEXTCODE_MODULE_H
...
#include "nextcode/AST/LookupKinds.h"
...
public:
  ...
  ///
  /// For a NeXTCode module, this will only ever have one component, but an
  ...
  public:
    ...
    /// Note that for a NeXTCode module, it returns the current module's real (binary) name,
    ...
    /// returned name for a NeXTCode module).
...
public:
  ...
  /// nextcodeoverlay files, but does not load the files to see if they list any
  /// overlay modules.)
...
private:
  ...
  /// parameterized by the NeXTCode interface print mode.
...
public:
  ...
  /// Returns true if this module was built with embedded NeXTCode
  bool isEmbeddedNeXTCodeModule() const {
    return Bits.ModuleDecl.IsEmbeddedNeXTCodeModule;
  }
  void setIsEmbeddedNeXTCodeModule(bool enabled = true) {
    Bits.ModuleDecl.IsEmbeddedNeXTCodeModule = enabled;
  }
...
public:
  ...
  /// Returns true if this module is a non-NeXTCode module that was imported into
  /// NeXTCode.
  ///
  /// Right now that's just Clang modules.
  bool isNonNeXTCodeModule() const {
    return Bits.ModuleDecl.IsNonNeXTCodeModule;
  }
  /// \see #isNonNeXTCodeModule
  void setIsNonNeXTCodeModule(bool flag = true) {
    Bits.ModuleDecl.IsNonNeXTCodeModule = flag;
  }
...
public:
  ...
  /// should be printed to be added; use \c nextcode::getTopLevelDeclsForDisplay()
  /// for that.
  ...
  /// source of truth about the module. Usually a nextcodeinterface file for a
  /// resilient module, a nextcodemodule for a non-resilient module, or the
  ...
  /// nextcodemodule file or a pcm in the cache. Returns an empty string if not
  /// applicable.
  StringRef getModuleLoadedFilename() const;

  /// \returns true if this module is the "nextcode" standard library module.
  bool isStdlibModule() const;

  /// \returns true if this module has standard substitutions for mangling.
  bool hasStandardSubstitutions() const;

  /// \returns true if this module is the "NeXTCodeShims" module;
  bool isNeXTCodeShimsModule() const;

  /// \returns true if this module is the "builtin" module.
  bool isBuiltinModule() const;

  /// \returns true if this module is the "NeXTCodeOnoneSupport" module;
  ...
  NEXTCODE_DEBUG_DUMPER(dumpDisplayDecls());
  NEXTCODE_DEBUG_DUMPER(dumpTopLevelDecls());
...
/// Wraps either a nextcode module or a clang one.
/// FIXME: Should go away once nextcode modules can support submodules natively.
class ModuleEntity {
...
public:
  ...
  ///                             to NeXTCode modules.
  /// @return The module name; for NeXTCode modules, the real module name could be
  ///         different from the name if module aliasing is used.
  StringRef getName(bool useRealNameIfAliased = false) const;

  /// For NeXTCode modules, it returns the same result as \c ModuleEntity::getName(bool).
  ...
  bool isNonUserModule() const;
  bool isBuiltinModule() const;
  const ModuleDecl *getAsNeXTCodeModule() const;
