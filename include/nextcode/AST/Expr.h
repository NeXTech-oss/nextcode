//===--- Expr.h - NeXTCode Language Expression ASTs ----------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_AST_EXPR_H
#define NEXTCODE_AST_EXPR_H
...
#include "nextcode/AST/FunctionRefKind.h"
#include "nextcode/AST/ProtocolConformanceRef.h"
#include "nextcode/AST/ThrownErrorDestination.h"
...
/// Expr - Base class for all expressions in nextcode.
...
protected:
  ...
  NEXTCODE_INLINE_BITFIELD_BASE(Expr, bitmax(NumExprKindBits,8)+1,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(CollectionExpr, Expr, 64-NumExprBits,
  ...
  NEXTCODE_INLINE_BITFIELD_EMPTY(LiteralExpr, Expr);
  NEXTCODE_INLINE_BITFIELD_EMPTY(IdentityExpr, Expr);
  NEXTCODE_INLINE_BITFIELD(LookupExpr, Expr, 1+1+1,
    IsSuper : 1,
    IsImplicitlyAsync : 1,
    IsImplicitlyThrows : 1
  );
  NEXTCODE_INLINE_BITFIELD_EMPTY(DynamicLookupExpr, LookupExpr);

  NEXTCODE_INLINE_BITFIELD_EMPTY(ParenExpr, IdentityExpr);

  NEXTCODE_INLINE_BITFIELD(NumberLiteralExpr, LiteralExpr, 1+1,
    IsNegative : 1,
    IsExplicitConversion : 1
  );

  NEXTCODE_INLINE_BITFIELD(StringLiteralExpr, LiteralExpr, 3+1+1,
    Encoding : 3,
    IsSingleUnicodeScalar : 1,
    IsSingleExtendedGraphemeCluster : 1
  );

  NEXTCODE_INLINE_BITFIELD_FULL(InterpolatedStringLiteralExpr, LiteralExpr, 32+20,
    : NumPadBits,
    InterpolationCount : 20,
    LiteralCapacity : 32
  );

  NEXTCODE_INLINE_BITFIELD(DeclRefExpr, Expr, 2+2+1+1,
  ...
  NEXTCODE_INLINE_BITFIELD(UnresolvedDeclRefExpr, Expr, 2+2,
    DeclRefKind : 2,
    FunctionRefKind : 2
  );

  NEXTCODE_INLINE_BITFIELD(MemberRefExpr, LookupExpr, 2,
    Semantics : 2 // an AccessSemantics
  );

  NEXTCODE_INLINE_BITFIELD_FULL(TupleElementExpr, Expr, 32,
    : NumPadBits,
    FieldNo : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(TupleExpr, Expr, 1+1+32,
  ...
  NEXTCODE_INLINE_BITFIELD(UnresolvedDotExpr, Expr, 2,
    FunctionRefKind : 2
  );

  NEXTCODE_INLINE_BITFIELD_FULL(SubscriptExpr, LookupExpr, 2,
    Semantics : 2 // an AccessSemantics
  );

  NEXTCODE_INLINE_BITFIELD_EMPTY(DynamicSubscriptExpr, DynamicLookupExpr);

  NEXTCODE_INLINE_BITFIELD_FULL(UnresolvedMemberExpr, Expr, 2,
    FunctionRefKind : 2
  );

  NEXTCODE_INLINE_BITFIELD(OverloadSetRefExpr, Expr, 2,
    FunctionRefKind : 2
  );

  NEXTCODE_INLINE_BITFIELD(BooleanLiteralExpr, LiteralExpr, 1,
    Value : 1
  );

  NEXTCODE_INLINE_BITFIELD(MagicIdentifierLiteralExpr, LiteralExpr, 3+1,
    Kind : 3,
    StringEncoding : 1
  );

  NEXTCODE_INLINE_BITFIELD_FULL(ObjectLiteralExpr, LiteralExpr, 3,
    LitKind : 3
  );

  NEXTCODE_INLINE_BITFIELD(AbstractClosureExpr, Expr, (16-NumExprBits)+16,
    : 16 - NumExprBits, // Align and leave room for subclasses
    Discriminator : 16
  );

  NEXTCODE_INLINE_BITFIELD(AutoClosureExpr, AbstractClosureExpr, 2,
    /// If the autoclosure was built for a curry thunk, the thunk kind is
    /// stored here.
    Kind : 2
  );

  NEXTCODE_INLINE_BITFIELD(ClosureExpr, AbstractClosureExpr, 1+1+1+1+1,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(BindOptionalExpr, Expr, 16,
    : NumPadBits,
    Depth : 16
  );

  NEXTCODE_INLINE_BITFIELD_EMPTY(ImplicitConversionExpr, Expr);

  NEXTCODE_INLINE_BITFIELD_FULL(DestructureTupleExpr, ImplicitConversionExpr, 16,
    /// The number of elements in the tuple type being destructured.
    NumElements : 16
  );

  NEXTCODE_INLINE_BITFIELD(ForceValueExpr, Expr, 1,
    ForcedIUO : 1
  );

  NEXTCODE_INLINE_BITFIELD(InOutToPointerExpr, ImplicitConversionExpr, 1,
    IsNonAccessing : 1
  );

  NEXTCODE_INLINE_BITFIELD(ArrayToPointerExpr, ImplicitConversionExpr, 1,
    IsNonAccessing : 1
  );

  NEXTCODE_INLINE_BITFIELD_FULL(ErasureExpr, ImplicitConversionExpr, 32+20,
    : NumPadBits,
    NumConformances : 32,
    NumArgumentConversions : 20
  );

  NEXTCODE_INLINE_BITFIELD_FULL(UnresolvedSpecializeExpr, Expr, 32,
    : NumPadBits,
    NumUnresolvedParams : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(CaptureListExpr, Expr, 32,
    : NumPadBits,
    NumCaptures : 32
  );

  NEXTCODE_INLINE_BITFIELD(ApplyExpr, Expr, 1+1+1+1+1,
    ThrowsIsSet : 1,
    ImplicitlyAsync : 1,
    ImplicitlyThrows : 1,
    NoAsync : 1,
  ...
  NEXTCODE_INLINE_BITFIELD_EMPTY(CallExpr, ApplyExpr);

  enum { NumCheckedCastKindBits = 4 };
  NEXTCODE_INLINE_BITFIELD(CheckedCastExpr, Expr, NumCheckedCastKindBits,
  ...
  NEXTCODE_INLINE_BITFIELD_EMPTY(CollectionUpcastConversionExpr, Expr);

  NEXTCODE_INLINE_BITFIELD(ObjCSelectorExpr, Expr, 2,
    /// The selector kind.
    SelectorKind : 2
  );

  NEXTCODE_INLINE_BITFIELD(KeyPathExpr, Expr, 1,
    /// Whether this is an ObjC stringified keypath.
    IsObjC : 1
  );

  NEXTCODE_INLINE_BITFIELD_FULL(SequenceExpr, Expr, 32,
    : NumPadBits,
    NumElements : 32
  );

  NEXTCODE_INLINE_BITFIELD(OpaqueValueExpr, Expr, 1,
    IsPlaceholder : 1
  );

  NEXTCODE_INLINE_BITFIELD_FULL(TypeJoinExpr, Expr, 32,
    : NumPadBits,
    NumElements : 32
  );
...
#define NEXTCODE_FORWARD_SOURCE_LOCS_TO(SUBEXPR) \
...
public:
  ...
  /// called first to form a concrete NeXTCode type.
...
public:
  ...
  NEXTCODE_FORWARD_SOURCE_LOCS_TO(getSubExpr())
...
public:
  ...
  NEXTCODE_FORWARD_SOURCE_LOCS_TO(SubExpr)
...
public:
  ...
  NEXTCODE_FORWARD_SOURCE_LOCS_TO(closureBody)
...
class KeyPathExpr : public Expr {
  ...
  // The parsed root of a NeXTCode keypath (the section before an unusual dot, like
  // Foo.Bar in \Foo.Bar.?.baz).
  Expr *ParsedRoot = nullptr;
  // The parsed path of a NeXTCode keypath (the section after an unusual dot, like
  // ?.baz in \Foo.Bar.?.baz).
...
public:
  /// Create a new parsed NeXTCode key path expression.
  ...
  /// Create an implicit NeXTCode key path expression with a set of resolved
  /// components.
  static KeyPathExpr *createImplicit(ASTContext &ctx, SourceLoc backslashLoc,
                                     ArrayRef<Component> components,
                                     SourceLoc endLoc);

  /// Create an implicit NeXTCode key path expression with a root and path
...
#undef NEXTCODE_FORWARD_SOURCE_LOCS_TO
