//===--- Types.h - NeXTCode Language Type ASTs ---------------------*- C++ -*-===//
//
/*
...
//===----------------------------------------------------------------------===//
//
// This file defines the TypeBase class and subclasses, which describe the NeXTCode
// and SIL ASTs. See also: Type.h.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_TYPES_H
#define NEXTCODE_TYPES_H
...
enum class TypeMatchFlags {
  ...
  /// to be non-escaping, but NeXTCode currently does not.
...
/// Base class for all types which describe the NeXTCode and SIL ASTs.
///
...
protected:
  ...
  NEXTCODE_INLINE_BITFIELD_BASE(TypeBase, bitmax(NumTypeKindBits,8) +
  ...
  NEXTCODE_INLINE_BITFIELD(ErrorType, TypeBase, 1,
    /// Whether there is an original type.
    HasOriginalType : 1
  );

  NEXTCODE_INLINE_BITFIELD(SugarType, TypeBase, 1,
    HasCachedType : 1
  );

  NEXTCODE_INLINE_BITFIELD_EMPTY(ParenType, SugarType);

  NEXTCODE_INLINE_BITFIELD_FULL(AnyFunctionType, TypeBase, NumAFTExtInfoBits+1+1+1+1+16,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(ArchetypeType, TypeBase, 1+1+16,
    HasSuperclass : 1,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(TypeVariableType, TypeBase, 7+29,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(ErrorUnionType, TypeBase, 32,
    // Number of terms in the union.
    NumTerms : 32
  );
    
  NEXTCODE_INLINE_BITFIELD(SILFunctionType, TypeBase, NumSILExtInfoBits+1+4+1+2+1+1+1,
  ...
  NEXTCODE_INLINE_BITFIELD(AnyMetatypeType, TypeBase, 2,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(ProtocolCompositionType, TypeBase, 1+32,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(ParameterizedProtocolType, TypeBase, 32,
    /// The number of type arguments.
    ArgCount : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(TupleType, TypeBase, 32,
    : NumPadBits,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(PackType, TypeBase, 32,
    : NumPadBits,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(SILPackType, TypeBase, 1+32,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(BoundGenericType, TypeBase, 32,
    : NumPadBits,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(TypeAliasType, SugarType, 1+1,
    : NumPadBits,
...
public:
  ...
  ///
  /// These are the types of the NeXTCode type system.
  ...
  /** Check if this type is equal to NeXTCode.NAME. */ \
  bool is##NAME();
  ...
  /// Check if this is a nominal type defined at the top level of the NeXTCode module
  bool isStdlibType();
  ...
  /// at the top level of the NeXTCode module
  ...
  /// Determines whether the given NeXTCode type is representable within
  /// the given foreign language.
  ///
  /// A given NeXTCode type is representable in the given foreign
  /// language if the NeXTCode type can be used from source code written
  /// in that language.
  ...
  // Return type underlying type of a nextcode_newtype annotated imported struct;
  // otherwise, return the null type.
  Type getNeXTCodeNewtypeUnderlyingType();
  ...
  NEXTCODE_DEBUG_DUMPER(dumpPrint());
...
/// additional bits. The type uses a bit to discriminate native NeXTCode objects
...
class BuiltinIntegerWidth {
  ...
  friend struct llvm::DenseMapInfo<nextcode::BuiltinIntegerWidth>;
...
public:
  ...
  using ExtInfo = nextcode::ASTExtInfo;
  using ExtInfoBuilder = nextcode::ASTExtInfoBuilder;
...
public:
  ...
  /// be derived from its NeXTCode type. Returns false otherwise, including if
...
public:
  ...
  /// Does this function have a blessed NeXTCode-native error result?
  ...
  /// Thick nextcode noescape function types are trivial.
...
/// be written directly at the NeXTCode level, instead it is triggered by adding
...
///
/// In NeXTCode 5 mode, a plain protocol name in type
...
/// for it.
///
/// L-values are not fully first-class in NeXTCode:
///
...
template<>
struct DenseMapInfo<nextcode::BuiltinIntegerWidth> {
  using BuiltinIntegerWidth = nextcode::BuiltinIntegerWidth;
