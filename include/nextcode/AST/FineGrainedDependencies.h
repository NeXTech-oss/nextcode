...
#ifndef NEXTCODE_AST_FINE_GRAINED_DEPENDENCIES_H
#define NEXTCODE_AST_FINE_GRAINED_DEPENDENCIES_H
...
public:
  ...
  public:
    /// Copies the name data for the given nextcodedeps file into this builder.
    Builder withName(StringRef nextcodeDeps) &&;
...
public:
  ...
  StringRef getNeXTCodeDepsFromASourceFileProvideNodeKey() const {
  ...
  NEXTCODE_DEBUG_DUMP { dump(llvm::errs()); }
...
template <>
struct std::hash<typename nextcode::fine_grained_dependencies::DependencyKey> {
  size_t
  operator()(const nextcode::fine_grained_dependencies::DependencyKey &key) const {
    return key.hash();
  }
};
template <>
struct std::hash<typename nextcode::fine_grained_dependencies::DeclAspect> {
  size_t
  operator()(const nextcode::fine_grained_dependencies::DeclAspect aspect) const {
    return size_t(aspect);
  }
};
template <>
struct std::hash<typename nextcode::fine_grained_dependencies::NodeKind> {
  size_t
  operator()(const nextcode::fine_grained_dependencies::NodeKind kind) const {
    return size_t(kind);
  }
};
...
/// definition of entity in the program (a Decl or a source file/nextcodedeps
...
public:
  ...
  /// whose nextcodedeps file has not been read by the driver. Later, when the
...
public:
  ...
  StringRef getNeXTCodeDepsOfJobThatProducedThisGraph() const;
  ...
  /// Read a nextcodedeps file at \p path and return a SourceFileDepGraph if
  /// successful. If \p allowNeXTCodeModule is true, try to load the information
  /// from a nextcodemodule file if appropriate.
  std::optional<SourceFileDepGraph> static loadFromPath(
      StringRef, bool allowNeXTCodeModule = false);

  /// Read a nextcodedeps file from \p buffer and return a SourceFileDepGraph if
  /// successful.
  std::optional<SourceFileDepGraph> static loadFromBuffer(llvm::MemoryBuffer &);
  std::optional<SourceFileDepGraph> static loadFromNeXTCodeModuleBuffer(
      llvm::MemoryBuffer &);
...
#endif // NEXTCODE_AST_FINE_GRAINED_DEPENDENCIES_H
