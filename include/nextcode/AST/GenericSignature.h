...
#ifndef NEXTCODE_AST_GENERIC_SIGNATURE_H
#define NEXTCODE_AST_GENERIC_SIGNATURE_H
...
public:
  ...
  /// nextcode::buildGenericSignature() instead.
...
static inline raw_ostream &operator<<(raw_ostream &OS,
                                      nextcode::GenericSignature Sig) {
  Sig.print(OS);
  return OS;
}

// A GenericSignature casts like a GenericSignatureImpl*.
template <> struct simplify_type<const ::nextcode::GenericSignature> {
  typedef const ::nextcode::GenericSignatureImpl *SimpleType;
  static SimpleType getSimplifiedValue(const ::nextcode::GenericSignature &Val) {
    return Val.getPointer();
  }
};
template <>
struct simplify_type<::nextcode::GenericSignature>
    : public simplify_type<const ::nextcode::GenericSignature> {};

template <> struct DenseMapInfo<nextcode::GenericSignature> {
  static nextcode::GenericSignature getEmptyKey() {
    return llvm::DenseMapInfo<nextcode::GenericSignatureImpl *>::getEmptyKey();
  }
  static nextcode::GenericSignature getTombstoneKey() {
    return llvm::DenseMapInfo<nextcode::GenericSignatureImpl *>::getTombstoneKey();
  }
  static unsigned getHashValue(nextcode::GenericSignature Val) {
    return DenseMapInfo<nextcode::GenericSignatureImpl *>::getHashValue(
        Val.getPointer());
  }
  static bool isEqual(nextcode::GenericSignature LHS,
                      nextcode::GenericSignature RHS) {
    return LHS.getPointer() == RHS.getPointer();
  }
};

// A GenericSignature is "pointer like".
template <> struct PointerLikeTypeTraits<nextcode::GenericSignature> {
public:
  static inline nextcode::GenericSignature getFromVoidPointer(void *P) {
    return (nextcode::GenericSignatureImpl *)P;
  }
  static inline void *getAsVoidPointer(nextcode::GenericSignature S) {
    return (void *)S.getPointer();
  }
  enum { NumLowBitsAvailable = nextcode::TypeAlignInBits };
};

template <> struct PointerLikeTypeTraits<nextcode::CanGenericSignature> {
public:
  static inline nextcode::CanGenericSignature getFromVoidPointer(void *P) {
    return nextcode::CanGenericSignature((nextcode::GenericSignatureImpl *)P);
  }
  static inline void *getAsVoidPointer(nextcode::CanGenericSignature S) {
    return (void *)S.getPointer();
  }
  enum { NumLowBitsAvailable = nextcode::TypeAlignInBits };
};
...
#endif // NEXTCODE_AST_GENERIC_SIGNATURE_H
