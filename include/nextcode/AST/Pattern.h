//===--- Pattern.h - NeXTCode Language Pattern-Matching ASTs -------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_PATTERN_H
#define NEXTCODE_PATTERN_H

#include "nextcode/Basic/AnyValue.h"
...
/// Pattern - Base class for all patterns in NeXTCode.
class alignas(8) Pattern : public ASTAllocated<Pattern> {
protected:
  // clang-format off
  union { uint64_t OpaqueBits;

  NEXTCODE_INLINE_BITFIELD_BASE(Pattern, bitmax(NumPatternKindBits,8)+1+1,
    Kind : bitmax(NumPatternKindBits,8),
    isImplicit : 1,
    hasInterfaceType : 1
  );

  NEXTCODE_INLINE_BITFIELD_FULL(TuplePattern, Pattern, 32,
    : NumPadBits,
    NumElements : 32
  );

  NEXTCODE_INLINE_BITFIELD(TypedPattern, Pattern, 1,
    IsPropagatedType : 1
  );

  NEXTCODE_INLINE_BITFIELD(BoolPattern, Pattern, 1,
    Value : 1
  );

  NEXTCODE_INLINE_BITFIELD(BindingPattern, Pattern, 2,
    /// Corresponds to VarDecl::Introducer
    Introducer : 2
  );

  NEXTCODE_INLINE_BITFIELD(AnyPattern, Pattern, 1,
