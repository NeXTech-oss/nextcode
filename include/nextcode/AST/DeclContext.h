//===--- DeclContext.h - NeXTCode Language Context ASTs ------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_DECLCONTEXT_H
#define NEXTCODE_DECLCONTEXT_H
...
public:
  ...
  LLVM_READONLY
  bool isPackageContext() const; // see nextcode/AST/Module.h

  /// isModuleContext - Return true if this is a subclass of Module.
  LLVM_READONLY
  bool isModuleContext() const; // see nextcode/AST/Module.h

  /// \returns true if this is a context with module-wide scope, e.g. a module
  /// or a source file.
  LLVM_READONLY
  bool isModuleScopeContext() const; // see nextcode/AST/Module.h
  ...
  NEXTCODE_DEBUG_DUMPER(dumpContext());
  unsigned printContext(llvm::raw_ostream &OS, unsigned indent = 0,
                        bool onlyAPartialLine = false) const;
  
  // Some Decls are DeclContexts, but not all. See nextcode/AST/Decl.h
...
public:
  ...
  /// category, it may be a NeXTCode extension instead.
...
namespace llvm {
  ...
  struct CastInfo<::nextcode::DeclContext, FromTy, std::enable_if_t<is_simple_type<FromTy>::value>>
      : public CastIsPossible<::nextcode::DeclContext, FromTy>,
        public DefaultDoCastIfPossible<::nextcode::DeclContext *, FromTy,
                                       CastInfo<::nextcode::DeclContext, FromTy>> {
    static inline ::nextcode::DeclContext *castFailed() { return nullptr; }

    static inline ::nextcode::DeclContext *doCast(const FromTy &val) {
      return ::nextcode::DeclContext::castDeclToDeclContext(val);
    }
  };

  template<class FromTy>
  struct CastInfo<::nextcode::IterableDeclContext, FromTy, std::enable_if_t<is_simple_type<FromTy>::value>>
      : public CastIsPossible<::nextcode::IterableDeclContext, FromTy>,
        public DefaultDoCastIfPossible<::nextcode::IterableDeclContext *, FromTy,
                                       CastInfo<::nextcode::IterableDeclContext, FromTy>> {
    static inline ::nextcode::IterableDeclContext *castFailed() { return nullptr; }

    static inline ::nextcode::IterableDeclContext *doCast(const FromTy &val) {
      return ::nextcode::IterableDeclContext::castDeclToIterableDeclContext(val);
    }
  };
}
