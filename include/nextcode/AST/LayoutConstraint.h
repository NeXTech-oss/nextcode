...
#ifndef NEXTCODE_LAYOUT_CONSTRAINT_H
#define NEXTCODE_LAYOUT_CONSTRAINT_H

#include "nextcode/AST/ASTAllocated.h"
#include "nextcode/AST/LayoutConstraintKind.h"
...
LLVM_DECLARE_TYPE_ALIGNMENT(nextcode::LayoutConstraintInfo, nextcode::TypeAlignInBits)

namespace llvm {
static inline raw_ostream &
operator<<(raw_ostream &OS, nextcode::LayoutConstraint LC) {
  LC->print(OS);
  return OS;
}

// A LayoutConstraint casts like a LayoutConstraintInfo*.
template <> struct simplify_type<const ::nextcode::LayoutConstraint> {
  typedef ::nextcode::LayoutConstraintInfo *SimpleType;
  static SimpleType getSimplifiedValue(const ::nextcode::LayoutConstraint &Val) {
    return Val.getPointer();
  }
};

template <>
struct simplify_type<::nextcode::LayoutConstraint>
    : public simplify_type<const ::nextcode::LayoutConstraint> {};

// LayoutConstraint hashes just like pointers.
template <> struct DenseMapInfo<nextcode::LayoutConstraint> {
  static nextcode::LayoutConstraint getEmptyKey() {
    return llvm::DenseMapInfo<nextcode::LayoutConstraintInfo *>::getEmptyKey();
  }
  static nextcode::LayoutConstraint getTombstoneKey() {
    return llvm::DenseMapInfo<nextcode::LayoutConstraintInfo *>::getTombstoneKey();
  }
  static unsigned getHashValue(nextcode::LayoutConstraint Val) {
    return DenseMapInfo<nextcode::LayoutConstraintInfo *>::getHashValue(
        Val.getPointer());
  }
  static bool isEqual(nextcode::LayoutConstraint LHS,
                      nextcode::LayoutConstraint RHS) {
    return LHS.getPointer() == RHS.getPointer();
  }
};

// A LayoutConstraint is "pointer like".
template <> struct PointerLikeTypeTraits<nextcode::LayoutConstraint> {
public:
  static inline void *getAsVoidPointer(nextcode::LayoutConstraint I) {
    return (void *)I.getPointer();
  }
  static inline nextcode::LayoutConstraint getFromVoidPointer(void *P) {
    return (nextcode::LayoutConstraintInfo *)P;
  }
