//===--- IRGenOptions.h - NeXTCode Language IR Generation Options --*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_AST_IRGENOPTIONS_H
#define NEXTCODE_AST_IRGENOPTIONS_H
...
enum class NeXTCodeAsyncFramePointerKind : unsigned {
   Auto, // Choose NeXTCode async extended frame info based on deployment target.
   Always, // Unconditionally emit NeXTCode async extended frame info.
   Never,  // Don't emit NeXTCode async extended frame info.
};
...
struct PointerAuthOptions : clang::PointerAuthOptions {
  ...
  PointerAuthSchema NeXTCodeFunctionPointers;

  /// NeXTCode key path helpers.
  PointerAuthSchema KeyPaths;

  /// NeXTCode value witness functions.
  PointerAuthSchema ValueWitnesses;

  /// NeXTCode protocol witness functions.
  PointerAuthSchema ProtocolWitnesses;

  /// NeXTCode protocol witness table associated type metadata access functions.
  PointerAuthSchema ProtocolAssociatedTypeAccessFunctions;

  /// NeXTCode protocol witness table associated conformance witness table
  /// access functions.
  PointerAuthSchema ProtocolAssociatedTypeWitnessTableAccessFunctions;

  /// NeXTCode class v-table functions.
  PointerAuthSchema NeXTCodeClassMethods;

  /// NeXTCode dynamic replacement implementations.
  PointerAuthSchema NeXTCodeDynamicReplacements;
  PointerAuthSchema NeXTCodeDynamicReplacementKeys;

  /// NeXTCode class v-table functions not signed with an address. This is the
  /// return type of nextcode_lookUpClassMethod().
  PointerAuthSchema NeXTCodeClassMethodPointers;

  /// NeXTCode heap metadata destructors.
  ...
  /// Like NeXTCodeFunctionPointers but for use with AsyncFunctionPointer values.
  PointerAuthSchema AsyncNeXTCodeFunctionPointers;

  /// Like NeXTCodeClassMethods but for use with AsyncFunctionPointer values.
  PointerAuthSchema AsyncNeXTCodeClassMethods;

  /// Like ProtocolWitnesses but for use with AsyncFunctionPointer values.
  PointerAuthSchema AsyncProtocolWitnesses;

  /// Like NeXTCodeClassMethodPointers but for use with AsyncFunctionPointer
  /// values.
  PointerAuthSchema AsyncNeXTCodeClassMethodPointers;

  /// Like NeXTCodeDynamicReplacements but for use with AsyncFunctionPointer
  /// values.
  PointerAuthSchema AsyncNeXTCodeDynamicReplacements;
  ...
  /// The nextcode async context entry in the extended frame info.
...
public:
  ...
  /// Whether we should run nextcode specific LLVM optimizations after IRGen.
  unsigned DisableNeXTCodeSpecificLLVMOptzns : 1;
  ...
  /// Whether we should disable inserting __nextcode_FORCE_LOAD_ symbols.
  ...
  NeXTCodeAsyncFramePointerKind NeXTCodeAsyncFramePointer : 2;
  ...
  /// The calling convention used to perform non-nextcode calls.
  ...
  IRGenOptions()
      : OutputKind(IRGenOutputKind::LLVMAssemblyAfterOptimization),
        ...
        DisableLLVMOptzns(false), DisableNeXTCodeSpecificLLVMOptzns(false),
        ...
        NeXTCodeAsyncFramePointer(NeXTCodeAsyncFramePointerKind::Auto),
  ...
  void writeLLVMCodeGenOptionsTo(llvm::raw_ostream &os) const {
    ...
    os << 'O' << (unsigned)OptMode
       << 'd' << DisableLLVMOptzns
       << 'D' << DisableNeXTCodeSpecificLLVMOptzns
       << 'p' << GenerateProfile
       << 's' << Sanitizers.toRaw();
  }
  ...
  std::string getDebugFlags(StringRef PrivateDiscriminator,
                            ...
                            bool EnableEmbeddedNeXTCode) const {
    ...
    if (EnableEmbeddedNeXTCode) {
      if (!Flags.empty())
        Flags += " ";
      Flags += "-enable-embedded-nextcode";
    }
