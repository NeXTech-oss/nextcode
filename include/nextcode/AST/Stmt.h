//===--- Stmt.h - NeXTCode Language Statement ASTs -----------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_AST_STMT_H
#define NEXTCODE_AST_STMT_H
...
/// Stmt - Base class for all statements in nextcode.
...
protected:
  ...
  NEXTCODE_INLINE_BITFIELD_BASE(Stmt, bitmax(NumStmtKindBits,8) + 1,
  ...
  NEXTCODE_INLINE_BITFIELD_FULL(BraceStmt, Stmt, 32,
    : NumPadBits,
    NumElements : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(CaseStmt, Stmt, 32,
    : NumPadBits,
    NumPatterns : 32
  );

  NEXTCODE_INLINE_BITFIELD_EMPTY(LabeledStmt, Stmt);

  NEXTCODE_INLINE_BITFIELD_FULL(DoCatchStmt, LabeledStmt, 32,
    : NumPadBits,
    NumCatches : 32
  );

  NEXTCODE_INLINE_BITFIELD_FULL(SwitchStmt, LabeledStmt, 32,
    : NumPadBits,
    CaseCount : 32
  );
    
  NEXTCODE_INLINE_BITFIELD(ReturnStmt, Stmt, 1,
    /// Whether the result is an implied return, e.g for an implicit single
    /// expression return.
    IsImplied : 1
  );

  NEXTCODE_INLINE_BITFIELD_FULL(YieldStmt, Stmt, 32,
    : NumPadBits,
    NumYields : 32
  );
...
public:
  ...
  /// "switch" (for "break") for consistency with C: NeXTCode doesn't
