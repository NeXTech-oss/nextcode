...
#ifndef NEXTCODE_AST_IDENTIFIER_H
#define NEXTCODE_AST_IDENTIFIER_H
...
public:
  ...
  static bool isEditorPlaceholder(StringRef name) {
    return nextcode::isEditorPlaceholder(name);
  }
...
namespace llvm {
  raw_ostream &operator<<(raw_ostream &OS, nextcode::Identifier I);
  raw_ostream &operator<<(raw_ostream &OS, nextcode::DeclName I);
  raw_ostream &operator<<(raw_ostream &OS, nextcode::DeclNameRef I);
  raw_ostream &operator<<(raw_ostream &OS, nextcode::ObjCSelector S);

  // Identifiers hash just like pointers.
  template<> struct DenseMapInfo<nextcode::Identifier> {
    static nextcode::Identifier getEmptyKey() {
      return nextcode::Identifier::getEmptyKey();
    }
    static nextcode::Identifier getTombstoneKey() {
      return nextcode::Identifier::getTombstoneKey();
    }
    static unsigned getHashValue(nextcode::Identifier Val) {
      return DenseMapInfo<const void*>::getHashValue(Val.get());
    }
    static bool isEqual(nextcode::Identifier LHS, nextcode::Identifier RHS) {
      return LHS == RHS;
    }
  };
  ...
  template<>
  struct PointerLikeTypeTraits<nextcode::Identifier> {
  public:
    static inline void *getAsVoidPointer(nextcode::Identifier I) {
      return const_cast<void *>(I.getAsOpaquePointer());
    }
    static inline nextcode::Identifier getFromVoidPointer(void *P) {
      return nextcode::Identifier::getFromOpaquePointer(P);
    }
    enum { NumLowBitsAvailable = nextcode::Identifier::NumLowBitsAvailable };
  };
...
raw_ostream &operator<<(raw_ostream &OS, nextcode::DeclBaseName D);

// DeclBaseNames hash just like pointers.
template<> struct DenseMapInfo<nextcode::DeclBaseName> {
  static nextcode::DeclBaseName getEmptyKey() {
    return nextcode::Identifier::getEmptyKey();
  }
  static nextcode::DeclBaseName getTombstoneKey() {
    return nextcode::Identifier::getTombstoneKey();
  }
  static unsigned getHashValue(nextcode::DeclBaseName Val) {
    return DenseMapInfo<const void *>::getHashValue(Val.getAsOpaquePointer());
  }
  static bool isEqual(nextcode::DeclBaseName LHS, nextcode::DeclBaseName RHS) {
    return LHS == RHS;
  }
};
...
template <typename T> struct PointerLikeTypeTraits;
template <> struct PointerLikeTypeTraits<nextcode::DeclBaseName> {
public:
  static inline void *getAsVoidPointer(nextcode::DeclBaseName D) {
    return const_cast<void *>(D.getAsOpaquePointer());
  }
  static inline nextcode::DeclBaseName getFromVoidPointer(void *P) {
    return nextcode::DeclBaseName::getFromOpaquePointer(P);
  }
  enum { NumLowBitsAvailable = PointerLikeTypeTraits<nextcode::Identifier>::NumLowBitsAvailable };
};
...
#include "nextcode/AST/ObjCSelectorFamily.def"
};
...
public:
  ...
  ///
  /// This should not be used to parse selectors written directly in NeXTCode
...
namespace llvm {
  ...
  template<typename T> struct PointerLikeTypeTraits;
  template<>
  struct PointerLikeTypeTraits<nextcode::DeclName> {
  public:
    static inline void *getAsVoidPointer(nextcode::DeclName name) {
      return name.getOpaqueValue();
    }
    static inline nextcode::DeclName getFromVoidPointer(void *ptr) {
      return nextcode::DeclName::getFromOpaqueValue(ptr);
    }
    enum { NumLowBitsAvailable = PointerLikeTypeTraits<nextcode::DeclBaseName>::NumLowBitsAvailable - 1 };
  };

  // DeclNames hash just like pointers.
  template<> struct DenseMapInfo<nextcode::DeclName> {
    static nextcode::DeclName getEmptyKey() {
      return nextcode::Identifier::getEmptyKey();
    }
    static nextcode::DeclName getTombstoneKey() {
      return nextcode::Identifier::getTombstoneKey();
    }
    static unsigned getHashValue(nextcode::DeclName Val) {
      return DenseMapInfo<void*>::getHashValue(Val.getOpaqueValue());
    }
    static bool isEqual(nextcode::DeclName LHS, nextcode::DeclName RHS) {
      return LHS.getOpaqueValue() == RHS.getOpaqueValue();
    }
  };
  ...
  template<typename T> struct PointerLikeTypeTraits;
  template<>
  struct PointerLikeTypeTraits<nextcode::DeclNameRef> {
  public:
    static inline void *getAsVoidPointer(nextcode::DeclNameRef name) {
      return name.getOpaqueValue();
    }
    static inline nextcode::DeclNameRef getFromVoidPointer(void *ptr) {
      return nextcode::DeclNameRef::getFromOpaqueValue(ptr);
    }
    enum { NumLowBitsAvailable = PointerLikeTypeTraits<nextcode::DeclName>::NumLowBitsAvailable };
  };

  // DeclNameRefs hash just like DeclNames.
  template<> struct DenseMapInfo<nextcode::DeclNameRef> {
    static nextcode::DeclNameRef getEmptyKey() {
      return nextcode::DeclNameRef(DenseMapInfo<nextcode::DeclName>::getEmptyKey());
    }
    static nextcode::DeclNameRef getTombstoneKey() {
      return nextcode::DeclNameRef(DenseMapInfo<nextcode::DeclName>::getTombstoneKey());
    }
    static unsigned getHashValue(nextcode::DeclNameRef Val) {
      return DenseMapInfo<nextcode::DeclName>::getHashValue(Val.getFullName());
    }
    static bool isEqual(nextcode::DeclNameRef LHS, nextcode::DeclNameRef RHS) {
      return DenseMapInfo<nextcode::DeclName>::isEqual(LHS.getFullName(),
                                                    RHS.getFullName());
    }
  };
  ...
  template<typename T> struct PointerLikeTypeTraits;
  template<>
  struct PointerLikeTypeTraits<nextcode::ObjCSelector> {
  public:
    static inline void *getAsVoidPointer(nextcode::ObjCSelector name) {
      return name.getOpaqueValue();
    }
    static inline nextcode::ObjCSelector getFromVoidPointer(void *ptr) {
      return nextcode::ObjCSelector::getFromOpaqueValue(ptr);
    }
    enum { NumLowBitsAvailable = 0 };
  };

  // ObjCSelectors hash just like pointers.
  template<> struct DenseMapInfo<nextcode::ObjCSelector> {
    static nextcode::ObjCSelector getEmptyKey() {
      return nextcode::ObjCSelector(DenseMapInfo<nextcode::DeclName>::getEmptyKey());
    }
    static nextcode::ObjCSelector getTombstoneKey() {
      return nextcode::ObjCSelector(
               DenseMapInfo<nextcode::DeclName>::getTombstoneKey());
    }
    static unsigned getHashValue(nextcode::ObjCSelector Val) {
      return DenseMapInfo<void*>::getHashValue(Val.getOpaqueValue());
    }
    static bool isEqual(nextcode::ObjCSelector LHS, nextcode::ObjCSelector RHS) {
      return LHS.getOpaqueValue() == RHS.getOpaqueValue();
    }
  };
} // end namespace llvm
