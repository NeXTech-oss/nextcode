...
#ifndef NEXTCODE_AST_SOURCEFILE_H
#define NEXTCODE_AST_SOURCEFILE_H
...
/// A file containing NeXTCode source code.
///
...
public:
  ...
  enum class ParsingFlags : uint8_t {
    ...
    /// Ensure that the NeXTCodeSyntax tree round trips correctly.
    RoundTrip = 1 << 5,

    /// Validate the new NeXTCodeSyntax parser diagnostics.
...
public:
  ...
  NEXTCODE_DEBUG_DUMPER(dumpSeparatelyImportedOverlays());
...
template<>
struct DenseMapInfo<nextcode::SourceFile::ObjCMethodConflict> {
  using ObjCMethodConflict = nextcode::SourceFile::ObjCMethodConflict;
  ...
  static inline unsigned getHashValue(ObjCMethodConflict a) {
    return hash_combine(hash_value(a.typeDecl),
                  DenseMapInfo<nextcode::ObjCSelector>::getHashValue(a.selector),
