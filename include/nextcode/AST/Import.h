...
#ifndef NEXTCODE_IMPORT_H
#define NEXTCODE_IMPORT_H
...
public:
  ...
  ///
  /// Although in theory NeXTCode could support scoped imports of nested
  ...
  /// import. (Although NeXTCode modules cannot currently contain submodules, NeXTCode
...
enum class ImplicitStdlibKind {
  ...
  /// The regular NeXTCode standard library should be implicitly imported.
  Stdlib
};
...
template<>
struct DenseMapInfo<nextcode::ImportOptions> {
  using ImportOptions = nextcode::ImportOptions;
...
template <>
class DenseMapInfo<nextcode::ImportedModule> {
  using ImportedModule = nextcode::ImportedModule;
  using ModuleDecl = nextcode::ModuleDecl;
public:
...
template<typename ModuleInfo>
struct DenseMapInfo<nextcode::AttributedImport<ModuleInfo>> {
  using AttributedImport = nextcode::AttributedImport<ModuleInfo>;

  using ModuleInfoDMI = DenseMapInfo<ModuleInfo>;
  using ImportOptionsDMI = DenseMapInfo<nextcode::ImportOptions>;
  using StringRefDMI = DenseMapInfo<StringRef>;
  using SourceLocDMI = DenseMapInfo<nextcode::SourceLoc>;
  ...
  static inline AttributedImport getEmptyKey() {
    return AttributedImport(
        ...
        std::nullopt, nextcode::AccessLevel::Public, {});
  }
