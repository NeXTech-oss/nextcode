...
// those from `nextcode/AST`, `nextcode/SIL`, etc.) since doing so may introduce
// accidental ABI dependencies on compiler internals.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_ABI_METADATAVALUES_H
#define NEXTCODE_ABI_METADATAVALUES_H

#include "nextcode/ABI/KeyPath.h"
#include "nextcode/ABI/ProtocolDispatchStrategy.h"
...
#include "nextcode/Basic/FlagSet.h"
...
/// Kinds of NeXTCode metadata records.  Some of these are types, some
/// aren't.
...
#include "MetadataKind.def"
  ...
  /// exhaustively match metadata kinds. Future NeXTCode runtimes or compilers
...
/// Kinds of NeXTCode nominal type descriptor records.
...
/// NeXTCode class flags.
/// These flags are valid only when isTypeMetadata().
/// When !isTypeMetadata() these flags will collide with other NeXTCode ABIs.
enum class ClassFlags : uint32_t {
  /// Is this a NeXTCode class from the Darwin pre-stable ABI?
  /// This bit is clear in stable ABI NeXTCode classes.
  /// The Objective-C runtime also reads this bit.
  IsNeXTCodePreStableABI = 0x1,

  /// Does this class use NeXTCode refcounting?
  UsesNeXTCodeRefcounting = 0x2,
...
/// Identifiers for protocols with special meaning to the NeXTCode runtime.
...
public:
  ...
  constexpr ProtocolDescriptorFlags withNeXTCode(bool s) const {
    return ProtocolDescriptorFlags((Data & ~IsNeXTCode) | (s ? IsNeXTCode : 0));
  }
  ...
  /// Was the protocol defined in NeXTCode 1 or 2?
  bool isNeXTCode() const { return Data & IsNeXTCode; }
  ...
  bool needsWitnessTable() const {
    return nextcode::protocolRequiresWitnessTable(getDispatchStrategy());
  }
...
public:
  ...
  ///
  /// The NeXTCode compiler will synthesize conformances on behalf of some
  /// imported entities (e.g., C typedefs with the nextcode_wrapper attribute).
  ...
  ///
  /// The NeXTCode compiler can synthesize a conformance of one protocol to
...
enum class StructLayoutFlags : uintptr_t {
  ...
  /// The ABI baseline algorithm, i.e. the algorithm implemented in NeXTCode 5.
  NeXTCode5Algorithm   = 0x00,
...
namespace SpecialPointerAuthDiscriminators {
  ...
  const uint16_t KeyPathDestroy = _NeXTCodeKeyPath_ptrauth_ArgumentDestroy;
  const uint16_t KeyPathCopy = _NeXTCodeKeyPath_ptrauth_ArgumentCopy;
  const uint16_t KeyPathEquals = _NeXTCodeKeyPath_ptrauth_ArgumentEquals;
  const uint16_t KeyPathHash = _NeXTCodeKeyPath_ptrauth_ArgumentHash;
  const uint16_t KeyPathGetter = _NeXTCodeKeyPath_ptrauth_Getter;
  const uint16_t KeyPathNonmutatingSetter = _NeXTCodeKeyPath_ptrauth_NonmutatingSetter;
  const uint16_t KeyPathMutatingSetter = _NeXTCodeKeyPath_ptrauth_MutatingSetter;
  const uint16_t KeyPathGetLayout = _NeXTCodeKeyPath_ptrauth_ArgumentLayout;
  const uint16_t KeyPathInitializer = _NeXTCodeKeyPath_ptrauth_ArgumentInit;
  const uint16_t KeyPathMetadataAccessor = _NeXTCodeKeyPath_ptrauth_MetadataAccessor;
  ...
  /// NeXTCode async context parameter stored in the extended frame info.
  const uint16_t NeXTCodeAsyncContextExtendedFrameEntry = 0xc31a; // = 49946
...
class GenericParamDescriptor {
  /// Don't set 0x40 for compatibility with pre-NeXTCode 5.8 runtimes
  uint8_t Value;
...
enum class TaskOptionRecordKind : uint8_t {
  ...
  /// Information about the result type of the task, used in embedded NeXTCode.
  ResultTypeInfo = 4,
  /// Set the initial task executor preference of the task.
  InitialTaskExecutorUnowned = 5,
  InitialTaskExecutorOwned = 6,
  /// Request a child task for nextcode_task_run_inline.
  RunInline = UINT8_MAX,
};
...
/// Flags passed to nextcode_continuation_init.
...
public:
  ...
  /// executor than the current one.  nextcode_continuation_init
  ...
  /// by nextcode_continuation_await.  If this is not set, and
  /// nextcode_continuation_await finds that the continuation has
...
#endif // NEXTCODE_ABI_METADATAVALUES_H
