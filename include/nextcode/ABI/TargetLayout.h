...
//===----------------------------------------------------------------------===//
//
// A lot of NeXTCode's runtime structures need to be parsed by code
...
#ifndef NEXTCODE_ABI_TARGETLAYOUT_H
#define NEXTCODE_ABI_TARGETLAYOUT_H

#include "nextcode/Runtime/Config.h"
#include "nextcode/Basic/RelativePointer.h"
#include "nextcode/ABI/CompactFunctionPointer.h"
...
struct InProcess {
  ...
  using CompactFunctionPointer =
      nextcode::RelativeDirectPointer<T, Nullable, Offset>;
#endif

  template<typename T>
  T *getStrippedSignedPointer(const T *pointer) const {
    return nextcode_ptrauth_strip(pointer);
  }
};
