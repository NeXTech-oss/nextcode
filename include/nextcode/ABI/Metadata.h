...
//===----------------------------------------------------------------------===//
//
// NeXTCode ABI describing metadata.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_ABI_METADATA_H
#define NEXTCODE_ABI_METADATA_H
...
#include "nextcode/ABI/GenericContext.h"
#include "nextcode/ABI/MetadataRef.h"
#include "nextcode/ABI/MetadataValues.h"
#include "nextcode/ABI/System.h"
#include "nextcode/ABI/TargetLayout.h"
#include "nextcode/ABI/TrailingObjects.h"
#include "nextcode/ABI/ValueWitnessTable.h"
#include "nextcode/Basic/Malloc.h"
#include "nextcode/Basic/FlaggedPointer.h"
...
struct nextcode_ptrauth_struct_context_descriptor(ContextDescriptor)
    TargetContextDescriptor;
template <typename Runtime>
class nextcode_ptrauth_struct_context_descriptor(TypeContextDescriptor)
    TargetTypeContextDescriptor;
template <typename Runtime>
class nextcode_ptrauth_struct_context_descriptor(ClassDescriptor)
    TargetClassDescriptor;
template <typename Runtime>
class nextcode_ptrauth_struct_context_descriptor(ValueTypeDescriptor)
    TargetValueTypeDescriptor;
template <typename Runtime>
class nextcode_ptrauth_struct_context_descriptor(EnumDescriptor)
    TargetEnumDescriptor;
template <typename Runtime>
class nextcode_ptrauth_struct_context_descriptor(StructDescriptor)
...
using HeapObjectDestroyer =
  NEXTCODE_CC(nextcode) void(NEXTCODE_CONTEXT HeapObject *);

/// The result of requesting type metadata.  Generally the return value of
/// a function.
///
/// For performance and ABI matching across NeXTCode/C++, functions returning
/// this type must use NEXTCODE_CC so that the components are returned as separate
/// values.
...
///
/// For performance, functions returning this type should use NEXTCODE_CC so
...
struct TargetTypeMetadataLayoutPrefix {
    TargetSignedPointer<Runtime, const uint8_t *
                                      __ptrauth_nextcode_type_layout_string>
        layoutString;
};
...
// FIXME: https://github.com/apple/nextcode/issues/43763
...
public:
  /// Is this a class object--the metadata record for a NeXTCode class (which also
  ...
  ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> const *
  getGenericArgs() const {
    ...
    auto asWords = reinterpret_cast<
      ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> const *>(this);
...
struct TargetHeapMetadataHeaderPrefix {
  ...
  TargetSignedPointer<Runtime, HeapObjectDestroyer *
                                   __ptrauth_nextcode_heap_object_destructor>
      destroy;
};
...
/// field of any heap object, whether it was managed by NeXTCode or by
...
/// not be the NeXTCode type metadata for the object's dynamic type.
template <typename Runtime>
...
struct TargetClassMetadataBounds : TargetMetadataBounds<Runtime> {
  ...
  /// Return the basic bounds of all NeXTCode class metadata.
  /// The immediate members offset will not be meaningful.
  static constexpr TargetClassMetadataBounds<Runtime> forNeXTCodeRootClass() {
  ...
  /// Return the bounds of a NeXTCode class metadata with the given address
...
/// all classes, even non-NeXTCode ones.
template <typename Runtime>
...
public:
  ...
  TargetSignedPointer<Runtime, const TargetClassMetadata *
                                   __ptrauth_nextcode_objc_superclass>
      Superclass;

  /// Is this object a valid nextcode type metadata?  That is, can it be
...
/// This is the class metadata object for all classes (NeXTCode and ObjC) in a
...
struct TargetAnyClassMetadataObjCInterop
  ...
  using TargetClassMetadataObjCInterop =
    // nextcode:: qualifier works around an MSVC quirk
    nextcode::TargetClassMetadata<Runtime, TargetAnyClassMetadataObjCInterop<Runtime>>;
  ...
  constexpr TargetAnyClassMetadataObjCInterop(
      ...
      : TargetAnyClassMetadata<Runtime>(isa, superclass),
        ...
        Data(NEXTCODE_CLASS_IS_NEXTCODE_MASK) {}
  ...
  /// order to indicate that this is a NeXTCode metatype and therefore
  ...
  /// safely downcast to ClassMetadata?
  bool isTypeMetadata() const {
    return (Data & NEXTCODE_CLASS_IS_NEXTCODE_MASK);
  }
  /// A different perspective on the same bit
  bool isPureObjC() const {
    return !isTypeMetadata();
...
private:
  /// An out-of-line NeXTCode-specific description of the type, or null
  /// if this is an artificial subclass.  We currently provide no
  /// supported mechanism for making a non-artificial subclass
  /// dynamically.
  TargetSignedPointer<Runtime, const TargetClassDescriptor<Runtime> * __ptrauth_nextcode_type_descriptor> Description;

public:
  /// A function for destroying instance variables, used to clean up after an
  /// early return from a constructor. If null, no clean up will be performed
  /// and all ivars must be trivial.
  TargetSignedPointer<Runtime, ClassIVarDestroyer * __ptrauth_nextcode_heap_object_destructor> IVarDestroyer;
  ...
  /// Given that this class is serving as the superclass of a NeXTCode class,
  ...
  getClassBoundsAsNeXTCodeSuperclass() const {
    using Bounds = TargetClassMetadataBounds<Runtime>;

    auto rootBounds = Bounds::forNeXTCodeRootClass();
  ...
  /// "is NeXTCode" bit for the current runtime. Depending on the deployment
  ...
  /// considers the "is NeXTCode" bit.
  void setAsTypeMetadata() {
    // If the wrong "is NeXTCode" bit is set, set the correct one.
    //
    ...
    auto otherNeXTCodeBit = (3ULL - NEXTCODE_CLASS_IS_NEXTCODE_MASK);
    assert(otherNeXTCodeBit == 1ULL || otherNeXTCodeBit == 2ULL);

    if ((this->Data & 3) == otherNeXTCodeBit) {
      this->Data ^= 3;
    }

    // Otherwise there should be nothing to do, since only the old "is
    // NeXTCode" bit is used for backward-deployed runtimes.
...
/// This includes NeXTCode heap metadata, followed by the vtable entries that
...
struct TargetDispatchClassMetadata : public TargetHeapMetadata<Runtime> {
  ...
  TargetSignedPointer<Runtime, InvokeCall __ptrauth_nextcode_dispatch_invoke_function> VTableInvoke;
};
...
struct TargetForeignClassMetadata : public TargetForeignTypeMetadata<Runtime> {
  ...
  ConstTargetMetadataPointer<Runtime, nextcode::TargetForeignClassMetadata>
    Superclass;
...
/// A foreign reference type is a non-NeXTCode, non-Objective-C foreign type with
/// reference semantics. Foreign reference types are pointers/reference to
/// value types marked with the `nextcode_attr("import_reference")` attribute.
///
...
struct TargetValueMetadata : public TargetMetadata<Runtime> {
  ...
  /// An out-of-line description of the type.
  TargetSignedPointer<Runtime, const TargetValueTypeDescriptor<Runtime> * __ptrauth_nextcode_type_descriptor> Description;
...
struct TargetFunctionGlobalActorMetadata {
  ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> GlobalActorType;
};
using FunctionGlobalActorMetadata = TargetFunctionGlobalActorMetadata<InProcess>;

template <typename Runtime>
struct TargetFunctionThrownErrorMetadata {
  ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> ThrownErrorType;
};
...
struct TargetFunctionTypeMetadata : public TargetMetadata<Runtime>,
  nextcode::ABI::TrailingObjects<
    TargetFunctionTypeMetadata<Runtime>,
    ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata>,
    ParameterFlags,
  ...
  using StoredSize = typename Runtime::StoredSize;
  using Parameter = ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata>;
...
public:
  ...
  ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> ResultType;
...
public:
  ...
  getGlobalActor() const {
    if (!hasGlobalActor())
      return ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata>();
    auto globalActorAddr =
      this->template getTrailingObjects<TargetFunctionGlobalActorMetadata<Runtime>>();
    return globalActorAddr->GlobalActorType;
  }
...
struct TargetMetatypeMetadata : public TargetMetadata<Runtime> {
  ...
  ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> InstanceType;
...
struct TargetTupleTypeMetadata : public TargetMetadata<Runtime> {
  ...
  /// The labels string;  see nextcode_getTupleTypeMetadata.
  TargetPointer<Runtime, const char> Labels;

  struct Element {
    /// The type of the element.
    ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> Type;
...
template <typename Runtime>
struct nextcode_ptrauth_struct_context_descriptor(ProtocolDescriptor)
...
using AssociatedWitnessTableAccessFunction =
  NEXTCODE_CC(nextcode) WitnessTable *(const Metadata *associatedType,
                                 const Metadata *self,
...
using AssociatedRelativeWitnessTableAccessFunction =
  NEXTCODE_CC(nextcode) RelativeWitnessTable *(const Metadata *associatedType,
                                 const Metadata *self,
...
/// common case (the only one NeXTCode currently supports), it has exactly
...
/// parameter type.  NeXTCode currently only supports existential
...
public:
  ...
  const GenericParamDescriptor *getReqSigParams() const {
    return Flags.hasImplicitReqSigParams()
               ? nextcode::targetImplicitGenericParamDescriptors<Runtime>()
  ...
  const GenericParamDescriptor *getGenSigParams() const {
    ...
    if (Flags.hasImplicitGenSigParams())
      return nextcode::targetImplicitGenericParamDescriptors<Runtime>();
...
public:
  ...
  TargetSignedPointer<Runtime, const ExtendedExistentialTypeShape *
                    __ptrauth_nextcode_nonunique_extended_existential_type_shape>
    Shape;
...
struct TargetGenericBoxHeapMetadata : public TargetBoxHeapMetadata<Runtime> {
  ...
  ConstTargetMetadataPointer<Runtime, nextcode::TargetMetadata> BoxedType;
...
struct TargetResilientWitnessTable final
  : public nextcode::ABI::TrailingObjects<
             TargetResilientWitnessTable<Runtime>,
             TargetResilientWitness<Runtime>> {
  uint32_t NumWitnesses;

  using TrailingObjects = nextcode::ABI::TrailingObjects<
                             TargetResilientWitnessTable<Runtime>,
                             TargetResilientWitness<Runtime>>;
  friend TrailingObjects;
...
struct TargetGenericWitnessTable {
  ...
  using PrivateDataType = void *[nextcode::NumGenericMetadataPrivateDataWords];
...
private:
  union {
    ...
    RelativeDirectPointerIntPair<TargetSignedPointer<Runtime, TargetContextDescriptor<Runtime> * __ptrauth_nextcode_type_descriptor>,
                                 TypeReferenceKind>
...
public:
  ...
  TargetContextDescriptor<Runtime> * __ptrauth_nextcode_type_descriptor *
  ...
  const nextcode::TargetWitnessTable<Runtime> *getWitnessTablePattern() const {
  ...
  /// type.
  const nextcode::TargetWitnessTable<Runtime> *
...
#if !defined(NDEBUG) && NEXTCODE_OBJC_INTEROP
  void dump() const;
#endif
...
template <typename Runtime>
struct nextcode_ptrauth_struct_context_descriptor(ModuleContextDescriptor)
...
template <typename Runtime>
struct nextcode_ptrauth_struct_context_descriptor(ExtensionContextDescriptor)
...
private:
  using TrailingGenericContextObjects =
      nextcode::TrailingGenericContextObjects<TargetExtensionContextDescriptor<Runtime>>;
...
template <typename Runtime>
struct nextcode_ptrauth_struct_context_descriptor(AnonymousContextDescriptor)
...
private:
  using TrailingGenericContextObjects =
      nextcode::TrailingGenericContextObjects<TargetAnonymousContextDescriptor<Runtime>,
                                           TargetGenericContextDescriptorHeader,
                                           TargetMangledContextName<Runtime>>;
  using TrailingObjects =
...
///
/// Only NeXTCode protocols are defined by a protocol descriptor, whereas
/// Objective-C (including protocols defined in NeXTCode as @objc) use the
...
template <typename Runtime>
struct nextcode_ptrauth_struct_context_descriptor(OpaqueTypeDescriptor)
...
private:
  using TrailingGenericContextObjects =
      nextcode::TrailingGenericContextObjects<TargetOpaqueTypeDescriptor<Runtime>,
                                           TargetGenericContextDescriptorHeader,
                                           RelativeDirectPointer<const char>,
                                           InvertibleProtocolSet>;
  using TrailingObjects =
...
struct TargetGenericMetadataInstantiationCache {
  ...
  PrivateData[nextcode::NumGenericMetadataPrivateDataWords];
};
...
class TargetGenericMetadataPatternTrailingObjects :
  protected nextcode::ABI::TrailingObjects<Self,
                                TargetGenericMetadataPartialPattern<Runtime>,
                                ExtraTrailingObjects...> {

  using TrailingObjects =
    nextcode::ABI::TrailingObjects<Self,
                                TargetGenericMetadataPartialPattern<Runtime>,
                                ExtraTrailingObjects...>;
  friend TrailingObjects;
...
public:
  ...
  MetadataResponse operator()(MetadataRequest request) const {
    using Fn0 = NEXTCODE_CC(nextcode) MetadataResponse(MetadataRequest request);
  ...
  MetadataResponse operator()(MetadataRequest request,
    ...
    using Fn1 = NEXTCODE_CC(nextcode) MetadataResponse(MetadataRequest request,
                                                 const void *arg0);
  ...
  MetadataResponse operator()(MetadataRequest request,
    ...
    using Fn2 = NEXTCODE_CC(nextcode) MetadataResponse(MetadataRequest request,
                                                 const void *arg0,
                                                 const void *arg1);
  ...
  MetadataResponse operator()(MetadataRequest request,
    ...
    using Fn3 = NEXTCODE_CC(nextcode) MetadataResponse(MetadataRequest request,
                                                 const void *arg0,
                                                 const void *arg1,
                                                 const void *arg2);
...
private:
  ...
  MetadataResponse applyMany(MetadataRequest request,
    ...
    using FnN = NEXTCODE_CC(nextcode) MetadataResponse(MetadataRequest request,
...
struct TargetResilientClassMetadataPattern {
  ...
  ///
  /// If this is null, the runtime instead calls nextcode_relocateClassMetadata(),
...
struct TargetCanonicalSpecializedMetadatasCachingOnceToken {
  TargetRelativeDirectPointer<Runtime, nextcode_once_t, /*Nullable*/ false> token;
};
...
public:
  ...
  nextcode_once_t *getCanonicalMetadataPrespecializationCachingOnceToken() const;
...
struct TargetResilientSuperclass {
  ...
  ///
  /// Note that NeXTCodeObject, the implicit superclass of all NeXTCode root
...
struct TargetObjCResilientClassStubInfo {
  ...
  ///
  /// We do not declare a struct type for class stubs since the NeXTCode runtime
  ...
  /// - Objective-C and NeXTCode can reference class stubs when emitting
  ///   categories (in NeXTCode, extensions with @objc members).
...
private:
  using TrailingGenericContextObjects =
    nextcode::TrailingGenericContextObjects<TargetClassDescriptor<Runtime>,
                                         TargetTypeGenericContextDescriptorHeader,
                                         TargetResilientSuperclass<Runtime>,
                                         TargetForeignMetadataInitialization<Runtime>,
                                         TargetSingletonMetadataInitialization<Runtime>,
...
private:
  using TrailingGenericContextObjects =
      nextcode::TrailingGenericContextObjects<TargetStructDescriptor<Runtime>,
                                           TargetTypeGenericContextDescriptorHeader,
...
private:
  using TrailingGenericContextObjects =
    nextcode::TrailingGenericContextObjects<TargetEnumDescriptor<Runtime>,
                                        TargetTypeGenericContextDescriptorHeader,
                                        ForeignMetadataInitialization,
                                        SingletonMetadataInitialization,
                                        MetadataListCount,
...
int32_t TargetTypeContextDescriptor<Runtime>::getGenericArgumentOffset() const {
  ...
  case ContextDescriptorKind::Struct:
    return llvm::cast<TargetStructDescriptor<Runtime>>(this)
        ->getGenericArgumentOffset();
  default:
    nextcode_unreachable("Not a type context descriptor.");
  }
}
...
TargetTypeContextDescriptor<Runtime>::getSingletonMetadataInitialization() const {
  ...
  case ContextDescriptorKind::Class:
    return llvm::cast<TargetClassDescriptor<Runtime>>(this)
        ->getSingletonMetadataInitialization();
  default:
    nextcode_unreachable("Not a enum, struct or class type descriptor.");
  }
}
...
template <typename Runtime>
inline nextcode_once_t *TargetTypeContextDescriptor<
  ...
  RelativeIndirectablePointer<
      ...
      TargetSignedPointer<InProcess,
                          DynamicReplacementKey *
                              __ptrauth_nextcode_dynamic_replacement_key>>
      replacedFunctionKey;
  union {
...
class DynamicReplacementScope
    : private nextcode::ABI::TrailingObjects<DynamicReplacementScope,
  ...
  using TrailingObjects =
      nextcode::ABI::TrailingObjects<DynamicReplacementScope,
                                  DynamicReplacementDescriptor>;
  friend TrailingObjects;
...
public:
  ...
  /// The NeXTCode function type, encoded as a mangled name.
