...
#ifndef NEXTCODE_ABI_GENERICCONTEXT_H
#define NEXTCODE_ABI_GENERICCONTEXT_H
...
struct TargetGenericContextDescriptorHeader {
  ...
  uint32_t getNumArguments() const {
    ...
    // must remain zero when backward deploying to NeXTCode 5.7 or
    // earlier.
...
public:
  ...
  llvm::StringRef getParam() const {
    return nextcode::Demangle::makeSymbolicMangledNameStringRef(Param.get());
  }
  ...
  llvm::StringRef getMangledTypeName() const {
    ...
    return nextcode::Demangle::makeSymbolicMangledNameStringRef(Type.get());
  }
...
class TargetGenericEnvironment
    : public nextcode::ABI::TrailingObjects<TargetGenericEnvironment<Runtime>,
               uint16_t, GenericParamDescriptor,
               TargetGenericRequirementDescriptor<Runtime>> {
  using GenericRequirementDescriptor =
    TargetGenericRequirementDescriptor<Runtime>;
  using TrailingObjects =
     nextcode::ABI::TrailingObjects<TargetGenericEnvironment<Runtime>,
...
class TrailingGenericContextObjects<TargetSelf<Runtime>,
  ...
  protected nextcode::ABI::TrailingObjects<TargetSelf<Runtime>,
...
protected:
  ...
  using GenericConditionalInvertibleProtocolRequirement =
    TargetConditionalInvertibleProtocolRequirement<Runtime>;
  using TrailingObjects = nextcode::ABI::TrailingObjects<Self,
