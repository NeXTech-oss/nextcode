...
//===----------------------------------------------------------------------===//
//
// NeXTCode ABI describing executors.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_ABI_EXECUTOR_H
#define NEXTCODE_ABI_EXECUTOR_H
...
/// NeXTCode.  The representation of nil in Optional<Builtin.Executor>
...
class SerialExecutorRef {
  HeapObject *Identity; // Not necessarily NeXTCode reference-countable
...
struct AsyncFunctionTypeImpl<
  ...
  using type = NEXTCODE_CC(nextcodeasync) void(NEXTCODE_ASYNC_CONTEXT AsyncContext *,
                                         ArgTys...);
};
...
struct AsyncContinuationTypeImpl<
  ...
  using type = NEXTCODE_CC(nextcodeasync) void(NEXTCODE_ASYNC_CONTEXT AsyncContext *,
                                         NEXTCODE_CONTEXT NeXTCodeError *);
};
