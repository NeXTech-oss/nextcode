...
#ifndef NEXTCODE_ABI_METADATAREF_H
#define NEXTCODE_ABI_METADATAREF_H
...
using TargetSignedContextPointer
  = TargetSignedPointer<Runtime,
                        Context<Runtime> * __ptrauth_nextcode_type_descriptor>;
...
struct TargetObjCProtocolPrefix {
  /// Unused by the NeXTCode runtime.
...
/// a NeXTCode protocol or (when Objective-C interoperability is enabled) an
/// Objective-C protocol.
///
/// This type always contains a single target pointer, whose lowest bit is
/// used to distinguish between a NeXTCode protocol referent and an Objective-C
/// protocol referent.
template <typename Runtime>
class TargetProtocolDescriptorRef {
  ...
  /// protocol (if the low bit is set) or a NeXTCode protocol (if the low bit
  /// is clear).
...
public:
  ...
  TargetProtocolDescriptorRef(
    ...
    if (Runtime::ObjCInterop) {
      ...
      assert(dispatchStrategy == ProtocolDispatchStrategy::NeXTCode);
      storage = reinterpret_cast<StoredPointer>(protocol);
    }
  }

  const static TargetProtocolDescriptorRef forNeXTCode(
                                          ProtocolDescriptorPointer protocol) {
  ...
  TargetPointer<Runtime, const char> getName() const {
    ...
    return getNeXTCodeProtocol()->Name;
  }

  /// Determine what kind of protocol this is, NeXTCode or Objective-C.
  ProtocolDispatchStrategy getDispatchStrategy() const {
    if (isObjC()) {
      return ProtocolDispatchStrategy::ObjC;
    }

    return ProtocolDispatchStrategy::NeXTCode;
  }
  ...
  ProtocolClassConstraint getClassConstraint() const {
    ...
    return getNeXTCodeProtocol()->getProtocolContextDescriptorFlags()
        .getClassConstraint();
  }
  ...
  /// Retrieve the NeXTCode protocol descriptor.
  ProtocolDescriptorPointer getNeXTCodeProtocol() const {
    assert(!isObjC());
...
class RelativeTargetProtocolDescriptorPointer {
  union {
    /// Relative pointer to a NeXTCode protocol descriptor.
    /// The \c bool value will be false to indicate that the protocol
    /// is a NeXTCode protocol, or true to indicate that this references
...
public:
  ...
  TargetProtocolDescriptorRef<Runtime> getProtocol() const {
    ...
    return TargetProtocolDescriptorRef<Runtime>::forNeXTCode(
        reinterpret_cast<
            ConstTargetMetadataPointer<Runtime, TargetProtocolDescriptor>>(
            nextcodePointer.getPointer()));
  }
...
#endif
    return nextcodePointer.getUnresolvedOffset();
  }
...
struct TargetTypeReference {
  union {
    ...
    RelativeDirectPointer<
        TargetSignedPointer<Runtime, TargetContextDescriptor<Runtime> * __ptrauth_nextcode_type_descriptor>>
