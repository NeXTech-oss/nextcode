...
//===----------------------------------------------------------------------===//
//
// NeXTCode ABI describing tasks.
//
//===----------------------------------------------------------------------===//

#ifndef NEXTCODE_ABI_TASK_H
#define NEXTCODE_ABI_TASK_H
...
#include "nextcode/Runtime/VoucherShims.h"
#include "nextcode/Basic/STLExtras.h"
#include "nextcode/Threading/ConditionVariable.h"
...
public:
  ...
  union {
    ...
    JobInvokeFunction * __ptrauth_nextcode_job_invoke_function RunJob;

    // A function to resume an AsyncTask.
    TaskContinuationFunction * __ptrauth_nextcode_task_resume_function ResumeTask;
  };
  ...
  ~Job() { nextcode_voucher_release(Voucher); }
  ...
  /// correctly, call nextcode_job_run or runJobInExecutorContext.
...
#if NEXTCODE_POINTER_IS_8_BYTES
...
private:
  NEXTCODE_ATTRIBUTE_UNUSED AsyncTask *Task;
...
/// type in a way that's compatible with regular NeXTCode and embedded NeXTCode. In
/// regular NeXTCode, just holds a Metadata pointer and dispatches to the value
/// witness table. In embedded NeXTCode, because we do not have any value witness
/// tables present at runtime, the witnesses are stored and referenced directly.
///
/// This structure is created from nextcode_task_create, where in regular NeXTCode, the
/// compiler provides the Metadata pointer, and in embedded NeXTCode, a
...
public:
  ...
  AsyncContext * __ptrauth_nextcode_task_resume_context ResumeContext;
  ...
  /// the job context correctly, call nextcode_job_run or
...
public:
  ...
  /// to use `nextcode_task_pushTaskExecutorPreference` and
  /// `nextcode_task_popTaskExecutorPreference`.
  ///
  ...
  /// `nextcode_task_pushTaskExecutorPreference`, and
  /// `nextcode_task_popTaskExecutorPreference(record)` method pair.
  ...
  public:
    ...
    /// Retrieve the error.
    NeXTCodeError *&getError() { return error; }
...
public:
  ...
  TaskContinuationFunction * __ptrauth_nextcode_async_context_yield
    YieldToParent;
...
public:
  ...
  void setErrorResult(NeXTCodeError *error) {
    ErrorResult = error;
  }
...
using AsyncVoidClosureEntryPoint =
  NEXTCODE_CC(nextcodeasync)
  void (NEXTCODE_ASYNC_CONTEXT AsyncContext *, NEXTCODE_CONTEXT void *);

/// This matches the ABI of a closure `<T>() async throws -> T`
using AsyncGenericClosureEntryPoint =
    NEXTCODE_CC(nextcodeasync)
    void(OpaqueValue *,
         NEXTCODE_ASYNC_CONTEXT AsyncContext *, NEXTCODE_CONTEXT void *);

/// This matches the ABI of the resume function of a closure
///  `() async throws -> ()`.
using AsyncVoidClosureResumeEntryPoint =
  NEXTCODE_CC(nextcodeasync)
  void(NEXTCODE_ASYNC_CONTEXT AsyncContext *, NEXTCODE_CONTEXT NeXTCodeError *);
...
public:
  ...
  AsyncVoidClosureEntryPoint *__ptrauth_nextcode_task_resume_function
      asyncEntryPoint;
  void *closureContext;
  NeXTCodeError *errorResult;
};
...
/// of NeXTCode's async convention and the ResumeTask interface.
class FutureAsyncContextPrefix {
public:
  OpaqueValue *indirectResult;
  // Async closure entry point adhering to compiler calling conv (e.g directly
  // passing the closure context instead of via the async context)
  AsyncGenericClosureEntryPoint *__ptrauth_nextcode_task_resume_function
      asyncEntryPoint;
  void *closureContext;
  NeXTCodeError *errorResult;
};
