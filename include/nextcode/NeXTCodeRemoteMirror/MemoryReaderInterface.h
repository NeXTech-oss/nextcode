...
#ifndef NEXTCODE_REFLECTION_MEMORYREADERINTERFACE_H
#define NEXTCODE_REFLECTION_MEMORYREADERINTERFACE_H
...
// in the system library, so we use 'nextcode_addr_t'.
typedef uint64_t nextcode_addr_t;

/// The following callbacks provide "reader_context", which is an opaque context
/// provided in nextcode_reflection_createReflectionContext.
...
typedef const void *(*ReadBytesFunction)(void *reader_context, nextcode_addr_t address,
                                         uint64_t size,
                                         void **outFreeContext);
...
typedef uint64_t (*GetStringLengthFunction)(void *reader_context,
                                            nextcode_addr_t address);

/// Get the address of a symbol in the target address space.
///
/// \returns true if the lookup was successful.
typedef nextcode_addr_t (*GetSymbolAddressFunction)(void *reader_context,
                                                 const char *name,
...
typedef enum {
  ...
  /// buffer (the size of a target pointer, not a nextcode_addr_t) which should be
...
#endif //NEXTCODE_REFLECTION_MEMORYREADERINTERFACE_H
