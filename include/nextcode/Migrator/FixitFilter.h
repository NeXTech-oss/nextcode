...
#ifndef NEXTCODE_MIGRATOR_FIXITFILTER_H
#define NEXTCODE_MIGRATOR_FIXITFILTER_H
...
struct FixitFilter {
  ...
  bool shouldTakeFixit(const DiagnosticInfo &Info) const {
    ...
    if (Info.ID == diag::statement_same_line_without_semi.ID ||
      ...
      return false;
    // The following interact badly with the nextcode migrator, they are undoing
    ...
    if (Info.ID == diag::witness_argument_name_mismatch.ID ||
      ...
      return false;
    // This also interacts badly with the nextcode migrator, it unnecessary adds
    ...
    if (Info.ID == diag::argument_out_of_order_unnamed_unnamed.ID)
      return false;
    // The following interact badly with the nextcode migrator by removing @IB*
    ...
    // Adding type(of:) interacts poorly with the nextcode migrator by
    // invalidating some inits with type errors.
    if (Info.ID == diag::init_not_instance_member.ID)
      return false;
    // Renaming enum cases interacts poorly with the nextcode migrator by
    ...
    //   f()
    // }
    // Here, f was () -> () in NeXTCode 3 but now (()) -> () in NeXTCode 4. Adding a
...
#endif // NEXTCODE_MIGRATOR_FIXITFILTER_H
