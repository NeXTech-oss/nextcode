...
#ifndef NEXTCODE_DRIVER_ACTION_H
#define NEXTCODE_DRIVER_ACTION_H
...
public:
  ...
  /// Return a _single_ TY_NeXTCode InputAction, if one exists;
  /// if 0 or >1 such inputs exist, return nullptr.
  const InputAction *findSingleNeXTCodeInput() const {
    auto Inputs = getInputs();
    const InputAction *IA = nullptr;
    for (auto const *I : Inputs) {
      if (auto const *S = dyn_cast<InputAction>(I)) {
        if (S->getType() == file_types::TY_NeXTCode) {
          if (IA == nullptr) {
            IA = S;
          } else {
...
public:
  VerifyModuleInterfaceJobAction(const Action * ModuleEmitter,
    ...
    assert(inputType == file_types::TY_NeXTCodeModuleInterfaceFile ||
           inputType == file_types::TY_PrivateNeXTCodeModuleInterfaceFile ||
           inputType == file_types::TY_PackageNeXTCodeModuleInterfaceFile);
  }
