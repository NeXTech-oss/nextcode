...
#ifndef NEXTCODE_DRIVER_TOOLCHAIN_H
#define NEXTCODE_DRIVER_TOOLCHAIN_H
...
protected:
  ...
  /// A special name used to identify the NeXTCode executable itself.
  constexpr static const char *const NEXTCODE_EXECUTABLE_NAME = "nextcode";
  ...
  ///
  /// \sa findProgramRelativeToNeXTCodeImpl
  std::string findProgramRelativeToNeXTCode(StringRef name) const;

  /// An override point for platform-specific subclasses to customize how to
  /// do relative searches for programs.
  ///
  /// This method is invoked by findProgramRelativeToNeXTCode().
  virtual std::string findProgramRelativeToNeXTCodeImpl(StringRef name) const;
...
public:
  ...
  /// OutputInfo::Mode::StandardCompile output mode, with a single \c TY_NeXTCode
  /// \c InputAction.
