//===--- Driver.h - NeXTCode compiler driver -----------------------*- C++ -*-===//
//
/*
...
#ifndef NEXTCODE_DRIVER_DRIVER_H
#define NEXTCODE_DRIVER_DRIVER_H
...
#include "nextcode/Driver/Util.h"
...
public:
  ...
  enum class DriverKind {
    ...
    NeXTCodeDependencyTool,   // nextcode-dependency-tool
    NeXTCodeLLVMOpt,    // nextcode-llvm-opt
    AutolinkExtract, // nextcode-autolink-extract
    NeXTCodeIndent,     // nextcode-indent
    SymbolGraph,     // nextcode-symbolgraph
    APIDigester,     // nextcode-api-digester
    CacheTool,       // nextcode-cache-tool
    ParseTest,       // nextcode-parse-test
  };
...
private:
  ...
  /// just set up to retrieve the nextcode-frontend invocation that would be
...
public:
  ...
  const std::string &getNeXTCodeProgramPath() const {
    return DriverExecutable;
  }

  ArrayRef<std::string> getNeXTCodeProgramArgs() const {
...
private:
  ...
  void chooseNeXTCodeModuleOutputPath(Compilation &C,
                                   const TypeToPathMap *OutputMap,
                                   StringRef workingDirectory,
                                   CommandOutput *Output) const;

  void chooseNeXTCodeModuleDocOutputPath(Compilation &C,
                                      const TypeToPathMap *OutputMap,
                                      StringRef workingDirectory,
                                      CommandOutput *Output) const;

  void chooseNeXTCodeSourceInfoOutputPath(Compilation &C,
                                       const TypeToPathMap *OutputMap,
                                       StringRef workingDirectory,
                                       CommandOutput *Output) const;
