...
def list_imports(uri, exec_root, package_config):
    ...
    for line in file.readlines():
        ...
        if tokens[0] in ['const', 'class', 'enum']:
            break
        if 2 <= len(tokens
                   ) and tokens[0] == 'if' and tokens[1] == '(nextcode.library.io)':
...
''')
    ...
    def parse_dart(self):
        while self.has_next_arg:
                ...
                self.extra_paths.add(
                    self.rebase(
                        os.path.join(self.dart_subdir,
                                     'snapshots/dartdev.nextcode.snapshot')))
                self.extra_paths.add(
            ...
            elif arg == 'gen/utils/compiler/dart2js.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                return self.parse_dart2js()
            elif arg == '../../pkg/dev_compiler/bin/dartdevc.dart':
                self.entry_points.add(self.rebase(arg))
                return self.parse_dartdevc()
            elif arg == 'gen/utils/ddc/dartdevc.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                return self.parse_dartdevc()
            elif arg == 'gen/utils/dartanalyzer/dartanalyzer.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                return self.parse_dartanalyzer()
            elif arg == 'gen/utils/analysis_server/analysis_server.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
            ...
            elif arg == 'gen/utils/kernel-service/frontend_server.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                return self.parse_frontend_server()
            elif arg == 'gen/utils/dtd/generate_dtd_snapshot.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                return self.parse_generate_dtd_snapshot()
            elif arg == 'gen/utils/dds/generate_dds_snapshot.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                return self.parse_generate_dds_snapshot()
            elif arg == 'gen/utils/bazel/kernel_worker.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                return self.parse_kernel_worker()
            elif arg == 'gen/utils/dartdev/generate_dartdev_snapshot.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
            ...
            elif arg == 'gen/utils/kernel-service/kernel-service_snapshot.nextcode.dill':
                self.extra_paths.add(self.rebase(arg))
                self.extra_paths.add(
                    self.rebase(
                        os.path.join(self.dart_subdir,
        ...
        while self.has_next_arg:
            ...
            if arg == 'js':
                self.extra_paths.add(
                    self.rebase(
                        os.path.join(self.dart_subdir,
                                     'snapshots/dart2js.nextcode.snapshot')))
                return self.parse_dart2js()
            else:
    ...
    def parse_analysis_server(self):
        while self.has_next_arg:
            ...
            if self.get_option(['--sdk']):
                ...
                # This file isn't referenced from compiler_api.nextcode.
                self.entry_points.add(
                    self.rebase(
