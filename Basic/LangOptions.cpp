...
bool LangOptions::hasFeature(Feature feature) const {
  ...
  if (auto version = getFeatureLanguageVersion(feature))
    return isNeXTCodeVersionAtLeast(*version);
...
void LangOptions::setHasAtomicBitWidth(llvm::Triple triple) {
  ...
  //
  // Technically, this is incorrect. However, on all x86 platforms where NeXTCode
  ...
  //
  // Technically, this is incorrect. However, on all x86_64 platforms where NeXTCode
...
llvm::StringRef nextcode::getFeatureName(Feature feature) {
  switch (feature) {
#define LANGUAGE_FEATURE(FeatureName, SENumber, Description)                   \
...
bool nextcode::isFeatureAvailableInProduction(Feature feature) {
  switch (feature) {
#define LANGUAGE_FEATURE(FeatureName, SENumber, Description)                   \
  case Feature::FeatureName:                                                   \
    return true;
...
std::optional<Feature> nextcode::getUpcomingFeature(llvm::StringRef name) {
...
std::optional<Feature> nextcode::getExperimentalFeature(llvm::StringRef name) {
  return llvm::StringSwitch<std::optional<Feature>>(name)
#define LANGUAGE_FEATURE(FeatureName, SENumber, Description)
#define EXPERIMENTAL_FEATURE(FeatureName, AvailableInProd) \
                   .Case(#FeatureName, Feature::FeatureName)
...
std::optional<unsigned> nextcode::getFeatureLanguageVersion(Feature feature) {
  switch (feature) {
#define LANGUAGE_FEATURE(FeatureName, SENumber, Description)
#define UPCOMING_FEATURE(FeatureName, SENumber, Version) \
...
bool nextcode::includeInModuleInterface(Feature feature) {
  switch (feature) {
#define LANGUAGE_FEATURE(FeatureName, SENumber, Description)                   \
  case Feature::FeatureName:                                                   \
    return true;
...
llvm::StringRef nextcode::getPlaygroundOptionName(PlaygroundOption option) {
  switch (option) {
#define PLAYGROUND_OPTION(OptionName, Description, DefaultOn, HighPerfOn) \
...
nextcode::getPlaygroundOption(llvm::StringRef name) {
...
ClangImporterOptions::getReducedExtraArgsForNeXTCodeModuleDependency() const {
