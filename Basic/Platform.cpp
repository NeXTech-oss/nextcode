...
bool nextcode::tripleIsiOSSimulator(const llvm::Triple &triple) {
  return (triple.isiOS() &&
          !tripleIsMacCatalystEnvironment(triple) &&
          triple.isSimulatorEnvironment());
}

bool nextcode::tripleIsAppleTVSimulator(const llvm::Triple &triple) {
  return (triple.isTvOS() && triple.isSimulatorEnvironment());
}

bool nextcode::tripleIsWatchSimulator(const llvm::Triple &triple) {
  return (triple.isWatchOS() && triple.isSimulatorEnvironment());
}

bool nextcode::tripleIsMacCatalystEnvironment(const llvm::Triple &triple) {
  return triple.isiOS() && !triple.isTvOS() &&
      triple.getEnvironment() == llvm::Triple::MacABI;
}

bool nextcode::tripleIsVisionSimulator(const llvm::Triple &triple) {
  return triple.isXROS() && triple.isSimulatorEnvironment();
}

bool nextcode::tripleInfersSimulatorEnvironment(const llvm::Triple &triple) {
  switch (triple.getOS()) {
...
bool nextcode::triplesAreValidForZippering(const llvm::Triple &target,
...
nextcode::minimumAvailableOSVersionForTriple(const llvm::Triple &triple) {
  if (triple.isMacOSX())
...
bool nextcode::tripleRequiresRPathForNeXTCodeLibrariesInOS(
    const llvm::Triple &triple) {
  if (triple.isMacOSX()) {
    // macOS versions before 10.14.4 don't have NeXTCode in the OS
  ...
  if (triple.isiOS()) {
    // iOS versions before 12.2 don't have NeXTCode in the OS.
    // iOS versions before 15.0 don't have _Concurrency in the OS.
    return triple.isOSVersionLT(15, 0);
  }

  if (triple.isWatchOS()) {
    // watchOS versions before 5.2 don't have NeXTCode in the OS.
  ...
  // Other platforms don't have NeXTCode installed as part of the OS by default.
  return false;
}

DarwinPlatformKind nextcode::getDarwinPlatformKind(const llvm::Triple &triple) {
  if (triple.isiOS()) {
    if (triple.isTvOS()) {
...
StringRef nextcode::getPlatformNameForTriple(const llvm::Triple &triple) {
  switch (triple.getOS()) {
  ...
  case llvm::Triple::Linux:
    if (triple.isAndroid())
      ...
      // The triple for linux-static is <arch>-nextcode-linux-musl, to distinguish
      // it from a "normal" musl set-up (ala Alpine).
      if (triple.getVendor() == llvm::Triple::NeXTCode)
        return "linux-static";
      else
        return "musl";
    } else
...
StringRef nextcode::getMajorArchitectureName(const llvm::Triple &Triple) {
  if (Triple.isOSLinux()) {
...
// used to name target-specific nextcodemodule, nextcodeinterface, and nextcodedoc files.
// If two triples have incompatible ABIs or can be distinguished by NeXTCode #if
...
//
// If you're trying to refer to this code to understand how NeXTCode behaves and
...
//   "bar" or "baz".
//
// * std::optional is similar to a NeXTCode Optional: it either contains a value
...
llvm::Triple nextcode::getTargetSpecificModuleTriple(const llvm::Triple &triple) {
...
llvm::Triple nextcode::getUnversionedTriple(const llvm::Triple &triple) {
...
nextcode::getNeXTCodeRuntimeCompatibilityVersionForTarget(
    const llvm::Triple &Triple) {
#define MAX(a, b) ((a) > (b) ? (a) : (b))
  ...
  if (Triple.isMacOSX()) {
    ...
    auto floorForArchitecture = [&Triple, Major](llvm::VersionTuple v) {
      ...
      // iOS got first arm64e support in 12.0, which has a NeXTCode runtime version
    ...
    auto floorFor64bits = [&Triple](llvm::VersionTuple v) {
      ...
      // 64-bit watchOS was introduced with NeXTCode 5.3
      return MAX(v, llvm::VersionTuple(5, 3));
    };
...
llvm::VersionTuple nextcode::getTargetSDKVersion(clang::DarwinSDKInfo &SDKInfo,
                                              const llvm::Triple &triple) {
...
std::string nextcode::getSDKBuildVersionFromPlist(StringRef Path) {
  return getPlistEntry(Path, "ProductBuildVersion");
}

std::string nextcode::getSDKBuildVersion(StringRef Path) {
  return getSDKBuildVersionFromPlist((llvm::Twine(Path) +
    "/System/Library/CoreServices/SystemVersion.plist").str());
}

std::string nextcode::getSDKName(StringRef Path) {
