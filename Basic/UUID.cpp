...
nextcode::UUID::UUID(FromRandom_t) {
#if defined(_WIN32)
  ::UUID uuid;
  ::CoCreateGuid(&uuid);
...
std::optional<nextcode::UUID> nextcode::UUID::fromString(const char *s) {
#if defined(_WIN32)
  ...
  nextcode::UUID result = UUID();
  memcpy(result.Value, &uuid, Size);
  return result;
#else
  nextcode::UUID result;
...
void nextcode::UUID::toString(llvm::SmallVectorImpl<char> &out) const {
...
int nextcode::UUID::compare(UUID y) const {
#if defined(_WIN32)
  RPC_STATUS s;
  ::UUID uuid1;
...
llvm::raw_ostream &nextcode::operator<<(llvm::raw_ostream &os, UUID uuid) {
